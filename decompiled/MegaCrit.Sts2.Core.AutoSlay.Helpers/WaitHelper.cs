...
public static class WaitHelper
{
    ...
    )
    {
        ...
        {
            string message =
                timeoutMessage ?? $"Condition not met after {actualTimeout.TotalSeconds}s";
        ...
        where T : Node
    {
        ...
        {
            await Until(
                () => root.HasNode(nodePath),
                ct,
                timeout,
                $"Node {nodePath} of type {typeof(T).Name} not found"
            );
        }
    ...
    private static void DumpSceneTreeContext(Node root, string nodePath, string reason)
    {
        ...
        StringBuilder.AppendInterpolatedStringHandler handler =
            new StringBuilder.AppendInterpolatedStringHandler(35, 1, stringBuilder2);
    ...
    )
    {
        foreach (Node child in node.GetChildren())
        {
            ...
            StringBuilder stringBuilder2 = stringBuilder;
            StringBuilder.AppendInterpolatedStringHandler handler =
                new StringBuilder.AppendInterpolatedStringHandler(0, 2, stringBuilder);
    ...
    )
    {
        ...
        {
            string message =
                timeoutMessage ?? $"Task not completed after {actualTimeout.TotalSeconds}s";
            throw new AutoSlayTimeoutException(message);
        }
    }

...
public class Watchdog
{
    ...
    public void Check()
    {
        ...
        if (
            timeSpan > AutoSlayConfig.watchdogLogInterval
            && DateTime.UtcNow - _lastLogTime > AutoSlayConfig.watchdogLogInterval
        )
        {
            _lastLogTime = DateTime.UtcNow;
            AutoSlayLog.Info(
                $"[Watchdog] No progress for {timeSpan.TotalSeconds:F1}s (last: {_lastActivity})"
            );
        }
        if (timeSpan > AutoSlayConfig.watchdogTimeout)
        {
            ...
            AutoSlayLog.Error(
                $"[Watchdog] Stuck detected! No progress for {timeSpan.TotalSeconds:F1}s\nLast activity: {_lastActivity}\n{value}"
            );
            throw new AutoSlayTimeoutException(
                $"Watchdog timeout: No progress for {timeSpan.TotalSeconds:F1}s. Last activity: {_lastActivity}"
            );
        }
    }
    ...
    public static string DumpState()
    {
        ...
        try
        {
            ...
            if (runState != null)
            {
                ...
                StringBuilder.AppendInterpolatedStringHandler handler =
                    new StringBuilder.AppendInterpolatedStringHandler(24, 3, stringBuilder2);
            ...
            if (NOverlayStack.Instance != null)
            {
                ...
                StringBuilder.AppendInterpolatedStringHandler handler =
                    new StringBuilder.AppendInterpolatedStringHandler(23, 1, stringBuilder2);
                    ...
                    foreach (Node child in nodeOrNull.GetChildren())
                    {
                        string value = (
                            (child is Control control)
                                ? (control.Visible ? "visible" : "hidden")
                                : ""
                        );
                        StringBuilder stringBuilder2 = stringBuilder;
        ...
        catch (Exception ex)
        {
            ...
            StringBuilder.AppendInterpolatedStringHandler handler =
                new StringBuilder.AppendInterpolatedStringHandler(21, 1, stringBuilder2);

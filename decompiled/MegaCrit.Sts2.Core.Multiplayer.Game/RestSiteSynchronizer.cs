...
public class RestSiteSynchronizer : IDisposable
{
    ...
    private readonly MegaCrit.Sts2.Core.Logging.Logger _logger =
        new MegaCrit.Sts2.Core.Logging.Logger("RestSiteSynchronizer", LogType.GameSync);
    ...
    )
    {
        ...
        _messageBuffer.RegisterMessageHandler<OptionIndexChosenMessage>(
            HandleRestSiteOptionChosenMessage
        );
        _messageBuffer.RegisterMessageHandler<RestSiteOptionHoveredMessage>(
            HandleRestSiteOptionHoveredMessage
        );
    }
    ...
    public void Dispose()
    {
        ...
        _messageBuffer.UnregisterMessageHandler<RestSiteOptionHoveredMessage>(
            HandleRestSiteOptionHoveredMessage
        );
    ...
    public void BeginRestSite()
    {
        ...
        foreach (Player player in _playerCollection.Players)
        {
            ...
            _restSites.Add(new PlayerRestSite { options = list });
            _logger.VeryDebug(
                $"Rest site began for player {player.NetId} with options: {string.Join(",", list)}"
            );
        }
    }
    ...
    {
        int playerSlotIndex = _playerCollection.GetPlayerSlotIndex(
            _playerCollection.GetPlayer(senderId)
        );
    ...
    private async Task<bool> ChooseOption(Player player, int optionIndex)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Player {player.NetId} attempted to choose rest site option index {optionIndex}, but there were only {restSite.options.Count} options available!"
            );
        }
        ...
        _logger.Debug(
            $"Rest site option index {optionIndex} chosen for player {player.NetId} with success {flag}. Option: {restSite.options[optionIndex].OptionId}"
        );
        ...
        player
            ...
            .RestSiteChoices.Add(option.OptionId);
    ...
    {
        int playerSlotIndex = _playerCollection.GetPlayerSlotIndex(
            LocalContext.GetMe(_playerCollection)
        );
    ...
    {
        int playerSlotIndex = _playerCollection.GetPlayerSlotIndex(
            _playerCollection.GetPlayer(playerId)
        );

...
public class RunLocationTargetedMessageBuffer
{
    ...
    {
        _logger.Debug(
            $"Run location changed to {location} (previously at: {CurrentLocation}), checking if we have enqueued messages"
        );
        ...
        for (int i = 0; i < _messagesWaitingOnLocationChange.Count; i++)
        {
            ...
            {
                _logger.Debug(
                    $"Handling enqueued message {blockedMessage.message} of type {blockedMessage.messageType} from {blockedMessage.senderId}"
                );
                CallHandlersOfType(
        ...
        {
            _logger.Error(
                $"After transitioning to {location}, there are still {_messagesWaitingOnLocationChange.Count} messages for other locations. This is likely indicates a bug. Messages:\n{string.Join("\n", _messagesWaitingOnLocationChange)}"
            );
        }
    }
    ...
    public void RegisterMessageHandler<T>(MessageHandlerDelegate<T> handler)
        where T : INetMessage, IRunLocationTargetedMessage
    {
        ...
        typeAndMessageHandlers.Value.handlers.Add(
            new MessageHandler { anonymizedHandler = AnonymousDelegate, originalHandler = handler }
        );
        ...
        where T : INetMessage, IRunLocationTargetedMessage
    {
        for (int i = 0; i < _messageHandlers.Count; i++)
        {
            ...
            {
                if (
                    typeAndMessageHandlers.handlers[j].originalHandler
                        is MessageHandlerDelegate<T> messageHandlerDelegate
                    && (Delegate?)messageHandlerDelegate == (Delegate?)handler
                )
                {
            ...
            {
                _gameService.UnregisterMessageHandler(
                    (MessageHandlerDelegate<T>)typeAndMessageHandlers.netServiceHandler
                );
    ...
    private void HandleMessage<T>(T message, ulong senderId)
        where T : INetMessage, IRunLocationTargetedMessage
    {
        _logger.VeryDebug(
            $"Handling map-targeted message {message} from {senderId} for location {message.Location}"
        );
        ...
        _logger.Debug(
            $"Message {message} from {senderId} is for location {message.Location}, enqueueing it because we are currently at location {CurrentLocation}"
        );

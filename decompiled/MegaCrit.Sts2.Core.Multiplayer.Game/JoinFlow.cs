...
public class JoinFlow
{
    ...
    private MegaCrit.Sts2.Core.Logging.Logger _logger = new MegaCrit.Sts2.Core.Logging.Logger(
        "JoinFlow",
        LogType.Network
    );
    ...
    public async Task<JoinResult> Begin(
    ...
    )
    {
        ...
        try
        {
            ...
            try
            {
                NetService.RegisterMessageHandler<InitialGameInfoMessage>(
                    HandleInitialGameInfoMessage
                );
                NetService.RegisterMessageHandler<ClientLobbyJoinResponseMessage>(
                    HandleJoinResponseMessage
                );
                NetService.RegisterMessageHandler<ClientLoadJoinResponseMessage>(
                    HandleLoadJoinResponseMessage
                );
                NetService.RegisterMessageHandler<ClientRejoinResponseMessage>(
                    HandleRejoinResponseMessage
                );
                ...
                _logger.Info(
                    "Initializer connection completed, awaiting initial game info message"
                );
                InitialGameInfoMessage initialMessage = await _connectCompletion.Task;
                if (initialMessage.connectionFailureReason.HasValue)
                {
                    _logger.Info(
                        $"Received initial join message with failure: {initialMessage.connectionFailureReason}"
                    );
                    throw new ClientConnectionFailedException(
                        "Got connection failure from host",
                        new NetErrorInfo(initialMessage.connectionFailureReason.Value)
                    );
                }
                RunSessionState state = initialMessage.sessionState;
                _logger.Info(
                    $"Got initial game info message. Version: {initialMessage.version} Hash: {initialMessage.idDatabaseHash} Type: {initialMessage.gameMode} State: {state}"
                );
                string text =
                ...
                {
                    throw new ClientConnectionFailedException(
                        "Version mismatch. Host: " + initialMessage.version + " Ours: " + text,
                        new NetErrorInfo(ConnectionFailureReason.VersionMismatch)
                    );
                }
                List<string> list =
                    ModManager.GetGameplayRelevantModNameList() ?? new List<string>();
                ...
                {
                    _logger.Warn(
                        $"Mod mismatch! Mods that host has that we don't: {string.Join(",", list3)}. Mods that we have that host doesn't: {string.Join(",", list4)}."
                    );
                    throw new ClientConnectionFailedException(
                        "Mod mismatch. Host mods: "
                            + string.Join(",", list2)
                            + " Local mods: "
                            + string.Join(",", list),
                ...
                if (initialMessage.idDatabaseHash != ModelIdSerializationCache.Hash)
                {
                    _logger.Warn(
                        "Our version "
                            + text
                            + " matches the host's, but our Model ID hash does not! Disconnecting"
                    );
                    throw new ClientConnectionFailedException(
                ...
                switch (state)
                {
                    ...
                    default:
                        ...
                        throw new InvalidOperationException(
                            $"Received invalid state {state} from connection!"
                        );
                }
            }
            catch (Exception ex)
        ...
        finally
        {
            ...
            NetService.UnregisterMessageHandler<InitialGameInfoMessage>(
                HandleInitialGameInfoMessage
            );
            NetService.UnregisterMessageHandler<ClientLobbyJoinResponseMessage>(
                HandleJoinResponseMessage
            );
            NetService.UnregisterMessageHandler<ClientLoadJoinResponseMessage>(
                HandleLoadJoinResponseMessage
            );
            NetService.UnregisterMessageHandler<ClientRejoinResponseMessage>(
                HandleRejoinResponseMessage
            );
            NetService.Disconnected -= OnDisconnected;
        }
    ...
    private async Task<ClientLobbyJoinResponseMessage> AttemptJoin(NetClientGameService gameService)
    {
        ...
        _logger.Info(
            $"Received {"ClientLobbyJoinResponseMessage"}: {clientLobbyJoinResponseMessage}"
        );
    ...
    )
    {
        ...
        _logger.Info(
            "Sending ClientLoadJoinRequestMessage and waiting for rejoin response message"
        );
        gameService.SendMessage(default(ClientLoadJoinRequestMessage));
        ClientLoadJoinResponseMessage clientLoadJoinResponseMessage =
            await _loadJoinCompletion.Task;
    ...
    private void HandleInitialGameInfoMessage(InitialGameInfoMessage message, ulong _)
    {
        ...
        {
            _logger.Warn(
                $"Received {"InitialGameInfoMessage"} when we weren't expecting it! Completion status: {_connectCompletion}"
            );
        }
        else
        {
    ...
    private void HandleRejoinResponseMessage(ClientRejoinResponseMessage message, ulong senderId)
    {
        ...
        {
            _logger.Warn(
                $"Received {"ClientRejoinResponseMessage"} when we weren't expecting it! Completion status: {_connectCompletion}"
            );
        }
        else
        {
    ...
    private void HandleLoadJoinResponseMessage(
    ...
    )
    {
        ...
        {
            _logger.Warn(
                $"Received {"ClientLoadJoinResponseMessage"} when we weren't expecting it! Completion status: {_connectCompletion}"
            );
        }
        else
        {
    ...
    private void HandleJoinResponseMessage(ClientLobbyJoinResponseMessage message, ulong senderId)
    {
        ...
        {
            _logger.Warn(
                $"Received {"ClientLobbyJoinResponseMessage"} when we weren't expecting it! Completion status: {_connectCompletion}"
            );
        }
        else
        {
    ...
    {
        _logger.Info(
            $"Disconnected during join flow, reason: {info.GetReason()}. Failing with an exception"
        );
        ClientConnectionFailedException exception = new ClientConnectionFailedException(
        ...
        TaskCompletionSource<ClientLoadJoinResponseMessage> loadJoinCompletion =
            _loadJoinCompletion;

...
public class OneOffSynchronizer : IDisposable
{
    ...
    )
    {
        ...
        messageBuffer.RegisterMessageHandler<TreasureChestOpenedMessage>(
            HandleTreasureChestOpenedMessage
        );
        messageBuffer.RegisterMessageHandler<CrystalSphereRewardsMessage>(
            HandleCrystalSphereRewardsMessage
        );
    }

    public void Dispose()
    {
        _messageBuffer.UnregisterMessageHandler<MerchantCardRemovalMessage>(
            HandleMerchantCardRemoval
        );
        _messageBuffer.UnregisterMessageHandler<TreasureChestOpenedMessage>(
            HandleTreasureChestOpenedMessage
        );
        _messageBuffer.UnregisterMessageHandler<CrystalSphereRewardsMessage>(
            HandleCrystalSphereRewardsMessage
        );
    }
    ...
    private void HandleMerchantCardRemoval(MerchantCardRemovalMessage message, ulong senderId)
    {
        ...
        if (player == LocalPlayer)
        {
            throw new InvalidOperationException(
                "MerchantCardRemovalMessage should not be sent to the player removing the card!"
            );
        }
        TaskHelper.RunSafely(DoMerchantCardRemoval(player, message.goldCost));
    }

    private async Task<bool> DoMerchantCardRemoval(
        Player player,
        int goldCost,
    ...
    private void HandleTreasureChestOpenedMessage(
        TreasureChestOpenedMessage message,
        ulong senderId
    )
    {
        Player player = _playerCollection.GetPlayer(senderId);
        ...
        {
            throw new InvalidOperationException(
                "TreasureChestOpenedMessage should not be sent to the player who opened the treasure chest!"
            );
        }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                $"Trying to sync crystal sphere rewards for non-local player {owner.NetId}!"
            );
        }
        List<SerializableCrystalSphereItem> rewards = revealed
            .Select((CrystalSphereItem r) => r.ToSerializable())
            .ToList();
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                "CrystalSphereRewardsMessage should not be sent to the player who completed the event!"
            );
        }
        ...
        {
            throw new InvalidOperationException(
                $"Received {"CrystalSphereRewardsMessage"} for player {player.NetId} while the player was in event {eventForPlayer.Id}!"
            );
        }
        List<CrystalSphereItem> revealed = message
            .rewards.Select(
                (SerializableCrystalSphereItem r) => CrystalSphereItem.FromSerializable(r, player)
            )
            .ToList();
    ...
    {
        List<Reward> rewards = revealed
            .Select((CrystalSphereItem r) => r.ToReward(owner, rng))
            .OfType<Reward>()
            .ToList();

...
public class ChecksumTracker : IDisposable
{
    ...
    private readonly List<QueuedRemoteChecksum> _queuedRemoteChecksums =
        new List<QueuedRemoteChecksum>();
    ...
    public ChecksumTracker(INetGameService netService, IRunState runState)
    {
        ...
        _netService.RegisterMessageHandler<StateDivergenceMessage>(
            OnReceivedStateDivergenceMessage
        );
    }
    ...
    public void Dispose()
    {
        ...
        _netService.UnregisterMessageHandler<StateDivergenceMessage>(
            OnReceivedStateDivergenceMessage
        );
    }
    ...
    public NetChecksumData GenerateChecksum(string context, GameAction? action)
    {
        ...
        {
            _netService.SendMessage(new ChecksumDataMessage { checksumData = netChecksumData });
        }
    ...
    private void OnReceivedChecksumDataMessage(ChecksumDataMessage message, ulong senderId)
    {
        ...
        if (num < 0)
        {
            _queuedRemoteChecksums.Add(
                new QueuedRemoteChecksum { data = remoteChecksumData, senderId = senderId }
            );
        }
        else
        {
    ...
    private void OnReceivedStateDivergenceMessage(StateDivergenceMessage message, ulong senderId)
    {
        ...
        {
            Log.Error(
                $"Received state divergence message for checksum ID {remoteChecksumData.id} that has fallen out of our list! (Our next ID: {NextId})"
            );
        }
        else
        {
        ...
        if (_netService.Type == NetGameType.Host)
        {
            (_netService as NetHostGameService)?.DisconnectClient(
                senderId,
                NetError.StateDivergence
            );
            NErrorPopup nErrorPopup = NErrorPopup.Create(
                new NetErrorInfo(NetError.StateDivergence, selfInitiated: true)
            );
            if (nErrorPopup != null)
            {
    ...
    private NetChecksumData ObtainAndTrackChecksum(string context, GameAction? action)
    {
        ...
        NetChecksumData netChecksumData = new NetChecksumData { id = NextId, checksum = checksum };
        NextId++;
        ...
        for (int i = 0; i < _queuedRemoteChecksums.Count; i++)
        {
            if (_queuedRemoteChecksums[i].data.id == trackedChecksum.data.id)
            {
                CompareChecksums(
                    trackedChecksum,
                    _queuedRemoteChecksums[i].data,
                    _queuedRemoteChecksums[i].senderId
                );
    ...
    )
    {
        if (localChecksum.data.id != remoteChecksum.id)
        {
            throw new InvalidOperationException(
                "Trying to compare two checksums with different IDs!"
            );
        }
        if (_netService.Type != NetGameType.Host)
        {
            throw new InvalidOperationException(
                "CompareChecksums should only be called on the host!"
            );
        }
        if (localChecksum.data.checksum != remoteChecksum.checksum)
        {
            ...
            {
                Log.Error(
                    $"State divergence detected! Checksum with ID {localChecksum.data.id} for client {remoteId} doesn't match host's!\nContext: {localChecksum.context}. Local: {localChecksum.data.checksum}. Remote: {remoteChecksum.checksum}."
                );
            }
    ...
    )
    {
        ...
        SentryService.CaptureException(
            new StateDivergenceException(message2),
            delegate(Scope scope)
            {
                ...
                scope.SetExtra(
                    "divergence.lobby_id",
                    _netService.GetRawLobbyIdentifier() ?? "unknown"
                );
    ...
    private void CheckAgainstReplayChecksum(NetChecksumData localData, string context)
    {
        ...
        int num = _replayChecksums.FindIndex(
            (ReplayChecksumData c) => c.checksumData.id == localData.id
        );
        if (num == -1)
        {
            Log.Error(
                $"Replay state diverged! Generated checksum for {context} with id {localData.id} that doesn't exist in the replay data"
            );
            return;
        }
    ...
    public uint GenerateChecksum(NetFullCombatState state)
    {
        ...
        ReadOnlySpan<byte> source = _packetWriter
            .Buffer.AsSpan()
            .Slice(0, _packetWriter.BytePosition);

...
public class FlavorSynchronizer : IDisposable
{
    ...
    private readonly Dictionary<Player, NSpeechBubbleVfx?> _endTurnPingDialogues =
        new Dictionary<Player, NSpeechBubbleVfx>();
    ...
    public void SendMapPing(MapCoord coord)
    {
        if (Time.GetTicksMsec() >= _nextAllowedPingTime)
        {
            _gameService.SendMessage(new MapPingMessage { coord = coord });
    ...
    private void CreateEndTurnPingDialogueIfNecessary(Player player)
    {
        if (NRun.Instance != null)
        {
            ...
            LocString locString = new LocString(
                "characters",
                player.Character.Id.Entry + ".banter." + text + ".endTurnPing"
            );
            value = NSpeechBubbleVfx.Create(

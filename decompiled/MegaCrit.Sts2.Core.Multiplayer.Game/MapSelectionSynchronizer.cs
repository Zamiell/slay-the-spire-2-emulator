...
public class MapSelectionSynchronizer
{
    ...
    public void PlayerVotedForMapCoord(Player player, MapLocation source, MapVote? destination)
    {
        ...
        {
            _logger.Warn(
                $"Received map vote from player {player.NetId} for source {source}, but we're currently accepting votes for {_acceptingVotesFromSource}"
            );
            return;
        }
        ...
        {
            _logger.Warn(
                $"Received map vote from player {player.NetId} for destination {destination}, but the map generation count is lower than our current: {MapGenerationCount}"
            );
            return;
        }
        ...
        {
            _logger.Debug(
                $"Received vote to move to {destination} from player {player.NetId} (slot {playerSlotIndex})"
            );
        }
        else
        {
            _logger.Debug(
                $"Received cancellation of vote from player {player.NetId} (slot {playerSlotIndex})"
            );
        }
        if (
            _votes.All(
                (MapVote? p) => p.HasValue && p.Value.mapGenerationCount == MapGenerationCount
            )
    ...
    private void MoveToMapCoord()
    {
        ...
        MoveToMapCoordAction action = new MoveToMapCoordAction(
    ...
    public void BeforeMapGenerated()
    {
        ...
        for (int i = 0; i < _votes.Count; i++)
        {
            ...
            if (mapVote.HasValue && mapVote.Value.mapGenerationCount < MapGenerationCount)
            {
                ...
                _logger.Debug(
                    $"Cancelling map vote for player {player.NetId} because the map has re-generated and their vote is old"
                );
                _votes[i] = null;

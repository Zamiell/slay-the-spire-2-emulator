...
public class TreasureRoomRelicSynchronizer
{
    ...
    public void BeginRelicPicking()
    {
        ...
        foreach (Player player in _playerCollection.Players)
        {
            ...
            if (Hook.ShouldGenerateTreasure(runState, player))
            {
                ...
                RelicModel item =
                    TryGetRelicForTutorial(runState.UnlockState)
                    ?? _sharedGrabBag.PullFromFront(rarity, runState)
        ...
        if (_currentRelics.Count > 0)
        {
            if (
                ...
                && _playerCollection.Players.Count > 1
            )
            {
                foreach (Player player2 in _playerCollection.Players)
                {
                    ...
                    {
                        PlayerVote playerVote = _votes[
                            _playerCollection.GetPlayerSlotIndex(player2)
                        ];
    ...
    public void PickRelicLocally(int? index)
    {
        ...
        {
            _logger.Debug(
                $"Relic index {index} ({_currentRelics?[index.Value]}) is being picked by local player {LocalPlayer.NetId}"
            );
        }
        else
        {
        ...
        _predictedVote = new PlayerVote { index = index, voteReceived = true };
    ...
    public void OnPicked(Player player, int? index)
    {
        ...
        {
            _logger.Debug(
                $"Player {player} picked relic at index {index}: {_currentRelics?[index.Value]}"
            );
        }
        else
        {
        ...
        if (index >= _currentRelics.Count)
        {
            throw new IndexOutOfRangeException(
                $"Attempted to pick relic at index {index}, but there are only {_currentRelics.Count} to choose from!"
            );
        }
        ...
        if (_predictedVote != null)
        {
            ...
            _predictedVote = null;
            if (
                _votes[_playerCollection.GetPlayerSlotIndex(LocalPlayer)].index
                != predictedVote.index
            )
            {
                this.VotesChanged?.Invoke();
    ...
    private void AwardRelics()
    {
        ...
        for (int j = 0; j < _votes.Count; j++)
        {
            ...
            {
                List<Player> valueOrDefault = dictionary.GetValueOrDefault(
                    playerVote.index.Value,
                    new List<Player>()
                );
            ...
            else if (item2.Value.Count > 1)
            {
                ...
                results.Add(
                    RelicPickingResult.GenerateRelicFight(
                        item2.Value,
                        relicModel,
                        () => _rng.NextItem(possibleMoves)
                    )
                );
            }
        }
        List<Player> list2 = _playerCollection
            .Players.Where(
                delegate(Player p)
                {
    ...
    {
        if (
            unlockState.NumberOfRuns == 0
            && LocalContext
                .GetMe(_playerCollection)
                .RunState.MapPointHistory.SelectMany((IReadOnlyList<MapPointHistoryEntry> l) => l)
                .Count((MapPointHistoryEntry p) => p.HasRoomOfType(RoomType.Treasure)) == 1
        )
        {

...
public class EventSynchronizer : IDisposable
{
    ...
    public bool IsShared =>
        (
            _canonicalEvent ?? throw new InvalidOperationException("Event is not in progress!")
        ).IsShared;
    ...
    )
    {
        ...
        _messageBuffer.RegisterMessageHandler<OptionIndexChosenMessage>(
            HandleEventOptionChosenMessage
        );
        _messageBuffer.RegisterMessageHandler<VotedForSharedEventOptionMessage>(
            HandleVotedForSharedEventOptionMessage
        );
        _messageBuffer.RegisterMessageHandler<SharedEventOptionChosenMessage>(
            HandleSharedEventOptionChosenMessage
        );
    }

    public void Dispose()
    {
        _messageBuffer.UnregisterMessageHandler<OptionIndexChosenMessage>(
            HandleEventOptionChosenMessage
        );
        _messageBuffer.UnregisterMessageHandler<VotedForSharedEventOptionMessage>(
            HandleVotedForSharedEventOptionMessage
        );
        _messageBuffer.UnregisterMessageHandler<SharedEventOptionChosenMessage>(
            HandleSharedEventOptionChosenMessage
        );
    }
    ...
    )
    {
        ...
        foreach (EventModel @event in _events)
        {
            ...
            {
                _logger.Warn(
                    $"Beginning new event {canonicalEvent}, but event {@event} for player {@event.Owner.NetId} is not yet finished!"
                );
                @event.EnsureCleanup();
            }
        }
        ...
        foreach (Player player in _playerCollection.Players)
        {
            ...
            _logger.VeryDebug(
                $"Event {eventModel.Id} began for player {player.NetId} with options: {string.Join(",", eventModel.CurrentOptions)}"
            );
        }
    }
    ...
    {
        _logger.Debug(
            $"Received {"VotedForSharedEventOptionMessage"} from player {senderId} for option {message.optionIndex} on page {message.pageIndex}"
        );
        if (!IsShared)
        {
            throw new InvalidOperationException(
                "Received VotedForSharedEventOptionMessage during a non-shared event!"
            );
        }
        ...
        {
            throw new InvalidOperationException(
                $"Received {"VotedForSharedEventOptionMessage"} for player {senderId} that doesn't exist!"
            );
        }
    ...
    private void PlayerVotedForSharedOptionIndex(Player player, uint optionIndex, uint pageIndex)
    {
        ...
        {
            _logger.Warn(
                $"Received message from player {player.NetId} voting for option {optionIndex} on page {pageIndex}, but we are on greater page {_pageIndex}. Ignoring vote"
            );
        }
        else if (pageIndex > _pageIndex)
        {
            _logger.Error(
                $"Received message from player {player.NetId} voting for option {optionIndex} on page {pageIndex}, but we are on lesser page {_pageIndex}. This is a bug!"
            );
        }
        else
        {
    ...
    private void ChooseSharedEventOption()
    {
        ...
        {
            throw new InvalidOperationException(
                "Only host should be choosing shared event option!"
            );
        }
        _logger.Debug(
            $"All votes received on host for shared event {_canonicalEvent}, choosing option"
        );
    ...
    {
        _logger.Debug(
            $"Received {"SharedEventOptionChosenMessage"} for option {message.optionIndex} on page {message.pageIndex}"
        );
        if (_netService.Type != NetGameType.Client)
        {
            throw new InvalidOperationException(
                $"Received {"SharedEventOptionChosenMessage"} on non-client! {_netService.Type}"
            );
        }
        if (_pageIndex != message.pageIndex)
        {
            throw new InvalidOperationException(
                $"Received {"SharedEventOptionChosenMessage"} for page {message.pageIndex} while we were on page {_pageIndex}!"
            );
        }
    ...
    private void HandleEventOptionChosenMessage(OptionIndexChosenMessage message, ulong senderId)
    {
        ...
        {
            _logger.Debug(
                $"Received {"OptionIndexChosenMessage"} from player {senderId} for event option index {message.optionIndex}"
            );
            if (IsShared)
            {
                throw new InvalidOperationException(
                    "Received OptionIndexChosenMessage during a shared event!"
                );
            }
            Player player = _playerCollection.GetPlayer(senderId);
            if (player == null)
            {
                throw new InvalidOperationException(
                    $"Received EventOptionChosenMessage for player {senderId} that doesn't exist!"
                );
            }
    ...
    private void ChooseOptionForSharedEvent(uint optionIndex)
    {
        ...
        {
            throw new InvalidOperationException(
                "ChooseOptionForSharedEvent called during non-shared event!"
            );
        }
    ...
    private void ChooseOptionForEvent(Player player, int optionIndex)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Option chosen for player {player} on {eventForPlayer}, but it is already finished!"
            );
        }
        ...
        _logger.Debug(
            $"Option index {optionIndex} chosen for player {player.NetId} in event {eventForPlayer.Id}. Choice key: {eventForPlayer.CurrentOptions[optionIndex].TextKey}"
        );

...
public class RewardSynchronizer : IDisposable
{
    ...
    private void SyncLocalCardEvent(CardModel card, bool skipped)
    {
        ...
        )
        {
            throw new InvalidOperationException(
                $"Tried to sync card event {card} during combat! This is not allowed"
            );
        }
    ...
    private void HandleRewardObtainedMessage(RewardObtainedMessage message, ulong senderId)
    {
        ...
        MapPointHistoryEntry historyEntryFor = player.RunState.GetHistoryEntryFor(
            message.location.mapLocation
        );
        ...
        switch (message.rewardType)
        {
            case RewardType.Card:
            {
                ...
                playerMapPointHistoryEntry?.CardChoices.Add(
                    new CardChoiceHistoryEntry(message.cardModel, !message.wasSkipped)
                );
                break;
            }
            case RewardType.Gold:
                ...
                Log.Debug(
                    $"Player {player.NetId} obtained {message.goldAmount} gold from gold reward"
                );
                break;
            case RewardType.Potion:
                if (!message.wasSkipped)
                {
                    ...
                    Log.Debug(
                        $"Player {player.NetId} obtained {message.potionModel?.Id} from potion reward"
                    );
                }
                else
                {
                    Log.Debug(
                        $"Player {player.NetId} skipped {message.potionModel?.Id} from potion reward"
                    );
                    playerMapPointHistoryEntry?.PotionChoices.Add(
                        new ModelChoiceHistoryEntry(message.potionModel.Id, !message.wasSkipped)
                    );
                }
                break;
            case RewardType.Relic:
                if (!message.wasSkipped)
                {
                    ...
                    Log.Debug(
                        $"Player {player.NetId} obtained {message.relicModel?.Id} from relic reward"
                    );
                }
                else
                {
                    Log.Debug(
                        $"Player {player.NetId} skipped {message.relicModel?.Id} from relic reward"
                    );
                    playerMapPointHistoryEntry?.RelicChoices.Add(
                        new ModelChoiceHistoryEntry(message.relicModel.Id, !message.wasSkipped)
                    );
                }
                break;
            default:
        ...
        foreach (BufferedMessage bufferedMessage in _bufferedMessages)
        {
            if (bufferedMessage.rewardMessage.HasValue)
            {
                HandleRewardObtainedMessage(
                    bufferedMessage.rewardMessage.Value,
                    bufferedMessage.senderId
                );
            }
            else if (bufferedMessage.goldLostMessage.HasValue)
            {
                HandleGoldLostMessage(
                    bufferedMessage.goldLostMessage.Value,
                    bufferedMessage.senderId
                );
            }
            else if (bufferedMessage.cardRemovedMessage != null)
            {
                HandleCardRemovedMessage(
                    bufferedMessage.cardRemovedMessage,
                    bufferedMessage.senderId
                );
            }
        }
    ...
    public async Task<bool> DoUnsyncedCardRemoval(Player player)
    {
        CardSelectorPrefs cardSelectorPrefs = new CardSelectorPrefs(
            new LocString("gameplay_ui", "COMBAT_REWARD_CARD_REMOVAL.selectionScreenPrompt"),
            1
        );

...
public class NActBanner : Control
{
    ...
    public static NActBanner? Create(ActModel act, int actIndex)
    {
        ...
        NActBanner nActBanner = PreloadManager
            .Cache.GetScene(_path)
            .Instantiate<NActBanner>(PackedScene.GenEditState.Disabled);
        nActBanner._act = act;
    ...
    private async Task AnimateVfx()
    {
        ...
        tween
            .TweenProperty(_actNumber, "position:y", 440f, 1.25)
            .SetDelay(0.5)
        ...
        tween.Chain();
        tween.TweenInterval(
            (SaveManager.Instance.PrefsSave.FastMode == FastModeType.Fast) ? 0.5 : 2.0
        );
        tween.Chain();
        tween

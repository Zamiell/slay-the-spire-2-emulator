...
public class NTransition : ColorRect
{
    ...
    private const string _fightTransitionPath =
        "res://materials/transitions/fight_transition_mat.tres";

    private const string _fadeTransitionPath =
    ...
    public async Task FadeOut(
        float time = 0.8f,
        string transitionPath = "res://materials/transitions/fade_transition_mat.tres",
    ...
    )
    {
        ...
        _tween
            .TweenProperty(_simpleTransition, "modulate:a", 1f, time)
        ...
        if (!(base.Material is ShaderMaterial shaderMaterial))
        {
            Log.Warn(
                "NTransition.Material is null or not a ShaderMaterial (actual: "
                    + (base.Material?.GetType().Name ?? "null")
                    + "). Skipping transition."
            );
        }
        else
        {
            ...
            {
                Log.Warn(
                    "NTransition.Material failed to load from cache (path: "
                        + transitionPath
                        + "). Skipping transition."
                );
                return;
    ...
    )
    {
        ...
        while (t < (double)time)
        {
            ...
            transitionMaterial.SetShaderParameter(
                _threshold,
                Ease.CubicIn((float)((double)time - t))
            );
            t += GetProcessDeltaTime();
            await this.AwaitProcessFrame();
    ...
    public async Task RoomFadeOut()
    {
        ...
        if (!TestMode.IsOn && SaveManager.Instance.PrefsSave.FastMode != FastModeType.Instant)
        {
            ...
            {
                _tween
                    .TweenProperty(_gradientTransition, "position:y", _targetGradientYPosition, 0.6)
                    .SetDelay(0.5);
            ...
            {
                _tween
                    .TweenProperty(_simpleTransition, "modulate:a", 1f, 0.3)
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Float,
                PropertyName._initialGradientYPosition,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Float,
                PropertyName._targetGradientYPosition,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._gradientTransition,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._simpleTransition,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._initialGradientYPosition,
            Variant.From(in _initialGradientYPosition)
        );
        info.AddProperty(
            PropertyName._targetGradientYPosition,
            Variant.From(in _targetGradientYPosition)
        );

...
public class NGame : Control
{
    ...
    {
        public static new readonly StringName _EnterTree = "_EnterTree";
        ...
        public static new readonly StringName _Notification = "_Notification";
    ...
    public event WindowChangeEventHandler WindowChange
    {
        add
        {
            backing_WindowChange = (WindowChangeEventHandler)
                Delegate.Combine(backing_WindowChange, value);
        }
        remove
        {
            backing_WindowChange = (WindowChangeEventHandler)
                Delegate.Remove(backing_WindowChange, value);
        }
    }
    ...
    public event PhobiaModeToggledEventHandler PhobiaModeToggled
    {
        add
        {
            backing_PhobiaModeToggled = (PhobiaModeToggledEventHandler)
                Delegate.Combine(backing_PhobiaModeToggled, value);
        }
        remove
        {
            backing_PhobiaModeToggled = (PhobiaModeToggledEventHandler)
                Delegate.Remove(backing_PhobiaModeToggled, value);
        }
    }
    ...
    public override void _EnterTree()
    {
        ...
        GetWindow()
            .Connect(
                Window.SignalName.FilesDropped,
                Callable.From<string[]>(FileDropHandler.OnFilesDropped)
            );
    ...
    private async Task GameStartupError()
    {
        ...
        else
        {
            ...
            await nGenericPopup.WaitForConfirmation(
                new LocString("main_menu_ui", "STARTUP_ERROR.description"),
                new LocString("main_menu_ui", "STARTUP_ERROR.title"),
                null,
                new LocString("main_menu_ui", "QUIT")
            );
            GetTree().Quit();
        }
    }
    ...
    private async Task GameStartup()
    {
        ...
        while (!cloudSavesTask.IsCompleted)
        {
            ...
            {
                Log.Error(
                    "Steam became uninitialized while the cloud sync was in-progress! This might result in unpredictable behavior"
                );
                break;
            }
        ...
        SentryService.AfterGameInit(
            PlatformUtil.GetPlatformBranch().ToName(),
            SaveManager.Instance.Progress.UniqueId,
            GetTree().Root
        );
        _screenShake.SetMultiplier(
            NScreenshakePaginator.GetShakeMultiplier(
        ...
        {
            NSceneBootstrapper child = SceneHelper.Instantiate<NSceneBootstrapper>(
                "debug/scene_bootstrapper"
            );
        ...
        {
            bool skipLogo =
                DebugSettings.DevSkip
                || SaveManager.Instance.SettingsSave.SkipIntroLogo
                || CommandLineHelper.HasArg("fastmp");
            await LaunchMainMenu(skipLogo);
            CheckShowSaveFileError(
                progressReadResult,
                prefsReadResult,
                OneTimeInitialization.SettingsReadResult
            );
            CheckShowLocalizationOverrideErrors();
        }
    ...
    public void ApplyDisplaySettings()
    {
        ...
        if (PlatformUtil.GetSupportedWindowMode().ShouldForceFullscreen())
        {
            ...
            {
                Log.Warn(
                    $"Settings has fullscreen set to false, but we're forcing fullscreen because the platform reports our supported window mode as {PlatformUtil.GetSupportedWindowMode()}"
                );
            }
            flag2 = true;
        }
        ...
        Log.Info(
            $"[Display] Min size: {DisplayServer.WindowGetMinSize()} Max size: {DisplayServer.WindowGetMaxSize()}"
        );
        ...
        switch (settingsSave.AspectRatioSetting)
        {
            ...
            default:
                throw new ArgumentOutOfRangeException(
                    $"Invalid Aspect Ratio: {settingsSave.AspectRatioSetting}"
                );
        }
        ...
        if (flag2 && num < 0)
        {
            ...
            Log.Info(
                $"[Display] Setting FULLSCREEN on Display: {settingsSave.TargetDisplay + 1} of {DisplayServer.GetScreenCount()}"
            );
            if (settingsSave.TargetDisplay >= DisplayServer.GetScreenCount())
            {
                Log.Warn(
                    $"[Display] FAILED: Display {settingsSave.TargetDisplay} is missing. Fallback to primary."
                );
        ...
        {
            Log.Info(
                $"[Display] Attempting WINDOWED mode on Display {settingsSave.TargetDisplay + 1} of {DisplayServer.GetScreenCount()} at position {settingsSave.WindowPosition}"
            );
            ...
            if (num >= 0)
            {
                ...
                Vector2I vector2I = new Vector2I(
                    int.Parse(System.Environment.GetCommandLineArgs()[num + 1]),
                    int.Parse(System.Environment.GetCommandLineArgs()[num + 2])
                );
                Vector2I vector2I2 = DisplayServer.ScreenGetPosition(
                    DisplayServer.WindowGetCurrentScreen()
                );
                Log.Info(
                    $"Applying window position from command line arg: {vector2I} ({string.Join(",", System.Environment.GetCommandLineArgs())} {vector2I2})"
                );
            ...
            else
            {
                ...
                {
                    Log.Info(
                        $"[Display] Going from fullscreen to windowed. Attempting to center window on screen {settingsSave.TargetDisplay}"
                    );
                ...
                {
                    Log.Warn(
                        "[Display] WARN: Game Window was offscreen. Resetting to top left corner."
                    );
                ...
                {
                    Log.Info(
                        $"[Display] FAILED: Display {settingsSave.TargetDisplay + 1} is missing. Fallback to primary."
                    );
                    ...
                    DisplayServer.WindowSetPosition(
                        DisplayServer.ScreenGetPosition(DisplayServer.GetPrimaryScreen())
                            + settingsSave.WindowPosition
                    );
                }
                else
                {
                    Vector2I vector2I5 = DisplayServer.ScreenGetPosition(
                        settingsSave.TargetDisplay
                    );
                    ...
                    Log.Info(
                        $"[Display] SUCCESS: {windowSize} Windowed mode in Display {settingsSave.TargetDisplay}: Position {vector2I5 + vector2I4} ({vector2I4})"
                    );
                    DisplayServer.WindowSetSize(windowSize);
                    DisplayServer.WindowSetPosition(vector2I5 + vector2I4);
                    Log.Info(
                        $"[Display] New size: {DisplayServer.WindowGetSize()} position: {DisplayServer.WindowGetPosition()}"
                    );
                }
            }
        }
        if (flag)
        {
            ...
            EmitSignal(
                SignalName.WindowChange,
                settingsSave.AspectRatioSetting == AspectRatioSetting.Auto
            );
        }
    }
    ...
    public void Quit()
    {
        ...
        if (
            !PlatformUtil.GetSupportedWindowMode().ShouldForceFullscreen()
            && !SaveManager.Instance.SettingsSave.Fullscreen
        )
        {
            SettingsSave settingsSave = SaveManager.Instance.SettingsSave;
            ...
            settingsSave.WindowPosition =
                ...
                - DisplayServer.ScreenGetPosition(SaveManager.Instance.SettingsSave.TargetDisplay);
            Log.Info(
                $"[Display] On exit, saving window size: {settingsSave.WindowSize} display: {settingsSave.TargetDisplay} position: {settingsSave.WindowPosition}"
            );
        }
    ...
    public void ReloadMainMenu()
    {
        if (MainMenu == null)
        {
            throw new InvalidOperationException(
                "Tried to reload main menu when not already on the main menu!"
            );
        }
    ...
    private async Task LoadMainMenu(bool openTimeline = false)
    {
        ...
        {
            Log.Info(
                "Saving in progress, waiting for it to be finished before loading the main menu"
            );
            try
            {
    ...
    )
    {
        ...
        RunState runState = RunState.CreateForNewRun(
            new global::_003C_003Ez__ReadOnlySingleElementList<Player>(
                Player.CreateForNewRun(character, unlockState, 1uL)
            ),
            acts.Select((ActModel a) => a.ToMutable()).ToList(),
            modifiers,
            gameMode,
            ascensionLevel,
            seed
                ...
                .Players.Select(
                    (LobbyPlayer p) =>
                        Player.CreateForNewRun(
                            p.character,
                            UnlockState.FromSerializable(p.unlockState),
                            p.id
                        )
                )
                .ToList(),
    ...
    public async Task LoadRun(RunState runState, SerializableRoom? preFinishedRoom)
    {
        ...
        await RunManager.Instance.LoadIntoLatestMapCoord(
            AbstractRoom.FromSerializable(preFinishedRoom, runState)
        );
    ...
    public override void _Input(InputEvent inputEvent)
    {
        ...
        else if (inputEvent.IsActionReleased(DebugHotkey.hideProceedButton))
        {
            ...
            Instance.AddChildSafely(
                NFullscreenTextVfx.Create(
                    IsDebugHidingProceedButton ? "Hide Proceed Button" : "Show Proceed Button"
                )
            );
        ...
        else if (inputEvent.IsActionReleased(DebugHotkey.hideHoverTips))
        {
            ...
            Instance.AddChildSafely(
                NFullscreenTextVfx.Create(
                    IsDebugHidingHoverTips ? "Hide HoverTips" : "Show HoverTips"
                )
            );
        }
        if (
            inputEvent is InputEventMouseButton { Pressed: not false }
        ...
        if (OS.GetName().Contains("Windows"))
        {
            ...
            OS.GetName().Contains("macOS")
            && inputEventKey.Pressed
            && inputEventKey.CtrlPressed
            && inputEventKey.MetaPressed
            && inputEventKey.Keycode == Key.F
        )
        {
            ToggleFullscreen();
        }
    ...
    )
    {
        ...
        if (!progressReadResult.Success && progressReadResult.Status != ReadSaveStatus.FileNotFound)
        {
            ...
            settingsReadResult != null
            && !settingsReadResult.Success
            && settingsReadResult.Status != ReadSaveStatus.FileNotFound
        )
        {
    ...
    private void CheckShowLocalizationOverrideErrors()
    {
        ...
        {
            List<IGrouping<string, LocValidationError>> list = (
                from e in LocManager.Instance.ValidationErrors
                group e by e.FilePath
            ).ToList();
            ...
            string body =
                "Errors found in the following localization override files:\n\n"
                + text
    ...
    private async Task<bool> InitializePlatform()
    {
        ...
        if (
            ...
            && (text.Equals("off", StringComparison.OrdinalIgnoreCase) || OS.HasFeature("editor"))
        )
        {
        ...
        if (!steamInitialized)
        {
            ...
            {
                Log.Error(
                    "Cannot show Steam error dialog: UI not initialized. Quitting immediately."
                );
                GetTree().Quit();
                return false;
            }
            NModalContainer.Instance.Add(nGenericPopup);
            await nGenericPopup.WaitForConfirmation(
                new LocString("main_menu_ui", "STEAM_INIT_ERROR.description"),
                new LocString("main_menu_ui", "STEAM_INIT_ERROR.title"),
                null,
                new LocString("main_menu_ui", "QUIT")
            );
            GetTree().Quit();
    ...
    private void OnSteamNoLongerRunning()
    {
        ...
        locString.Add(
            "details",
            $"{SteamInitializer.InitResult}: {SteamInitializer.InitErrorMessage}"
        );
        TaskHelper.RunSafely(
            nGenericPopup.WaitForConfirmation(
                locString,
                new LocString("main_menu_ui", "STEAM_STOPPED_ERROR.title"),
                null,
    ...
    public static void LogResourceStats(string context)
    {
        ...
        ulong renderingInfo = RenderingServer.GetRenderingInfo(
        ...
        Log.Info(
            $"[Startup] Resource stats ({context}): StaticMem={FormatBytes(staticMemoryUsage)}, VRAM={FormatBytes(renderingInfo)}, Objects={value:N0}, Resources={value2:N0}, Nodes={value3:N0}, CachedAssets={value4:N0}"
        );
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                ...
                MethodFlags.Normal | MethodFlags.Static,
                null,
                null
            )
        );
        ...
        list.Add(
            new MethodInfo(
                MethodName.InitializeGraphicsPreferences,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ApplyDisplaySettings,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetInspectRelicScreen,
                new PropertyInfo(
                    Variant.Type.Object,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetInspectCardScreen,
                new PropertyInfo(
                    Variant.Type.Object,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.DebugModifyTimescale,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ActivateWorldEnvironment,
                new PropertyInfo(
                    Variant.Type.Object,
                    "",
                    PropertyHint.None,
                    "",
                    PropertyUsageFlags.Default,
                    new StringName("WorldEnvironment"),
                    exported: false
                ),
                MethodFlags.Normal,
                null,
        ...
        list.Add(
            new MethodInfo(
                MethodName.DeactivateWorldEnvironment,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetScreenShakeTarget,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ClearScreenShakeTarget,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetScreenshakeMultiplier,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.CheckShowLocalizationOverrideErrors,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnSteamNoLongerRunning,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.IsGameFocusedWindow,
                new PropertyInfo(
                    Variant.Type.Bool,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ScreenShake(
                VariantUtils.ConvertTo<ShakeStrength>(in args[0]),
                VariantUtils.ConvertTo<ShakeDuration>(in args[1]),
                VariantUtils.ConvertTo<float>(in args[2])
            );
            ret = default(godot_variant);
            return true;
        }
        if (method == MethodName.ScreenRumble && args.Count == 3)
        {
            ScreenRumble(
                VariantUtils.ConvertTo<ShakeStrength>(in args[0]),
                VariantUtils.ConvertTo<ShakeDuration>(in args[1]),
                VariantUtils.ConvertTo<RumbleStyle>(in args[2])
            );
            ret = default(godot_variant);
            return true;
        }
        ...
        {
            ret = VariantUtils.CreateFrom<string>(
                FormatBytes(VariantUtils.ConvertTo<ulong>(in args[0]))
            );
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.RootSceneContainer,
                PropertyHint.None,
                "",
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.HoverTipsContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.RemoteCursorContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.ReactionContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.String,
                PropertyName.DebugSeedOverride,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.InspectRelicScreen,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.InspectCardScreen,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.ScreenshakeTarget,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._inspectionContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName.RootSceneContainer,
            Variant.From<NSceneContainer>(RootSceneContainer)
        );
        ...
        info.AddProperty(
            PropertyName.TimeoutOverlay,
            Variant.From<NMultiplayerTimeoutOverlay>(TimeoutOverlay)
        );
        info.AddProperty(PropertyName.AudioManager, Variant.From<NAudioManager>(AudioManager));
        info.AddProperty(
            PropertyName.RemoteCursorContainer,
            Variant.From<NRemoteMouseCursorContainer>(RemoteCursorContainer)
        );
        ...
        info.AddProperty(
            PropertyName.ReactionContainer,
            Variant.From<NReactionContainer>(ReactionContainer)
        );
        ...
        info.AddProperty(
            PropertyName.InspectRelicScreen,
            Variant.From<NInspectRelicScreen>(InspectRelicScreen)
        );
        info.AddProperty(
            PropertyName.InspectCardScreen,
            Variant.From<NInspectCardScreen>(InspectCardScreen)
        );
        info.AddProperty(
            PropertyName.FeedbackScreen,
            Variant.From<NSendFeedbackScreen>(FeedbackScreen)
        );
        info.AddProperty(
            PropertyName.WorldEnvironment,
            Variant.From<WorldEnvironment>(WorldEnvironment)
        );
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<WindowChangeEventHandler>(
                SignalName.WindowChange,
                out var value22
            )
        )
        ...
        if (
            info.TryGetSignalEventDelegate<PhobiaModeToggledEventHandler>(
                SignalName.PhobiaModeToggled,
                out var value23
            )
        )
        {

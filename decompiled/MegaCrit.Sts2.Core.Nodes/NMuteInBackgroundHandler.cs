...
public class NMuteInBackgroundHandler : Node
{
    ...
    public override void _Notification(int what)
    {
        if ((long)what == 1005)
        {
            if (!_loggedEnvironment)
            {
                ...
                Log.Info(
                    $"MuteInBackground: environment [displayServer={DisplayServer.GetName()}, os={OS.GetName()}]"
                );
            }
            _lastFocusOutMsec = Time.GetTicksMsec();
            Log.Info(
                $"MuteInBackground: FocusOut received [tween={_tween != null}, windowFocused={GetWindow().HasFocus()}]"
            );
            Mute();
        }
        else if ((long)what == 1004)
        {
            ...
            Log.Info(
                $"MuteInBackground: FocusIn received [tween={_tween != null}, msSinceFocusOut={value}, windowFocused={GetWindow().HasFocus()}]"
            );
            Unmute();
        }
    }
    ...
    private void Mute()
    {
        ...
        if (prefsSave != null && settingsSave != null && prefsSave.MuteInBackground)
        {
            ...
            _tween = CreateTween();
            _tween.TweenMethod(
                Callable.From<float>(SetMasterVolume),
                settingsSave.VolumeMaster,
                0f,
                1.0
            );
            if (flag)
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Int,
                PropertyName._lastFocusOutMsec,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._loggedEnvironment,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

...
public class NHandCardHolder : NCardHolder
{
    ...
    public new class MethodName : NCardHolder.MethodName
    {
        ...
        public static new readonly StringName Clear = "Clear";

        public static new readonly StringName OnFocus = "OnFocus";

        public static new readonly StringName OnUnfocus = "OnUnfocus";

        public static new readonly StringName OnMousePressed = "OnMousePressed";

        public static new readonly StringName OnMouseReleased = "OnMouseReleased";

        public static new readonly StringName DoCardHoverEffects = "DoCardHoverEffects";
        ...
        public static new readonly StringName SetCard = "SetCard";
    ...
    public event HolderFocusedEventHandler HolderFocused
    {
        add
        {
            backing_HolderFocused = (HolderFocusedEventHandler)
                Delegate.Combine(backing_HolderFocused, value);
        }
        remove
        {
            backing_HolderFocused = (HolderFocusedEventHandler)
                Delegate.Remove(backing_HolderFocused, value);
        }
    }
    ...
    public event HolderUnfocusedEventHandler HolderUnfocused
    {
        add
        {
            backing_HolderUnfocused = (HolderUnfocusedEventHandler)
                Delegate.Combine(backing_HolderUnfocused, value);
        }
        remove
        {
            backing_HolderUnfocused = (HolderUnfocusedEventHandler)
                Delegate.Remove(backing_HolderUnfocused, value);
        }
    }
    ...
    public event HolderMouseClickedEventHandler HolderMouseClicked
    {
        add
        {
            backing_HolderMouseClicked = (HolderMouseClickedEventHandler)
                Delegate.Combine(backing_HolderMouseClicked, value);
        }
        remove
        {
            backing_HolderMouseClicked = (HolderMouseClickedEventHandler)
                Delegate.Remove(backing_HolderMouseClicked, value);
        }
    }

    public static NHandCardHolder Create(NCard card, NPlayerHand hand)
    {
        NHandCardHolder nHandCardHolder = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NHandCardHolder>(PackedScene.GenEditState.Disabled);
    ...
    private async Task AnimAngle(CancellationTokenSource cancelToken)
    {
        ...
        {
            base.RotationDegrees = Mathf.Lerp(
                base.RotationDegrees,
                _targetAngle,
                (float)GetProcessDeltaTime() * 10f
            );
    ...
    private async Task AnimPosition(CancellationTokenSource cancelToken)
    {
        ...
        if (
            this.IsValid()
            && !base.Hitbox.IsEnabled
            && base.Position.DistanceSquaredTo(_targetPosition) < 200f
        )
        {
            base.Hitbox.SetEnabled(enabled: true);
        }
        ...
        if (method == MethodName.Create && args.Count == 2)
        {
            ret = VariantUtils.CreateFrom<NHandCardHolder>(
                Create(
                    ...
                    VariantUtils.ConvertTo<NPlayerHand>(in args[1])
                )
            );
            return true;
        }
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<HolderFocusedEventHandler>(
                SignalName.HolderFocused,
                out var value9
            )
        )
        {
        ...
        if (
            info.TryGetSignalEventDelegate<HolderUnfocusedEventHandler>(
                SignalName.HolderUnfocused,
                out var value10
            )
        )
        {
        ...
        if (
            info.TryGetSignalEventDelegate<HolderMouseClickedEventHandler>(
                SignalName.HolderMouseClicked,
                out var value11
            )
        )
        {
    ...
    [EditorBrowsable(EditorBrowsableState.Never)]
    internal static new List<MethodInfo> GetGodotSignalList()
    {

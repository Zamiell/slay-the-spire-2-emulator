...
public class AttackCommand
{
    ...
    private readonly List<Func<Creature, Node2D?>> _customHitVfxNodes =
        new List<Func<Creature, Node2D>>();
        ...
        if (IsMultiTargeted)
        {
            ...
            if (Attacker == null)
            {
                throw new InvalidOperationException(
                    "We require an attacker to be able to grab its opponents"
                );
            }
            return _combatState.GetOpponentsOf(Attacker);
        }
        throw new InvalidOperationException(
            "No targets set, a Targeting method must be called before Execute"
        );
    }
    ...
    public AttackCommand TargetingRandomOpponents(
    ...
    public AttackCommand WithAttackerAnim(
        string? animName,
        float delay,
        Creature? visualAttacker = null
    )
    {
        if (_attackerAnimName == null)
        {
            throw new InvalidOperationException(
                "WithAttackerAnim was called before FromCard/FromMonster/FromOsty, should be called after."
            );
        }
    ...
    public AttackCommand WithAttackerFx(
        string? vfx = null,
        string? sfx = null,
        string? tmpSfx = null
    )
    ...
    public async Task<AttackCommand> Execute(PlayerChoiceContext? choiceContext)
    {
        ...
        if (
            ...
            && (combatState == null || combatState.IsLiveCombat())
        )
        {
            return this;
        }
        ...
        for (int i = 0; (decimal)i < attackCount; i++)
        {
                        ...
                        .Where(
                            (Creature c) =>
                                _results
                                    ...
                                    .All((DamageResult r) => r.Receiver != c)
                        )
                        .ToList();
                    if (validTargets.Count == 0)
                    {
                        throw new InvalidOperationException(
                            "No valid targets for attack with duplicates disallowed. If you're in a test, you probably need to add more enemies. If you're in real gameplay, something is wrong."
                        );
                    }
                }
                Rng combatTargets = (Attacker.Player ?? Attacker.PetOwner)
                    .RunState
                    .Rng
                    .CombatTargets;
            ...
            AddResultsInternal(
                await CreatureCmd.Damage(
                    amount: (_calculatedDamageVar == null)
                        ? _damagePerHit
                        : _calculatedDamageVar.Calculate(singleTarget),
                    choiceContext: choiceContext ?? new BlockingPlayerChoiceContext(),
                    targets: (singleTarget != null)
                        ? ((IEnumerable<Creature>)new List<Creature>(1) { singleTarget })
                        : ((IEnumerable<Creature>)validTargets),
                    props: DamageProps,
                    dealer: Attacker,
        ...
        CombatManager.Instance.History.CreatureAttacked(
            combatState,
            Attacker,
            _results.SelectMany((List<DamageResult> r) => r).ToList()
        );

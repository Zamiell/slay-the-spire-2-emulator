...
public class NMerchantInventory : Control, IScreenContext
{
    ...
    public event InventoryClosedEventHandler InventoryClosed
    {
        add
        {
            backing_InventoryClosed = (InventoryClosedEventHandler)
                Delegate.Combine(backing_InventoryClosed, value);
        }
        remove
        {
            backing_InventoryClosed = (InventoryClosedEventHandler)
                Delegate.Remove(backing_InventoryClosed, value);
        }
    }
    ...
    private async Task DoOpenAnimation()
    {
        ...
        _inventoryTween
            .TweenProperty(_backstop, "modulate:a", 0.8f, 1.0)
            .SetEase(Tween.EaseType.InOut)
            .SetTrans(Tween.TransitionType.Sine)
            .FromCurrent();
        _inventoryTween
            .TweenProperty(_slotsContainer, "position:y", 80f, 0.699999988079071)
    ...
    private void Close()
    {
        ...
        _inventoryTween = CreateTween().SetParallel();
        _inventoryTween
            .TweenProperty(_backstop, "modulate:a", 0f, 0.800000011920929)
            .SetEase(Tween.EaseType.InOut)
            .SetTrans(Tween.TransitionType.Sine)
            .FromCurrent();
        _inventoryTween
            .TweenProperty(_slotsContainer, "position:y", -1000f, 0.5)
            .SetEase(Tween.EaseType.Out)
    ...
    {
        UpdateNavigation();
        NMerchantSlot lastSlot = GetAllSlots()
            .FirstOrDefault((NMerchantSlot s) => s.Entry == entry);
        if (lastSlot != null && !_isInputBlocked)
        {
            (from s in GetAllSlots() where s.Visible && s != lastSlot select s)
                .MinBy((NMerchantSlot s) => (s.GlobalPosition - lastSlot.GlobalPosition).Length())
                ?.TryGrabFocus();
        }
    ...
    private IEnumerable<NMerchantCard> GetCardSlots()
    {
        return new IEnumerable<Node>[2]
        {
            ...
            .SelectMany((IEnumerable<Node> n) => n)
            .OfType<NMerchantCard>();
    }
    ...
    {
        List<NMerchantSlot> source =
            (
                from c in _characterCardContainer?.GetChildren().OfType<NMerchantSlot>()
                where c.Visible
                select c
            ).ToList() ?? new List<NMerchantSlot>();
        List<NMerchantSlot> list =
            (
                from c in _colorlessCardContainer?.GetChildren().OfType<NMerchantSlot>()
                where c.Visible
                select c
            ).ToList() ?? new List<NMerchantSlot>();
        List<NMerchantSlot> list2 =
            (
                from c in _relicContainer?.GetChildren().OfType<NMerchantSlot>()
                where c.Visible
                select c
            ).ToList() ?? new List<NMerchantSlot>();
        List<NMerchantSlot> list3 =
            (
                from c in _potionContainer?.GetChildren().OfType<NMerchantSlot>()
                where c.Visible
                select c
            ).ToList() ?? new List<NMerchantSlot>();
        ...
        {
            IEnumerable<NMerchantSlot> enumerable =
                new global::_003C_003Ez__ReadOnlySingleElementList<NMerchantSlot>(_cardRemovalNode);
            second = enumerable;
        }
        ...
        {
            list4[num].FocusNeighborLeft = (
                (num > 0) ? list4[num - 1].GetPath() : list4[num].GetPath()
            );
            list4[num].FocusNeighborRight = (
                (num < list4.Count - 1) ? list4[num + 1].GetPath() : list4[num].GetPath()
            );
        }
        for (int num2 = 0; num2 < list6.Count; num2++)
        {
            list6[num2].FocusNeighborLeft = (
                (num2 > 0) ? list6[num2 - 1].GetPath() : list6[num2].GetPath()
            );
            list6[num2].FocusNeighborRight = (
                (num2 < list6.Count - 1) ? list6[num2 + 1].GetPath() : list6[num2].GetPath()
            );
        }
        for (int num3 = 0; num3 < list5.Count; num3++)
        {
            list5[num3].FocusNeighborLeft = (
                (num3 > 0) ? list5[num3 - 1].GetPath() : list5[num3].GetPath()
            );
            list5[num3].FocusNeighborRight = (
                (num3 < list5.Count - 1) ? list5[num3 + 1].GetPath() : list5[num3].GetPath()
            );
        }
    ...
    {
        List<NMerchantSlot> source =
            _characterCardContainer?.GetChildren().OfType<NMerchantSlot>().ToList()
            ?? new List<NMerchantSlot>();
        List<NMerchantSlot> first =
            _colorlessCardContainer?.GetChildren().OfType<NMerchantSlot>().ToList()
            ?? new List<NMerchantSlot>();
        List<NMerchantSlot> second =
            _relicContainer?.GetChildren().OfType<NMerchantSlot>().ToList()
            ?? new List<NMerchantSlot>();
        List<NMerchantSlot> second2 =
            _potionContainer?.GetChildren().OfType<NMerchantSlot>().ToList()
            ?? new List<NMerchantSlot>();
    ...
    private void OnActiveScreenUpdated()
    {
        ...
        {
            if (
                _characterCardContainer != null
                && NControllerManager.Instance.IsUsingController
                && _inventoryTween != null
                && _inventoryTween.IsRunning()
            )
            {
                float num = 80f - _slotsContainer.Position.Y;
                MerchantHand.PointAtTarget(
                    _characterCardContainer.GetChild<NMerchantCard>(0),
                    Vector2.Down * num
                );
            }
            if (!_isInputBlocked)
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateHorizontalNavigation,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateVerticalNavigation,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._characterCardContainer,
                PropertyHint.None,
                "",
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._colorlessCardContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._merchantDialogue,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._characterCardContainer,
            Variant.From(in _characterCardContainer)
        );
        info.AddProperty(
            PropertyName._colorlessCardContainer,
            Variant.From(in _colorlessCardContainer)
        );
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<InventoryClosedEventHandler>(
                SignalName.InventoryClosed,
                out var value16
            )
        )
        {

...
public class ENetHost : NetHost
{
    ...
    private readonly List<HandshakeAwaitingResponse> _receivedHandshakes =
        new List<HandshakeAwaitingResponse>();

    private readonly MegaCrit.Sts2.Core.Logging.Logger _logger =
        new MegaCrit.Sts2.Core.Logging.Logger("ENetHost", LogType.Network);

    public override bool IsConnected => _isConnected;

    public override IEnumerable<ulong> ConnectedPeerIds =>
        _connectedPeers.Select((ClientConnection c) => c.netId);
    ...
    private async Task DoClientHandshake(ENetPacketPeer peer)
    {
        ...
        {
            _logger.Info(
                $"Second client attempted to connect with peer ID {handshake.Value.conn.netId}, disconnecting them"
            );
            ENetPacket eNetPacket = ENetPacket.FromHandshakeResponse(
                new ENetHandshakeResponse
                {
        ...
        else
        {
            ...
            ENetPacket eNetPacket2 = ENetPacket.FromHandshakeResponse(
                new ENetHandshakeResponse
                {
                    netId = handshake.Value.conn.netId,
    ...
    private void HandlePacketReceived(ENetServiceData data)
    {
        ...
        else
        {
            ...
            {
                _logger.Error(
                    $"Received a non-handshake packet length {packetData.Length} for a peer, but no connection exists!"
                );
            }
            else
            {
                _handler.OnPacketReceived(
                    connectionByPeer2.Value.netId,
                    eNetPacket.AsAppMessage(),
                    data.mode,
                    data.channel
                );
            }
    ...
    )
    {
        ...
        {
            _logger.Error(
                $"Tried to send message to client with ID {peerId}, but no client with that ID is connected!"
            );
        }
        else
        {
            ...
            connectionById.Value.peer.Send(
                channel,
                eNetPacket.AllBytes,
                ENetUtil.FlagsFromMode(mode)
            );
        }
    }

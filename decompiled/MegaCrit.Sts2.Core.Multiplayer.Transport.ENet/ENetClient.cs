...
public class ENetClient : NetClient
{
    ...
    private readonly MegaCrit.Sts2.Core.Logging.Logger _logger =
        new MegaCrit.Sts2.Core.Logging.Logger("ENetClient", LogType.Network);
    ...
    )
    {
        ...
        while (
            !_connection.TryService(out output)
            || output.Value.type != ENetConnection.EventType.Connect
        )
        {
    ...
    )
    {
        ...
        ENetPacket eNetPacket = ENetPacket.FromHandshakeRequest(
            new ENetHandshakeRequest { netId = netId }
        );
        ...
        while (!receivedAck)
        {
            ...
            if (
                _connection.TryService(out var output)
                && output.Value.type == ENetConnection.EventType.Receive
            )
            {
                ...
                {
                    _logger.Error(
                        $"Received non-success code during handshake ({eNetHandshakeResponse.status})!"
                    );
            ...
            if ((ulong)num <= 4uL)
            {
                switch (num)
                {
                    case ENetConnection.EventType.None:
                        _logger.Error(
                            $"Got error from ENetConnection! Error: {output.Value.error} TODO: Expand me"
                        );
                        continue;
                    ...
                    case ENetConnection.EventType.Receive:
                        _logger.Debug(
                            $"Received disconnect on client. Already disconnected: {!_isConnected}"
                        );
                        if (_isConnected)
                        {
                            _isConnected = false;
                            _handler.OnDisconnectedFromHost(
                                HostNetId,
                                new NetErrorInfo(NetError.UnknownNetworkError, selfInitiated: false)
                            );
                        }
                        continue;
    ...
    private void HandleMessageReceived(ENetServiceData data)
    {
        if (_isConnected)
        {
            ...
            else if (eNetPacket.PacketType == ENetPacketType.Disconnection)
            {
                ...
                _logger.Debug(
                    $"Received disconnection packet with reason: {eNetDisconnection.reason}"
                );
                _handler.OnDisconnectedFromHost(
                    HostNetId,
                    new NetErrorInfo(eNetDisconnection.reason, selfInitiated: false)
                );
                _isConnected = false;
            }
            else
            {
                _logger.Error(
                    $"Got unexpected packet of type {eNetPacket.PacketType} while we were connected to the host!"
                );
            }
        }
    }
    ...
    public override void DisconnectFromHost(NetError reason, bool now = false)
    {
        ...
        {
            ENetPacket eNetPacket = ENetPacket.FromDisconnection(
                new ENetDisconnection { reason = reason }
            );
        ...
        if (_isConnected)
        {
            ...
            _handler.OnDisconnectedFromHost(
                HostNetId,
                new NetErrorInfo(reason, selfInitiated: true)
            );
            _connection?.Destroy();
        }
    }
    ...
    )
    {
        if (!_isConnected)
        {
            throw new InvalidOperationException(
                "Tried to send message to host while disconnected!"
            );
        }
        AssertClientStarted();
    ...
    private void AssertClientStarted()
    {
        ...
        {
            throw new InvalidOperationException(
                "Must call ConnectToHost first and wait for connection!"
            );
        }
    }

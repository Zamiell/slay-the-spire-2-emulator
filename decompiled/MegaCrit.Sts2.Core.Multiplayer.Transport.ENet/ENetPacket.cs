...
public class ENetPacket
{
    ...
    public ENetHandshakeRequest AsHandshakeRequest()
    {
        if (PacketType != ENetPacketType.HandshakeRequest)
        {
            throw new InvalidOperationException(
                $"Attempted to interpret ENet packet of type {PacketType} as handshake request"
            );
        }
        ulong netId = BinaryPrimitives.ReadUInt64BigEndian(_packetBytes[1..]);
        return new ENetHandshakeRequest { netId = netId };
    }

    public static ENetPacket FromHandshakeResponse(ENetHandshakeResponse response)
    {
        byte[] array = new byte[10] { 1, (byte)response.status, 0, 0, 0, 0, 0, 0, 0, 0 };
        Span<byte> span = array.AsSpan();
    ...
    public ENetHandshakeResponse AsHandshakeResponse()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Attempted to interpret ENet packet of type {PacketType} as handshake response"
            );
        }
        ...
        return new ENetHandshakeResponse { netId = netId, status = status };
    }

    public static ENetPacket FromDisconnection(ENetDisconnection disconnection)
    {
        return new ENetPacket(new byte[2] { 2, (byte)disconnection.reason });
    }
    ...
    public ENetDisconnection AsDisconnection()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Attempted to interpret ENet packet of type {PacketType} as disconnection"
            );
        }
        NetError reason = (NetError)_packetBytes[1];
        return new ENetDisconnection { reason = reason };
    }

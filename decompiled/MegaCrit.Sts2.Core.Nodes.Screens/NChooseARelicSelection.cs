...
public class NChooseARelicSelection : Control, IOverlayScreen, IScreenContext
{
    ...
    private readonly TaskCompletionSource<IEnumerable<RelicModel>> _completionSource =
        new TaskCompletionSource<IEnumerable<RelicModel>>();
    ...
    private static string ScenePath =>
        SceneHelper.GetScenePath("screens/choose_a_relic_selection_screen");
    ...
    public Control DefaultFocusedControl
    {
        get
        {
            List<NRelicBasicHolder> list = _relicRow
                .GetChildren()
    ...
    public static NChooseARelicSelection? ShowScreen(IReadOnlyList<RelicModel> relics)
    {
        ...
        NChooseARelicSelection nChooseARelicSelection = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NChooseARelicSelection>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        for (int i = 0; i < _relics.Count; i++)
        {
            ...
            _cardTween
                .TweenProperty(
                    holder,
                    "position",
                    holder.Position + vector + Vector2.Right * 200f * i,
                    0.5
                )
        ...
        NRelicBasicHolder nRelicBasicHolder = _relicRow
            .GetChildren()
            .OfType<NRelicBasicHolder>()
            .ToList()[list.Count / 2];
        ...
        for (int num = 0; num < _relicRow.GetChildCount(); num++)
        {
            ...
            child.FocusNeighborLeft = (
                (num > 0)
                    ? _relicRow.GetChild(num - 1).GetPath()
                    : _relicRow.GetChild(_relicRow.GetChildCount() - 1).GetPath()
            );
            child.FocusNeighborRight = (
                (num < _relicRow.GetChildCount() - 1)
                    ? _relicRow.GetChild(num + 1).GetPath()
                    : _relicRow.GetChild(0).GetPath()
            );
        }
    }

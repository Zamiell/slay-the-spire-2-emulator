...
public class NRewardsScreen : Control, IOverlayScreen, IScreenContext
{
    ...
    {
        NRewardsScreen nRewardsScreen = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NRewardsScreen>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        {
            _headerLabel.SetTextAutoSize(
                new LocString("gameplay_ui", "COMBAT_REWARD_HEADER_MUGGED").GetFormattedText()
            );
        }
        else
        {
            _headerLabel.SetTextAutoSize(
                new LocString("gameplay_ui", "COMBAT_REWARD_HEADER_LOOT").GetFormattedText()
            );
        }
        GetViewport()
            .Connect(Viewport.SignalName.GuiFocusChanged, Callable.From<Control>(ProcessGuiFocus));
        _rewardsContainer.Connect(
        ...
        foreach (Reward item in list)
        {
            ...
            if (item is LinkedRewardSet linkedReward)
            {
                ...
                option.Connect(
                    ...
                    Callable.From<NLinkedRewardSet>(RewardCollectedFrom)
                );
            }
            else
            {
                ...
                option.Connect(
                    ...
                    Callable.From<NRewardButton>(RewardCollectedFrom)
                );
                option.Connect(
                    NRewardButton.SignalName.RewardSkipped,
                    Callable.From<NRewardButton>(RewardSkippedFrom)
                );
                option.Connect(
                    Control.SignalName.FocusEntered,
                    Callable.From(() => _lastRewardFocused = option)
                );
            }
    ...
    public void RewardCollectedFrom(Control button)
    {
        ...
        RemoveButton(button);
        _lastRewardFocused = (
            (_rewardButtons.Count > 0)
                ? _rewardButtons[Mathf.Min(a, _rewardButtons.Count - 1)]
                : null
        );
        UpdateScreenState();
    ...
    private void UpdateScreenState()
    {
        if (_rewardButtons.Count == 0)
        {
            ...
            {
                Log.Error(
                    "All rewards have been taken, but the rewards set is not complete on the backend!"
                );
            }
            if (_isTerminal)
            {
        ...
        for (int i = 0; i < _rewardButtons.Count; i++)
        {
            ...
            _rewardButtons[i].FocusNeighborTop = (
                (i > 0) ? _rewardButtons[i - 1].GetPath() : _rewardButtons[i].GetPath()
            );
            _rewardButtons[i].FocusNeighborBottom = (
                (i < _rewardButtons.Count - 1)
                    ? _rewardButtons[i + 1].GetPath()
                    : _rewardButtons[i].GetPath()
            );
        }
    }
    ...
    private void OnProceedButtonPressed(NButton _)
    {
        if (RunManager.Instance.debugAfterCombatRewardsOverride != null && _isTerminal)
        {
            ...
            _isTerminal
            && (
                _runState.CurrentRoom.RoomType == RoomType.Boss
                || _runState.CurrentRoom.IsVictoryRoom
            )
        )
        {
            if (
                _runState.Map.SecondBossMapPoint != null
                && _runState.CurrentMapCoord == _runState.Map.BossMapPoint.coord
            )
            {
    ...
    {
        if (
            (!_skipDisallowed || !_proceedButton.IsSkip)
            && Hook.ShouldProceedToNextMapPoint(_runState)
            && !_proceedButton.IsEnabled
        )
        {
    ...
    public void AfterOverlayShown()
    {
        ...
        if (!IsComplete)
        {
            ...
            _fadeTween
                .TweenProperty(_rewardsWindow, "position:y", _rewardsWindow.Position.Y, 0.5)
    ...
    private void ProcessGuiFocus(Control focusedControl)
    {
        if (
            ...
            && _rewardButtons.Contains(focusedControl)
        )
        {
    ...
    private void UpdateScrollPosition(double delta)
    {
        ...
        {
            _rewardsContainer.Position = _rewardsContainer.Position.Lerp(
                _targetDragPos,
                Mathf.Clamp((float)delta * 15f, 0f, 1f)
            );
            ...
            if (!_scrollbarPressed && CanScroll)
            {
                _scrollbar.SetValueWithoutAnimation(
                    Mathf.Clamp(_rewardsContainer.Position.Y / ScrollLimitBottom, 0f, 1f) * 100f
                );
            }
        }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.RewardCollectedFrom,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.TryEnableProceedButton,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.HideWaitingForPlayersScreen,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._rewardsContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._rewardContainerMask,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._lastRewardFocused,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<CompletedEventHandler>(

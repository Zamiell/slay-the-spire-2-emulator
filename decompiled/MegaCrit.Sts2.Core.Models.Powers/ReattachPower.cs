...
public sealed class ReattachPower : PowerModel
{
    ...
    public async Task DoReattach()
    {
        ...
        {
            (NCombatRoom.Instance?.GetCreatureNode(base.Owner))
    ...
    private void DoFadeOutOnAllSegments()
    {
        ...
        Task deathAnimationTask = TaskHelper.RunSafely(
            PlayVfxAndThenRemoveNodes(nMonsterDeathVfx, list)
        );
    ...
    {
        return from c in base
                .Owner.CombatState.GetTeammatesOf(base.Owner)
                .Except(new global::_003C_003Ez__ReadOnlySingleElementList<Creature>(base.Owner))

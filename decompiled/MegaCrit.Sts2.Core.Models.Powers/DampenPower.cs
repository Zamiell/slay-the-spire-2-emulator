...
public sealed class DampenPower : PowerModel
{
    private class Data
    {
        ...
        public readonly Dictionary<CardModel, int> downgradedCardsToOldUpgradeLevels =
    ...
    {
        IEnumerable<CardModel> enumerable = base.Owner.Player.PlayerCombatState.AllCards.Where(
            (CardModel c) => c.IsUpgraded
        );
        foreach (CardModel item in enumerable)
        {
            GetInternalData<Data>()
                .downgradedCardsToOldUpgradeLevels.Add(item, item.CurrentUpgradeLevel);
    ...
    public override Task AfterRemoved(Creature oldOwner)
    {
        foreach (
            KeyValuePair<
                CardModel,
                int
            > downgradedCardsToOldUpgradeLevel in GetInternalData<Data>().downgradedCardsToOldUpgradeLevels
        )
        {

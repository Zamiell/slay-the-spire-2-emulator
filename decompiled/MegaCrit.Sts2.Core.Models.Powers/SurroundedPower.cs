...
public sealed class SurroundedPower : PowerModel
{
    ...
    public override async Task BeforePotionUsed(PotionModel potion, Creature? target)
    {
        if (
            ...
            && target != null
            && potion.Owner == base.Owner.Player
        )
        {
    ...
    )
    {
        if (!wasRemovalPrevented && creature.Side != base.Owner.Side)
        {
            ...
            if (
                hittableEnemies.Count != 0
                && (
                    hittableEnemies.All((Creature e) => e.HasPower<BackAttackLeftPower>())
                    || hittableEnemies.All((Creature e) => e.HasPower<BackAttackRightPower>())
                )
            )
            {
    ...
    private async Task FaceDirection(Direction direction)
    {
        ...
        IEnumerable<Node2D> enumerable = source.Select(
            (Creature c) => NCombatRoom.Instance?.GetCreatureNode(c)?.Body
        );
        ...
        if (LocalContext.GetMe(base.Owner.CombatState) == base.Owner.Player)
        {
            NRunMusicController.Instance?.UpdateMusicParameter(
                "kaiser_crab_direction",
                (Facing == Direction.Left) ? 1 : 2
            );
        }
    }

...
public sealed class DoomPower : PowerModel
{
    ...
    {
        return creatures
            .Where((Creature c) => c.GetPower<DoomPower>()?.IsOwnerDoomed() ?? false)
            .ToList();
    }
    ...
    private static async Task PlayVfx(Creature creature)
    {
        ...
        if (creature.IsMonster)
        {
            flag =
                Hook.ShouldDie(
                    ...
                    creature,
                    out AbstractModel _
                ) && creature.Monster.ShouldDisappearFromDoom;
        }
        ...
        List<Creature> source = (
            from c in creature.CombatState.GetTeammatesOf(creature)
            where c.IsAlive
            select c
        ).ToList();
        if (flag)
    ...
    private static void StartDoomAnim(NCreature creature, bool shouldDie)
    {
        ...
        NDoomVfx nDoomVfx = NDoomVfx.Create(
            creature.Visuals,
        ...
        if (shouldDie)
        {
            ...
            global::_003CPrivateImplementationDetails_003E.InlineArrayElementRef<
                global::_003C_003Ey__InlineArray2<Task>,
                Task
            >(ref buffer, 1) = nDoomVfx.VfxTask;
            creature.DeathAnimationTask = Task.WhenAll(

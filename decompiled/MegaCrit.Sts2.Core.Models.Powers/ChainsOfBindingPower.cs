...
public sealed class ChainsOfBindingPower : PowerModel
{
    ...
    {
        if (
            card.Owner == base.Owner.Player
            && base.CombatState.CurrentSide == base.Owner.Side
            && ModelDb.Affliction<Bound>().CanAfflict(card)
        )
        {
            int num = CombatManager
                .Instance.History.Entries.OfType<CardAfflictedEntry>()
                .Count(
                    (CardAfflictedEntry e) =>
    ...
    )
    {
        ...
        IEnumerable<CardModel> enumerable =
            base.Owner.Player?.PlayerCombatState?.AllCards ?? Array.Empty<CardModel>();

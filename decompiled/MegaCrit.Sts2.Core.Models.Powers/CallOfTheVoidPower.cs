...
public sealed class CallOfTheVoidPower : PowerModel
{
    ...
    )
    {
        ...
        IReadOnlyList<CardModel> readOnlyList = base
            .Owner.Player.Character.CardPool.GetUnlockedCards(
                player.UnlockState,
            ...
            .Where(
                delegate(CardModel c)
                {
                    ...
                    bool flag = (
                        (rarity == CardRarity.Basic || rarity == CardRarity.Ancient) ? true : false
                    );
                    return !flag;
                }
            )
        ...
        if (readOnlyList.Count > 0)
        {
            ...
            for (int num = 0; num < base.Amount; num++)
            {
                CardCmd.ApplyKeyword(
                    array[num] = CardFactory
                        .GetDistinctForCombat(player, readOnlyList, 1, combatCardGeneration)
                        .First(),

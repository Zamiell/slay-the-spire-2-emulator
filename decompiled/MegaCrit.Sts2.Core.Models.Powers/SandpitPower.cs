        ...
        if (!TestMode.IsOn && power == this)
        {
            ...
            if (LocalContext.IsMe(base.Target))
            {
                ...
                NRunMusicController.Instance?.UpdateMusicParameter(
                    TheInsatiable.TheInsatiableTrackName,
                    num
                );
            }
        }
    ...
    public override async Task AfterRemoved(Creature oldOwner)
    {
        ...
        if (TestMode.IsOff)
        {
            ...
            foreach (Creature allAffectedCreature in AllAffectedCreatures)
            {
                ...
                tween
                    .Parallel()
                    .TweenProperty(creatureNode2, "global_position:x", num, 0.699999988079071)
        ...
        foreach (Creature allAffectedCreature2 in AllAffectedCreatures)
        {
            if (TestMode.IsOff)
            {
                NCreature creatureNode3 = NCombatRoom.Instance.GetCreatureNode(
                    allAffectedCreature2
                );
        ...
        foreach (Creature allAffectedCreature in AllAffectedCreatures)
        {
            ...
            if (creatureNode4 != null)
            {
                ...
                tween.TweenProperty(
                    creatureNode4,
                    "global_position:x",
                    creatureNode4.GlobalPosition.X + num9,
                    0.25
                );
            }
        }

...
public sealed class GalvanicPower : PowerModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlySingleElementList<DynamicVar>(
            new StringVar(
                "AfflictionTitle",
                ModelDb.Affliction<Galvanized>().Title.GetFormattedText()
            )
        );

    protected override IEnumerable<IHoverTip> ExtraHoverTips =>
        HoverTipFactory.FromAffliction<Galvanized>(base.Amount);
    ...
    public override async Task BeforeCombatStart()
    {
        foreach (Creature item in base.Owner.CombatState.Allies.ToList())
        {
            ...
            IEnumerable<CardModel> enumerable = item.Player.PlayerCombatState.AllCards.Where(

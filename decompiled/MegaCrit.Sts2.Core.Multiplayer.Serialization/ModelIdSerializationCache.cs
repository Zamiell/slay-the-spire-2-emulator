...
{
    private static readonly Dictionary<string, int> _categoryNameToNetIdMap = new Dictionary<
        string,
        int
    >
    {
    ...
    public static void Init()
    {
        ...
        foreach (Mod mod in ModManager.Mods)
        {
            ...
            foreach (
                Type item6 in ReflectionHelper.GetSubtypesFromAssembly(
                    mod.assembly,
        ...
        list.Sort(
            delegate((Type, Mod?) p1, (Type, Mod?) p2)
            {
                (Type, Mod?) tuple = p1;
                ...
                {
                    Log.Warn(
                        $"Two AbstractModels {item} and {item3} from mod {item2.manifest?.id} share an ID! This might break multiplayer."
                    );
                }
                return num2;
            }
        ...
        Log.Info(
            $"ModelIdSerializationCache initialized. Categories: {MaxCategoryId} Entries: {MaxEntryId} Epochs: {MaxEpochId} Hash: {Hash}"
        );
    }
    ...
    public static int GetNetIdForCategory(string category)
    {
        if (!_categoryNameToNetIdMap.TryGetValue(category, out var value))
        {
            throw new ArgumentException(
                "ModelId category " + category + " could not be mapped to any net ID!"
            );
        }
        return value;
    }
    ...
    public static string GetCategoryForNetId(int netId)
    {
        if (netId < 0 || netId >= MaxCategoryId)
        {
            throw new ArgumentOutOfRangeException(
                $"ModelId category ID {netId} is out of range! We have {_netIdToCategoryNameMap.Count} categories"
            );
        }
    ...
    public static int GetNetIdForEntry(string entry)
    {
        ...
        {
            throw new ArgumentException(
                "ModelId entry " + entry + " could not be mapped to any net ID!"
            );
        }
        return value;
    }
    ...
    public static string GetEntryForNetId(int netId)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(
                $"ModelId entry ID {netId} is out of range! We have {_netIdToEntryNameMap.Count} entries"
            );
        }
    ...
    public static int GetNetIdForEpochId(string epochId)
    {
        ...
        {
            throw new ArgumentException(
                "Epoch ID " + epochId + " could not be mapped to any net ID!"
            );
        }
        return value;
    }
    ...
    public static string GetEpochIdForNetId(int netId)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(
                $"Epoch ID {netId} is out of range! We have {_netIdToEpochNameMap.Count} entries"
            );
        }

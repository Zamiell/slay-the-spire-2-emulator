...
{
    public static T ReadModel<T>(this PacketReader reader)
    ...
    public static ModelId ReadModelIdAssumingType<T>(this PacketReader reader)
        where T : AbstractModel
    {
        if (typeof(T) == typeof(AbstractModel))
        {
        ...
        string entryForNetId = ModelIdSerializationCache.GetEntryForNetId(
            reader.ReadInt(ModelIdSerializationCache.EntryIdBitSize)
        );
        ...
        return new ModelId(
            ModelId.SlugifyCategory(ModelDb.GetCategoryType(typeof(T)).Name),
            entryForNetId
        );
    }
    ...
    {
        string categoryForNetId = ModelIdSerializationCache.GetCategoryForNetId(
            reader.ReadInt(ModelIdSerializationCache.CategoryIdBitSize)
        );
        string entryForNetId = ModelIdSerializationCache.GetEntryForNetId(
            reader.ReadInt(ModelIdSerializationCache.EntryIdBitSize)
        );
    ...
    {
        return ModelIdSerializationCache.GetEpochIdForNetId(
            reader.ReadInt(ModelIdSerializationCache.EpochIdBitSize)
        );
    }
    ...
    public static List<T> ReadModelList<T>(this PacketReader reader)
        where T : AbstractModel
    {
    ...
    public static List<ModelId> ReadModelIdListAssumingType<T>(this PacketReader reader)
        where T : AbstractModel
    {
        List<ModelId> list = new List<ModelId>();
        int num = reader.ReadInt();

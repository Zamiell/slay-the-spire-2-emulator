...
public class PacketWriter
{
        ...
        where T : struct, Enum
    {
        if (!typeof(int).IsAssignableFrom(Enum.GetUnderlyingType(typeof(T))))
        {
            throw new InvalidOperationException(
                $"Trying to write enum type {typeof(T)} that is not assignable to int!"
            );
        }
    ...
    public void WriteFloat(float val, QuantizeParams? quantizeParams = null)
    {
        if (quantizeParams.HasValue)
        {
            ...
            uint val2 = Quantize(
                val,
                quantizeParams.Value.min,
                quantizeParams.Value.max,
                quantizeParams.Value.bits
            );
    ...
    public void WriteList<T>(IReadOnlyList<T> list, int lengthBits = 32)
        where T : IPacketSerializable, new()
    {
        if ((ulong)list.Count >= (ulong)(1L << lengthBits))
        {
            throw new IndexOutOfRangeException(
                $"List length {list.Count} is too large to fit in bit size {lengthBits}!"
            );
        }
    ...
    private void ResizeBufferIfNecessary(int bitsBeingWritten)
    {
        ...
        if (num2 != Buffer.Length)
        {
            ...
            {
                Log.Warn(
                    $"Warning: Packet writer is growing from {Buffer.Length} bytes to {array.Length} bytes!"
                );
            }
            Buffer = array;
        }

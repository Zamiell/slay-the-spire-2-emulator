...
public class NetTypeCache<
    [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.Interfaces)] TBase
>
    where TBase : class
{
        ...
        for (int num = 0; num < types.Count; num++)
        {
            ...
            if (!type.GetInterfaces().Contains<Type>(typeof(TBase)))
            {
                throw new InvalidOperationException(
                    $"Type {types[num]} does not implement interface {typeof(TBase)}!"
                );
            }
            _typeToId[type] = num;
        }

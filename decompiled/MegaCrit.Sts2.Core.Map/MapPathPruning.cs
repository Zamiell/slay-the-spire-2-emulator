...
public static class MapPathPruning
{
    ...
    {
        bool flag = false;
        flag |= RepairPointType(
            map,
            MapPointType.Shop,
            pointTypeCounts.NumOfShops,
            rng,
            isValidPointType
        );
        flag |= RepairPointType(
        ...
        while (PrunePaths(grid, startMapPoints, matchingSegments, rng))
        {
            ...
            if (num > 50)
            {
                throw new InvalidOperationException(
                    $"Unable to prune matching segments in {num} iterations"
                );
            }
    ...
    public static List<List<MapPoint[]>> FindMatchingSegments(MapPoint startingMapPoint)
    {
        ...
        SortedDictionary<string, List<MapPoint[]>> segments = new SortedDictionary<
            string,
            List<MapPoint[]>
    ...
    private static string GenerateSegmentKey(IReadOnlyList<MapPoint> segment)
    {
        ...
        if (mapPoint.coord.row == 0)
        {
            ...
            StringBuilder.AppendInterpolatedStringHandler handler =
                new StringBuilder.AppendInterpolatedStringHandler(3, 3, stringBuilder2);
        ...
        else
        {
            ...
            StringBuilder.AppendInterpolatedStringHandler handler =
                new StringBuilder.AppendInterpolatedStringHandler(4, 4, stringBuilder2);
        ...
        stringBuilder.Append(
            string.Join(",", segment.Select((MapPoint point) => (int)point.PointType))
        );
    ...
    {
        return existingSegments.Any(
            (MapPoint[] existingSegment) => OverlappingSegment(existingSegment, segment)
        );
    }
    ...
    {
        return segments
            .Values.Where((List<MapPoint[]> segmentList) => segmentList.Count > 1)
            .ToList();
    }
    ...
    )
    {
        ...
        for (int i = 0; i < segment.Length - 1; i++)
        {
            ...
            if (
                mapPoint.Children.Count > 1
                || mapPoint.parents.Count > 1
                || mapPoint.parents.Any(
                    (MapPoint n) => n.Children.Count == 1 && !IsRemoved(grid, n)
                )
            )
            {
                continue;
            ...
            if (!source.Any((MapPoint n) => n.Children.Count > 1 && n.parents.Count == 1))
            {
                ...
                if (
                    !mapPoint
                        .Children.Where((MapPoint c) => !segment.Contains(c))
                        .Any((MapPoint c) => c.parents.Count == 1)
                )
                {

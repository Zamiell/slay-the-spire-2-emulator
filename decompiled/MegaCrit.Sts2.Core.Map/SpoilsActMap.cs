...
public sealed class SpoilsActMap : ActMap
{
    ...
    private MapCoord GenerateNextCoord(MapPoint current)
    {
        ...
        foreach (int item3 in list)
        {
            ...
            if (
                (point == null || point.parents.Contains(current) || point.parents.Count < 3)
                && (
                    current == StartingMapPoint
                    || current.Children.Count < 3
                    || (point != null && current.Children.Contains(point))
                )
            )
            {
                ...
                return new MapCoord { col = nextColumn, row = row };
            }
        }
        ...
        {
            throw new InvalidOperationException(
                $"Fallback step from ({current.coord.col}, {current.coord.row}) to column {num3} exceeds adjacency"
            );
        }
        return new MapCoord { col = num3, row = row };
    }
    ...
    private void AssignPointTypes()
    {
        ...
        foreach (
            MapPoint item in from p in GetAllMapPoints()
    ...
    private void AssignRemainingTypesToRandomPoints(Queue<MapPointType> pointTypesToBeAssigned)
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            foreach (MapPoint item in list)
            {
                ...
                MapPointType nextValidPointType = GetNextValidPointType(
                    pointTypesToBeAssigned,
                    item
                );
    ...
    private MapPointType GetNextValidPointType(
        Queue<MapPointType> pointTypesQueue,
        MapPoint mapPoint
    )
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (
                _pointTypeCounts.ShouldIgnoreMapPointRulesForMapPointType(mapPointType)
                || IsValidPointType(mapPointType, mapPoint)
            )
            {
                return mapPointType;
            }
    ...
    private static bool IsValidWithParents(MapPointType pointType, MapPoint mapPoint)
    {
        ...
        {
            return !mapPoint
                .parents.Concat(mapPoint.Children)
                .Any((MapPoint p) => pointType == p.PointType);
        }
        return true;
    }

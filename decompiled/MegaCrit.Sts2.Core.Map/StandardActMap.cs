...
public sealed class StandardActMap : ActMap
{
    ...
    private MapCoord GenerateNextCoord(MapPoint current)
    {
        ...
        foreach (int item in list2)
        {
            ...
            {
                return new MapCoord { col = num5, row = row };
            }
        }
    ...
    private void AssignPointTypes()
    {
        ...
        foreach (
            MapPoint item in from x in GetAllMapPoints()
    ...
    private static bool RowsContainPointType(
        MapPointType pointType,
        IEnumerable<List<MapPoint>> rows
    )
    {
        return rows.SelectMany(
                (List<MapPoint> row) => row.Where((MapPoint p) => p.PointType == pointType)
            )
            .Any();
    }
    ...
    )
    {
        ...
        foreach (List<MapPoint> row in rows)
        {
            ...
            using IEnumerator<MapPoint> enumerator2 = row.Where(
                    (MapPoint r) => r.PointType == MapPointType.Unassigned
                )
                .GetEnumerator();

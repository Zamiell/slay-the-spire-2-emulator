...
public class AssetLoadingSession
{
    ...
    public void Process()
    {
        ...
        Log.Debug(
            $"Preloading '{_name}' Process: toLoad={_toLoad.Count} loading={_loading.Count} finalizing={_finalizing.Count} vfx={_vfxScenes.Count}"
        );
        if (
            _toLoad.Count == 0
            && _loading.Count == 0
        ...
        {
            Log.Info(
                $"Preloading '{_name}' Complete: assets={_totalLoaded} time_elapsed={_stopwatch.ElapsedMilliseconds:N0}ms"
            );
            _stopwatch.Stop();
    ...
    private void CheckLoadingStatus()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            ResourceLoader.ThreadLoadStatus threadLoadStatus = ResourceLoader.LoadThreadedGetStatus(
                result
            );
            if ((ulong)threadLoadStatus <= 3uL)
            {
                switch (threadLoadStatus)
                {
                    ...
                    {
                        Log.Warn(
                            $"Threaded load status {threadLoadStatus} for {result}, falling back to sync load"
                        );
                        Resource resource = ResourceLoader.Load<Resource>(
                            result,
                            null,
                            ResourceLoader.CacheMode.Reuse
                        );
    ...
    {
        Log.Info(
            $"LOADING_STATUS: ToLoad={_toLoad.Count} Loading={_loading.Count} Finishing={_finalizing.Count} VfxScenes={_vfxScenes.Count}"
        );
    }

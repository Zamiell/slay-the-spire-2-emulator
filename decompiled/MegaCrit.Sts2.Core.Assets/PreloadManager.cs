...
public static class PreloadManager
{
    ...
    public static async Task LoadMainMenuEssentials()
    {
        ...
        {
            await (
                await LoadAssetSets("MainMenuEssentials", AssetSets.MainMenuEssentials)
            ).WaitForCompletion();
        }
    }
    ...
    public static async Task LoadRunAssets(IEnumerable<CharacterModel> characters)
    {
        if (!TestMode.IsOn)
        {
            ...
            await (
                await LoadAssetSets(
                    "characters=" + string.Join(',', list.Select((CharacterModel c) => c.Id.Entry)),
                    AssetSets.CommonAssets,
                    AssetSets.RunSet
                )
            ).WaitForCompletion();
            GC.Collect();
    ...
    )
    {
        ...
        IEnumerable<CardModel> source2 = allSharedCardPools.SelectMany(
            (CardPoolModel pool) => pool.AllCards
        );
        if (!isMultiplayer)
        {
            source2 = source2.Where(
                (CardModel card) =>
        ...
        return new IEnumerable<string>[8]
        {
            ...
            source.SelectMany(
                (CharacterModel c) =>
                    c.CardPool.AllCards.SelectMany((CardModel card) => card.RunAssetPaths)
            ),

...
public class NTopBarGold : NClickableControl
{
    ...
    private async Task UpdateGoldAnim()
    {
        ...
        try
        {
            ...
            tween.TweenProperty(
                _goldPopupLabel,
                "position:y",
                _goldPopupLabel.Position.Y + 30f,
                0.25
            );
            ...
            while (_additionalGold != 0)
            {
                ...
                _additionalGold = (
                    (_additionalGold > 0) ? (_additionalGold - num) : (_additionalGold + num)
                );
                _goldPopupLabel.SetTextAutoSize(
                    ((_additionalGold >= 0) ? "+" : "") + _additionalGold
                );
                _goldLabel.SetTextAutoSize($"{_player.Gold - _additionalGold}");
                await Task.Delay(
                    (int)Mathf.Lerp(10f, 20f, Mathf.Max(0, 10 - Mathf.Abs(_additionalGold))),
                    ct
                );
            }
    ...
    protected override void OnFocus()
    {
        ...
        NHoverTipSet
            ...
            ?.SetGlobalPosition(base.GlobalPosition + new Vector2(0f, base.Size.Y + 20f));
    }

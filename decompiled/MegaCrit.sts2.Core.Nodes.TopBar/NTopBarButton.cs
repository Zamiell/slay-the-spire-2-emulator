...
public abstract class NTopBarButton : NButton
{
    ...
    protected virtual async Task AnimPressDown(CancellationTokenSource cancelToken)
    {
        if (_icon.IsValid())
        {
            ...
            while (num < 0.25f)
            {
                _icon.Rotation = Mathf.LerpAngle(
                    startAngle,
                    targetAngle,

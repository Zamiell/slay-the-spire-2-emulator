...
public class LocManager
{
    ...
    private static readonly Dictionary<string, string> _weblateToGameLanguage = new Dictionary<
        string,
        string
    >
    {
    ...
    private static readonly Dictionary<string, string> _gameToWeblateLanguage =
        _weblateToGameLanguage.ToDictionary<KeyValuePair<string, string>, string, string>(
            (KeyValuePair<string, string> kvp) => kvp.Value,
            (KeyValuePair<string, string> kvp) => kvp.Key
        );
    ...
    public IReadOnlyList<LocValidationError> ValidationErrors { get; private set; } =
        Array.Empty<LocValidationError>();
    ...
    public LocManager()
    {
        ...
        {
            Log.Warn(
                "Running in .NET globalization-invariant mode. Locale-specific formatting will be disabled."
            );
        }
        ...
        if (string.IsNullOrEmpty(text))
        {
            ...
            {
                Log.Warn(
                    $"Could not initialize language from platform language: {rawLanguage} (resolved: {text}). Defaulting to english"
                );
                text = "eng";
            }
            else
            {
                Log.Info(
                    "Initializing language for the first time from platform locale: "
                        + rawLanguage
                        + " -> "
                        + text
                );
    ...
    private CultureInfo CultureInfoFromThreeLetterCode(string language)
    {
        ...
        if (text == null)
        {
            string text2 =
                "Language code "
                + language
                + " could not be mapped to CultureInfo! Add a new manual mapping";
            Log.Error(text2);
    ...
    {
        using Godot.FileAccess fileAccess = Godot.FileAccess.Open(
            "res://localization/completion.json",
            Godot.FileAccess.ModeFlags.Read
        );
        if (fileAccess == null)
        {
            throw new LocException(
                "Cannot find language completion file: res://localization/completion.json"
            );
        }
        string asText = fileAccess.GetAsText();
        _languageKeyCount = JsonSerializer.Deserialize(
            asText,
            LocManagerSerializerContext.Default.DictionaryStringInt32
        );
    }
    ...
    public string SmartFormat(LocString locString, Dictionary<string, object> variables)
    {
        ...
        CultureInfo provider = (
            table.IsLocalKey(locString.LocEntryKey) ? CultureInfo : _englishCultureInfo
        );
        try
        {
            ...
            when (((ex is FormattingException || ex is ParsingErrors) ? 1 : 0) != 0)
        {
            if (TestMode.IsOn)
            {
                throw;
    ...
    private static string ToString(Dictionary<string, object> variables)
    {
        return "{"
            + string.Join(
                ",",
                variables.Select<KeyValuePair<string, object>, string>(
                    (KeyValuePair<string, object> kp) => $"{kp.Key}:{kp.Value}"
                )
            )
            + "}";
    }
    ...
    public void StartOverridingLanguageAsEnglish()
    {
        if (_engTables == null)
        {
            ...
            {
                (Dictionary<string, LocTable>, bool, List<LocValidationError>) tuple =
                    LoadTablesFromPath("eng", allowOverride: false);
                (_engTables, _, _) = tuple;
                if (tuple.Item2)
                {
                    throw new InvalidOperationException(
                        "Overrides should never be active when overriding as english!"
                    );
                }
            }
        }
    ...
    public void StopOverridingLanguageAsEnglish()
    {
        ...
        {
            Log.Error(
                "StopOverridingLanguageAsEnglish called, but we aren't overriding with english!"
            );
            return;
        }
        SetLanguageInternal(
            _stateBeforeOverridingWithEnglish.language,
            _stateBeforeOverridingWithEnglish.tables,
            _stateBeforeOverridingWithEnglish.overridesActive,
            _stateBeforeOverridingWithEnglish.validationErrors.ToList()
        );
    ...
    private static (
        ...
        bool overridesActive,
        List<LocValidationError> validationErrors
    ) LoadTablesFromPath(string language, bool allowOverride = true)
    {
        ...
        {
            Log.Warn(
                $"Dir path {text} for language {language} does not exist, falling back to eng"
            );
        ...
        foreach (string item2 in enumerable)
        {
            ...
            if (flag)
            {
                dictionary3 = dictionary3.ToDictionary(
                    (KeyValuePair<string, string> kvp) => kvp.Key,
                    (KeyValuePair<string, string> kvp) => ConvertToW(kvp.Value)
                );
            }
            ...
            {
                Log.Info(
                    $"Found loc table from mod: {language} {item2}. Merging with base loc table"
                );
                Dictionary<string, string> dictionary4 = LoadTable(moddedLocTable);
                if (flag)
                {
                    dictionary4 = dictionary4.ToDictionary(
                        (KeyValuePair<string, string> kvp) => kvp.Key,
                        (KeyValuePair<string, string> kvp) => ConvertToW(kvp.Value)
                    );
                }
    ...
    private static Dictionary<string, string> LoadTable(string path)
    {
        ...
        try
        {
            return JsonSerializer.Deserialize(
                asText,
                LocManagerSerializerContext.Default.DictionaryStringString
            );
        }
        catch (Exception e)
        {
    ...
    private static IEnumerable<string> ListLocalizationFiles(string path)
    {
        ...
        return (from s in dirAccess.GetFiles() where s.EndsWith(".json") select s).ToArray();
    }
    ...
    )
    {
        ...
        try
        {
            ...
            {
                if (
                    item.Key.StartsWith("EXTENSION.")
                    || LocValidator.ValidateFormatString(item.Value, out string errorMessage)
                )
                {
                ...
                validationErrors.Add(
                    new LocValidationError(overrideFilePath, item.Key, errorMessage)
                );
    ...
    )
    {
        ...
        global::_003C_003Ey__InlineArray5<string> buffer =
            default(global::_003C_003Ey__InlineArray5<string>);
        global::_003CPrivateImplementationDetails_003E.InlineArrayElementRef<
            global::_003C_003Ey__InlineArray5<string>,
            string
        >(ref buffer, 0) = globalizedOverrideDir;
        global::_003CPrivateImplementationDetails_003E.InlineArrayElementRef<
            global::_003C_003Ey__InlineArray5<string>,
            string
        >(ref buffer, 1) = "slaythespire2";
        global::_003CPrivateImplementationDetails_003E.InlineArrayElementRef<
            global::_003C_003Ey__InlineArray5<string>,
            string
        >(ref buffer, 2) = fileNameWithoutExtension;
        global::_003CPrivateImplementationDetails_003E.InlineArrayElementRef<
            global::_003C_003Ey__InlineArray5<string>,
            string
        >(ref buffer, 3) = value;

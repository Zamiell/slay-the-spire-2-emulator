...
internal class LocManagerSerializerContext : JsonSerializerContext, IJsonTypeInfoResolver
{
    ...
    public JsonTypeInfo<Dictionary<string, int>> DictionaryStringInt32 =>
        _DictionaryStringInt32
        ?? (
            _DictionaryStringInt32 =
                (JsonTypeInfo<Dictionary<string, int>>)
                    base.Options.GetTypeInfo(typeof(Dictionary<string, int>))
        );
    ...
    public static LocManagerSerializerContext Default { get; } =
        new LocManagerSerializerContext(new JsonSerializerOptions(s_defaultOptions));

    protected override JsonSerializerOptions? GeneratedSerializerOptions { get; } =
        s_defaultOptions;

    private JsonTypeInfo<Dictionary<string, int>> Create_DictionaryStringInt32(
    ...
    )
    {
        if (
            !TryGetTypeInfoForRuntimeCustomConverter(
                options,
                out JsonTypeInfo<Dictionary<string, int>> jsonTypeInfo
            )
        )
        {
            JsonCollectionInfoValues<Dictionary<string, int>> collectionInfo =
                new JsonCollectionInfoValues<Dictionary<string, int>>
                {

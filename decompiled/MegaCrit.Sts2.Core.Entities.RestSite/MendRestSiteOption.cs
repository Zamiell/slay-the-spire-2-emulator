...
public sealed class MendRestSiteOption : RestSiteOption
{
    ...
    {
        return Hook.ModifyRestSiteHealAmount(
            player.RunState,
            player.Creature,
            HealRestSiteOption.GetBaseHealAmount(player.Creature)
        );
    }
    ...
    public override async Task<bool> OnSelect()
    {
        ...
        if (LocalContext.IsMe(base.Owner))
        {
            ...
            if (usingController)
            {
                List<NRestSiteCharacter> list = NRestSiteRoom
                    .Instance.characterAnims.Where((NRestSiteCharacter c) => c.Player != base.Owner)
                    .ToList();
                for (int num = 0; num < list.Count; num++)
                {
                    ...
                    list[num].Hitbox.FocusNeighborRight = (
                        (num < list.Count - 1)
                            ? list[num + 1].Hitbox.GetPath()
                            : list[0].Hitbox.GetPath()
                    );
                }
            ...
            finally
            {
                ...
                {
                    foreach (
                        NRestSiteCharacter characterAnim in NRestSiteRoom.Instance.characterAnims
                    )
                    {
    ...
    private void OnNodeHovered(Node node)
    {
        ...
        if (player != null)
        {
            ...
            Description.Add(
                "Name",
                PlatformUtil.GetPlayerName(RunManager.Instance.NetService.Platform, player.NetId)
            );

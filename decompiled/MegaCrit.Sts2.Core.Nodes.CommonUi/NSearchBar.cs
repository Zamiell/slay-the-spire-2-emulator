...
public class NSearchBar : Control
{
    ...
    public event QueryChangedEventHandler QueryChanged
    {
        add
        {
            backing_QueryChanged = (QueryChangedEventHandler)
                Delegate.Combine(backing_QueryChanged, value);
        }
        remove
        {
            backing_QueryChanged = (QueryChangedEventHandler)
                Delegate.Remove(backing_QueryChanged, value);
        }
    }
    ...
    public event QuerySubmittedEventHandler QuerySubmitted
    {
        add
        {
            backing_QuerySubmitted = (QuerySubmittedEventHandler)
                Delegate.Combine(backing_QuerySubmitted, value);
        }
        remove
        {
            backing_QuerySubmitted = (QuerySubmittedEventHandler)
                Delegate.Remove(backing_QuerySubmitted, value);
        }
    }
    ...
    public override void _Ready()
    {
        ...
        _clearButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From((Action<NButton>)ClearText)
        );
    }
    ...
    )
    {
        ...
        if (method == MethodName.Normalize && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<string>(
                Normalize(VariantUtils.ConvertTo<string>(in args[0]))
            );
            return true;
        }
        ...
        {
            ret = VariantUtils.CreateFrom<string>(
                RemoveHtmlTags(VariantUtils.ConvertTo<string>(in args[0]))
            );
            return true;
        }
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<QueryChangedEventHandler>(
        ...
        if (
            info.TryGetSignalEventDelegate<QuerySubmittedEventHandler>(
                SignalName.QuerySubmitted,
                out var value4
            )
        )
        {

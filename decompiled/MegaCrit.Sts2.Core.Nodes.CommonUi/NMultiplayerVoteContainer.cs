...
public class NMultiplayerVoteContainer : Control
{
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlySingleElementList<string>(
            SceneHelper.GetScenePath("ui/multiplayer_vote_icon")
        );
        ...
        foreach (Player player in _allPlayers)
        {
            if (_playerVotedDelegate(player))
            {
                ...
                if (num < 0)
                {
                    ...
                    voteIcon2.node.GetNode<TextureRect>("Outline").Texture = player
                        .Character
                        .IconOutlineTexture;
    ...
    public void BouncePlayers()
    {
        foreach (VoteIcon vote in _votes)
        {
            ...
            vote.tween.TweenProperty(vote.node, "scale", Vector2.One * 1.3f, 0.15)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Quad);
            vote.tween.TweenProperty(vote.node, "position:y", -10f, 0.15)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Quad);
            vote.tween.Chain()
                .TweenProperty(vote.node, "scale", Vector2.One, 0.2)
                .SetEase(Tween.EaseType.In)
                .SetTrans(Tween.TransitionType.Quad);
            vote.tween.TweenProperty(vote.node, "position:y", 0f, 0.3)
                .SetEase(Tween.EaseType.In)
                .SetTrans(Tween.TransitionType.Quad);
        }
    }
    ...
    private void AnimVoteOut(VoteIcon vote, bool animate)
    {
        ...
        if (animate)
        {
            vote.tween?.Kill();
            vote.tween = CreateTween().SetParallel();
            vote.tween.TweenProperty(vote.node, "modulate:a", 0f, 0.1)
                .SetDelay(0.15000000596046448);
            vote.tween.TweenProperty(vote.node, "position:y", 20f, 0.25)

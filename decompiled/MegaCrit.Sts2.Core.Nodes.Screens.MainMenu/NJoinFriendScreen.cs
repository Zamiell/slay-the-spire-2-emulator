...
public class NJoinFriendScreen : NSubmenu
{
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlyArray<string>(
            new string[2] { _scenePath, NJoinFriendButton.scenePath }
        );
    ...
    public static NJoinFriendScreen? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NJoinFriendScreen>(PackedScene.GenEditState.Disabled);
    }
    ...
    public override void _Ready()
    {
        ...
        GetNode<MegaLabel>("TitleLabel")
            .SetTextAutoSize(
                new LocString("main_menu_ui", "JOIN_FRIENDS_MENU.title").GetFormattedText()
            );
        _noFriendsLabel.SetTextAutoSize(
            new LocString("main_menu_ui", "JOIN_FRIENDS_MENU.noFriends").GetFormattedText()
        );
        _refreshButton.Connect(
    ...
    private async Task ShowFriends()
    {
        ...
        if (SteamInitializer.Initialized)
        {
            IEnumerable<ulong> enumerable = await PlatformUtil.GetFriendsWithOpenLobbies(
                PlatformType.Steam
            );
            ...
            foreach (ulong item in enumerable)
            {
                ...
                SteamClientConnectionInitializer connInitializer =
                    SteamClientConnectionInitializer.FromPlayer(item);
                nJoinFriendButton.Connect(
                    NClickableControl.SignalName.Released,
    ...
    public async Task JoinGameAsync(IClientConnectionInitializer connInitializer)
    {
        ...
        {
            Log.Warn(
                $"Tried to join game with connection {connInitializer} while we were already joining a game! Ignoring this attempt"
            );
            return;
        }
        ...
        try
        {
            ...
            if (joinResult.sessionState == RunSessionState.InLobby)
            {
                ...
                {
                    NCharacterSelectScreen submenuType =
                        _stack.GetSubmenuType<NCharacterSelectScreen>();
                    submenuType.InitializeMultiplayerAsClient(
                        _currentJoinFlow.NetService,
                        joinResult.joinResponse.Value
                    );
                ...
                if (joinResult.gameMode == GameMode.Daily)
                {
                    ...
                    submenuType2.InitializeMultiplayerAsClient(
                        _currentJoinFlow.NetService,
                        joinResult.joinResponse.Value
                    );
                ...
                submenuType3.InitializeMultiplayerAsClient(
                    _currentJoinFlow.NetService,
                    joinResult.joinResponse.Value
                );
            ...
            else if (joinResult.sessionState == RunSessionState.InLoadedLobby)
            {
                if (joinResult.gameMode == GameMode.Standard)
                {
                    NMultiplayerLoadGameScreen submenuType4 =
                        _stack.GetSubmenuType<NMultiplayerLoadGameScreen>();
                    submenuType4.InitializeAsClient(
                        _currentJoinFlow.NetService,
                        joinResult.loadJoinResponse.Value
                    );
                ...
                else if (joinResult.gameMode == GameMode.Daily)
                {
                    ...
                    submenuType5.InitializeAsClient(
                        _currentJoinFlow.NetService,
                        joinResult.loadJoinResponse.Value
                    );
                ...
                {
                    NCustomRunLoadScreen submenuType6 =
                        _stack.GetSubmenuType<NCustomRunLoadScreen>();
                    submenuType6.InitializeAsClient(
                        _currentJoinFlow.NetService,
                        joinResult.loadJoinResponse.Value
                    );
            ...
            else if (joinResult.sessionState == RunSessionState.Running)
            {
                NErrorPopup nErrorPopup = NErrorPopup.Create(
                    new NetErrorInfo(NetError.RunInProgress, selfInitiated: false)
                );
                if (nErrorPopup != null)
                {
        ...
        {
            Log.Error(
                $"Received unexpected exception {ex3.GetType()} while joining game! Disconnecting with InternalError"
            );
            NErrorPopup nErrorPopup3 = NErrorPopup.Create(
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.RefreshButtonClicked,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._loadingFriendsIndicator,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName.DebugFriendsButtons,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._loadingFriendsIndicator,
            Variant.From(in _loadingFriendsIndicator)
        );

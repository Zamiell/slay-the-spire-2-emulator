...
public abstract class NSubmenuStack : Control
{
    ...
    public event StackModifiedEventHandler StackModified
    {
        add
        {
            backing_StackModified = (StackModifiedEventHandler)
                Delegate.Combine(backing_StackModified, value);
        }
        remove
        {
            backing_StackModified = (StackModifiedEventHandler)
                Delegate.Remove(backing_StackModified, value);
        }
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.InitializeForMainMenu,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<StackModifiedEventHandler>(
                SignalName.StackModified,
                out var value2
            )
        )
        {

...
public class NShortSubmenuButton : NButton
{
    ...
    {
        string imagePath = ImageHelper.GetImagePath(
            "packed/main_menu/submenu_icon_" + key.ToLowerInvariant() + ".png"
        );
    ...
    public override void _Ready()
    {
        if (GetType() != typeof(NShortSubmenuButton))
        {
            throw new InvalidOperationException(
                "Don't call base._Ready(). Use ConnectSignals() instead."
            );
        }
        ConnectSignals();
    }
    ...
    private void RefreshLabels()
    {
        ...
        else
        {
            ...
            {
                Log.Warn(
                    $"Short submenu button {base.Name} tried to find locked description for {_locKeyPrefix} but couldn't"
                );
                locString2 = new LocString("main_menu_ui", _locKeyPrefix + ".description");
            }
        }
        _description.Text = locString2.GetFormattedText();
        _description.ApplyLocaleFontSubstitution(
            FontType.Regular,
    ...
    protected override void OnUnfocus()
    {
        ...
        _tween = CreateTween().SetParallel();
        _tween
            .TweenProperty(this, "scale", Vector2.One, 0.3)
    ...
    protected override void OnPress()
    {
        ...
        _tween = CreateTween().SetParallel();
        _tween
            .TweenProperty(this, "scale", Vector2.One * 0.98f, 0.2)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetIconAndLocalization,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        if (method == MethodName.GetImagePath && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<string>(
                GetImagePath(VariantUtils.ConvertTo<string>(in args[0]))
            );
            return true;
        }

...
public class NContinueRunInfo : Control
{
    ...
    public void AnimShow()
    {
        ...
        _visTween
            .TweenProperty(
                this,
                "position",
                _initPosition + new Vector2(0f, -20f),
                0.20000000298023224
            )
    ...
    private void ShowInfo(SerializableRun save)
    {
        ...
        DateTime dateTime = TimeZoneInfo.ConvertTimeFromUtc(
            DateTimeOffset.FromUnixTimeSeconds(save.SaveTime).UtcDateTime,
            TimeZoneInfo.Local
        );
        string rawText = new LocString(
            "main_menu_ui",
            "CONTINUE_RUN_INFO.savedTimeFormat"
        ).GetRawText();
        ...
        string formattedText2 = new LocString(
            "main_menu_ui",
            "CONTINUE_RUN_INFO.floor"
        ).GetFormattedText();
        int num = save.VisitedMapCoords.Count;
        for (int i = 0; i < save.CurrentActIndex; i++)
        {
            num += ModelDb
                .GetById<ActModel>(save.Acts[i].Id)
                .GetNumberOfFloors(save.Players.Count > 1);
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._runInfoContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

...
public class NullPlatformUtilStrategy : IPlatformUtilStrategy
{
    ...
    {
        if (
            CommandLineHelper.TryGetValue("clientId", out string value)
        ...
        if (godotFileIo.FileExists("mp_names.json"))
        {
            ...
            _mpNames = JsonSerializer.Deserialize(
                json,
                JsonSerializationUtility.GetTypeInfo<List<NullMultiplayerName>>()
            );
        }
    }
    ...
    public string? GetThreeLetterLanguageCode()
    {
        ...
        Log.Error(
            $"CultureInfo {cultureInfo} could not be mapped to a three-letter language code!"
        );
        return null;
    }

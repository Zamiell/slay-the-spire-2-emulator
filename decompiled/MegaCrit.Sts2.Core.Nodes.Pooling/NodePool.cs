...
{
    private static System.Collections.Generic.Dictionary<Type, INodePool> _pools =
        new System.Collections.Generic.Dictionary<Type, INodePool>();

    public static NodePool<T> Init<T>(string scenePath, int prewarmCount)
        where T : Node, IPoolable
    {
        ...
        if (_pools.TryGetValue(typeFromHandle, out INodePool _))
        {
            throw new InvalidOperationException(
                $"Tried to init NodePool for type {typeof(T)} but it's already initialized!"
            );
        }
    ...
    public static IPoolable Get(Type type)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get pool for type {type} before it was initialized!"
            );
        }
        return value.Get();
    }
...
public class NodePool<T> : INodePool
    where T : Node, IPoolable
{
    ...
    public void Free(T obj)
    {
        if (!_usedObjects.Contains(obj))
        {
            ...
            {
                Log.Error(
                    $"Tried to free object {obj} ({obj.GetType()}) back to pool {typeof(NodePool<T>)} but it's already been freed!"
                );
            }
            else
            {
                Log.Error(
                    $"Tried to free object {obj} ({obj.GetType()}) back to pool {typeof(NodePool<T>)} but it's not part of the pool!"
                );
    ...
    private void DisconnectSignal(Callable callable, Signal signal)
    {
        ...
        if (
            (target == null && callable.Method == null)

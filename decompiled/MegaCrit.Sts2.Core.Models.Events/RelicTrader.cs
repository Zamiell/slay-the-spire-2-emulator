...
public sealed class RelicTrader : EventModel
{
    ...
    private IReadOnlyList<RelicModel> OwnedRelics
    {
        get
        {
            ...
            {
                _ownedRelics = GetValidRelics(base.Owner)
                    .ToList()
                    .StableShuffle(base.Rng)
                    .Take(3)
                    .ToList();
            }
            return _ownedRelics;
    ...
    public override void CalculateVars()
    {
        ...
        {
            ((StringVar)base.DynamicVars["TopRelicOwned"]).StringValue = OwnedRelics[0]
                .Title.GetFormattedText();
            ((StringVar)base.DynamicVars["TopRelicNew"]).StringValue = NewRelics[0]
                .Title.GetFormattedText();
        }
        if (OwnedRelics.Count > 1 && NewRelics.Count > 1)
        {
            ((StringVar)base.DynamicVars["MiddleRelicOwned"]).StringValue = OwnedRelics[1]
                .Title.GetFormattedText();
            ((StringVar)base.DynamicVars["MiddleRelicNew"]).StringValue = NewRelics[1]
                .Title.GetFormattedText();
        }
        if (OwnedRelics.Count > 2 && NewRelics.Count > 2)
        {
            ((StringVar)base.DynamicVars["BottomRelicOwned"]).StringValue = OwnedRelics[2]
                .Title.GetFormattedText();
            ((StringVar)base.DynamicVars["BottomRelicNew"]).StringValue = NewRelics[2]
                .Title.GetFormattedText();
        }
    }

...
public sealed class TheArchitect : EventModel
{
    ...
    private static readonly LocString _emptyLocString = new LocString(
        "ancients",
        "PROCEED.description"
    );

    private static readonly LocString _continueLocString = new LocString(
        "ancients",
        "THE_ARCHITECT.CONTINUE"
    );

    private static readonly LocString _respondLocString = new LocString(
        "ancients",
        "THE_ARCHITECT.RESPOND"
    );
    ...
    protected override IReadOnlyList<EventOption> GenerateInitialOptions()
    {
        ...
        {
            return new global::_003C_003Ez__ReadOnlySingleElementList<EventOption>(
                CreateProceedOption()
            );
        }
        CurrentLineIndex = 0;
        return new global::_003C_003Ez__ReadOnlySingleElementList<EventOption>(
            CreateOptionForCurrentLine()
        );
    }
    ...
    public override void OnRoomEnter()
    {
        ...
        ArchitectCreature = NCombatRoom
            .Instance?.CreatureNodes.FirstOrDefault(
                (NCreature n) => n.Entity.Side == CombatSide.Enemy
            )
            ?.Entity;
    ...
    private static string CharKey<T>()
    ...
    private static AncientDialogueSet DefineDialogues()
    {
        ...
        ancientDialogueSet.CharacterDialogues = new Dictionary<
            string,
            IReadOnlyList<AncientDialogue>
        >
        {
            [CharKey<Ironclad>()] = new global::_003C_003Ez__ReadOnlyArray<AncientDialogue>(
                new AncientDialogue[3]
                {
                    new AncientDialogue("", "")
                    {
                        VisitIndex = 0,
            ...
            [CharKey<Silent>()] = new global::_003C_003Ez__ReadOnlyArray<AncientDialogue>(
                new AncientDialogue[4]
                {
                    new AncientDialogue("")
                    {
                        VisitIndex = 0,
            ...
            [CharKey<Defect>()] = new global::_003C_003Ez__ReadOnlyArray<AncientDialogue>(
                new AncientDialogue[3]
                {
                    new AncientDialogue("", "", "")
                    {
            ...
            [CharKey<Necrobinder>()] = new global::_003C_003Ez__ReadOnlyArray<AncientDialogue>(
                new AncientDialogue[4]
                {
                    new AncientDialogue("", "")
                    {
            ...
            [CharKey<Regent>()] = new global::_003C_003Ez__ReadOnlyArray<AncientDialogue>(
                new AncientDialogue[3]
                {
                    new AncientDialogue("", "", "")
                    {
    ...
    {
        int charVisits =
            SaveManager.Instance.Progress.GetStatsForCharacter(base.Owner.Character.Id)?.TotalWins
            ?? 0;
        int wins = SaveManager.Instance.Progress.Wins;
        List<AncientDialogue> items = DialogueSet
            .GetValidDialogues(
                base.Owner.Character.Id,
                charVisits,
                wins,
    ...
    private EventOption CreateOptionForCurrentLine()
    {
        ...
        return new EventOption(
            title: (ancientDialogueLine.NextButtonText != null)
                ? ancientDialogueLine.NextButtonText
                : (
                    (ancientDialogueLine.Speaker != AncientDialogueSpeaker.Ancient)
                        ? _continueLocString
                        : _respondLocString
                ),
            eventModel: this,
            onChosen: AdvanceDialogue,
            description: _emptyLocString,
            textKey: $"{base.Id.Entry}.dialogue.{CurrentLineIndex}",
    ...
    private async Task AdvanceDialogue()
    {
        ...
        if (CurrentLineIndex < Dialogue.Lines.Count)
        {
            ...
            eventOptions = new global::_003C_003Ez__ReadOnlySingleElementList<EventOption>(
                CreateOptionForCurrentLine()
            );
        }
        else
    ...
    {
        if (
            (attackers != ArchitectAttackers.Player && attackers != ArchitectAttackers.Both)
            || 1 == 0
        )
        {
            return false;
        ...
        for (int i = 0; i < shuffledVfx.Count; i++)
        {
            ...
            vfxContainer?.AddChildSafely(
                NDamageNumVfx.Create(ArchitectCreature, damageParts[i], requireInteractable: false)
            );
            vfxContainer?.AddChildSafely(
                NHitSparkVfx.Create(ArchitectCreature, requireInteractable: false)
            );
    ...
    private async Task AnimArchitectAttackIfNecessary(ArchitectAttackers attackers)
    {
        ...
        if (flag && ArchitectCreature != null)
        {
            ...
            NCombatRoom.Instance?.CombatVfxContainer.AddChildSafely(
                NFireBurstVfx.Create(base.Owner.Creature, 1f)
            );
            await Cmd.Wait(0.5f);
        }
    }
    ...
    private async Task PlayCurrentLine()
    {
        ...
        {
            if (
                Dialogue.StartAttackers != ArchitectAttackers.None
                || Dialogue.EndAttackers != ArchitectAttackers.None
            )
            {
    ...
    {
        SpeechBubble = TalkCmd.Play(
            line.LineText,
            speaker,
            (line.Speaker == AncientDialogueSpeaker.Ancient)
                ? VfxColor.DarkGray
                : base.Owner.Character.SpeechBubbleColor,
            VfxDuration.Forever
        );
    }
    ...
    {
        return NCombatRoom
            .Instance?.GetCreatureNode(ArchitectCreature)
            ?.SpineAnimation.GetAnimationState();
    }
}

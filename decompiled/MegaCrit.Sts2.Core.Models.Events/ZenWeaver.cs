...
public sealed class ZenWeaver : EventModel
{
    ...
    public override bool IsAllowed(IRunState runState)
    {
        return runState.Players.All(
            (Player p) => (decimal)p.Gold >= base.DynamicVars["EmotionalAwarenessCost"].BaseValue
        );
    }
    ...
    protected override IReadOnlyList<EventOption> GenerateInitialOptions()
    {
        ...
        EventOption eventOption2 = (
            (gold < base.DynamicVars["EmotionalAwarenessCost"].IntValue)
                ? CreateLockedOption()
                : new EventOption(
                    this,
                    EmotionalAwareness,
        ...
        EventOption eventOption3 = (
            (gold < base.DynamicVars["ArachnidAcupunctureCost"].IntValue)
                ? CreateLockedOption()
                : new EventOption(
                    this,
                    ArachnidAcupuncture,
    ...
    private async Task BreathingTechniques()
    {
        await PlayerCmd.LoseGold(
            base.DynamicVars["BreathingTechniquesCost"].IntValue,
            base.Owner,
            GoldLossType.Spent
        );

...
public sealed class RanwidTheElder : EventModel
{
    ...
    private LocString PotionChoiceDescription =>
        new LocString("events", "RANWID_THE_ELDER.pages.INITIAL.options.POTION.description");

    private LocString RelicChoiceTitle =>
        new LocString("events", "RANWID_THE_ELDER.pages.INITIAL.options.RELIC.title");

    private LocString RelicChoiceDescription =>
        new LocString("events", "RANWID_THE_ELDER.pages.INITIAL.options.RELIC.description");
    ...
    protected override IReadOnlyList<EventOption> GenerateInitialOptions()
    {
        ...
        {
            list.Add(
                new EventOption(this, null, "RANWID_THE_ELDER.pages.INITIAL.options.POTION_LOCKED")
            );
        }
        list.Add(new EventOption(this, GiveGold, "RANWID_THE_ELDER.pages.INITIAL.options.GOLD"));
        RelicModel relic = base.Rng.NextItem(
            base.Owner.Relics.Where((RelicModel r) => r.IsTradable)
        );
        if (relic != null)
        {
        ...
        {
            list.Add(
                new EventOption(this, null, "RANWID_THE_ELDER.pages.INITIAL.options.RELIC_LOCKED")
            );
        }
        return list;
    }

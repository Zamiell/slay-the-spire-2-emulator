...
public sealed class EndlessConveyor : EventModel
{
    ...
    {
        RollDish();
        ((StringVar)base.DynamicVars["CurrentDishTitle"]).StringValue =
            CurrentDish.title.GetFormattedText();
    }
    ...
    private async Task GrabSomethingOffTheBelt()
    {
        if (_currentDish.id != "GOLDEN_FYSH")
        {
            await PlayerCmd.LoseGold(
                base.DynamicVars.Gold.IntValue,
                base.Owner,
                GoldLossType.Spent
            );
        }
        ...
        RollDish();
        ((StringVar)base.DynamicVars["LastDishTitle"]).StringValue = (
            (StringVar)base.DynamicVars["CurrentDishTitle"]
        ).StringValue;
        ((StringVar)base.DynamicVars["CurrentDishTitle"]).StringValue =
            CurrentDish.title.GetFormattedText();
        SetEventState(
            L10NLookup("ENDLESS_CONVEYOR.pages.GRAB_SOMETHING_OFF_THE_BELT.description"),
    ...
    private EventOption GenerateGrabSomethingOffTheBeltOption()
    {
        if (base.Owner.Gold >= base.DynamicVars.Gold.IntValue)
        {
            return new EventOption(
                this,
                GrabSomethingOffTheBelt,
                _currentDish.optionKey,
                _currentDish.hoverTips
            );
        }
    ...
    {
        IEnumerable<PotionModel> items = base
            .Owner.Character.PotionPool.GetUnlockedPotions(base.Owner.UnlockState)
            .Concat(
                ModelDb.PotionPool<SharedPotionPool>().GetUnlockedPotions(base.Owner.UnlockState)
            );
        ...
        if (potionModel != null)
        {
            await RewardsCmd.OfferCustom(
                base.Owner,
                new List<Reward>(1) { new PotionReward(potionModel.ToMutable(), base.Owner) }
            );
        }
    }
    ...
    private void RollDish()
    {
        ...
        if (NumOfGrabs % 5 == 0)
        {
            ...
            _currentDish = new Dish(
                "SEAPUNK_SALAD",
                SeapunkSalad,
                HoverTipFactory.FromCardWithCardHoverTips<FeedingFrenzy>(),
                0f
            );
            return;
        }
        ...
        {
            list2.Add(
                new Dish("SUSPICIOUS_CONDIMENT", SuspiciousCondiment, Array.Empty<IHoverTip>(), 3f)
            );
        }
    ...
    {
        IEnumerable<CardModel> enumerable = base.Owner.Deck.Cards.Where(

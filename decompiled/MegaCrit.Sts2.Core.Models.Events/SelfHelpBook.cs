...
{
    private const string _readTheBackDescriptionKey =
        "SELF_HELP_BOOK.pages.READ_THE_BACK.description";

    private const string _readPassageDescriptionKey =
        "SELF_HELP_BOOK.pages.READ_PASSAGE.description";

    private const string _readEntireBookDescriptionKey =
        "SELF_HELP_BOOK.pages.READ_ENTIRE_BOOK.description";
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[6]
            {
                new StringVar(
                    "Enchantment1",
                    ModelDb.Enchantment<Sharp>().Title.GetFormattedText()
                ),
                new StringVar(
                    "Enchantment2",
                    ModelDb.Enchantment<Nimble>().Title.GetFormattedText()
                ),
                new StringVar(
                    "Enchantment3",
                    ModelDb.Enchantment<Swift>().Title.GetFormattedText()
                ),
    ...
    protected override IReadOnlyList<EventOption> GenerateInitialOptions()
    {
        ...
        {
            list.Add(
                new EventOption(this, SkipBook, "SELF_HELP_BOOK.pages.INITIAL.options.NO_OPTIONS")
            );
        }
        return list;
    }
    ...
    {
        await SelectAndEnchant<Sharp>(
            2,
            CardType.Attack,
            L10NLookup("SELF_HELP_BOOK.pages.READ_THE_BACK.description")
        );
    }

    private async Task ReadPassage()
    {
        await SelectAndEnchant<Nimble>(
            2,
            CardType.Skill,
            L10NLookup("SELF_HELP_BOOK.pages.READ_PASSAGE.description")
        );
    }

    private async Task ReadEntireBook()
    {
        await SelectAndEnchant<Swift>(
            2,
            CardType.Power,
            L10NLookup("SELF_HELP_BOOK.pages.READ_ENTIRE_BOOK.description")
        );
    }
        ...
        where T : EnchantmentModel
    {
        ...
        return PileType
                .Deck.GetPile(player)
                .Cards.FirstOrDefault((CardModel c) => DeckFilter(c, enchantment, typeRestriction))
            != null;
    }
        ...
        where T : EnchantmentModel
    {
        ...
        CardModel cardModel = (
            await CardSelectCmd.FromDeckForEnchantment(
                ...
                enchantment,
                amount,
                (CardModel? c) => c.Type == typeRestriction,
                prefs
            )
        ).FirstOrDefault();
        if (cardModel != null)

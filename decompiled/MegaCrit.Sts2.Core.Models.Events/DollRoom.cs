...
public sealed class DollRoom : EventModel
{
    ...
    protected override IReadOnlyList<EventOption> GenerateInitialOptions()
    {
        return new global::_003C_003Ez__ReadOnlyArray<EventOption>(
            new EventOption[3]
            {
                ...
                ).ThatDoesDamage(base.DynamicVars["TakeTimeHpLoss"].BaseValue),
                new EventOption(
                    this,
                    Examine,
                    "DOLL_ROOM.pages.INITIAL.options.EXAMINE"
                ).ThatDoesDamage(base.DynamicVars["ExamineHpLoss"].BaseValue),
            }
        );
    }
    ...
    private async Task TakeSomeTime()
    {
        await CreatureCmd.Damage(
            ...
            base.Owner.Creature,
            (DamageVar)base.DynamicVars["TakeTimeHpLoss"],
            null,
            null
        );
    ...
    private async Task Examine()
    {
        await CreatureCmd.Damage(
            new ThrowingPlayerChoiceContext(),
            base.Owner.Creature,
            (DamageVar)base.DynamicVars["ExamineHpLoss"],
            null,
            null
        );

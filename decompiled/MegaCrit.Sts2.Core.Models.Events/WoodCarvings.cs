...
public sealed class WoodCarvings : EventModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[3]
            {
                ...
                new StringVar(
                    "SnakeEnchantment",
                    ModelDb.Enchantment<Slither>().Title.GetFormattedText()
                ),
        ...
        return runState.Players.All(
            (Player p) =>
                CardPile
                    .Get(PileType.Deck, p)
                    .Cards.Any(
                        (CardModel c) => c != null && c.Rarity == CardRarity.Basic && c.IsRemovable
                    )
        );
    }
    ...
    protected override IReadOnlyList<EventOption> GenerateInitialOptions()
    {
        ...
        EventOption eventOption = (
            (!cards.Any((CardModel c) => ModelDb.Enchantment<Slither>().CanEnchant(c)))
                ? new EventOption(this, null, "WOOD_CARVINGS.pages.INITIAL.options.SNAKE_LOCKED")
                : new EventOption(
                    this,
                    Snake,

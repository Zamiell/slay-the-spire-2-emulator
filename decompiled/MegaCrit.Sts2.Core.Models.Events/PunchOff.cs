...
public sealed class PunchOff : EventModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlySingleElementList<DynamicVar>(new GoldVar(0));
    ...
    private async Task PunchEachOther()
    {
        ...
        while (!_punchCts.IsCancellationRequested)
        {
            ...
            vfxContainer?.AddChildSafely(
                NHitSparkVfx.Create(rightEnemy, requireInteractable: false)
            );
            ...
            vfxContainer?.AddChildSafely(
                NHitSparkVfx.Create(leftEnemy, requireInteractable: false)
            );
    ...
    private Task TakeThem()
    {
        _punchCts?.Cancel();
        SetEventState(
            L10NLookup("PUNCH_OFF.pages.I_CAN_TAKE_THEM.description"),
            new global::_003C_003Ez__ReadOnlySingleElementList<EventOption>(
                new EventOption(this, Fight, "PUNCH_OFF.pages.I_CAN_TAKE_THEM.options.FIGHT")
            )
        );
        return Task.CompletedTask;
    }
    ...
    private Task Fight()
    {
        ...
        EnterCombatWithoutExitingEvent<PunchOffEventEncounter>(
            list,

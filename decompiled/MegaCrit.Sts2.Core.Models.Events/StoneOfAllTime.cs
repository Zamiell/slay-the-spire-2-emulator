...
public sealed class StoneOfAllTime : EventModel
{
    ...
    {
        get { return _drinkAndLiftPotion; }
        set
        {
            AssertMutable();
    ...
    protected override IReadOnlyList<EventOption> GenerateInitialOptions()
    {
        ...
        EventOption eventOption2 = (
            (
                CardPile
                    .Get(PileType.Deck, base.Owner)
                    .Cards.Count((CardModel c) => ModelDb.Enchantment<Vigorous>().CanEnchant(c)) < 1
            )
                ? new EventOption(this, null, "STONE_OF_ALL_TIME.pages.INITIAL.options.PUSH_LOCKED")
                : new EventOption(
                    this,
                    Push,
                    "STONE_OF_ALL_TIME.pages.INITIAL.options.PUSH",
                    HoverTipFactory.FromEnchantment<Vigorous>(
                        base.DynamicVars["PushVigorousAmount"].IntValue
                    )
                ).ThatDoesDamage(base.DynamicVars["PushHpLoss"].BaseValue)
        );
        return new global::_003C_003Ez__ReadOnlyArray<EventOption>(
            new EventOption[2] { eventOption, eventOption2 }
        );
    }
    ...
    private async Task Lift()
    {
        ...
        await CreatureCmd.GainMaxHp(
            base.Owner.Creature,
            base.DynamicVars["DrinkMaxHpGain"].BaseValue
        );
    ...
    private async Task Push()
    {
        await CreatureCmd.Damage(
            ...
            base.Owner.Creature,
            base.DynamicVars["PushHpLoss"].BaseValue,
        ...
        )
        {
            CardCmd.Enchant(
                vigorous.ToMutable(),
                item,
                base.DynamicVars["PushVigorousAmount"].BaseValue
            );

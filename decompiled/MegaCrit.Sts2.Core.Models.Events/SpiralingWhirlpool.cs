...
public sealed class SpiralingWhirlpool : EventModel
{
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlySingleElementList<DynamicVar>(new HealVar(0m));

    public override void CalculateVars()
    {
        base.DynamicVars.Heal.BaseValue = (
            (base.Owner != null) ? ((decimal)base.Owner.Creature.MaxHp * 0.33m) : 0m
        );
    }
    ...
    public override bool IsAllowed(IRunState runState)
    {
        return runState.Players.All(
            (Player p) => p.Deck.Cards.Any(ModelDb.Enchantment<Spiral>().CanEnchant)
        );
    }

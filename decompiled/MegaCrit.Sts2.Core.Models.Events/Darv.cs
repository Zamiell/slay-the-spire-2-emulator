...
public class Darv : AncientEventModel
{
    ...
    public override IEnumerable<EventOption> AllPossibleOptions =>
        (
            from r in _validRelicSets.SelectMany((ValidRelicSet s) => s.relics)
            select RelicOption(r.ToMutable())
        ).Concat(
            new global::_003C_003Ez__ReadOnlySingleElementList<EventOption>(
                RelicOption<DustyTome>()
            )
        );
            ...
            [AncientEventModel.CharKey<Silent>()] =
                new global::_003C_003Ez__ReadOnlyArray<AncientDialogue>(
                    new AncientDialogue[3]
                    {
                        ...
                        new AncientDialogue("event:/sfx/npcs/darv/darv_excited") { VisitIndex = 1 },
                        new AncientDialogue(
    ...
    {
        List<EventOption> source = (
            from rs in _validRelicSets
            where rs.filter(base.Owner)
            select RelicOption(base.Rng.NextItem(rs.relics).ToMutable())
        )
            .ToList()
            .UnstableShuffle(base.Rng);
    ...
    static Darv()
    {
        ...
        num2++;
        span[num2] = new ValidRelicSet(
            (Player owner) =>
                !owner.RunState.Modifiers.Any((ModifierModel m) => m.ClearsPlayerDeck),
            new RelicModel[1] { ModelDb.Relic<PandorasBox>() }
        );
        num2++;
        ...
        num2++;
        span[num2] = new ValidRelicSet(
            (Player owner) => owner.RunState.CurrentActIndex == 1,
            new RelicModel[2] { ModelDb.Relic<Ectoplasm>(), ModelDb.Relic<Sozu>() }
        );
        num2++;
        span[num2] = new ValidRelicSet(
            (Player owner) => owner.RunState.CurrentActIndex == 2,
            new RelicModel[2] { ModelDb.Relic<PhilosophersStone>(), ModelDb.Relic<VelvetChoker>() }
        );

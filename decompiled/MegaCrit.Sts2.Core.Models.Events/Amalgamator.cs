        ...
        return new global::_003C_003Ez__ReadOnlyArray<EventOption>(
            new EventOption[2]
            {
                new EventOption(
                    this,
                    CombineStrikes,
                    InitialOptionKey("COMBINE_STRIKES"),
                    HoverTipFactory.FromCardWithCardHoverTips<UltimateStrike>()
                ),
                new EventOption(
                    this,
                    CombineDefends,
                    InitialOptionKey("COMBINE_DEFENDS"),
    ...
    private async Task CombineStrikes()
    {
        List<CardModel> cards = (
            await CardSelectCmd.FromDeckForRemoval(
                ...
                player: base.Owner,
                filter: (CardModel c) => IsValid(CardTag.Strike, c)
            )
        ).ToList();
    ...
    private async Task CombineDefends()
    {
        List<CardModel> cards = (
            await CardSelectCmd.FromDeckForRemoval(
                prefs: new CardSelectorPrefs(CardSelectorPrefs.RemoveSelectionPrompt, 2),
                player: base.Owner,
                filter: (CardModel c) => IsValid(CardTag.Defend, c)
            )
        ).ToList();
        await CardPileCmd.RemoveFromDeck(cards);
        NDebugAudioManager.Instance?.Play("card_smith.mp3", 1f, PitchVariance.Small);

...
public sealed class TabletOfTruth : EventModel
{
        ...
        return new global::_003C_003Ez__ReadOnlyArray<EventOption>(
            new EventOption[2]
            {
                new EventOption(
                    ...
                    (Player p) =>
                        (decimal)p.Creature.MaxHp <= base.DynamicVars["DecipherMaxHpLoss"].BaseValue
                ),
    ...
    private async Task Decipher()
    {
        ...
        {
            SetEventFinished(
                L10NLookup($"TABLET_OF_TRUTH.pages.DECIPHER_{DecipherCount}.description")
            );
            return;
        }
            ...
            new global::_003C_003Ez__ReadOnlyArray<EventOption>(
                new EventOption[2]
                {
                    new EventOption(
                        this,
                        Decipher,
                        $"TABLET_OF_TRUTH.pages.DECIPHER_{DecipherCount}.options.DECIPHER"
                    ).ThatWillKillPlayerIf(
                        (Player p) =>
                            (decimal)p.Creature.MaxHp
                            <= base.DynamicVars["DecipherMaxHpLoss"].BaseValue
                    ),

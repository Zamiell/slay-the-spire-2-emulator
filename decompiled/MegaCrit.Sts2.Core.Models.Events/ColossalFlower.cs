...
public sealed class ColossalFlower : EventModel
{
        ...
        return new global::_003C_003Ez__ReadOnlyArray<EventOption>(
            new EventOption[2]
            {
                new EventOption(
                    this,
                    ExtractCurrentPrize,
                    $"COLOSSAL_FLOWER.pages.INITIAL.options.EXTRACT_CURRENT_PRIZE_{NumberOfDigs + 1}"
                ),
                new EventOption(
                    this,
                    ReachDeeper,
                    $"COLOSSAL_FLOWER.pages.INITIAL.options.REACH_DEEPER_{NumberOfDigs + 1}"
                ).ThatDoesDamage(_prizeDamage[NumberOfDigs]),
            }
        );
    }
    ...
    private async Task ReachDeeper()
    {
        ...
        {
            SetEventState(
                L10NLookup($"COLOSSAL_FLOWER.pages.REACH_DEEPER_{NumberOfDigs}.description"),
                new global::_003C_003Ez__ReadOnlyArray<EventOption>(
                    new EventOption[2]
                    {
                        new EventOption(
                            this,
                            ExtractCurrentPrize,
                            $"COLOSSAL_FLOWER.pages.REACH_DEEPER_{NumberOfDigs}.options.EXTRACT_CURRENT_PRIZE_{NumberOfDigs + 1}"
                        ),
                        new EventOption(
                            this,
                            ReachDeeper,
                            $"COLOSSAL_FLOWER.pages.REACH_DEEPER_{NumberOfDigs}.options.REACH_DEEPER_{NumberOfDigs + 1}"
                        ).ThatDoesDamage(_prizeDamage[NumberOfDigs]),
                    }
                )
            );
        ...
        {
            SetEventState(
                L10NLookup("COLOSSAL_FLOWER.pages.REACH_DEEPER_2.description"),
                new global::_003C_003Ez__ReadOnlyArray<EventOption>(
                    new EventOption[2]
                    {
                        new EventOption(

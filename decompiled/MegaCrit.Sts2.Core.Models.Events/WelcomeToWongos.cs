...
public sealed class WelcomeToWongos : EventModel
{
    ...
    protected override IReadOnlyList<EventOption> GenerateInitialOptions()
    {
        ...
        FeaturedItem = RelicFactory.PullNextRelicFromFront(
            owner,
            RelicRarity.Rare,
            (RelicModel r) => r.IsAllowedInShops
        );
        ((StringVar)base.DynamicVars["RandomRelic"]).StringValue =
    ...
    private async Task BuyBargainBin()
    {
        await PlayerCmd.LoseGold(
            base.DynamicVars["BargainBinCost"].BaseValue,
            base.Owner,
            GoldLossType.Spent
        );
        RelicModel relic = RelicFactory
    ...
    {
        await PlayerCmd.LoseGold(
            base.DynamicVars["MysteryBoxCost"].BaseValue,
            base.Owner,
            GoldLossType.Spent
        );
    ...
    {
        await PlayerCmd.LoseGold(
            base.DynamicVars["FeaturedItemCost"].BaseValue,
            base.Owner,
            GoldLossType.Spent
        );
    ...
    {
        Player owner = base.Owner;
        CardModel cardModel = base.Rng.NextItem(
            owner.Deck.Cards.Where((CardModel c) => c.IsUpgraded)
        );
        if (cardModel != null)
        {

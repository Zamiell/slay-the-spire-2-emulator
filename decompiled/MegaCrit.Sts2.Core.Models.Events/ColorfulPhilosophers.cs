...
public sealed class ColorfulPhilosophers : EventModel
{
        ...
        foreach (CardPoolModel cardPool in CardPoolColorOrder)
        {
            if (character.CardPool != cardPool && list2.Contains(cardPool))
            {
                list.Add(
                    new EventOption(
                        this,
                        () => OfferRewards(cardPool),
                        "COLORFUL_PHILOSOPHERS.pages.INITIAL.options."
                            + cardPool.EnergyColorName.ToUpperInvariant()
                    )
                );
            }
        }
    ...
    private async Task OfferRewards(CardPoolModel pool)
    {
        CardCreationOptions options = new CardCreationOptions(
            new global::_003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(pool),
        ...
        CardCreationOptions options3 = new CardCreationOptions(
            new global::_003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(pool),
            CardCreationSource.Other,
            CardRarityOddsType.Uniform,

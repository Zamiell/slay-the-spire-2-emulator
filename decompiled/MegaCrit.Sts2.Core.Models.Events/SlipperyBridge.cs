...
public sealed class SlipperyBridge : EventModel
{
    ...
    public override bool IsAllowed(IRunState runState)
    {
        ...
        {
            return runState.Players.All(
                (Player p) => p.Deck.Cards.Any((CardModel c) => c.IsRemovable)
            );
        }
        return false;
    }
    ...
    private void GetNewRandomCard()
    {
        ...
        {
            list = base
                .Owner.Deck.Cards.Where((CardModel c) => c.Rarity != CardRarity.Basic)
                .ToList();
        }
        else
        {
            ...
            list = base
                .Owner.Deck.Cards.Where((CardModel c) => c.GetType() != RandomCardToLose.GetType())
                .ToList();
        }
    ...
    private async Task HoldOn()
    {
        ...
        string textKey =
            "SLIPPERY_BRIDGE.pages.HOLD_ON_" + holdOnSuffix + ".options.HOLD_ON_" + holdOnSuffix2;
        SetEventState(
            L10NLookup("SLIPPERY_BRIDGE.pages.HOLD_ON_" + holdOnSuffix + ".description"),

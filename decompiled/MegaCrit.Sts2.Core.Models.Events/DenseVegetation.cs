...
public sealed class DenseVegetation : EventModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[3] { new GoldVar(0), new HealVar(0m), new HpLossVar(8m) }
        );
    ...
    public override void CalculateVars()
    {
        ...
        base.DynamicVars.Heal.BaseValue = (
            (base.Owner != null) ? HealRestSiteOption.GetHealAmount(base.Owner) : 0m
        );
    }
    ...
    protected override IReadOnlyList<EventOption> GenerateInitialOptions()
    {
        return new global::_003C_003Ez__ReadOnlyArray<EventOption>(
            new EventOption[2]
            {
                ...
                ).ThatDoesDamage(base.DynamicVars.HpLoss.BaseValue),
    ...
    private async Task TrudgeOn()
    {
        ...
        if (LocalContext.IsMe(base.Owner) && container != null)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                Node2D node2D = VfxCmd.PlayNonCombatVfx(
                    container,
                    vector + vector2,
                    "vfx/vfx_attack_slash"
                );
                Node2D node2D2 = VfxCmd.PlayNonCombatVfx(
                    container,
                    vector + vector2,
                    "vfx/events/dense_vegetation_slice_vfx"
                );
                NDebugAudioManager.Instance.Play("slash_attack.mp3", 0.8f, PitchVariance.Medium);
                node2D.RotationDegrees =
                    (0f - MegaCrit.Sts2.Core.Random.Rng.Chaotic.NextFloat()) * 180f;
                node2D2.RotationDegrees =
                    (0f - MegaCrit.Sts2.Core.Random.Rng.Chaotic.NextFloat()) * 180f;
    ...
    private async Task Rest()
    {
        ...
        SetEventState(
            L10NLookup("DENSE_VEGETATION.pages.REST.description"),
            new global::_003C_003Ez__ReadOnlySingleElementList<EventOption>(
                new EventOption(this, Fight, "DENSE_VEGETATION.pages.REST.options.FIGHT")
            )
        );
    }

...
public sealed class WarHistorianRepy : EventModel
{
    ...
    private async Task InitialUnlockCage()
    {
        ...
        {
            SetEventState(
                L10NLookup("WAR_HISTORIAN_REPY.pages.UNLOCK_CAGE.description"),
    ...
    private async Task InitialUnlockChest()
    {
        ...
        {
            SetEventState(
                L10NLookup("WAR_HISTORIAN_REPY.pages.UNLOCK_CHEST.description"),
                new global::_003C_003Ez__ReadOnlySingleElementList<EventOption>(
                    new EventOption(
                        this,
                        SecondUnlockCage,
    ...
    {
        CardModel cardModel = base.Owner.Deck.Cards.FirstOrDefault(
            (CardModel c) => c is LanternKey
        );
        if (cardModel != null)
        {
    ...
    private async Task RemoveLanternKeysForSecondChoice()
    {
        if (base.Owner.RunState.Players.Count > 1)
        {
            List<CardModel> list = base
                .Owner.Deck.Cards.Where((CardModel c) => c is LanternKey)
                .ToList();

...
public sealed class TeaMaster : EventModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[5]
            {
                ...
                new StringVar(
                    "BoneTeaDescription",
                    ModelDb.Relic<BoneTea>().DynamicDescription.GetFormattedText()
                ),
                new StringVar(
                    "EmberTeaDescription",
                    ModelDb.Relic<EmberTea>().DynamicDescription.GetFormattedText()
                ),
                new StringVar(
                    "TeaOfDiscourtesyDescription",
                    ModelDb.Relic<TeaOfDiscourtesy>().DynamicDescription.GetFormattedText()
                ),
            }
        );
    ...
    protected override IReadOnlyList<EventOption> GenerateInitialOptions()
    {
        ...
        {
            list.Add(
                new EventOption(this, null, "TEA_MASTER.pages.INITIAL.options.BONE_TEA_LOCKED")
            );
        }
        ...
        {
            list.Add(
                new EventOption(this, null, "TEA_MASTER.pages.INITIAL.options.EMBER_TEA_LOCKED")
            );
        }
        list.Add(
            new EventOption(
    ...
    private async Task BoneTea()
    {
        await PlayerCmd.LoseGold(
            base.DynamicVars["BoneTeaCost"].BaseValue,
            base.Owner,
            GoldLossType.Spent
        );
    ...
    {
        await PlayerCmd.LoseGold(
            base.DynamicVars["EmberTeaCost"].BaseValue,
            base.Owner,
            GoldLossType.Spent
        );

...
public class NetQualityTracker : IDisposable
{
    ...
    private readonly MegaCrit.Sts2.Core.Logging.Logger _logger =
        new MegaCrit.Sts2.Core.Logging.Logger("NetQualityTracker", LogType.Network);
    ...
    public void Dispose()
    {
        _netService.UnregisterMessageHandler<HeartbeatRequestMessage>(
            HandleHeartbeatRequestMessage
        );
        _netService.UnregisterMessageHandler<HeartbeatResponseMessage>(
    ...
    public void Update()
    {
        ...
        else
        {
            ...
            {
                _logger.Debug(
                    $"\t{stat2.PeerId} - Ping: {stat2.PingMsec}. Packet Loss: {stat2.PacketLoss}."
                );
            }
        }
    }
    ...
    private void HandleHeartbeatRequestMessage(HeartbeatRequestMessage message, ulong senderId)
    {
        _netService.SendMessage(
            new HeartbeatResponseMessage { counter = message.counter, isLoading = _isLoading },
            senderId
        );
    }

...
public class ConnectionStats
{
    ...
    {
        PeerId = peerId;
        _logger = new MegaCrit.Sts2.Core.Logging.Logger(
    ...
    public HeartbeatRequestMessage GenerateHeartbeat(ulong timeMsec)
    {
        ...
        HeartbeatStatus value = new HeartbeatStatus { counter = _nextIndex, sentMsec = timeMsec };
        HeartbeatStatus? heartbeatStatus = _statuses[num];
        if (heartbeatStatus.HasValue && !heartbeatStatus.Value.receivedMsec.HasValue)
        {
            _logger.VeryDebug(
                $"Heartbeat {heartbeatStatus.Value.counter} ({heartbeatStatus.Value.sentMsec}) was never received, marking as lost"
            );
            OnPacketLost();
        }
        _statuses[num] = value;
        HeartbeatRequestMessage result = new HeartbeatRequestMessage { counter = _nextIndex };
        _nextIndex++;
        return result;
    }
    ...
    public void OnHeartbeatReceived(HeartbeatResponseMessage message, ulong timeMsec)
    {
        ...
        {
            _logger.VeryDebug(
                $"Counter {message.counter} is less than {num} and greater than {_nextIndex}"
            );
            return;
        }
        ...
        {
            _logger.VeryDebug(
                $"Counter in message {message.counter} does not match counter at index {num2}, which is {valueOrDefault.counter}"
            );
            return;
        }

...
public class AutoSlayer
{
    ...
    public static T GetCurrentScreen<T>()
        where T : Node
    {
    ...
    private async Task PlayRunAsync(string seed, CancellationToken ct)
    {
        await WaitHelper.Until(
            () => NGame.Instance != null,
            ct,
            AutoSlayConfig.gameInitTimeout,
            "Game instance not initialized"
        );
        ...
        await WaitHelper.Until(
            ...
            ct,
            AutoSlayConfig.runStateTimeout,
            "Run state not initialized"
        );
        RunState runState = RunManager.Instance.DebugOnlyGetState();
        await WaitHelper.Until(
            () =>
                runState.CurrentRoom != null
                && runState.CurrentRoom.RoomType != RoomType.Unassigned,
            ct,
            AutoSlayConfig.nodeWaitTimeout,
            "Room type not assigned"
        );
        while (runState.TotalFloor < 49)
        {
            ...
            if (roomType == RoomType.Boss)
            {
                ...
                if (
                    postBossRoomType == RoomType.Event
                ...
                await WaitHelper.Until(
                    () => runState.VisitedMapCoords.Count == 0,
                    ct,
    ...
    private async Task DrainOverlayScreensAsync(CancellationToken ct)
    {
        ...
        while (true)
        {
            ...
            if (handledScreens.Contains(currentOverlay))
            {
                ...
                {
                    AutoSlayLog.Error(
                        $"Infinite loop detected: screen {currentOverlay.GetType().Name} not closing after {3} attempts"
                    );
                    throw new InvalidOperationException(
                        "Screen "
                            + currentOverlay.GetType().Name
                            + " not closing after being handled"
                    );
                }
                AutoSlayLog.Warn(
                    $"Screen {currentOverlay.GetType().Name} still present after handling (attempt {consecutiveFailures})"
                );
            }
            else
            {
    ...
    private async Task WaitForMainMenuAsync(CancellationToken ct)
    {
        ...
        await WaitHelper.Until(
            () =>
                root.GetNodeOrNull<Control>("/root/Game/RootSceneContainer/MainMenu")
                    ?.IsVisibleInTree()
                ?? false,
            ct,
    ...
    private async Task PlayMainMenuAsync(CancellationToken ct)
    {
        ...
        if (node.Visible)
        {
            ...
            await WaitHelper.Until(
                ...
                ct,
                AutoSlayConfig.nodeWaitTimeout,
                "Abandon run confirmation popup did not appear"
            );
            ...
            await WaitHelper.Until(
                ...
                ct,
                AutoSlayConfig.nodeWaitTimeout,
                "Abandon run confirmation popup did not close"
            );
        }
        ...
        if (standardButton?.Visible ?? false)
        {
            ...
            if (control == null || !control.Visible)
            {
                ...
                await WaitHelper.Until(
                    () =>
                        mainMenu.GetNodeOrNull<Control>("Submenus/CharacterSelectScreen")?.Visible
                        ?? false,
                    ct,
                    AutoSlayConfig.nodeWaitTimeout,
        ...
        List<NCharacterSelectButton> items = list.Where((NCharacterSelectButton b) => !b.IsLocked)
            .ToList();
        ...
        await Task.Delay(100, ct);
        NButton button = await WaitHelper.ForNode<NButton>(
            mainMenu,
    ...
    private async Task AbandonRunAsync(CancellationToken ct)
    {
        ...
        await UiHelper.Click(
            await WaitHelper.ForNode<NButton>(
                root,

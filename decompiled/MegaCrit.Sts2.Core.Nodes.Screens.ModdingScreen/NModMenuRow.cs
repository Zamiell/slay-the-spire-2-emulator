...
public class NModMenuRow : NClickableControl
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/modding/modding_screen_row"
    );
    ...
    public static NModMenuRow? Create(NModdingScreen screen, Mod mod)
    {
        ...
        NModMenuRow nModMenuRow = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NModMenuRow>(PackedScene.GenEditState.Disabled);
        nModMenuRow.Mod = mod;
    ...
    public override void _Ready()
    {
        if (Mod != null)
        {
            ...
            node.IsTicked = !(
                SaveManager.Instance.SettingsSave.ModSettings?.IsModDisabled(Mod) ?? false
            );
    ...
    )
    {
        ...
        if (method == MethodName.GetPlatformIcon && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<Texture2D>(
                GetPlatformIcon(VariantUtils.ConvertTo<ModSource>(in args[0]))
            );
            return true;
        }
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._selectionHighlight,
                PropertyHint.None,
                "",

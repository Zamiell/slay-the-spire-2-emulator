...
public class NModdingScreen : NSubmenu
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/modding/modding_screen"
    );
    ...
    public static NModdingScreen? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NModdingScreen>(PackedScene.GenEditState.Disabled);
    }
    ...
    public override void _Ready()
    {
        ...
        node.Connect(
            ...
            Callable.From<NButton>(OnGetModsPressed)
        );
        node2.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnMakeModsPressed)
        );
        node.GetNode<MegaLabel>("Visuals/Label")
            .SetTextAutoSize(
                new LocString("settings_ui", "MODDING_SCREEN.GET_MODS_BUTTON").GetFormattedText()
            );
        node2
            .GetNode<MegaLabel>("Visuals/Label")
            .SetTextAutoSize(
                new LocString("settings_ui", "MODDING_SCREEN.MAKE_MODS_BUTTON").GetFormattedText()
            );
        GetNode<MegaRichTextLabel>("%InstalledModsTitle")
            .SetTextAutoSize(
                new LocString(
                    "settings_ui",
        ...
        GetNode<MegaRichTextLabel>("%PendingChangesLabel")
            .SetTextAutoSize(
                new LocString(
                    "settings_ui",
                    "MODDING_SCREEN.PENDING_CHANGES_WARNING"
                ).GetFormattedText()
            );
        node2.Visible = false;
        node.Visible = false;
    ...
    public void OnModEnabledOrDisabled()
    {
        foreach (Mod mod in ModManager.Mods)
        {
            ...
            if (
                (mod.state == ModLoadState.Disabled && !flag)
                || (mod.state == ModLoadState.Loaded && flag)
            )
            {
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnModEnabledOrDisabled,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._modInfoContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._pendingChangesWarning,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._pendingChangesWarning,
            Variant.From(in _pendingChangesWarning)
        );
    }

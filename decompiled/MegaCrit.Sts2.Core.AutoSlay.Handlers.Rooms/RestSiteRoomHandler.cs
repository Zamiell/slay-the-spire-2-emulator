...
public class RestSiteRoomHandler : IRoomHandler, IHandler
{
    ...
    public async Task HandleAsync(Rng random, CancellationToken ct)
    {
        ...
        NRestSiteRoom room = await WaitHelper.ForNode<NRestSiteRoom>(
            root,
            "/root/Game/RootSceneContainer/Run/RoomContainer/RestSiteRoom",
            ct
        );
        List<NRestSiteButton> list = (
            from b in UiHelper.FindAll<NRestSiteButton>(room)
        ...
        await WaitHelper.Until(
            ...
            "Rest site option did not respond"
        );

...
public class CombatRoomHandler : IRoomHandler, IHandler
{
    public RoomType[] HandledTypes =>
        new RoomType[3] { RoomType.Monster, RoomType.Elite, RoomType.Boss };
        ...
        while (CombatManager.Instance.IsInProgress && turnCount < 100)
        {
            ...
            await WaitHelper.Until(
                delegate
                {
                    ...
                    return (
                            playerCombatState3 != null
                            && playerCombatState3.Phase == PlayerTurnPhase.Play
                        ) || !CombatManager.Instance.IsInProgress;
                },
                ct,
                TimeSpan.FromSeconds(30L),
                "Play phase not started"
            );
            ...
            while (cardsPlayed < 50)
            {
                ...
                {
                    AutoSlayer.CurrentWatchdog?.Reset(
                ...
                List<CardModel> list = pile
                    .Cards.Where(
                        (CardModel c) =>
                            c.CanPlay(out reason, out preventer) && !attemptedCards.Contains(c)
                    )
                    .ToList();
                if (list.Count == 0)
                {
            ...
            if (
                playerCombatState2 != null
                && playerCombatState2.Phase == PlayerTurnPhase.Play
                && CombatManager.Instance.IsInProgress
            )
            {
    ...
    )
    {
        ...
        return potion.TargetType switch
        {
            ...
            TargetType.AnyAlly => combatState.PlayerCreatures.FirstOrDefault(
                (Creature c) => c.IsAlive
            ),
            TargetType.AnyPlayer => combatState.PlayerCreatures.FirstOrDefault(
                (Creature c) => c.IsAlive
            ),
            TargetType.Self => combatState.PlayerCreatures.FirstOrDefault(
                (Creature c) => c.IsAlive
            ),
            _ => null,
        };

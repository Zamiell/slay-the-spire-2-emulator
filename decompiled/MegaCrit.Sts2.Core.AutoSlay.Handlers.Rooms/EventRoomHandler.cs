...
public class EventRoomHandler : IRoomHandler, IHandler
{
    ...
    public async Task HandleAsync(Rng random, CancellationToken ct)
    {
        ...
        while (iterations < 50)
        {
            ...
            if (!GodotObject.IsInstanceValid(eventRoom) || !eventRoom.IsInsideTree())
            {
                ...
                if (runState != null && runState.CurrentRoomCount > 1)
                {
                    ...
                    if (baseRoom != null && baseRoom.RoomType == RoomType.Event)
                    {
                        ...
                        Node nodeOrNull = root.GetNodeOrNull(
                        ...
                        List<NEventOptionButton> list = (
                            from o in UiHelper.FindAll<NEventOptionButton>(eventRoom)
            ...
            List<NEventOptionButton> list3 = list2
                .Where(
                    (NEventOptionButton o) =>
                        o.Option.WillKillPlayer == null
                        || o.Event.Owner == null
                        || !o.Option.WillKillPlayer(o.Event.Owner)
                )
                .ToList();
            if (list3.Count > 0)
            {
            ...
            AutoSlayLog.Action(
                $"Selecting event option: {choice.Event.Id.Entry} (option: {choice.Option.Title.GetFormattedText()})"
            );
            HashSet<NEventOptionButton> previousButtons = new HashSet<NEventOptionButton>(
                from o in UiHelper.FindAll<NEventOptionButton>(eventRoom)
                where !o.Option.IsLocked
                select o
            );
            ...
            if (CombatManager.Instance.IsInProgress)
            {
                ...
                await WaitHelper.Until(
                    delegate
                    {
                        if (GodotObject.IsInstanceValid(eventRoom) && eventRoom.IsInsideTree())
                        {
                            ...
                            if (instance2 == null || !instance2.IsOpen)
                            {
                                ...
                                {
                                    return UiHelper
                                        .FindAll<NEventOptionButton>(eventRoom)
                                        .Any((NEventOptionButton o) => !o.Option.IsLocked);
                                }
                            }
                        }
                        return true;
                    ...
                    "Event did not resume after combat layout combat"
                );
                if (!GodotObject.IsInstanceValid(eventRoom) || !eventRoom.IsInsideTree())
                {
                    AutoSlayLog.Action("Event room gone after combat");
    ...
    private async Task HandleEventCombat(CancellationToken ct)
    {
        ...
        while (CombatManager.Instance.IsInProgress && killAttempts < 20)
        {
            ...
            {
                List<PowerModel> list = creature
                    .Powers.Where((PowerModel p) => p.ShouldStopCombatFromEnding())
                    .ToList();
                foreach (PowerModel item in list)
                {
                    AutoSlayLog.Info(
                        $"Removing blocking power {item.GetType().Name} from {creature}"
                    );
            ...
            {
                AutoSlayLog.Action(
                    $"Killing {list2.Count} event combat enemies (attempt {killAttempts + 1})"
                );
            ...
            else
            {
                ...
                {
                    AutoSlayLog.Info(
                        "Event combat still in progress but no enemies for 10s, breaking loop"
                    );
                    break;
                }
    ...
    {
        Node root = ((SceneTree)Engine.GetMainLoop()).Root;
        return await WaitHelper.ForNode<Node>(
            root,
            "/root/Game/RootSceneContainer/Run/RoomContainer/EventRoom",
            ct
        );
    ...
    {
        NAncientEventLayout nAncientEventLayout = UiHelper.FindFirst<NAncientEventLayout>(
            eventRoom
        );
        ...
        await WaitHelper.Until(
            delegate
            {
                ...
                if (waitCycles % 50 == 0)
                {
                    ...
                    AutoSlayLog.Info(
                        $"Waiting for event options: {childCount} children in event room"
                    );
    ...
    private async Task HandleFakeMerchantEvent(NFakeMerchant fakeMerchant, CancellationToken ct)
    {
        ...
        await WaitHelper.Until(
            ...
            ct,
            TimeSpan.FromSeconds(10L),
            "FakeMerchant proceed button not available"
        );
    ...
    )
    {
        ...
        while (clicks < 50)
        {
            ...
            List<NEventOptionButton> list = (
                from b in UiHelper.FindAll<NEventOptionButton>(ancientLayout)
        ...
        await WaitHelper.Until(
            () =>
                UiHelper
                    .FindAll<NEventOptionButton>(ancientLayout)
                    .Any((NEventOptionButton b) => b.IsEnabled && !b.Option.IsLocked),
            ct,
            TimeSpan.FromSeconds(10L),

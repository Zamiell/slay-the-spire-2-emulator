...
public class NRunHistoryPlayerIcon : NButton
{
    ...
    public static readonly string scenePath = SceneHelper.GetScenePath(
        "screens/run_history_screen/run_history_player_icon"
    );
    ...
    public void LoadRun(RunHistoryPlayer player, RunHistory history)
    {
        ...
        _ascensionLabel.SetTextAutoSize(
            (history.Ascension > 0) ? history.Ascension.ToString() : string.Empty
        );
        ...
        if (history.Players.Count > 1)
        {
            locString3.Add(
                "PlayerName",
                PlatformUtil.GetPlayerName(history.PlatformType, player.Id)
            );
        ...
        {
            _hoverTips.Add(
                AscensionHelper.GetHoverTip(
                    characterModel,
                    history.Ascension,
                    history.GameMode.AreAchievementsAndEpochsLocked()
                )
            );
        }
        else

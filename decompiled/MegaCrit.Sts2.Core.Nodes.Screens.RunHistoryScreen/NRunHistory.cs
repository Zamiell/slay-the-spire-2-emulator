...
public class NRunHistory : NSubmenu
{
    public new class MethodName : NSubmenu.MethodName
    {
        ...
        public static new readonly StringName OnSubmenuShown = "OnSubmenuShown";

        public static new readonly StringName OnSubmenuHidden = "OnSubmenuHidden";
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/run_history_screen/run_history"
    );
    ...
    private static readonly LocString _leftQuote = new LocString(
        "game_over_screen",
        "ENCOUNTER_QUOTE_LEFT"
    );

    private static readonly LocString _rightQuote = new LocString(
        "game_over_screen",
        "ENCOUNTER_QUOTE_RIGHT"
    );
    ...
    public static NRunHistory? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NRunHistory>(PackedScene.GenEditState.Disabled);
    }
    ...
    public override void _Ready()
    {
        ...
        _prevButton.Connect(
            ...
            Callable.From<NButton>(OnLeftButtonButtonReleased)
        );
        _nextButton = GetNode<NRunHistoryArrowButton>("RightArrow");
        _nextButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnRightButtonButtonReleased)
        );
    ...
    private void OnLeftButtonButtonReleased(NButton _)
    {
        ...
        _screenTween
            .TweenProperty(_screenContents, "position", Vector2.Zero, 0.5)
            .SetTrans(Tween.TransitionType.Expo)
            .SetEase(Tween.EaseType.Out)
            .From(Vector2.Zero + new Vector2(-1000f, 0f));
        _screenTween
            .TweenProperty(_screenContents, "modulate:a", 1f, 0.4)
    ...
    private Task RefreshAndSelectRun(int index)
    {
        ...
        try
        {
            ReadSaveResult<RunHistory> readSaveResult = SaveManager.Instance.LoadRunHistory(
                _runNames[index]
            );
            ...
            {
                Log.Error(
                    $"Could not load run {_runNames[index]} at index {index}: {readSaveResult.ErrorMessage} ({readSaveResult.Status})"
                );
    ...
    public override void _Input(InputEvent inputEvent)
    {
        ...
        if (!flag && ActiveScreenContext.Instance.IsCurrent(this))
        {
            ...
            if (
                (control2 == null || !IsAncestorOf(control2))
                && (
                    inputEvent.IsActionPressed(MegaInput.left)
                    || inputEvent.IsActionPressed(MegaInput.right)
                    || inputEvent.IsActionPressed(MegaInput.up)
                    || inputEvent.IsActionPressed(MegaInput.down)
    ...
    private void DisplayRun(RunHistory history)
    {
        ...
        foreach (
            NRunHistoryPlayerIcon item in _playerIconContainer
                .GetChildren()
                .OfType<NRunHistoryPlayerIcon>()
        )
        {
        ...
        {
            NRunHistoryPlayerIcon playerIcon = PreloadManager
                .Cache.GetScene(NRunHistoryPlayerIcon.scenePath)
                .Instantiate<NRunHistoryPlayerIcon>(PackedScene.GenEditState.Disabled);
            ...
            playerIcon.Connect(
                NClickableControl.SignalName.Released,
                Callable.From<NClickableControl>(
                    delegate
                    {
        ...
        {
            _playerIconContainer.GetChild<Control>(num).FocusNeighborLeft = (
                (num > 0)
                    ? _playerIconContainer.GetChild<Control>(num - 1).GetPath()
                    : _playerIconContainer
                        .GetChild<Control>(_playerIconContainer.GetChildCount() - 1)
                        .GetPath()
            );
            _playerIconContainer.GetChild<Control>(num).FocusNeighborRight = (
                (num < _playerIconContainer.GetChildCount() - 1)
                    ? _playerIconContainer.GetChild<Control>(num + 1).GetPath()
                    : _playerIconContainer.GetChild<Control>(0).GetPath()
            );
        }
        if (!flag)
        {
            ...
            {
                Log.Warn(
                    $"Local player with ID {localPlayerId} not found in multiplayer run history file! Defaulting to first player"
                );
            }
    ...
    private void SelectPlayer(NRunHistoryPlayerIcon playerIcon)
    {
        ...
        if (_history.Players.Count != 1)
        {
            ...
            locString.Add(
                "PlayerName",
                PlatformUtil.GetPlayerName(_history.PlatformType, playerIcon.Player.Id)
            );
        }
        ...
        LoadBadges(
            _history
                .Players.FirstOrDefault((RunHistoryPlayer p) => p.Id == player.NetId)
                .Badges.ToList()
        );
    ...
    private void LoadGoldHpAndPotionInfo(NRunHistoryPlayerIcon icon)
    {
        ...
        else
        {
            ...
            PlayerMapPointHistoryEntry playerMapPointHistoryEntry =
                mapPointHistoryEntry.PlayerStats.First(
                    (PlayerMapPointHistoryEntry stat) => stat.PlayerId == icon.Player.Id
                );
            _hpLabel.SetTextAutoSize(
                $"{playerMapPointHistoryEntry.CurrentHp}/{playerMapPointHistoryEntry.MaxHp}"
            );
            _goldLabel.SetTextAutoSize($"{playerMapPointHistoryEntry.CurrentGold}");
        }
        _potionHolder.FreeChildren();
        RunHistoryPlayer runHistoryPlayer = _history.Players.First(
            (RunHistoryPlayer player) => player.Id == icon.Player.Id
        );
        List<PotionModel> list = runHistoryPlayer
            .Potions.Select(PotionModel.FromSerializable)
            .ToList();
        ...
        Player owner = Player.CreateForNewRun(
            SaveUtil.CharacterOrDeprecated(icon.Player.Character),
            unlockState,
            icon.Player.Id
        );
    ...
    )
    {
        ...
        switch (gameOverType)
        {
            ...
            {
                LocString randomWithPrefix2 = LocString.GetRandomWithPrefix(
                    "run_history",
                    "MAP_POINT_HISTORY.abandon",
                    rng
                );
            ...
            case GameOverType.EventDeath:
            {
                ...
                LocString locString = LocString.GetIfExists(
                    eventModel.LocTable,
                    eventModel.Id.Entry + ".loss"
                );
                if (locString == null)
                {
            ...
            {
                EncounterModel encounterModel = SaveUtil.EncounterOrDeprecated(
                    history.KilledByEncounter
                );
            ...
            {
                LocString randomWithPrefix = LocString.GetRandomWithPrefix(
                    "run_history",
                    "MAP_POINT_HISTORY.falseVictory",
                    rng
                );
    ...
    private void LoadDeathQuote(RunHistory history, ModelId characterId)
    {
        ...
        if (history.Win)
        {
            _deathQuoteLabel.AddThemeColorOverride(
            ...
            StringBuilder.AppendInterpolatedStringHandler handler =
                new StringBuilder.AppendInterpolatedStringHandler(0, 1, stringBuilder2);
        ...
        else if (history.KilledByEvent != ModelId.none)
        {
            ...
            LocString locString = (
                (!LocString.Exists("events", text))
                    ? new LocString("run_history", "DEFAULT_EVENT_LOSS_MESSAGE")
                    : new LocString("events", text)
            );
            characterModel.AddDetailsTo(locString);
            locString.Add("event", eventModel.Title);
            StringBuilder stringBuilder2 = stringBuilder;
    ...
    private void LoadTimeDetails(RunHistory history)
    {
        ...
        DateTime dateTime = TimeZoneInfo.ConvertTimeFromUtc(
            DateTimeOffset.FromUnixTimeSeconds(history.StartTime).UtcDateTime,
            TimeZoneInfo.Local
        );
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnLeftButtonButtonReleased,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnRightButtonButtonReleased,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.LoadGoldHpAndPotionInfo,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._playerIconContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._selectedPlayerIcon,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

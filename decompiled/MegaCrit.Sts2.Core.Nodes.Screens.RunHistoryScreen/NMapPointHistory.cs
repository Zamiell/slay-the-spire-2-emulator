...
public class NMapPointHistory : Control
{
    ...
    private List<NMapPointHistoryEntry> MapHistories =>
        _actHistories.SelectMany((NActHistoryEntry a) => a.Entries).ToList();
    ...
    public void LoadHistory(RunHistory history)
    {
        ...
        for (int i = 0; i < history.MapPointHistory.Count; i++)
        {
            ...
            NActHistoryEntry nActHistoryEntry = NActHistoryEntry.Create(
                title,
                history,
                history.MapPointHistory[i],
                num
            );
    ...
    {
        deckHistory.Connect(
            NDeckHistory.SignalName.Hovered,
            Callable.From<NDeckHistoryEntry>(HighlightRelevantEntries)
        );
        deckHistory.Connect(
            NDeckHistory.SignalName.Unhovered,
            Callable.From((Action<NDeckHistoryEntry>)UnHighlightEntries)
        );
    }

    public void SetRelicHistory(NRelicHistory relicHistory)
    {
        relicHistory.Connect(
            NRelicHistory.SignalName.Hovered,
            Callable.From<NRelicBasicHolder>(HighlightRelevantEntries)
        );
        relicHistory.Connect(
            NRelicHistory.SignalName.Unhovered,
            Callable.From((Action<NRelicBasicHolder>)UnHighlightEntries)
        );
    }
    ...
    private void HighlightRelevantEntries(NDeckHistoryEntry historyEntry)
    {
        ...
        {
            MapHistories
                .FirstOrDefault((NMapPointHistoryEntry e) => e.FloorNum == floorNumber)
                ?.Highlight();
        }
    }
    ...
    private void HighlightRelevantEntries(NRelicBasicHolder holder)
    {
        if (holder.Relic.Model.FloorAddedToDeck > 0)
        {
            MapHistories
                .FirstOrDefault(
                    (NMapPointHistoryEntry e) => e.FloorNum == holder.Relic.Model.FloorAddedToDeck
                )
                ?.Highlight();
        }
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.HighlightRelevantEntries,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,

...
public class NRelicHistory : VBoxContainer
{
    ...
    private readonly LocString _relicCategories = new LocString(
        "run_history",
        "RELIC_HISTORY.categories"
    );
    ...
    private void OnRelicClicked(NRelic node)
    {
        ...
        foreach (
            NRelicBasicHolder item in _relicsContainer.GetChildren().OfType<NRelicBasicHolder>()
        )
        {

...
public class SteamClient : NetClient
{
    ...
    {
        _logger.Info(
            $"Attempting to connect to lobby. Our player id: {NetId}. Other player's id: {steamPlayerId}."
        );
        if (!SteamFriends.GetFriendGamePlayed(new CSteamID(steamPlayerId), out var pFriendGameInfo))
        {
            Log.Warn(
                "Tried to join a friend's Steam game, but they're not playing any game. Likely a stale invite"
            );
            return Task.FromResult(
                (NetErrorInfo?)new NetErrorInfo(NetError.InvalidJoin, selfInitiated: false)
            );
        }
        if (
            pFriendGameInfo.m_gameID != new CGameID(2868840uL)
            || pFriendGameInfo.m_steamIDLobby.m_SteamID == 0L
        )
        {
            Log.Warn(
                "Tried to join a friend's Steam game, but they are not playing STS2 or they are not in a lobby. Likely a stale invite"
            );
            return Task.FromResult(
                (NetErrorInfo?)new NetErrorInfo(NetError.InvalidJoin, selfInitiated: false)
            );
    ...
    public async Task<NetErrorInfo?> ConnectToLobby(
        ulong lobbyId,
        CancellationToken cancelToken = default(CancellationToken)
    )
    {
        ...
        await using (cancelToken.Register(CancelConnection))
        {
            ...
            using SteamCallResult<LobbyEnter_t> callResult = new SteamCallResult<LobbyEnter_t>(
                call,
                cancelToken
            );
            ...
            {
                EChatRoomEnterResponse eChatRoomEnterResponse = (EChatRoomEnterResponse)
                    lobbyEnter_t.m_EChatRoomEnterResponse;
                if (
                    eChatRoomEnterResponse != EChatRoomEnterResponse.k_EChatRoomEnterResponseSuccess
                )
                {
                ...
                else
                {
                    ...
                    _netStatusChangedCallback =
                        new Callback<SteamNetConnectionStatusChangedCallback_t>(OnNetStatusChanged);
                    ...
                    {
                        CleanupConnection(
                            (int)connectionResult.disconnectionReason.Value,
                            connectionResult.debugReason
                        );
                        result = new NetErrorInfo(
                            connectionResult.disconnectionReason.Value,
                            connectionResult.debugReason,
                            selfInitiated: false
                        );
                    }
                    else if (
                        _conn.Value.m_HSteamNetConnection
                        != connectionResult.connection.Value.m_HSteamNetConnection
                    )
                    {
                        _logger.Error(
                            "Got different connection back from OnNetStatusChanged than we expected!"
                        );
                        SteamNetworkingSockets.CloseConnection(
                            connectionResult.connection.Value,
                            0,
                            null,
                            bEnableLinger: false
                        );
    ...
    private void OnNetStatusChanged(SteamNetConnectionStatusChangedCallback_t data)
    {
        ...
        )
        {
            ...
            if (_connectingTaskCompletionSource == null)
            {
                ...
                DisconnectFromHostInternal(
                    SteamDisconnectionReason.InternalError,
                    "Not Connecting",
                    now: true,
                    selfInitiated: false
                );
            ...
            {
                _connectingTaskCompletionSource.SetResult(
                    new ConnectionResult { connection = data.m_hConn }
                );
            }
        }
        else if (
        ...
        {
            _logger.Info(
                $"Steam connection closed because of problem. Reason: {data.m_info.m_eEndReason}, {data.m_info.m_szEndDebug}"
            );
            HandleDisconnection(
                (SteamDisconnectionReason)data.m_info.m_eEndReason,
        ...
        {
            _logger.Info(
                $"Steam connection closed by host. Reason: {data.m_info.m_eEndReason}, {data.m_info.m_szEndDebug}"
            );
            HandleDisconnection(
                (SteamDisconnectionReason)data.m_info.m_eEndReason,
                data.m_info.m_szEndDebug
    ...
    private void HandleDisconnection(SteamDisconnectionReason reason, string debugReason)
    {
        ...
        {
            _connectingTaskCompletionSource.SetResult(
                new ConnectionResult { disconnectionReason = reason, debugReason = debugReason }
            );
        }
        else
        {
    ...
    )
    {
        ...
        try
        {
            ...
            EResult eResult = SteamNetworkingSockets.SendMessageToConnection(
                _conn.Value,
                gCHandle.AddrOfPinnedObject(),
                (uint)length,
                SteamUtil.FlagsFromMode(mode),
    ...
    {
        _logger.Debug(
            $"Disconnecting from host (now: {now} reason: {reason} debug: {debugReason})"
        );
        ...
        _handler.OnDisconnectedFromHost(
            steamID,
            new NetErrorInfo(reason, debugReason, selfInitiated)
        );
    }

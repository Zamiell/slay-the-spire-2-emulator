...
public static class SteamUtil
{
    ...
    public static void ProcessMessages(HSteamNetConnection conn, INetHandler handler, Logger logger)
    {
        ...
        do
        {
            num = SteamNetworkingSockets.ReceiveMessagesOnConnection(
                conn,
                _messageBuffer,
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                SteamNetworkingMessage_t steamNetworkingMessage_t =
                    Marshal.PtrToStructure<SteamNetworkingMessage_t>(num2);
                byte[] array = new byte[steamNetworkingMessage_t.m_cbSize];
                Marshal.Copy(
                    steamNetworkingMessage_t.m_pData,
                    array,
                    0,
                    steamNetworkingMessage_t.m_cbSize
                );
                NetTransferMode netTransferMode = ModeFromFlags(steamNetworkingMessage_t.m_nFlags);
                logger.VeryDebug(
                    $"Received packet of size {array.Length} from sender {steamNetworkingMessage_t.m_identityPeer.GetSteamID64()} ({netTransferMode}, {steamNetworkingMessage_t.m_nChannel})"
                );
                handler.OnPacketReceived(
                    steamNetworkingMessage_t.m_identityPeer.GetSteamID().m_SteamID,
                    array,
                    netTransferMode,
                    steamNetworkingMessage_t.m_nChannel
                );

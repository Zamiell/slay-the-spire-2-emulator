...
public class SteamHost : NetHost
{
    ...
    public override IEnumerable<ulong> ConnectedPeerIds =>
        _connections.Select((ClientConnection c) => c.netId.GetSteamID64());
    ...
    public async Task<NetErrorInfo?> StartHost(int maxPlayers)
    {
        ...
        using SteamCallResult<LobbyCreated_t> callResult = new SteamCallResult<LobbyCreated_t>(
            call,
        ...
        _socket = SteamNetworkingSockets.CreateListenSocketP2P(
            0,
            0,
            new SteamNetworkingConfigValue_t[2]
            {
                new SteamNetworkingConfigValue_t
                {
                    m_eValue = ESteamNetworkingConfigValue.k_ESteamNetworkingConfig_TimeoutInitial,
                    m_eDataType = ESteamNetworkingConfigDataType.k_ESteamNetworkingConfig_Int32,
                    m_val = new SteamNetworkingConfigValue_t.OptionValue { m_int32 = 20000 },
                },
                new SteamNetworkingConfigValue_t
                {
                    m_eValue =
        ...
        _netStatusChangedCallback = new Callback<SteamNetConnectionStatusChangedCallback_t>(
            OnNetStatusChanged
        );
        _isConnected = true;
        return null;
    ...
    public override void SetHostIsClosed(bool isClosed)
    {
        SteamMatchmaking.SetLobbyType(
            _lobbyId.Value,
            (!isClosed) ? ELobbyType.k_ELobbyTypeFriendsOnly : ELobbyType.k_ELobbyTypePrivate
        );
    }
    ...
    {
        _logger.Debug(
            $"Connection status changed: ({data.m_info.m_identityRemote.GetSteamID64()}: {data.m_eOldState} -> {data.m_info.m_eState}"
        );
        if (
        ...
        )
        {
            ...
            {
                _logger.Warn(
                    $"Player with steam id {data.m_info.m_identityRemote.GetSteamID64()} attempted to join the game, but they are not in the lobby (id {_lobbyId.Value})"
                );
                CloseConnectionAndRemove(
                    data.m_hConn,
                    NetError.TryAgainLater.ToSteam(),
            ...
            _logger.Info(
                $"Accepting new connection with user {data.m_info.m_identityRemote.GetSteamID64()}"
            );
            EResult eResult = SteamNetworkingSockets.AcceptConnection(data.m_hConn);
            if (eResult != EResult.k_EResultOK)
            {
                _logger.Error(
                    $"Tried to accept connection with user {data.m_info.m_identityRemote.GetSteamID64()} but it returned result {eResult}!"
                );
                CloseConnectionAndRemove(
                    data.m_hConn,
                    SteamDisconnectionReason.AppInternalError,
                    $"Connection accept failure: {eResult}",
                    now: true,
                    selfInitiated: false
                );
            }
        ...
        {
            _connections.Add(
                new ClientConnection { conn = data.m_hConn, netId = data.m_info.m_identityRemote }
            );
        ...
        {
            _logger.Info(
                $"Steam connection closed by peer. Reason: {data.m_info.m_eEndReason}, {data.m_info.m_szEndDebug}"
            );
            CloseConnectionAndRemove(
                data.m_hConn,
                (SteamDisconnectionReason)data.m_info.m_eEndReason,
    ...
    )
    {
        ...
        if (num >= 0)
        {
            ...
            _handler.OnPeerDisconnected(
                clientConnection.netId.GetSteamID64(),

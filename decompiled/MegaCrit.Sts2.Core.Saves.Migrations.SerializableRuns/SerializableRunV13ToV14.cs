...
public class SerializableRunV13ToV14 : MigrationBase<SerializableRun>
{
    ...
    {
        Log.Info(
            "SerializableRun migration v13 -> v14: Migrating SerializableRun.CardPoolId to CardPoolIds"
        );
        if (
            !(saveData.GetRawNode("pre_finished_room") is JsonObject jsonObject)
            || !(jsonObject["extra_rewards"] is JsonObject jsonObject2)
        )
        {
            return;
        }

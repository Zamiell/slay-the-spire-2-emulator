...
public class NetHostGameService : INetHostHandler, INetHandler, INetHostGameService, INetGameService
{
    ...
    public ulong NetId =>
        (
            _netHost
            ?? throw new InvalidOperationException("Tried to get NetId while not connected!")
        ).NetId;
    ...
    public void SendMessage<T>(T message, ulong peerId)
        where T : INetMessage
    {
        ...
        where T : INetMessage
    {
        ...
        {
            int length;
            byte[] bytes = _messageBus.SerializeMessage(
                overrideSenderId ?? _netHost.NetId,
                message,
                out length
            );
    ...
    {
        if (
            !_messageBus.TryDeserializeMessage(
                packetBytes,
        ...
        {
            Log.Error(
                $"Tried to deserialize packet of size {packetBytes.Length} as message, but we were not able to!"
            );
            return;
        }
    ...
    {
        _connectedPeers.Add(new NetClientData { peerId = peerId, readyForBroadcasting = false });

...
public class NetMessageBus
{
    ...
    private readonly Dictionary<Type, List<CallbackPair>> _messageHandlers =
        new Dictionary<Type, List<CallbackPair>>();

    private readonly List<CallbackPair> _cachedPairList = new List<CallbackPair>();

    private bool _isBufferingMessages;

    private readonly List<(INetMessage, ulong)> _bufferedMessages =
        new List<(INetMessage, ulong)>();
    ...
    public void SendMessageToAllHandlers(INetMessage message, ulong senderId)
    {
        ...
        {
            _logger.Debug(
                $"Received message of type {message.GetType()} but we are currently buffering messages."
            );
            _bufferedMessages.Add((message, senderId));
            return;
        }
        if (
            !_messageHandlers.TryGetValue(message.GetType(), out List<CallbackPair> value)
            || value.Count == 0
        )
        {
            Log.Error(
                $"Received message of type {message.GetType()}, but no message handlers are registered for that type!"
            );
            return;
        }
        ...
        _logger.LogMessage(
            message.LogLevel,
            $"Received message {message}, sending to {_cachedPairList.Count} handlers",
            0
        );
        foreach (CallbackPair cachedPair in _cachedPairList)
        {
            ...
            {
                _logger.Error(
                    $"Exception encountered while processing message {message}: {value2}"
                );
            }
        }
    }
        ...
        where T : INetMessage
    {
        if (typeof(T) == typeof(INetMessage))
        {
            throw new InvalidOperationException(
                "RegisterMessageHandler must be called with a concrete implementation of INetMessage!"
            );
        }
        ...
        where T : INetMessage
    {
        ...
        {
            throw new InvalidOperationException(
                "UnregisterMessageHandler must be called with a concrete implementation of INetMessage!"
            );
        }
        if (_messageHandlers.TryGetValue(typeof(T), out List<CallbackPair> value))
        {
            value.RemoveAll(
                (CallbackPair p) =>
                    (Delegate?)(MessageHandlerDelegate<T>)p.originalHandler == (Delegate?)handler
            );
        }
    }
}

...
public class CombatState : ICombatState, ICardScope
{
    ...
    public IReadOnlyList<Creature> PlayerCreatures =>
        Creatures.Where((Creature c) => c.IsPlayer).ToList();

    public IReadOnlyList<Player> Players =>
        PlayerCreatures.Select((Creature c) => c.Player).ToList();
    ...
    public IReadOnlyList<Creature> HittableEnemies =>
        Enemies.Where((Creature e) => e.IsHittable).ToList();
    ...
    public Creature CreateCreature(MonsterModel monster, CombatSide side, string? slot)
    {
        ...
        monster.Rng = new Rng(
            (uint)(
                (RunState.Rng.Seed + RunState.CurrentMapCoord?.col)
                ?? ((long?)RunState.CurrentMapCoord?.row)
                ?? (RunState.CurrentActIndex + creature.CombatId.Value)
            )
        );
        ...
        else
        {
            if (!_allies.Contains(creature))
            {
                throw new InvalidOperationException(
                    $"Removed creature '{creature}' was not found."
                );
            }
    ...
    public async Task<Creature?> GetCreatureAsync(uint? combatId, double timeoutSec)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Timed out waiting for creature with target index {combatId} to spawn!"
            );
        }
        ...
        if (Encounter != null)
        {
            _enemies.Sort(
                (Creature a, Creature b) =>
                    Encounter.Slots.IndexOf<string>(a.SlotName)
                    - Encounter.Slots.IndexOf<string>(b.SlotName)
            );
        }
    }
    ...
    public void SetEnemyIndex(Creature creature, int index)
    {
        ...
        {
            throw new InvalidOperationException(
                "Cannot modify turn order of a combat with pre-set slots"
            );
        }
    ...
    private void AddCard(CardModel card)
    {
        ...
        {
            throw new InvalidOperationException(
                "Card " + card.Id.Entry + " combat state is set to a different combat."
            );
        }
        _allCards.Add(card);
    }
    ...
    public void AddCreature(Creature creature)
    {
        ...
        {
            throw new InvalidOperationException(
                "Creature is already in this combat, but AddCreature was called on it again."
            );
        }
        list.Add(creature);
        this.CreaturesChanged?.Invoke(this);
    ...
    private bool Contains(AbstractModel model)
    {
                    ...
                    if (!(model is CardModel cardModel))
                    {
                        if (!(model is AfflictionModel afflictionModel))
                        {
                            if (!(model is EnchantmentModel enchantmentModel))
                            {
                                ...
                                return !orbModel.HasBeenRemovedFromState
                                    && orbModel.Owner.IsActiveForHooks;
                            }
                            return enchantmentModel.HasCard
                                && !enchantmentModel.Card.HasBeenRemovedFromState
                                && enchantmentModel.Card.Owner.IsActiveForHooks;
                        }
                        return afflictionModel.HasCard
                            && !afflictionModel.Card.HasBeenRemovedFromState
                            && afflictionModel.Card.Owner.IsActiveForHooks;
                    }
        ...
        return powerModel.Owner.CombatState != null

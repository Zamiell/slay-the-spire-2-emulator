        ...
        if (TestMode.IsOn)
        {
            if (this.CombatStateChanged != null)
            {
                throw new InvalidOperationException(
                    "Backend should not be subscribing to CombatStateChanged!"
                );
            }
        }
        else if (
            _combatStateChangedDeferredTask == null
            || _combatStateChangedDeferredTask.IsCompleted
        )
        {
            _combatStateChangedDeferredTask = TaskHelper.RunSafely(
                CallCombatStateChangedDeferred()
            );
        }
    }

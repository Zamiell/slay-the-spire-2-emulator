...
public class CombatManager
{
    ...
    {
        get { return _playerActionsDisabled; }
        private set
        {
    ...
    public bool IsEnding
    {
        get
        {
            ...
            if (
                _state != null
                && _state.Enemies.Any((Creature e) => e != null && e.IsAlive && e.IsPrimaryEnemy)
            )
            {
                return false;
            }
    ...
    public void SetUpCombat(CombatState state)
    {
        if (_state != null)
        {
            throw new InvalidOperationException(
                "Make sure to reset the combat before setting up a new one."
            );
        }
        _state = state;
    ...
    private async Task StartTurn(Func<Task>? actionDuringEnemyTurn = null)
    {
        ...
        using (_playerReadyLock.EnterScope())
        {
            ...
            {
                creaturesStartingTurn = _playersTakingExtraTurn
                    .Select((Player p) => p.Creature)
                    .ToList();
                playersStartingTurn = _playersTakingExtraTurn.ToList();
            }
            else
            {
                creaturesStartingTurn =
                    _state?.CreaturesOnCurrentSide.ToList() ?? new List<Creature>();
                CombatState? state2 = _state;
                playersStartingTurn = (
                    (state2 == null || state2.CurrentSide != CombatSide.Player)
                        ? new List<Player>()
        ...
        if (state3 != null && state3.CurrentSide == CombatSide.Player)
        {
            ...
            int num =
                LocalContext.GetMe(playersStartingTurn)?.PlayerCombatState?.TurnNumber ?? (-1);
            if (num > 1)
            {
        ...
        List<(HookPlayerChoiceContext, Task)> setupPlayerTurnContext =
            new List<(HookPlayerChoiceContext, Task)>();
        foreach (Player item5 in playersStartingTurn)
        {
            if (LocalContext.NetId.HasValue)
            {
                HookPlayerChoiceContext playerChoiceContext = new HookPlayerChoiceContext(
                    item5,
        ...
        if (state4 != null && state4.CurrentSide == CombatSide.Player)
        {
            foreach (Player item6 in playersStartingTurn)
            {
                if (item6.PlayerCombatState != null && LocalContext.NetId.HasValue)
                {
                    ...
                    Task task2 = item6.PlayerCombatState.OrbQueue.AfterTurnStart(
            ...
            foreach (Player player2 in _state.Players)
            {
                ...
                {
                    Log.Info(
                        $"Setting player {player2.NetId} to ready at start of turn. IsDead: {player2.Creature.IsDead}. IsStartingTurn: {playersStartingTurn.Contains(player2)}"
                    );
            ...
            foreach (var item7 in playersStartingTurn.Zip(setupPlayerTurnContext))
            {
                ...
                {
                    Task task3 = RunAutoPrePlayPhase(
                        hookPlayerChoiceContext2,
                        setupPlayerTurnTask,
                        player
                    );
    ...
    private async Task SetupPlayerTurn(Player player, HookPlayerChoiceContext playerChoiceContext)
    {
        ...
        {
            Log.Warn(
                $"Combat state is null. Assuming that the run has been cleaned up. (CombatState: {_state} PlayerCombatState: {player.PlayerCombatState})"
            );
            return;
        }
        CombatState state = _state;
        ...
        if (player.PlayerCombatState.TurnNumber == 1)
        {
            ...
            List<CardModel> list = pile
                .Cards.Where((CardModel c) => c.Enchantment?.ShouldStartAtBottomOfDrawPile ?? false)
                .ToList();
            foreach (CardModel item in list)
            {
                pile.MoveToBottomInternal(item);
            }
            List<CardModel> list2 = pile
                .Cards.Where((CardModel c) => c.Keywords.Contains(CardKeyword.Innate))
                .Except(list)
                .ToList();
    ...
    )
    {
        ...
        if (AllPlayersReadyToEndTurn())
        {
            ...
            GameAction currentlyRunningAction = RunManager
                .Instance
                .ActionExecutor
                .CurrentlyRunningAction;
            if (
                currentlyRunningAction != null
                && ActionQueueSet.IsGameActionPlayerDriven(currentlyRunningAction)
            )
            {
    ...
    public void SetReadyToBeginEnemyTurn(Player player, Func<Task>? actionDuringEnemyTurn = null)
    {
        ...
        using (_playerReadyLock.EnterScope())
        {
            ...
            flag =
                _playersReadyToBeginEnemyTurn.Count == _state.Players.Count
    ...
    private async Task EndEnemyTurn()
    {
        if (IsInProgress)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"EndEnemyTurn called while the current side is {_state.CurrentSide}!"
                );
            }
            await WaitForUnpause();
            CombatCt.ThrowIfCancellationRequested();
    ...
    public async Task EndCombatInternal()
    {
        ...
        bool flag =
            ...
            && runState.CurrentMapCoord == runState.Map.SecondBossMapPoint.coord;
        bool flag2 =
            runState.Map.SecondBossMapPoint == null
    ...
    public async Task EndPlayerTurnPhaseOneInternal()
    {
        ...
        {
            throw new InvalidOperationException(
                $"EndPlayerTurn called while the current side is {_state?.CurrentSide}!"
            );
        }
        await WaitForUnpause();
        ...
        {
            playersEndingTurn = (
                (_playersTakingExtraTurn.Count > 0)
                    ? _playersTakingExtraTurn.ToList()
                    : (_state?.Players.ToList() ?? new List<Player>())
            );
        }
        List<(Player, HookPlayerChoiceContext)> autoPostPlayContexts =
            new List<(Player, HookPlayerChoiceContext)>();
        ...
        RunManager.Instance.ChecksumTracker.GenerateChecksum(
            "After player turn phase one end",
            null
        );
        await CheckWinCondition();
    }
    ...
    private async Task DoTurnEnd(Player player, PlayerChoiceContext choiceContext)
    {
        ...
        foreach (CardModel card in pile.Cards)
        {
            if (card.HasTurnEndInHandEffect)
            {
                ...
                && Hook.ShouldEtherealTrigger(player.Creature.CombatState, card)
            )
            {
                list.Add(card);
            }
    ...
    public async Task EndPlayerTurnPhaseTwoInternal()
    {
        ...
        {
            throw new InvalidOperationException(
                $"EndPlayerTurnPhaseTwo called while the current side is {_state.CurrentSide}!"
            );
        }
        List<Player> playersEndingTurn;
        using (_playerReadyLock.EnterScope())
    ...
    public async Task SwitchFromPlayerToEnemySide(Func<Task>? actionDuringEnemyTurn = null)
    {
        ...
        using (_playerReadyLock.EnterScope())
        {
            ...
            foreach (Player player in _state.Players)
            {
                ...
                {
                    Log.Info(
                        $"Player {player.NetId} ({player.Character.Id.Entry}) is taking an extra turn"
                    );

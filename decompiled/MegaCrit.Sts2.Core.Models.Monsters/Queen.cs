...
public sealed class Queen : MonsterModel
{
    ...
    public override IEnumerable<string> AssetPaths =>
        base.AssetPaths.Concat(ModelDb.Monster<TorchHeadAmalgam>().AssetPaths);

    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 419, 400);
    ...
    private int ExecutionDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 18, 15);

    private bool HasAmalgamDied
    {
        get { return _hasAmalgamDied; }
        set
        {
            AssertMutable();
    ...
    {
        get { return _burnBrightForMeState; }
        set
        {
            AssertMutable();
    ...
    public override void BeforeRemovedFromRoom()
    {
        ...
        {
            NCombatRoom
                .Instance?.GetCreatureNode(base.Creature)
                ?.SpineAnimation.SetAnimation("tracks/empty", loop: true, 1);
        }
    }
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        ConditionalBranchState conditionalBranchState2 = new ConditionalBranchState(
            "BURN_BRIGHT_FOR_ME_BRANCH"
        );
        MoveState moveState3 = new MoveState(
            "OFF_WITH_YOUR_HEAD_MOVE",
            OffWithYourHeadMove,
            new MultiAttackIntent(OffWithYourHeadDamage, 5)
        );
        MoveState moveState4 = new MoveState(
            "EXECUTION_MOVE",
            ExecutionMove,
            new SingleAttackIntent(ExecutionDamage)
        );
    ...
    private async Task PuppetStringsMove(IReadOnlyList<Creature> targets)
    {
        ...
        await PowerCmd.Apply<ChainsOfBindingPower>(
    ...
    private async Task BurnBrightForMeMove(IReadOnlyList<Creature> targets)
    {
        ...
        int strengthAmount = AscensionHelper.GetValueIfAscension(
    ...
    {
        if (
            moveStateId != "PUPPET_STRINGS_MOVE"
            && moveStateId != "BURN_BRIGHT_FOR_ME_MOVE"
            && moveStateId != "ENRAGE_MOVE"
        )
        {

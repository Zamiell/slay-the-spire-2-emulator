...
public sealed class TwoTailedRat : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 18, 17);

    public override int MaxInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 22, 21);
    ...
    {
        get { return _turnsUntilSummonable; }
        set
        {
            AssertMutable();
    ...
    {
        get { return _callForBackupCount; }
        set
        {
            AssertMutable();
    ...
    public override void SetupSkins(MegaSprite spine, MegaSkeleton skeleton)
    {
        ...
        megaSkin.AddSkin(
            data.FindSkin(MegaCrit.Sts2.Core.Random.Rng.Chaotic.NextItem(_barnacleOptions))
        );
        megaSkin.AddSkin(
            data.FindSkin(MegaCrit.Sts2.Core.Random.Rng.Chaotic.NextItem(_headOptions))
        );
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState = new MoveState(
            "SCRATCH_MOVE",
            ScratchMove,
            new SingleAttackIntent(ScratchDamage)
        );
        MoveState moveState2 = new MoveState(
            "DISEASE_BITE_MOVE",
            DiseaseBiteMove,
            new SingleAttackIntent(DiseaseBiteDamage)
        );
        ...
        RandomBranchState randomBranchState = (RandomBranchState)(
            moveState4.FollowUpState = (
                moveState3.FollowUpState = (
                    moveState2.FollowUpState = (
                        moveState.FollowUpState = new RandomBranchState("RAND")
                    )
                )
            )
        );
        randomBranchState.AddBranch(
            moveState,
    ...
    private async Task CallForBackup(IReadOnlyList<Creature> targets)
    {
        ...
        string nextSlot = base.CombatState.Encounter.Slots.LastOrDefault(
            (string s) => base.CombatState.Enemies.All((Creature c) => c.SlotName != s),
            string.Empty
        );
        ...
        List<TwoTailedRat> list = base
            .Creature.CombatState.Enemies.Select((Creature c) => c.Monster)
            .OfType<TwoTailedRat>()
            .ToList();
        int maxCallForBackupCount = list.Max((TwoTailedRat c) => c.CallForBackupCount + 1);
        list.ForEach(
            delegate(TwoTailedRat r)
            {
    ...
    private bool CanSummon()
    {
        ...
        List<Creature> list = (
            from c in base.Creature.CombatState.GetTeammatesOf(base.Creature)

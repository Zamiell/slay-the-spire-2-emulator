...
public sealed class CeremonialBeast : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 262, 252);

    public override int MaxInitialHp => MinInitialHp;

    private int PlowAmount =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 160, 150);
    ...
    {
        get { return _isStunnedByPlowRemoval; }
        set
        {
            AssertMutable();
    ...
    private async Task PlowMove(IReadOnlyList<Creature> targets)
    {
        ...
        await Cmd.Wait(0.5f);
        base.Creature.GetVfxContainer()
            ?.AddChildSafely(
                NHorizontalLinesVfx.Create(
                    new Color("BFFFC880"),
                    1.2000000476837158,
        ...
        NGame.Instance?.ScreenShake(
            ...
            180f + MegaCrit.Sts2.Core.Random.Rng.Chaotic.NextFloat(-10f, 10f)
        );
        await DamageCmd.Attack(PlowDamage).FromMonster(this).WithNoAttackerAnim().Execute(null);

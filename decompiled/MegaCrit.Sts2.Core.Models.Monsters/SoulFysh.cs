...
public sealed class SoulFysh : MonsterModel
{
    ...
    private const string _intangibleSfx =
    ...
    private const string _reappearSfx =
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 221, 211);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState = new MoveState(
            "BECKON_MOVE",
            BeckonMove,
            new StatusIntent(BeckonMoveAmount)
        );
        MoveState moveState2 = new MoveState(
            "DE_GAS_MOVE",
            DeGasMove,
            new SingleAttackIntent(DeGasDamage)
        );
    ...
    private async Task BeckonMove(IReadOnlyList<Creature> targets)
    {
        ...
        foreach (Creature target in targets)
        {
            ...
            array[0] = await CardPileCmd.AddGeneratedCardToCombat(
                card,
                PileType.Draw,
                null,
    ...
    private async Task GazeMove(IReadOnlyList<Creature> targets)
    {
        await DamageCmd
            .Attack(GazeDamage)
            .FromMonster(this)
            .WithAttackerAnim("AttackBeckon", 0.6f)
    ...
    private async Task ScreamMove(IReadOnlyList<Creature> targets)
    {
        ...
        await DamageCmd
            .Attack(ScreamDamage)
            .FromMonster(this)
            .WithAttackerAnim("AttackDebuffTrigger", 0.65f)

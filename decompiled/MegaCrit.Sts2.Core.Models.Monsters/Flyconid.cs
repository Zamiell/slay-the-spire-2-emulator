...
public sealed class Flyconid : MonsterModel
{
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 51, 47);

    public override int MaxInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 53, 49);
    ...
    private async Task VulnerableSporesMove(IReadOnlyList<Creature> targets)
    {
        if (TestMode.IsOff)
        {
            base.Creature.GetCreatureNode()
                ?.Visuals.GetCurrentBody()
                .GetNode<NFlyconidSporesVfx>("%VfxController")
                ?.SetSporeTypeIsVulnerable(isVulnerable: true);
        }
        SfxCmd.Play(CastSfx);
    ...
    private async Task FrailSporesMove(IReadOnlyList<Creature> targets)
    {
        if (TestMode.IsOff)
        {
            base.Creature.GetCreatureNode()
                ?.Visuals.GetCurrentBody()
                .GetNode<NFlyconidSporesVfx>("%VfxController")
                ?.SetSporeTypeIsVulnerable(isVulnerable: false);
        }
        await DamageCmd
            .Attack(SporeDamage)
            .FromMonster(this)

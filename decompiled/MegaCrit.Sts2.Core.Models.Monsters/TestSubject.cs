...
public sealed class TestSubject : MonsterModel
{
    ...
    public int FirstFormHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 111, 100);

    public int SecondFormHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 212, 200);

    public int ThirdFormHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 313, 300);

    private int EnrageAmount =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 3, 2);

    private int BiteDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 22, 20);
    ...
    {
        get { return _extraMultiClawCount; }
        set
        {
            AssertMutable();
    ...
    public override async Task AfterAddedToRoom()
    {
        ...
        await PowerCmd.Apply<AdaptablePower>(
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState3 = new MoveState(
            "MULTI_CLAW_MOVE",
            MultiClawMove,
            new MultiAttackIntent(MultiClawDamage, () => MultiClawTotalCount)
        );
        MoveState moveState4 = new MoveState(
            "PHASE3_LACERATE_MOVE",
            Phase3LacerateMove,
            new MultiAttackIntent(Phase3LacerateDamage, 3)
        );
        MoveState moveState5 = new MoveState(
            "BIG_POUNCE",
            BigPounceMove,
            new SingleAttackIntent(BigPounceDamage)
        );
        MoveState moveState6 = new MoveState(
            "BURNING_GROWL_MOVE",
            BurningGrowlMove,
        ...
        if (base.Creature.CombatState != null)
        {
            ...
            switch (Respawns)
            {
                case 1:
                    ...
                    await PowerCmd.Apply<PainfulStabsPower>(
                        new ThrowingPlayerChoiceContext(),
                        base.Creature,
                        1m,
                        base.Creature,
                ...
                case 2:
                    ...
                    await PowerCmd.Apply<NemesisPower>(
                        new ThrowingPlayerChoiceContext(),
                        base.Creature,
                        1m,
                        base.Creature,
    ...
    private async Task BiteMove(IReadOnlyList<Creature> targets)
    {
        await DamageCmd
            .Attack(BiteDamage)
            .FromMonster(this)
            .WithAttackerAnim("BiteTrigger", 0.25f)
    ...
    private async Task BurningGrowlMove(IReadOnlyList<Creature> targets)
    {
        ...
        await CardPileCmd.AddToCombatAndPreview<Burn>(
            targets,
            PileType.Discard,
            BurningGrowlBurnCount,
            null
        );
        await PowerCmd.Apply<StrengthPower>(
            new ThrowingPlayerChoiceContext(),
            base.Creature,
            BurningGrowlStrengthGain,
            base.Creature,
            null
        );
    }
    ...
    {
        AssertMutable();
        decimal scaledHp = MegaCrit.Sts2.Core.Entities.Creatures.Creature.ScaleHpForMultiplayer(
            baseRespawnHp,
            base.CombatState.Encounter,
            base.CombatState.Players.Count,
            base.CombatState.RunState.CurrentActIndex
        );
    ...
    {
        (NCombatRoom.Instance?.GetCreatureNode(base.Creature))
            ?.GetSpecialNode<CanvasGroup>("%CanvasGroup")
    ...
    public override CreatureAnimator GenerateAnimator(MegaSprite controller)
    {
        ...
        AnimState animState7 = new AnimState("regenerate1") { BoundsContainer = "RespawnBounds1" };
        ...
        AnimState animState13 = new AnimState("regenerate2") { BoundsContainer = "RespawnBounds2" };

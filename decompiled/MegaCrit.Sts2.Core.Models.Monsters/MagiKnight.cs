...
{
    private static readonly LocString _dampenDialogue = new LocString(
        "powers",
        "DAMPEN_POWER.banter"
    );
    ...
    private int PowerShieldBlock =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 9, 5);

    private int SpearDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 11, 10);

    private int BombDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 40, 35);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState = new MoveState(
            "POWER_SHIELD_MOVE",
            PowerShieldMove,
            new SingleAttackIntent(PowerShieldDamage),
            new DefendIntent()
        );
        ...
        MoveState moveState4 = new MoveState(
            "MAGIC_BOMB",
            MagicBombMove,
            new SingleAttackIntent(BombDamage)
        );
        MoveState moveState5 = new MoveState(
            "RAM_MOVE",
            SpearMove,
            new SingleAttackIntent(SpearDamage)
        );
    ...
    {
        await DamageCmd
            .Attack(PowerShieldDamage)
            .FromMonster(this)
            .WithAttackerAnim("ShieldAttack", 0.6f)
            .WithAttackerFx(
                null,
                "event:/sfx/enemy/enemy_attacks/magi_knight/magi_knight_cast_shield"
            )
    ...
    private async Task MagicBombMove(IReadOnlyList<Creature> targets)
    {
        if (TestMode.IsOff && base.CombatState.IsLiveCombat())
        {
            ...
            Node2D specialNode = creatureNode2.GetSpecialNode<Node2D>(
                "Visuals/AttackDistanceControl"
            );
            if (specialNode != null && vector.HasValue)
            {
                float x = creatureNode2.Visuals.GetCurrentBody().Scale.X;
                specialNode.Position =
                    Vector2.Left * ((creatureNode2.GlobalPosition.X - vector.Value.X - 600f) / x);
            }
        }
        await DamageCmd
            .Attack(BombDamage)
            .FromMonster(this)
            .WithAttackerAnim("BombCast", 1.2f)
            .WithAttackerFx(
                null,
                "event:/sfx/enemy/enemy_attacks/magi_knight/magi_knight_attack_bomb"
            )
    ...
    private async Task SpearMove(IReadOnlyList<Creature> targets)
    {
        await DamageCmd
            .Attack(SpearDamage)
            .FromMonster(this)
            .WithAttackerAnim("RamAttack", 1.2f)
            .WithAttackerFx(
                null,
                "event:/sfx/enemy/enemy_attacks/magi_knight/magi_knight_attack_ram"
            )

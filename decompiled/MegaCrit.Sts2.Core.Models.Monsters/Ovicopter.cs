...
public sealed class Ovicopter : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 126, 124);

    public override int MaxInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 132, 130);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState3 = new MoveState(
            "TENDERIZER_MOVE",
            TenderizerMove,
            new SingleAttackIntent(TenderizerDamage),
            new DebuffIntent()
        );
        MoveState moveState4 = new MoveState(
            "NUTRITIONAL_PASTE_MOVE",
            NutritionalPasteMove,
            new BuffIntent()
        );
    ...
    private async Task LayEggsMove(IReadOnlyList<Creature> targets)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            string text = base.CombatState.Encounter?.Slots.LastOrDefault(

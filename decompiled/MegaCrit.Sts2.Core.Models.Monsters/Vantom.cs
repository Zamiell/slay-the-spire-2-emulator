...
public sealed class Vantom : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 183, 173);

    public int SlipperyAmt =>
    ...
    private int DismemberDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 30, 26);
    ...
    public void ScaleTo(float scale, float duration)
    {
        Node2D node2D = base
            ...
            ?.GetSpecialNode<Node2D>("Visuals/ScalingBone");
        if (node2D != null)
        {
            ScaleTween?.FastForwardToCompletion();
            ScaleTween = node2D.CreateTween();
            ScaleTween
                .TweenProperty(node2D, "scale", Vector2.One * scale, duration)

...
public sealed class InfestedPrism : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 171, 161);
    ...
    private int PulsateBlock =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 22, 20);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState2 = new MoveState(
            "RADIATE_MOVE",
            RadiateMove,
            new SingleAttackIntent(RadiateDamage),
            new DefendIntent()
        );
        MoveState moveState3 = new MoveState(
            "WHIRLWIND_MOVE",
            WhirlwindMove,
            new MultiAttackIntent(WhirlwindDamage, WhirlwindRepeat)
        );
        MoveState moveState4 = new MoveState(
            "PULSATE_MOVE",
            PulsateMove,
            new SingleAttackIntent(PulsateDamage),
            new BuffIntent(),
            new DefendIntent()
        );

...
public sealed class WaterfallGiant : MonsterModel
{
    ...
    private const string _ambientSfx =
        "event:/sfx/enemy/enemy_attacks/waterfall_giant/waterfall_giant_ambient";
    ...
    public int SiphonHeal =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 15, 10);

    private int PressurizeAmount =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 20, 15);
    ...
    private int BasePressureGunDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 23, 20);
    ...
    {
        get { return _currentPressureGunDamage; }
        set
        {
            AssertMutable();
    ...
    {
        get { return _steamEruptionDamage; }
        set
        {
            AssertMutable();
    ...
    {
        get { return _pressureBuildupIdx; }
        set
        {
            AssertMutable();
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState5 = new MoveState(
            "PRESSURE_GUN_MOVE",
            PressureGunMove,
            new SingleAttackIntent(() => CurrentPressureGunDamage),
            new BuffIntent()
        );
        MoveState moveState6 = new MoveState(
            "PRESSURE_UP_MOVE",
            PressureUpMove,
            new SingleAttackIntent(PressureUpDamage),
            new BuffIntent()
        );
        ...
        MoveState moveState7 = new MoveState(
            "EXPLODE_MOVE",
            ExplodeMove,
            new DeathBlowIntent(() => SteamEruptionDamage)
        );
    ...
    private async Task PressurizeMove(IReadOnlyList<Creature> targets)
    {
        ...
        await PowerCmd.Apply<SteamEruptionPower>(
    ...
    {
        await DamageCmd
            .Attack(PressureUpDamage)
            .FromMonster(this)
            .WithAttackerAnim("AttackBuff", 0.15f)
            .WithAttackerFx(
                null,
                "event:/sfx/enemy/enemy_attacks/waterfall_giant/waterfall_giant_attack_stomp"
            )
    ...
    private async Task StompMove(IReadOnlyList<Creature> targets)
    {
        await DamageCmd
            .Attack(StompDamage)
            .FromMonster(this)
            .WithAttackerAnim("AttackDebuff", 0.3f)
            .WithAttackerFx(
                null,
                "event:/sfx/enemy/enemy_attacks/waterfall_giant/waterfall_giant_attack_stomp"
            )
    ...
    {
        await DamageCmd
            .Attack(CurrentPressureGunDamage)
            .FromMonster(this)
            .WithAttackerAnim("Attack", 0.3f)
            .WithAttackerFx(
                null,
    ...
    private void IncrementBuildUpAnimationTrack()
    {
        if (TestMode.IsOff)
        {
            ...
            base.Creature.GetCreatureNode()
                ?.SpineAnimation.SetAnimation($"_tracks/buildup{value}", loop: true, 1);
        }
    }

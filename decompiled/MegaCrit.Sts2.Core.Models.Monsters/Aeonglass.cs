...
public sealed class Aeonglass : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 535, 512);
    ...
    private int EbbDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 32, 26);
    ...
    {
        get { return _witherUpgradeCount; }
        set
        {
            AssertMutable();
    ...
    private int IncreasingIntensityTotalStrength =>
        IncreasingIntensityBaseStrength + AdditionalStrength;
    ...
    public override async Task AfterAddedToRoom()
    {
        ...
        {
            WitheringPresencePower witheringPresencePower = (WitheringPresencePower)
                ModelDb.Power<WitheringPresencePower>().ToMutable();
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState = new MoveState(
            "EBB_MOVE",
            EbbMove,
            new SingleAttackIntent(EbbDamage),
            new DebuffIntent()
        );
        MoveState moveState2 = new MoveState(
            "EYE_LASERS_MOVE",
            EyeLasersMove,
            new MultiAttackIntent(EyeLasersDamage, EyeLasersRepeat)
        );
        MoveState moveState3 = new MoveState(
            "INCREASING_INTENSITY_MOVE",
            IncreasingIntensityMove,
            new StatusIntent(WitherAmount),
            new BuffIntent(),
            new DefendIntent()
        );
    ...
    {
        await DamageCmd
            .Attack(EyeLasersDamage)
            .FromMonster(this)
            .WithHitCount(EyeLasersRepeat)
            .WithAttackerAnim("Attack", 0.15f)
            .WithHitFx("vfx/vfx_attack_blunt")
            .Execute(null);
    }
    ...
    private async Task IncreasingIntensityMove(IReadOnlyList<Creature> targets)
    {
        ...
        await PowerCmd.Apply<StrengthPower>(
            new ThrowingPlayerChoiceContext(),
            base.Creature,
            IncreasingIntensityTotalStrength,
            base.Creature,
            null
        );
        AdditionalStrength++;

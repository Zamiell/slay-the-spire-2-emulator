...
public sealed class Axebot : MonsterModel
{
    ...
    private int BootUpBlock =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 15, 10);
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 76, 70);

    public override int MaxInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 86, 78);

    public override DamageSfxType TakeDamageSfxType => DamageSfxType.Armor;

    public int StockAmount
    {
        get { return _stockOverrideAmount ?? 2; }
        set
        {
            AssertMutable();
    ...
    {
        get { return _shouldPlaySpawnAnimation; }
        set
        {
            AssertMutable();
    ...
    public override async Task AfterAddedToRoom()
    {
        if (StockAmount > 0)
        {
            await PowerCmd.Apply<StockPower>(
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState2 = new MoveState(
            "ONE_TWO_MOVE",
            OneTwoMove,
            new MultiAttackIntent(OneTwoDamage, 2)
        );
        MoveState moveState3 = (MoveState)(
            moveState.FollowUpState = new MoveState(
    ...
    {
        await DamageCmd
            .Attack(HammerUppercutDamage)
            .FromMonster(this)
            .WithAttackerAnim("uppercut", 0.8f)
    ...
    public override CreatureAnimator GenerateAnimator(MegaSprite controller)
    {
        ...
        CreatureAnimator creatureAnimator = new CreatureAnimator(
            _shouldPlaySpawnAnimation ? animState5 : animState,
            controller
        );

...
{
    private static readonly LocString _forbiddenIncantationDialogue = new LocString(
        "monsters",
        "DEVOTED_SCULPTOR.moves.FORBIDDEN_INCANTATION.banter"
    );

    private readonly int _ritualGain = 9;

    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 172, 162);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState2 = (MoveState)(
            moveState.FollowUpState = new MoveState(
                "SAVAGE_MOVE",
                SavageMove,
                new SingleAttackIntent(SavageDamage)
            )
        );

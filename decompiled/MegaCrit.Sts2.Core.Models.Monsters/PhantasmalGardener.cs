...
public sealed class PhantasmalGardener : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 27, 26);
    ...
    private int LashDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 7, 7);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState3 = new MoveState(
            "FLAIL_MOVE",
            FlailMove,
            new MultiAttackIntent(FlailDamage, FlailRepeat)
        );
    ...
    private async Task BiteMove(IReadOnlyList<Creature> targets)
    {
        NCombatRoom
            ...
            ?.SetDefaultScaleTo(CurrentScale, 0.75f);
        await DamageCmd
            .Attack(BiteDamage)
            .FromMonster(this)
    ...
    private async Task FlailMove(IReadOnlyList<Creature> targets)
    {
        NCombatRoom
            .Instance?.GetCreatureNode(base.Creature)
            ?.SetDefaultScaleTo(CurrentScale, 0.35f);
        await DamageCmd
            .Attack(FlailDamage)
            .WithHitCount(FlailRepeat)
            .OnlyPlayAnimOnce()
            .FromMonster(this)
    ...
    public override CreatureAnimator GenerateAnimator(MegaSprite controller)
    {
        ...
        creatureAnimator.AddAnyState(
            "Hit",
            animState5,
            () => !base.Creature.GetPower<SkittishPower>().HasGainedBlockThisTurn
        );
        creatureAnimator.AddAnyState(
            "Hit",
            animState6,

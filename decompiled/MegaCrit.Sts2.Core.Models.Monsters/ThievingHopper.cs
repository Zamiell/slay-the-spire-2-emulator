...
public sealed class ThievingHopper : MonsterModel
{
    private static readonly Func<CardModel, bool>[] _stealPriorities = new Func<CardModel, bool>[4]
    {
        ...
        delegate(CardModel c)
        {
            ...
            if (flag2)
            {
                ...
                bool flag3 = (
                    (rarity == CardRarity.Basic || rarity == CardRarity.Quest) ? true : false
                );
                flag2 = flag3;
            }
            return flag2;
    ...
    private const string _stealSfx =
    ...
    public const string hoverLoop =
        "event:/sfx/enemy/enemy_attacks/thieving_hopper/thieving_hopper_hover_loop";

    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 84, 79);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState2 = new MoveState(
            "NAB_MOVE",
            NabMove,
            new SingleAttackIntent(NabDamage)
        );
        MoveState moveState3 = new MoveState(
            "HAT_TRICK_MOVE",
            HatTrickMove,
            new SingleAttackIntent(HatTrickDamage)
        );
    ...
    private async Task ThieveryMove(IReadOnlyList<Creature> targets)
    {
        ...
        if (creatureNode != null)
        {
            ...
            if (specialNode != null && creatureNode2 != null)
            {
                specialNode.Position =
                    Vector2.Right
                    * (creatureNode2.GlobalPosition.X - creatureNode.GlobalPosition.X);
            }
        }
        ...
        foreach (Creature target in targets)
        {
            ...
            List<CardModel> list = (
                from c in CardPile.GetCards(
                    target.Player ?? target.PetOwner,
                    PileType.Draw,
                    PileType.Discard
                )
    ...
    private async Task FlutterMove(IReadOnlyList<Creature> targets)
    {
        ...
        await PowerCmd.Apply<FlutterPower>(
            new ThrowingPlayerChoiceContext(),
            base.Creature,
            5m,
            base.Creature,

...
public sealed class LivingFog : MonsterModel
{
    ...
    private const string _attackBlowSfx =
    ...
    private const string _appearsSfx =
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState = new MoveState(
            "ADVANCED_GAS_MOVE",
            AdvancedGasMove,
            new SingleAttackIntent(AdvancedGasDamage),
            new CardDebuffIntent()
        );
        MoveState moveState2 = new MoveState(
            "BLOAT_MOVE",
            BloatMove,
            new SingleAttackIntent(BloatDamage),
            new SummonIntent()
        );
        MoveState moveState3 = new MoveState(
            "SUPER_GAS_BLAST_MOVE",
            SuperGasBlastMove,
            new SingleAttackIntent(SuperGasBlastDamage)
        );
    ...
    {
        await DamageCmd
            .Attack(AdvancedGasDamage)
            .FromMonster(this)
            .WithAttackerAnim("Cast", 1.25f)
            .WithAttackerFx(
                null,
                "event:/sfx/enemy/enemy_attacks/living_fog/living_fog_attack_blow"
            )
            ...
            .Execute(null);
        await PowerCmd.Apply<SmoggyPower>(

...
public sealed class KinFollower : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 62, 58);

    public override int MaxInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 63, 59);

    private int QuickSlashDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 5, 5);

    private int BoomerangDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 2, 2);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState = new MoveState(
            "QUICK_SLASH_MOVE",
            QuickSlashMove,
            new SingleAttackIntent(QuickSlashDamage)
        );
        MoveState moveState2 = new MoveState(
            "BOOMERANG_MOVE",
            BoomerangMove,
            new MultiAttackIntent(BoomerangDamage, 2)
        );
    ...
    {
        await DamageCmd
            .Attack(QuickSlashDamage)
            .FromMonster(this)
            .WithAttackerAnim("SlashTrigger", 0.2f)
            .WithAttackerFx(
                null,
                "event:/sfx/enemy/enemy_attacks/the_kin_minion/the_kin_minion_quick_slash"
            )
        ...
        if (TestMode.IsOff)
        {
            ...
            if (specialNode != null && vector.HasValue)
            {
                specialNode.Position =
                    Vector2.Left
                    * (creatureNode2.GlobalPosition.X - vector.Value.X)
                    / creatureNode2.Body.Scale;
            }
        }
        await DamageCmd
            .Attack(BoomerangDamage)
            .WithHitCount(2)
            .FromMonster(this)

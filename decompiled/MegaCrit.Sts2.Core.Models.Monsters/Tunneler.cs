...
public sealed class Tunneler : MonsterModel
{
    ...
    private int BlockGain =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 37, 32);

    private int BelowDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 26, 23);
    ...
    private async Task BurrowMove(IReadOnlyList<Creature> targets)
    {
        ...
        await PowerCmd.Apply<BurrowedPower>(
            ...
            if (node2D != null && creatureNode != null)
            {
                if (targets.Count > 0)
                {
                    node2D.Position =
                        Vector2.Right
                        * (
                            targets[0].GetCreatureNode().GlobalPosition.X
                            - creatureNode.GlobalPosition.X
                        )
                        * 3f
                        / creatureNode.Visuals.Scale;
                }
                else
                {
    ...
    public override CreatureAnimator GenerateAnimator(MegaSprite controller)
    {
        ...
        creatureAnimator.AddAnyState(
            "Hit",
            animState2,
            () => !base.Creature.HasPower<BurrowedPower>() && !IsStunned
        );
        creatureAnimator.AddAnyState(
            "Hit",
            animState5,
        ...
        creatureAnimator.AddAnyState(
            "Attack",
            animState3,
            () => !base.Creature.HasPower<BurrowedPower>()
        );
        creatureAnimator.AddAnyState(
            "BurrowAttack",
            animState8,

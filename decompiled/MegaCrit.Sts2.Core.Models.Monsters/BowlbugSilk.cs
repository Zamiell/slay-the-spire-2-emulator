...
public sealed class BowlbugSilk : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 41, 40);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState2 = (MoveState)(
            moveState.FollowUpState = new MoveState("TOXIC_SPIT_MOVE", SpitMove, new DebuffIntent())
        );
    ...
    private async Task SpitMove(IReadOnlyList<Creature> targets)
    {
        if (TestMode.IsOff)
        {
            ...
            {
                node2D.Position =
                    Vector2.Right * (vector.Value.X - creatureNode2.GlobalPosition.X) * 4f;
            }
        }

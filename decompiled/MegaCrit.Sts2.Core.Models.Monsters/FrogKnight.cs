...
public sealed class FrogKnight : MonsterModel
{
    ...
    private const string _chargeSfx =
        "event:/sfx/enemy/enemy_attacks/frog_knight/frog_knight_charge";

    private const string _tongueLashSfx =
        "event:/sfx/enemy/enemy_attacks/frog_knight/frog_knight_tongue_lash";

    private bool _hasBeetleCharged;

    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 199, 191);

    public override int MaxInitialHp => MinInitialHp;

    private int StrikeDownEvilDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 23, 21);
    ...
    private int PlatingAmount =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 19, 15);

    private bool HasBeetleCharged
    {
        get { return _hasBeetleCharged; }
        set
        {
            AssertMutable();
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState2 = new MoveState(
            "STRIKE_DOWN_EVIL",
            StrikeDownEvilMove,
            new SingleAttackIntent(StrikeDownEvilDamage)
        );
        ...
        MoveState moveState4 = new MoveState(
            "BEETLE_CHARGE",
            BeetleChargeMove,
            new SingleAttackIntent(BeetleChargeDamage)
        );
    ...
    {
        await DamageCmd
            .Attack(TongueLashDamage)
            .FromMonster(this)
            .WithAttackerAnim("Lash", 0.3f)
            .WithAttackerFx(
                null,
                "event:/sfx/enemy/enemy_attacks/frog_knight/frog_knight_tongue_lash"
            )
    ...
    private async Task BeetleChargeMove(IReadOnlyList<Creature> targets)
    {
        ...
        await DamageCmd
            .Attack(BeetleChargeDamage)
            .FromMonster(this)
            .WithAttackerAnim("charge", 0.6f)

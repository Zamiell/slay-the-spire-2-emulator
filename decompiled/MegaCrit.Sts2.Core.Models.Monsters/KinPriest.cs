...
public sealed class KinPriest : MonsterModel
{
    ...
    private static readonly LocString _ritualApplyLine = MonsterModel.L10NMonsterLookup(
        "KIN_PRIEST.moves.RITUAL.speakLine1"
    );

    private static readonly LocString _followersDeathLine = MonsterModel.L10NMonsterLookup(
        "KIN_PRIEST.followersDeathLine"
    );
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 199, 190);
    ...
    )
    {
        if (creature.Monster is KinFollower)
        {
            ...
            IReadOnlyList<Creature> readOnlyList = base.Creature.CombatState?.GetTeammatesOf(
                base.Creature
            );
            ...
            if (
                !readOnlyList.Any(
                    (Creature c) => c != null && c.Monster is KinFollower && c.IsAlive
                )
            )
            {
                Creature creature2 = readOnlyList.FirstOrDefault(
                    (Creature c) => c != null && c.Monster is KinPriest && c.IsAlive
                );
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState3 = new MoveState(
            "BEAM_MOVE",
            BeamMove,
            new MultiAttackIntent(BeamDamage, 3)
        );
            ...
            .AfterAttackerAnim(
                delegate
                {
                    (NCombatRoom.Instance?.GetCreatureNode(base.Creature))
                        ?.GetSpecialNode<NKinPriestBeamVfx>("Visuals/Beam")
                        ?.Fire();
                    SfxCmd.Play(
                        "event:/sfx/enemy/enemy_attacks/the_kin_priest/the_kin_priest_soul_beam"
                    );

...
public sealed class Fabricator : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 155, 150);
    ...
    private bool CanFabricate =>
        base.Creature.CombatState.GetTeammatesOf(base.Creature).Count((Creature c) => c.IsAlive)
        < 4;
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState2 = new MoveState(
            ...
            FabricatingStrikeMove,
            new SingleAttackIntent(FabricatingStrikeDamage),
            new SummonIntent()
        );
        MoveState moveState3 = new MoveState(
            "DISINTEGRATE_MOVE",
            DisintegrateMove,
            new SingleAttackIntent(DisintegrateDamage)
        );
    ...
    {
        await DamageCmd
            .Attack(FabricatingStrikeDamage)
            .FromMonster(this)
    ...
    private async Task SpawnBot(IEnumerable<MonsterModel> options)
    {
        ...
        {
            List<MonsterModel> items = options
                .Where((MonsterModel m) => m != _lastSpawned)
                .ToList();
            Creature target = await CreatureCmd.Add(
                (_lastSpawned = base.RunRng.MonsterAi.NextItem(items)).ToMutable(),
                base.CombatState,
                CombatSide.Enemy,
                base.CombatState.Encounter.GetNextSlot(base.CombatState)
            );

...
public sealed class ToughEgg : MonsterModel
{
    ...
    public override async Task AfterAddedToRoom()
    {
        ...
        if (!IsHatched)
        {
            ...
            await PowerCmd.Apply<HatchPower>(
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState2 = (MoveState)(
            moveState.FollowUpState = new MoveState(
                "NIBBLE_MOVE",
                NibbleMove,
                new SingleAttackIntent(NibbleDamage)
            )
        );
    ...
    private async Task HatchMove(IReadOnlyList<Creature> targets)
    {
        ...
        List<PowerModel> list = base
            .Creature.Powers.Where((PowerModel p) => !(p is MinionPower))
            .ToList();
    ...
    private async Task Hatch()
    {
        ...
        decimal amount = MegaCrit.Sts2.Core.Entities.Creatures.Creature.ScaleHpForMultiplayer(
            base.RunRng.Niche.NextInt(HatchlingMinHp, HatchlingMaxHp),
            base.CombatState.Encounter,
            base.Creature.CombatState.Players.Count,
            base.Creature.CombatState.Players[0].RunState.CurrentActIndex
        );

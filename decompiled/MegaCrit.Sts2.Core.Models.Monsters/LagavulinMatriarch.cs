...
public sealed class LagavulinMatriarch : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 233, 222);
    ...
    )
    {
        ...
        base.Creature.GetCreatureNode()
            ?.SpineAnimation.SetAnimation("_tracks/eyes_dead", loop: false, 1);
        SleepingVfx?.Stop();
        SleepingVfx = null;
        return Task.CompletedTask;
    }
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        ConditionalBranchState conditionalBranchState = (ConditionalBranchState)(
            moveState.FollowUpState = new ConditionalBranchState("SLEEP_BRANCH")
        );
    ...
    private async Task Slash2Move(IReadOnlyList<Creature> targets)
    {
        ...
        await DamageCmd
            .Attack(Slash2Damage)
            .FromMonster(this)
            .WithAttackerAnim("AttackHeavy", 0.2f)
            .WithAttackerFx(
                null,
                "event:/sfx/enemy/enemy_attacks/lagavulin_matriarch/lagavulin_matriarch_slam"
            )
    ...
    )
    {
        ...
        list.Insert(
            1,
            BestiaryMonsterMove.FromNonStateMove(GetBestiaryMoveName("WAKE_UP"), WakeUpMove)
        );
        return list;
    }
}

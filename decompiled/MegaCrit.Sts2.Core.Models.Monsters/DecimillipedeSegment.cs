...
public abstract class DecimillipedeSegment : MonsterModel
{
    ...
    public static readonly string rocksVfxPath = SceneHelper.GetScenePath(
        "vfx/vfx_decimillipede_rocks"
    );

    private const string _healSfx =
        "event:/sfx/enemy/enemy_attacks/decimillipede/decimillipede_heal";

    private const string _attackTriple =
        "event:/sfx/enemy/enemy_attacks/decimillipede/decimillipede_attack_triple";

    private const string _attackBuff =
        "event:/sfx/enemy/enemy_attacks/decimillipede/decimillipede_attack_buff";

    private const string _attackWeaken =
    ...
    private string PhobiaModeAliveTexture =>
        ImageHelper.GetImagePath(
            "monsters/phobia_mode/" + base.Id.Entry.ToLowerInvariant() + "_phobia.png"
        );

    private string PhobiaModeDeadTexture =>
        ImageHelper.GetImagePath(
            "monsters/phobia_mode/" + base.Id.Entry.ToLowerInvariant() + "_shriveled_phobia.png"
        );
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 46, 40);
    ...
    public override async Task AfterAddedToRoom()
    {
        ...
        List<Creature> source = (
            from c in base.CombatState.GetTeammatesOf(base.Creature)
        ...
        while (source.Any((Creature c) => (decimal)c.MaxHp == maxHp))
        {
            ...
            {
                maxHp = MegaCrit.Sts2.Core.Entities.Creatures.Creature.ScaleHpForMultiplayer(
                    MinInitialHp,
                    base.CombatState.Encounter,
                    count,
                    currentActIndex
    ...
    private async Task ReattachMove(IReadOnlyList<Creature> targets)
    {
        ...
        ChangePhobiaModeTexture(
            ResourceLoader.Load<Texture2D>(
                PhobiaModeAliveTexture,
                null,

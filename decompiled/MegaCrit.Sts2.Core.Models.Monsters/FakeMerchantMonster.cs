...
public sealed class FakeMerchantMonster : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 175, 165);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState3 = new MoveState(
            "THROW_RELIC_MOVE",
            ThrowRelicMove,
            new SingleAttackIntent(ThrowRelicDamage),
            new DebuffIntent()
        );
    ...
    private async Task ThrowRelicMove(IReadOnlyList<Creature> targets)
    {
        ...
        await DamageCmd
            .Attack(ThrowRelicDamage)
            .FromMonster(this)
            .WithAttackerAnim("throw", 0.15f)
            .WithAttackerFx(null, AttackSfx)
            .WithHitFx("vfx/vfx_attack_slash")
            .Execute(null);
    ...
    {
        LocString locString = MegaCrit.Sts2.Core.Random.Rng.Chaotic.NextItem(
            GetLinesForMove(moveId)
        );
        if (locString != null)
        {

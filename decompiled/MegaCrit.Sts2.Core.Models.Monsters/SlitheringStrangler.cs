...
public sealed class SlitheringStrangler : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 54, 53);

    public override int MaxInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 56, 55);
    ...
    private async Task ThwackMove(IReadOnlyList<Creature> targets)
    {
        await DamageCmd
            .Attack(ThwackDamage)
            .FromMonster(this)
            .WithAttackerAnim("AttackDefendTrigger", 0.2f)
            .WithAttackerFx(
                null,
                "event:/sfx/enemy/enemy_attacks/slithering_strangler/slithering_strangler_attack_headbutt"
            )

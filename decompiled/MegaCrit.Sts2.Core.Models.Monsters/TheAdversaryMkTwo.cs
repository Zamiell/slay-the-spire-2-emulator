...
public sealed class TheAdversaryMkTwo : MonsterModel
{
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 200, 200);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState = new MoveState(
            "BASH_MOVE",
            BashMove,
            new SingleAttackIntent(BashDamage)
        );
        MoveState moveState2 = new MoveState(
            "FLAME_BEAM_MOVE",
            FlameBeamMove,
            new SingleAttackIntent(FlameBeamDamage)
        );
        MoveState moveState3 = new MoveState(
            "BARRAGE_MOVE",
            BarrageMove,
            new MultiAttackIntent(BarrageDamage, BarrageRepeat),
            new BuffIntent()
        );
    ...
    {
        await DamageCmd
            .Attack(BarrageDamage)
            .WithHitCount(BarrageRepeat)
            .FromMonster(this)
            .WithAttackerAnim("Attack", 0.15f)
            .WithAttackerFx(null, AttackSfx)
            .WithHitFx("vfx/vfx_attack_slash")

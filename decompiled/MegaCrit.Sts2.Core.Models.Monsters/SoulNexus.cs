...
public sealed class SoulNexus : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 254, 234);

    public override int MaxInitialHp => MinInitialHp;

    private int SoulBurnDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 31, 29);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState = new MoveState(
            "SOUL_BURN_MOVE",
            SoulBurnMove,
            new SingleAttackIntent(SoulBurnDamage)
        );
        MoveState moveState2 = new MoveState(
            "MAELSTROM_MOVE",
            MaelstromMove,
            new MultiAttackIntent(MaelstromDamage, MaelstromRepeat)
        );
        MoveState moveState3 = new MoveState(
            "DRAIN_LIFE_MOVE",
            DrainLifeMove,
            new SingleAttackIntent(DrainLifeDamage),
            new DebuffIntent(strong: true)
        );
        RandomBranchState randomBranchState = (RandomBranchState)(
            moveState3.FollowUpState = (
                moveState2.FollowUpState = (moveState.FollowUpState = new RandomBranchState("RAND"))
            )
        );
    ...
    {
        await DamageCmd
            .Attack(MaelstromDamage)
            .WithHitCount(MaelstromRepeat)
            .FromMonster(this)
            .OnlyPlayAnimOnce()

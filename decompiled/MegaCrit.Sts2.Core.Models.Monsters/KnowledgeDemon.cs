...
public sealed class KnowledgeDemon : MonsterModel
{
    ...
    private const string _clapSfx =
        "event:/sfx/enemy/enemy_attacks/knowledge_demon/knowledge_demon_clap";

    private const string _flameSfx =
        "event:/sfx/enemy/enemy_attacks/knowledge_demon/knowledge_demon_flame";

    private const string _slapSfx =
        "event:/sfx/enemy/enemy_attacks/knowledge_demon/knowledge_demon_slap";

    private static readonly LocString _curseOfKnowledgeStartLine = MonsterModel.L10NMonsterLookup(
        "KNOWLEDGE_DEMON.moves.CURSE_OF_KNOWLEDGE.startLine"
    );

    private static readonly LocString _curseOfKnowledgeDoneLine = MonsterModel.L10NMonsterLookup(
    ...
    private static readonly IReadOnlyList<IReadOnlyList<IChoosable>> _curseOfKnowledgeSets =
        new global::_003C_003Ez__ReadOnlyArray<IReadOnlyList<IChoosable>>(
            new IReadOnlyList<IChoosable>[3]
            {
                new global::_003C_003Ez__ReadOnlyArray<IChoosable>(
                    new IChoosable[2] { ModelDb.Card<Disintegration>(), ModelDb.Card<MindRot>() }
                ),
                new global::_003C_003Ez__ReadOnlyArray<IChoosable>(
                    new IChoosable[2] { ModelDb.Card<Disintegration>(), ModelDb.Card<Sloth>() }
                ),
                new global::_003C_003Ez__ReadOnlyArray<IChoosable>(
                    new IChoosable[2] { ModelDb.Card<Disintegration>(), ModelDb.Card<WasteAway>() }
                ),
            }
        );
    ...
    {
        get { return _curseOfKnowledgeCounter; }
        set
        {
            AssertMutable();
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 399, 379);
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState3 = new MoveState(
            ...
            new MultiAttackIntent(KnowledgeOverwhelmingDamage, 3)
        );
        MoveState moveState4 = new MoveState(
            "PONDER_MOVE",
            PonderMove,
            new SingleAttackIntent(PonderDamage),
            new HealIntent(),
            new BuffIntent()
        );
        ConditionalBranchState conditionalBranchState = new ConditionalBranchState(
    ...
    private async Task CurseOfKnowledge(IReadOnlyList<Creature> targets)
    {
        if (CurseOfKnowledgeCounter >= _curseOfKnowledgeSets.Count)
        {
            throw new InvalidOperationException(
                $"There are no valid sets at this index {CurseOfKnowledgeCounter}"
            );
        }
        TalkCmd.Play(
            _curseOfKnowledgeStartLine,
            base.Creature,
            VfxColor.Gold,
    ...
    private async Task ChooseCurse(Creature target)
    {
        ...
        List<CardModel> cards = _curseOfKnowledgeSets[CurseOfKnowledgeCounter]
            .Select(
                delegate(IChoosable c)
                {
    ...
    private async Task KnowledgeOverwhelmingMove(IReadOnlyList<Creature> targets)
    {
        ...
        await DamageCmd
            .Attack(KnowledgeOverwhelmingDamage)
            .WithHitCount(3)
            .FromMonster(this)

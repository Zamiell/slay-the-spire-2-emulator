...
public sealed class MechaKnight : MonsterModel
{
    ...
    private const string _flamethrowerSfx =
        "event:/sfx/enemy/enemy_attacks/mechaknight/mechaknight_flamethrower";

    private const string _heavyAttackSfx =
        "event:/sfx/enemy/enemy_attacks/mechaknight/mechaknight_heavy_attack";

    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 320, 300);
    ...
    private async Task ChargeMove(IReadOnlyList<Creature> targets)
    {
        await DamageCmd
            ...
            .WithHitVfxNode(
                (Creature _) =>
                    NBigSlashImpactVfx.Create(
                        VfxCmd.GetSideCenter(CombatSide.Player, base.CombatState).Value,
                        180f,
                        new Color("#80dbff")
                    )
            )

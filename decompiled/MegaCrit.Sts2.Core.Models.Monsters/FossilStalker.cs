...
public sealed class FossilStalker : MonsterModel
{
    ...
    public override int MinInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 54, 51);

    public override int MaxInitialHp =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 56, 53);

    private int TackleDamage =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 11, 9);
    ...
    public override async Task AfterAddedToRoom()
    {
        ...
        await PowerCmd.Apply<SuckPower>(
    ...
    protected override MonsterMoveStateMachine GenerateMoveStateMachine()
    {
        ...
        MoveState moveState2 = new MoveState(
            "LATCH_MOVE",
            LatchMove,
            new SingleAttackIntent(LatchDamage)
        );
        MoveState moveState3 = new MoveState(
            "LASH_MOVE",
            LashAttack,
            new MultiAttackIntent(LashDamage, LashRepeat)
        );
        RandomBranchState randomBranchState = (RandomBranchState)(
            moveState3.FollowUpState = (
                moveState.FollowUpState = (moveState2.FollowUpState = new RandomBranchState("RAND"))
            )
        );

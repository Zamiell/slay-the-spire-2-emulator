...
public class NLinkedRewardSet : Control
{
    ...
    private static string ChainImagePath =>
        ImageHelper.GetImagePath("/ui/reward_screen/reward_chain.png");
    ...
    {
        NLinkedRewardSet nLinkedRewardSet = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NLinkedRewardSet>(PackedScene.GenEditState.Disabled);
    ...
    private void Reload()
    {
        ...
        for (int i = 0; i < LinkedRewardSet.Rewards.Count; i++)
        {
            ...
            if (i < LinkedRewardSet.Rewards.Count - 1)
            {
                ...
                textureRect.GlobalPosition =
                    _chainsContainer.GlobalPosition
                    + Vector2.Down * i * (3f + nRewardButton.CustomMinimumSize.Y);
            }
        }
    }

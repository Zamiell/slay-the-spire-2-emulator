...
public class NRewardButton : NButton
{
    ...
    private static readonly StringName _fontOutlineColor = new StringName(
        "theme_override_colors/font_outline_color"
    );

    private static readonly StringName _defaultColor = new StringName(
    ...
    public event RewardClaimedEventHandler RewardClaimed
    {
        add
        {
            backing_RewardClaimed = (RewardClaimedEventHandler)
                Delegate.Combine(backing_RewardClaimed, value);
        }
        remove
        {
            backing_RewardClaimed = (RewardClaimedEventHandler)
                Delegate.Remove(backing_RewardClaimed, value);
        }
    }
    ...
    public event RewardSkippedEventHandler RewardSkipped
    {
        add
        {
            backing_RewardSkipped = (RewardSkippedEventHandler)
                Delegate.Combine(backing_RewardSkipped, value);
        }
        remove
        {
            backing_RewardSkipped = (RewardSkippedEventHandler)
                Delegate.Remove(backing_RewardSkipped, value);
        }
    }

    public static NRewardButton Create(Reward reward, NRewardsScreen screen)
    {
        NRewardButton nRewardButton = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NRewardButton>(PackedScene.GenEditState.Disabled);
    ...
    private void SetReward(Reward reward)
    {
        if (reward is LinkedRewardSet)
        {
            throw new ArgumentException(
                "You aren't allowed to apply a RewardChainSet to a NRewardButton"
            );
        }
        Reward = reward;
        if (IsNodeReady())
        ...
        if (await RunManager.Instance.RewardsSetSynchronizer.SelectLocalReward(Reward))
        {
            if (TestMode.IsOff)
            {
                ...
                if (reward is RelicReward relicReward)
                {
                    ...
                    {
                        globalUi.RelicInventory.AnimateRelic(
                            relicReward.ClaimedRelic,
                            _iconContainer.GlobalPosition
                        );
                    }
                }
                else if (reward is PotionReward potionReward)
                {
                    globalUi.TopBar.PotionContainer.AnimatePotion(
                        potionReward.ClaimedPotion,
                        _iconContainer.GlobalPosition
                    );
                }
            }
    ...
    protected override void OnPress()
    {
        ...
        _currentTween
            .TweenMethod(Callable.From<float>(UpdateShaderParam), _hsvHover, _hsvDown, 0.5)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
        _currentTween
            .TweenProperty(_label, "modulate:a", 1f, 0.5)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
    ...
    protected override void OnUnfocus()
    {
        ...
        _currentTween
            .TweenMethod(
                Callable.From<float>(UpdateShaderParam),
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<RewardClaimedEventHandler>(
                SignalName.RewardClaimed,
                out var value10
            )
        )
        {
        ...
        if (
            info.TryGetSignalEventDelegate<RewardSkippedEventHandler>(
                SignalName.RewardSkipped,
                out var value11
            )
        )
        {

...
public static class MegaLabelHelper
{
    ...
    public static void AssertThemeFontOverride(Control control, StringName fontOverrideName)
    {
        ...
        throw new InvalidOperationException(
            $"{control.GetType().Name} '{control.GetPath()}' has no theme font override. Please set one to avoid a Godot engine bug."
        );
    }
    ...
    public static List<BbcodeObject> ParseBbcode(string bbcode)
    {
        ...
        for (int i = 0; i < bbcode.Length; i++)
        {
            ...
            )
            {
                ...
                _cachedBbcodeObjects.Add(
                    new BbcodeObject { tag = text, type = BbcodeObjectType.BeginTag }
                );
                stack.Push(text);
            ...
            )
            {
                if (bbcodeParsingState != BbcodeParsingState.InTagEnvironment)
                {
                    ...
                    {
                        _cachedBbcodeObjects.Add(
                            new BbcodeObject { text = "[", type = BbcodeObjectType.Text }
                        );
                    }
                    else if (text2 == "rb")
                    {
                    ...
                    else
                    {
                        if (bbcodeParsingState == BbcodeParsingState.InEndTag)
                        {
                            ...
                            {
                                throw new InvalidOperationException(
                                    $"Found end tag {text2} with no tag on the stack. ({bbcode})"
                                );
                            }
                            if (stack.Peek() != text2)
                            {
                                throw new InvalidOperationException(
                                    $"Found end tag {text2}, expected {stack.Peek()}. ({bbcode})"
                                );
                            }
                            stack.Pop();
                        }
                        ...
                        _cachedBbcodeObjects.Add(
                            new BbcodeObject
                            {
                                ...
                                type = (
                                    (bbcodeParsingState == BbcodeParsingState.InTag)
                                        ? BbcodeObjectType.BeginTag
                                        : BbcodeObjectType.EndTag
                                ),
                            }
                        );
                    }
        ...
        {
            _cachedBbcodeObjects.Add(
                new BbcodeObject { text = stringBuilder.ToString(), type = BbcodeObjectType.Text }
            );
        }
    ...
    {
        Vector2 vector = EstimateTextSize(
            paragraph,
            text,
            font,
            fontSize,

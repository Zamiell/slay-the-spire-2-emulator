...
public class AncientDialogueSet
{
    ...
    > CharacterDialogues { get; init; }

    public required IReadOnlyList<AncientDialogue> AgnosticDialogues { get; init; } =
        Array.Empty<AncientDialogue>();
    ...
    )
    {
        ...
        {
            return new global::_003C_003Ez__ReadOnlySingleElementList<AncientDialogue>(
                FirstVisitEverDialogue
            );
        }
        IReadOnlyList<AncientDialogue> readOnlyList = null;
        if (
            CharacterDialogues.TryGetValue(
                characterId.Entry,
                out IReadOnlyList<AncientDialogue> value
            )
        )
        {
            readOnlyList = value;
            List<AncientDialogue> list = readOnlyList
                .Where((AncientDialogue d) => d.VisitIndex == charVisits)
                .ToList();
            if (list.Count > 0)
            {
                return list;
        ...
        {
            List<AncientDialogue> list2 = AgnosticDialogues
                .Where((AncientDialogue d) => d.VisitIndex == charVisits)
                .ToList();
            if (list2.Count > 0)
            {
    ...
    public void PopulateLocKeys(string ancientEntry)
    {
        ...
        foreach (
            KeyValuePair<
                string,
                IReadOnlyList<AncientDialogue>
            > characterDialogue in CharacterDialogues
        )
        {

        ...
        for (int i = 0; i < Lines.Count; i++)
        {
            ...
            if (flag && !flag2)
            {
                throw new InvalidOperationException(
                    $"Dialogue {ancientEntry}.talk.{charEntry}.{dialogueIndex}: line 0 has 'r' suffix but line {i} does not."
                );
            }
            if (!flag && flag2)
            {
                throw new InvalidOperationException(
                    $"Dialogue {ancientEntry}.talk.{charEntry}.{dialogueIndex}: line 0 has no 'r' suffix but line {i} does."
                );
            }

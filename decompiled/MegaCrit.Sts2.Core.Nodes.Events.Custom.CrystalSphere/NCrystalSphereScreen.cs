...
public class NCrystalSphereScreen : Control, IOverlayScreen, IScreenContext
{
    ...
    private readonly LocString _instructionsTitleLoc = new LocString(
        "events",
        "CRYSTAL_SPHERE.minigame.instructions.title"
    );

    private readonly LocString _instructionsDescriptionLoc = new LocString(
        "events",
        "CRYSTAL_SPHERE.minigame.instructions.description"
    );

    private readonly LocString _divinationsRemainLoc = new LocString(
        "events",
        "CRYSTAL_SPHERE.minigame.divinationsRemain"
    );

    private const string _scenePath =
        "res://scenes/events/custom/crystal_sphere/crystal_sphere_screen.tscn";
    ...
    public Control DefaultFocusedControl
    {
        get
        {
            List<NCrystalSphereCell> list = (
                from c in _cellContainer.GetChildren().OfType<NCrystalSphereCell>()
    ...
    {
        NCrystalSphereScreen nCrystalSphereScreen = PreloadManager
            .Cache.GetScene("res://scenes/events/custom/crystal_sphere/crystal_sphere_screen.tscn")
    ...
    public override void _Ready()
    {
        ...
        _bigDivinationButton.SetLabel(
            new LocString("events", "CRYSTAL_SPHERE.button.DIVINATION_LABEL_BIG")
        );
        _smallDivinationButton.SetLabel(
            new LocString("events", "CRYSTAL_SPHERE.button.DIVINATION_LABEL_SMALL")
        );
        ...
        _instructionsDescriptionLabel.SetTextAutoSize(
            _instructionsDescriptionLoc.GetFormattedText()
        );
        _dialogue = GetNode<NCrystalSphereDialogue>("%Dialogue");
        Vector2 vector = Vector2.One * -(57 * _entity.GridSize.X) * 0.5f;
        NCrystalSphereCell[,] array = new NCrystalSphereCell[
            _entity.GridSize.X,
            _entity.GridSize.Y
        ];
        ...
        foreach (CrystalSphereItem item in _entity.Items)
        {
            ...
            nCrystalSphereItem.Position =
                vector + 57f * new Vector2(item.Position.X, item.Position.Y);
            item.Revealed += OnItemRevealed;
        }
        _bigDivinationButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(SetBigDivination)
        );
        _smallDivinationButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(SetSmallDivination)
        );
        ...
        for (int num = 0; num < array.GetLength(0); num++)
        {
            for (int num2 = 0; num2 < array.GetLength(1); num2++)
            {
                ...
                control.FocusNeighborTop = (
                    (num2 > 0) ? array[num, num2 - 1].GetPath() : array[num, num2].GetPath()
                );
                control.FocusNeighborBottom = (
                    (num2 < array.GetLength(1) - 1)
                        ? array[num, num2 + 1].GetPath()
                        : array[num, num2].GetPath()
                );
                control.FocusNeighborLeft = (
                    (num > 0) ? array[num - 1, num2].GetPath() : array[num, num2].GetPath()
                );
                control.FocusNeighborRight = (
                    (num < array.GetLength(0) - 1)
                        ? array[num + 1, num2].GetPath()
                        : array[num, num2].GetPath()
                );
            }
        }
    ...
    private async Task OnCellClicked(NCrystalSphereCell cell)
    {
        if (_entity.DivinationCount > 0)
        {
            ...
            List<NCrystalSphereCell> source = (
                from c in _cellContainer.GetChildren().OfType<NCrystalSphereCell>()
                where c.Entity.IsHidden
                select c
            ).ToList();
            source
                .OrderBy(
                    (NCrystalSphereCell c1) =>
                        new Vector2I(cell.Entity.X, cell.Entity.Y).DistanceTo(
                            new Vector2I(c1.Entity.X, c1.Entity.Y)
                        )
                )
                .First()
                .TryGrabFocus();
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateDivinationsLeft,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._bigDivinationButton,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._smallDivinationButton,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._divinationsLeftLabel,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._instructionsTitleLabel,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._instructionsDescriptionLabel,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._instructionsContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._smallDivinationButton,
            Variant.From(in _smallDivinationButton)
        );
        info.AddProperty(
            PropertyName._divinationsLeftLabel,
            Variant.From(in _divinationsLeftLabel)
        );
        ...
        info.AddProperty(
            PropertyName._instructionsTitleLabel,
            Variant.From(in _instructionsTitleLabel)
        );
        info.AddProperty(
            PropertyName._instructionsDescriptionLabel,
            Variant.From(in _instructionsDescriptionLabel)
        );
        info.AddProperty(
            PropertyName._instructionsContainer,
            Variant.From(in _instructionsContainer)
        );

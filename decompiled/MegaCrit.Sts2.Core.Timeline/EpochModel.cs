...
[GenerateSubtypes(
    DynamicallyAccessedMemberTypes = DynamicallyAccessedMemberTypes.PublicParameterlessConstructor
)]
public abstract class EpochModel
{
    ...
    private static IEnumerable<string> EpochIds =>
    ...
    public static IReadOnlyList<string> AllEpochIds =>
        _allEpochIds ?? (_allEpochIds = EpochIds.ToList());

    public string Year =>
        new LocString("eras", StringHelper.Slugify(Era.ToString()) + ".year").GetFormattedText();

    public string EraName =>
        new LocString("eras", StringHelper.Slugify(Era.ToString()) + ".name").GetFormattedText();
    ...
    public virtual string UnlockText =>
        new LocString("epochs", Id + ".unlockText").GetFormattedText();
    ...
    public Texture2D Portrait =>
        ResourceLoader.Load<Texture2D>(PackedPortraitPath, null, ResourceLoader.CacheMode.Reuse);

    public string PackedPortraitPath =>
        ImageHelper.GetImagePath("atlases/epoch_atlas.sprites/" + Id.ToLowerInvariant() + ".tres");

    public Texture2D RealPortrait =>
        ResourceLoader.Load<Texture2D>(ResolvedPortraitPath, null, ResourceLoader.CacheMode.Reuse);

    private bool HasRealPortrait => ResourceLoader.Exists(RealPortraitPath);

    private string RealPortraitPath =>
        ImageHelper.GetImagePath("timeline/epoch_portraits/" + Id.ToLowerInvariant() + ".png");

    private string PlaceholderPortraitPath =>
        ImageHelper.GetImagePath(
            "timeline/epoch_portraits/placeholder/" + Id.ToLowerInvariant() + ".png"
        );
    ...
    public static EpochModel Get<T>()
        where T : EpochModel
    {
        return Get(GetId<T>());
    }
    ...
    protected static void QueueTimelineExpansion(EpochModel[] epochs)
    {
        ...
        {
            SerializableEpoch serializableEpoch =
                SaveManager.Instance.Progress.Epochs.FirstOrDefault(
                    (SerializableEpoch e) => e.Id == epoch.Id
                );

...
public static class HandPosHelper
{
    ...
    {
        new Vector2[1] { new Vector2(0f, -50f) },
        new Vector2[2] { new Vector2(-100f, -50f), new Vector2(100f, -50f) },
        new Vector2[3] { new Vector2(-180f, -50f), new Vector2(0f, -59f), new Vector2(180f, -50f) },
        new Vector2[4]
        {
    ...
    public static Vector2 GetPosition(int handSize, int cardIndex)
    {
        if (handSize - 1 >= _cardPositionData.Length)
        {
            throw new ArgumentOutOfRangeException(
                $"Hand size {handSize} is greater than {_cardPositionData.Length + 1}!"
            );
        }
        if (cardIndex >= _cardPositionData[handSize - 1].Length)
        {
            throw new ArgumentOutOfRangeException(
                $"Card index {cardIndex} is greater than {_cardPositionData[handSize - 1].Length}!"
            );
        }

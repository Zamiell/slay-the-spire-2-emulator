...
public static class OneTimeInitialization
{
    ...
    public static void ExecuteVeryEarly()
    {
        ...
        ModManager.Initialize(
            ...
            ReleaseInfoManager.Instance.SemVer
        );
    ...
    private static void PrewarmJit()
    {
        ...
        foreach (Type subtype in ReflectionHelper.GetSubtypes<IPacketSerializable>())
        {
            ...
            RuntimeHelpers.PrepareMethod(
                typeFromHandle.GetMethod("WriteList").MethodHandle,
                new RuntimeTypeHandle[1] { subtype.TypeHandle }
            );
            RuntimeHelpers.PrepareMethod(
                typeFromHandle.GetMethod("Write").MethodHandle,
                new RuntimeTypeHandle[1] { subtype.TypeHandle }
            );
            RuntimeHelpers.PrepareMethod(
                typeFromHandle2.GetMethod("ReadList").MethodHandle,
                new RuntimeTypeHandle[1] { subtype.TypeHandle }
            );
            RuntimeHelpers.PrepareMethod(
                typeFromHandle2.GetMethod("Read").MethodHandle,
                new RuntimeTypeHandle[1] { subtype.TypeHandle }
            );
        }

...
{
    public const BindingFlags allAccessLevels =
    ...
    public static Type[] ModTypes
    {
        get
        {
            if (_modTypes == null)
            {
                _modTypes = (from m in ModManager.GetLoadedMods() select m.assembly)
                    .OfType<Assembly>()
    ...
    private static IEnumerable<Type> GetSubtypesFromList(IList<Type> list, Type parentType)
    {
        return list.Where(
            (Type type) =>
                ...
                && !type.IsAbstract
                && !type.IsInterface
                && InheritsOrImplements(type, parentType)
        );
    }
    ...
    public static IEnumerable<Type> GetSubtypesInMods<T>()
        where T : class
    {
    ...
    public static bool InheritsOrImplements(
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.Interfaces)] Type derived,
        Type baseType
    )
    {

...
public sealed class MadScience : CardModel
{
    ...
    private string GetPortraitPath(CardType cardType)
    {
        return ImageHelper.GetImagePath(
            "atlases/card_atlas.sprites/event/" + GetPortraitFilename(cardType) + ".tres"
        );
    }
    ...
    private string GetPortraitFilename(CardType cardType)
    {
        return cardType switch
        {
            ...
            _ => throw new InvalidOperationException(
                $"Mad Science is invalid type {TinkerTimeType}."
            ),
        };
    }
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        if (
            (tinkerTimeRider == TinkerTime.RiderEffect.Sapping || (uint)(tinkerTimeRider - 3) <= 3u)
                ? true
                : false
        )
        {
    ...
    {
        int hits = (
            (TinkerTimeRider != TinkerTime.RiderEffect.Violence)
                ? 1
                : base.DynamicVars["ViolenceHits"].IntValue
        );
        ...
        switch (TinkerTimeRider)
        {
            case TinkerTime.RiderEffect.Expertise:
                await PowerCmd.Apply<StrengthPower>(
                    choiceContext,
                    base.Owner.Creature,
                    base.DynamicVars["ExpertiseStrength"].BaseValue,
                    base.Owner.Creature,
                    this
                );
                await PowerCmd.Apply<DexterityPower>(
                    choiceContext,
                    base.Owner.Creature,
                    base.DynamicVars["ExpertiseDexterity"].BaseValue,
                    base.Owner.Creature,
                    this
                );
                break;
            case TinkerTime.RiderEffect.Curious:
                await PowerCmd.Apply<CuriousPower>(
                    choiceContext,
                    base.Owner.Creature,
                    base.DynamicVars["CuriousReduction"].BaseValue,
                    base.Owner.Creature,
                    this
                );
                break;
        ...
        switch (rider)
        {
            case TinkerTime.RiderEffect.Sapping:
                await PowerCmd.Apply<WeakPower>(
                    choiceContext,
                    target,
                    base.DynamicVars["SappingWeak"].BaseValue,
                    base.Owner.Creature,
                    this
                );
                await PowerCmd.Apply<VulnerablePower>(
                    choiceContext,
                    target,
                    base.DynamicVars["SappingVulnerable"].BaseValue,
                    base.Owner.Creature,
                    this
                );
                break;
            case TinkerTime.RiderEffect.Choking:
                await PowerCmd.Apply<StranglePower>(
                    choiceContext,
                    target,
                    base.DynamicVars["ChokingDamage"].BaseValue,
                    base.Owner.Creature,
                    this
                );
                break;
            case TinkerTime.RiderEffect.Energized:
                await PlayerCmd.GainEnergy(
                    base.DynamicVars["EnergizedEnergy"].IntValue,
                    base.Owner
                );
                break;
            case TinkerTime.RiderEffect.Wisdom:
                await CardPileCmd.Draw(
                    choiceContext,
                    base.DynamicVars["WisdomCards"].IntValue,
                    base.Owner
                );
                break;

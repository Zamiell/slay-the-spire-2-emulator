...
public sealed class BeatDown : CardModel
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        IEnumerable<CardModel> enumerable = pile
            .Cards.Where(
                (CardModel c) =>
                    c.Type == CardType.Attack && !c.Keywords.Contains(CardKeyword.Unplayable)
            )
            .ToList()
        ...
        foreach (CardModel item in enumerable)
        {
            if (!CombatManager.Instance.IsOverOrEnding)
            {
                ...
                {
                    Creature target = base.Owner.RunState.Rng.CombatTargets.NextItem(

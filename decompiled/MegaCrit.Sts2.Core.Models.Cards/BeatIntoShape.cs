...
public sealed class BeatIntoShape : CardModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[4]
            {
                ...
                new CalculatedVar("CalculatedForge").WithMultiplier(
                    (CardModel card, Creature? target) =>
                        CombatManager
                            .Instance.History.Entries.OfType<DamageReceivedEntry>()
                            .Count(
                                (DamageReceivedEntry e) =>
                                    e.Receiver == target
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        decimal amount = ((CalculatedVar)base.DynamicVars["CalculatedForge"]).Calculate(
            cardPlay.Target
        );
        amount -=
            (decimal)attackCommand.Results.Count() * base.DynamicVars.CalculationExtra.BaseValue;

...
public sealed class HelixDrill : CardModel
{
            ...
            new DynamicVar[4]
            {
                ...
                new CalculatedVar("CalculatedHits").WithMultiplier(
                    delegate(CardModel card, Creature? _)
                    {
                        ...
                        int num = (
                            from e in CombatManager.Instance.History.Entries.OfType<EnergySpentEntry>()
                            where
                                e.HappenedThisTurn(card.CombatState) && e.Actor.Player == card.Owner
                            select e
                        ).Sum((EnergySpentEntry c) => c.Amount);
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        await DamageCmd
            ...
            .WithHitCount(
                (int)((CalculatedVar)base.DynamicVars["CalculatedHits"]).Calculate(cardPlay.Target)
            )
            .FromCard(this)

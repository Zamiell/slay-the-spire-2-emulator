...
public sealed class Rattle : CardModel
{
    ...
    protected override HashSet<CardTag> CanonicalTags =>
        new HashSet<CardTag> { CardTag.OstyAttack };
            ...
            new DynamicVar[4]
            {
                ...
                new CalculatedVar("CalculatedHits").WithMultiplier(
                    (CardModel card, Creature? _) =>
                        1
                        + CombatManager
                            .Instance.History.Entries.OfType<CreatureAttackedEntry>()
                            .Count(
                                (CreatureAttackedEntry e) =>
                                    e.Actor == card.Owner.Osty
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        {
            await DamageCmd
                .Attack(base.DynamicVars.OstyDamage.BaseValue)
                .FromOsty(base.Owner.Osty, this)
                .Targeting(cardPlay.Target)
                .WithHitCount(
                    (int)
                        ((CalculatedVar)base.DynamicVars["CalculatedHits"]).Calculate(
                            cardPlay.Target
                        )
                )

...
public sealed class FlakCannon : CardModel
{
        ...
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[4]
            {
                ...
                new CalculatedVar("CalculatedHits").WithMultiplier(
                    (CardModel card, Creature? _) => GetStatuses(card.Owner).Count()
                ),
            }
        );
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        int statusCount = (int)
            ((CalculatedVar)base.DynamicVars["CalculatedHits"]).Calculate(cardPlay.Target);
    ...
    {
        return owner.PlayerCombatState.AllCards.Where(
            (CardModel c) => c.Type == CardType.Status && c.Pile.Type != PileType.Exhaust
        );
    }

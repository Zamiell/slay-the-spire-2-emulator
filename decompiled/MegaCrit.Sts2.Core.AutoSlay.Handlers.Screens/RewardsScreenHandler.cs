...
public class RewardsScreenHandler : IScreenHandler, IHandler
{
    ...
    public async Task HandleAsync(Rng random, CancellationToken ct)
    {
        ...
        while (true)
        {
            ...
            bool hasPotionSlots =
                LocalContext.GetMe(RunManager.Instance.DebugOnlyGetState())?.HasOpenPotionSlots
                ?? false;
            NRewardButton nRewardButton = UiHelper
                .FindAll<NRewardButton>(screen)
                .FirstOrDefault(
                    (NRewardButton b) =>
                        b.IsEnabled
                        && !attemptedButtons.Contains(b)
                        && (!(b.Reward is PotionReward) || hasPotionSlots)
                );
            ...
            AutoSlayLog.Action(
                "Clicking reward button: " + (nRewardButton.Reward?.GetType().Name ?? "unknown")
            );

    ...
    public async Task HandleAsync(Rng random, CancellationToken ct)
    {
        ...
        if (proceedButton?.IsEnabled ?? false)
        {
            ...
            if (GodotObject.IsInstanceValid(screen) && screen.IsVisibleInTree())
            {
                ...
                {
                    AutoSlayLog.Info(
                        "Map opened, manually removing Crystal Sphere screen from overlay stack"
                    );
        ...
        while (clicks < maxClicks)
        {
            ...
            List<NCrystalSphereCell> list = (
                from c in UiHelper.FindAll<NCrystalSphereCell>(nodeOrNull)
            ...
            if (list.Count >= lastClickableCount)
            {
                ...
                {
                    AutoSlayLog.Info(
                        $"No progress after {noProgressCount} clicks, divinations likely exhausted"
                    );
                    break;
                }
            }
            ...
            AutoSlayLog.Info(
                $"Clicking crystal sphere cell at ({nCrystalSphereCell.Entity.X}, {nCrystalSphereCell.Entity.Y}), click {clicks + 1}, {list.Count} clickable"
            );
            nCrystalSphereCell.EmitSignal(
        ...
        await WaitHelper.Until(
            ...
            ct,
            TimeSpan.FromSeconds(15L),
            "Crystal Sphere: neither proceed button nor rewards screen appeared"
        );

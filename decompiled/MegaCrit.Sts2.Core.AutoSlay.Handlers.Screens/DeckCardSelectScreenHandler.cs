...
public class DeckCardSelectScreenHandler : IScreenHandler, IHandler
{
    ...
    public async Task HandleAsync(Rng random, CancellationToken ct)
    {
        ...
        await WaitHelper.Until(
            ...
            "No cards found in card select screen"
        );
        ...
        for (int i = 0; i < maxSelections; i++)
        {
            ...
            if (
                (previewContainer?.Visible ?? false) || (nodeOrNull != null && nodeOrNull.IsEnabled)
            )
            {
                break;
            }
        ...
        NConfirmButton confirmButton = previewContainer?.GetNodeOrNull<NConfirmButton>(
            "%PreviewConfirm"
        );
        ...
        if (confirmButton != null)
        {
            await WaitHelper.Until(
                ...
                ct,
                TimeSpan.FromSeconds(5L),
                "Card select confirm button did not become enabled"
            );
            ...
            await WaitHelper.Until(
                ...
                "Card select screen did not close after confirmation"
            );
        }
        else
        {

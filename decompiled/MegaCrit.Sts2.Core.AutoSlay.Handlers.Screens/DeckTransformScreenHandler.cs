...
public class DeckTransformScreenHandler : IScreenHandler, IHandler
{
    ...
    public async Task HandleAsync(Rng random, CancellationToken ct)
    {
        ...
        NDeckTransformSelectScreen screen =
            AutoSlayer.GetCurrentScreen<NDeckTransformSelectScreen>();
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            if (mainConfirmButton != null && mainConfirmButton.IsEnabled)
            {
                ...
                await WaitHelper.Until(
                    ...
                    "Preview container did not appear after confirm"
                );
                break;
            }
            List<NGridCardHolder> list = (
                from c in UiHelper.FindAll<NGridCardHolder>(screen)
        ...
        await WaitHelper.Until(
            ...
            ct,
            TimeSpan.FromSeconds(5L),
            "Preview container did not appear"
        );
        await Task.Delay(500, ct);
        NConfirmButton previewConfirmButton = previewContainer?.GetNodeOrNull<NConfirmButton>(
            "Confirm"
        );
        ...
        await WaitHelper.Until(
            () => previewConfirmButton.IsEnabled,
            ct,
            TimeSpan.FromSeconds(5L),
            "Preview confirm button did not become enabled"
        );
        ...
        await WaitHelper.Until(
            ...
            "Transform screen did not close after confirmation"
        );

...
public class DeckEnchantScreenHandler : IScreenHandler, IHandler
{
    ...
    public async Task HandleAsync(Rng random, CancellationToken ct)
    {
        ...
        await WaitHelper.Until(
            ...
            "No cards found in enchant screen"
        );
        ...
        {
            singlePreviewContainer = screen.GetNodeOrNull<Control>(
            ...
            if (
                (singlePreviewContainer?.Visible ?? false)
                || (multiPreviewContainer?.Visible ?? false)
                || (nodeOrNull != null && nodeOrNull.IsEnabled)
            )
            {
                break;
            }
            List<NGridCardHolder> list = cards
                .Where((NGridCardHolder c) => !selectedCards.Contains(c))
                .ToList();
            if (list.Count == 0)
            {
                break;
        ...
        if (control == null || !control.Visible)
        {
            ...
            if (
                (control2 == null || !control2.Visible)
                && nodeOrNull != null
                && nodeOrNull.IsEnabled
            )
            {
        ...
        await WaitHelper.Until(
            delegate
            {
                ...
                multiPreviewContainer = screen.GetNodeOrNull<Control>(
            ...
            ct,
            TimeSpan.FromSeconds(5L),
            "Enchant preview container did not appear"
        );
        await Task.Delay(500, ct);
        NConfirmButton confirmButton = visiblePreviewContainer?.GetNodeOrNull<NConfirmButton>(
            "Confirm"
        );
        ...
        if (confirmButton != null)
        {
            await WaitHelper.Until(
                ...
                ct,
                TimeSpan.FromSeconds(5L),
                "Enchant confirm button did not become enabled"
            );
            ...
            await WaitHelper.Until(
                ...
                "Enchant screen did not close after confirmation"
            );
        }
        else
        {

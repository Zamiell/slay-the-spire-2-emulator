...
public class DeckUpgradeScreenHandler : IScreenHandler, IHandler
{
    ...
    public async Task HandleAsync(Rng random, CancellationToken ct)
    {
        ...
        for (int i = 0; i < maxSelections; i++)
        {
            ...
            if (
                (nodeOrNull != null && nodeOrNull.Visible)
                || (nodeOrNull2 != null && nodeOrNull2.Visible)
            )
            {
                break;
            }
        ...
        await WaitHelper.Until(
            delegate
            {
                Control nodeOrNull3 = screen.GetNodeOrNull<Control>(
                    "%UpgradeSinglePreviewContainer"
                );
                Control nodeOrNull4 = screen.GetNodeOrNull<Control>(
            ...
            "Upgrade preview did not appear"
        );
        ...
        await WaitHelper.Until(
            ...
            ct,
            TimeSpan.FromSeconds(5L),
            "Upgrade confirm button did not become enabled"
        );
        ...
        await WaitHelper.Until(
            ...
            "Upgrade screen did not close after confirmation"
        );
        AutoSlayLog.ExitScreen("NDeckUpgradeSelectScreen");
    }
}

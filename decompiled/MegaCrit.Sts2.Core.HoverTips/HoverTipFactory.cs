...
{
    private static readonly Dictionary<CardKeyword, HoverTip> _keywordHoverTips =
        new Dictionary<CardKeyword, HoverTip>();

    private static readonly Dictionary<ModelId, HoverTip> _potionHoverTips =
        new Dictionary<ModelId, HoverTip>();

    public static IEnumerable<IHoverTip> FromEnchantment<T>(int amount = 1)
    ...
    public static IEnumerable<IHoverTip> FromAffliction<T>(int amount = 1)
    ...
    public static IHoverTip FromPower<T>(int? amount = null)
        where T : PowerModel
    {
    ...
    public static IEnumerable<IHoverTip> FromPowerWithPowerHoverTips<T>(int? amount = null)
        where T : PowerModel
    {
        return new IHoverTip[1] { FromPower<T>(amount) }.Concat(ModelDb.Power<T>().HoverTips);
    }

    public static IHoverTip FromPotion<T>()
        where T : PotionModel
    {
    ...
    public static IHoverTip FromOrb<T>()
        where T : OrbModel
    {
        return ModelDb.Orb<T>().DumbHoverTip;
    }

    public static IEnumerable<IHoverTip> FromRelic<T>()
        where T : RelicModel
    {
    ...
    public static IEnumerable<IHoverTip> FromRelicExcludingItself<T>()
        where T : RelicModel
    {
        RelicModel relic = ModelDb.Relic<T>();
    ...
    public static IEnumerable<IHoverTip> FromCardWithCardHoverTips<T>(bool upgrade = false)
        where T : CardModel
    {
        return new IHoverTip[1] { FromCard<T>(upgrade) }.Concat(ModelDb.Card<T>().HoverTips);
    }

    public static IHoverTip FromCard<T>(bool upgrade = false)
        where T : CardModel
    {
    ...
    {
        string localCharacterEnergyIconPrefix =
        ...
        return new HoverTip(
            ...
            PreloadManager.Cache.GetTexture2D(
                (localCharacterEnergyIconPrefix != null)
                    ? EnergyIconHelper.GetPath(localCharacterEnergyIconPrefix)
                    : path
            )
        );
    }

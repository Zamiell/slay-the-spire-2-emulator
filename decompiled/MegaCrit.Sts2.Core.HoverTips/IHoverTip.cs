...
{
    private static readonly string _summonStaticId = HoverTipFactory
        .Static(StaticHoverTip.SummonStatic)
        .Id;

    private static readonly string _summonDynamicId = HoverTipFactory
        .Static(StaticHoverTip.SummonDynamic, new SummonVar(0m))
        .Id;
    ...
    static IEnumerable<IHoverTip> RemoveDupes(IEnumerable<IHoverTip> tips)
    {
        ...
        foreach (IHoverTip hoverTip in tips)
        {
            ...
            IHoverTip hoverTip2 = list.FirstOrDefault(
                (IHoverTip tip) => tip.Id == hoverTip.Id && !tip.IsInstanced
            );
            if (hoverTip2 == null)
            {
                list.Add(hoverTip);

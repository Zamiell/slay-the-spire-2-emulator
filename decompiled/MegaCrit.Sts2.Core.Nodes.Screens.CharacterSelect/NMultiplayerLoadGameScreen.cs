...
public class NMultiplayerLoadGameScreen : NSubmenu, ILoadRunLobbyListener
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/multiplayer_load_game_screen"
    );
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlyArray<string>(
            new string[2] { _scenePath, "res://scenes/screens/char_select/char_select_button.tscn" }
        );
    ...
    public static NMultiplayerLoadGameScreen? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NMultiplayerLoadGameScreen>(PackedScene.GenEditState.Disabled);
    }
    ...
    private void UpdateRichPresence()
    {
        if (_runLobby.NetService.Type.IsMultiplayer())
        {
            PlatformUtil.SetRichPresence(
                "LOADING_MP_LOBBY",
                _runLobby.NetService.GetRawLobbyIdentifier(),
                _runLobby.ConnectedPlayerIds.Count
            );
        }
    }
    ...
    public async Task<bool> ShouldAllowRunToBegin()
    {
        ...
        locString.Add(
            "MissingCount",
            _runLobby.Run.Players.Count - _runLobby.ConnectedPlayerIds.Count
        );
    ...
    private async Task StartRun()
    {
        Log.Info(
            "Loading a multiplayer run. Players: "
                + string.Join(",", _runLobby.ConnectedPlayerIds)
                + "."
        );
        SerializablePlayer serializablePlayer = _runLobby.Run.Players.First(
            (SerializablePlayer p) => p.NetId == _runLobby.NetService.NetId
        );
        SfxCmd.Play(
    ...
    private void AfterMultiplayerStarted()
    {
        NGame.Instance.RemoteCursorContainer.Initialize(
            _runLobby.InputSynchronizer,
            _runLobby.ConnectedPlayerIds
        );
        ...
        Control control = PreloadManager
            .Cache.GetScene(byId.CharacterSelectBg)

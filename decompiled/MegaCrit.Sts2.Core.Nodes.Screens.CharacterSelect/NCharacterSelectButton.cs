...
public class NCharacterSelectButton : NButton
{
    ...
    public new class MethodName : NButton.MethodName
    {
        ...
        public static readonly StringName AnimateSaturationToCurrentState =
            "AnimateSaturationToCurrentState";
    ...
    private static readonly string _playerIconScenePath = SceneHelper.GetScenePath(
        "screens/char_select/char_select_player_icon"
    );

    private static readonly string _unlockedIconPath = ImageHelper.GetImagePath(
        "packed/character_select/char_select_lock3_unlocked.png"
    );
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlyArray<string>(
            new string[2] { _playerIconScenePath, _unlockedIconPath }
        );
    ...
    public void Init(CharacterModel character, ICharacterSelectButtonDelegate del)
    {
        ...
        {
            IsRandom = true;
            _isLocked = ModelDb.AllCharacters.Any(
                (CharacterModel c) => !unlockState.Characters.Contains(c)
            );
        }
        else
        {
    ...
    protected override void OnFocus()
    {
        if (!_isSelected)
        {
            ...
            if (_isLocked)
            {
                HoverTip hoverTip = new HoverTip(
                    new LocString("main_menu_ui", "CHARACTER_SELECT.locked.title"),
                    _character.GetUnlockText()
                );
                NHoverTipSet
                    .CreateAndShow(this, hoverTip)
        ...
        if (_currentOutline != null)
        {
            if (_isSelected)
            {
                float a = Mathf.Lerp(
                    0.35f,
                    1f,
                    (Mathf.Cos((float)Time.GetTicksMsec() * 0.001f * 1.6f * (float)Math.PI) + 1f)
                        * 0.5f
                );
    ...
    public async Task AnimateUnlock()
    {
        ...
        {
            Vector2 vector =
                Vector2.Right.Rotated(Rng.Chaotic.NextFloat((float)Math.PI * 2f)) * num;
        ...
        tween
            .TweenProperty(_iconAdd, "modulate:a", 0f, 1.0)
    ...
    private void RefreshState()
    {
        State state = (
            _isSelected
                ? State.SelectedLocally
                : ((_remoteSelectedPlayers.Count > 0) ? State.SelectedRemotely : State.NotSelected)
        );
        State state2 = _state;
        if (state2 != state)
        {
    ...
    private void RefreshPlayerIcons()
    {
        if (_delegate != null && _delegate.Lobby.NetService.Type != NetGameType.Singleplayer)
        {
            ...
            {
                TextureRect child = PreloadManager
                    .Cache.GetScene(_playerIconScenePath)
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnRemotePlayerSelected,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnRemotePlayerDeselected,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetSaturationForCurrentState,
                new PropertyInfo(
                    Variant.Type.Float,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetValueForCurrentState,
                new PropertyInfo(
                    Variant.Type.Float,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.AnimateSaturationToCurrentState,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,

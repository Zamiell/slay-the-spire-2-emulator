        ...
        ICharacterSelectButtonDelegate
{
    public new class MethodName : NSubmenu.MethodName
    {
        ...
        public static readonly StringName CheckForMultiplayerAscensionPopup =
        ...
        public static new readonly StringName OnSubmenuOpened = "OnSubmenuOpened";

        public static new readonly StringName OnSubmenuClosed = "OnSubmenuClosed";
    ...
    {
        public static new readonly StringName InitialFocusedControl = "InitialFocusedControl";
        ...
        public static new readonly StringName _backButton = "_backButton";
        ...
        public static readonly StringName _characterUnlockAnimationBackstop =
            "_characterUnlockAnimationBackstop";
        ...
        public static readonly StringName _delayEmbarkForCharacterSelect =
            "_delayEmbarkForCharacterSelect";
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
    ...
    private const string _sceneCharSelectButtonPath =
    ...
    public static NCharacterSelectScreen? Create()
    {
        ...
        return ResourceLoader
            .Load<PackedScene>(_scenePath, null, ResourceLoader.CacheMode.Reuse)
            .Instantiate<NCharacterSelectScreen>(PackedScene.GenEditState.Disabled);
    }
    ...
    public override void _Ready()
    {
        ...
        _relicTitle = GetNode<MegaRichTextLabel>(
        ...
        GetNode<MegaRichTextLabel>("%WaitingForPlayers").Text = new LocString(
            "main_menu_ui",
            "CHARACTER_SELECT.waitingForPlayers"
        ).GetFormattedText();
    ...
    public void InitializeMultiplayerAsHost(INetGameService gameService, int maxPlayers)
    {
        if (gameService.Type != NetGameType.Host)
        {
            throw new InvalidOperationException(
                $"Initialized character select screen with GameService of type {gameService.Type} when hosting!"
            );
        }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                $"Initialized character select screen with GameService of type {gameService.Type} when joining!"
            );
        }
    ...
    {
        if (
            SaveManager.Instance.Progress.MaxMultiplayerAscension > 0
            && !SaveManager.Instance.SeenPopup("ascension_multiplayer_ftue")
        )
        {
            NAscensionMultiplayerFtue nAscensionMultiplayerFtue =
                NAscensionMultiplayerFtue.Create();
    ...
    private void InitCharacterButtons()
    {
        ...
        {
            NCharacterSelectButton nCharacterSelectButton =
                _charSelectButtonScene.Instantiate<NCharacterSelectButton>(
        ...
        UpdateRandomCharacterVisibility();
        List<NCharacterSelectButton> list = (
            from c in _charButtonContainer.GetChildren().OfType<NCharacterSelectButton>()
            where c.Visible
            select c
        ).ToList();
        for (int num = 0; num < list.Count; num++)
        {
            ...
            list[num].FocusNeighborRight = (
                (num < list.Count - 1) ? list[num + 1].GetPath() : list[0].GetPath()
            );
        }
    }
    ...
    {
        foreach (
            NCharacterSelectButton item in _charButtonContainer
                .GetChildren()
    ...
    public override void OnSubmenuOpened()
    {
        ...
        else
        {
            TaskHelper.RunSafely(
                PlayUnlockCharacterAnimation(SaveManager.Instance.Progress.PendingCharacterUnlock)
            );
        }
    ...
    private async Task PlayUnlockCharacterAnimation(ModelId character)
    {
        ...
        foreach (
            NCharacterSelectButton button in _charButtonContainer
                .GetChildren()
                .OfType<NCharacterSelectButton>()
        )
        {
    ...
    private void OnLocalCharacterChangedForRandom(CharacterModel characterModel)
    {
        ...
        Control control = PreloadManager
            .Cache.GetScene(characterModel.CharacterSelectBg)
    ...
    {
        Log.Info(
            $"Embarking on a singleplayer {_lobby.LocalPlayer.character.Id.Entry} run. Ascension: {_lobby.Ascension} Seed: {seed}"
        );
        ...
        await NGame.Instance.Transition.FadeOut(
            0.8f,
            _lobby.LocalPlayer.character.CharacterSelectTransitionPath
        );
        await NGame.Instance.StartNewSingleplayerRun(
            _lobby.LocalPlayer.character,
            shouldSave: true,
            acts,
    ...
    {
        Log.Info(
            $"Embarking on a multiplayer run. Players: {string.Join(",", _lobby.Players)}. Ascension: {_lobby.Ascension} Seed: {seed}"
        );
        if (_delayEmbarkForCharacterSelect)
        {
            await Cmd.Wait(1f);
        ...
        if (settings != null && settings.BootstrapInMultiplayer)
        {
            using (new NetLoadingHandle(_lobby.NetService))
            {
                ...
                await PreloadManager.LoadRunAssets(
                    runState.Players.Select((Player p) => p.Character)
                );
                ...
                switch (_settings.RoomType)
                {
                    ...
                    default:
                        await RunManager.Instance.EnterRoomDebug(
                            _settings.RoomType,
                            MapPointType.Unassigned,
                            _settings.RoomType.IsCombatRoom()
                                ? _settings.Encounter.ToMutable()
                                : null,
                            showTransition: false
                        );
                        break;
        ...
        {
            await NGame.Instance.StartNewMultiplayerRun(
                _lobby,
                shouldSave: true,
                acts,
                Array.Empty<ModifierModel>(),
    ...
    )
    {
        ...
        _infoPanelTween
            .TweenProperty(_infoPanel, "position", _infoPanel.Position, 0.5)
        ...
        if (!charSelectButton.IsLocked)
        {
            ...
            control.Name = characterModel.Id.Entry + "_bg";
            _bgContainer.AddChildSafely(control);
            string formattedText = new LocString(
                "characters",
                characterModel.CharacterSelectTitle
            ).GetFormattedText();
            _name.SetTextAutoSize(formattedText);
            _description.Text = new LocString(
                "characters",
                characterModel.CharacterSelectDesc
            ).GetFormattedText();
        ...
        {
            _embarkButton.Disable();
            string formattedText2 = new LocString(
                "main_menu_ui",
                "CHARACTER_SELECT.locked.title"
            ).GetFormattedText();
            ...
            if (!_selectedButton.IsRandom)
            {
                ...
                _relicTitle.Text = new LocString(
                    "main_menu_ui",
                    "CHARACTER_SELECT.lockedRelic.title"
                ).GetFormattedText();
                _relicDescription.Text = new LocString(
                    "main_menu_ui",
                    "CHARACTER_SELECT.lockedRelic.description"
                ).GetFormattedText();
    ...
    private void UpdateRichPresence()
    {
        if (_lobby.NetService.Type.IsMultiplayer())
        {
            PlatformUtil.SetRichPresence(
                "STANDARD_MP_LOBBY",
                _lobby.NetService.GetRawLobbyIdentifier(),
                _lobby.Players.Count
            );
        }
    }
    ...
    private void RefreshButtonSelectionForPlayer(LobbyPlayer player)
    {
        ...
        {
            if (
                item.RemoteSelectedPlayers.Contains(player.id)
    ...
    {
        NGame.Instance.RemoteCursorContainer.Initialize(
            _lobby.InputSynchronizer,
            _lobby.Players.Select((LobbyPlayer p) => p.id)
        );
        ...
        UpdateRichPresence();
        UpdateRandomCharacterVisibility();
        MegaCrit.Sts2.Core.Logging.Logger.logLevelTypeMap[LogType.Network] = (
            (_lobby.NetService.Type == NetGameType.Singleplayer) ? LogLevel.Info : LogLevel.Debug
        );
        MegaCrit.Sts2.Core.Logging.Logger.logLevelTypeMap[LogType.Actions] = (
            (_lobby.NetService.Type == NetGameType.Singleplayer)
                ? LogLevel.Info
                : LogLevel.VeryDebug
        );
        MegaCrit.Sts2.Core.Logging.Logger.logLevelTypeMap[LogType.GameSync] = (
            (_lobby.NetService.Type == NetGameType.Singleplayer)
                ? LogLevel.Info
                : LogLevel.VeryDebug
        );
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.CheckForMultiplayerAscensionPopup,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.InitializeSingleplayer,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.InitCharacterButtons,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateRandomCharacterVisibility,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.DebugUnlockAllCharacters,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnAscensionPanelLevelChanged,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.MaxAscensionChanged,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._relicDescription,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._relicIconOutline,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._charButtonContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._readyAndWaitingContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._actDropdownLabel,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._remotePlayerContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._characterUnlockAnimationBackstop,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._randomCharacterButton,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                ...
                PropertyName._infoPanelPosFinalVal,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._delayEmbarkForCharacterSelect,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._charSelectButtonScene,
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.InitialFocusedControl,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName.ShouldShowActDropdown,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._readyAndWaitingContainer,
            Variant.From(in _readyAndWaitingContainer)
        );
        ...
        info.AddProperty(
            PropertyName._remotePlayerContainer,
            Variant.From(in _remotePlayerContainer)
        );
        info.AddProperty(
            PropertyName._characterUnlockAnimationBackstop,
            Variant.From(in _characterUnlockAnimationBackstop)
        );
        info.AddProperty(
            PropertyName._randomCharacterButton,
            Variant.From(in _randomCharacterButton)
        );
        info.AddProperty(PropertyName._infoPanelTween, Variant.From(in _infoPanelTween));
        info.AddProperty(
            PropertyName._infoPanelPosFinalVal,
            Variant.From(in _infoPanelPosFinalVal)
        );
        info.AddProperty(
            PropertyName._delayEmbarkForCharacterSelect,
            Variant.From(in _delayEmbarkForCharacterSelect)
        );
        info.AddProperty(
            PropertyName._charSelectButtonScene,
            Variant.From(in _charSelectButtonScene)
        );
    }

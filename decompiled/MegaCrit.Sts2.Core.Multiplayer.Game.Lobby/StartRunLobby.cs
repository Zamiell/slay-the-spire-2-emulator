...
public class StartRunLobby
{
    ...
    )
    {
        ...
        NetService.RegisterMessageHandler<ClientLobbyJoinRequestMessage>(
            HandleClientLobbyJoinRequestMessage
        );
        NetService.RegisterMessageHandler<ClientLoadJoinRequestMessage>(
            HandleClientLoadJoinRequestMessage
        );
        NetService.RegisterMessageHandler<ClientRejoinRequestMessage>(
        ...
        NetService.RegisterMessageHandler<LobbyAscensionChangedMessage>(
            HandleAscensionChangedMessage
        );
        NetService.RegisterMessageHandler<LobbySeedChangedMessage>(HandleSeedChangedMessage);
        NetService.RegisterMessageHandler<LobbyModifiersChangedMessage>(
            HandleModifiersChangedMessage
        );
    ...
    {
        NetService.UnregisterMessageHandler<ClientLobbyJoinRequestMessage>(
            HandleClientLobbyJoinRequestMessage
        );
        NetService.UnregisterMessageHandler<ClientLoadJoinRequestMessage>(
            HandleClientLoadJoinRequestMessage
        );
        NetService.UnregisterMessageHandler<ClientRejoinRequestMessage>(
            HandleClientRejoinRequestMessage
        );
        ...
        NetService.UnregisterMessageHandler<LobbyAscensionChangedMessage>(
            HandleAscensionChangedMessage
        );
        NetService.UnregisterMessageHandler<LobbySeedChangedMessage>(HandleSeedChangedMessage);
        NetService.UnregisterMessageHandler<LobbyModifiersChangedMessage>(
            HandleModifiersChangedMessage
        );
    ...
    {
        LobbyPlayer? result = TryAddPlayerInFirstAvailableSlot(
            unlockState,
    ...
    )
    {
        if (NetService.Type != NetGameType.Host)
        {
            throw new InvalidOperationException(
                "Received ClientLobbyJoinRequestMessage as non-host!"
            );
        }
        INetHostGameService netHostGameService = (INetHostGameService)NetService;
        try
        {
            ...
            {
                _logger.Warn(
                    $"Client {senderId} sent ClientLobbyJoinRequestMessage but we are at maximum players!"
                );
            ...
            LobbyPlayer? lobbyPlayer = TryAddPlayerInFirstAvailableSlot(
                message.unlockState,
            ...
            _logger.Debug(
                $"Sending ClientLobbyJoinResponseMessage length ({message2.playersInLobby.Count}) to ({lobbyPlayer.Value.id})"
            );
    ...
    private void HandleClientLoadJoinRequestMessage(ClientLoadJoinRequestMessage _, ulong senderId)
    {
        ...
        {
            throw new InvalidOperationException(
                "Received ClientLoadJoinRequestMessage as non-host!"
            );
        }
    ...
    {
        _logger.Debug(
            $"Received LobbyPlayerChangedCharacterMessage for {senderId} {message.character}"
        );
    ...
    {
        _logger.Debug(
            "Received ModifiersChangedMessage, new modifiers: "
                + string.Join(",", message.modifiers.Select((SerializableModifier m) => m.Id))
        );
        _modifiers.Clear();
        _modifiers.AddRange(message.modifiers.Select(ModifierModel.FromSerializable));
    ...
    {
        _logger.Debug(
            $"Received LobbyPlayerSetReadyMessage for player {senderId} with value {message.ready}"
        );
    ...
    private void HandleLobbyBeginRunMessage(LobbyBeginRunMessage message, ulong senderId)
    {
        ...
        BeginRunLocally(
            message.seed,
            message.modifiers.Select(ModifierModel.FromSerializable).ToList()
        );
    }
    ...
    private void BeginRunLocally(string seed, List<ModifierModel> modifiers)
    {
        ...
        List<ActModel> list = ActModel
            .GetRandomList(rng, GetUnlockState(), NetService.Type.IsMultiplayer())
            .ToList();
        ...
        {
            CharacterStats orCreateCharacterStats =
                SaveManager.Instance.Progress.GetOrCreateCharacterStats(Players[0].character.Id);
    ...
    private void SetSingleplayerAscensionAfterCharacterChanged(ModelId characterId)
    {
        ...
        CharacterStats orCreateCharacterStats =
            SaveManager.Instance.Progress.GetOrCreateCharacterStats(characterId);
        ...
        if (characterId == ModelDb.GetId<RandomCharacter>())
        {
            ...
            orCreateCharacterStats == null
            || orCreateCharacterStats.MaxAscension <= 0
            || !flag
        )
        {
            MaxAscension = 0;
            ...
            {
                _logger.Info(
                    $"{characterId} has not revealed the Ascension Epoch, disabling Ascension."
                );
            }
            else
            {
        ...
        else
        {
            ...
            SyncAscensionChange(
                Math.Min(
                    orCreateCharacterStats.PreferredAscension,
                    orCreateCharacterStats.MaxAscension
                )
            );
            LobbyListener.MaxAscensionChanged();
        ...
        if (
            ...
            && GetMaxAscensionAcrossAllCharacters() > 0
            && !SaveManager.Instance.SeenPopup("ascension_singleplayer_ftue")
        )
        {
            NAscensionSingleplayerFtue nAscensionSingleplayerFtue =
                NAscensionSingleplayerFtue.Create();
    ...
    private void UpdatePreferredAscension()
    {
        ...
        if (NetService.Type == NetGameType.Singleplayer)
        {
            ...
            {
                CharacterStats orCreateCharacterStats =
                    SaveManager.Instance.Progress.GetOrCreateCharacterStats(
                        LocalPlayer.character.Id
                    );
                if (
                    orCreateCharacterStats.MaxAscension != 0
                ...
                {
                    _logger.Info(
                        $"Setting preferred Ascension for {LocalPlayer.character.Id} to {Ascension}"
                    );
                    orCreateCharacterStats.PreferredAscension = Ascension;
    ...
    public void SetSeed(string? seed)
    {
        ...
        Seed = seed;
        LobbySeedChangedMessage message = new LobbySeedChangedMessage { seed = seed };
        NetService.SendMessage(message);
        LobbyListener.SeedChanged();
    }
    ...
    public void SetReady(bool ready)
    {
        ...
        {
            throw new InvalidOperationException(
                "Tried to set local player ready, but they are not in the list of players in the lobby!"
            );
        }
        LobbyPlayer value = Players[num];
        value.isReady = ready;
        Players[num] = value;
        LobbyPlayerSetReadyMessage message = new LobbyPlayerSetReadyMessage { ready = ready };
        NetService.SendMessage(message);
    ...
    private async Task BeginHandshakeTimeout(ConnectingPlayer connectingPlayer)
    {
        ...
        if (!connectingPlayer.timeoutCancelToken.IsCancellationRequested)
        {
            ...
            {
                _logger.Info(
                    $"Disconnecting player {connectingPlayer.id} because they did not respond to the initial game join handshake within {HandshakeTimeout}ms"
                );
                INetHostGameService netHostGameService = (INetHostGameService)NetService;
    ...
    private void OnDisconnectedFromClientAsHost(ulong playerId, NetErrorInfo info)
    {
        ...
        {
            _logger.Info(
                $"Player {playerId} not found in players list. Assuming they disconnected during the handshake"
            );
            return;
        }
        LobbyPlayer lobbyPlayer = Players[num];
        PlayerLeftMessage message = new PlayerLeftMessage { playerId = playerId };
        NetService.SendMessage(message);
        Players.RemoveAt(num);
        InputSynchronizer.OnPlayerDisconnected(lobbyPlayer.id);
        LobbyListener.RemotePlayerDisconnected(lobbyPlayer);
    ...
    private UnlockState GetUnlockState()
    {
        ...
        return new UnlockState(
            Players.Select((LobbyPlayer p) => UnlockState.FromSerializable(p.unlockState))
        );
    }

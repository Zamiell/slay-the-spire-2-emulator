...
public class RunLobby
{
    ...
    )
    {
        ...
        _netService.RegisterMessageHandler<ClientLobbyJoinRequestMessage>(
            HandleClientLobbyJoinRequestMessage
        );
        _netService.RegisterMessageHandler<ClientLoadJoinRequestMessage>(
            HandleClientLoadJoinRequestMessage
        );
        _netService.RegisterMessageHandler<ClientRejoinRequestMessage>(
        ...
        if (
            ...
            && netService is NetHostGameService netHostGameService
        )
        {
    ...
    public void Dispose()
    {
        _netService.UnregisterMessageHandler<ClientLobbyJoinRequestMessage>(
            HandleClientLobbyJoinRequestMessage
        );
        _netService.UnregisterMessageHandler<ClientLoadJoinRequestMessage>(
            HandleClientLoadJoinRequestMessage
        );
        _netService.UnregisterMessageHandler<ClientRejoinRequestMessage>(
            HandleClientRejoinRequestMessage
        );
        ...
        if (
            _netService.Type == NetGameType.Host
            && _netService is NetHostGameService netHostGameService
        )
        {
            netHostGameService.ClientConnected -= OnConnectedToClientAsHost;
            netHostGameService.ClientDisconnected -= OnDisconnectedFromClientAsHost;
    ...
    private void HandleClientRejoinRequestMessage(
    ...
    )
    {
        ...
        PlayerRejoinedMessage message2 = new PlayerRejoinedMessage { playerId = senderId };
    ...
    {
        _logger.Info(
            $"Player {playerId} disconnected from host. Is in connected players: {_connectedPlayerIds.Contains(playerId)}. Reason: {info.GetReason()}"
        );

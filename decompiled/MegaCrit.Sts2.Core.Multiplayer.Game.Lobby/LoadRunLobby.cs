...
public class LoadRunLobby
{
    ...
    public void CleanUp(bool disconnectSession)
    {
        ...
        NetService.UnregisterMessageHandler<PlayerReconnectedMessage>(
            HandlePlayerReconnectedMessage
        );
    ...
    )
    {
        ...
        try
        {
            ...
            {
                _logger.Warn(
                    $"Client {senderId} sent ClientLoadJoinRequestMessage but they are not in the loaded run!"
                );
    ...
    private void HandleClientLobbyJoinRequestMessage(
        ClientLobbyJoinRequestMessage _,
        ulong senderId
    )
    {
        if (NetService.Type != NetGameType.Host)
    ...
    {
        if (
            (NetService.Type == NetGameType.Host || NetService.Type == NetGameType.Singleplayer)
            && IsAboutToBeginGame()
        )
        {

...
public sealed class SharedRelicPool : RelicPoolModel
{
    ...
    {
        return new global::_003C_003Ez__ReadOnlyArray<RelicModel>(
            new RelicModel[118]
            {
    ...
    public override IEnumerable<RelicModel> GetUnlockedRelics(UnlockState unlockState)
    {
        ...
        {
            list.RemoveAll(
                (RelicModel r) => Relic1Epoch.Relics.Any((RelicModel relic) => relic.Id == r.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Relic2Epoch>())
        {
            list.RemoveAll(
                (RelicModel r) => Relic2Epoch.Relics.Any((RelicModel relic) => relic.Id == r.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Relic3Epoch>())
        {
            list.RemoveAll(
                (RelicModel r) => Relic3Epoch.Relics.Any((RelicModel relic) => relic.Id == r.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Relic4Epoch>())
        {
            list.RemoveAll(
                (RelicModel r) => Relic4Epoch.Relics.Any((RelicModel relic) => relic.Id == r.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Relic5Epoch>())
        {
            list.RemoveAll(
                (RelicModel r) => Relic5Epoch.Relics.Any((RelicModel relic) => relic.Id == r.Id)
            );
        }
        return list;
    }

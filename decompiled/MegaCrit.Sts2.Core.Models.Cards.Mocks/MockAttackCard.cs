...
public sealed class MockAttackCard : MockCardModel
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        int hitCount = (
            base.EnergyCost.CostsX
                ? ResolveEnergyXValue()
                : ((!HasStarCostX) ? _hitCount : ResolveStarXValue())
        );
        AttackCommand attackCommand = DamageCmd
            .Attack(
                _fromOsty
                    ? base.DynamicVars.OstyDamage.BaseValue
                    : base.DynamicVars.Damage.BaseValue
            )
            .WithHitCount(hitCount);
        attackCommand = (
            (!_fromOsty)
                ? attackCommand.FromCard(this)
                : attackCommand.FromOsty(base.Owner.Osty, this)
        );

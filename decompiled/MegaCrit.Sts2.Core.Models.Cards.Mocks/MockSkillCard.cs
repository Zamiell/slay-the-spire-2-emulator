...
public sealed class MockSkillCard : MockCardModel
{
    ...
    public MockSkillCard MockPower<TPower>(int amount, TargetType targetType)
        where TPower : PowerModel
    {
        ...
        if (_powerApplications.Any((PowerApplication a) => a.targetType != targetType))
        {
            throw new InvalidOperationException(
                "Cannot have multiple power applications with different target types."
            );
        }
        _targetType = targetType;
        _powerApplications.Add(
    ...
    public MockSkillCard MockCreateCards<TCard>(int amount)
        where TCard : CardModel
    {
        AssertMutable();
        _cardCreations.Add(
            new CardCreation { canonicalCard = ModelDb.Card<TCard>(), amount = amount }
        );
        return this;
    }
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
            ...
            await CardCmd.Discard(
                ...
                await CardSelectCmd.FromHandForDiscard(
                    prefs: new CardSelectorPrefs(
                        new LocString("cards", "MOCK_SKILL_CARD.discardSelectionPrompt"),
                        base.DynamicVars["Discard"].IntValue
                    ),
        ...
        {
            foreach (
                Creature item in application.targetType switch
                {
                    ...
                    TargetType.Self => new global::_003C_003Ez__ReadOnlySingleElementList<Creature>(
                        base.Owner.Creature
                    ),
            ...
            {
                PowerModel power = ModelDb
                    .GetById<PowerModel>(ModelDb.GetId(application.powerType))
                    .ToMutable();

...
public class Creature
{
    ...
    public string Name
    {
        get
        {
            ...
            return PlatformUtil.GetPlayerNameRaw(
    ...
    public void SetUniqueMonsterHpValue(IReadOnlyList<Creature> creaturesOnSide, Rng rng)
    {
        ...
        hashSet.ExceptWith(
            from e in creaturesOnSide.Except(
                new global::_003C_003Ez__ReadOnlySingleElementList<Creature>(this)
            )
            select e.MaxHp
        );
        MonsterMaxHpBeforeModification = (
            _currentHp = (
                _maxHp = (
                    (hashSet.Count <= 0) ? rng.NextInt(minInitialHp, num) : rng.NextItem(hashSet)
                )
            )
        );
    }

    public void ScaleMonsterHpForMultiplayer(
    ...
    public IEnumerable<T> GetPowerInstances<T>()
        where T : PowerModel
    {
    ...
    public void ApplyPowerInternal(PowerModel power)
    {
        ...
        if (
            ...
            && _powers.Any((PowerModel p) => p.GetType() == power.GetType())
        )
        {
            throw new InvalidOperationException(
                "Trying to add multiple instances of a non-instanced power to a creature."
            );
        }
        _powers.Add(power);
    ...
    public void InvokePowerModified(PowerModel power, int change, bool silent)
    {
        if (change > 0)
        {
            ...
            power.StackType.Equals(PowerStackType.Counter)
            && power.AllowNegative
            && change < 0
        )
        {
    ...
    public IEnumerable<PowerModel> RemoveAllPowersInternalExcept(
        IEnumerable<PowerModel>? except = null
    )
    {
    ...
    public IEnumerable<PowerModel> RemoveAllPowersAfterDeath()
    {
        return RemoveAllPowersInternalExcept(
            _powers.Where(
                (PowerModel p) =>
                    !p.ShouldPowerBeRemovedAfterOwnerDeath() || !Hook.ShouldPowerBeRemovedOnDeath(p)
            )
        );
    }

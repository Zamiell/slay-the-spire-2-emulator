...
public class NBgLayerDebug : Control
{
    ...
    )
    {
        ...
        {
            AddLayer(
                VariantUtils.ConvertTo<LayerVisibility>(in args[0]),
                VariantUtils.ConvertTo<PackedScene>(in args[1])
            );
            ret = default(godot_variant);
            return true;
        }
        if (method == MethodName.ToLayerName && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<string>(
                ToLayerName(VariantUtils.ConvertTo<LayerVisibility>(in args[0]))
            );
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                ...
                PropertyName.ReloadLayersCallable,

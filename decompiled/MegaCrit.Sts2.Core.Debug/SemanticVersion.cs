...
public class SemanticVersion : IComparable<SemanticVersion>, IEquatable<SemanticVersion>
{
    ...
    public static bool TryFromString(string str, out SemanticVersion? version)
    {
        try
        {
            ...
            when (((ex is InvalidOperationException || ex is FormatException) ? 1 : 0) != 0)
        {
            version = null;
            return false;
        }
    ...
    public static SemanticVersion FromString(string version)
    {
                ...
                switch (parseState)
                {
                    ...
                    case ParseState.Patch:
                        throw new InvalidOperationException(
                            $"Version {version} has a . in an invalid place! Parse state is {parseState}, index is {i}"
                        );
        ...
        switch (parseState)
        {
            ...
            default:
                throw new InvalidOperationException(
                    $"Version terminated in an invalid place! Parse state is {parseState}"
                );

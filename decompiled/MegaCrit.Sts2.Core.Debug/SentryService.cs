...
public static class SentryService
{
    ...
    private static readonly StringName _setShouldSampleEventMethod = new StringName(
        "set_should_sample_event"
    );

    private static readonly StringName _setPlatformBranchMethod = new StringName(
        "set_platform_branch"
    );

    private static readonly StringName _setCsharpContextMethod = new StringName(
        "set_csharp_context"
    );
    ...
    public static void Initialize()
    {
        ...
        _sentryInstance = SentrySdk.Init(
            delegate(SentryOptions options)
            {
                options.Dsn = dsn;
                ...
                options.SetBeforeSend(
                    delegate(SentryEvent sentryEvent, SentryHint hint)
                    {
    ...
    public static void AfterGameInit(string? platformBranch, string uniqueId, Node treeRoot)
    {
        ...
        _sentryInit?.Call(
            _setCsharpContextMethod,
            _sessionId,
            AssemblyHasher.GetMainAssemblyHash()
        );
        ...
        else
        {
            SentrySdk.ConfigureScope(
                delegate(Scope scope)
                {
                    scope.User = new SentryUser { Id = uniqueId };
                }
            );
    ...
    )
    {
        try
        {
            if (Engine.HasSingleton(_sentrySdkSingleton))
            {
                ...
                GodotObject godotObject = ClassDB
                    .ClassCallStatic(_sentryBreadcrumbClass, _createMethod, message)
                    .AsGodotObject();
    ...
    private static void SetPlatformBranch(string? branch)
    {
        ...
        _sentryInit?.Call(
            _setShouldSampleEventMethod,
            Callable.From((Func<bool>)ShouldSampleEvent)
        );
        if (branch != null)
        {
    ...
    )
    {
        if (IsEnabled)
        {
            SentryEvent evt = new SentryEvent { Message = message, Level = level };
    ...
    public static void InitializeForTesting()
    {
        ...
        _sentryInstance = SentrySdk.Init(
            delegate(SentryOptions options)
            {
                ...
                options.Release =
                    ReleaseInfoManager.Instance.ReleaseInfo?.Version ?? "dev-console-test";
                options.Debug = false;
        ...
        try
        {
            ...
            if (RunManager.Instance.IsInProgress && runState != null)
            {
                ...
                if (players.Count > 0)
                {
                    scope.SetExtra(
                        "game.characters",
                        string.Join(", ", players.Select((Player p) => p.Character.Id))
                    );
            ...
            if (combatState != null)
            {
                ...
                scope.SetExtra(
                    ...
                    string.Join(
                        ", ",
                        combatState.Enemies.Select(
                            (Creature e) => e.Monster?.Id.ToString() ?? "unknown"
                        )
                    )
                );
                List<string> list = combatState
                    .Players.Select((Player p) => $"{p.Creature.CurrentHp}/{p.Creature.MaxHp}")
                    .ToList();
                if (list.Count > 0)
                {

...
public static class FileDropHandler
{
    ...
    private static async Task OnRunHistoryDropped(string file)
    {
        ...
        ReadSaveResult<RunHistory> readSaveResult = JsonSerializationUtility.FromJson<RunHistory>(
            fileAccess.GetAsText()
        );
        if (!readSaveResult.Success)
        {
            Log.Error(
                $"Couldn't read {file}: {readSaveResult.ErrorMessage} ({readSaveResult.Status})"
            );
            return;
        }

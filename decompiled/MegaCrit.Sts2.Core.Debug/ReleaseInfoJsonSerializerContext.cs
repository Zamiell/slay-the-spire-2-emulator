...
[JsonSourceGenerationOptions(
    ...
    Converters = new Type[] { typeof(CustomDateTimeConverter) }
)]
...
internal class ReleaseInfoJsonSerializerContext : JsonSerializerContext, IJsonTypeInfoResolver
{
    ...
    public JsonTypeInfo<ReleaseInfo> ReleaseInfo =>
        _ReleaseInfo
        ?? (
            _ReleaseInfo = (JsonTypeInfo<ReleaseInfo>)base.Options.GetTypeInfo(typeof(ReleaseInfo))
        );

    public JsonTypeInfo<DateTime> DateTime =>
        _DateTime
        ?? (_DateTime = (JsonTypeInfo<DateTime>)base.Options.GetTypeInfo(typeof(DateTime)));
    ...
    public static ReleaseInfoJsonSerializerContext Default { get; } =
        new ReleaseInfoJsonSerializerContext(new JsonSerializerOptions(s_defaultOptions));
    ...
    private JsonTypeInfo<ReleaseInfo> Create_ReleaseInfo(JsonSerializerOptions options)
    {
        if (
            !TryGetTypeInfoForRuntimeCustomConverter(
                options,
                out JsonTypeInfo<ReleaseInfo> jsonTypeInfo
            )
        )
        {
            JsonObjectInfoValues<ReleaseInfo> jsonObjectInfoValues =
                new JsonObjectInfoValues<ReleaseInfo>();
            ...
            jsonObjectInfoValues.ConstructorAttributeProviderFactory = () =>
                typeof(ReleaseInfo).GetConstructor(
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic,
                    null,
                    Array.Empty<Type>(),
                    null
    ...
    private static JsonPropertyInfo[] ReleaseInfoPropInit(JsonSerializerOptions options)
    {
        ...
        jsonPropertyInfoValues.AttributeProviderFactory = () =>
            typeof(ReleaseInfo).GetProperty(
                "Commit",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic,
                null,
                typeof(string),
        ...
        JsonPropertyInfoValues<DateTime> jsonPropertyInfoValues2 =
            new JsonPropertyInfoValues<DateTime>();
        ...
        jsonPropertyInfoValues2.Converter =
            (JsonConverter<DateTime>)
                ExpandConverter(typeof(DateTime), new CustomDateTimeConverter(), options);
    ...
    private JsonTypeInfo<DateTime> Create_DateTime(JsonSerializerOptions options)
    {
        if (
            !TryGetTypeInfoForRuntimeCustomConverter(
                options,
                out JsonTypeInfo<DateTime> jsonTypeInfo
            )
        )
        {
            jsonTypeInfo = JsonMetadataServices.CreateValueInfo<DateTime>(
                options,

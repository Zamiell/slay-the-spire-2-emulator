...
public class NBolasVfx : Node2D
{
    ...
    public static NBolasVfx? Create(Creature owner, Creature target)
    {
        ...
        NBolasVfx nBolasVfx = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NBolasVfx>(PackedScene.GenEditState.Disabled);
        ...
        float y =
            Mathf.Min(nBolasVfx._startPosition.Y, nBolasVfx._endPosition.Y)
            - Rng.Chaotic.NextFloat(400f, 500f);
        nBolasVfx._controlPosition = new Vector2(
            (nBolasVfx._startPosition.X + nBolasVfx._endPosition.X) * 0.5f,
            y
        );
        return nBolasVfx;
    }
    ...
    private async Task FlyBolasFly()
    {
        ...
        _tween
            .TweenMethod(Callable.From<float>(FollowCurve), 0f, 1f, 0.6000000238418579)
    ...
    {
        base.GlobalPosition = MathHelper.BezierCurve(
            _startPosition,
            _endPosition,
            _controlPosition,
            progressPercent

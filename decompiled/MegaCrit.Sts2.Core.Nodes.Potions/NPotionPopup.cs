...
public class NPotionPopup : Control
{
    ...
    {
        NPotionPopup nPotionPopup = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NPotionPopup>(PackedScene.GenEditState.Disabled);
    ...
    {
        base.GlobalPosition =
            _holder.GlobalPosition
            + Vector2.Down * _holder.Size.Y * 1.5f
            + Vector2.Right * _holder.Size * 0.5f
            + Vector2.Left * base.Size * 0.5f;
        _hoverTipBounds = GetNode<Control>("%HoverTipBounds");
        NHoverTipSet.CreateAndShow(
            _hoverTipBounds,
            _holder.Potion.Model.HoverTips,
        ...
        _useButton.Connect(
            ...
            Callable.From<NButton>(OnUseButtonPressed)
        );
        _discardButton = GetNode<NPotionPopupButton>("%DiscardButton");
        _discardButton.SetLocKey("POTION_POPUP.discard");
        _discardButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnDiscardButtonPressed)
        );
        ...
        else
        {
            ...
            bool flag = (
                ((uint)(targetType - 2) <= 1u || targetType == TargetType.TargetedNoCreature)
                    ? true
                    : false
            );
            locKey = (
                (!flag && !Potion.CanThrowAtAlly()) ? "POTION_POPUP.drink" : "POTION_POPUP.throw"
            );
        }
        ...
        _tween
            .TweenProperty(this, "modulate", Colors.White, 0.10000000149011612)
            .SetTrans(Tween.TransitionType.Sine);
        _tween
            .TweenProperty(
                _popupContainer,
                "position:y",
                _popupContainer.Position.Y + 25f,
                0.15000000596046448
            )
        ...
        if (Potion != null)
        {
            ...
            if (num < 0)
            {
                throw new InvalidOperationException(
                    $"Tried to discard potion {_holder.Potion.Model} but it's not in the player's belt!"
                );
            }
            _holder.DisableUntilPotionRemoved();
            DiscardPotionGameAction action = new DiscardPotionGameAction(
                owner,
                (uint)num,
    ...
    public void Remove()
    {
        if (!IsMarkedForRemoval)
        {
            ...
            _tween = CreateTween().SetParallel();
            _tween
                .TweenProperty(this, "modulate", Colors.Transparent, 0.10000000149011612)
                .SetTrans(Tween.TransitionType.Sine);
            _tween
                .TweenProperty(_popupContainer, "position:y", -25f, 0.20000000298023224)
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnDiscardButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NPotionPopup>(
                Create(VariantUtils.ConvertTo<NPotionHolder>(in args[0]))
            );
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName.IsMarkedForRemoval,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName.InACardSelectScreen,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

...
public class NOrb : NClickableControl
{
    ...
    {
        NOrb nOrb = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NOrb>(PackedScene.GenEditState.Disabled);
    ...
    public void UpdateVisuals(bool isEvoking)
    {
        ...
        if (_sprite == null)
        {
            ...
            _curTween
                .TweenProperty(_sprite, "scale", Vector2.One, 0.5)
                .From(Vector2.Zero)
        ...
        if (!(model is PlasmaOrb))
        {
            if (!(model is DarkOrb))
            {
                if (model is GlassOrb)
                {
                    ...
                    _sprite.Modulate = (
                        (Model.PassiveVal == 0m) ? Model.DarkenedColor : Colors.White
                    );
    ...
    protected override void OnFocus()
    {
        if (Model != null || _isLocal)
        {
            ...
            {
                IEnumerable<IHoverTip> enumerable2 = new List<IHoverTip>
                {
                    OrbModel.EmptySlotHoverTipHoverTip,
                };
                enumerable = enumerable2;
            }
            IEnumerable<IHoverTip> hoverTips = enumerable;
            NHoverTipSet
                .CreateAndShow(_bounds, hoverTips, HoverTip.GetHoverTipAlignment(_bounds))
                ?.SetFollowOwner();

...
public class NOrbManager : Control
{
    ...
    public static NOrbManager Create(NCreature creature, bool isLocal)
    {
        if (creature.Entity.Player == null)
        {
            throw new InvalidOperationException(
                "NOrbManager can only be applied to player creatures"
            );
        }
        NOrbManager nOrbManager = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NOrbManager>(PackedScene.GenEditState.Disabled);
    ...
    public void AddOrbAnim()
    {
        ...
        if (nOrb == null)
        {
            ...
            nOrb = (NOrb)
                _orbContainer.GetChildren().First((Node node) => ((NOrb)node).Model == null);
        }
    ...
    private void UpdateControllerNavigation()
    {
        for (int i = 0; i < _orbs.Count; i++)
        {
            ...
            _orbs[i].FocusNeighborLeft = (
                (i < _orbs.Count - 1) ? _orbs[i + 1].GetPath() : _orbs[0].GetPath()
            );
    ...
    private void TweenLayout()
    {
        ...
        if (capacity != 0)
        {
            ...
            for (int i = 0; i < capacity; i++)
            {
                ...
                _curTween
                    .TweenProperty(_orbs[i], "position", vector, 0.44999998807907104)
    ...
    public void ClearOrbs()
    {
        ...
        foreach (NOrb orb in _orbs)
        {
            _curTween
                .Parallel()
                .TweenProperty(orb, "position", Vector2.Zero, 1.0)
                .SetEase(Tween.EaseType.InOut)
                .SetTrans(Tween.TransitionType.Sine);
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateControllerNavigation,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NOrbManager>(
                Create(
                    VariantUtils.ConvertTo<NCreature>(in args[0]),
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.DefaultFocusOwner,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

...
public class TestCardSelector : ICardSelector
{
    ...
    private readonly Queue<TaskCompletionSource<IEnumerable<CardModel>>> _cardsToSelectTaskQueue =
        new Queue<TaskCompletionSource<IEnumerable<CardModel>>>();

    private readonly Queue<TaskCompletionSource<IEnumerable<int>>> _indicesToSelectTaskQueue =
        new Queue<TaskCompletionSource<IEnumerable<int>>>();
    ...
    {
        TaskCompletionSource<IEnumerable<CardModel>> taskCompletionSource =
    ...
    {
        TaskCompletionSource<IEnumerable<int>> taskCompletionSource =
            new TaskCompletionSource<IEnumerable<int>>();
    ...
    {
        PrepareToSelectCardReward(
            (IReadOnlyList<CardCreationResult> c, IReadOnlyList<CardRewardAlternative> _) =>
                new CardRewardSelection { card = c[index].Card }
        );
    }
    ...
    {
        PrepareToSelectCardReward(
            (IReadOnlyList<CardCreationResult> _, IReadOnlyList<CardRewardAlternative> a) =>
                new CardRewardSelection { alternative = a[index] }
        );
    }
    ...
    )
    {
        if (_shouldBlock)
        {
            ...
            throw new InvalidOperationException(
                "Test told us to block, but it did not finish within 5 seconds!"
            );
        }

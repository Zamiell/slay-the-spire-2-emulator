...
public class NDebugAudioManager : Node
{
    ...
    public int Play(
        string streamName,
        float volume = 1f,
        PitchVariance variance = PitchVariance.None
    )
    {
    ...
    private void StopInternal(int soundIndex)
    {
        ...
        playingSound.player.Disconnect(
            AudioStreamPlayer.SignalName.Finished,
            playingSound.callable
        );
    ...
    {
        AudioServer.Singleton.SetBusVolumeDb(
            AudioServer.Singleton.GetBusIndex(_master),
            Mathf.LinearToDb(Mathf.Pow(linearVolume, 2f))
        );
    }
    ...
    {
        AudioServer.Singleton.SetBusVolumeDb(
            AudioServer.Singleton.GetBusIndex(_sfx),
            Mathf.LinearToDb(Mathf.Pow(linearVolume, 2f))
        );
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetMasterAudioVolume,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
            ...
            new MethodInfo(
                ...
                new List<PropertyInfo>
                {
                    new PropertyInfo(
                        ...
                        "",
                        PropertyUsageFlags.Default,
                        new StringName("AudioStreamPlayer"),
                        exported: false
                    ),
                },
                null
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetRandomPitchScale,
                new PropertyInfo(
                    Variant.Type.Float,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.Play && args.Count == 3)
        {
            ret = VariantUtils.CreateFrom<int>(
                Play(
                    ...
                    VariantUtils.ConvertTo<PitchVariance>(in args[2])
                )
            );
            return true;
        }
        ...
        if (method == MethodName.GetRandomPitchScale && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<float>(
                GetRandomPitchScale(VariantUtils.ConvertTo<PitchVariance>(in args[0]))
            );
            return true;
        }

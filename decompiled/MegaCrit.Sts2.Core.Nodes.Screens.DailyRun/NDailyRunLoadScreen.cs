...
public class NDailyRunLoadScreen : NSubmenu, ILoadRunLobbyListener
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/daily_run/daily_run_load_screen"
    );

    private static readonly LocString _ascensionLoc = new LocString(
        "main_menu_ui",
        "DAILY_RUN_MENU.ASCENSION"
    );
    ...
    public static NDailyRunLoadScreen? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NDailyRunLoadScreen>(PackedScene.GenEditState.Disabled);
    }
    ...
    public override void _Ready()
    {
        ...
        _remotePlayerContainer = GetNode<NRemoteLoadLobbyPlayerContainer>(
            "%RemotePlayerLoadContainer"
        );
    ...
    public void InitializeAsHost(INetGameService gameService, SerializableRun run)
    {
        if (gameService.Type != NetGameType.Host)
        {
            throw new InvalidOperationException(
                $"Initialized daily run load screen with net service of type {gameService.Type} when hosting!"
            );
        }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                $"Initialized daily run load screen with net service of type {gameService.Type} when joining!"
            );
        }
    ...
    public override void OnSubmenuOpened()
    {
        ...
        _leaderboard.Initialize(
            _lobby.Run.DailyTime.Value,
            _lobby.Run.Players.Select((SerializablePlayer p) => p.NetId),
    ...
    private void InitializeDisplay()
    {
        ...
        SerializablePlayer serializablePlayer = _lobby.Run.Players.First(
            (SerializablePlayer p) => p.NetId == _lobby.NetService.NetId
        );
        CharacterModel byId = ModelDb.GetById<CharacterModel>(serializablePlayer.CharacterId);
        _characterContainer.Fill(
            byId,
            serializablePlayer.NetId,
            _lobby.Run.Ascension,
            _lobby.NetService
        );
    ...
    private void UpdateRichPresence()
    {
        ...
        if (lobby != null && lobby.NetService.Type.IsMultiplayer())
        {
            PlatformUtil.SetRichPresence(
                ...
                _lobby.ConnectedPlayerIds.Count
            );
        }
    }
    ...
    public async Task<bool> ShouldAllowRunToBegin()
    {
        ...
        return await nGenericPopup.WaitForConfirmation(
            locString,
    ...
    {
        Log.Info(
            "Loading a multiplayer run. Players: "
                + string.Join(",", _lobby.ConnectedPlayerIds)
                + "."
        );
        SerializablePlayer serializablePlayer = _lobby.Run.Players.First(
            (SerializablePlayer p) => p.NetId == _lobby.NetService.NetId
        ...
        SfxCmd.Play(
            ModelDb.GetById<CharacterModel>(serializablePlayer.CharacterId).CharacterTransitionSfx
        );
        await NGame.Instance.Transition.FadeOut(
            0.8f,
            ModelDb
                .GetById<CharacterModel>(serializablePlayer.CharacterId)
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.RemotePlayerDisconnected,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            _remotePlayerContainer = VariantUtils.ConvertTo<NRemoteLoadLobbyPlayerContainer>(
                in value
            );
            return true;
        }

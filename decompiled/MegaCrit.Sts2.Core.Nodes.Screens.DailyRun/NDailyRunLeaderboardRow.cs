...
public class NDailyRunLeaderboardRow : Control
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/daily_run/daily_run_leaderboard_row"
    );
    ...
    public static NDailyRunLeaderboardRow? Create(LeaderboardEntry entry, bool isYou)
    {
        ...
        NDailyRunLeaderboardRow nDailyRunLeaderboardRow = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NDailyRunLeaderboardRow>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        IEnumerable<string> values = _entry.userIds.Select(
            (ulong id) => PlatformUtil.GetPlayerNameRaw(LeaderboardManager.CurrentPlatform, id)
        );
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.FormatHoursAndMinutes,
                new PropertyInfo(
                    Variant.Type.String,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.FormatHoursAndMinutes && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<string>(
                FormatHoursAndMinutes(VariantUtils.ConvertTo<int>(in args[0]))
            );
            return true;
        }

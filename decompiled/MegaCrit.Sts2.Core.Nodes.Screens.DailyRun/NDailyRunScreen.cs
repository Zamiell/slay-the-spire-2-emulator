...
public class NDailyRunScreen : NSubmenu, IStartRunLobbyListener
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/daily_run/daily_run_screen"
    );

    private static readonly LocString _timeLeftLoc = new LocString(
        "main_menu_ui",
        "DAILY_RUN_MENU.TIME_LEFT"
    );

    public static readonly string dateFormat = LocManager
        .Instance.GetTable("main_menu_ui")
        .GetRawText("DAILY_RUN_MENU.DATE_FORMAT");

    private static readonly string _timeLeftFormat = LocManager
        .Instance.GetTable("main_menu_ui")
        .GetRawText("DAILY_RUN_MENU.TIME_FORMAT");
    ...
    private readonly List<NDailyRunScreenModifier> _modifierContainers =
        new List<NDailyRunScreenModifier>();
    ...
    public static NDailyRunScreen? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NDailyRunScreen>(PackedScene.GenEditState.Disabled);
    }
    ...
    public override void _Ready()
    {
        ...
        _characterContainer = GetNode<NDailyRunCharacterContainer>(
            "ChallengeContainer/CenterContainer/HBoxContainer/CharacterContainer"
        );
        ...
        _titleLabel.SetTextAutoSize(
            new LocString("main_menu_ui", "DAILY_RUN_MENU.DAILY_TITLE").GetFormattedText()
        );
        _disclaimer.SetTextAutoSize(
            new LocString("main_menu_ui", "DAILY_RUN_MENU.disclaimer").GetFormattedText()
        );
        _modifiersTitleLabel.SetTextAutoSize(
            new LocString("main_menu_ui", "DAILY_RUN_MENU.MODIFIERS").GetFormattedText()
        );
        _dateLabel.SetTextAutoSize(
            new LocString("main_menu_ui", "DAILY_RUN_MENU.FETCHING_TIME").GetFormattedText()
        );
        foreach (
            NDailyRunScreenModifier item in _modifiersContainer
                .GetChildren()
                .OfType<NDailyRunScreenModifier>()
        )
        {
    ...
    private void InitializeLeaderboard()
    {
        _leaderboard.Initialize(
            _lobby.DailyTime.Value.serverTime,
    ...
    private async Task<TimeServerResult> GetTimeServerTime()
    {
        ...
        if (TimeServer.RequestTimeTask?.IsCompleted ?? false)
        {
            ...
            if (!result.HasValue)
            {
                try
                {
                    ...
                    when (((ex is HttpRequestException || ex is TaskCanceledException) ? 1 : 0) != 0
                    )
                {
        ...
        else
        {
            try
            {
                ...
                when (((ex2 is HttpRequestException || ex2 is TaskCanceledException) ? 1 : 0) != 0)
            {
    ...
    {
        return _lobby.DailyTime.Value.serverTime
            + (DateTimeOffset.UtcNow - _lobby.DailyTime.Value.localReceivedTime);
    }
    ...
    private void SetupLobbyParams(StartRunLobby lobby)
    {
        ...
        string text = SeedHelper.CanonicalizeSeed(
            serverRelativeTime.ToString($"dd_MM_yyyy_{lobby.Players.Count}p")
        );
        ...
        if ((uint)(type - 1) <= 1u)
        {
            ...
            if (
                list.Any(
                    (ModifierModel m) => lobby.Modifiers.FirstOrDefault(m.IsEquivalent) == null
                )
            )
            {
    ...
    private void InitializeDisplay()
    {
        ...
        {
            throw new InvalidOperationException(
                "Tried to initialize daily run display before lobby was initialized!"
            );
        }
        DateTimeOffset serverRelativeTime = GetServerRelativeTime();
        _endOfDay =
            new DateTimeOffset(
                serverRelativeTime.Year,
                serverRelativeTime.Month,
                serverRelativeTime.Day,
                0,
                0,
                0,
                TimeSpan.Zero
        ...
        _characterContainer.Fill(
            character,
            _lobby.LocalPlayer.id,
            _lobby.Ascension,
            _lobby.NetService
        );
    ...
    private List<ModifierModel> RollModifiers(Rng rng)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            {
                throw new InvalidOperationException(
                    "There were not enough good modifiers to fill the daily!"
                );
            }
            ModifierModel modifierModel = canonicalModifier.ToMutable();
            if (modifierModel is CharacterCards characterCards)
            {
                IEnumerable<CharacterModel> second = _lobby.Players.Select(
                    (LobbyPlayer p) => p.character
                );
                characterCards.CharacterModel = rng.NextItem(
                    ModelDb.AllCharacters.Except(second)
                ).Id;
            }
            list.Add(modifierModel);
            list2.Remove(canonicalModifier);
            IReadOnlySet<ModifierModel> readOnlySet =
                ModelDb.MutuallyExclusiveModifiers.FirstOrDefault(
                    (IReadOnlySet<ModifierModel> s) => s.Contains(canonicalModifier)
                );
            if (readOnlySet == null)
            {
                continue;
    ...
    public void PlayerChanged(LobbyPlayer player, bool isRandomCharacterResolution)
    {
        ...
        {
            throw new InvalidOperationException(
                "Random character is not currently allowed in daily!"
            );
        }
    ...
    {
        Log.Info(
            $"Embarking on a DAILY {_lobby.LocalPlayer.character.Id.Entry} run with {_lobby.Players.Count} players. Ascension: {_lobby.Ascension} Seed: {seed}"
        );
    ...
    {
        Log.Info(
            $"Embarking on a DAILY multiplayer run. Players: {string.Join(",", _lobby.Players)}. Ascension: {_lobby.Ascension} Seed: {seed}"
        );
        SfxCmd.Play(_lobby.LocalPlayer.character.CharacterTransitionSfx);
        await NGame.Instance.Transition.FadeOut(
            0.8f,
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.InitializeLeaderboard,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.AfterLobbyInitialized,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._characterContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._modifiersTitleLabel,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._modifiersContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

...
public class NGeneralStatsGrid : Control
{
    ...
    private static readonly string _achievementsIconPath = ImageHelper.GetImagePath(
        "atlases/stats_screen_atlas.sprites/stats_achievements.tres"
    );

    private static readonly string _playtimeIconPath = ImageHelper.GetImagePath(
        "atlases/stats_screen_atlas.sprites/stats_clock.tres"
    );

    private static readonly string _cardsIconPath = ImageHelper.GetImagePath(
        "atlases/stats_screen_atlas.sprites/stats_cards.tres"
    );

    private static readonly string _winLossIconPath = ImageHelper.GetImagePath(
        "atlases/stats_screen_atlas.sprites/stats_swords.tres"
    );

    private static readonly string _monsterIconPath = ImageHelper.GetImagePath(
        "atlases/stats_screen_atlas.sprites/stats_monsters.tres"
    );

    private static readonly string _ancientsIconPath = ImageHelper.GetImagePath(
        "atlases/stats_screen_atlas.sprites/stats_ancients.tres"
    );

    private static readonly string _relicIconPath = ImageHelper.GetImagePath(
        "atlases/stats_screen_atlas.sprites/stats_chest.tres"
    );

    private static readonly string _potionIconPath = ImageHelper.GetImagePath(
        "atlases/stats_screen_atlas.sprites/stats_potions_seen.tres"
    );

    private static readonly string _eventsIconPath = ImageHelper.GetImagePath(
        "atlases/stats_screen_atlas.sprites/stats_questionmark.tres"
    );

    private static readonly string _streakIconPath = ImageHelper.GetImagePath(
        "atlases/stats_screen_atlas.sprites/stats_chain.tres"
    );
    ...
    private static HoverTip PlaytimeTip =>
        new HoverTip(
            new LocString("stats_screen", "TIP_PLAYTIME.header"),
            new LocString("stats_screen", "TIP_PLAYTIME.description")
        );

    private static HoverTip WinsLossesTip =>
        new HoverTip(
            new LocString("stats_screen", "TIP_WIN_LOSS.header"),
            new LocString("stats_screen", "TIP_WIN_LOSS.description")
        );
    ...
    public void LoadStats()
    {
        ...
        int numerator2 = progressSave.Epochs.Count(
            (SerializableEpoch epoch) => epoch.State >= EpochState.Revealed
        );
        if (
            EpochModel.AllEpochIds.All(
                (string id) => progressSave.Epochs.Any((SerializableEpoch epoch) => epoch.Id == id)
            )
        )
        {
        ...
        locString.Add(
            "Amount",
            StringHelper.RatioFormat(
                instance.GetTotalUnlockedCards(),
                SaveManager.GetUnlockableCardCount()
            )
        );
        ...
        locString.Add(
            "Amount",
            StringHelper.RatioFormat(progressSave.DiscoveredCards.Count, ModelDb.AllCards.Count())
        );
        ...
        if (aggregateAscensionProgress > 0)
        {
            ...
            locString.Add(
                "Amount",
                StringHelper.RatioFormat(
                    aggregateAscensionProgress,
                    SaveManager.GetAggregateAscensionCount()
                )
            );
        ...
        locString.Add(
            "Amount",
            StringHelper.RatioFormat(instance.Progress.EnemyStats.Count, ModelDb.Monsters.Count())
        );
        ...
        locString.Add(
            "Amount",
            StringHelper.RatioFormat(
                instance.GetTotalUnlockedRelics(),
                SaveManager.GetUnlockableRelicCount()
            )
        );
        ...
        locString.Add(
            "Amount",
            StringHelper.RatioFormat(progressSave.DiscoveredRelics.Count, ModelDb.AllRelics.Count())
        );
        ...
        locString.Add(
            "Amount",
            StringHelper.RatioFormat(
                instance.GetTotalUnlockedPotions(),
                SaveManager.GetUnlockablePotionCount()
            )
        );
        ...
        locString.Add(
            "Amount",
            StringHelper.RatioFormat(progressSave.DiscoveredEvents.Count, ModelDb.AllEvents.Count())
        );
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NStatEntry>(
                CreateSection(VariantUtils.ConvertTo<string>(in args[0]))
            );
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._achievementsEntry,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._characterStatContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._characterStatContainer,
            Variant.From(in _characterStatContainer)
        );

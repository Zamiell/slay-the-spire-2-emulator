...
public class NStatEntry : NClickableControl
{
    ...
    private static string ScenePath =>
        SceneHelper.GetScenePath("screens/stats_screen/stats_screen_section");

    public static NStatEntry Create(string imgUrl)
    {
        NStatEntry nStatEntry = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NStatEntry>(PackedScene.GenEditState.Disabled);
        ...
        if (_hoverTip.HasValue)
        {
            if (base.GlobalPosition.X < GetViewport().GetVisibleRect().Size.X * 0.4f)
            {
                NHoverTipSet
                    ...
                    ?.SetGlobalPosition(
                        new Vector2(base.GlobalPosition.X - 392f, base.GlobalPosition.Y)
                    );
            }
            else
            {
                NHoverTipSet
                    .CreateAndShow(this, _hoverTip)
                    ?.SetGlobalPosition(
                        new Vector2(base.GlobalPosition.X + 532f, base.GlobalPosition.Y)
                    );
            }
        }
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._controllerFocusReticle,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._controllerFocusReticle,
            Variant.From(in _controllerFocusReticle)
        );
    }

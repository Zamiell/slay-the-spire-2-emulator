...
public class NAchievementHolder : Control
{
    ...
    public static NAchievementHolder? Create(Achievement achievement)
    {
        ...
        NAchievementHolder nAchievementHolder = PreloadManager
            .Cache.GetScene(SceneHelper.GetScenePath("screens/stats_screen/achievement_holder"))
            .Instantiate<NAchievementHolder>(PackedScene.GenEditState.Disabled);
    ...
    public void RefreshUnlocked()
    {
        ...
        if (IsUnlocked)
        {
            _infoLabel.Text =
                "[b][gold]"
                + new LocString("achievements", text + ".title").GetRawText()
                + "[/gold][/b]\n"
        ...
        else
        {
            _infoLabel.Text =
                "[b][red]"
                + new LocString("achievements", "LOCKED.title").GetRawText()
                + "[/red][/b]\n"
                + new LocString("achievements", text + ".description").GetFormattedText();
        }
        SetLockVisuals();
    ...
    private void SetDateLabel()
    {
        ...
        {
            if (
                !SaveManager.Instance.Progress.UnlockedAchievements.TryGetValue(
                    _achievement,
                    out var value
                )
            )
            ...
            string variable = dateTime.ToString(
                new LocString("achievements", "UNLOCK_DATE.format").GetRawText(),
                dateTimeFormat
            );
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NAchievementHolder>(
                Create(VariantUtils.ConvertTo<Achievement>(in args[0]))
            );
            return true;
        }

...
public class NRelicInventory : FlowContainer
{
    ...
    public event RelicsChangedEventHandler RelicsChanged
    {
        add
        {
            backing_RelicsChanged = (RelicsChangedEventHandler)
                Delegate.Combine(backing_RelicsChanged, value);
        }
        remove
        {
            backing_RelicsChanged = (RelicsChangedEventHandler)
                Delegate.Remove(backing_RelicsChanged, value);
        }
    }
    ...
    private void Remove(RelicModel relic)
    {
        ...
        {
            NRelicInventoryHolder nRelicInventoryHolder = _relicNodes.First(
                (NRelicInventoryHolder n) => n.Relic.Model == relic
            );
    ...
    )
    {
        if (LocalContext.IsMine(relic))
        {
            ...
            TaskHelper.RunSafely(
                nRelicInventoryHolder.PlayNewlyAcquiredAnimation(startPosition, startScale)
            );
        }
    }
    ...
    public void AnimShow()
    {
        ...
        _curTween
            .TweenProperty(this, "global_position:y", _originalPos.Y, 0.25)
    ...
    public void AnimHide()
    {
        ...
        _curTween
            .TweenProperty(
                this,
                "global_position:y",
                _originalPos.Y - 68f * (float)GetLineCount() - 90f,
                0.25
            )
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
    ...
    private void UpdateNavigation()
    {
        for (int i = 0; i < RelicNodes.Count; i++)
        {
            ...
            nRelicInventoryHolder.FocusNeighborRight = (
                (i < RelicNodes.Count - 1) ? RelicNodes[i + 1].GetPath() : RelicNodes[0].GetPath()
            );
            Control firstPotionControl = NRun.Instance
                .GlobalUi
                .TopBar
                .PotionContainer
                .FirstPotionControl;
            nRelicInventoryHolder.FocusNeighborTop = (
                (firstPotionControl != null && GodotObject.IsInstanceValid(firstPotionControl))
                    ? firstPotionControl.GetPath()
                    : nRelicInventoryHolder.GetPath()
            );
            ...
            if (multiplayerPlayerContainer.GetChildCount() > 0)
            {
                ...
                nRelicInventoryHolder.FocusNeighborBottom = (
            ...
            {
                nRelicInventoryHolder.FocusNeighborBottom =
                    NRun.Instance.GlobalUi.TopBar.ActiveScreenProxy.GetPath();
            }
        }
    }
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<RelicsChangedEventHandler>(
                SignalName.RelicsChanged,
                out var value5
            )
        )
        {

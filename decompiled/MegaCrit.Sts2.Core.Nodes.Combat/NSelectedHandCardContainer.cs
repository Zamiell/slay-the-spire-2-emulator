...
public class NSelectedHandCardContainer : Control
{
    ...
    public List<NSelectedHandCardHolder> Holders =>
        GetChildren().OfType<NSelectedHandCardHolder>().ToList();
    ...
    public NSelectedHandCardHolder Add(NHandCardHolder originalHolder)
    {
        ...
        NSelectedHandCardHolder nSelectedHandCardHolder = NSelectedHandCardHolder.Create(
            originalHolder
        );
        nSelectedHandCardHolder.Connect(
            NCardHolder.SignalName.Pressed,
            Callable.From<NCardHolder>(DeselectHolder),
            4u
        );
    ...
    private void RefreshHolderPositions()
    {
        ...
        if (count != 0)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                num += x;
                Holders[i].FocusNeighborLeft = (
                    (i > 0) ? Holders[i - 1].GetPath() : Holders[Holders.Count - 1].GetPath()
                );
                Holders[i].FocusNeighborRight = (
                    (i < Holders.Count - 1) ? Holders[i + 1].GetPath() : Holders[0].GetPath()
                );
            }
        }
    }
    ...
    {
        NSelectedHandCardHolder holder = Holders.First(
            (NSelectedHandCardHolder child) => child.CardNode.Model == card
        );
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.RefreshHolderPositions,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.Add && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<NSelectedHandCardHolder>(
                Add(VariantUtils.ConvertTo<NHandCardHolder>(in args[0]))
            );
            return true;
        }

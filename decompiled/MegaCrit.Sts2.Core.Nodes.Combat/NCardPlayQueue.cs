...
public class NCardPlayQueue : Control
{
    ...
    public void OnLocalCardPlayed(PlayCardAction action, NCardHolder? holder, CardModel card)
    {
        ...
        {
            QueueItem item = new QueueItem { card = nCard, action = action };
    ...
    private void OnActionEnqueued(GameAction action)
    {
        ...
        NCreature creatureNode = NCombatRoom.Instance.GetCreatureNode(
            playCardAction.Player.Creature
        );
        ...
        Vector2 globalPosition =
            playerIntentHandler.CardIntent.GlobalPosition
            + playerIntentHandler.CardIntent.Size * 0.5f;
        nCard.GlobalPosition = globalPosition;
        nCard.Scale = Vector2.One * 0.25f;
        this.AddChildSafely(nCard);
        this.MoveChildSafely(nCard, 0);
        QueueItem item = new QueueItem { card = nCard, action = playCardAction };
        _playQueue.Add(item);
    ...
    public void ReAddCardAfterPlayerChoice(NCard card, GameAction action)
    {
        ...
        QueueItem item = new QueueItem { card = card, action = action };
        card.Reparent(this);
    ...
    private void UpdateCardVisuals(QueueItem item)
    {
        ...
        item.card.UpdateVisuals(
            item.card.Model.Pile?.Type ?? PileType.None,
    ...
    private void TweenCardForCancellation(QueueItem item)
    {
        ...
        item.currentTween.TweenProperty(item.card, "position:y", 30f, 0.5)
            .AsRelative()
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
        item.currentTween.TweenProperty(item.card, "modulate:a", 0f, 0.5)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
    ...
    private void TweenCardToQueuePosition(QueueItem item, int queueIndex)
    {
        item.currentTween?.Kill();
        item.currentTween = CreateTween().SetParallel();
        item.currentTween.TweenProperty(
                item.card,
                "position",
                GetPositionForQueueIndex(item.card, queueIndex),
                0.3499999940395355
            )
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
        item.currentTween.TweenProperty(
                item.card,
                "scale",
                GetScaleForQueueIndex(queueIndex),
                0.3499999940395355
            )
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
        item.currentTween.TweenProperty(item.card, "modulate:a", 1f, 0.3499999940395355)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
        item.currentTween.Play();
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.RemoveCardFromQueueForCancellation,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.RemoveCardFromQueue,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.TweenAllToQueuePosition,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetScaleForQueueIndex,
                new PropertyInfo(
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetPositionForQueueIndex,
                new PropertyInfo(
                    Variant.Type.Vector2,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.GetScaleForQueueIndex && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<Vector2>(
                GetScaleForQueueIndex(VariantUtils.ConvertTo<int>(in args[0]))
            );
            return true;
        }

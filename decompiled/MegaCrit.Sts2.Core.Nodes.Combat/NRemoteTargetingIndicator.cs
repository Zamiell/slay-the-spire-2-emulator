...
public class NRemoteTargetingIndicator : Node2D
{
    ...
    public override void _Process(double delta)
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            Vector2 position = MathHelper.BezierCurve(
                _fromPosition,
                _toPosition,
                zero,
                (float)i / 101f
        ...
        foreach (Creature item in _player.Creature.CombatState?.Enemies ?? Array.Empty<Creature>())
        {
            ...
            if (
                nCreature != null
                && nCreature.Hitbox.GetGlobalRect().HasPoint(base.GlobalPosition + _toPosition)
            )
            {
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.DoTargetingCreatureTween,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._isTargetingCreature,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

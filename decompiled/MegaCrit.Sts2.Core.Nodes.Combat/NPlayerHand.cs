...
public class NPlayerHand : Control
{
    ...
    public new class MethodName : Control.MethodName
    {
        ...
        public static readonly StringName RevalidateSelectionAfterStateChange =
        ...
        public static new readonly StringName _UnhandledInput = "_UnhandledInput";
    }
    ...
    private readonly Dictionary<NHandCardHolder, int> _holdersAwaitingQueue =
        new Dictionary<NHandCardHolder, int>();
    ...
    public IReadOnlyList<NHandCardHolder> ActiveHolders =>
        Holders.Where((NHandCardHolder child) => child.IsVisibleInTree()).ToList();

    private IReadOnlyList<NHandCardHolder> Holders =>
        CardHolderContainer.GetChildren().OfType<NHandCardHolder>().ToList();
            ...
            if (ActiveHolders.Count > 0)
            {
                if (_lastFocusedHolderIdx >= 0)
                {
                    return ActiveHolders[
                        Mathf.Clamp(_lastFocusedHolderIdx, 0, ActiveHolders.Count - 1)
                    ];
                }
                return ActiveHolders[ActiveHolders.Count / 2];
            }
            if (
                CurrentMode == Mode.SimpleSelect
    ...
    public event ModeChangedEventHandler ModeChanged
    {
        add
        {
            backing_ModeChanged = (ModeChangedEventHandler)
                Delegate.Combine(backing_ModeChanged, value);
        }
        remove
        {
            backing_ModeChanged = (ModeChangedEventHandler)
                Delegate.Remove(backing_ModeChanged, value);
        }
    }
    ...
    public override void _Ready()
    {
        ...
        _selectedHandCardContainer = GetNode<NSelectedHandCardContainer>(
            "%SelectedHandCardContainer"
        );
        _selectedHandCardContainer.Hand = this;
        _selectModeConfirmButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnSelectModeConfirmButtonPressed)
        );
        _selectModeConfirmButton.Disable();
        _selectedHandCardContainer.Connect(
            Node.SignalName.ChildExitingTree,
            Callable.From<Node>(OnCardDeselected)
        );
        _selectedHandCardContainer.Connect(
            Node.SignalName.ChildEnteredTree,
            Callable.From<Node>(OnCardSelected)
        );
        PeekButton = GetNode<NPeekButton>("%PeekButton");
        PeekButton.Disable();
        PeekButton.AddTargets(
            _selectModeBackstop,
    ...
    {
        return ((IEnumerable<NCardHolder>)Holders)
            .Concat((IEnumerable<NCardHolder>)_selectedHandCardContainer.Holders)
            .Concat(_holdersAwaitingQueue.Keys)
            .FirstOrDefault((NCardHolder h) => h.CardNode != null && h.CardNode.Model == card);
    }
    ...
    private void AddCardHolder(NHandCardHolder holder, int index)
    {
        ...
        holder.Connect(
            NHandCardHolder.SignalName.HolderMouseClicked,
            Callable.From<NCardHolder>(OnHolderPressed)
        );
        holder.Connect(
            NHandCardHolder.SignalName.HolderFocused,
            Callable.From<NHandCardHolder>(OnHolderFocused)
        );
        holder.Connect(
            NHandCardHolder.SignalName.HolderUnfocused,
    ...
    private void RefreshLayout()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            nHandCardHolder.FocusNeighborRight = (
                (i < ActiveHolders.Count - 1)
                    ? ActiveHolders[i + 1].GetPath()
                    : ActiveHolders[0].GetPath()
            );
    ...
    private void UpdateHandDisabledState(ICombatState state)
    {
        ...
        if (
            ...
            && me != null
            && !CombatManager.Instance.PlayersTakingExtraTurn.Contains(me)
        )
        {
            flag = true;
        }
        if (flag)
        {
            bool flag2 =
                me == null
                || !state
                    .Players.Except(new global::_003C_003Ez__ReadOnlySingleElementList<Player>(me))
                    .All(CombatManager.Instance.IsPlayerReadyToEndTurn);
    ...
    private void StartCardPlay(NHandCardHolder holder, bool startedViaShortcut)
    {
        ...
        _currentCardPlay = (
            NControllerManager.Instance.IsUsingController
                ...
                : (
                    (NCardPlay)
                        NMouseCardPlay.Create(
                            holder,
                            _selectCardShortcuts[_draggedHolderIndex],
                            startedViaShortcut
                        )
                )
        );
        this.AddChildSafely(_currentCardPlay);
        _currentCardPlay.Connect(
    ...
    public void DeselectCard(NCard card)
    {
        ...
        NHandCardHolder nHandCardHolder = Add(
            card,
            PileType.Hand.GetPile(card.Model.Owner).Cards.IndexOf<CardModel>(card.Model)
        );
    ...
    private void RevalidateSelectionAfterStateChange()
    {
        ...
        UpdateSelectModeCardVisibility();
        bool flag = Holders.Any(
            (NHandCardHolder h) => h.CardNode != null && filter(h.CardNode.Model)
        );
    ...
    public void AnimIn()
    {
        ...
        _animInTween
            .TweenProperty(this, "position", _showPosition, 0.800000011920929)
    ...
    public void AnimOut()
    {
        ...
        _animOutTween
            .TweenProperty(this, "position", _hidePosition, 0.800000011920929)
    ...
    private void AnimDisable()
    {
        if (!_isDisabled)
        {
            ...
            _animEnableTween
                .TweenProperty(this, "position", _disablePosition, 0.2)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Cubic);
            _animEnableTween
                .TweenProperty(this, "modulate", _disableModulate, 0.2)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Cubic);
            _isDisabled = true;
            DisableControllerNavigation();
    ...
    private void AnimEnable()
    {
        if (_isDisabled)
        {
            _animEnableTween = CreateTween().SetParallel();
            _animEnableTween
                .TweenProperty(this, "position", _showPosition, 0.2)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Cubic);
            _animEnableTween
    ...
    private void UpdateSelectedCardContainer(int count)
    {
        ...
        _selectedCardScaleTween
            .TweenProperty(_selectedHandCardContainer, "position:y", num2, 0.5)
            .SetTrans(Tween.TransitionType.Quad)
            .SetEase(Tween.EaseType.InOut);
        _selectedCardScaleTween
            .TweenProperty(_selectedHandCardContainer, "scale", Vector2.One * num, 0.5)
            .SetTrans(Tween.TransitionType.Quad)
            .SetEase(Tween.EaseType.InOut);
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.ForceRefreshCardIndices,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnPeekButtonToggled,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateSelectModeCardVisibility,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.CancelHandSelectionIfNecessary,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.AreCardActionsAllowed,
                new PropertyInfo(
                    Variant.Type.Bool,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.SelectCardInSimpleMode,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.SelectCardInUpgradeMode,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.RevalidateSelectionAfterStateChange,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnSelectModeConfirmButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.CheckIfSelectionComplete,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.RefreshSelectModeConfirmButton,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.FlashPlayableHolders,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateSelectedCardContainer,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.EnableControllerNavigation,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.DisableControllerNavigation,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.IsAwaitingPlay && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<bool>(
                IsAwaitingPlay(VariantUtils.ConvertTo<NHandCardHolder>(in args[0]))
            );
            return true;
        }
        if (method == MethodName.Add && args.Count == 2)
        {
            ret = VariantUtils.CreateFrom<NHandCardHolder>(
                Add(
                    VariantUtils.ConvertTo<NCard>(in args[0]),
                    VariantUtils.ConvertTo<int>(in args[1])
                )
            );
            return true;
        }
        ...
        {
            AddCardHolder(
                VariantUtils.ConvertTo<NHandCardHolder>(in args[0]),
                VariantUtils.ConvertTo<int>(in args[1])
            );
            ret = default(godot_variant);
            return true;
    ...
    )
    {
        ...
        {
            _selectedHandCardContainer = VariantUtils.ConvertTo<NSelectedHandCardContainer>(
                in value
            );
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Array,
                PropertyName._selectCardShortcuts,
                PropertyHint.None,
                "",
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.CardHolderContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._selectModeBackstop,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._upgradePreviewContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._selectedHandCardContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._selectModeConfirmButton,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName.IsInCardSelection,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Int,
                PropertyName._draggedHolderIndex,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Int,
                PropertyName._lastFocusedHolderIdx,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._selectedCardScaleTween,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName.CardHolderContainer,
            Variant.From<Control>(CardHolderContainer)
        );
        ...
        info.AddProperty(
            PropertyName._upgradePreviewContainer,
            Variant.From(in _upgradePreviewContainer)
        );
        info.AddProperty(
            PropertyName._selectedHandCardContainer,
            Variant.From(in _selectedHandCardContainer)
        );
        info.AddProperty(PropertyName._upgradePreview, Variant.From(in _upgradePreview));
        info.AddProperty(
            PropertyName._selectModeConfirmButton,
            Variant.From(in _selectModeConfirmButton)
        );
        ...
        info.AddProperty(
            PropertyName._lastFocusedHolderIdx,
            Variant.From(in _lastFocusedHolderIdx)
        );
        ...
        info.AddProperty(
            PropertyName._selectedCardScaleTween,
            Variant.From(in _selectedCardScaleTween)
        );
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<ModeChangedEventHandler>(

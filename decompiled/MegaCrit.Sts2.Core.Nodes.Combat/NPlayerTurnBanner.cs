...
public class NPlayerTurnBanner : Control
{
    ...
    public static NPlayerTurnBanner? Create(int roundNumber)
    {
        ...
        NPlayerTurnBanner nPlayerTurnBanner = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NPlayerTurnBanner>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        if (CombatManager.Instance.PlayersTakingExtraTurn.Count > 0)
        {
            _label.SetTextAutoSize(
                new LocString("gameplay_ui", "PLAYER_TURN_EXTRA").GetFormattedText()
            );
        }
        else
        {
    ...
    private async Task Display()
    {
        ...
        tween.SetParallel();
        tween
            .TweenProperty(this, "modulate:a", 1f, 1.0)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
        tween
            .TweenProperty(_label, "position", _label.Position + new Vector2(0f, -50f), 1.5)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
        tween
            .TweenProperty(_turnLabel, "position", _turnLabel.Position + new Vector2(0f, 50f), 1.5)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
        if (await tween.AwaitFinished(this))
        {
            ...
            tween
                .TweenProperty(this, "modulate:a", 0f, 0.30000001192092896)
                .SetEase(Tween.EaseType.Out)
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NPlayerTurnBanner>(
                Create(VariantUtils.ConvertTo<int>(in args[0]))
            );
            return true;
        }

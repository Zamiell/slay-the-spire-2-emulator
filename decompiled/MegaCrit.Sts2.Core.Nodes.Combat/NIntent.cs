...
public class NIntent : Control
{
    ...
    public override void _Ready()
    {
        ...
        _intentHolder.Modulate = (
            NCombatUi.IsDebugHidingIntent ? Colors.Transparent : Colors.White
        );
    }
    ...
    private void UpdateVisuals()
    {
            ...
            for (int i = 0; i < animationFrameCount; i++)
            {
                _animationFrames.Add(
                    PreloadManager.Cache.GetTexture2D(
                        IntentAnimData.GetAnimationFrame(animation, i)
                    )
                );
            }
        }
        ...
        string text = (
            (intent is AttackIntent attackIntent)
                ? (attackIntent.GetIntentLabel(_targets, _owner).GetFormattedText() ?? "")
                : (
                    (!(intent is StatusIntent))
                        ? string.Empty
                        : (_intent.GetIntentLabel(_targets, _owner).GetFormattedText() ?? "")
                )
        );
    ...
    {
        _intentHolder.Position =
            Vector2.Up
            * (
                Mathf.Sin((float)Time.GetTicksMsec() * 0.001f * (float)Math.PI + _timeOffset) * 10f
                + 8f
            );
    ...
    {
        NIntent nIntent = PreloadManager
            .Cache.GetScene("res://scenes/combat/intent.tscn")
            .Instantiate<NIntent>(PackedScene.GenEditState.Disabled);
    ...
    private void OnHovered()
    {
        ...
        {
            NCombatRoom
                .Instance?.GetCreatureNode(_owner)
                ?.ShowHoverTips(
                    new global::_003C_003Ez__ReadOnlySingleElementList<IHoverTip>(
                        _intent.GetHoverTip(_targets, _owner)
                    )
                );
        }
    }
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NIntent>(
                Create(VariantUtils.ConvertTo<float>(in args[0]))
            );
            return true;
        }

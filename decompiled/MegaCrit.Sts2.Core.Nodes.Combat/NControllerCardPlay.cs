...
public class NControllerCardPlay : NCardPlay
{
    ...
    public new class MethodName : NCardPlay.MethodName
    {
        ...
        public static new readonly StringName Cleanup = "Cleanup";
    }
    ...
    {
        add { backing_Canceled = (CanceledEventHandler)Delegate.Combine(backing_Canceled, value); }
        remove
        {
    ...
    public override void _Input(InputEvent inputEvent)
    {
        ...
        if (
            inputEventAction.IsActionPressed(MegaInput.cancel)
            || inputEventAction.IsActionPressed(MegaInput.topPanel)
        )
        {
    ...
    private async Task SingleCreatureTargeting(TargetType targetType)
    {
        ...
        switch (targetType)
        {
            case TargetType.AnyEnemy:
                list = (
                    from c in owner.CombatState.GetOpponentsOf(owner)
                    where c.IsHittable
                    select c
                ).ToList();
                break;
            case TargetType.AnyAlly:
                list = base
                    .Card.CombatState.PlayerCreatures.Where(
                        (Creature c) => c.IsHittable && c != owner
                    )
                    .ToList();
                break;
        }
        ...
        List<NCreature> list2 = list.Select((Creature c) => NCombatRoom.Instance.GetCreatureNode(c))
            .OfType<NCreature>()
            .ToList();
        if (list2.Count == 0)
        {
        ...
        try
        {
            ...
            NCombatRoom.Instance.RestrictControllerNavigation(
                list2.Select((NCreature n) => n.Hitbox)
            );
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.MultiCreatureTargeting,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NControllerCardPlay>(
                Create(VariantUtils.ConvertTo<NHandCardHolder>(in args[0]))
            );
            return true;
        }
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<ConfirmedEventHandler>(
                SignalName.Confirmed,
                out var value4
            )
        )
        ...
        if (
            info.TryGetSignalEventDelegate<CanceledEventHandler>(
                SignalName.Canceled,
                out var value5
            )
        )

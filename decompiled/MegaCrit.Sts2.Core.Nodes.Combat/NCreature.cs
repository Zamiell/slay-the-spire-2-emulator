...
public class NCreature : Control
{
    ...
    public new class PropertyName : Control.PropertyName
    {
        ...
        public static readonly StringName PowerAppliedVfxSpawnPosition =
            "PowerAppliedVfxSpawnPosition";
    ...
    private readonly Dictionary<string, (string, float)> _sfxLoops =
        new Dictionary<string, (string, float)>();

    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlySingleElementList<string>(_scenePath);
    ...
    public T? GetSpecialNode<T>(string name)
        where T : Node
    {
    ...
    public static NCreature? Create(Creature entity)
    {
        ...
        NCreature nCreature = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NCreature>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        else
        {
            bool flag3 =
                ...
                && Time.GetTicksMsec() - NCombatRoom.Instance.CreatedMsec < 1000;
            _stateDisplay.AnimateIn(
                flag3
                    ? HealthBarAnimMode.SpawnedAtCombatStart
    ...
    {
        base._EnterTree();
        NGame.Instance?.Connect(
            NGame.SignalName.PhobiaModeToggled,
            Callable.From(UpdatePhobiaMode)
        );
    ...
    private void UpdateBounds(Node boundsContainer)
    {
        ...
        IntentContainer.Position =
            boundsContainer.GetNode<Marker2D>("IntentPos").Position - IntentContainer.Size * 0.5f;
        IntentContainer.Position = new Vector2(
            IntentContainer.Position.X,
            IntentContainer.Position.Y * Visuals.Scale.X
        );
    ...
    public Task UpdateIntent(IEnumerable<Creature> targets)
    {
        ...
        List<Node> list = IntentContainer
            .GetChildren()
            .TakeLast(IntentContainer.GetChildCount() - i)
            .ToList();
    ...
    public async Task PerformIntent()
    {
        ...
        {
            IntentContainer.Modulate = new Color(
                IntentContainer.Modulate.R,
                IntentContainer.Modulate.G,
                IntentContainer.Modulate.B,
                0f
            );
            return;
        }
    ...
    private Task RevealIntents()
    {
        ...
        _intentFadeTween
            .TweenProperty(IntentContainer, "modulate:a", 1f, 1.0)
    ...
    private void OnFocus()
    {
        ...
        if (_isRemotePlayerOrPet)
        {
            ...
            NCombatRoom
                .Instance?.GetCreatureNode(me?.Creature)
                ?.SetRemotePlayerFocused(remotePlayerFocused: true);
        }
        else
        {
    ...
    private void OnUnfocus()
    {
        if (!_isInBestiary)
        {
            ...
            if (_isRemotePlayerOrPet)
            {
                ...
                NCombatRoom
                    .Instance?.GetCreatureNode(me?.Creature)
                    ?.SetRemotePlayerFocused(remotePlayerFocused: false);
            }
            else
            {
    ...
    public void ShowHoverTips(IEnumerable<IHoverTip> hoverTips)
    {
        if (!NCombatRoom.Instance.Ui.Hand.InCardPlay)
        {
            ...
            NHoverTipSet.CreateAndShow(
                Hitbox,
                hoverTips,
                HoverTip.GetHoverTipAlignment(this, 0.5f)
            );
        }
    }
    ...
    public void SetRemotePlayerFocused(bool remotePlayerFocused)
    {
        if (!LocalContext.IsMe(Entity))
        {
            throw new InvalidOperationException(
                "This should only be called on the local player's creature node!"
            );
        }
    ...
    private void OnPowerIncreased(PowerModel power, int amount, bool silent)
    {
        ...
        if (vfx != null)
        {
            if (flag)
            {
                NPowerAppliedBuffVfx buffVfx = NPowerAppliedBuffVfx.Create(
                    PowerAppliedVfxSpawnPosition
                );
                Callable
                    .From(
            ...
            {
                NPowerAppliedDebuffVfx debuffVfx = NPowerAppliedDebuffVfx.Create(
                    PowerAppliedVfxSpawnPosition
                );
                Callable
                    .From(
    ...
    public Tween AnimDisableUi()
    {
        ...
        tween
            .TweenProperty(_stateDisplay, "modulate:a", 0f, 0.5)
            .SetDelay(0.5)
    ...
    public Tween AnimEnableUi()
    {
        Tween tween = CreateTween();
        tween
            .TweenProperty(_stateDisplay, "modulate:a", 1f, 0.5)
            .SetDelay(0.5)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
        return tween;
    ...
    private async Task AnimDie(bool shouldRemove, CancellationToken cancelToken)
    {
        ...
        else
        {
            ...
            {
                await Cmd.Wait(
                    Entity.Monster.DeathAnimLengthOverride,
                    cancelToken,
    ...
    public void AnimHideIntent(double delay = 0.0)
    {
        _intentFadeTween?.Kill();
        _intentFadeTween = CreateTween().SetParallel();
        PropertyTweener propertyTweener = _intentFadeTween.TweenProperty(
            IntentContainer,
            "modulate:a",
            0f,
            0.5
    ...
    public void ScaleTo(float size, double duration)
    {
        if (!Entity.IsMonster || Entity.Monster.CanChangeScale)
        {
            ...
            _scaleTween
                .TweenMethod(
                    Callable.From<Vector2>(DoScaleTween),
                    Visuals.Scale,
                    Vector2.One * _tempScale * Visuals.DefaultScale,
                    duration
                )
    ...
    {
        float num = Mathf.Lerp(
            Osty.ScaleRange.X,
            Osty.ScaleRange.Y,
            Mathf.Clamp(ostyHealth / 150f, 0f, 1f)
        );
        NCreature nCreature = NCombatRoom.Instance?.GetCreatureNode(Entity.PetOwner.Creature);
        _scaleTween = CreateTween();
        _scaleTween
            .TweenProperty(Visuals, "scale", Vector2.One * num * Visuals.DefaultScale, duration)
            .SetEase(Tween.EaseType.InOut)
            .SetTrans(Tween.TransitionType.Sine);
        if (LocalContext.IsMe(Entity.PetOwner))
        {
            _scaleTween
                ...
                .TweenProperty(
                    this,
                    "position",
                    nCreature.Position + GetOstyOffsetFromPlayer(Entity),
                    duration
                );
        }
        _scaleTween.TweenCallback(
            Callable.From(
                delegate
                {
                    UpdateBounds(Visuals);
    ...
    public static Vector2 GetOstyOffsetFromPlayer(Creature osty)
    {
        ...
        return Vector2.Right * nCreature.Hitbox.Size.X * 0.5f
            + Osty.MinOffset.Lerp(Osty.MaxOffset, Mathf.Clamp((float)osty.MaxHp / 150f, 0f, 1f));
    }
    ...
    public void AnimShake()
    {
        if (
            IsInsideTree()
            && (_shakeTween == null || !_shakeTween.IsRunning())
            && !Visuals.IsPlayingHurtAnimation()
        )
        {
            Visuals.Position = Vector2.Zero;
            ...
            _shakeTween
                .TweenMethod(
                    Callable.From(
                        ...
                        {
                            Visuals.Position =
                                Vector2.Right * 10f * Mathf.Sin(t * 4f) * Mathf.Sin(t * 0.5f);
                        }
                    ),
                    0f,
                    (float)Math.PI * 2f,
    ...
    private void SetOrbManagerPosition()
    {
        ...
        {
            OrbManager.Scale = (
                (Visuals.Scale.X > 1f) ? Vector2.One : Visuals.Scale.Lerp(Vector2.One, 0.5f)
            );
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.ConnectSpineAnimatorSignals,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetRemotePlayerFocused,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetAnimationTrigger,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetCurrentAnimationLength,
                new PropertyInfo(
                    Variant.Type.Float,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetCurrentAnimationTimeRemaining,
                new PropertyInfo(
                    Variant.Type.Float,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ToggleIsInteractable,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetOrbManagerPosition,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ShowMultiselectReticle,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.HideMultiselectReticle,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ShowSingleSelectReticle,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.HideSingleSelectReticle,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<float>(
                StartDeathAnim(VariantUtils.ConvertTo<bool>(in args[0]))
            );
            return true;
        }
        ...
        {
            SetScaleAndHue(
                VariantUtils.ConvertTo<float>(in args[0]),
                VariantUtils.ConvertTo<float>(in args[1])
            );
            ret = default(godot_variant);
            return true;
        }
        if (method == MethodName.ScaleTo && args.Count == 2)
        {
            ScaleTo(
                VariantUtils.ConvertTo<float>(in args[0]),
                VariantUtils.ConvertTo<double>(in args[1])
            );
            ret = default(godot_variant);
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._isRemotePlayerOrPet,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._selectionReticle,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Vector2,
                PropertyName.PowerAppliedVfxSpawnPosition,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName.IsPlayingDeathAnimation,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName.HasSpineAnimation,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.PlayerIntentHandler,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName.PlayerIntentHandler,
            Variant.From<NMultiplayerPlayerIntentHandler>(PlayerIntentHandler)
        );

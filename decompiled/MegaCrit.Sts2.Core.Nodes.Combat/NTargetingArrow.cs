...
public class NTargetingArrow : Node2D
{
    ...
    private static readonly string _segmentHeadPath = ImageHelper.GetImagePath(
        "ui/combat/targeting_arrow_head.png"
    );

    private static readonly string _segmentBlockPath = ImageHelper.GetImagePath(
        "ui/combat/targeting_arrow_segment.png"
    );
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlyArray<string>(
            new string[2] { _segmentHeadPath, _segmentBlockPath }
        );
    ...
    public void SetHighlightingOn(bool isEnemy)
    {
        ...
        _arrowHeadTween
            .TweenProperty(_arrowHead, "scale", _arrowHeadHoverScale, 1.0)
    ...
    private void UpdateSegments(Vector2 initialPos, Vector2 finalPos, Vector2 controlPoint)
    {
        for (int i = 0; i < 19; i++)
        {
            ...
            _segments[i].Position = MathHelper.BezierCurve(
                initialPos,
                finalPos,
                controlPoint,
                (float)i / 20f
            ...
            if (i == 0)
            {
                _segments[i].Rotation = (
                    (_segments[i].GlobalPosition.Y > 540f) ? 0f : ((float)Math.PI)
                );
            }
            else
            {
                _segments[i].Rotation =
                    (_segments[i].Position - _segments[i - 1].Position).Angle()
                    + (float)Math.PI / 2f;
            }
        }
        _segments[0].Rotation =
            (_segments[0].Position - _segments[1].Position).Angle() - (float)Math.PI / 2f;
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateArrowPosition,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.UpdateSegments && args.Count == 3)
        {
            UpdateSegments(
                ...
                VariantUtils.ConvertTo<Vector2>(in args[1]),

...
public class NTargetManager : Node2D
{
    ...
    public event CreatureHoveredEventHandler CreatureHovered
    {
        add
        {
            backing_CreatureHovered = (CreatureHoveredEventHandler)
                Delegate.Combine(backing_CreatureHovered, value);
        }
        remove
        {
            backing_CreatureHovered = (CreatureHoveredEventHandler)
                Delegate.Remove(backing_CreatureHovered, value);
        }
    }
    ...
    public event CreatureUnhoveredEventHandler CreatureUnhovered
    {
        add
        {
            backing_CreatureUnhovered = (CreatureUnhoveredEventHandler)
                Delegate.Combine(backing_CreatureUnhovered, value);
        }
        remove
        {
            backing_CreatureUnhovered = (CreatureUnhoveredEventHandler)
                Delegate.Remove(backing_CreatureUnhovered, value);
        }
    }
    ...
    public event NodeHoveredEventHandler NodeHovered
    {
        add
        {
            backing_NodeHovered = (NodeHoveredEventHandler)
                Delegate.Combine(backing_NodeHovered, value);
        }
        remove
        {
            backing_NodeHovered = (NodeHoveredEventHandler)
                Delegate.Remove(backing_NodeHovered, value);
        }
    }
    ...
    public event NodeUnhoveredEventHandler NodeUnhovered
    {
        add
        {
            backing_NodeUnhovered = (NodeUnhoveredEventHandler)
                Delegate.Combine(backing_NodeUnhovered, value);
        }
        remove
        {
            backing_NodeUnhovered = (NodeUnhoveredEventHandler)
                Delegate.Remove(backing_NodeUnhovered, value);
        }
    }
    ...
    public event TargetingBeganEventHandler TargetingBegan
    {
        add
        {
            backing_TargetingBegan = (TargetingBeganEventHandler)
                Delegate.Combine(backing_TargetingBegan, value);
        }
        remove
        {
            backing_TargetingBegan = (TargetingBeganEventHandler)
                Delegate.Remove(backing_TargetingBegan, value);
        }
    }
    ...
    public event TargetingEndedEventHandler TargetingEnded
    {
        add
        {
            backing_TargetingEnded = (TargetingEndedEventHandler)
                Delegate.Combine(backing_TargetingEnded, value);
        }
        remove
        {
            backing_TargetingEnded = (TargetingEndedEventHandler)
                Delegate.Remove(backing_TargetingEnded, value);
        }
    }
    ...
    )
    {
        if (!validTargetsType.IsSingleTarget())
        {
            throw new InvalidOperationException(
                $"Tried to begin targeting with invalid ActionTarget {validTargetsType}!"
            );
        }
    ...
    public void OnNodeHovered(Node node)
    {
        ...
        if (_targetMode == TargetMode.Controller)
        {
            ...
            {
                _targetingArrow.UpdateDrawingTo(
                    nMultiplayerPlayerState.GlobalPosition
                        + Vector2.Right * nMultiplayerPlayerState.Hitbox.Size.X
                        + Vector2.Down * nMultiplayerPlayerState.Hitbox.Size.Y / 2f
                );
            }
        }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.AllowedToTargetNode,
                new PropertyInfo(
                    Variant.Type.Bool,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.AllowedToTargetNode && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<bool>(
                AllowedToTargetNode(VariantUtils.ConvertTo<Node>(in args[0]))
            );
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Int,
                PropertyName._validTargetsType,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Int,
                PropertyName.LastTargetingFinishedFrame,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName.LastTargetingFinishedFrame,
            Variant.From<long>(LastTargetingFinishedFrame)
        );
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<CreatureHoveredEventHandler>(
                SignalName.CreatureHovered,
                out var value6
            )
        )
        {
        ...
        if (
            info.TryGetSignalEventDelegate<CreatureUnhoveredEventHandler>(
                SignalName.CreatureUnhovered,
                out var value7
            )
        )
        {
        ...
        if (
            info.TryGetSignalEventDelegate<NodeHoveredEventHandler>(
                SignalName.NodeHovered,
                out var value8
            )
        )
        {
        ...
        if (
            info.TryGetSignalEventDelegate<NodeUnhoveredEventHandler>(
                SignalName.NodeUnhovered,
                out var value9
            )
        )
        {
        ...
        if (
            info.TryGetSignalEventDelegate<TargetingBeganEventHandler>(
                SignalName.TargetingBegan,
                out var value10
            )
        )
        {
        ...
        if (
            info.TryGetSignalEventDelegate<TargetingEndedEventHandler>(
                SignalName.TargetingEnded,
                out var value11
            )
        )
        {

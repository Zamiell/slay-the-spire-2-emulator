...
public class NPeekButton : NButton
{
    ...
    {
        add { backing_Toggled = (ToggledEventHandler)Delegate.Combine(backing_Toggled, value); }
        remove { backing_Toggled = (ToggledEventHandler)Delegate.Remove(backing_Toggled, value); }
    }
    ...
    public override void _Ready()
    {
        ...
        if (NCombatRoom.Instance != null)
        {
            ...
            {
                NCombatRoom.Instance.Connect(
                    Node.SignalName.Ready,
                    Callable.From(OnCombatRoomReady)
                );
            }
        }
        ...
        for (Node parent = GetParent(); parent != null; parent = parent.GetParent())
        {
            if (parent is IOverlayScreen overlayScreenParent)
            {
                ...
                NOverlayStack.Instance?.Connect(
                    NOverlayStack.SignalName.Changed,
                    Callable.From(OnOverlayStackChanged)
                );
    ...
    private void OnOverlayStackChanged()
    {
        if (
            IsPeeking
            && _overlayScreenParent != null
            && NCapstoneContainer.Instance?.CurrentCapstoneScreen == null
            && _overlayScreenParent == NOverlayStack.Instance?.Peek()
        )
        {
    ...
    public void Wiggle()
    {
        ...
        _wiggleTween
            ...
            .TweenMethod(
                Callable.From(
                    ...
                    {
                        _visuals.Scale =
                            Vector2.One + Vector2.One * 0.15f * Mathf.Sin(t) * Mathf.Sin(t * 0.5f);
                    }
                ),
                0f,
                (float)Math.PI,
        ...
        _wiggleTween
            .Chain()
            .TweenProperty(_flash, "modulate:a", 0f, 0.3)
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnOverlayStackChanged,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.CurrentCardMarker,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

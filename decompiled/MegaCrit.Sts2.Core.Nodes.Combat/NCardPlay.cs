...
public abstract class NCardPlay : Node
{
    ...
    {
        add { backing_Finished = (FinishedEventHandler)Delegate.Combine(backing_Finished, value); }
        remove
        {
    ...
    protected void TryPlayCard(Creature? target)
    {
        ...
        bool flag = (
            (targetType == TargetType.AnyEnemy || targetType == TargetType.AnyAlly) ? true : false
        );
    ...
    protected void CenterCard()
    {
        ...
        Holder.SetTargetPosition(
            new Vector2(size.X / 2f, size.Y - Holder.Hitbox.Size.Y * 0.75f / 2f)
        );
    ...
    {
        if (
            !SaveManager.Instance.SeenFtue("cannot_play_card_ftue")
    ...
    protected void ShowMultiCreatureTargetingVisuals()
    {
        ...
        if ((uint)(targetType - 3) <= 1u)
        {
            ...
            CardNode.UpdateVisuals(
                (CardNode.Model?.Pile?.Type).GetValueOrDefault(),
        ...
        {
            IEnumerable<Creature> enumerable = Card.CombatState.PlayerCreatures.Where(
                (Creature c) => c.IsAlive
            );
            {
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.HideTargetingVisuals,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ShowMultiCreatureTargetingVisuals,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.AutoDisableCannotPlayCardFtueCheck,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._isTryingToPlayCard,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<FinishedEventHandler>(
                SignalName.Finished,
                out var value4
            )
        )

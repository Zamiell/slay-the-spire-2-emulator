...
public class EnergyIconsFormatter : IFormatter
{
    ...
    public bool TryEvaluateFormat(IFormattingInfo formattingInfo)
    {
        ...
        {
            Log.Warn(
                "No energy prefix found for EnergyIconsFormatter! Using colorless as a fallback."
            );
            if (text == null)
            {
                text = "colorless";
        ...
        string text4 = (
            (result > 0 && result < 4)
                ? string.Concat(Enumerable.Repeat(text3, result))
                : (
                    (!(formattingInfo.CurrentValue is DynamicVar dynamicVar))
                        ? $"{result}{text3}"
                        : (dynamicVar.ToHighlightedString(inverse: false) + text3)
                )
        );

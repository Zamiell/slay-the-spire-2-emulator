...
public class ShowIfUpgradedFormatter : IFormatter
{
    ...
    public bool TryEvaluateFormat(IFormattingInfo formattingInfo)
    {
        ...
        {
            throw new LocException(
                $"Format expression cannot contain more than 2 options. num_of_options={list.Count} format={formattingInfo.Format}."
            );
        }
        ...
        switch (ifUpgradedVar.upgradeDisplay)
        {
            ...
            default:
                throw new ArgumentOutOfRangeException(
                    "upgradeDisplay",
                    $"Unexpected value: {ifUpgradedVar.upgradeDisplay}"
                );
        }
        return true;
    }

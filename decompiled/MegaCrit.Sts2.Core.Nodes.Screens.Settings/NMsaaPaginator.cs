...
public class NMsaaPaginator : NPaginator, IResettableSettingNode
{
    public new class MethodName : NPaginator.MethodName
    {
        ...
        public static new readonly StringName OnIndexChanged = "OnIndexChanged";
    ...
    protected override void OnIndexChanged(int index)
    {
        ...
        RenderingServer.ViewportSetMsaa2D(
            ...
            GetMsaa(SaveManager.Instance.SettingsSave.Msaa)
        );
    }
    ...
    )
    {
        ...
        if (method == MethodName.GetMsaaLabel && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<string>(
                GetMsaaLabel(VariantUtils.ConvertTo<int>(in args[0]))
            );
            return true;
        }
        if (method == MethodName.GetMsaa && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<RenderingServer.ViewportMsaa>(
                GetMsaa(VariantUtils.ConvertTo<int>(in args[0]))
            );
            return true;
        }

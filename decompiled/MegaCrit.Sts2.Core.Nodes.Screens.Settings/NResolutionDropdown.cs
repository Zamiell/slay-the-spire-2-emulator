...
public class NResolutionDropdown : NSettingsDropdown
{
    ...
    {
        ClearDropdownItems();
        Vector2I boundaryResolution = DisplayServer.ScreenGetSize(
            SaveManager.Instance.SettingsSave.TargetDisplay
        );
        foreach (Vector2I resolutionWhite in GetResolutionWhiteList())
        {
            if (DoesResolutionFit(resolutionWhite, boundaryResolution))
            {
                NResolutionDropdownItem nResolutionDropdownItem =
                    _dropdownItemScene.Instantiate<NResolutionDropdownItem>(
                        PackedScene.GenEditState.Disabled
                    );
                _dropdownItems.AddChildSafely(nResolutionDropdownItem);
                nResolutionDropdownItem.Connect(
    ...
    private void OnDropdownItemSelected(NDropdownItem nDropdownItem)
    {
        ...
        {
            CloseDropdown();
            SaveManager.Instance.SettingsSave.WindowPosition =
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.RefreshCurrentlySelectedResolution,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.PopulateDropdownItems,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.DoesResolutionFit && args.Count == 2)
        {
            ret = VariantUtils.CreateFrom<bool>(
                DoesResolutionFit(
                    VariantUtils.ConvertTo<Vector2I>(in args[0]),
                    VariantUtils.ConvertTo<Vector2I>(in args[1])
                )
            );
            return true;
        }
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._dropdownItemScene,

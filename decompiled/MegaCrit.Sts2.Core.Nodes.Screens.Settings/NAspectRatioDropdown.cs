...
public class NAspectRatioDropdown : NSettingsDropdown, IResettableSettingNode
{
    ...
    private static string AspectRatioDropdownItemScenePath =>
        SceneHelper.GetScenePath("ui/aspect_ratio_dropdown_item");

    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlySingleElementList<string>(
            AspectRatioDropdownItemScenePath
        );
    ...
    public void SetFromSettings()
    {
        ...
        _currentOptionLabel.SetTextAutoSize(
            GetAspectRatioSettingString(_currentAspectRatioSetting)
        );
    }

    private void AddDropdownItem(AspectRatioSetting aspectRatioSetting)
    {
        NAspectRatioDropdownItem nAspectRatioDropdownItem = ResourceLoader
            .Load<PackedScene>(
                AspectRatioDropdownItemScenePath,
                null,
                ResourceLoader.CacheMode.Reuse
            )
            .Instantiate<NAspectRatioDropdownItem>(PackedScene.GenEditState.Disabled);
        _dropdownItems.AddChildSafely(nAspectRatioDropdownItem);
        nAspectRatioDropdownItem.Connect(
            NDropdownItem.SignalName.Selected,
            Callable.From<NDropdownItem>(OnDropdownItemSelected)
        );
    ...
    private void OnDropdownItemSelected(NDropdownItem nDropdownItem)
    {
        ...
        {
            CloseDropdown();
            SaveManager.Instance.SettingsSave.AspectRatioSetting =
                nAspectRatioDropdownItem.aspectRatioSetting;
            SetFromSettings();
    ...
    private static string GetAspectRatioSettingString(AspectRatioSetting aspectRatioSettingString)
    {
        ...
        {
            AspectRatioSetting.Auto => new LocString(
                "settings_ui",
                "ASPECT_RATIO_AUTO"
            ).GetFormattedText(),
            AspectRatioSetting.FourByThree => new LocString(
                "settings_ui",
                "ASPECT_RATIO_FOUR_BY_THREE"
            ).GetFormattedText(),
            AspectRatioSetting.SixteenByTen => new LocString(
                "settings_ui",
                "ASPECT_RATIO_SIXTEEN_BY_TEN"
            ).GetFormattedText(),
            AspectRatioSetting.SixteenByNine => new LocString(
                "settings_ui",
                "ASPECT_RATIO_SIXTEEN_BY_NINE"
            ).GetFormattedText(),
            AspectRatioSetting.TwentyOneByNine => new LocString(
                "settings_ui",
                "ASPECT_RATIO_TWENTY_ONE_BY_NINE"
            ).GetFormattedText(),
            _ => throw new ArgumentOutOfRangeException(
                $"Invalid Aspect Ratio: {aspectRatioSettingString}"
            ),
        };
    }
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnDropdownItemSelected,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetAspectRatioSettingString,
                new PropertyInfo(
                    Variant.Type.String,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.GetAspectRatioSettingString && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<string>(
                GetAspectRatioSettingString(VariantUtils.ConvertTo<AspectRatioSetting>(in args[0]))
            );
            return true;
        }
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Int,
                PropertyName._currentAspectRatioSetting,
                PropertyHint.None,
                "",
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._currentAspectRatioSetting,
            Variant.From(in _currentAspectRatioSetting)
        );
    }

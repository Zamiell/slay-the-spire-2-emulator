...
public class NInputSettingsEntry : NButton
{
    ...
    private static readonly Dictionary<StringName, string> _commandToLocTitle = new Dictionary<
        StringName,
        string
    >
    {
        { MegaInput.accept, "endTurn" },
        { MegaInput.select, "confirmCard" },
        { MegaInput.viewDiscardPile, "viewDiscard" },
        { MegaInput.viewDrawPile, "viewDraw" },
        { MegaInput.viewDeckAndTabLeft, "viewDeck" },
        { MegaInput.viewExhaustPileAndTabRight, "viewExhaust" },
        { MegaInput.viewMap, "viewMap" },
        { MegaInput.cancel, "cancel" },
        { MegaInput.peek, "peek" },
        { MegaInput.up, "up" },
        { MegaInput.topPanel, "topPanel" },
        { MegaInput.down, "down" },
        { MegaInput.left, "left" },
        { MegaInput.right, "right" },
        { MegaInput.selectCard1, "selectCard1" },
        { MegaInput.selectCard2, "selectCard2" },
        { MegaInput.selectCard3, "selectCard3" },
        { MegaInput.selectCard4, "selectCard4" },
        { MegaInput.selectCard5, "selectCard5" },
        { MegaInput.selectCard6, "selectCard6" },
        { MegaInput.selectCard7, "selectCard7" },
        { MegaInput.selectCard8, "selectCard8" },
        { MegaInput.selectCard9, "selectCard9" },
        { MegaInput.selectCard10, "selectCard10" },
        { MegaInput.releaseCard, "releaseCard" },
    };
    ...
    {
        NInputSettingsEntry nInputSettingsEntry = ResourceLoader
            .Load<PackedScene>(
                "res://scenes/screens/settings_screen/input_settings_entry.tscn",
                null,
                ResourceLoader.CacheMode.Reuse
            )
            .Instantiate<NInputSettingsEntry>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        _inputLabel.Text = new LocString(
            "settings_ui",
            "INPUT_SETTINGS.INPUT_TITLE." + text
        ).GetFormattedText();
        NInputManager.Instance.Connect(
            NInputManager.SignalName.InputRebound,
            Callable.From(UpdateInput)
        );
    ...
    private void UpdateInput()
    {
        if (IsVisibleInTree())
        {
            ...
            _controllerBindingIcon.Modulate = (
                NControllerManager.Instance.ShouldAllowControllerRebinding
                    ? Colors.White
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NInputSettingsEntry>(
                Create(VariantUtils.ConvertTo<string>(in args[0]))
            );
            return true;
        }
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._controllerBindingIcon,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._controllerBindingIcon,
            Variant.From(in _controllerBindingIcon)
        );
    }

...
public class NLanguageDropdown : NSettingsDropdown
{
    ...
    private static readonly Dictionary<string, string> _languageCodeToName = new Dictionary<
        string,
        string
    >
    {
    ...
    private void PopulateOptions()
    {
        ...
        {
            NLanguageDropdownItem nLanguageDropdownItem =
                _dropdownItemScene.Instantiate<NLanguageDropdownItem>(
                    PackedScene.GenEditState.Disabled
                );
            _dropdownItems.AddChildSafely(nLanguageDropdownItem);
            nLanguageDropdownItem.Connect(
    ...
    private void OnDropdownItemSelected(NDropdownItem nDropdownItem)
    {
        ...
        if (!(nLanguageDropdownItem.LanguageCode == CurrentLanguage))
        {
            ...
            _currentOptionLabel.SetTextAutoSize(
                GetLanguageNameForCode(nLanguageDropdownItem.LanguageCode)
            );
    ...
    public static string GetLanguageNameForCode(string languageCode)
    {
        if (!_languageCodeToName.TryGetValue(languageCode.ToUpperInvariant(), out string value))
        {
            throw new InvalidOperationException(
                "Tried to get language name for code " + languageCode + " but it doesn't exist!"
            );
        }
        return value;
    }
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetLanguageNameForCode,
                new PropertyInfo(
                    Variant.Type.String,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.GetLanguageNameForCode && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<string>(
                GetLanguageNameForCode(VariantUtils.ConvertTo<string>(in args[0]))
            );
            return true;
        }

...
public class NSettingsPanel : Control
{
    ...
    private bool IsSettingsOption(Control c)
    {
        ...
        if (
            ...
            || c is NTickbox
            || c is NButton
            || c is NDropdownPositioner
            || c is NSettingsSlider
        )
        {
            return true;
        }
    ...
    )
    {
        ...
        if (method == MethodName.IsSettingsOption && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<bool>(
                IsSettingsOption(VariantUtils.ConvertTo<Control>(in args[0]))
            );
            return true;
        }

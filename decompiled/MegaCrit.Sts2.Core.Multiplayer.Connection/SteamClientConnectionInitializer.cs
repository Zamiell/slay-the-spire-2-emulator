...
public class SteamClientConnectionInitializer : IClientConnectionInitializer
{
    ...
    {
        return new SteamClientConnectionInitializer { _playerSteamId = playerSteamId };
    }

    public static SteamClientConnectionInitializer FromLobby(ulong lobbySteamId)
    {
        return new SteamClientConnectionInitializer { _lobbySteamId = lobbySteamId };
    }
    ...
    )
    {
        if (gameService.IsConnected)
        {
            throw new InvalidOperationException(
                "NetClientGameService must not be connected when passed to SteamClientConnectionInitializer!"
            );
        }

    ...
    public RoomType Roll(IEnumerable<RoomType> blacklist, IRunState runState)
    {
        if (runState.UnlockState.NumberOfRuns == 0)
        {
            int num = runState
                ...
                .Count((MapPointHistoryEntry p) => p.MapPointType == MapPointType.Unknown);
            if (num < 2)
            {
        ...
        IReadOnlySet<RoomType> roomTypes = _nonEventOdds
            .Keys.Append(RoomType.Event)
            .Except(blacklist)
            .ToHashSet();
        roomTypes = Hook.ModifyUnknownMapPointRoomTypes(runState, roomTypes);
        RoomType roomType = (
            roomTypes.Contains(RoomType.Event) ? RoomType.Event : roomTypes.Order().First()
        );

...
public class CardRarityOdds : AbstractOdds
{
    ...
    public float RarityGrowth =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.Scarcity, 0.005f, 0.01f);

    public static float RegularRareOdds =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.Scarcity, 0.0149f, 0.03f);

    public static float EliteCommonOdds =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.Scarcity, 0.549f, 0.5f);

    public static float EliteRareOdds =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.Scarcity, 0.05f, 0.1f);

    public static float ShopCommonOdds =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.Scarcity, 0.585f, 0.54f);

    public static float ShopRareOdds =>
        AscensionHelper.GetValueIfAscension(AscensionLevel.Scarcity, 0.045f, 0.09f);
    ...
    public CardRarity Roll(CardRarityOddsType type)
    {
        CardRarity cardRarity = RollWithoutChangingFutureOdds(
            type,
            (type == CardRarityOddsType.BossEncounter) ? 0f : base.CurrentValue
        );

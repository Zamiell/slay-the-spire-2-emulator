...
public class MapPointHistoryEntry : IPacketSerializable
{
    ...
    {
        PlayerMapPointHistoryEntry playerMapPointHistoryEntry = PlayerStats.FirstOrDefault(
            (PlayerMapPointHistoryEntry e) => e.PlayerId == playerId
        );
        if (playerMapPointHistoryEntry == null)
        {
            throw new InvalidOperationException(
                $"Player with ID {playerId} not found in player stats for this run history! We have {string.Join(",", PlayerStats.Select((PlayerMapPointHistoryEntry p) => p.PlayerId))}"
            );
        }
    ...
    public MapPointHistoryEntry Anonymized()
    {
        return new MapPointHistoryEntry
        {
            ...
            PlayerStats = PlayerStats
                .Select((PlayerMapPointHistoryEntry p) => p.Anonymized())
                .ToList(),
        };
    }
}

...
public class DeterministicModelComparer : IComparer<AbstractModel>
{
    ...
    private static int CompareModelsWithSameOwnerAndId(AbstractModel model1, AbstractModel model2)
    {
        ...
        if (
            model1 is EnchantmentModel enchantmentModel
            && model2 is EnchantmentModel enchantmentModel2
        )
        {
            return CompareCardModelsWithSameOwnerAndId(
                enchantmentModel.Card,
                enchantmentModel2.Card
            );
        }
        if (model1 is RelicModel relicModel && model2 is RelicModel relicModel2)
        {
            return relicModel
                .Owner.Relics.IndexOf(relicModel)
                .CompareTo(relicModel2.Owner.Relics.IndexOf(relicModel2));
        }
        if (model1 is PotionModel potionModel && model2 is PotionModel potionModel2)
        {
            return potionModel
                .Owner.PotionSlots.IndexOf<PotionModel>(potionModel)
                .CompareTo(potionModel2.Owner.PotionSlots.IndexOf<PotionModel>(potionModel2));
        }
        if (model1 is PowerModel powerModel && model2 is PowerModel powerModel2)
        {
            return powerModel
                .Owner.Powers.IndexOf(powerModel)
                .CompareTo(powerModel2.Owner.Powers.IndexOf(powerModel2));
        }
        throw new InvalidOperationException(
            $"Tried to compare equivalent models {model1} and {model2} but we can't map them to any know model types!"
        );
    }

    private static int CompareCardModelsWithSameOwnerAndId(
    ...
    )
    {
        ...
        return cardModel1
            .Pile.Cards.IndexOf(cardModel1)
            .CompareTo(cardModel2.Pile.Cards.IndexOf(cardModel2));
    }
}

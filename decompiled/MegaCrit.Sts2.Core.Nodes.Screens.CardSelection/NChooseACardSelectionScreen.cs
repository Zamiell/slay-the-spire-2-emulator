...
public class NChooseACardSelectionScreen : Control, IOverlayScreen, IScreenContext, ICardSelector
{
    ...
    private readonly TaskCompletionSource<IEnumerable<CardModel>> _completionSource =
        new TaskCompletionSource<IEnumerable<CardModel>>();
    ...
    private static string ScenePath =>
        SceneHelper.GetScenePath("screens/card_selection/choose_a_card_selection_screen");
    ...
    )
    {
        ...
        NChooseACardSelectionScreen nChooseACardSelectionScreen = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NChooseACardSelectionScreen>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        for (int i = 0; i < _cards.Count; i++)
        {
            ...
            nGridCardHolder.Connect(
                ...
                Callable.From<NCardHolder>(SelectHolder)
            );
            nGridCardHolder.Connect(
                NCardHolder.SignalName.AltPressed,
                Callable.From<NCardHolder>(OpenPreviewScreen)
            );
            nCard.UpdateVisuals(PileType.None, CardPreviewMode.Normal);
            nGridCardHolder.Scale = nGridCardHolder.SmallScale;
            _cardTween
                .TweenProperty(nGridCardHolder, "position", vector + Vector2.Right * 340f * i, 0.5)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Expo);
            _cardTween
                .TweenProperty(nGridCardHolder, "modulate", Colors.White, 1.0)
                .SetEase(Tween.EaseType.Out)
        ...
        if (_canSkip)
        {
            _skipButton.Connect(
                ...
                Callable.From<NButton>(OnSkipButtonReleased)
            );
    ...
    {
        _inspectPrompt.Modulate = (
            NControllerManager.Instance.IsUsingController ? Colors.White : Colors.Transparent
        );
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnSkipButtonReleased,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,

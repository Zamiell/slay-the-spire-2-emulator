...
public class NCardRewardSelectionScreen : Control, IOverlayScreen, IScreenContext
{
    ...
    private static string ScenePath =>
        SceneHelper.GetScenePath("screens/card_selection/card_reward_selection_screen");

    public static IEnumerable<string> AssetPaths =>
        new string[1] { ScenePath }.Concat(NCardRewardAlternativeButton.AssetPaths);
    ...
    )
    {
        ...
        NCardRewardSelectionScreen nCardRewardSelectionScreen = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NCardRewardSelectionScreen>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        _banner.label.SetTextAutoSize(
            new LocString("gameplay_ui", "CHOOSE_CARD_HEADER").GetRawText()
        );
        _banner.AnimateIn();
        ...
        NControllerManager.Instance.Connect(
            ...
            Callable.From(UpdateControllerIcons)
        );
        NControllerManager.Instance.Connect(
    ...
    )
    {
        ...
        foreach (
            NCardRewardAlternativeButton item2 in _rewardAlternativesContainer
                .GetChildren()
        ...
        for (int i = 0; i < _options.Count; i++)
        {
            ...
            holder.Connect(
                ...
                Callable.From<NCardHolder>(InspectCard)
            );
            holder.Connect(
                Control.SignalName.FocusEntered,
                Callable.From(() => _lastFocusedControl = holder)
            );
            nCard.UpdateVisuals(PileType.None, CardPreviewMode.Normal);
            holder.Scale = holder.SmallScale;
            _cardTween
                .TweenProperty(holder, "position", vector + Vector2.Right * 350f * i, 0.5)
        ...
        for (int num = 0; num < _extraOptions.Count; num++)
        {
            ...
            NCardRewardAlternativeButton nCardRewardAlternativeButton =
                NCardRewardAlternativeButton.Create(
                    cardRewardAlternative.Title.GetFormattedText(),
                    cardRewardAlternative.Hotkey
                );
            _rewardAlternativesContainer.AddChildSafely(nCardRewardAlternativeButton);
            nCardRewardAlternativeButton.Connect(
        ...
        for (int num2 = 0; num2 < _cardRow.GetChildCount(); num2++)
        {
            ...
            child.FocusNeighborLeft = (
                (num2 > 0)
                    ? _cardRow.GetChild(num2 - 1).GetPath()
                    : _cardRow.GetChild(_cardRow.GetChildCount() - 1).GetPath()
            );
            child.FocusNeighborRight = (
                (num2 < _cardRow.GetChildCount() - 1)
                    ? _cardRow.GetChild(num2 + 1).GetPath()
                    : _cardRow.GetChild(0).GetPath()
            );
        }
    ...
    public NCardHolder GetCardHolder(CardModel card)
    {
        return _cardRow
            ...
            .First((NGridCardHolder h) => h.CardModel == card);
    }
    ...
    private void SelectCard(NCardHolder cardHolder)
    {
        if (_completionSource == null)
        {
            throw new InvalidOperationException(
                "CardsSelected must be awaited before a card is selected!"
            );
        }
        _completionSource.SetResult(
            _options.FirstIndex((CardCreationResult o) => o.Card == cardHolder.CardModel)
        );
    }
    ...
    public void AfterOverlayOpened()
    {
        ...
        _buttonTween
            .TweenProperty(
                _rewardAlternativesContainer,
                "position",
                _rewardAlternativesContainer.Position,
                0.5
            )
            .SetEase(Tween.EaseType.Out)
    ...
    private void PowerCardFtueCheck()
    {
        if (!SaveManager.Instance.SeenFtue("power_card_ftue"))
        {
            ...
            NGridCardHolder nGridCardHolder = source.FirstOrDefault(
                (NGridCardHolder h) => h.CardModel.Type == CardType.Power
            );
    ...
    private void UpdateControllerIcons()
    {
        ...
        _inspectPrompt.GetNode<TextureRect>("ControllerIcon").Texture =
            NInputManager.Instance.GetHotkeyIcon(MegaInput.accept);
        _inspectPrompt
            .GetNode<MegaLabel>("Label")
            .SetTextAutoSize(new LocString("gameplay_ui", "TO_INSPECT_PROMPT").GetFormattedText());
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnAlternateRewardSelected,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateControllerIcons,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._rewardAlternativesContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._lastFocusedControl,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._rewardAlternativesContainer,
            Variant.From(in _rewardAlternativesContainer)
        );

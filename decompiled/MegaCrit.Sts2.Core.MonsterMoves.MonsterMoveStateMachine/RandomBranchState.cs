...
public class RandomBranchState : MonsterState
{
    ...
    public override string GetNextState(Creature owner, Rng rng)
    {
        ...
        throw new InvalidOperationException(
            "No valid state found in RandomBranchState " + Id + "!"
        );
    }
    ...
    private static float GetStateWeight(StateWeight stateWeight, Creature owner)
    {
        ...
        {
            float num2 = (
                stateWeight.repeatType.Equals(MoveRepeatType.CannotRepeat)
                    ? 1
            ...
            int num3 = 0;
            while (
                (float)moveStateMachine.StateLog.Count >= num2
                && (float)num3 < num2
                && moveStateMachine.StateLog.Count - num3 > 0
            )
            {
                MonsterState monsterState = moveStateMachine.States[stateWeight.stateId];
                if (
                    moveStateMachine.StateLog[moveStateMachine.StateLog.Count - 1 - num3]
                    != monsterState
                )
                {
                    num = 1f;
        ...
        {
            IEnumerable<MonsterState> source = moveStateMachine
                .StateLog.Where((MonsterState state) => state.IsMove)
                .Reverse()

...
public class MerchantRoom : AbstractRoom
{
    ...
    public List<MerchantInventory> Inventories { get; private set; } =
        new List<MerchantInventory>();
    ...
    public override async Task EnterInternal(IRunState? runState, bool isRestoringRoomStackBase)
    {
        if (isRestoringRoomStackBase)
        {
            throw new InvalidOperationException(
                "MerchantRoom does not support room stack reconstruction."
            );
        }
        _runState = runState;
        ...
        NRun.Instance?.SetCurrentRoom(
            NMerchantRoom.Create(this, runState?.Players ?? Array.Empty<Player>())
        );
        if (runState != null)
        {
        ...
        for (int i = 0; i < Inventories.Count; i++)
        {
            ...
            foreach (MerchantCardEntry characterCardEntry in merchantInventory.CharacterCardEntries)
            {
                if (characterCardEntry.IsStocked)
                {
                    playerMapPointHistoryEntry.CardChoices.Add(
                        new CardChoiceHistoryEntry(
                            characterCardEntry.CreationResult.Card,
                            wasPicked: false
                        )
                    );
                }
            ...
            foreach (MerchantCardEntry colorlessCardEntry in merchantInventory.ColorlessCardEntries)
            {
                if (colorlessCardEntry.IsStocked)
                {
                    playerMapPointHistoryEntry.CardChoices.Add(
                        new CardChoiceHistoryEntry(
                            colorlessCardEntry.CreationResult.Card,
                            wasPicked: false
                        )
                    );
                }
            ...
            foreach (MerchantRelicEntry relicEntry in merchantInventory.RelicEntries)
            {
                if (relicEntry.IsStocked)
                {
                    playerMapPointHistoryEntry.RelicChoices.Add(
                        new ModelChoiceHistoryEntry(relicEntry.Model.Id, wasPicked: false)
                    );
                }
            }
            foreach (MerchantPotionEntry potionEntry in merchantInventory.PotionEntries)
            {
                if (potionEntry.IsStocked)
                {
                    playerMapPointHistoryEntry.PotionChoices.Add(
                        new ModelChoiceHistoryEntry(potionEntry.Model.Id, wasPicked: false)
                    );
                }
            }
        }

...
public class BackgroundAssets
{
    ...
    public IEnumerable<string> AssetPaths =>
        from s in new string[2] { BackgroundScenePath, FgLayer ?? string.Empty }.Concat(BgLayers)
    ...
    public BackgroundAssets(string title, Rng rng)
    {
        ...
        while (next != "")
        {
            ...
            else
            {
                ...
                {
                    throw new InvalidOperationException(
                        "files must either contain '_fg_' or '_bg_'"
                    );
                }
    ...
    )
    {
        List<string> list = new List<string>();
        foreach (
            KeyValuePair<string, List<string>> item2 in bgLayers.OrderBy<
                KeyValuePair<string, List<string>>,
                string
            >((KeyValuePair<string, List<string>> kv) => kv.Key)
        )
        {

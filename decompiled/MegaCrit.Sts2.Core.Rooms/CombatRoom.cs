...
public class CombatRoom : AbstractRoom, ICombatRoomVisuals
{
    ...
    public static new CombatRoom FromSerializable(
    ...
    )
    {
        ...
        EncounterModel encounterModel = SaveUtil
            .EncounterOrDeprecated(serializableRoom.EncounterId)
            .ToMutable();
        ...
        foreach (
            ...
            > extraReward in serializableRoom.ExtraRewards
        )
        {
            ...
            List<Reward> value2 = source
                .Select((SerializableReward sr) => Reward.FromSerializable(sr, player))
                .ToList();
    ...
    public override async Task EnterInternal(IRunState? runState, bool isRestoringRoomStackBase)
    {
        if (isRestoringRoomStackBase)
        {
            throw new InvalidOperationException(
                "CombatRoom does not support room stack reconstruction."
            );
        }
    ...
    public override SerializableRoom ToSerializable()
    {
        ...
        {
            serializableRoom.ExtraRewards[player2.NetId] = source
                .Select((Reward r) => r.ToSerializable())
                .ToList();
        }
    ...
    private async Task StartCombat(IRunState? runState)
    {
        ...
        foreach (var (monsterModel, slot) in Encounter.MonstersWithSlots)
        {
            ...
            CombatState
                .RunState.CurrentMapPointHistoryEntry?.Rooms.Last()
    ...
    public void OnCombatEnded()
    {
        GoldProportion =
            1f
            - (float)CombatState.EscapedCreatures.Count / (float)Encounter.MonstersWithSlots.Count;
    }

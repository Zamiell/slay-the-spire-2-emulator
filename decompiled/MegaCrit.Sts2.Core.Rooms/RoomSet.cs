...
public class RoomSet
{
    ...
    public EncounterModel NextNormalEncounter =>
        normalEncounters[normalEncountersVisited % normalEncounters.Count];

    public EncounterModel NextEliteEncounter =>
        eliteEncounters[eliteEncountersVisited % eliteEncounters.Count];
    ...
    public static RoomSet FromSave(SerializableRoomSet save)
    {
        ...
        roomSet.events.AddRange(
            from e in save.EventIds.Select(SaveUtil.EventOrDeprecated)
            where !(e is DeprecatedEvent)
            select e
        );
        roomSet.eventsVisited = save.EventsVisited;
        roomSet.normalEncounters.AddRange(
            from e in save.NormalEncounterIds.Select(SaveUtil.EncounterOrDeprecated)
            where !(e is DeprecatedEncounter)
            select e
        );
        roomSet.normalEncountersVisited = save.NormalEncountersVisited;
        roomSet.eliteEncounters.AddRange(
            from e in save.EliteEncounterIds.Select(SaveUtil.EncounterOrDeprecated)
            where !(e is DeprecatedEncounter)
            select e
        );
        ...
        roomSet._boss = (
            (save.BossId != null) ? SaveUtil.EncounterOrDeprecated(save.BossId) : null
        );
        roomSet.SecondBoss = (
            (save.SecondBossId != null) ? SaveUtil.EncounterOrDeprecated(save.SecondBossId) : null
        );
        roomSet._ancient = (
            (save.AncientId != null) ? SaveUtil.AncientEventOrDeprecated(save.AncientId) : null
        );
        return roomSet;
    }

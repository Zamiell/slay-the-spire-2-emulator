...
public class CloudSaveStore : ICloudSaveStore, ISaveStore
{
    ...
    public void RenameFile(string sourcePath, string destinationPath)
    {
        ...
        {
            Log.Warn(
                $"Cloud rename failed for {sourcePath} -> {destinationPath}, local rename preserved: {ex.Message}"
            );
        }
    }
    ...
    public void DeleteTemporaryFiles(string directoryPath)
    {
        ...
        {
            Log.Warn(
                "Cloud delete temporary files failed for " + directoryPath + ": " + ex.Message
            );
        }
    }
    ...
    public async Task SyncCloudToLocal(string path)
    {
        ...
        {
            Log.Warn(
                "SteamRemoteStorage: Failed to sync "
                    + path
                    + " from cloud, skipping: "
                    + ex.Message
            );
    ...
    private async Task SyncCloudToLocalInternal(string path)
    {
        ...
        if (flag)
        {
            ...
            DateTimeOffset? dateTimeOffset = (
                flag2
                    ? new DateTimeOffset?(LocalStore.GetLastModifiedTime(path))
                    : ((DateTimeOffset?)null)
            );
            ...
            if (!flag3)
            {
                ...
                if (string.IsNullOrWhiteSpace(value))
                {
                    Log.Warn(
                        "Local file "
                            + path
                            + " appears corrupt (empty content) despite matching cloud timestamp, forcing re-download from cloud"
                    );
                    flag3 = true;
                }
            }
            ...
            {
                Log.Info(
                    $"Copying {path} from cloud to local. Local file exists: {flag2} Cloud save time: {lastModifiedTime} Local save time: {dateTimeOffset}"
                );
            ...
            {
                Log.Debug(
                    $"Skipping sync for {path}, last modified time matches on local and remote ({lastModifiedTime})"
                );
            }
        }
        else if (flag2)
    ...
    public async Task OverwriteCloudWithLocal(string path, bool forgetImmediately = false)
    {
        if (LocalStore.FileExists(path))
        {
            ...
            {
                Log.Warn(
                    "Cloud write failed for " + path + ", local file preserved: " + ex2.Message
                );
                SyncLocalTimestamp(path);
                return;
            }
        ...
        try
        {
            ...
            {
                Log.Debug(
                    "Deleting file " + path + " from cloud because it doesn't exist on local"
                );
                CloudStore.DeleteFile(path);
            }
        }
    ...
    )
    {
        ...
        if (byteLimit.HasValue || fileLimit.HasValue)
        {
            list.Sort(
                (string p1, string p2) =>
                    LocalStore
                        .GetLastModifiedTime(directoryPath + "/" + p2)
                        .CompareTo(LocalStore.GetLastModifiedTime(directoryPath + "/" + p1))
            );
        }
        foreach (string item in list)
        {
            if (!filePathsRead.Contains(item) && ShouldSyncFileToCloud(item))
            {
                ...
                bool flag =
                    (byteLimit.HasValue && i + bytesToWrite > byteLimit.Value)
                    || (fileLimit.HasValue && totalFilesWritten + 1 > fileLimit.Value);
                if (flag)
                {
                    Log.Info(
                        $"File {item} will be immediately forgotten after writing to cloud. Bytes written:{i + bytesToWrite}. Files written: {totalFilesWritten + 1}"
                    );
                }
    ...
    )
    {
        try
        {
            ForgetFilesInDirectoryBeforeWritingIfNecessaryInternal(
                directoryPath,
                bytesToBeWritten,
                byteLimit,
                fileLimit
    ...
    )
    {
        ...
        {
            list.Sort(
                (string p1, string p2) => GetLastModifiedTime(p2).CompareTo(GetLastModifiedTime(p1))
            );
            while (num > byteLimit || num2 > fileLimit)
            {
                ...
                num2--;
                Log.Info(
                    $"Forgetting file {text3} from cloud storage because we're past our quota. Bytes after forgetting: {num}. Files after forgetting: {num2}"
                );
    ...
    private void SyncLocalTimestamp(string path)
    {
        ...
        {
            Log.Warn(
                "Failed to sync timestamp for "
                    + path
                    + ", will re-sync on next launch: "
                    + ex.Message
            );
        }
    }

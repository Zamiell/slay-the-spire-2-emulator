...
public class FileAccessStream : Stream
{
    ...
    {
        get { return (long)_file.GetPosition(); }
    ...
    public FileAccessStream(string filePath, Godot.FileAccess.ModeFlags flags)
    {
        ...
        _file =
            ...
            ?? throw new IOException(
                $"Opening file {filePath} with flags {flags} failed: {Godot.FileAccess.GetOpenError()}"
            );
    }
    ...
    public override void Write(byte[] buffer, int offset, int count)
    {
        ...
        {
            throw new IOException(
                $"Failed to write {num} bytes to file {_filePath}: {_file.GetError()}"
            );
        }
    }

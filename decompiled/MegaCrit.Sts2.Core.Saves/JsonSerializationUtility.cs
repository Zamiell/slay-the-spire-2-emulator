...
{
    public static IJsonTypeInfoResolver DefaultResolver { get; } =
        MegaCritSerializerContext.Default;

    public static JsonSerializerOptions Options { get; } =
        new JsonSerializerOptions(MegaCritSerializerContext.DefaultGeneratedSerializerOptions)
        {
            Encoder = JavaScriptEncoder.UnsafeRelaxedJsonEscaping,
            TypeInfoResolver = MegaCritSerializerContext
                .Default.WithAddedModifier(AlphabetizeProperties)
                .WithAddedModifier(
                    JsonSerializeConditionAttribute.CheckJsonSerializeConditionsModifier
                ),
        };
    ...
    public static void AlphabetizeProperties(JsonTypeInfo info)
    {
        if (info.Kind == JsonTypeInfoKind.Object)
        {
            ...
            list.Sort(
                (JsonPropertyInfo p1, JsonPropertyInfo p2) =>
                    string.CompareOrdinal(p1.Name, p2.Name)
            );
        ...
        where T : ISaveSchema, new()
    {
        ...
        catch (JsonException ex)
        {
            ...
            Log.Error(
                $"Failed to deserialize type={typeof(T)} at path={value}, line={ex.LineNumber}, position={ex.BytePositionInLine}: {ex.Message}"
            );
            return new ReadSaveResult<T>(
                ReadSaveStatus.JsonParseError,
                $"JSON error at {value} (line {ex.LineNumber}): {ex.Message}"
            );
        }
    }
}

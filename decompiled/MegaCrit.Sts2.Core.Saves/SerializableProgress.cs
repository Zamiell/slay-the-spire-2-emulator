...
public class SerializableProgress : ISaveSchema
{
    ...
    public List<SerializableUnlockedAchievement> UnlockedAchievements { get; set; } =
        new List<SerializableUnlockedAchievement>();
    ...
    public SerializableProgress()
    {
        ...
        static string GenerateUniqueId(int length = 7)
        {
            return new string(
                (
                    from s in Enumerable.Repeat("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", length)
                    select s[Rng.Chaotic.NextInt(s.Length)]
                ).ToArray()
            );
        }
    }

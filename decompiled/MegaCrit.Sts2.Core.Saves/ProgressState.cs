...
{
    private static readonly Dictionary<string, Achievement> _achievementsByName =
        BuildAchievementLookup();

    private readonly Dictionary<ModelId, CharacterStats> _characterStats =
        new Dictionary<ModelId, CharacterStats>();

    private readonly Dictionary<ModelId, CardStats> _cardStats =
        new Dictionary<ModelId, CardStats>();

    private readonly Dictionary<ModelId, EncounterStats> _encounterStats =
        new Dictionary<ModelId, EncounterStats>();

    private readonly Dictionary<ModelId, EnemyStats> _enemyStats =
        new Dictionary<ModelId, EnemyStats>();

    private readonly Dictionary<ModelId, AncientStats> _ancientStats =
        new Dictionary<ModelId, AncientStats>();
    ...
    private readonly Dictionary<Achievement, long> _unlockedAchievements =
        new Dictionary<Achievement, long>();
    ...
    public long FastestVictory
    {
        get
        {
            ...
            return _characterStats.Values.Min(
                (CharacterStats c) => (c.FastestWinTime != -1) ? c.FastestWinTime : 999999999
            );
        }
    }
    ...
    )
    {
        ...
        ProgressState progressState = new ProgressState
        {
            ...
            PreferredMultiplayerAscension = ClampAscension(
                save.PreferredMultiplayerAscension,
                "PreferredMultiplayerAscension",
                ctx
            ),
            MaxMultiplayerAscension = ClampAscension(
                save.MaxMultiplayerAscension,
                "MaxMultiplayerAscension",
                ctx
            ),
            TestSubjectKills = ClampNonNegativeInt(save.TestSubjectKills, "TestSubjectKills", ctx),
            PendingCharacterUnlock = ValidateModelId<CharacterModel>(
                save.PendingCharacterUnlock,
        ...
        ParseDiscoveredSet<CardModel>(
            save.DiscoveredCards,
            progressState._discoveredCards,
            "DiscoveredCards",
            ctx
        );
        ParseDiscoveredSet<RelicModel>(
            save.DiscoveredRelics,
            progressState._discoveredRelics,
            "DiscoveredRelics",
            ctx
        );
        ParseDiscoveredSet<PotionModel>(
            save.DiscoveredPotions,
            progressState._discoveredPotions,
            "DiscoveredPotions",
            ctx
        );
        ParseDiscoveredSet<EventModel>(
            save.DiscoveredEvents,
            progressState._discoveredEvents,
            "DiscoveredEvents",
            ctx
        );
        ParseDiscoveredSet<ActModel>(
            save.DiscoveredActs,
            progressState._discoveredActs,
            "DiscoveredActs",
            ctx
        );
    ...
    public SerializableProgress ToSerializable()
    {
        return new SerializableProgress
        {
            ...
            UnlockedAchievements = _unlockedAchievements
                .Select(
                    (KeyValuePair<Achievement, long> kvp) =>
                        new SerializableUnlockedAchievement
                        {
                            Achievement = JsonNamingPolicy.SnakeCaseLower.ConvertName(
                                kvp.Key.ToString()
                            ),
    ...
    public CharacterStats GetOrCreateCharacterStats(ModelId characterId)
    {
        ...
        CharacterStats characterStats = new CharacterStats { Id = characterId };
    ...
    public CardStats GetOrCreateCardStats(ModelId cardId)
    {
        ...
        CardStats cardStats = new CardStats { Id = cardId };
    ...
    {
        SerializableEpoch serializableEpoch = _epochs.FirstOrDefault(
            (SerializableEpoch e) => e.Id == epochId
        );
    ...
    public void UnlockSlot(string epochId)
    {
        ...
        {
            Log.Info(
                "Attempted to get slot "
                    + epochId
                    + " but we already have an Epoch! Set to Obtained"
            );
            serializableEpoch.State = EpochState.Obtained;
            return;
        }
        Log.Error(
            $"Slot unlocked for {epochId} but it's in an invalid state: {serializableEpoch.State}"
        );
    }
    ...
    public void RevealEpoch(string epochId)
    {
        ...
        if (serializableEpoch == null)
        {
            throw new InvalidOperationException(
                $"Invalid epoch {epochId} passed to {"RevealEpoch"}!"
            );
        }
    ...
    public EncounterStats GetOrCreateEncounterStats(ModelId encounterId)
    {
        ...
        EncounterStats encounterStats = new EncounterStats { Id = encounterId };
    ...
    public EnemyStats GetOrCreateEnemyStats(ModelId enemyId)
    {
        ...
        EnemyStats enemyStats = new EnemyStats { Id = enemyId };
    ...
    public AncientStats GetOrCreateAncientStats(ModelId ancientId)
    {
        ...
        AncientStats ancientStats = new AncientStats { Id = ancientId };
    ...
    private static void ParseCharacterStats(
        List<CharacterStats> source,
        Dictionary<ModelId, CharacterStats> target,
        DeserializationContext ctx
    )
    {
    ...
    private static void ParseCardStats(
        List<CardStats> source,
        Dictionary<ModelId, CardStats> target,
        DeserializationContext ctx
    )
    {
    ...
    private static void ParseEncounterStats(
        List<EncounterStats> source,
        Dictionary<ModelId, EncounterStats> target,
        DeserializationContext ctx
    )
    {
        ...
        for (int i = 0; i < source.Count; i++)
        {
            ...
            if (!target.TryAdd(encounterStats.Id, encounterStats))
            {
                ...
                MergeFightStatsList(
                    target[encounterStats.Id].FightStats,
                    encounterStats.FightStats
                );
                ctx.PopPath();
            }
            else
    ...
    private static void ParseEnemyStats(
        List<EnemyStats> source,
        Dictionary<ModelId, EnemyStats> target,
        DeserializationContext ctx
    )
    {
    ...
    private static void ParseAncientStats(
        List<AncientStats> source,
        Dictionary<ModelId, AncientStats> target,
        DeserializationContext ctx
    )
    {
        ...
        for (int i = 0; i < source.Count; i++)
        {
            ...
            if (!target.TryAdd(ancientStats.Id, ancientStats))
            {
                ...
                MergeAncientCharacterStatsList(
                    target[ancientStats.Id].CharStats,
                    ancientStats.CharStats
                );
                ctx.PopPath();
            }
            else
    ...
    )
    {
        ...
        for (int i = 0; i < source.Count; i++)
        {
            ...
            {
                ctx.Warn(
                    $"Invalid epoch state {serializableEpoch.State} for {serializableEpoch.Id}, skipping"
                );
                ctx.PopPath();
            }
            ...
            {
                ctx.Warn(
                    $"Epoch {serializableEpoch.Id} has unused state {serializableEpoch.State}, skipping"
                );
                ctx.PopPath();
            }
    ...
    {
        Dictionary<string, SerializableEpoch> dictionary =
            new Dictionary<string, SerializableEpoch>();
        ...
        foreach (SerializableEpoch item in list)
        {
            ...
            foreach (EpochModel epochModel2 in timelineExpansion)
            {
                if (dictionary.TryGetValue(epochModel2.Id, out var value))
                {
                    ...
                    {
                        ctx.Warn(
                            $"Epoch {epochModel2.Id} was ObtainedNoSlot but parent {item.Id} is Revealed, promoting to Obtained"
                        );
                ...
                {
                    ctx.Warn(
                        $"Epoch {epochModel2.Id} slot missing but parent {item.Id} is Revealed, creating as NotObtained"
                    );
                    SerializableEpoch serializableEpoch = new SerializableEpoch(
                        epochModel2.Id,
                        EpochState.NotObtained
                    );
    ...
    )
    {
        ...
        for (int i = 0; i < source.Count; i++)
        {
            ...
            if (
                !_achievementsByName.TryGetValue(
                    serializableUnlockedAchievement.Achievement,
                    out var value
                )
            )
            {
                ctx.Warn(
                    $"Unknown achievement \"{serializableUnlockedAchievement.Achievement}\" at index {i}, skipping"
                );
            }
    ...
    private static void ClampCharacterStatsFields(CharacterStats stats, DeserializationContext ctx)
    {
        ...
        {
            ctx.Warn(
                $"PreferredAscension ({stats.PreferredAscension}) exceeds allowed ({10}), clamping"
            );
    ...
    private static void ClampFightStatsFields(
        List<FightStats>? fightStats,
        DeserializationContext ctx
    )
    {
        if (fightStats == null)
        ...
        for (int num = fightStats.Count - 1; num >= 0; num--)
        {
            ...
            if (
                fight.Character == ModelId.none
                || ModelDb.GetByIdOrNull<CharacterModel>(fight.Character) == null
            )
            {
            ...
            {
                FightStats fightStats2 = fightStats
                    .Take(num)
                    .FirstOrDefault((FightStats f) => f.Character == fight.Character);
                if (fightStats2 != null)
                {
    ...
    )
    {
        ...
        for (int num = charStats.Count - 1; num >= 0; num--)
        {
            ...
            if (
                stats.Character == ModelId.none
                || ModelDb.GetByIdOrNull<CharacterModel>(stats.Character) == null
            )
            {
            ...
            {
                AncientCharacterStats ancientCharacterStats = charStats
                    .Take(num)
                    .FirstOrDefault((AncientCharacterStats c) => c.Character == stats.Character);
                if (ancientCharacterStats != null)
                {
                    ...
                    ancientCharacterStats.Losses = Math.Max(
    ...
    private static void MergeCharacterStats(CharacterStats existing, CharacterStats incoming)
    {
        ...
        existing.PreferredAscension = Math.Max(
            existing.PreferredAscension,
            incoming.PreferredAscension
        );
        existing.FastestWinTime = MergeFastestWinTime(
            existing.FastestWinTime,
            incoming.FastestWinTime
        );
    }
    ...
    private static void MergeFightStatsList(List<FightStats>? existing, List<FightStats>? incoming)
    {
        ...
        {
            FightStats fightStats = existing.FirstOrDefault(
                (FightStats f) => f.Character == incomingFight.Character
            );
            if (fightStats != null)
            {
    ...
    )
    {
        ...
        {
            AncientCharacterStats ancientCharacterStats = existing.FirstOrDefault(
                (AncientCharacterStats c) => c.Character == incomingStats.Character
            );
            if (ancientCharacterStats != null)
            {
                ancientCharacterStats.Wins = Math.Max(
                    ancientCharacterStats.Wins,
                    incomingStats.Wins
                );
                ancientCharacterStats.Losses = Math.Max(
                    ancientCharacterStats.Losses,
                    incomingStats.Losses
                );
            }
            else
            {

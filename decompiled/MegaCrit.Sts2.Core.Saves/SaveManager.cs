...
public class SaveManager : IProfileIdProvider
{
    ...
    {
        get { return _progressSaveManager.Progress; }
    ...
    public int CurrentProfileId =>
        _currentProfileId
        ?? throw new InvalidOperationException("InitProfileId must be called on SaveManager!");

    public Task? CurrentRunSaveTask { get; private set; }

    public event Action? Saved
    {
        add { _runSaveManager.Saved += value; }
        remove { _runSaveManager.Saved -= value; }
    }
    ...
    private static SaveManager ConstructDefault()
    {
        ...
        ISaveStore saveStore2 = new GodotFileIo(
        ...
        {
            Log.Info(
                $"Steam is enabled, we will write saves to steam storage. Enabled for account: {SteamRemoteStorage.IsCloudEnabledForAccount()}, app: {SteamRemoteStorage.IsCloudEnabledForApp()} "
            );
    ...
    public void InitProfileId(int? profileId = null)
    {
        ...
        string profileScopedBasePath = UserDataPathProvider.GetProfileScopedBasePath(
            CurrentProfileId
        );
    ...
    {
        return _saveStore.GetFullPath(
            Path.Combine(UserDataPathProvider.GetProfileDir(CurrentProfileId), userData)
        );
    }
    ...
    private IEnumerable<Task> EnumerateCloudSyncTasks(CloudSaveStore cloudStore)
    {
        ...
        for (int i = 1; i <= 3; i++)
        {
            yield return cloudStore.SyncCloudToLocal(
                ProgressSaveManager.GetProgressPathForProfile(i)
            );
            yield return cloudStore.SyncCloudToLocal(
                RunSaveManager.GetRunSavePath(i, "current_run.save")
            );
            yield return cloudStore.SyncCloudToLocal(
                RunSaveManager.GetRunSavePath(i, "current_run_mp.save")
            );
            yield return cloudStore.SyncCloudToLocal(PrefsSaveManager.GetPrefsPath(i));
            foreach (
                Task item in cloudStore.SyncCloudToLocalDirectory(
                    RunHistorySaveManager.GetHistoryPath(i)
                )
            )
            {
                yield return item;
    ...
    private void CleanupStaleCurrentRunSaveForProfile(int profileId, string runSaveFileName)
    {
        ...
        try
        {
            ...
            {
                Log.Warn(
                    "Could not extract start_time from " + text2 + ", skipping staleness check"
                );
                return;
            }
            ...
            {
                Log.Warn(
                    $"Deleting stale {runSaveFileName} for profile {profileId}: run with StartTime {value} already exists in history at {text4}"
                );
        ...
        {
            Log.Warn(
                $"Error checking for stale current_run.save in profile {profileId}: {ex.Message}"
            );
        }
    }
    ...
    private void CleanupTemporaryFiles()
    {
        ...
        for (int i = 1; i <= 3; i++)
        {
            string directoryPath = Path.Combine(
                UserDataPathProvider.GetProfileDir(i),
    ...
    public async Task<bool> TryFirstTimeCloudSync()
    {
        ...
        Log.Info(
            "Looks like the first time we enabled cloud saves, and we have local saves. Uploading local saves to the cloud"
        );
        List<Task> tasks = new List<Task>();
    ...
    private IEnumerable<Task> EnumerateFirstTimeCloudSyncTasks(CloudSaveStore cloudStore)
    {
        ...
        for (int i = 1; i <= 3; i++)
        {
            yield return cloudStore.OverwriteCloudWithLocal(
                ProgressSaveManager.GetProgressPathForProfile(i)
            );
            yield return cloudStore.OverwriteCloudWithLocal(
                RunSaveManager.GetRunSavePath(i, "current_run.save")
            ...
            foreach (
                Task item in cloudStore.OverwriteCloudWithLocalDirectory(
                    RunHistorySaveManager.GetHistoryPath(i),
                    5242880,
                    100
                )
    ...
    public ReadSaveResult<SerializableRun> LoadAndCanonicalizeMultiplayerRunSave(
        ulong localPlayerId
    )
    {
    ...
    public static ReadSaveResult<T> FromJson<
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicMethods)] T
    >(string json)
    ...
    {
        SerializableEpoch serializableEpoch = Progress.Epochs.FirstOrDefault(

...
public class GodotFileIo : ISaveStore
{
    ...
    public string? ReadFile(string path)
    {
        ...
        if (fileAccess == null)
        {
            ...
            throw new SaveException(
                $"Failed to open file for reading. path='{path}' error={openError}"
            );
        }
    ...
    public void WriteFile(string path, byte[] bytes)
    {
        ...
        {
            throw new SaveException(
                $"Failed to open file for writing. path='{text}' error={Godot.FileAccess.GetOpenError()}"
            );
        }
        ...
        throw new SaveException(
            $"Failed to write {bytes.Length} bytes to path={path} save_dir={SaveDir}. Error: {fileAccess.GetError()}"
        );
    }
    ...
    public async Task WriteFileAsync(string path, byte[] bytes)
    {
        ...
        await using FileAccessStream stream = new FileAccessStream(
            tempPath,
            Godot.FileAccess.ModeFlags.Write
        );
    ...
    public void RenameFile(string sourcePath, string destinationPath)
    {
        ...
        {
            throw new SaveException(
                "Cannot rename file: source does not exist. source=" + GetFullPath(sourcePath)
            );
        }
        ...
        for (int i = 1; i <= 4; i++)
        {
            ...
            if (
                Godot.FileAccess.FileExists(destinationPath)
                && !Godot.FileAccess.FileExists(sourcePath)
            )
            {
                Log.Warn(
                    $"Rename reported error={error} but destination exists, treating as success. source={sourcePath}"
                );
                return;
            }
            if (i < 4)
            {
                Log.Warn(
                    $"Rename failed (attempt {i}/{4}), retrying. error={error} source={sourcePath}"
                );
                Thread.Sleep(50);
            }
        }
        Thread.Sleep(100);
        if (
            Godot.FileAccess.FileExists(destinationPath) && !Godot.FileAccess.FileExists(sourcePath)
        )
        {
            Log.Warn(
                "Rename appeared to fail but destination exists after delay, treating as success. source="
                    + sourcePath
            );
            return;
        }
        throw new SaveException(
            $"Failed to rename file. error={error} source={sourcePath} destination={destinationPath} source_exists={Godot.FileAccess.FileExists(sourcePath)} destination_exists={Godot.FileAccess.FileExists(destinationPath)}"
        );
    }
    ...
    public void DeleteDirectory(string directoryPath)
    {
        ...
        foreach (string text in files)
        {
            ...
            if (error != Error.Ok)
            {
                throw new InvalidOperationException(
                    $"Got error {error} trying to delete file {text} in directory {directoryPath}"
                );
            }
        }
        ...
        {
            throw new InvalidOperationException(
                $"Got error {error2} trying to delete directory {directoryPath}"
            );
        }
    }
    ...
    public void DeleteTemporaryFiles(string directoryPath)
    {
        ...
        foreach (string text in files)
        {
            if (text.EndsWith(".tmp"))
            {
                ...
                {
                    Log.Warn(
                        $"Couldn't delete temporary file {text} in {directoryPath}, error={error}"
                    );
                }
            }
        }
    ...
    private void CopyBackup(string fullPath)
    {
        ...
        {
            Log.Warn(
                $"Failed to open source for backup copy. path={fullPath} error={Godot.FileAccess.GetOpenError()}"
            );
            return;
        }
        byte[] buffer = fileAccess.GetBuffer((long)fileAccess.GetLength());
        fileAccess.Close();
        using Godot.FileAccess fileAccess2 = Godot.FileAccess.Open(
            text,
            Godot.FileAccess.ModeFlags.Write
        );
        if (fileAccess2 == null)
        {
            Log.Warn(
                $"Failed to open backup for writing. path={text} error={Godot.FileAccess.GetOpenError()}"
            );
            return;
        }

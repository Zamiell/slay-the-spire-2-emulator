...
public class NProfileScreen : NSubmenu
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/profiles/profile_screen"
    );
    ...
    protected override Control InitialFocusedControl =>
        _profileButtons[SaveManager.Instance.CurrentProfileId - 1];
    ...
    public static NProfileScreen? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NProfileScreen>(PackedScene.GenEditState.Disabled);
    }
    ...
    public override void _Ready()
    {
        ...
        GetNode<MegaLabel>("%ChooseProfileMessage")
            .SetTextAutoSize(
                new LocString(
                    "main_menu_ui",
                    "PROFILE_SCREEN.BUTTON.chooseProfileMessage"
                ).GetFormattedText()
            );
        ...
        {
            Log.Error(
                $"There are {_profileButtons.Count} profile buttons, but max profile count in ProfileSaveManager is {3}! This might result in subtle bugs"
            );
        }
        for (int i = 0; i < _profileButtons.Count; i++)
        {
            ...
            _profileButtons[i].FocusNeighborRight = (
                (i < _profileButtons.Count - 1)
                    ? _profileButtons[i + 1].GetPath()
                    : _profileButtons[0].GetPath()
            );
            ...
            _deleteButtons[i].FocusNeighborRight = (
                (i < _deleteButtons.Count - 1)
                    ? _deleteButtons[i + 1].GetPath()
                    : _deleteButtons[0].GetPath()
            );
        }
    }

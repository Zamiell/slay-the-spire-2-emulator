...
public class NProfileButton : NButton
{
    ...
    public void Initialize(NProfileScreen profileScreen, int profileId)
    {
        ...
        GodotFileIo godotFileIo = new GodotFileIo(
            UserDataPathProvider.GetProfileScopedPath(profileId, "saves")
        );
        ...
        if (SaveManager.Instance.CurrentProfileId == profileId)
        {
            locString3.Add(
                "Playtime",
                TimeFormatting.Format(SaveManager.Instance.Progress.TotalPlaytime)
            );
        }
        else
        {
            string json = godotFileIo.ReadFile(path);
            JsonObject jsonObject = JsonSerializer.Deserialize(
                json,
                JsonSerializationUtility.GetTypeInfo<JsonObject>()
            );
            if (
                jsonObject.TryGetPropertyValue("total_playtime", out JsonNode jsonNode)
        ...
        if (godotFileIo.FileExists(path2))
        {
            ...
            dateTimeOffset = (
                (dateTimeOffset > lastModifiedTime) ? dateTimeOffset : lastModifiedTime
            );
        }
        ...
        if (godotFileIo.FileExists(path3))
        {
            ...
            dateTimeOffset = (
                (dateTimeOffset > lastModifiedTime2) ? dateTimeOffset : lastModifiedTime2
            );
        }
    ...
    private async Task SwitchToThisProfile()
    {
        ...
        NGame.Instance.CheckShowSaveFileError(
            progressReadResult,
            prefsReadResult,
    ...
    )
    {
        ...
        {
            Initialize(
                VariantUtils.ConvertTo<NProfileScreen>(in args[0]),
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._currentProfileIndicator,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._currentProfileIndicator,
            Variant.From(in _currentProfileIndicator)
        );

...
public class NInspectRelicScreen : Control, IScreenContext
{
    ...
    public static NInspectRelicScreen? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NInspectRelicScreen>(PackedScene.GenEditState.Disabled);
    }
    ...
    public void Open(IReadOnlyList<RelicModel> relics, RelicModel relic)
    {
        ...
        _screenTween
            .TweenProperty(_leftButton, "position:x", _leftButtonX, 0.25)
        ...
        _screenTween
            .TweenProperty(_rightButton, "position:x", _rightButtonX, 0.25)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Back)
        ...
        _popupTween
            .TweenProperty(_popup, "position", _popupPosition, 0.25)
            .SetEase(Tween.EaseType.Out)
    ...
    public override void _Ready()
    {
        ...
        _backstop.Connect(
            ...
            Callable.From<NButton>(OnBackstopPressed)
        );
        _leftButton = GetNode<NGoldArrowButton>("LeftArrow");
        _leftButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnLeftButtonPressed)
        );
        _rightButton = GetNode<NGoldArrowButton>("RightArrow");
        _rightButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnRightButtonPressed)
        );
    ...
    private void UpdateRelicDisplay()
    {
        ...
        {
            _nameLabel.SetTextAutoSize(
                new LocString("inspect_relic_screen", "LOCKED_TITLE").GetFormattedText()
            );
            _rarityLabel.SetTextAutoSize(string.Empty);
            _relicImage.SelfModulate = Colors.White;
            _description.SetTextAutoSize(
                new LocString("inspect_relic_screen", "LOCKED_DESCRIPTION").GetFormattedText()
            );
            _flavor.Text = string.Empty;
            SetRarityVisuals(RelicRarity.Common);
            _relicImage.Texture = PreloadManager.Cache.GetTexture2D(
        ...
        {
            _nameLabel.SetTextAutoSize(
                new LocString("inspect_relic_screen", "UNDISCOVERED_TITLE").GetFormattedText()
            );
            _rarityLabel.SetTextAutoSize(string.Empty);
            _relicImage.SelfModulate = StsColors.ninetyPercentBlack;
            _description.SetTextAutoSize(
                new LocString("inspect_relic_screen", "UNDISCOVERED_DESCRIPTION").GetFormattedText()
            );
            _flavor.Text = string.Empty;
        ...
        else
        {
            ...
            LocString locString = new LocString(
                "gameplay_ui",
                "RELIC_RARITY." + relicModel.Rarity.ToString().ToUpperInvariant()
            );
        ...
        {
            NHoverTipSet
                .CreateAndShow(this, relicModel.HoverTipsExcludingRelic)
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnRightButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnLeftButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,

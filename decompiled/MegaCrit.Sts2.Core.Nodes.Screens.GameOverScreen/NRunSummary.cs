...
public class NRunSummary : Control
{
    ...
    public Control? DefaultFocusedControl =>
        _discoveredContents
            .GetChildren()
            .OfType<Control>()
            .FirstOrDefault((Control c) => c.IsVisible());
    ...
    public async Task AnimateInDiscoveries(RunState runState, CancellationToken ct)
    {
        ...
        if (
            player.DiscoveredCards.Count
                + player.DiscoveredRelics.Count
                + player.DiscoveredPotions.Count
                + player.DiscoveredEnemies.Count
                + player.DiscoveredEpochs.Count
            == 0
        )
        {
        ...
        {
            string discoveryBodyText = GetDiscoveryBodyText(
                player.DiscoveredCards,
                (ModelId id) => SaveUtil.CardOrDeprecated(id).Title,
                "game_over_screen",
                "DISCOVERY_BODY_CARD",
                "CardCount"
            );
            _discoveredCards.SetHoverTip(
                new HoverTip(
                    new LocString("game_over_screen", "DISCOVERY_HEADER_CARD"),
                    discoveryBodyText
                )
            );
        ...
        {
            string discoveryBodyText2 = GetDiscoveryBodyText(
                player.DiscoveredRelics,
                (ModelId id) => SaveUtil.RelicOrDeprecated(id).Title.GetFormattedText(),
                "game_over_screen",
                "DISCOVERY_BODY_RELIC",
                "RelicCount"
            );
            _discoveredRelics.SetHoverTip(
                new HoverTip(
                    new LocString("game_over_screen", "DISCOVERY_HEADER_RELIC"),
                    discoveryBodyText2
                )
            );
        ...
        {
            string discoveryBodyText3 = GetDiscoveryBodyText(
                player.DiscoveredPotions,
                (ModelId id) => SaveUtil.PotionOrDeprecated(id).Title.GetFormattedText(),
                "game_over_screen",
                "DISCOVERY_BODY_POTION",
                "PotionCount"
            );
            _discoveredPotions.SetHoverTip(
                new HoverTip(
                    new LocString("game_over_screen", "DISCOVERY_HEADER_POTION"),
                    discoveryBodyText3
                )
            );
        ...
        {
            string discoveryBodyText4 = GetDiscoveryBodyText(
                player.DiscoveredEnemies,
                (ModelId id) => SaveUtil.MonsterOrDeprecated(id).Title.GetFormattedText(),
                "game_over_screen",
                "DISCOVERY_BODY_ENEMY",
                "EnemyCount"
            );
            _discoveredEnemies.SetHoverTip(
                new HoverTip(
                    new LocString("game_over_screen", "DISCOVERY_HEADER_ENEMY"),
                    discoveryBodyText4
                )
            );
    ...
    private async Task DiscoveryAnimHelper(Control node)
    {
        ...
        _tween
            .TweenProperty(node, "position:y", 0f, 0.3)
    ...
    {
        Control[] array = (
            from c in _discoveredContents.GetChildren().OfType<Control>()
            where c.IsVisible()
            select c
        ).ToArray();
        for (int num = 0; num < array.Length; num++)
        {
            ...
            control.FocusNeighborTop = (
                (focusNeighborTop != null) ? focusNeighborTop.GetPath() : control.GetPath()
            );
            control.FocusNeighborBottom = control.GetPath();
            control.FocusNeighborLeft = (
                (num > 0) ? array[num - 1].GetPath() : array[^1].GetPath()
            );
            control.FocusNeighborRight = (
                (num < array.Length - 1) ? array[num + 1].GetPath() : array[0].GetPath()
            );
        }
    }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._discoveryContainer,
                PropertyHint.None,
                "",
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._discoveredContents,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._discoveredRelics,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._discoveredPotions,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._discoveredEnemies,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._discoveredEpochs,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

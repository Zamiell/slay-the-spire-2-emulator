...
public class NGameOverScreen : NClickableControl, IOverlayScreen, IScreenContext
{
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlySingleElementList<string>(ScenePath);
    ...
    public override void _Ready()
    {
        ...
        _history = RunManager.Instance.History ?? new RunHistory { Win = win };
        ...
        _continueButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OpenSummaryScreen)
        );
        _continueButton.Disable();
        _viewRunButton = GetNode<NViewRunButton>("%ViewRunButton");
        _viewRunButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OpenRunHistoryScreen)
        );
        _mainMenuButton = GetNode<NReturnToMainMenuButton>("%MainMenuButton");
        _mainMenuButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnMainMenuButtonPressed)
        );
        ...
        _leaderboardButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(ShowLeaderboard)
        );
        ...
        _discoveryLabel.SetTextAutoSize(
            new LocString("game_over_screen", "DISCOVERY_HEADER").GetFormattedText()
        );
    ...
    private void InitializeBannerAndQuote()
    {
        ...
        if (_history.Win)
        {
            _banner.label.SetTextAutoSize(
                new LocString("game_over_screen", "BANNER.falseWin").GetRawText()
            );
            ...
            if (
                ascensionLevel < 10
                && ascensionLevel > 0
                && _runState.AscensionLevel >= _localPlayer.MaxAscensionWhenRunStarted
                && _runState.GameMode == GameMode.Standard
            )
            {
                stringBuilder.Append("\n\n");
                LocString locString2 = new LocString(
                    "game_over_screen",
                    "VICTORY_UNLOCKED_ASCENSION"
                );
        ...
        else
        {
            ...
            IReadOnlyList<LocString> locStringsWithPrefix = table.GetLocStringsWithPrefix(
                "BANNER.lose"
            );
            _banner.label.SetTextAutoSize(Rng.Chaotic.NextItem(locStringsWithPrefix).GetRawText());
            IReadOnlyList<LocString> locStringsWithPrefix2 = table.GetLocStringsWithPrefix(
                "QUOTES"
            );
            _deathQuote.Text = Rng.Chaotic.NextItem(locStringsWithPrefix2).GetFormattedText();
        }
        _encounterQuote = NRunHistory.GetDeathQuote(
            _history,
            id,
            NRunHistory.GetGameOverType(_history)
        );
    }
    ...
    private async Task AnimateInQuote()
    {
        ...
        if (this.IsValid())
        {
            ...
            {
                _quoteTween
                    .TweenProperty(_victoryDamageLabel, "visible_ratio", 1f, 2.0)
            ...
            {
                _quoteTween
                    .TweenProperty(_deathQuote, "position:y", 156f, 2.0)
                    .SetEase(Tween.EaseType.Out)
    ...
    public static NGameOverScreen? Create(RunState runState, SerializableRun serializableRun)
    {
        ...
        NGameOverScreen nGameOverScreen = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NGameOverScreen>(PackedScene.GenEditState.Disabled);
    ...
    private async Task AnimateRunSummary()
    {
        Tween tween = CreateTween();
        tween
            .TweenProperty(_banner, "position:y", _banner.Position.Y - 32f, 0.5)
            .SetEase(Tween.EaseType.Out)
        ...
        {
            _badgeContainer.GetChild<NBadge>(i).FocusNeighborLeft = (
                (i > 0)
                    ? _badgeContainer.GetChild<NBadge>(i - 1).GetPath()
                    : _badgeContainer
                        .GetChild<NBadge>(_badgeContainer.GetChildCount() - 1)
                        .GetPath()
            );
            _badgeContainer.GetChild<NBadge>(i).FocusNeighborRight = (
                (i < _badgeContainer.GetChildCount() - 1)
                    ? _badgeContainer.GetChild<NBadge>(i + 1).GetPath()
                    : _badgeContainer.GetChild<NBadge>(0).GetPath()
            );
            _badgeContainer.GetChild<NBadge>(i).FocusNeighborTop = _badgeContainer
                .GetChild<NBadge>(i)
                .GetPath();
            _badgeContainer.GetChild<NBadge>(i).FocusNeighborBottom = (
                (_summaryContainer.DefaultFocusedControl != null)
                    ? _summaryContainer.DefaultFocusedControl.GetPath()
                    : _badgeContainer.GetChild<NBadge>(i).GetPath()
            );
        }
        _summaryContainer.SetControllerNav(
            (_badgeContainer.GetChildCount() > 0) ? _badgeContainer.GetChild<Control>(0) : null
        );
        if (_badgeContainer.GetChildCount() > 0)
        {
        ...
        if (_history.GameMode == GameMode.Daily)
        {
            _leaderboard.Initialize(
                RunManager.Instance.DailyTime.Value,
                _runState.Players.Select((Player p) => p.NetId),
    ...
    private async Task AnimateScoreLines()
    {
        _scoreLines.Clear();
        AddScoreLine(
            "SCORE_LINE.floorsClimbed",
            "FloorCount",
            _runState.TotalFloor,
            $"+{ScoreUtility.GetScoreForFloor(_serializableRun.MapPointHistory)}",
            "res://images/ui/game_over_screen/score_floor.png"
        );
        int amount = _serializableRun
            .MapPointHistory.SelectMany((List<MapPointHistoryEntry> actEntries) => actEntries)
            .Sum((MapPointHistoryEntry e) => e.GetEntry(_localPlayer.NetId).GoldGained);
        AddScoreLine(
            "SCORE_LINE.goldGained",
            "GoldAmount",
            amount,
            $"+{ScoreUtility.GetScoreForGoldGained(_serializableRun.MapPointHistory, _serializableRun.Players.Count)}",
        ...
        {
            AddScoreLine(
                "SCORE_LINE.elitesKilled",
                "EliteCount",
                elitesKilledCount,
                $"+{ScoreUtility.GetScoreForElitesKilled(elitesKilledCount)}",
                "res://images/ui/game_over_screen/score_elite.png"
            );
        }
        int bossesSlainCount = ScoreUtility.GetBossesSlainCount(
            _serializableRun.MapPointHistory,
            _history.Win
        );
        if (bossesSlainCount > 0)
        {
            AddScoreLine(
                ...
                "BossCount",
                bossesSlainCount,
                $"+{ScoreUtility.GetScoreForBossesSlain(bossesSlainCount)}",
        ...
        {
            AddScoreLine(
                "SCORE_LINE.ascension",
                "AscensionLevel",
                ascension,
                "x" + GetAscensionMulti(ascension),
                "res://images/ui/game_over_screen/score_ascension.png"
            );
        }
    ...
    {
        List<Badge> badges = ScoreUtility.GetBadges(
            _serializableRun,
            _localPlayer.NetId,
            _history.Win
        );
    ...
    {
        CharacterStats characterStats = SaveManager.Instance.Progress.CharacterStats[
            _localPlayer.Character.Id
        ];
        foreach (Badge badge in badgesToSave)
        {
            BadgeStats badgeStats = characterStats.Badges.FirstOrDefault(
                (BadgeStats b) => b.Id.Equals(badge.Id) && b.Rarity == badge.Rarity
            );
            if (badgeStats == null)
            {
    ...
    private async Task AnimateScoreBar()
    {
        ...
        if (unlocksRemaining > 0)
        {
            ...
            if (currentScore + _score >= _scoreThreshold)
            {
                ...
                string locEntryKey = (
                    (newThreshold == 0) ? "SCORE.unlockedAllMessage" : "SCORE.unlockedEpochMessage"
                );
                node.SetTextAutoSize(
                    new LocString("game_over_screen", locEntryKey).GetFormattedText()
                );
                ...
                scoreTween
                    ...
                    .SetTrans(Tween.TransitionType.Cubic);
                scoreTween
                    .TweenProperty(_scoreFg, "scale:x", 1f, 1.0)
                    .SetEase(Tween.EaseType.Out)
                    .SetTrans(Tween.TransitionType.Cubic);
                scoreTween.Chain();
                ...
                scoreTween
                    .TweenProperty(node, "position:y", -60f, 0.25)
                    .SetEase(Tween.EaseType.Out)
                ...
                if (
                    _scoreUnlockedEpochId != null
                    && !SaveManager.Instance.IsEpochRevealed(_scoreUnlockedEpochId)
                )
                {
                ...
                else
                {
                    ...
                    scoreTween.Chain();
                    scoreTween
                        .TweenMethod(Callable.From<int>(TweenScore), 0, currentScore, 1.0)
                        .SetEase(Tween.EaseType.Out)
                        .SetTrans(Tween.TransitionType.Cubic);
                    scoreTween
                        .TweenProperty(
                            _scoreFg,
                            "scale:x",
                            (float)currentScore / (float)newThreshold,
                            1.0
                        )
                        .SetEase(Tween.EaseType.Out)
                        .SetTrans(Tween.TransitionType.Cubic)
            ...
            else
            {
                ...
                scoreTween.TweenProperty(
                    _scoreFg,
                    "scale:x",
                    (float)(currentScore + _score) / (float)_scoreThreshold,
                    1.0
                );
    ...
    {
        _banner.ChangeText(
            new LocString("main_menu_ui", "DAILY_RUN_MENU.LEADERBOARDS.title").GetRawText()
        );
    ...
    {
        Control child = ResourceLoader
            .Load<PackedScene>(
                "res://scenes/screens/run_history_screen/run_history_screen_via_game_over_screen.tscn",
                null,
    ...
    private void MoveCreaturesToDifferentLayerAndDisableUi()
    {
        ...
        else if (NRestSiteRoom.Instance != null)
        {
            ...
            foreach (Player player in _runState.Players)
            {
                ...
                NRestSiteCharacter characterForPlayer =
                    NRestSiteRoom.Instance.GetCharacterForPlayer(player);
                ...
                nCreatureVisuals.Position +=
                    vector
                    * new Vector2(
                        Math.Sign(nCreatureVisuals.Scale.X),
                        Math.Sign(nCreatureVisuals.Scale.Y)
                    );
            }
        }
        else
    ...
    private async Task AnimateIn()
    {
        ...
        if (NEventRoom.Instance != null)
        {
            ...
            foreach (
                NCreatureVisuals item in _creatureContainer.GetChildren().OfType<NCreatureVisuals>()
            )
            {
        ...
        backstopTween
            .TweenMethod(Callable.From<float>(UpdateBackstopMaterial), shaderParameter, 1f, 1.5)
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.DiscoveredAnyEpochs,
                new PropertyInfo(
                    Variant.Type.Bool,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.InitializeBannerAndQuote,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OpenRunHistoryScreen,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnMainMenuButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.MoveCreaturesToDifferentLayerAndDisableUi,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateBackstopMaterial,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.AddScoreLine && args.Count == 5)
        {
            AddScoreLine(
                ...
                VariantUtils.ConvertTo<string>(in args[1]),
                VariantUtils.ConvertTo<int>(in args[2]),
                VariantUtils.ConvertTo<string>(in args[3]),
                VariantUtils.ConvertTo<string>(in args[4])
            );
            ret = default(godot_variant);
            return true;
        }
        ...
        {
            ret = VariantUtils.CreateFrom<int>(
                GetScoreThreshold(VariantUtils.ConvertTo<int>(in args[0]))
            );
            return true;
        }
        ...
        if (method == MethodName.GetAscensionMulti && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<string>(
                GetAscensionMulti(VariantUtils.ConvertTo<int>(in args[0]))
            );
            return true;
        }
    ...
    [EditorBrowsable(EditorBrowsableState.Never)]
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        List<PropertyInfo> list = new List<PropertyInfo>();
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._continueButton,
                PropertyHint.None,
                "",
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._leaderboardButton,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._scoreLineContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._unlocksRemaining,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.String,
                PropertyName._scoreUnlockedEpochId,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._creatureContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._summaryContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._fullBlackBackstop,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._victoryDamageLabel,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._screenshakeContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._isAnimatingSummary,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._backstopMaterial,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName.UseSharedBackstop,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.FocusedControlFromTopBar,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._scoreUnlockedEpochId,
            Variant.From(in _scoreUnlockedEpochId)
        );
        ...
        info.AddProperty(
            PropertyName._screenshakeContainer,
            Variant.From(in _screenshakeContainer)
        );

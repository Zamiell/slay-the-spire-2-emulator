...
public class NScoreLine : Control
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/game_over_screen/score_line"
    );
    ...
    {
        NScoreLine nScoreLine = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NScoreLine>(PackedScene.GenEditState.Disabled);
    ...
    public async Task AnimateIn()
    {
        if (this.IsValid())
        {
            ...
            _tween
                .TweenProperty(this, "position:x", base.Position.X, 0.3)
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NScoreLine>(
                Create(
                    VariantUtils.ConvertTo<string>(in args[0]),
                    VariantUtils.ConvertTo<string>(in args[1]),
                    VariantUtils.ConvertTo<Texture2D>(in args[2])
                )
            );
            return true;
        }

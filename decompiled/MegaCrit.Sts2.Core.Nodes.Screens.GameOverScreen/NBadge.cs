...
public class NBadge : NButton
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/game_over_screen/badge"
    );
    ...
    public static NBadge? Create(Badge badgeModel)
    {
        ...
        NBadge nBadge = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NBadge>(PackedScene.GenEditState.Disabled);
        ...
        if (
            LocString.Exists(
                "badges",
                badgeModel.Id + "." + GetRarityPrefix(badgeModel.Rarity) + "Title"
            )
        )
        {
            nBadge._title = new LocString(
                "badges",
                badgeModel.Id + "." + GetRarityPrefix(badgeModel.Rarity) + "Title"
            );
            nBadge._description = new LocString(
                "badges",
                badgeModel.Id + "." + GetRarityPrefix(badgeModel.Rarity) + "Description"
            );
        }
        else
        {
    ...
    public static NBadge? Create(string id, BadgeRarity rarity)
    {
        ...
        nBadge.GetNode<TextureRect>("%Icon").Texture = PreloadManager.Cache.GetTexture2D(
            ImageHelper.GetImagePath("ui/game_over_screen/badge_" + id.ToLowerInvariant() + ".png")
        );
        if (LocString.Exists("badges", id + "." + GetRarityPrefix(rarity) + "Title"))
        {
            ...
            nBadge._description = new LocString(
                "badges",
                id + "." + GetRarityPrefix(rarity) + "Description"
            );
        }
        else
        {
    ...
    public async Task AnimateIn()
    {
        if (this.IsValid())
        {
            ...
            _tween
                .TweenProperty(this, "position:y", base.Position.Y, 0.25)
    ...
    protected override void OnFocus()
    {
        base.OnFocus();
        NHoverTipSet
            .CreateAndShow(_hoverNode, _hoverTip)
            ?.SetGlobalPosition(_hoverNode.GlobalPosition + new Vector2(10f, -132f));
    ...
    private static Texture2D GetBadgeBaseTexture(BadgeRarity rarity)
    {
        return rarity switch
        {
            BadgeRarity.Bronze => PreloadManager.Cache.GetTexture2D(
                ImageHelper.GetImagePath("ui/game_over_screen/badge_bronze.png")
            ),
            BadgeRarity.Silver => PreloadManager.Cache.GetTexture2D(
                ImageHelper.GetImagePath("ui/game_over_screen/badge_silver.png")
            ),
            BadgeRarity.Gold => PreloadManager.Cache.GetTexture2D(
                ImageHelper.GetImagePath("ui/game_over_screen/badge_gold.png")
            ),
            _ => PreloadManager.Cache.GetTexture2D(
                ImageHelper.GetImagePath("atlases/power_atlas.sprites/missing_power.tres")
            ),
        };
    }
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetBadgeBaseTexture,
                new PropertyInfo(
                    Variant.Type.Object,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NBadge>(
                Create(
                    VariantUtils.ConvertTo<string>(in args[0]),
                    VariantUtils.ConvertTo<BadgeRarity>(in args[1])
                )
            );
            return true;
        }
        ...
        if (method == MethodName.GetRarityPrefix && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<string>(
                GetRarityPrefix(VariantUtils.ConvertTo<BadgeRarity>(in args[0]))
            );
            return true;
        }
        ...
        if (method == MethodName.GetBadgeBaseTexture && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<Texture2D>(
                GetBadgeBaseTexture(VariantUtils.ConvertTo<BadgeRarity>(in args[0]))
            );
            return true;
        }

...
public class RunState : IRunState, ICardScope, IPlayerCollection
{
    ...
    public int TotalFloor =>
        MapPointHistory.Sum((IReadOnlyList<MapPointHistoryEntry> c) => c.Count);
    ...
    public MapPointHistoryEntry? CurrentMapPointHistoryEntry =>
        MapPointHistory.LastOrDefault()?.LastOrDefault();
    ...
    )
    {
        ...
        RunState result = CreateShared(
            ...
            runRngSet,
            odds,
            new RelicGrabBag(refreshAllowed: true),
            ascensionLevel
        );
    ...
    public static RunState FromSerializable(SerializableRun save)
    {
        ...
        RunState runState = CreateShared(
            players2,
            ...
            runRngSet,
            RunOddsSet.FromSerializable(save.SerializableOdds, runRngSet.UnknownMapPoint),
        ...
        runState._mapPointHistory.AddRange(
            new global::_003C_003Ez__ReadOnlyArray<List<MapPointHistoryEntry>>(
                save.MapPointHistory.ToArray()
            )
        );
        ...
        RunState runState = CreateShared(
            players
                ?? new global::_003C_003Ez__ReadOnlySingleElementList<Player>(
                    Player.CreateForNewRun<Deprived>(
                        MegaCrit.Sts2.Core.Unlocks.UnlockState.all,
                        1uL
                    )
                ),
            (acts ?? ActModel.GetDefaultList()).Select((ActModel a) => a.ToMutable()).ToList(),
            modifiers ?? Array.Empty<ModifierModel>(),
            gameMode,
            0,
            runRngSet,
            new RunOddsSet(runRngSet.UnknownMapPoint),
            new RelicGrabBag(refreshAllowed: true),
            ascensionLevel
        );
    ...
    )
    {
        ...
        runState.MultiplayerScalingModel = (MultiplayerScalingModel)
            ModelDb.Singleton<MultiplayerScalingModel>().MutableClone();
        runState.MultiplayerScalingModel.Initialize(runState);
        runState.BadgeModels = ModelDb
            .BadgeModels.Select((BadgeModel m) => (BadgeModel)m.MutableClone())
            .ToList();
        return runState;
    }
    ...
    public T CreateCard<T>(Player owner)
        where T : CardModel
    {
        ...
        if (card.HasBeenRemovedFromState)
        {
            if (!ContainsCard(card))
            {
                throw new InvalidOperationException(
                    $"Tried to add card {card} to RunState that has HasBeenRemovedFromState set as true, but it does not belong to this state!"
                );
            }
    ...
    )
    {
        ...
        mapPointHistoryEntry.Rooms.Add(
            new MapPointRoomHistoryEntry { RoomType = initialRoomType, ModelId = roomModelId }
        );
    ...
    {
        if (
            location.actIndex >= _mapPointHistory.Count
            || !location.coord.HasValue
    ...
    public void AddPlayerDebug(Player player, int index)
    {
        ...
        CurrentMapPointHistoryEntry.PlayerStats.Add(
            new PlayerMapPointHistoryEntry { PlayerId = player.NetId }
        );
    ...
    public void RemoveStaleVisitedMapCoords(ActMap map)
    {
        ...
        {
            Log.Error(
                $"Removed {num} stale visited map coord(s) that don't exist in the current map"
            );
        }
    }
        ...
        if (!(model is RelicModel relicModel))
        {
            if (!(model is PotionModel potionModel))
            {
                if (!(model is CardModel cardModel))
                {
                            ...
                            if (!(model is BadgeModel))
                            {
                                if (!(model is ModifierModel))
                                {
                                    ...
                                    throw new ArgumentOutOfRangeException(
                                        "model",
                                        model,
                                        $"Invalid model type {model.GetType()} ({model})"
                                    );
                                }
                                return true;
                            }
                    ...
                    return enchantmentModel.HasCard
                        && !enchantmentModel.Card.HasBeenRemovedFromState
                        && enchantmentModel.Card.Owner.IsActiveForHooks;
                }

...
public record CardCreationOptions
{
    ...
    public IEnumerable<CardModel> GetPossibleCards(Player player)
    {
        IEnumerable<CardModel> enumerable = (
            (CardPools.Count <= 0)
                ? CustomCardPool
                : (
                    from c in CardPools.SelectMany(
                        (CardPoolModel p) =>
                            p.GetUnlockedCards(
                                player.UnlockState,
        ...
        if (enumerable == null)
        {
            throw new InvalidOperationException(
                "Tried to get possible cards from CardCreationOptions but neither the pool nor custom pool were set!"
            );
        }
        return enumerable;
    }

    public CardCreationOptions WithCustomPool(
        IEnumerable<CardModel> customPool,
        CardRarityOddsType? rarityOdds = null
    )
    {
    ...
    public CardCreationOptions WithCardPools(
    ...
    private void AssertUniformOddsIfSingleRarityPool()
    {
        if (CustomCardPool != null && RarityOdds != CardRarityOddsType.Uniform)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"You have passed a custom card pool with only one rarity to {"CardCreationOptions"} and a rarity odds of {RarityOdds}! This is invalid - card pools with only one rarity must use Uniform rarity odds."
                );
            }
        }
    }
                            ...
                            (
                                (
                                    EqualityComparer<Type>.Default.GetHashCode(EqualityContract)
                                        * -1521134295
                                    + EqualityComparer<List<CardPoolModel>>.Default.GetHashCode(
                                        _cardPools
                                    )
                                ) * -1521134295
                                + EqualityComparer<Func<CardModel, bool>>.Default.GetHashCode(
                                    CardPoolFilter
                                )
                            ) * -1521134295
                            + EqualityComparer<IEnumerable<CardModel>>.Default.GetHashCode(
                                CustomCardPool
                            )
                        ) * -1521134295
                        + EqualityComparer<CardCreationSource>.Default.GetHashCode(Source)
                    ) * -1521134295
                    + EqualityComparer<CardRarityOddsType>.Default.GetHashCode(RarityOdds)
                ) * -1521134295
                + EqualityComparer<CardCreationFlags>.Default.GetHashCode(Flags)
            ) * -1521134295
            + EqualityComparer<Rng>.Default.GetHashCode(RngOverride);
    }
    ...
    public virtual bool Equals(CardCreationOptions? other)
    {
        if ((object)this != other)
        {
            if (
                ...
                && EqualityComparer<List<CardPoolModel>>.Default.Equals(
                    _cardPools,
                    other._cardPools
                )
                && EqualityComparer<Func<CardModel, bool>>.Default.Equals(
                    CardPoolFilter,
                    other.CardPoolFilter
                )
                && EqualityComparer<IEnumerable<CardModel>>.Default.Equals(
                    CustomCardPool,
                    other.CustomCardPool
                )
                && EqualityComparer<CardCreationSource>.Default.Equals(Source, other.Source)
                && EqualityComparer<CardRarityOddsType>.Default.Equals(RarityOdds, other.RarityOdds)
                && EqualityComparer<CardCreationFlags>.Default.Equals(Flags, other.Flags)
            )
            {

...
public static class ScoreUtility
{
    ...
    {
        List<MapPointRoomHistoryEntry> list = history
            .SelectMany((IReadOnlyList<MapPointHistoryEntry> actEntries) => actEntries)
            .SelectMany((MapPointHistoryEntry e) => e.Rooms)
            .ToList();
    ...
    )
    {
        return history
                .SelectMany((IReadOnlyList<MapPointHistoryEntry> actEntries) => actEntries)
                .SelectMany((MapPointHistoryEntry e) => e.PlayerStats)
                .Sum((PlayerMapPointHistoryEntry p) => p.GoldGained) / (100 * playerCount);
    }
    ...
    public static List<Badge> GetBadges(SerializableRun run, ulong playerId, bool won)
    {
        ...
        {
            if (
                (!item.RequiresWin || won)
                && (!item.MultiplayerOnly || run.Players.Count != 1)
                && item.IsObtained()
            )
            {
                list.Add(item);
    ...
    public static DecodedDailyScore DecodeDailyScore(int encodedScore)
    {
        ...
        bool isValid =
            flag && num2 >= 0 && num2 <= 99 && num3 >= 0 && num3 <= 99 && num4 >= 0 && num4 <= 9999;

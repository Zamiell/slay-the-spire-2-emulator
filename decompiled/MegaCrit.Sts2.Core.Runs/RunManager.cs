...
public class RunManager : IRunLobbyListener
{
    ...
    public void SetUpNewSinglePlayer(
        RunState state,
        bool shouldSave,
    ...
    public void SetUpReplay(RunState state, CombatReplay replay)
    {
        ...
        InitializeShared(
            ...
            new PeerInputSynchronizer(netService),
            shouldSave: true,
            serializableRun.DailyTime,
            serializableRun.StartTime,
            serializableRun.RunTime,
            serializableRun.WinTime,
            serializableRun.NumReloads
        );
        InitializeRunLobby(netService, state);
    ...
    )
    {
        ...
        ActionQueueSynchronizer = new ActionQueueSynchronizer(
            State,
            ActionQueueSet,
            RunLocationTargetedBuffer,
            NetService
        );
        PlayerChoiceSynchronizer = new PlayerChoiceSynchronizer(NetService, State);
        MapSelectionSynchronizer = new MapSelectionSynchronizer(
            NetService,
        ...
        RewardSynchronizer = new RewardSynchronizer(
            RunLocationTargetedBuffer,
            NetService,
            State,
            netId
        );
        RewardsSetSynchronizer = new RewardsSetSynchronizer(
            RunLocationTargetedBuffer,
            NetService,
            State,
            netId
        );
        RestSiteSynchronizer = new RestSiteSynchronizer(
            RunLocationTargetedBuffer,
            NetService,
            State,
            netId
        );
        OneOffSynchronizer = new OneOffSynchronizer(
            RunLocationTargetedBuffer,
            NetService,
            State,
            netId
        ...
        CombatReplayWriter = new CombatReplayWriter(
    ...
    private void InitializeRunLobby(INetGameService netService, RunState state)
    {
        ...
        {
            RunLobby = new RunLobby(
                state.GameMode,
                netService,
                this,
                state,
                state.Players.Select((Player p) => p.NetId)
            );
    ...
    {
        State.SharedRelicGrabBag.Populate(
            ...
            if (savedMap != null)
            {
                if (SavedMapsToLoad == null)
                {
                    Dictionary<int, SerializableActMap> dictionary = (
                        SavedMapsToLoad = new Dictionary<int, SerializableActMap>()
                    );
                }
    ...
    public static SerializableRun CanonicalizeSave(SerializableRun save, ulong localPlayerId)
    {
        if (save.Players.FirstOrDefault((SerializablePlayer p) => p.NetId == localPlayerId) == null)
        {
            throw new InvalidOperationException(
                $"Save is invalid! Players does not contain local player Id. IDs in save file: {string.Join(",", save.Players.Select((SerializablePlayer p) => p.NetId))}. Local ID: {localPlayerId}."
            );
        }
        ...
        SerializableRun serializableRun = new SerializableRun
        {
            ...
            Acts = runState
                .Acts.Zip(
                    save.Acts,
                    delegate(ActModel act, SerializableActModel savedAct)
                    {
            ...
            MapPointHistory = runState
                .MapPointHistory.Select((IReadOnlyList<MapPointHistoryEntry> l) => l.ToList())
                .ToList(),
    ...
    )
    {
        ...
        if (
            (previousMapPointEntry != null && previousMapPointEntry.HasRoomOfType(RoomType.Shop))
            || (
                nextMapPoints.Count > 0
                && nextMapPoints.All((MapPoint p) => p.PointType == MapPointType.Shop)
            )
        )
        {
    ...
    public void GenerateRooms()
    {
        List<AncientEventModel> list = State
            .UnlockState.SharedAncients.ToList()
    ...
    public async Task GenerateMap()
    {
        ...
        if (
            SavedMapsToLoad != null
            && SavedMapsToLoad.TryGetValue(State.CurrentActIndex, out SerializableActMap value)
        )
        {
    ...
    public async Task LoadIntoLatestMapCoord(AbstractRoom? preFinishedRoom)
    {
        if (State != null)
        {
            if (State.VisitedMapCoords.Count > 0)
            {
                ...
                await runManager.EnterMapCoordInternal(
                    visitedMapCoords[visitedMapCoords.Count - 1],
                    preFinishedRoom,
                    saveGame: false
                );
            }
    ...
    )
    {
        ...
        using (new NetLoadingHandle(NetService))
        {
            ...
            {
                HashSet<RoomType> blacklist = BuildRoomTypeBlacklist(
            ...
            AbstractRoom abstractRoom = (
                (preFinishedRoom == null) ? CreateRoom(roomType, pointType) : preFinishedRoom
            );
            ActionExecutor.Pause();
            if (preFinishedRoom == null)
            {
                State.AppendToMapPointHistory(
                    pointType,
                    abstractRoom.RoomType,
                    abstractRoom.ModelId
                );
            }
            if (
                abstractRoom is CombatRoom
                {
                    IsPreFinished: not false,
                    ParentEventId: not null
                } combatRoom
            )
            {
                EventRoom room = new EventRoom(
                    ModelDb.GetById<EventModel>(combatRoom.ParentEventId)
                );
                    ...
                    (model as EventModel)
                        ?? (
                            (mapPointType == MapPointType.Ancient)
                                ? State.Act.PullAncient()
                                : State.Act.PullNextEvent(State)
                        )
                );
        ...
        if (!TestMode.IsOn)
        {
            ...
            if (state == null || state.Players.Count <= 1)
            {
                ...
                if (
                    state2 != null
                    && state2
                        .MapPointHistory.SelectMany((IReadOnlyList<MapPointHistoryEntry> l) => l)
                        .Any((MapPointHistoryEntry e) => e.MapPointType == MapPointType.Unassigned)
                )
                {
                    return false;
                }
    ...
    public async Task EnterRoomWithoutExitingCurrentRoom(AbstractRoom room, bool fadeToBlack)
    {
        ...
        using (new NetLoadingHandle(NetService))
        {
            ...
            await CombatStateSynchronizer.WaitForSync();
            State.CurrentMapPointHistoryEntry?.Rooms.Add(
                new MapPointRoomHistoryEntry { RoomType = room.RoomType, ModelId = room.ModelId }
            );
            await EnterRoomInternal(room);
    ...
    private void UpdateRichPresence()
    {
        if (!TestMode.IsOn && State != null)
        {
            PlatformUtil.SetRichPresence(
                "IN_RUN",
                NetService.GetRawLobbyIdentifier(),
                State.Players.Count
            );
            PlatformUtil.SetRichPresenceValue(
                "Character",
                LocalContext.GetMe(State).Character.Id.Entry
            );
    ...
    private async Task AbandonInternal()
    {
        ...
        {
            NErrorPopup nErrorPopup = NErrorPopup.Create(
                new NetErrorInfo(NetError.HostAbandoned, selfInitiated: false)
            );
            if (nErrorPopup != null)
            {
    ...
    private void StateDiverged(NetFullCombatState state)
    {
        ...
        {
            Log.Info(
                "Abandoning run and returning to main menu because our state diverged from host's"
            );
    ...
    public SerializableRun OnEnded(bool isVictory)
    {
        ...
        {
            MapPointRoomHistoryEntry mapPointRoomHistoryEntry =
                state.CurrentMapPointHistoryEntry.Rooms.Last();
            mapPointRoomHistoryEntry.TurnsTaken =
                me.PlayerCombatState?.TurnNumber ?? combatRoom.CombatState.RoundNumber;
        }
        ...
        if (DailyTime.HasValue)
        {
            ...
            if ((uint)(type - 1) <= 1u)
            {
                ...
                TaskHelper.RunSafely(
                    DailyRunUtility.UploadScore(DailyTime.Value, score2, serializableRun.Players)
                );
            }
    ...
    private static void CheckUpdateEnemyDiscoveryAfterLoss(Player player, ModelId monster)
    {
        ...
        EnemyStats enemyStats = (
            SaveManager.Instance.Progress.EnemyStats.TryGetValue(monster, out value) ? value : null
        );
        if (enemyStats == null)
        {

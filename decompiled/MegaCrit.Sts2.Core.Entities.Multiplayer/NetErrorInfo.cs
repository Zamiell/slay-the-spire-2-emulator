...
public readonly struct NetErrorInfo
{
    ...
    public NetErrorInfo(
        ...
        ConnectionFailureExtraInfo? extraInfo = null
    )
    {
        _reason = null;
        _steamReason = null;
    ...
    public NetErrorInfo(
        SteamDisconnectionReason steamReason,
        string? debugReason,
        bool selfInitiated
    )
    {
    ...
    public NetError GetReason()
    {
        ...
        throw new InvalidOperationException(
            "Tried to get DisconnectionReason from DisconnectionInfo without any assigned errors"
        );
    }
        ...
        if (_connectionReason.HasValue)
        {
            if (_connectionReason == ConnectionFailureReason.ModMismatch)
            {
                ...
                if (list != null && list.Count > 0)
                {
                    ...
                    locString.Add(
                        "mods",
                        string.Join(", ", _connectionExtraInfo.missingModsOnHost)
                    );
                ...
                if (list != null && list.Count > 0)
                {
                    ...
                    locString2.Add(
                        "mods",
                        string.Join(", ", _connectionExtraInfo.missingModsOnLocal)
                    );

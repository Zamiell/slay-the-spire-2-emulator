    ...
    public static NetDeckCard FromModel(CardModel card)
    {
        if (!card.IsMutable)
        {
            throw new InvalidOperationException(
                "Immutable instances of CardModel should not be serialized through NetDeckCard! Send the ModelId instead."
            );
        }
        if (card.Pile == null)
        {
            throw new InvalidOperationException(
                "Cannot use NetDeckCard to serialize a card without a pile!"
            );
        }
        if (card.Pile.Type != PileType.Deck)
        {
            throw new InvalidOperationException(
                $"Cannot use {"NetDeckCard"} to serialize card {card} that is in a non-deck pile!"
            );
        }
        return new NetDeckCard { DeckIndex = (uint)card.Pile.Cards.IndexOf(card) };
    }

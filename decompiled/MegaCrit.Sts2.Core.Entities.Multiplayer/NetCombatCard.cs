    ...
    public static NetCombatCard FromModel(CardModel card)
    {
        if (!card.IsMutable)
        {
            throw new InvalidOperationException(
                "Immutable instances of CardModel should not be serialized through NetCombatCard! Send the ModelId instead."
            );
        }
        ...
        {
            throw new InvalidOperationException(
                "Cannot use NetCombatCard to serialize a card in a non-combat pile!"
            );
        }
        return new NetCombatCard { CombatCardIndex = NetCombatCardDb.Instance.GetCardId(card) };
    }

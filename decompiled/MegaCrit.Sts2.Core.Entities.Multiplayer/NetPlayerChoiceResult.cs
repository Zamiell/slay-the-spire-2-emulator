...
public struct NetPlayerChoiceResult : IPacketSerializable
{
    ...
    public override string ToString()
    {
        return type switch
        {
            PlayerChoiceType.CanonicalCard => "NetPlayerChoiceResult canonical "
                + string.Join(",", canonicalCards ?? new List<CardModel>()),
            PlayerChoiceType.CombatCard => "NetPlayerChoiceResult combat "
                + string.Join(",", combatCards ?? new List<NetCombatCard>()),
            PlayerChoiceType.DeckCard => "NetPlayerChoiceResult deck "
                + string.Join(",", deckCards ?? new List<NetDeckCard>()),
            PlayerChoiceType.MutableCard =>
                $"{"NetPlayerChoiceResult"} mutable cards {string.Join(",", mutableCards ?? new List<SerializableCard>())}, owner: {mutableCardOwner}",
            PlayerChoiceType.Player => $"{"NetPlayerChoiceResult"} player ID {playerId}",
            PlayerChoiceType.Index => "NetPlayerChoiceResult indexes "
                + string.Join(",", indexes ?? new List<int>()),

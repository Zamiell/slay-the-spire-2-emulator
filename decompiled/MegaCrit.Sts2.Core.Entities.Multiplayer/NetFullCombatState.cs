...
public class NetFullCombatState : IPacketSerializable
{
    public struct CreatureState : IPacketSerializable
    {
        ...
        public CreatureState Anonymized()
        {
            ...
            result.playerId = (
                playerId.HasValue
                    ? new ulong?(IdAnonymizer.Anonymize(playerId.Value))
                    : ((ulong?)null)
            );
            return result;
        }
    ...
    public static NetFullCombatState FromRun(IRunState runState, GameAction? justFinishedAction)
    {
        ...
        netFullCombatState.nextChoiceIds.AddRange(
            RunManager.Instance.PlayerChoiceSynchronizer.ChoiceIds
        );
        netFullCombatState.nextRewardIds.AddRange(
            RunManager.Instance.RewardsSetSynchronizer.GetNextRewardIds()
        );
        netFullCombatState.lastExecutedHookId = (
            (justFinishedAction is GenericHookGameAction genericHookGameAction)
                ? new uint?(genericHookGameAction.HookId)
                : ((uint?)null)
        );
        netFullCombatState.lastExecutedActionId = justFinishedAction?.Id;
        IReadOnlyList<Creature> readOnlyList =
            runState.Players[0].Creature.CombatState?.Creatures ?? Array.Empty<Creature>();
        foreach (Creature item3 in readOnlyList)
        {
            ...
            {
                item.powers.Add(new PowerState { id = power.Id, amount = power.Amount });
            }
        ...
        foreach (Player player in runState.Players)
        {
            ...
            {
                item2.potions.Add(new PotionState { id = potion.Id });
            }
            foreach (RelicModel relic in player.Relics)
            {
                item2.relics.Add(new RelicState { relic = relic.ToSerializable() });
            }
    ...
    public override string ToString()
    {
        ...
        StringBuilder.AppendInterpolatedStringHandler handler =
            new StringBuilder.AppendInterpolatedStringHandler(12, 1, stringBuilder2);
        ...
        foreach (PlayerState player in Players)
        {
                ...
                foreach (CardState card in pile.cards)
                {
                    ...
                    if (card.energyCost.HasValue)
                    {
                        ...
                        handler.AppendFormatted(
                            string.Join(",", new ReadOnlySpan<object>(in reference))
                        );
            ...
            {
                if (
                    player.relicGrabBag.RelicIdLists.TryGetValue(
                        relicRarity,
                        out List<ModelId> value2
                    )
                )
                {
                    stringBuilder2 = stringBuilder;
                    ...
                    handler.AppendLiteral(": ");
                    handler.AppendFormatted(
                        string.Join(",", value2.Select((ModelId m) => m.Entry))
                    );

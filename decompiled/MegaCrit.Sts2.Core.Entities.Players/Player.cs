...
public class Player
{
    ...
    public IEnumerable<PotionModel> Potions =>
        _potionSlots.Where((PotionModel p) => p != null).OfType<PotionModel>();
    ...
    )
    {
        ...
        MaxAscensionWhenRunStarted =
            (SaveManager.Instance?.Progress.GetStatsForCharacter(Character.Id))?.MaxAscension ?? 0;
    }
    ...
    {
        Player player = new Player(
            ModelDb.GetById<CharacterModel>(save.CharacterId),
            save.NetId,
            save.CurrentHp,
            save.MaxHp,
            save.MaxEnergy,
            save.Gold,
            save.MaxPotionSlotCount,
            save.BaseOrbSlotCount,
            MegaCrit.Sts2.Core.Runs.RelicGrabBag.FromSerializable(save.RelicGrabBag),
            MegaCrit.Sts2.Core.Unlocks.UnlockState.FromSerializable(save.UnlockState),
    ...
    public void InitializeSeed(string seed)
    {
        PlayerRng = new PlayerRngSet(
            (uint)((ulong)StringHelper.GetDeterministicHashCode(seed) + NetId)
        );
        PlayerOdds = new PlayerOddsSet(PlayerRng);
    }
    ...
    private void PopulateStartingInventory()
    {
        ...
        foreach (
            PotionModel item in Character.StartingPotions.Select((PotionModel p) => p.ToMutable())
        )
        {
    ...
    private void LoadInventory(SerializablePlayer save)
    {
        ...
        {
            throw new InvalidOperationException(
                "A player's inventory must be loaded before being added to a run."
            );
        }
    ...
    public SerializablePlayer ToSerializable()
    {
        return new SerializablePlayer
        {
            ...
            Potions = PotionSlots
                .Select((PotionModel p, int i) => p?.ToSerializable(i))
    ...
    public void SyncWithSerializedPlayer(SerializablePlayer player)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to sync player that has net ID {NetId} with SerializablePlayer that has net ID {player.NetId}!"
            );
        }
        if (player.CharacterId != Character.Id)
        {
            throw new InvalidOperationException(
                $"Character changed for player {NetId}! This is not allowed"
            );
        }
        ...
        PopulateDeck(
            player.Deck.Select((SerializableCard c) => RunState.LoadCard(c, this)),
            silent: true
        );
    ...
    public PotionModel? GetPotionAtSlotIndex(int index)
    {
        if (index < 0 || index >= _potionSlots.Count)
        {
            throw new IndexOutOfRangeException(
                $"Index {index} is not a valid potion slot index! Player has {_potionSlots.Count} potion slots"
            );
        }
    ...
    public PotionProcureResult AddPotionInternal(
        PotionModel potion,
        int slotIndex = -1,
        bool silent = false
    )
    {
        ...
        PotionProcureResult potionProcureResult = new PotionProcureResult { potion = potion };
        if (slotIndex < 0)
        {
        ...
        if (slotIndex >= 0)
        {
            ...
            {
                Log.Warn(
                    $"Tried to add potion {potion} at slot index {slotIndex} which is already filled with potion {_potionSlots[slotIndex]}!"
                );
                if (!silent)
                {
    ...
    private void RemovePotionInternal(PotionModel potion)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to remove potion you don't have: {potion.Id}"
            );
        }
    ...
    {
        List<RelicModel> list = Character
            .StartingRelics.Select((RelicModel r) => r.ToMutable())
            .ToList();
    ...
    private void LoadPotions(List<SerializablePotion> serializablePotions, bool silent = false)
    {
        ...
        {
            AddPotionInternal(
                PotionModel.FromSerializable(serializablePotion),
                serializablePotion.SlotIndex,
                silent
            );
        }
    }

...
public class NRelicCollectionCategory : VBoxContainer
{
    ...
    public static readonly string scenePath = SceneHelper.GetScenePath(
        "screens/relic_collection/relic_collection_subcategory"
    );
    ...
    private readonly List<NRelicCollectionCategory> _subCategories =
        new List<NRelicCollectionCategory>();
    ...
    public Control? DefaultFocusedControl
    {
        get
        {
            ...
            if (
                GodotObject.IsInstanceValid(_relicsContainer)
                && _relicsContainer.GetChildCount() > 0
            )
            {
    ...
    private NRelicCollectionCategory CreateForSubcategory()
    {
        return PreloadManager
            ...
            .Instantiate<NRelicCollectionCategory>(PackedScene.GenEditState.Disabled);
    }
    ...
    )
    {
        ...
        _relicModelCache.AddRange(
            ModelDb.AllRelics.Where((RelicModel relic) => relic.Rarity == relicRarity)
        );
        if (relicRarity == RelicRarity.Starter)
        {
            List<RelicModel> list = ModelDb
                .AllCharacters.SelectMany((CharacterModel c) => c.StartingRelics)
                .ToList();
            ...
            nRelicCollectionCategory.LoadSubcategory(
                _collection,
                null,
                list,
                seenRelics,
            ...
            IEnumerable<RelicModel> relics = list.Select(
                (RelicModel r) => ModelDb.Relic<TouchOfOrobas>().GetUpgradedStarterRelic(r)
            );
            ...
            nRelicCollectionCategory2.LoadSubcategory(
                _collection,
                null,
                relics,
                seenRelics,
        ...
        if (relicRarity == RelicRarity.Ancient)
        {
            ...
            if (ModelDb.Acts.Except(list3).Any())
            {
                throw new InvalidOperationException(
                    "The act list in NRelicCollectionCategory is out of date!"
                );
            }
            List<AncientEventModel> list4 = list3
                .Select((ActModel a) => a.AllAncients)
                .SelectMany((IEnumerable<AncientEventModel> a) => a)
                .Concat(ModelDb.AllSharedAncients)
                .Distinct()
                .ToList();
            HashSet<AncientEventModel> hashSet = list3
                .Select((ActModel a) => a.GetUnlockedAncients(unlockState))
                .SelectMany((IEnumerable<AncientEventModel> a) => a)
                .Concat(unlockState.SharedAncients)
                .Distinct()
                .ToHashSet();
            IReadOnlyDictionary<ModelId, AncientStats> ancientStats = SaveManager
                .Instance
                .Progress
                .AncientStats;
            ...
            for (int num3 = 0; num3 < list4.Count; num3++)
            {
                ...
                if (hashSet.Contains(ancientEventModel))
                {
                    ...
                    this.MoveChildSafely(
                        nRelicCollectionCategory3,
                        _headerLabel.GetIndex() + num3 + 1
                    );
                    RelicModel[] array = ancientEventModel
                        .AllPossibleOptions.Select((EventOption o) => o.Relic?.CanonicalInstance)
                        .OfType<RelicModel>()
                        .Intersect(_relicModelCache)
                        .OrderBy<RelicModel, string>(
                            (RelicModel r) => r.Title.GetFormattedText(),
                            LocManager.Instance.StringComparer
                        )
                        .ToArray();
                    bool flag =
                        ancientStats.ContainsKey(ancientEventModel.Id)
                        || array.Any((RelicModel r) => seenRelics.Contains(r));
                    ...
                    nRelicCollectionCategory3.LoadSubcategory(
                        _collection,
                        locString2,
                        array,
                        seenRelics,
        ...
        list6.Sort(
            (RelicModel p1, RelicModel p2) =>
                LocManager.Instance.StringComparer.Compare(
                    p1.Title.GetFormattedText(),
                    p2.Title.GetFormattedText()
                )
        );
    ...
    )
    {
        ...
        foreach (RelicModel relic in relics)
        {
            ModelVisibility visibility = (
                (!unlockedRelics.Contains(relic))
                    ? ModelVisibility.Locked
                    : (
                        seenRelics.Contains(relic)
                            ? ModelVisibility.Visible
                            : ModelVisibility.NotSeen
                    )
            );
            NRelicCollectionEntry nRelicCollectionEntry = NRelicCollectionEntry.Create(
                relic,
                visibility
            );
            _relicsContainer.AddChildSafely(nRelicCollectionEntry);
            nRelicCollectionEntry.Connect(
                NClickableControl.SignalName.Released,
                Callable.From<NRelicCollectionEntry>(OnRelicEntryPressed)
            );
        }
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.CreateForSubcategory,
                new PropertyInfo(
                    Variant.Type.Object,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnRelicEntryPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,

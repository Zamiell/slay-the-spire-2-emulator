...
public class NRelicCollection : NSubmenu
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/relic_collection/relic_collection"
    );
    ...
    public static NRelicCollection? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NRelicCollection>(PackedScene.GenEditState.Disabled);
    }
    ...
    private async Task LoadRelics()
    {
        ...
        HashSet<RelicModel> seenRelics = SaveManager
            .Instance.Progress.DiscoveredRelics.Select(ModelDb.GetByIdOrNull<RelicModel>)
            .OfType<RelicModel>()
            .ToHashSet();
        ...
        _starter.LoadRelics(
            RelicRarity.Starter,
            this,
            new LocString("relic_collection", "STARTER"),
            seenRelics,
            unlockState,
            allUnlockedRelics
        );
        _common.LoadRelics(
            RelicRarity.Common,
            this,
            new LocString("relic_collection", "COMMON"),
            seenRelics,
            unlockState,
            allUnlockedRelics
        );
        _uncommon.LoadRelics(
            RelicRarity.Uncommon,
            this,
            new LocString("relic_collection", "UNCOMMON"),
            seenRelics,
            unlockState,
            allUnlockedRelics
        );
        _rare.LoadRelics(
            RelicRarity.Rare,
            this,
            new LocString("relic_collection", "RARE"),
            seenRelics,
            unlockState,
            allUnlockedRelics
        );
        _shop.LoadRelics(
            RelicRarity.Shop,
            this,
            new LocString("relic_collection", "SHOP"),
            seenRelics,
            unlockState,
            allUnlockedRelics
        );
        _ancient.LoadRelics(
            RelicRarity.Ancient,
            this,
            new LocString("relic_collection", "ANCIENT"),
            seenRelics,
            unlockState,
            allUnlockedRelics
        );
        _event.LoadRelics(
            RelicRarity.Event,
            this,
            new LocString("relic_collection", "EVENT"),
            seenRelics,
            unlockState,
            allUnlockedRelics
        );
        ...
        for (int i = 0; i < list.Count; i++)
        {
            for (int j = 0; j < list[i].Count; j++)
            {
                ...
                control.FocusNeighborRight = (
                    (j < list[i].Count - 1) ? list[i][j + 1].GetPath() : list[i][0].GetPath()
                );
                if (i > 0)
                {
                    control.FocusNeighborTop = (
                        (j < list[i - 1].Count)
                            ? list[i - 1][j].GetPath()
                            : list[i - 1][list[i - 1].Count - 1].GetPath()
                    );
                }
                else
                {
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetLastFocusedRelic,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,

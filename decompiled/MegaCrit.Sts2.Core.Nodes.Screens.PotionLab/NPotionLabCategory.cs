...
public class NPotionLabCategory : VBoxContainer
{
    ...
    )
    {
        ...
        IEnumerable<PotionModel> enumerable = ModelDb.AllPotions.Where(
            (PotionModel relic) => relic.Rarity == potionRarity || relic.Rarity == secondRarity
        );
        ...
        foreach (PotionModel item3 in list2.Concat(list))
        {
            ModelVisibility visibility = (
                (!allUnlockedPotions.Contains(item3))
                    ? ModelVisibility.Locked
                    : (
                        seenPotions.Contains(item3)

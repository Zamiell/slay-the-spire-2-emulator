...
{
    public static readonly UnlockState none = new UnlockState(
        Array.Empty<string>(),
        Array.Empty<ModelId>(),
        0
    );

    public static readonly UnlockState all = new UnlockState(
        EpochModel.AllEpochIds,
        ModelDb.AllEncounters.Select((EncounterModel e) => e.Id),
        999999999
    );
    ...
    public IEnumerable<RelicModel> Relics =>
        ModelDb
            .AllRelicPools.Select((RelicPoolModel p) => p.GetUnlockedRelics(this))
            .SelectMany((IEnumerable<RelicModel> r) => r);

    public IEnumerable<PotionModel> Potions =>
        ModelDb
            .AllPotionPools.Select((PotionPoolModel p) => p.GetUnlockedPotions(this))
            .SelectMany((IEnumerable<PotionModel> r) => r);

    public IEnumerable<CardPoolModel> CharacterCardPools =>
        Characters.Select((CharacterModel c) => c.CardPool);

    public IEnumerable<CardModel> Cards =>
        CardPools
            .SelectMany((CardPoolModel p) => p.AllCards)
            .Concat(Characters.SelectMany((CharacterModel c) => c.StartingDeck).Distinct())
            .Distinct();

    public IEnumerable<CardPoolModel> CardPools =>
        CharacterCardPools.Concat(ModelDb.AllSharedCardPools).Distinct();
    ...
    {
        _unlockedEpochIds = (
            from e in progress.Epochs
            where e.State == EpochState.Revealed
            select e.Id
        ).ToHashSet();
        _encountersSeen = progress
            .EncounterStats.Keys.Where(
                (ModelId id) => ModelDb.GetByIdOrNull<AbstractModel>(id) is EncounterModel
            )
            .ToHashSet();
    ...
    public UnlockState(IEnumerable<UnlockState> unlockStatesEnumerable)
    {
        ...
        _unlockedEpochIds = source
            .Select((UnlockState s) => s._unlockedEpochIds)
            .SelectMany((HashSet<string> m) => m)
            .Distinct()
            .ToHashSet();
        _encountersSeen = source
            .Select((UnlockState s) => s._encountersSeen)
            .SelectMany((HashSet<ModelId> b) => b)
            .Distinct()
            .ToHashSet();
    ...
    public static UnlockState FromSerializable(SerializableUnlockState unlockState)
    {
        ...
        List<ModelId> encountersSeen = (
            from e in unlockState.EncountersSeen.Select(SaveUtil.EncounterOrDeprecated)
            select e.Id
        )
            .ToHashSet()
            .ToList();
        return new UnlockState(
            unlockState.UnlockedEpochs,
            encountersSeen,
            unlockState.NumberOfRuns
        );
    }
}

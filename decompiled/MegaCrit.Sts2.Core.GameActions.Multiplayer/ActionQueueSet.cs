...
public class ActionQueueSet
{
    ...
    private readonly List<ActionWaitingForResumption> _actionsWaitingForResumption =
        new List<ActionWaitingForResumption>();
    ...
    public ActionQueueSet(IReadOnlyList<Player> players)
    {
        ...
        {
            _actionQueues.Add(
                new ActionQueue { actions = new List<GameAction>(), ownerId = player.NetId }
            );
        }
    }
    ...
    public void EnqueueWithoutSynchronizing(GameAction gameAction)
    {
        ...
        if (gameAction.Id.HasValue)
        {
            throw new InvalidOperationException(
                $"Attempting to enqueue GameAction {gameAction} which already has an ID {gameAction.Id}, indicating it was previously enqueued to the queue!"
            );
        }
        ...
        {
            _logger.Debug(
                $"Attempted to enqueue PlayCardAction {gameAction} to player queue owned by {gameAction.OwnerId}, but it's currently cancelling all play card actions due to player choice"
            );
            gameAction.Cancel();
            return;
        }
        if (
            queue.isCancellingPlayerDrivenCombatActions
            && IsGameActionPlayerDriven(gameAction)
            && gameAction.ActionType != GameActionType.NonCombat
            && gameAction.ActionType != GameActionType.Any
        )
        {
            _logger.Debug(
                $"Attempted to enqueue GameAction {gameAction} to player queue owned by {gameAction.OwnerId}, but it's currently cancelling all non-hook actions due to end of turn"
            );
            gameAction.Cancel();
            return;
        }
        ...
        {
            _logger.Debug(
                $"Attempted to enqueue GameAction {gameAction} to player queue owned by {gameAction.OwnerId}, but it's currently cancelling all combat actions"
            );
            gameAction.Cancel();
        }
        else
        {
            _logger.Debug(
                $"Enqueueing action {gameAction} to player queue owned by {gameAction.OwnerId}"
            );
    ...
    public GameAction? GetReadyAction()
    {
        ...
        foreach (ActionQueue actionQueue in _actionQueues)
        {
            ...
            while (
                actionQueue.actions.Count > 0
                && actionQueue.actions[0].State == GameActionState.Canceled
            )
            {
                _logger.Warn(
                    $"Removing canceled action {actionQueue.actions[0]} from front of player queue {actionQueue.ownerId}"
                );
            ...
            {
                _logger.VeryDebug(
                    $"We are currently in combat and candidate action {gameAction2} has type {gameAction2.ActionType}"
                );
                continue;
            }
            if (
                actionQueue.isPaused
                && gameAction2.ActionType == GameActionType.CombatPlayPhaseOnly
            )
            {
                _logger.VeryDebug(
                    $"Queue for player {actionQueue.ownerId} is paused and candidate action {gameAction2} has type {gameAction2.ActionType}"
                );
                continue;
            }
            ...
            {
                _logger.VeryDebug(
                    $"Action {gameAction2} at front of player queue {actionQueue.ownerId} is waiting for player choice"
                );
                continue;
            }
            if (
                gameAction2.State != GameActionState.WaitingForExecution
                && gameAction2.State != GameActionState.ReadyToResumeExecuting
            )
            {
                throw new InvalidOperationException(
                    $"GameAction {gameAction2} at the front of player action queue {actionQueue.ownerId} is in invalid state {gameAction2.State}!"
                );
            }
            if (gameAction == null || gameAction2.Id < gameAction.Id)
            {
                _logger.VeryDebug(
                    $"Action {gameAction2} with id {gameAction2.Id.Value} belonging to {actionQueue.ownerId} becomes new ready action"
                );
            ...
            {
                _logger.VeryDebug(
                    $"Action {gameAction2} has id {gameAction2.Id.Value} greater than current ready action {gameAction} with ID {gameAction.Id.Value}"
                );
            }
        }
        if (gameAction != null)
    ...
    public void PauseActionForPlayerChoice(GameAction action, PlayerChoiceOptions options)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Attempting to pause action {action} that is not at the front of the owner {action.Id}'s queue!"
            );
        }
        ...
        {
            CancelNonExecutingActionsOfType<PlayCardAction>(
                action.OwnerId,
                actionWaitingForResumption?.newId
            );
        ...
        foreach (ActionQueue actionQueue in _actionQueues)
        {
            ...
            for (int i = 0; i < actionQueue.actions.Count; i++)
            {
                ...
                if (
                    ...
                    && gameAction.ActionType != GameActionType.NonCombat
                    && gameAction.ActionType != GameActionType.Any
                    && gameAction.State == GameActionState.WaitingForExecution
                )
                {
    ...
    public void CombatEnded()
    {
        ...
        foreach (ActionQueue actionQueue in _actionQueues)
        {
            for (int i = 0; i < actionQueue.actions.Count; i++)
            {
                ...
                {
                    _logger.VeryDebug(
                        $"Not cancelling action {gameAction}, type: {gameAction.ActionType}, state: {gameAction.State}"
                    );
                }
            }
    ...
    private void CancelNonExecutingActionsOfType<T>(ulong ownerId, uint? maxActionId)
        where T : GameAction
    {
        ...
        foreach (ActionQueue actionQueue in _actionQueues)
        {
            ...
            for (int i = 0; i < actionQueue.actions.Count; i++)
            {
                ...
                if (
                    gameAction is T
                    && gameAction.State == GameActionState.WaitingForExecution
                    && (!maxActionId.HasValue || !(gameAction.Id.Value >= maxActionId))
                )
                {
    ...
    public void ResumeActionWithoutSynchronizing(uint id)
    {
        ...
        if (
            TryGetAction(id, out GameAction gameAction, out ActionQueue queue)
            && gameAction.State == GameActionState.GatheringPlayerChoice
        )
        {
    ...
    {
        ActionQueue actionQueue = _actionQueues.FirstOrDefault(
            (ActionQueue q) => q.ownerId == playerId
        );
        if (actionQueue == null)
        {
            throw new InvalidOperationException(
                $"Tried to get local action queue for nonexistent player with ID {playerId}!"
            );
        }
        return actionQueue;
    }
    ...
    private void PopAction(GameAction action)
    {
        ...
        foreach (ActionQueue actionQueue in _actionQueues)
        {
            ...
            foreach (GameAction action2 in actionQueue.actions)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Tried to pop action {action}, but it is not the top-most action for player {actionQueue.ownerId}!"
                    );
                }
            }
        }
        ...
        throw new InvalidOperationException(
            $"Tried to pop action {action}, but we didn't find it in any queue!"
        );
    }

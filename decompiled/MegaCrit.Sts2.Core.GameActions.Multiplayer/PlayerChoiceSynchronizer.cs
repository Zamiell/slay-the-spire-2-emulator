...
public class PlayerChoiceSynchronizer : IDisposable
{
    ...
    public uint ReserveChoiceId(Player player)
    {
        ...
        _logger.VeryDebug(
            $"Reserved choice id {num} for player {player.NetId}, next is {_choiceIds[playerSlotIndex]}"
        );
        return num;
    }
    ...
    public void SyncLocalChoice(Player player, uint choiceId, PlayerChoiceResult result)
    {
        if (!ValidateChoiceId(player, choiceId))
        {
            throw new InvalidOperationException(
                $"Tried to sync local choice with ID {choiceId} for player {player.NetId}, but player's choice ID is {GetChoiceId(player)}!"
            );
        }
        ...
        _logger.Debug(
            $"Sending player choice id {choiceId} for player {player.NetId}, result {result}"
        );
    ...
    public async Task<PlayerChoiceResult> WaitForRemoteChoice(Player player, uint choiceId)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to wait for remote choice with ID {choiceId} for player {player.NetId}, but player's choice ID is {GetChoiceId(player)}!"
            );
        }
        int num = _receivedChoices.FindIndex(
            (ReceivedChoice c) => c.choiceId == choiceId && c.senderId == player.NetId
        );
        ReceivedChoice item;
        if (num >= 0)
        {
            _logger.Debug(
                $"Was going to wait for remote choice {choiceId} for player {player.NetId} but we've already received it"
            );
        ...
        PlayerChoiceResult playerChoiceResult = PlayerChoiceResult.FromNetData(
            player,
            _players,
            netData
        );
        _logger.Debug(
            $"Finished waiting for remote choice {choiceId} for player {player.NetId}: {playerChoiceResult}"
        );
    ...
    {
        _logger.Debug(
            $"Received choice from {senderId} for choice ID {message.choiceId}: {message.result}"
        );

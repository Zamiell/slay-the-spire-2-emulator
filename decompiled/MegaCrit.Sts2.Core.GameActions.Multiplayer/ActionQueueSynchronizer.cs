...
public class ActionQueueSynchronizer
{
    ...
    )
    {
        ...
        _messageBuffer.RegisterMessageHandler<RequestEnqueueActionMessage>(
            HandleRequestEnqueueActionMessage
        );
        _messageBuffer.RegisterMessageHandler<ActionEnqueuedMessage>(HandleActionEnqueuedMessage);
        _messageBuffer.RegisterMessageHandler<RequestEnqueueHookActionMessage>(
            HandleRequestEnqueueHookActionMessage
        );
        _messageBuffer.RegisterMessageHandler<HookActionEnqueuedMessage>(
            HandleHookActionEnqueuedMessage
        );
        _messageBuffer.RegisterMessageHandler<RequestResumeActionAfterPlayerChoiceMessage>(
            HandleRequestResumeActionAfterPlayerChoiceMessage
        );
        _messageBuffer.RegisterMessageHandler<ResumeActionAfterPlayerChoiceMessage>(
            HandleResumeActionAfterPlayerChoiceMessage
        );
    ...
    public void Dispose()
    {
        _messageBuffer.UnregisterMessageHandler<RequestEnqueueActionMessage>(
            HandleRequestEnqueueActionMessage
        );
        _messageBuffer.UnregisterMessageHandler<ActionEnqueuedMessage>(HandleActionEnqueuedMessage);
        _messageBuffer.UnregisterMessageHandler<RequestEnqueueHookActionMessage>(
            HandleRequestEnqueueHookActionMessage
        );
        _messageBuffer.UnregisterMessageHandler<HookActionEnqueuedMessage>(
            HandleHookActionEnqueuedMessage
        );
        _messageBuffer.UnregisterMessageHandler<RequestResumeActionAfterPlayerChoiceMessage>(
            HandleRequestResumeActionAfterPlayerChoiceMessage
        );
        _messageBuffer.UnregisterMessageHandler<ResumeActionAfterPlayerChoiceMessage>(
            HandleResumeActionAfterPlayerChoiceMessage
        );
    }
    ...
    public void SetCombatState(ActionSynchronizerCombatState combatState)
    {
        ...
        {
            _logger.Debug(
                $"Combat state was previously {combatState2}. Signaling queues that combat has begun"
            );
        ...
        switch (combatState)
        {
            case ActionSynchronizerCombatState.NotInCombat:
                _logger.Debug(
                    $"Combat state becomes {combatState}. Cancelling deferred actions and cancelling all remaining combat actions"
                );
            ...
            case ActionSynchronizerCombatState.PlayPhase:
                _logger.Debug(
                    $"Combat state becomes {combatState}. Requesting {_requestedActionsWaitingForPlayerTurn.Count} actions and unpausing action queues"
                );
            ...
            case ActionSynchronizerCombatState.EndTurnPhaseOne:
                _logger.Debug(
                    $"Combat state becomes {combatState} (from {combatState2}). Starting to cancel all player-driven actions"
                );
    ...
    {
        if (
            action.ActionType == GameActionType.CombatPlayPhaseOnly
            && CombatState == ActionSynchronizerCombatState.NotPlayPhase
        )
        {
            _logger.Debug(
                $"Attempted to request enqueue of action {action} during the enemy turn. Deferring the action until the player turn"
            );
        ...
        switch (_netService.Type)
        {
            ...
            {
                _logger.Debug(
                    $"Sending message to host requesting to enqueue action {action} at {Log.Timestamp}"
                );
    ...
    {
        GenericHookGameAction hookActionForId = GetHookActionForId(
            _nextHookId,
            ownerId,
            gameActionType
        );
    ...
    public void RequestEnqueueHookAction(GenericHookGameAction action)
    {
        switch (_netService.Type)
        {
            ...
            {
                _logger.Debug(
                    $"Sending message to host requesting to enqueue HOOK action with id {action.HookId} at {Log.Timestamp}"
                );
    ...
    public void RequestResumeActionAfterPlayerChoice(GameAction action)
    {
        switch (_netService.Type)
        {
            ...
            {
                _logger.Debug(
                    $"Sending message to host requesting resumption of action {action}, id {action.Id}, at {Log.Timestamp}"
                );
                RequestResumeActionAfterPlayerChoiceMessage message =
                    new RequestResumeActionAfterPlayerChoiceMessage
                    {
    ...
    private void EnqueueAction(GameAction action, ulong actionOwnerId)
    {
        ...
        {
            _logger.Debug(
                $"Sending message to clients to enqueue action {action} at {Log.Timestamp}"
            );
    ...
    private void EnqueueHookAction(GenericHookGameAction gameAction)
    {
        ...
        {
            _logger.Debug(
                $"Sending message to clients to enqueue hook action {gameAction} at {Log.Timestamp}"
            );
    ...
    private void ResumeActionAfterPlayerChoice(uint id)
    {
        ...
        {
            _logger.Debug(
                $"Sending message to clients to resume action id {id} at {Log.Timestamp}"
            );
    ...
    )
    {
        ...
        _logger.Debug(
            $"Received request enqueue action for action {message.action} ({senderId}) at {Log.Timestamp}"
        );
    ...
    private void HandleActionEnqueuedMessage(ActionEnqueuedMessage message, ulong _)
    {
        if (_netService.Type != NetGameType.Client)
        {
            throw new InvalidOperationException(
                "Received action enqueued message while not client!"
            );
        }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                "Received hook action enqueue request while not host!"
            );
        }
        _logger.Debug(
            $"Received HOOK request enqueue action from {senderId}, hook id: {message.hookActionId}, at {Log.Timestamp}"
        );
        GenericHookGameAction hookActionForId = GetHookActionForId(
    ...
    private void HandleHookActionEnqueuedMessage(HookActionEnqueuedMessage message, ulong _)
    {
        ...
        {
            throw new InvalidOperationException(
                "Received hook action enqueued message while not host!"
            );
        }
        _logger.Debug(
            $"Received HOOK request enqueue action, hook id: {message.hookActionId}, at {Log.Timestamp}"
        );
        GenericHookGameAction hookActionForId = GetHookActionForId(
            message.hookActionId,
            message.ownerId,
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                "Received action ready request message while not host!"
            );
        }
        _logger.Debug(
            $"Received action ready request message for action {afterPlayerChoiceMessage.actionId}, sender {senderId}, at {Log.Timestamp}"
        );
    ...
    )
    {
        ...
        _logger.Debug(
            $"Received action ready message for action {afterPlayerChoiceMessage.actionId} at {Log.Timestamp}"
        );
        ResumeActionAfterPlayerChoice(afterPlayerChoiceMessage.actionId);
    }
    ...
    private GameAction NetActionToGameAction(INetAction action, ulong actionOwnerId)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Action owner ID {actionOwnerId} for action {action} could not be mapped to a Player!"
            );
        }
    ...
    {
        GenericHookGameAction action = _hookActions.Find(
            (GenericHookGameAction a) => a.HookId == id
        );
        if (action == null)
        {
            action = new GenericHookGameAction(id, ownerId, gameActionType);
            action.ExecutionStartedTask.ContinueWith(
                delegate
                {
        ...
        else
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Attempted to get hook for owner {ownerId} with hook ID {id}, but the hook already existed and had owner {action.OwnerId}!"
                );
            }
            if (action.ActionType != gameActionType)
            {
                throw new InvalidOperationException(
                    $"Generating GenericHookGameAction with type {gameActionType}. Found one already enqueued, but with a mismatching game action type {action.ActionType}!"
                );
            }
        }
        return action;

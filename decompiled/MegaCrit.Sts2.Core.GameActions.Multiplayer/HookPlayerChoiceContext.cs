...
public class HookPlayerChoiceContext : PlayerChoiceContext
{
    ...
    private readonly TaskCompletionSource _taskAssignedCompletionSource =
        new TaskCompletionSource();

    private readonly TaskCompletionSource _pausedBeforeTaskAssignedCompletionSource =
        new TaskCompletionSource();
    ...
    private ActionQueueSynchronizer ActionQueueSynchronizer =>
        _actionQueueSynchronizer ?? RunManager.Instance.ActionQueueSynchronizer;
    ...
    )
    {
        ...
        Owner = (
            (source2 is CardModel cardModel)
                ? cardModel.Owner
                : (
                    (source2 is RelicModel relicModel)
                        ? relicModel.Owner
                        : (
                            (source2 is PotionModel potionModel)
                                ? potionModel.Owner
                                : (
                                    (source2 is AfflictionModel afflictionModel)
                                        ? afflictionModel.Card.Owner
                                        : (
                                            (!(source2 is EnchantmentModel enchantmentModel))
                                                ? null
                                                : enchantmentModel.Card.Owner
                                        )
                                )
                        )
                )
    ...
    public override async Task SignalPlayerChoiceBegun(PlayerChoiceOptions options)
    {
        if (_gameAction != null)
        {
            ...
            {
                Log.Error(
                    $"Tried to interrupt action {_gameAction} but the currently running action is {ActionExecutor.CurrentlyRunningAction}!"
                );
                return;
            }
        }
        else
        {
            if (Owner == null)
            {
                throw new InvalidOperationException(
                    $"HookPlayerChoiceContext is assigned a model {Source} with no owner, but the model has requested a player choice! This is not supported"
                );
            }
            ...
            if (Task == null)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "HookPlayerChoiceContext was never passed a task to await!"
                    );
                }
            }
    ...
    public async Task WaitForCompletion()
    {
        ...
        {
            throw new InvalidOperationException(
                "Tried to call WaitForCompletion before AssignTaskAndWaitForPauseOrCompletion was called!"
            );
        }
        await Task;
        if (GameAction != null)

...
public class NetCombatCardDb
{
    ...
    public void StartCombat(IReadOnlyList<Player> players)
    {
        ...
        foreach (Player player in players)
        {
            ...
            foreach (
                CardModel item in player.PlayerCombatState.AllPiles.SelectMany(
                    (CardPile p) => p.Cards
                )
            )
            {
    ...
    public uint GetCardId(CardModel card)
    {
        if (!TryGetCardId(card, out var id))
        {
            throw new InvalidOperationException(
                $"Card {card} could not be found in combat ID database!"
            );
        }
        return id;
    }
    ...
    public bool TryGetCardId(CardModel card, out uint id)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get ID for canonical card {card}! Use the ModelId instead"
            );
        }
    ...
    private void IdCardIfNecessary(CardModel card)
    {
        if (!_cardToId.ContainsKey(card))
        {
            ...
            Log.LogMessage(
                LogLevel.Debug,
                LogType.Network,
                $"ID card {card} owned by {card.Owner.NetId} in pile {card.Pile?.Type} with id: {_nextId}"
            );

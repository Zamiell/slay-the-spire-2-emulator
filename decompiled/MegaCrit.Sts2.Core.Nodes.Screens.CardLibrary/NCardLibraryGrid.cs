...
public class NCardLibraryGrid : NCardGrid
{
    ...
    public new class MethodName : NCardGrid.MethodName
    {
        ...
        public static new readonly StringName InitGrid = "InitGrid";

        public static new readonly StringName UpdateGridNavigation = "UpdateGridNavigation";
    }

    public new class PropertyName : NCardGrid.PropertyName
    {
        public static new readonly StringName IsCardLibrary = "IsCardLibrary";

        public static new readonly StringName CenterGrid = "CenterGrid";
    ...
    public void RefreshVisibility()
    {
        ...
        _unlockedCards = ModelDb
            .AllCardPools.Select(
                (CardPoolModel p) => p.GetUnlockedCards(unlockState, CardMultiplayerConstraint.None)
            )
            .SelectMany((IEnumerable<CardModel> c) => c)
            .ToHashSet();
    }
        ...
        for (int i = 0; i < _cardRows.Count; i++)
        {
            for (int j = 0; j < _cardRows[i].Count; j++)
            {
                ...
                nCardHolder.FocusNeighborRight = (
                    (j < _cardRows[i].Count - 1) ? _cardRows[i][j + 1].GetPath() : null
                );

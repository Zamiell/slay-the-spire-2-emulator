...
public class NCardLibraryStats : Control
{
    ...
    private static string ScenePath =>
        SceneHelper.GetScenePath("screens/card_library/card_library_stats");

    public static NCardLibraryStats Create()
    {
        return PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NCardLibraryStats>(PackedScene.GenEditState.Disabled);
    }
    ...
    public void UpdateStats(CardModel card)
    {
        ...
        CardStats value;
        long num = (
            (!SaveManager.Instance.Progress.CardStats.TryGetValue(card.Id, out value))
                ? 0
                : value.TimesWon
        );

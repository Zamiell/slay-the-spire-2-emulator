...
public class CardReward : Reward
{
    ...
    private static string UncommonRewardIcon =>
        ImageHelper.GetImagePath("ui/reward_screen/reward_icon_uncommon.png");
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlyArray<string>(
            new string[3] { RareRewardIcon, UncommonRewardIcon, RewardIcon }
        );
        ...
        if (_cardsWereManuallySet && !_hasBeenRerolled)
        {
            ...
            )
            {
                TaskHelper.RunSafely(
                    Hook.AfterModifyingCardRewardOptions(base.Player.RunState, modifiers)
                );
            }
        }
        else if (_cards.Count <= 0)
        {
            IEnumerable<CardCreationResult> collection = CardFactory.CreateForReward(
                base.Player,
                OptionCount,
                cardCreationOptions
            );
            ...
            IReadOnlyList<CardRewardAlternative> extraOptions = CardRewardAlternative.Generate(
                this
            );
    ...
    private void OnRelicObtained(RelicModel relic)
    {
        if (_cards == null)
        {
            throw new InvalidOperationException(
                "cards must be set first before you can update them"
            );
        }
    ...
    protected override async Task<bool> OnSelect()
    {
        ...
        IReadOnlyList<CardRewardAlternative> cardRewardOption = CardRewardAlternative.Generate(
            this
        );
        ...
        while (!endSelection)
        {
            ...
            if (LocalContext.IsMe(base.Player))
            {
                ...
                {
                    CardRewardSelection? selection = CardSelectCmd.Selector?.GetSelectedCardReward(
                        _cards,
                        cardRewardOption
                    );
                    ...
                    if (selection.Value.alternative != null)
                    {
                        ...
                        num =
                            _cards.Count
                            + cardRewardOption.FirstIndex(
                                (CardRewardAlternative r) => r == selection.Value.alternative
                            );
                    }
                    else
                    {
                        ...
                        num = (
                            (obtainedCard != null)
                                ? new int?(
                                    _cards.FirstIndex(
                                        (CardCreationResult c) => c.Card == selection.Value.card
                                    )
                                )
                                : ((int?)null)
                        );
            ...
            {
                num = (
                    await _synchronizer.WaitForRemoteChoice(base.Player, choiceId)
                ).AsIndexOrNull();
            }
            ...
            if (num.HasValue)
            {
                if (num < _cards.Count)
                {
                    ...
                    rewardComplete = true;
                    endSelection = !Hook.ShouldAllowSelectingMoreCardRewards(
                        base.Player.RunState,
                        base.Player,
                        this
                    );
                ...
                else
                {
                    ...
                    {
                        Log.Error(
                            $"Received bad player choice index {num} for a card reward with {_cards.Count} cards and {cardRewardOption.Count} alternatives!"
                        );
                        continue;
                    }
                    cardRewardAlternative = cardRewardOption[num.Value - _cards.Count];
                    rewardComplete =
                        cardRewardAlternative.AfterSelected
            ...
            {
                CardPileAddResult cardPileAddResult = await CardPileCmd.Add(
                    obtainedCard,
                    PileType.Deck
                );
                    ...
                    if (cardHolder != null)
                    {
                        ...
                        NRun.Instance.GlobalUi.TopBar.TrailContainer.AddChildSafely(
                            NCardFlyVfx.Create(
                                ...
                                PileType.Deck,
                                isAddingToPile: true,
                                obtainedCard.Owner.Character.TrailPath
                            )
                        );
                    }
                    Log.Info(
                        $"Player {base.Player.NetId} obtained {obtainedCard.Id} from card reward"
                    );
                }
            }
        ...
        foreach (CardModel item in chosenCardIds)
        {
            base.Player.RunState.CurrentMapPointHistoryEntry.GetEntry(base.Player.NetId)
                .CardChoices.Add(new CardChoiceHistoryEntry(item, wasPicked: true));
        }
        if (rewardComplete)
        {
    ...
    public override SerializableReward ToSerializable()
    {
        ...
        {
            string text = (
                (Options.CustomCardPool == null) ? "NULL" : string.Join(",", Options.CustomCardPool)
            );
            throw new NotImplementedException(
                "Tried to serialize a CardReward without any card pools! This is not currently supported. Custom card pool is: "
                    + text
            );
        }
        ...
        {
            throw new NotImplementedException(
                "Tried to serialize a CardReward with a card pool filter! This is not currently supported."
            );
        }
        ...
        {
            throw new NotImplementedException(
                "Tried to serialize a CardReward with card creation flags! "
                    + $"This is not currently supported. Flags: {cardCreationFlags}"
            );
        }

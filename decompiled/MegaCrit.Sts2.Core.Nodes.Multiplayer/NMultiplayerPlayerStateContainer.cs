...
public class NMultiplayerPlayerStateContainer : Control
{
        ...
        if (inputEvent.IsActionReleased(DebugHotkey.hideMpHealthBars))
        {
            ...
            NGame.Instance.AddChildSafely(
                NFullscreenTextVfx.Create(
                    (!base.Visible) ? "Hide MP Health bars" : "Show MP Health bars"
                )
            );
        }
    }
    ...
    public void Initialize(RunState runState)
    {
        ...
        foreach (
            Player item in _runState.Players.Except(
                new global::_003C_003Ez__ReadOnlySingleElementList<Player>(me)
            )
        )
        {
        ...
        UpdatePosition();
        NRun.Instance.GlobalUi.RelicInventory.Connect(
            NRelicInventory.SignalName.RelicsChanged,
            Callable.From(UpdatePositionAfterOneFrame)
        );
        GetViewport()
            .Connect(Viewport.SignalName.SizeChanged, Callable.From(UpdatePositionAfterOneFrame));
        for (int i = 0; i < GetChildCount(); i++)
        {
            ...
            hitbox.FocusNeighborTop = (
                (i > 0) ? GetChild<NMultiplayerPlayerState>(i - 1).Hitbox.GetPath() : null
            );
            hitbox.FocusNeighborBottom = (
                (i < GetChildCount() - 1)
                    ? GetChild<NMultiplayerPlayerState>(i + 1).Hitbox.GetPath()
                    : null
            );
        }
    }
    ...
    private Vector2 GetTargetPosition()
    {
        ...
        else
        {
            ...
            float num = relicInventory.GetThemeConstant(
                ThemeConstants.FlowContainer.VSeparation,
                "FlowContainer"
            );
            result =
                relicInventory.GetDefaultPosition() + (float)lineCount * (y + num) * Vector2.Down;
        }
        if (_hidden)
        {
    ...
    {
        _nodes
            .FirstOrDefault((NMultiplayerPlayerState n) => n.Player == player)
            ?.OnCreatureHovered();
    }
    ...
    public void AnimHide()
    {
        ...
        _tween = CreateTween();
        _tween
            .TweenProperty(this, "position", GetTargetPosition(), 0.20000000298023224)
            .SetTrans(Tween.TransitionType.Quad)
            .SetEase(Tween.EaseType.InOut);
        _tween
            .TweenProperty(this, "modulate:a", 0f, 0.20000000298023224)
            .SetTrans(Tween.TransitionType.Quad)
            .SetEase(Tween.EaseType.InOut);
    }
    ...
    public void AnimShow()
    {
        ...
        _tween = CreateTween();
        _tween
            .TweenProperty(this, "position", GetTargetPosition(), 0.25)
            .SetTrans(Tween.TransitionType.Quad)
            .SetEase(Tween.EaseType.Out);
        _tween
            .TweenProperty(this, "modulate:a", 1f, 0.15000000596046448)
            .SetTrans(Tween.TransitionType.Quad)
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdatePositionAfterOneFrame,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,

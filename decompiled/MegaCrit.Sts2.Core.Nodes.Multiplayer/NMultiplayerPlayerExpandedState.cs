...
public class NMultiplayerPlayerExpandedState : Control, ICapstoneScreen, IScreenContext
{
    private class CardGroupKey
    {
        ...
        public override bool Equals(object? obj)
        {
            ...
            if (
                _card.Id.Equals(cardGroupKey._card.Id)
                && _card.CurrentUpgradeLevel == cardGroupKey._card.CurrentUpgradeLevel
                && _card.Enchantment?.Id == cardGroupKey._card.Enchantment?.Id
            )
            {
        ...
        public override int GetHashCode()
        {
            return HashCode.Combine(
                _card.Id,
                _card.CurrentUpgradeLevel,
                _card.Enchantment?.Id,
                _card.Enchantment?.Amount
            );
        }
    }
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "ui/multiplayer_player_expanded_state"
    );
    ...
    {
        NMultiplayerPlayerExpandedState nMultiplayerPlayerExpandedState = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NMultiplayerPlayerExpandedState>(PackedScene.GenEditState.Disabled);
    ...
    public void AfterCapstoneOpened()
    {
        ...
        globalUi.MoveChildSafely(
            globalUi.AboveTopBarVfxContainer,
            globalUi.CapstoneContainer.GetIndex()
        );
        globalUi.MoveChildSafely(
            globalUi.CardPreviewContainer,
            globalUi.CapstoneContainer.GetIndex()
        );
        globalUi.MoveChildSafely(
            globalUi.MessyCardPreviewContainer,
            globalUi.CapstoneContainer.GetIndex()
        );
    }
    ...
    public override void _Ready()
    {
        ...
        locString.Add(
            "PlayerName",
            PlatformUtil.GetPlayerName(RunManager.Instance.NetService.Platform, _player.NetId)
        );
        ...
        LocString locString2 = new LocString(
            "gameplay_ui",
            "MULTIPLAYER_EXPANDED_STATE.relicHeader"
        );
        _relicsHeader.Text = locString2.GetFormattedText();
        LocString locString3 = new LocString(
            "gameplay_ui",
            "MULTIPLAYER_EXPANDED_STATE.cardHeader"
        );
        _cardsHeader.Text = locString3.GetFormattedText();
        LocString locString4 = new LocString(
            "gameplay_ui",
            "MULTIPLAYER_EXPANDED_STATE.potionHeader"
        );
        ...
        foreach (
            IGrouping<CardGroupKey, CardModel> item in from x in _player.Deck.Cards
            group x by new CardGroupKey(x)
        )
        {
            NDeckHistoryEntry nDeckHistoryEntry = NDeckHistoryEntry.Create(
                item.First(),
                item.Count()
            );
            nDeckHistoryEntry.Connect(
                NDeckHistoryEntry.SignalName.Clicked,
                Callable.From<NDeckHistoryEntry>(ShowEntry)
            );
            _cardContainer.AddChildSafely(nDeckHistoryEntry);
        }
        _backButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(BackButtonPressed)
        );
        UpdateNavigation();
    }
    ...
    private void OnRelicClicked(NRelic node)
    {
        ...
        foreach (
            NRelicBasicHolder item in _relicContainer.GetChildren().OfType<NRelicBasicHolder>()
        )
        {
    ...
    private void UpdateNavigation()
    {
        for (int i = 0; i < _relicContainer.GetChildCount(); i++)
        {
            ...
            child.FocusNeighborLeft = (
                (i > 0)
                    ? _relicContainer.GetChild<NRelicBasicHolder>(i - 1).GetPath()
                    : _relicContainer.GetChild<NRelicBasicHolder>(i).GetPath()
            );
            child.FocusNeighborRight = (
                (i < _relicContainer.GetChildCount() - 1)
                    ? _relicContainer.GetChild<NRelicBasicHolder>(i + 1).GetPath()
                    : _relicContainer.GetChild<NRelicBasicHolder>(i).GetPath()
            );
            ...
            {
                child.FocusNeighborBottom = _potionContainer
                    .GetChild<Control>(Mathf.Min(i, _potionContainer.GetChildCount() - 1))
                    ?.GetPath();
            }
            else if (_cardContainer.GetChildCount() > 0)
            {
                child.FocusNeighborBottom = _cardContainer
                    .GetChild<Control>(Mathf.Min(i, _cardContainer.GetChildCount() - 1))
                    ?.GetPath();
            }
            else
            {
        ...
        for (int j = 0; j < _potionContainer.GetChildCount(); j++)
        {
            ...
            child2.FocusNeighborLeft = (
                (j > 0)
                    ? _potionContainer.GetChild<NPotionHolder>(j - 1).GetPath()
                    : _potionContainer.GetChild<NPotionHolder>(j).GetPath()
            );
            child2.FocusNeighborRight = (
                (j < _potionContainer.GetChildCount() - 1)
                    ? _potionContainer.GetChild<NPotionHolder>(j + 1).GetPath()
                    : _potionContainer.GetChild<NPotionHolder>(j).GetPath()
            );
            if (_relicContainer.GetChildCount() > 0)
            {
                child2.FocusNeighborTop = _relicContainer
                    .GetChild<Control>(Mathf.Min(j, _relicContainer.GetChildCount() - 1))
                    ?.GetPath();
            }
            else
            {
            ...
            {
                child2.FocusNeighborBottom = _cardContainer
                    .GetChild<Control>(Mathf.Min(j, _cardContainer.GetChildCount() - 1))
                    ?.GetPath();
            }
            else
            {
        ...
        for (int k = 0; k < _cardContainer.GetChildCount(); k++)
        {
            ...
            child3.FocusNeighborLeft = (
                (k > 0)
                    ? _cardContainer.GetChild<NDeckHistoryEntry>(k - 1).GetPath()
                    : _cardContainer.GetChild<NDeckHistoryEntry>(k).GetPath()
            );
            child3.FocusNeighborRight = (
                (k < _cardContainer.GetChildCount() - 1)
                    ? _cardContainer.GetChild<NDeckHistoryEntry>(k + 1).GetPath()
                    : _cardContainer.GetChild<NDeckHistoryEntry>(k).GetPath()
            );
            if (_potionContainer.GetChildCount() > 0)
            {
                child3.FocusNeighborTop = _potionContainer
                    .GetChild<Control>(Mathf.Min(k, _potionContainer.GetChildCount() - 1))
                    ?.GetPath();
            }
            else if (_relicContainer.GetChildCount() > 0)
            {
                child3.FocusNeighborTop = _relicContainer
                    .GetChild<Control>(Mathf.Min(k, _relicContainer.GetChildCount() - 1))
                    ?.GetPath();
            }
            else
            {
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.AfterCapstoneOpened,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.AfterCapstoneClosed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,

...
public class NRemoteMouseCursorContainer : Control
{
    ...
    public override void _Ready()
    {
        GetViewport()
            .Connect(
                ...
                Callable.From<Control>(OnGuiFocusChanged)
            );
    }
    ...
    {
        GetCursor(playerId)
            ?.UpdateImage(_synchronizer.GetMouseDown(playerId), GetDrawingMode(playerId));
    }
    ...
    private void UpdateCursorVisibility()
    {
        ...
        foreach (NRemoteMouseCursor cursor in _cursors)
        {
            ...
            bool flag2 = (
                (
                    (uint)(screenType2 - 5) <= 3u
                    || screenType2 == NetScreenType.RemotePlayerExpandedState
                )
                    ? true
                    : false
            );
    ...
    private void OnGuiFocusChanged(Control focused)
    {
        if (_synchronizer != null)
        {
            ...
            if (instance != null && instance.IsUsingController)
            {
                ...
                _synchronizer.SyncLocalControllerFocus(
                    focused.GlobalPosition + focused.Size * 0.5f,
                    this
                );
            }
        }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.ForceUpdateAllCursors,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.DrawingCursorStateChanged,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateCursorVisibility,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.GetCursorPosition && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<Vector2>(
                GetCursorPosition(VariantUtils.ConvertTo<ulong>(in args[0]))
            );
            return true;
        }
        ...
        if (method == MethodName.GetCursor && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<NRemoteMouseCursor>(
                GetCursor(VariantUtils.ConvertTo<ulong>(in args[0]))
            );
            return true;
        }

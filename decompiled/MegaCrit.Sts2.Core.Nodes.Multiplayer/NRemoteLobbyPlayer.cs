...
public class NRemoteLobbyPlayer : Control
{
    ...
    {
        NRemoteLobbyPlayer nRemoteLobbyPlayer = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NRemoteLobbyPlayer>(PackedScene.GenEditState.Disabled);
    ...
    )
    {
        ...
        nRemoteLobbyPlayer._character = ModelDb.GetById<CharacterModel>(
            runLobby.Run.Players.First((SerializablePlayer p) => p.NetId == playerId).CharacterId
        );
    ...
    {
        SerializablePlayer serializablePlayer = runLobby.Run.Players.First(

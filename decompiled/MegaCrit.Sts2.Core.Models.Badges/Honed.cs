...
public class Honed : Badge
{
    ...
    {
        List<SerializableCard> source = _localPlayer
            .Deck.Where(
                (SerializableCard c) => SaveUtil.CardOrDeprecated(c.Id).Rarity != CardRarity.Basic
            )
            .ToList();
        return (from c in source group c by c.Id).Any(
            (IGrouping<ModelId, SerializableCard> g) => g.Count() >= 5
        );
    }
}

...
public class NDevConsole : Panel
{
    ...
    public static NDevConsole Instance =>
        _instance ?? throw new InvalidOperationException("Dev console used before being created.");
    ...
    {
        _outputBuffer.Text +=
            "[color=#888888]Use 'F11' to toggle console fullscreen. Press 'up arrow' to use the last command. You can autocomplete commands with 'tab'.[/color]";
        _outputBuffer.Text += "\n\n";
    }
        ...
        else if (inputEventKey.Keycode == Key.Up)
        {
            ...
            else if (_devConsole.historyIndex < _devConsole.history.Count)
            {
                ...
                if (_devConsole.historyIndex < _devConsole.history.Count - 1)
                {
                    ...
                    while (
                        _devConsole.historyIndex < _devConsole.history.Count - 1
                        && _devConsole.history[_devConsole.historyIndex] == text
                    )
                    {
                        _devConsole.historyIndex++;
                    }
        ...
        else if (inputEventKey.Keycode == Key.Down)
        {
            ...
            else if (_devConsole.historyIndex < _devConsole.history.Count)
            {
                ...
                if (_devConsole.historyIndex > 0)
                {
                    _devConsole.historyIndex--;
                    while (
                        _devConsole.historyIndex > 0
                        && _devConsole.history[_devConsole.historyIndex] == text2
                    )
                    {
                        _devConsole.historyIndex--;
                    }
    ...
    private void UpdateGhostText()
    {
        ...
        if (
            completionResults.Candidates.Count == 1
            && !string.IsNullOrEmpty(completionResults.CommonPrefix)
        )
        {
            ...
            {
                GD.PushError(
                    $"BUG: CommonPrefix '{commonPrefix}' doesn't start with input '{text}'"
                );
                HideGhostText();
            }
            else
    ...
    private void RenderSelectionMenu()
    {
        ...
        list.Add(
            _tabCompletion.LastCompletionResult?.Type switch
            {
                CompletionType.Command => "Select command:",
                CompletionType.Subcommand => "Select "
                    + _tabCompletion.LastCompletionResult.ArgumentContext
                    + " action:",
                CompletionType.Argument => "Select "
                    + _tabCompletion.LastCompletionResult.ArgumentContext
                    + " argument:",
                _ => "Select option:",
            }
        ...
        list.Add(
            "[color=gray]Tab/"
                + _symbolUp
                + _symbolDown
                + ": navigate, Enter: accept, Esc: cancel, Type to filter[/color]"
        );
        list.Add("");
    ...
    private void AddCandidateToDisplay(List<string> displayLines, int index)
    {
        if (index >= 0 && index < _tabCompletion.CompletionCandidates.Count)
        {
            ...
            string item = (
                (index == _tabCompletion.SelectionIndex)
                    ? $"[color=yellow]{_symbolPrompt} {text}[/color]"
                    : ("  " + text)
            );
    ...
    private void NavigateSelection(int direction)
    {
        if (_tabCompletion.InSelectionMode && _tabCompletion.CompletionCandidates.Count != 0)
        {
            _tabCompletion.SelectionIndex =
                (
                    _tabCompletion.SelectionIndex
                    + direction
                    + _tabCompletion.CompletionCandidates.Count
                ) % _tabCompletion.CompletionCandidates.Count;
            RenderSelectionMenu();
        }
    }
    ...
    {
        if (
            !_tabCompletion.InSelectionMode
            || _tabCompletion.SelectionIndex < 0
            || _tabCompletion.SelectionIndex >= _tabCompletion.CompletionCandidates.Count
        )
        {
            return;
        }
        ...
        else
        {
            ...
            string[] array2 = (
                (array.Length > 1) ? array.Take(array.Length - 1).ToArray() : Array.Empty<string>()
            );
    ...
    private void UpdatePromptStyle()
    {
        ...
        _promptLabel.AddThemeColorOverride(
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.HandleReadlineKeybinding,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.AutocompleteCommand,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.RenderSelectionMenu,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnToggleMaximizeButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.MoveInputCursorToEndOfLine,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,

...
public class NDebugInfoLabelManager : Node
{
    ...
    public override void _Ready()
    {
        ...
        _moddedWarning.Connect(
            Control.SignalName.MouseEntered,
            Callable.From(OnModdedWarningHovered)
        );
        _moddedWarning.Connect(
            Control.SignalName.MouseExited,
            Callable.From(OnModdedWarningUnhovered)
        );
    ...
    private void UpdateText(string? commitId)
    {
        ...
        if (isMainMenu)
        {
            ...
            LocString[] array = ModManager
                .Mods.SelectMany((Mod m) => m.errors ?? new List<LocString>())
                .ToArray();
            if (array.Length != 0)
            {
        ...
        if (inputEvent.IsActionReleased(DebugHotkey.hideVersionInfo))
        {
            ...
            NGame.Instance.AddChildSafely(
                NFullscreenTextVfx.Create(
                    _releaseInfo.Visible ? "Show Version Info" : "Hide Version Info"
                )
            );
        }
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnModdedWarningHovered,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnModdedWarningUnhovered,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._modWarningContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

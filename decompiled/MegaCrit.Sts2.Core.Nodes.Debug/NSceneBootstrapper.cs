...
public class NSceneBootstrapper : Node
{
    ...
    private async Task StartNewRun()
    {
        ...
        {
            Log.Error(
                "No type implementing IBootstrapSettings found in the project! To use the bootstrap scene, copy src/Core/Nodes/Debug/BootstrapSettings.cs.template and rename it to BootstrapSettings.cs"
            );
            return;
        }
        ...
        RunState runState = RunState.CreateForNewRun(
            ...
            list.Select((ActModel a) => a.ToMutable()).ToList(),
            settings.Modifiers,
            GameMode.Standard,
            settings.Ascension,
            seed
        ...
        await PreloadManager.LoadRunAssets(
            new global::_003C_003Ez__ReadOnlySingleElementList<CharacterModel>(settings.Character)
        );
        ...
        switch (settings.RoomType)
        {
            ...
            case RoomType.Event:
            {
                AbstractRoom abstractRoom = await RunManager.Instance.EnterRoomDebug(
                    settings.RoomType,

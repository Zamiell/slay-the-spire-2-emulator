...
public class NControllerMapDrawingInput : NMapDrawingInput
{
    ...
    {
        public static new readonly StringName Create = "Create";
    ...
    public static NMapDrawingInput Create()
    {
        return PreloadManager
            .Cache.GetScene("res://scenes/screens/map/controller_map_drawing_input.tscn")
            .Instantiate<NControllerMapDrawingInput>(PackedScene.GenEditState.Disabled);
    }
    ...
    public override void _Ready()
    {
        ...
        {
            _cursorTex = ImageTexture.CreateFromImage(
                PreloadManager.Cache.GetAsset<Image>(
                    "res://images/packed/common_ui/cursor_quill.png"
                )
            );
            _cursorTiltedTex = ImageTexture.CreateFromImage(
                PreloadManager.Cache.GetAsset<Image>(
        ...
        _cursor.GetNode<TextureRect>("TextureRect").Position = (
            (base.DrawingMode == DrawingMode.Drawing) ? _drawingIconPos : _eraserIconPos
        );
    }
    ...
    public override void _Process(double delta)
    {
        if (Input.IsActionPressed(MegaInput.select))
        {
            if (!_isPressed)
            {
                _drawings.BeginLineLocal(
                    _drawings.GetGlobalTransform().Inverse() * _cursor.GlobalPosition,
                    null
                );
        ...
        if (_direction.Length() > 0f)
        {
            ...
            _cursor.GlobalPosition = _cursor.GlobalPosition.Clamp(
                NGame.Instance.GlobalPosition,
                NGame.Instance.GlobalPosition + NGame.Instance.Size
            );
            if (_drawings.IsLocalDrawing())
            {
                _drawings.UpdateCurrentLinePositionLocal(
                    _drawings.GetGlobalTransform().Inverse() * _cursor.GlobalPosition
                );
            }
        }
    ...
    [EditorBrowsable(EditorBrowsableState.Never)]
    internal static new bool InvokeGodotClassStaticMethod(
        in godot_string_name method,
        NativeVariantPtrArgs args,
        out godot_variant ret
    )

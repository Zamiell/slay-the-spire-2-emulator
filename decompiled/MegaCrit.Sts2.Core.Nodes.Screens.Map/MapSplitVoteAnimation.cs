...
public class MapSplitVoteAnimation
{
    ...
    public async Task TryPlay(MapCoord selectedCoord)
    {
        ...
        if (!flag)
        {
            ...
            _sortedPlayers = (
                from p in (
                    from p in _mapScreen.PlayerVoteDictionary.ToList()
                    where p.Value.HasValue
                    select p
                )
                    .OrderBy(
                        (KeyValuePair<Player, MapCoord?> p) => p.Value,
                        Comparer<MapCoord?>.Create(MapCoordComparer)
                    )
                    .ThenBy(
                        (KeyValuePair<Player, MapCoord?> p) =>
                            _mapPointDictionary[p.Value.Value].VoteContainer.GetVoteIndex(p.Key)
                    )
                select p.Key
            ).ToList();
            ...
            tween
                .TweenMethod(
                    Callable.From<float>(TickSplitVoteAnimation),
                    0f,
                    1f,
                    1.2000000476837158
                )
    ...
    private void HighlightPlayer(Player? player)
    {
        if (_currentlyHighlightedPlayer != player)
        {
            if (_currentlyHighlightedPlayer != null)
            {
                ...
                _mapPointDictionary[value]
                    .VoteContainer.SetPlayerHighlighted(
                        _currentlyHighlightedPlayer,
                        isHighlighted: false
                    );
            }
            ...
            if (player != null)
            {
                ...
                _mapPointDictionary[value2]
                    .VoteContainer.SetPlayerHighlighted(player, isHighlighted: true);
            }
        }
    }
}

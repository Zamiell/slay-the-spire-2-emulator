...
public abstract class NMapDrawingInput : Control
{
    ...
    public static NMapDrawingInput Create(
        ...
        bool stopOnMouseRelease = false
    )
    {
        NMapDrawingInput nMapDrawingInput = (
            stopOnMouseRelease
                ? new NMouseHeldMapDrawingInput()
                : (
                    (!NControllerManager.Instance.IsUsingController)
                        ? new NMouseModeMapDrawingInput()
                        : NControllerMapDrawingInput.Create()
                )
        );
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NMapDrawingInput>(
                Create(
                    VariantUtils.ConvertTo<NMapDrawings>(in args[0]),

...
public class NMapScreen : Control, IScreenContext, INetCursorPositionTranslator
{
    ...
    {
        public static new readonly StringName _Ready = "_Ready";

        public static new readonly StringName _ExitTree = "_ExitTree";
        ...
        public static new readonly StringName _Process = "_Process";

        public static readonly StringName UpdateScrollPosition = "UpdateScrollPosition";

        public static new readonly StringName _GuiInput = "_GuiInput";
        ...
        public static new readonly StringName _Input = "_Input";
        ...
        public static readonly StringName OnClearMapDrawingButtonPressed =
            "OnClearMapDrawingButtonPressed";
    ...
    private readonly Dictionary<MapCoord, NMapPoint> _mapPointDictionary =
        new Dictionary<MapCoord, NMapPoint>();

    private readonly Dictionary<(MapCoord, MapCoord), IReadOnlyList<TextureRect>> _paths =
        new Dictionary<(MapCoord, MapCoord), IReadOnlyList<TextureRect>>();
    ...
    private readonly double _mapAnimStartDelay = (
        (SaveManager.Instance.PrefsSave.FastMode == FastModeType.Fast) ? 0.5 : 1.0
    );

    private readonly double _mapAnimDuration = (
        (SaveManager.Instance.PrefsSave.FastMode == FastModeType.Fast) ? 1.5 : 3.0
    );
    ...
    public Dictionary<Player, MapCoord?> PlayerVoteDictionary { get; } =
        new Dictionary<Player, MapCoord?>();

    public NMapDrawings Drawings { get; private set; }

    public static IEnumerable<string> AssetPaths =>
        NMapDrawings.AssetPaths.Append("res://scenes/ui/map_dot.tscn");

    public Control DefaultFocusedControl
    {
        get
        {
            NMapPoint nMapPoint = _mapPointDictionary.Values.FirstOrDefault(
                (NMapPoint n) => n.IsEnabled
            );
            if (nMapPoint != null)
            {
                return nMapPoint;
    ...
    {
        add { backing_Opened = (OpenedEventHandler)Delegate.Combine(backing_Opened, value); }
        remove { backing_Opened = (OpenedEventHandler)Delegate.Remove(backing_Opened, value); }
    }

    public event ClosedEventHandler Closed
    {
        add { backing_Closed = (ClosedEventHandler)Delegate.Combine(backing_Closed, value); }
        remove { backing_Closed = (ClosedEventHandler)Delegate.Remove(backing_Closed, value); }
    }

    public override void _Ready()
    {
        GetNode<MegaLabel>("MapLegend/Header")
            .SetTextAutoSize(new LocString("map", "LEGEND_HEADER").GetFormattedText());
        ...
        _backButton.Connect(
            ...
            Callable.From<NButton>(OnBackButtonPressed)
        );
        _backButton.Disable();
        ...
        _mapDrawingButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnMapDrawingButtonPressed)
        );
        _mapErasingButton = GetNode<NMapEraseButton>("%EraseButton");
        _mapErasingButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnMapErasingButtonPressed)
        );
        _mapClearButton = GetNode<NMapClearButton>("%ClearButton");
        _mapClearButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnClearMapDrawingButtonPressed)
        );
        ...
        Callable
            .From(() =>
                NCapstoneContainer.Instance.Connect(
                    NCapstoneContainer.SignalName.Changed,
                    Callable.From(OnCapstoneChanged)
                )
            )
            .CallDeferred();
        ...
        {
            list[num].FocusNeighborTop = (
                (num > 0) ? list[num - 1].GetPath() : list[num].GetPath()
            );
            list[num].FocusNeighborBottom = (
                (num < list.Count - 1) ? list[num + 1].GetPath() : list[num].GetPath()
            );
        ...
        NControllerManager.Instance.Connect(
            ...
            Callable.From(UpdateHotkeyDisplay)
        );
        NControllerManager.Instance.Connect(
            NControllerManager.SignalName.ControllerDetected,
            Callable.From(UpdateHotkeyDisplay)
        );
        NInputManager.Instance.Connect(
            NInputManager.SignalName.InputRebound,
            Callable.From(UpdateHotkeyDisplay)
        );
        UpdateHotkeyDisplay();
    }
    ...
    public void SetMap(ActMap map, uint seed, bool clearDrawings)
    {
        ...
        foreach (MapPoint allMapPoint in map.GetAllMapPoints())
        {
            ...
            nNormalMapPoint.Position =
                new Vector2(allMapPoint.coord.col, allMapPoint.coord.row) * vector2 + vector;
        ...
        if (map.StartingMapPoint.PointType == MapPointType.Ancient)
        {
            ...
            _startingPointNode.Position = new Vector2(
                -80f,
                (float)map.StartingMapPoint.coord.row * (0f - _distY) + 720f
            );
        }
        else
        {
            ...
            _startingPointNode.Position = new Vector2(
                -80f,
                (float)map.StartingMapPoint.coord.row * (0f - _distY) + 800f
            );
        }
        ...
        for (int i = 0; i < visitedMapCoords.Count - 1; i++)
        {
            if (
                !_paths.TryGetValue(
                    (visitedMapCoords[i], visitedMapCoords[i + 1]),
                    out IReadOnlyList<TextureRect> value
                )
            )
            {
                continue;
        ...
        for (int j = 0; j < map.GetRowCount(); j++)
        {
            ...
            List<NMapPoint> list = pointsInRow
                .Select((MapPoint p) => _mapPointDictionary[p.coord])
                .ToList();
            for (int num2 = 0; num2 < list.Count; num2++)
            {
                list[num2].FocusNeighborLeft = (
                    (num2 > 0) ? list[num2 - 1].GetPath() : list[num2].GetPath()
                );
                list[num2].FocusNeighborRight = (
                    (num2 < list.Count - 1) ? list[num2 + 1].GetPath() : list[num2].GetPath()
                );
        ...
        foreach (MapPoint child in mapPoint.Children)
        {
            if (!_mapPointDictionary.TryGetValue(child.coord, out NMapPoint value))
            {
                throw new InvalidOperationException(
                    $"Map point child with coord {child.coord} is not in the map point dictionary!"
                );
            }
    ...
    private void RecalculateTravelability()
    {
        if (_runState.VisitedMapCoords.Any())
        {
            ...
            foreach (MapCoord visitedMapCoord in _runState.VisitedMapCoords)
            {
                ...
                Log.Error(
                    $"VisitedMapCoord {visitedMapCoord} not found in map point dictionary, map may have been regenerated"
                );
            }
            IReadOnlyList<MapCoord> visitedMapCoords = _runState.VisitedMapCoords;
            ...
            {
                IEnumerable<MapPoint> travelablePointsFrom = MapTravel.GetTravelablePointsFrom(
                    _runState,
                    value2.Point
                );
                {
            ...
            Log.Error(
                $"Last visited coord {mapCoord} not found in map, falling back to starting point"
            );
    ...
    private void InitMapVotes()
    {
        ...
        {
            MapCoord? mapCoord = RunManager
                .Instance.MapSelectionSynchronizer.GetVote(player)
                ?.coord;
    ...
    public void OnMapPointSelectedLocally(NMapPoint point)
    {
        ...
        {
            OnPlayerVoteChangedInternal(
                me,
                RunManager.Instance.MapSelectionSynchronizer.GetVote(me)?.coord,
                point.Point.coord
            );
            MapLocation source = new MapLocation(
                _runState.CurrentMapCoord,
                _runState.CurrentActIndex
            );
            MapVote value2 = new MapVote
            {
                coord = point.Point.coord,
                mapGenerationCount = RunManager
                    .Instance
                    .MapSelectionSynchronizer
                    .MapGenerationCount,
            };
            VoteForMapCoordAction action = new VoteForMapCoordAction(
                LocalContext.GetMe(_runState),
                source,
                value2
            );
    ...
    public async Task TravelToMapCoord(MapCoord coord)
    {
        ...
        MapSplitVoteAnimation mapSplitVoteAnimation = new MapSplitVoteAnimation(
            this,
            _runState,
            _mapPointDictionary
        );
        ...
        if (visitedMapCoords.Any())
        {
            if (
                _paths.TryGetValue(
                    (visitedMapCoords[visitedMapCoords.Count - 1], node.Point.coord),
                    out IReadOnlyList<TextureRect> value
                )
            )
            {
                ...
                foreach (TextureRect tick in value)
                {
                    ...
                    tween
                        .TweenProperty(tick, "scale", _tickTraveledScale, 0.4)
    ...
    private IReadOnlyList<TextureRect> CreatePath(Vector2 start, Vector2 end)
    {
        ...
        for (int i = 1; i < num3; i++)
        {
            ...
            TextureRect textureRect = PreloadManager
                .Cache.GetScene("res://scenes/ui/map_dot.tscn")
    ...
    private void UpdateScrollPosition(double delta)
    {
        ...
        {
            _mapContainer.Position = _mapContainer.Position.Lerp(
                _targetDragPos,
                (float)delta * 15f
            );
    ...
    private void ProcessMouseEvent(InputEvent inputEvent)
    {
        ...
        {
            Drawings.UpdateCurrentLinePositionLocal(
                Drawings.GetGlobalTransform().Inverse() * inputEventMouseMotion2.GlobalPosition
            );
        }
    }
    ...
    private void ProcessMouseDrawingEvent(InputEvent inputEvent)
    {
        if (
            !_isInputDisabled
            && (_actAnimTween == null || !_actAnimTween.IsRunning())
            && _drawingInput == null
            && inputEvent is InputEventMouseButton { Pressed: not false } inputEventMouseButton
        )
        {
            if (inputEventMouseButton.ButtonIndex == MouseButton.Right)
            {
                _drawingInput = NMapDrawingInput.Create(
                    Drawings,
                    DrawingMode.Drawing,
                    stopOnMouseRelease: true
                );
            }
            ...
            _drawingInput?.Connect(
                NMapDrawingInput.SignalName.Finished,
                Callable.From(
                    delegate
                    {
                        _drawingInput = null;
    ...
    private void ProcessScrollEvent(InputEvent inputEvent)
    {
        if (CanScroll())
        {
            ...
            if (
                (inputEvent is InputEventMouseButton || inputEvent is InputEventPanGesture)
                    ? true
                    : false
            )
            {
    ...
    private void ProcessControllerEvent(InputEvent inputEvent)
    {
        ...
        else if (inputEvent.IsActionPressed(MegaInput.down) && CanScroll())
        {
            ...
            inputEvent.IsActionPressed(MegaInput.right)
            || inputEvent.IsActionPressed(MegaInput.left)
    ...
    private async Task StartOfActAnim()
    {
        ...
        _actAnimTween
            .TweenProperty(_mapContainer, "position:y", -600f, _mapAnimDuration)
            .SetEase(Tween.EaseType.InOut)
            .SetTrans(Tween.TransitionType.Expo);
        _actAnimTween
            .TweenCallback(Callable.From(SetInterruptable))
            .SetDelay(_mapAnimDuration * 0.25);
    ...
    private async Task AnimClose()
    {
        ...
        _tween
            .TweenProperty(_mapContainer, "modulate", StsColors.transparentBlack, 0.25)
            .SetDelay(0.1);
        _tween
            .TweenProperty(_mapContainer, "position:y", _mapContainer.Position.Y + 200f, 0.25)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
        _tween.TweenProperty(_mapLegend, "modulate:a", 0f, 0.15);
        _tween
            .TweenProperty(_mapLegend, "position:x", MapLegendX + 120f, 0.25)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
    ...
    public NMapScreen Open(bool isOpenedFromTopBar = false)
    {
        ...
        if (
            (
                (_runState.CurrentActIndex == 0 && _runState.ExtraFields.StartedWithNeow)
                    ? (_runState.ActFloor == 1)
                    : (_runState.ActFloor == 0)
            ) && !_hasPlayedAnimation
        )
        {
            if (
        ...
        _tween
            .TweenProperty(_mapContainer, "modulate", Colors.White, 0.25)
            .From(StsColors.transparentBlack);
        _tween.TweenProperty(_mapLegend, "modulate", Colors.White, 0.25).SetDelay(0.1);
        _tween
            .TweenProperty(_mapLegend, "position:x", MapLegendX, 0.25)
            .From(MapLegendX + 120f)
            .SetEase(Tween.EaseType.Out)
        ...
        if (_runState.VisitedMapCoords.Count != 0)
        {
            ...
            MapCoord mapCoord = visitedMapCoords[visitedMapCoords.Count - 1];
            if (
                _bossPointNode.Point.coord.row != mapCoord.row
                && _startingPointNode.Point.coord.row != mapCoord.row
            )
            {
    ...
    {
        if (
            (GetViewport().GuiGetFocusOwner() is NMapPoint || HasFocus())
            && ActiveScreenContext.Instance.IsCurrent(this)
        )
        {
            if (inputEvent.IsActionReleased(DebugHotkey.unlockCharacters))
            {
                ...
                NGame.Instance.AddChildSafely(
                    NFullscreenTextVfx.Create(_mapLegend.Visible ? "Show Legend" : "Hide Legend")
                );
            }
            if (IsVisibleInTree())
            {
    ...
    private void OnLegendHotkeyPressed()
    {
        ...
        NMapPoint nMapPoint = _mapPointDictionary.Values.LastOrDefault(
            (NMapPoint n) => n.IsEnabled
        );
        if (nMapPoint != null)
        {
    ...
    private void UpdateHotkeyDisplay()
    {
        ...
        _drawingToolsHotkeyIcon.Texture = NInputManager.Instance.GetHotkeyIcon(
            MegaInput.viewExhaustPileAndTabRight
        );
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.RecalculateTravelability,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnMapPointSelectedLocally,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.RefreshAllMapPointVotes,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.RemoveAllMapPointsAndPaths,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ProcessMouseDrawingEvent,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ProcessControllerEvent,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetDebugTravelEnabled,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.RefreshAllPointVisuals,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.PlayStartOfActAnimation,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.TryCancelStartOfActAnim,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnBackButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnMapDrawingButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnMapErasingButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateDrawingButtonStates,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnClearMapDrawingButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnLegendHotkeyPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnDrawingToolsHotkeyPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetNetPositionFromScreenPosition,
                new PropertyInfo(
                    Variant.Type.Vector2,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetMapPositionFromNetPosition,
                new PropertyInfo(
                    Variant.Type.Vector2,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetScreenPositionFromNetPosition,
                new PropertyInfo(
                    Variant.Type.Vector2,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateHotkeyDisplay,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<Vector2>(
                GetLineEndpoint(VariantUtils.ConvertTo<NMapPoint>(in args[0]))
            );
            return true;
        }
        ...
        {
            ret = VariantUtils.CreateFrom<NMapScreen>(
                Open(VariantUtils.ConvertTo<bool>(in args[0]))
            );
            return true;
        }
        ...
        {
            ret = VariantUtils.CreateFrom<Vector2>(
                GetNetPositionFromScreenPosition(VariantUtils.ConvertTo<Vector2>(in args[0]))
            );
            return true;
        }
        ...
        {
            ret = VariantUtils.CreateFrom<Vector2>(
                GetMapPositionFromNetPosition(VariantUtils.ConvertTo<Vector2>(in args[0]))
            );
            return true;
        }
        ...
        {
            ret = VariantUtils.CreateFrom<Vector2>(
                GetScreenPositionFromNetPosition(VariantUtils.ConvertTo<Vector2>(in args[0]))
            );
            return true;
        }
        if (method == MethodName.IsNodeOnScreen && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<bool>(
                IsNodeOnScreen(VariantUtils.ConvertTo<NMapPoint>(in args[0]))
            );
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName.IsDebugTravelEnabled,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._secondBossPointNode,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._startingPointNode,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._drawingToolsHotkeyIcon,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._mapDrawingButton,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._mapErasingButton,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._legendHotkeyIcon,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._hasPlayedAnimation,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Float,
                PropertyName._controllerScrollAmount,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Float,
                PropertyName._mapScrollAnimTimer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Float,
                PropertyName._mapAnimStartDelay,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._canInterruptAnim,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.DefaultFocusedControl,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName.IsDebugTravelEnabled,
            Variant.From<bool>(IsDebugTravelEnabled)
        );
        ...
        info.AddProperty(
            PropertyName._drawingToolsHotkeyIcon,
            Variant.From(in _drawingToolsHotkeyIcon)
        );
        ...
        info.AddProperty(
            PropertyName._controllerScrollAmount,
            Variant.From(in _controllerScrollAmount)
        );

...
public class NMapDrawings : Control
{
    ...
    public new class MethodName : Control.MethodName
    {
        ...
        public static readonly StringName UpdateCurrentLinePositionLocal =
            "UpdateCurrentLinePositionLocal";
        ...
        public static readonly StringName UpdateVisibilityFromSettings =
    ...
    private static readonly string _lineDrawScenePath = SceneHelper.GetScenePath(
        "screens/map/map_line_draw"
    );

    private static readonly string _lineEraseScenePath = SceneHelper.GetScenePath(
        "screens/map/map_line_erase"
    );

    private static readonly string _playerDrawingPath = SceneHelper.GetScenePath(
        "screens/map/map_drawing"
    );
    ...
    public static IEnumerable<string> AssetPaths =>
        SelfAssetPaths.Concat(NMapDrawButton.AssetPaths);
    ...
    )
    {
        ...
        _netService.RegisterMessageHandler<MapDrawingModeChangedMessage>(
            HandleMapDrawingModeChangedMessage
        );
    ...
    public override void _ExitTree()
    {
        ...
        _netService.UnregisterMessageHandler<ClearMapDrawingsMessage>(
            HandleClearMapDrawingsMessage
        );
        _netService.UnregisterMessageHandler<MapDrawingModeChangedMessage>(
            HandleMapDrawingModeChangedMessage
        );
    ...
    public void StopLineLocal()
    {
        ...
        NetMapDrawingEvent ev = new NetMapDrawingEvent { type = MapDrawingEventType.EndLine };
        QueueOrSendEvent(ev);
    }
    ...
    private void HandleDrawingMessage(MapDrawingMessage message, ulong senderId)
    {
        ...
        foreach (NetMapDrawingEvent @event in message.Events)
        {
            if (@event.type == MapDrawingEventType.BeginLine)
            {
                ...
                BeginLine(
                    drawingStateForPlayer,
                    FromNetPosition(@event.position),
                    @event.overrideDrawingMode
                );
            }
            else if (@event.type == MapDrawingEventType.ContinueLine)
            {
                ...
                {
                    if (
                        message.drawingMode.HasValue
                        && drawingStateForPlayer.drawingMode != message.drawingMode
                    )
                    {
    ...
    private void BeginLine(DrawingState state, Vector2 position, DrawingMode? overrideDrawingMode)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Player {state.playerId} is not currently in a drawing mode and no override was passed!"
            );
        }
        state.overrideDrawingMode = overrideDrawingMode;
        state.currentlyDrawingLine = CreateLineForPlayer(
            player,
    ...
    private void UpdateCurrentLinePosition(DrawingState state, Vector2 position)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to update current line position for player {state.playerId}, but they are not currently drawing a line!"
            );
        }
    ...
    {
        DrawingState drawingState = _drawingStates.FirstOrDefault(
            (DrawingState s) => s.playerId == playerId
        );
        if (drawingState == null)
        {
            Control control = PreloadManager
                .Cache.GetScene(_playerDrawingPath)
    ...
    public SerializableMapDrawings GetSerializableMapDrawings()
    {
        ...
        foreach (DrawingState drawingState in _drawingStates)
        {
            SerializablePlayerMapDrawings serializablePlayerMapDrawings =
                new SerializablePlayerMapDrawings { playerId = drawingState.playerId };
            serializableMapDrawings.drawings.Add(serializablePlayerMapDrawings);
            foreach (Line2D item in drawingState.drawViewport.GetChildren().OfType<Line2D>())
            {
                SerializableMapDrawingLine serializableMapDrawingLine =
                    new SerializableMapDrawingLine { mapPoints = new List<Vector2>() };
    ...
    public void LoadDrawings(SerializableMapDrawings drawings)
    {
        foreach (SerializablePlayerMapDrawings drawing in drawings.drawings)
        {
            ...
            {
                Log.Warn(
                    $"Player {drawing.playerId} has map drawings, but doesn't exist in the run!"
                );
                continue;
            }
    ...
    private void UpdateLocalCursor()
    {
        ...
        {
            Image asset = PreloadManager.Cache.GetAsset<Image>(
                "res://images/packed/common_ui/cursor_quill.png"
            );
            Image asset2 = PreloadManager.Cache.GetAsset<Image>(
                "res://images/packed/common_ui/cursor_quill_tilted.png"
            );
        ...
        {
            Image asset3 = PreloadManager.Cache.GetAsset<Image>(
                "res://images/packed/common_ui/cursor_eraser.png"
            );
            Image asset4 = PreloadManager.Cache.GetAsset<Image>(
                "res://images/packed/common_ui/cursor_eraser_tilted.png"
            );
    ...
    public void RepositionBasedOnBackground(Control mapBg)
    {
        base.Position = new Vector2(
            mapBg.Position.X + (mapBg.Size.X - base.Size.X) * 0.5f,
            mapBg.Position.Y
        );
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateCurrentLinePositionLocal,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetDrawingModeLocal,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ClearDrawnLinesLocal,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetLocalDrawingMode,
                new PropertyInfo(
                    Variant.Type.Int,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateVisibilityFromSettings,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.RepositionBasedOnBackground,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.IsDrawing && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<bool>(
                IsDrawing(VariantUtils.ConvertTo<ulong>(in args[0]))
            );
            return true;
        }
        ...
        {
            ret = VariantUtils.CreateFrom<DrawingMode>(
                GetDrawingMode(VariantUtils.ConvertTo<ulong>(in args[0]))
            );
            return true;
        }
        ...
        {
            ret = VariantUtils.CreateFrom<DrawingMode>(
                GetLocalDrawingMode(VariantUtils.ConvertTo<bool>(in args[0]))
            );
            return true;
        }
        if (method == MethodName.ToNetPosition && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<Vector2>(
                ToNetPosition(VariantUtils.ConvertTo<Vector2>(in args[0]))
            );
            return true;
        }
        ...
        {
            ret = VariantUtils.CreateFrom<Vector2>(
                FromNetPosition(VariantUtils.ConvertTo<Vector2>(in args[0]))
            );
            return true;
        }

...
{
    private readonly Dictionary<string, AbstractConsoleCmd> _commands =
        new Dictionary<string, AbstractConsoleCmd>();
    ...
    public DevConsole(bool shouldAllowDebugCommands)
    {
        ...
        LoadCommandHistory();
        foreach (
            Type item in AbstractConsoleCmdSubtypes.All.Concat(
                ReflectionHelper.GetSubtypesInMods<AbstractConsoleCmd>()
            )
        )
        {
            AbstractConsoleCmd abstractConsoleCmd = (AbstractConsoleCmd)
    ...
    public CompletionResult GetCompletionResults(string inputBuffer)
    {
        string[] array = (
            inputBuffer.EndsWith(' ')
                ? inputBuffer.Trim().Split(' ').Append(string.Empty).ToArray()
                : inputBuffer.Trim().Split(' ')
        );
        ...
        if (_commands.TryGetValue(text.Trim().ToLowerInvariant(), out AbstractConsoleCmd value))
        {
            ...
            CompletionType type = (
                (array2.Length <= 1) ? CompletionType.Subcommand : CompletionType.Argument
            );
            int argumentIndex = array2.Length - 1;
            CompletionResult argumentCompletions = value.GetArgumentCompletions(
        ...
        List<string> possibleTokens = _commands
            .Values.Select((AbstractConsoleCmd s) => s.CmdName)
            .ToList();
    ...
    )
    {
        ...
        if (list5.Count == 1)
        {
            string commonPrefix = (
                (list.Count == 1)
                    ? (ReplaceLastToken(originalInput, list5.First()) + " ")
                    : originalInput
            );
            return new CompletionResult
            {
    ...
    private static string ReplaceLastToken(string text, string replacement)
    {
        ...
        string[] array2 = (
            (array.Length != 0) ? array.Take(array.Length - 1).ToArray() : Array.Empty<string>()
        );
    ...
    private static string LongestCommonSubstring(List<string> cmdCandidates)
    {
        ...
        for (int num2 = 0; num2 < int.MaxValue; num2++)
        {
            foreach (string cmdCandidate in cmdCandidates)
            {
                if (
                    num == num2
                    || char.ToLowerInvariant(cmdCandidate[num2])
                        != char.ToLowerInvariant(text[num2])
                )
                {
    ...
    public CmdResult ProcessCommand(string inputValue)
    {
        ...
        if (
            ...
            && _commands.TryGetValue(array[0].ToLowerInvariant(), out AbstractConsoleCmd value)
            && value.IsNetworked
            && me != null
        )
        {
            RunManager.Instance.ActionQueueSynchronizer.RequestEnqueue(
                new ConsoleCmdGameAction(me, inputValue, CombatManager.Instance.IsInProgress)
            );
    ...
    private CmdResult HelpCmd(string[] args)
    {
        if (args.Length != 0)
        {
            ...
            return new CmdResult(
                success: true,
                $"[gold]{value.CmdName}[/gold] {value.Args}\n\t{value.Description}"
            );
        }
        ...
        foreach (KeyValuePair<string, AbstractConsoleCmd> command in _commands)
        {
            ...
            StringBuilder.AppendInterpolatedStringHandler handler =
                new StringBuilder.AppendInterpolatedStringHandler(18, 2, stringBuilder2);

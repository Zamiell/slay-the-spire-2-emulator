...
public static class ForgeCmd
{
    ...
    )
    {
        ...
        if (blades.Count == 0)
        {
            SovereignBlade sovereignBlade = player.Creature.CombatState.CreateCard<SovereignBlade>(
                player
            );
    ...
    {
        return player
            .PlayerCombatState.AllCards.Where(
    ...
    private static void PreviewSovereignBlade(IReadOnlyCollection<SovereignBlade> blades)
    {
        ...
        List<SovereignBlade> list = blades
            .Where((SovereignBlade c) => c.Pile.Type == PileType.Hand)
            .ToList();
        List<SovereignBlade> list2 = blades
            .Where((SovereignBlade c) => c.Pile.Type != PileType.Hand)
            .ToList();
        foreach (SovereignBlade item in list)
        {
            NCardSmithVfx child = NCardSmithVfx.Create(
                NCombatRoom.Instance.Ui.Hand.GetCard(item),
                playSfx: false
            );

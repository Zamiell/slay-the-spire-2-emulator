...
public static class PlayerCmd
{
    ...
    public static async Task GainStars(decimal amount, Player player)
    {
        if (
            ...
            && Hook.ShouldGainStars(player.Creature.CombatState, amount, player)
        )
        {
    ...
    public static async Task<Creature> AddPet<T>(Player player)
    ...
    {
        questCard
            .Owner.RunState.CurrentMapPointHistoryEntry?.GetEntry(questCard.Owner.NetId)
            .CompletedQuests.Add(questCard.Id);
    }
}

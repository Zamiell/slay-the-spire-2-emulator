...
{
    public static async Task<T> Obtain<T>(Player player)
        where T : RelicModel
    {
    ...
    public static async Task<RelicModel> Obtain(RelicModel relic, Player player, int index = -1)
    {
        ...
        runState
            .CurrentMapPointHistoryEntry?.GetEntry(player.NetId)
    ...
    {
        relic
            .Owner.RunState.CurrentMapPointHistoryEntry?.GetEntry(relic.Owner.NetId)
            .RelicsRemoved.Add(relic.Id);

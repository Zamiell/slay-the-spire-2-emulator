...
public static class OrbCmd
{
    ...
    public static void RemoveSlots(Player player, int amount)
    {
        if (!CombatManager.Instance.IsOverOrEnding)
        {
            ...
            NCombatRoom
                .Instance?.GetCreatureNode(player.Creature)
                .OrbManager?.RemoveSlotAnim(amount);
        }
    }

    public static async Task Channel<T>(PlayerChoiceContext choiceContext, Player player)
        where T : OrbModel
    {
    ...
    )
    {
        ...
        if (dequeue)
        {
            ...
            NCombatRoom
                .Instance?.GetCreatureNode(player.Creature)
                ?.OrbManager?.EvokeOrbAnim(evokedOrb);
        }

...
public static class CardCmd
{
    ...
    )
    {
        ...
        {
            await MoveToResultPileWithoutPlaying(choiceContext, card);
            LocString playerDialogueLine = UnplayableReason.BlockedByHook.GetPlayerDialogueLine(
                preventer
            );
            if (playerDialogueLine != null)
            {
                card.Owner.Creature.GetVfxContainer()
                    ?.AddChildSafely(
        ...
        if (card.TargetType == TargetType.AnyEnemy)
        {
            if (target == null)
            {
                target = card.Owner.RunState.Rng.CombatTargets.NextItem(
        ...
        {
            IEnumerable<Creature> items = combatState.Allies.Where(
                (Creature c) => c != null && c.IsAlive && c.IsPlayer && c != card.Owner.Creature
            );
            if (target == null)
            {
    ...
    )
    {
        ...
        ICombatState combatState =
            discardCards[0].CombatState ?? discardCards[0].Owner.Creature.CombatState;
    ...
    {
        return (
            await Transform(new CardTransformation(original, replacement).Yield(), null, style)
        ).FirstOrDefault();
    }
    ...
    )
    {
        ...
        List<(CardTransformation, CardPile, int, CardModel)> transformationsWithOriginalData =
            new List<(CardTransformation, CardPile, int, CardModel)>();
        ...
        for (int i = 0; i < array.Length; i++)
        {
            ...
            if (!item.Original.IsTransformable)
            {
                throw new InvalidOperationException(
                    "Can't transform " + item.Original.Id.Entry + " because it's un-transformable."
                );
            }
            ...
            {
                throw new InvalidOperationException(
                    "Can't transform " + item.Original.Id.Entry + " because it has no pile."
                );
            }
            ...
            {
                throw new InvalidOperationException(
                    $"Attempting to transform un-transformable card {item.Original}!"
                );
            }
        ...
        foreach (var item6 in transformationsWithOriginalData)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Attempting to transform card {original} to {replacement2}, but the replacement has a different owner!"
                );
            }
            if (pile2.Type == PileType.Deck)
            {
                List<AbstractModel> modifyingModels;
                CardModel cardAdded = Hook.ModifyCardBeingAddedToDeck(
                    runState,
                    replacement2,
                    out modifyingModels
                );
                ...
                runState
                    .CurrentMapPointHistoryEntry?.GetEntry(original.Owner.NetId)
                    .CardsTransformed.Add(
                        new CardTransformationHistoryEntry(original, replacement2)
                    );
            }
            ...
            else
            {
                ...
                CombatManager.Instance.History.CardGenerated(
                    combatState,
                    replacement2,
                    replacement2.Owner
                );
        ...
        for (int j = 0; j < results.Count; j++)
        {
            ...
            if (cardAdded2.Pile.Type == PileType.Hand)
            {
                if (!TestMode.IsOn)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            $"Couldn't get hand node for original card {transformationsArr[j].Original}!"
                        );
                    }
                    if (playQueue.IsAncestorOf(nCard))
                    {
                        playQueue.RemoveCardFromQueueForCancellation(
                            nCard,
                            forceReturnToHand: true
                        );
                    }
                    hand.TryCancelCardPlay(original2);
                    tasksToAwait.Add(
                        TaskHelper.RunSafely(
                            NCardTransformVfx.PlayAnimOnCardInHand(nCard, cardAdded2)
                        )
                    );
                    await Cmd.Wait(0.2f);
                }
            ...
            else if (style != CardPreviewStyle.None && TestMode.IsOff)
            {
                (
                    (Node)(
                        style switch
                        {
                            CardPreviewStyle.EventLayout => NRun.Instance
                                ?.GlobalUi
                                .EventCardPreviewContainer,
                            CardPreviewStyle.GridLayout => NRun.Instance
                                ?.GlobalUi
                                .GridCardPreviewContainer,
                            CardPreviewStyle.HorizontalLayout => NCombatRoom
                                .Instance
                                ?.Ui
                                .CardPreviewContainer
                                ?? NRun.Instance?.GlobalUi.CardPreviewContainer,
                            CardPreviewStyle.MessyLayout => NCombatRoom
                                .Instance
                                ?.Ui
                    ...
                    NCardTransformVfx.Create(
                        original2,
                        cardAdded2,
                        results[j].modifyingModels?.OfType<RelicModel>()
                    )
                );
            }
        }
        ...
        for (int j = 0; j < results.Count; j++)
        {
            ...
            {
                await Hook.AfterCardGeneratedForCombat(
                    cardPileAddResult.cardAdded.CombatState,
                    cardPileAddResult.cardAdded,
                    cardPileAddResult.cardAdded.Owner
                );
            }
    ...
    public static T? Enchant<T>(CardModel card, decimal amount)
        ...
        where T : AfflictionModel
    {
        ...
        if (cardList.Count > 0 && style != CardPreviewStyle.None)
        {
            ...
            {
                throw new InvalidOperationException(
                    "All cards passed to AfflictAndPreview must have the same owner!"
                );
            }
    ...
    public static async Task<T?> Afflict<T>(CardModel card, decimal amount)
        where T : AfflictionModel
    {
    ...
    )
    {
        ...
        else
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Cannot afflict {card.Id} with {affliction.Id} because it already has affliction {card.Affliction.Id}."
                );
            }
    ...
    public static TaskCompletionSource? Preview(
        CardModel card,
        float time = 1.2f,
        CardPreviewStyle style = CardPreviewStyle.HorizontalLayout
    )
    ...
    )
    {
        if (
            ...
            && result.success
            && LocalContext.IsMine(result.cardAdded)
        )
        {
            PreviewInternal(
                result.cardAdded,
                isAddingCardsToPile: true,
                result.modifyingModels?.OfType<RelicModel>() ?? null,
                time,
                style
            );
        }
    ...
    )
    {
        ...
        {
            Log.Warn(
                "Horizontal layout is being used with more than five cards! They will go offscreen"
            );
        }
    ...
    )
    {
        ...
        tween
            .TweenProperty(node, "scale", Vector2.One, 0.25)
            .From(Vector2.Zero)
                ...
                Callable.From(
                    delegate
                    {
                        ...
                        if (node2 != null)
                        {
                            ...
                            nCardFlyVfx = NCardFlyVfx.Create(
                                node,
                                pileType2,
                                isAddingCardsToPile,
                        ...
                        if (nCardFlyVfx != null && node2 != null)
                        {
                            ...
                            nCardFlyVfx.SwooshAwayCompletion.Task.ContinueWith(
                                delegate
                                {
                                    source.SetResult();
                                }

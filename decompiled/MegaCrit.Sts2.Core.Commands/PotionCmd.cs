...
{
    public static async Task<PotionProcureResult> TryToProcure<T>(Player player)
        where T : PotionModel
    {
    ...
    )
    {
        ...
        if (result.success)
        {
            player
                ...
                .PotionChoices.Add(new ModelChoiceHistoryEntry(potion.Id, wasPicked: true));
    ...
    {
        potion
            .Owner.RunState.CurrentMapPointHistoryEntry?.GetEntry(potion.Owner.NetId)
            .PotionDiscarded.Add(potion.Id);
        potion.Discard();
        await Hook.AfterPotionDiscarded(
            potion.Owner.RunState,
            potion.Owner.Creature.CombatState,
            potion
        );
    }
}

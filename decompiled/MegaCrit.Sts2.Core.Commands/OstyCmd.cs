...
public static class OstyCmd
{
    ...
    )
    {
        ...
        Creature osty = combatState.Allies.FirstOrDefault(
            (Creature c) => c.Monster is Osty && c.PetOwner == summoner
        );
        ...
        else
        {
            ...
            if (isReviving)
            {
                if (osty.IsAlive)
                {
                    throw new InvalidOperationException(
                        "We shouldn't make it here if Osty is still alive!"
                    );
                }
            ...
            else
            {
                ...
                if (ostyNode != null && source is CardModel)
                {
                    ...
                    tween
                        .TweenProperty(ostyNode, "modulate", Colors.White, 0.3499999940395355)
                        .SetDelay(0.10000000149011612);

...
public static class CardPileCmd
{
    public static async Task RemoveFromDeck(CardModel card, bool showPreview = true)
    {
        await RemoveFromDeck(
            new global::_003C_003Ez__ReadOnlySingleElementList<CardModel>(card),
            showPreview
        );
    }
        ...
        foreach (CardModel card in cards)
        {
            if (card.Pile.Type != PileType.Deck)
            {
                throw new InvalidOperationException(
                    "You cannot remove a card that is not in the deck."
                );
            }
            card.Owner.RunState.CurrentMapPointHistoryEntry?.GetEntry(card.Owner.NetId)
                .CardsRemoved.Add(card.ToSerializable());
            ...
            if (showPreview && LocalContext.IsMine(card))
            {
                ...
                if (nCard != null)
                {
                    ...
                    tween
                        .TweenProperty(nCard, "scale", Vector2.One * 1f, 0.25)
                        .From(Vector2.Zero)
                    ...
                    tween
                        .Parallel()
                        .TweenProperty(nCard, "modulate", Colors.Black, 0.2)
                        .SetDelay(1.5);
    ...
    public static async Task RemoveFromCombat(
    ...
    )
    {
        ...
        foreach (CardModel card in cards)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Card must be in a combat pile for it to be removed"
                );
            }
            ...
            for (int i = 0; i < cardNodes.Count; i++)
            {
                ...
                tween
                    ...
                    .TweenProperty(
                        ...
                        (SaveManager.Instance.PrefsSave.FastMode == FastModeType.Fast) ? 0.2f : 0.3f
                    );
            }
            if (tween != null)
            {
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                $"Attempted to add card {card} to pile, but it has no owner!"
            );
        }
    ...
    )
    {
        ...
        return await Add(
            cards,
            newPileType.GetPile(cards.First().Owner),
            position,
            clonedBy,
            skipVisuals
        );
    ...
    )
    {
        ...
        {
            return cards
                .Select((CardModel c) => new CardPileAddResult { cardAdded = c, success = false })
                .ToList();
        }
        List<CardPileAddResult> results = new List<CardPileAddResult>();
        ...
        foreach (CardModel card5 in cards)
        {
            ...
            if (
                card5.HasBeenRemovedFromState
                || creature.IsDead
                || (card5.IsInCombat && creature.CombatState == null)
            )
            {
            ...
            if (newPile.Type == PileType.Deck)
            {
                if (!card5.Owner.RunState.ContainsCard(card5))
                {
                    if (card5.Owner.RunState is NullRunState)
                    {
                        throw new InvalidOperationException(
                            "Tried to add card "
                                + card5.Id.Entry
                                + " to deck for an owner with a NullRunState!"
                        );
                    }
                    throw new InvalidOperationException(
                        card5.Id.Entry
                ...
                && !creature.CombatState.ContainsCard(card5)
            )
            {
                throw new InvalidOperationException(
                    card5.Id.Entry
            ...
            throw new InvalidOperationException(
                "Tried to add cards with different owners to the same pile!"
            );
        }
        ...
        if (newPile.Type == PileType.Deck)
        {
            for (int i = 0; i < results.Count; i++)
            {
                ...
                if (
                    Hook.ShouldAddToDeck(
                        owningPlayer.RunState,
                        result.cardAdded,
                ...
                )
                {
                    ...
                    runState
                        .CurrentMapPointHistoryEntry?.GetEntry(owningPlayer.NetId)
                        .CardsGained.Add(result.cardAdded.ToSerializable());
        ...
        for (int i = 0; i < results.Count; i++)
        {
            ...
            bool isFullHandAdd =
                targetPile.Type == PileType.Hand
                && targetPile.Cards.Count >= CardPile.MaxCardsInHand;
            if (isFullHandAdd)
            {
            ...
            if (TestMode.IsOff && flag && !skipVisuals)
            {
                ...
                flag2 =
                    ...
                    && (isFullHandAdd || oldPile != null || targetPile.Type == PileType.Hand);
            ...
            if (flag6)
            {
                ...
                flag5 = (
                    (type == PileType.Draw || type == PileType.Discard || type == PileType.Deck)
                        ? true
                        : false
                );
                flag6 = flag5;
            ...
            {
                cardNode = CreateCardNodeAndUpdateVisuals(
                    card,
                    targetPile.Type,
                    owningPlayerIsLocal
                );
            ...
            else if (targetPile.Type == PileType.Deck)
            {
                ...
                CardModel cardModel = Hook.ModifyCardBeingAddedToDeck(
                    card.Owner.RunState,
                    card,
                    out modifyingModels
                );
            ...
            targetPile.AddInternal(
                card2,
                position switch
                {
                    CardPilePosition.Bottom => -1,
                    CardPilePosition.Top => 0,
                    CardPilePosition.Random => card.Owner.RunState.Rng.Shuffle.NextInt(
                        targetPile.Cards.Count + 1
                    ),
            ...
            {
                ThinkCmd.Play(
                    new LocString("combat_messages", "HAND_FULL"),
                    owningPlayer.Creature,
                    2.0
                );
            }
            if (
                oldPile == null
                || oldPile.Type != PileType.Play
                || newPile.Type == PileType.Hand
                || card.IsDupe
            )
            {
        ...
        if (cardNodes.Count != 0)
        {
            ...
            foreach (NCard cardNode2 in cardNodes)
            {
                ...
                CardPile oldPile2 = results
                    .Find((CardPileAddResult r) => r.cardAdded == card3)
                    .oldPile;
                ...
                if (flag8)
                {
                    tween
                        ...
                        .TweenProperty(
                            cardNode2,
                            "position",
                            cardNode2.Position + Vector2.Down * 25f,
                ...
                switch (card3.Pile.Type)
                {
                    case PileType.Exhaust:
                        ...
                        if (
                            oldPile2 != null
                            && oldPile2.Type != PileType.Hand
                            && oldPile2.Type != PileType.Play
                        )
                        {
                        ...
                        tween
                            ...
                            .TweenCallback(
                                Callable.From(
                                    delegate
                                    {
                                        NCombatRoom.Instance.Ui.AddChildSafely(
                                            NExhaustVfx.Create(cardNode2)
                                        );
                                    }
                                )
                            );
                    ...
                    default:
                        tween.TweenCallback(
                            Callable.From(
                                ...
                                {
                                    Node node = (
                                        (card3.Pile.Type != PileType.Deck)
                                            ? card3.Owner.Creature.GetVfxContainer()
                                            : NRun.Instance.GlobalUi.TopBar.TrailContainer
                                    );
                                    cardNode2.Reparent(node);
                                    NCardFlyVfx child2 = NCardFlyVfx.Create(
                                        cardNode2,
                                        card3.Pile.Type,
                                        isAddingToPile: true,
                                        card3.Owner.Character.TrailPath
                                    );
        ...
        if (cardsWithoutNodesChangingPiles.Count != 0)
        {
            ...
            {
                CardPile oldPile3 = results
                    .Find((CardPileAddResult r) => r.cardAdded == card4)
                    .oldPile;
                Node vfxContainer = (
                    (card4.Pile.Type != PileType.Deck)
                        ? card4.Owner.Creature.GetVfxContainer()
                        : NRun.Instance.GlobalUi.TopBar.TrailContainer
                );
                if (tween != null)
                {
                    tween.TweenCallback(
                        Callable.From(
                            delegate
                            {
                                NCardFlyShuffleVfx child2 = NCardFlyShuffleVfx.Create(
                                    oldPile3,
                                    card4.Pile,
                                    card4.Owner.Character.TrailPath
                                );
                ...
                {
                    NCardFlyShuffleVfx child = NCardFlyShuffleVfx.Create(
                        oldPile3,
                        card4.Pile,
                        card4.Owner.Character.TrailPath
                    );
        ...
        foreach (CardPileAddResult item3 in results)
        {
            if (item3.success)
            {
                ...
                await Hook.AfterCardChangedPiles(
                    cardAdded.Owner.RunState,
                    cardAdded.CombatState,
                    cardAdded,
                    item3.oldPile?.Type ?? PileType.None,
                    clonedBy
                );
            }
        }
    ...
    public static async Task AddDuringManualCardPlay(CardModel card)
    {
        ...
        {
            throw new InvalidOperationException(
                card.Id.Entry + " must be added to a CombatState before playing it."
            );
        }
    ...
    )
    {
        ...
        {
            nCard.Position = NCombatRoom
                .Instance.GetCreatureNode(card.Owner.Creature)
                .IntentContainer.GlobalPosition;
        }
    ...
    )
    {
        ...
        {
            tween
                .TweenProperty(cardNode, "position", targetPosition, num)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Cubic);
        }
        ...
        {
            tween
                .TweenProperty(cardNode, "scale", Vector2.One * 0.8f, 0.25)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Cubic);
        }
        ...
        {
            tween
                .TweenProperty(cardNode, "scale", Vector2.One, num)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Cubic)
                .From(Vector2.Zero);
        }
    ...
    public static async Task Shuffle(PlayerChoiceContext choiceContext, Player player)
    {
        ...
        if (CombatManager.Instance.DebugForcedTopCardOnNextShuffle != null)
        {
            if (!list.Remove(CombatManager.Instance.DebugForcedTopCardOnNextShuffle))
            {
                throw new InvalidOperationException(
                    "Could not find card "
                        + CombatManager.Instance.DebugForcedTopCardOnNextShuffle.Id.Entry
                        + " in discard pile."
                );
            }
        ...
        foreach (CardModel item2 in list)
        {
            if (!drawPileCards.Contains(item2))
            {
                ...
                float num =
                    timeBetweenCardAdds
                    + Rng.Chaotic.NextFloat(
                        (0f - randomTimeBetweenCardAdds) * 0.5f,
                        randomTimeBetweenCardAdds * 0.5f
                    );
                waitTimeAccumulator += num;
                if (
                    (double)waitTimeAccumulator
                    >= ((SceneTree)Engine.GetMainLoop()).Root.GetProcessDeltaTime()
                )
                {
                    await Cmd.Wait(num);
    ...
    )
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            CardModel cardModel = position switch
            {
                ...
                CardPilePosition.Random => player.RunState.Rng.CombatCardSelection.NextItem(
                    drawPile.Cards
                ),
                _ => throw new ArgumentOutOfRangeException("position", position, null),
            };
        ...
        where T : CardModel
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            CardModel cardModel = (
                (combatState != null) ? combatState.CreateCard<T>(player) : null
            );
            if (cardModel != null)
            {
    ...
    public static async Task<CardModel?> AddCurseToDeck<T>(Player owner)
        where T : CardModel
    {
        return (
            await AddCursesToDeck(
                new global::_003C_003Ez__ReadOnlySingleElementList<CardModel>(ModelDb.Card<T>()),
                owner
            )
        )
            .FirstOrDefault()
    ...
    {
        if (
            PileType.Draw.GetPile(player).Cards.Count + PileType.Discard.GetPile(player).Cards.Count
            == 0
        )
        {

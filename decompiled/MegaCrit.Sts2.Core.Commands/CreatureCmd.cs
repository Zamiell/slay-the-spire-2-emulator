...
public static class CreatureCmd
{
    ...
    public static async Task Add(Creature creature)
    {
        ...
        if (combatState == null)
        {
            throw new InvalidOperationException(
                "Attempted to add a creature with no combat state."
            );
        }
        ...
        if (combatState.CurrentSide != CombatSide.Enemy && creature.IsMonster)
        {
            creature.PrepareForNextTurn(
                combatState.Players.Select((Player p) => p.Creature),
                rollNewMove: false
            );
        }
        MapPointRoomHistoryEntry mapPointRoomHistoryEntry =
            combatState.RunState.CurrentMapPointHistoryEntry?.Rooms.Last();
        if (
            mapPointRoomHistoryEntry != null
            && creature.Monster != null
            && !mapPointRoomHistoryEntry.MonsterIds.Contains(creature.Monster.Id)
        )
        {
    ...
    )
    {
        return await Damage(
            choiceContext,
            new global::_003C_003Ez__ReadOnlySingleElementList<Creature>(target),
            amount,
            props,
            dealer,
            null
    ...
    )
    {
        ...
        IRunState runState = IRunState.GetFrom(
            targetList.Append<Creature>(dealer).OfType<Creature>()
        );
        foreach (Creature originalTarget in targetList)
        {
            ...
            decimal unblockedDamage = Hook.ModifyHpLost(
                runState,
                combatState,
                originalTarget,
            ...
            Creature unblockedDamageTarget = (
                (combatState == null)
                    ? originalTarget
            ...
            DamageResult unblockedDamageResult = unblockedDamageTarget.LoseHpInternal(
                unblockedDamage,
                props
            );
            ...
            {
                decimal originalTargetDamage = Hook.ModifyHpLost(
                    runState,
                    combatState,
                    originalTarget,
                ...
                DamageResult damageResult = (
                    (!(originalTargetDamage > 0m))
                        ? new DamageResult(originalTarget, props)
                        : originalTarget.LoseHpInternal(originalTargetDamage, props)
                );
            ...
            foreach (DamageResult item in damageResults)
            {
                ...
                if (damage > 0)
                {
                    ...
                    MapPointHistoryEntry mapPointHistoryEntry = receiver
                        .Player
                        ?.RunState
                        .CurrentMapPointHistoryEntry;
                    if (mapPointHistoryEntry != null)
                    {
                        mapPointHistoryEntry.GetEntry(receiver.Player.NetId).DamageTaken +=
                            item.UnblockedDamage;
                    }
                }
                ...
                if (
                    LocalContext.IsMe(originalTarget)
                    && (
                        !CombatManager.Instance.IsInProgress
                        || originalTarget.GetHpPercentRemaining() <= 0.25
                    )
                )
                {
                ...
                {
                    SfxCmd.PlayDamage(
                        originalTarget.Monster,
                        unblockedDamageResult.UnblockedDamage
                    );
                }
        ...
        foreach (DamageResult unblockedDamageResult in results)
        {
            ...
            {
                await Hook.AfterDamageReceived(
                    choiceContext,
                    runState,
                    combatState,
                    originalTarget,
            ...
            if (unblockedDamageResult.WasFullyBlocked && CombatManager.Instance.IsInProgress)
            {
                ...
                vfxContainer2?.AddChildSafely(
                    NBlockSparkVfx.Create(unblockedDamageResult.Receiver)
                );
                vfxContainer2?.AddChildSafely(
                    NDamageBlockedVfx.Create(unblockedDamageResult.Receiver)
                );
                NGame.Instance?.ScreenShake(ShakeStrength.Weak, ShakeDuration.Short);
            }
        }
    ...
    {
        if (
            (creature.CombatState == null && !creature.IsPlayer)
            || (creature.CombatState != null && !creature.CombatState.IsLiveCombat())
        )
        {
            return;
        }
        ICombatState combatState = creature.CombatState;
        IRunState runState = IRunState.GetFrom(
            new global::_003C_003Ez__ReadOnlySingleElementList<Creature>(creature)
        );
        ...
        )
        {
            ...
            if (nCreature != null)
            {
                deathAnimLength = nCreature.StartDeathAnim(
                    shouldRemoveFromCombat && creature.IsMonster
                );
            ...
            List<Creature> teammates =
                (from t in combatState?.GetTeammatesOf(creature) where t.IsAlive select t).ToList()
                ?? new List<Creature>();
            if (
                shouldRemoveFromCombat
                && creature.Side == CombatSide.Enemy
                && (combatState?.Enemies.Contains(creature) ?? false)
            )
            {
            ...
            if (creature.Side == CombatSide.Enemy)
            {
                if (
                    isPrimaryEnemy
                    && teammates.Count != 0
                    && teammates.All((Creature t) => t.IsSecondaryEnemy)
                )
                {
            ...
            else if (creature.IsPlayer)
            {
                ...
                if (
                    combatState != null
                    && !combatState.Players.All((Player p) => p.Creature.IsDead)
                )
                {
        ...
        else
        {
            ...
            {
                throw new InvalidOperationException(
                    "Combat is ending, but something is continually preventing the last creature from being killed!"
                );
            }
    ...
    public static async Task Heal(Creature creature, decimal amount, bool playAnim = true)
    {
        ...
        if (playAnim)
        {
            if (
                CombatManager.Instance.IsInProgress
                || creature.Player?.RunState.CurrentRoom is CombatRoom
            )
            {
                if (creature != null && creature.Monster is Osty)
                {
            ...
            else if (creature.Player?.RunState.CurrentRoom is RestSiteRoom && TestMode.IsOff)
            {
                NRestSiteCharacter nRestSiteCharacter = NRestSiteRoom.Instance?.Characters.First(
                    (NRestSiteCharacter c) => c.Player == creature.Player
                );
        ...
        MapPointHistoryEntry mapPointHistoryEntry = creature
            .Player
            ?.RunState
            .CurrentMapPointHistoryEntry;
        ...
        {
            await Hook.AfterCurrentHpChanged(
                creature.Player?.RunState ?? creature.CombatState.RunState,
    ...
    public static async Task GainMaxHp(Creature creature, decimal amount)
    {
        if (amount < 0m)
        {
            throw new ArgumentException(
                "amount must be non-negative. Use LoseMaxHp for max HP loss."
            );
        }
    ...
    )
    {
        ...
        {
            throw new ArgumentException(
                "amount must be non-negative. Use GainMaxHp for max HP gain."
            );
        }
        ...
        if (newMaxHp < (decimal)creature.CurrentHp)
        {
            await Damage(
                choiceContext,
                creature,
                (decimal)creature.CurrentHp - newMaxHp,
                isFromCard
    ...
    public static async Task TriggerAnim(Creature creature, string triggerName, float waitTime)
    {
        ...
        if (creatureNode == null)
        {
            ...
            {
                Log.Error(
                    $"Attempted to play animation on creature {creature} but its creature node doesn't exist!"
                );
            }
            return;
        }

...
public static class PowerCmd
{
    ...
    )
    {
        ...
        await Hook.BeforePowerAmountChanged(
            combatState,
            power,
            amount,
            target,
    ...
    )
    {
        return basePower.InstanceType switch
        {
            ...
            PowerInstanceType.InstancedPerApplier => target
                .GetPowerInstances(basePower.Id)
                .FirstOrDefault((PowerModel p) => p.Applier == applier),
    ...
    )
    {
        ...
        if (applier != null && combatState.ContainsCreature(applier))
        {
            modifiedOffset = Hook.ModifyPowerAmountGiven(
                combatState,
                power,
                applier,
                modifiedOffset,
    ...
    public static async Task Remove<T>(Creature creature)
        where T : PowerModel
    {

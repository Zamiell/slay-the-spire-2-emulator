...
public static class CardSelectCmd
{
    ...
    private static readonly Stack<MegaCrit.Sts2.Core.TestSupport.ICardSelector> _selectorStack =
        new Stack<MegaCrit.Sts2.Core.TestSupport.ICardSelector>();
    ...
    public static void Reset()
    {
        ...
        {
            Log.Warn(
                $"CardSelectCmd.Reset: clearing {_selectorStack.Count} leaked selector(s) from the stack."
            );
            _selectorStack.Clear();
        }
    }
    ...
    {
        string text =
            "A selection screen was about to be shown with 0 options. Returning empty to prevent softlock.";
        Log.Error(text);
    ...
    )
    {
        ...
        if (ShouldSelectLocalCard(player))
        {
            ...
            {
                NChooseACardSelectionScreen nChooseACardSelectionScreen =
        ...
        else
        {
            ...
            if (ShouldSelectLocalCard(player))
            {
                if (Selector != null)
                {
                    ...
                    result = (
                        await Selector.GetSelectedCards(options, prefs.MinSelect, prefs.MaxSelect)
                    ).ToList();
                }
                else
                {
                    NSimpleCardSelectScreen nSimpleCardSelectScreen =
                        NSimpleCardSelectScreen.Create(cards, prefs);
                ...
                List<int> indexes = result
                    .Select((CardModel c) => cards.FindIndex((CardCreationResult r) => r.Card == c))
                    .ToList();
    ...
    )
    {
        ...
        else
        {
            ...
            if (ShouldSelectLocalCard(player))
            {
                ...
                {
                    result = (
                        await Selector.GetSelectedCards(cards, prefs.MinSelect, prefs.MaxSelect)
                    ).ToList();
                }
                else
                {
        ...
        else
        {
            ...
            if (ShouldSelectLocalCard(player))
            {
                if (Selector != null)
                {
                    ...
                    {
                        list = (from c in list orderby c.Rarity, c.Id select c).ToList();
                    }
                    result = (
                        await Selector.GetSelectedCards(list, prefs.MinSelect, prefs.MaxSelect)
                    ).ToList();
                }
                else
                {
                    NPlayerHand.Instance?.CancelAllCardPlay();
                    NCombatPileCardSelectScreen nCombatPileCardSelectScreen =
    ...
    {
        List<CardModel> list = PileType
            .Deck.GetPile(player)
            .Cards.Where((CardModel c) => c.IsUpgradable)
            .ToList();
        if (list.Count == 0)
        {
            return Array.Empty<CardModel>();
        ...
        else
        {
            ...
            if (ShouldSelectLocalCard(player))
            {
                if (Selector != null)
                {
                    enumerable = await Selector.GetSelectedCards(
                        list,
                        prefs.MinSelect,
                        prefs.MaxSelect
                    );
                ...
                {
                    NDeckUpgradeSelectScreen nDeckUpgradeSelectScreen =
                        NDeckUpgradeSelectScreen.ShowScreen(list, prefs, player.RunState);
    ...
    )
    {
        List<CardModel> list = PileType
            .Deck.GetPile(player)
            .Cards.Where((CardModel c) => c.Type != CardType.Quest && c.IsTransformable)
            .ToList();
        if (list.Count == 0)
        {
            return Array.Empty<CardModel>();
        ...
        else
        {
            ...
            if (ShouldSelectLocalCard(player))
            {
                ...
                else
                {
                    ...
                    NDeckTransformSelectScreen nDeckTransformSelectScreen =
                        NDeckTransformSelectScreen.ShowScreen(list, cardToTransformation, prefs);
    ...
    {
        IReadOnlyList<CardModel> cards = PileType
            .Deck.GetPile(player)
            .Cards.Where(
                (CardModel c) => enchantment.CanEnchant(c) && (additionalFilter?.Invoke(c) ?? true)
            )
            .ToList();
    ...
    )
    {
        ...
        else
        {
            ...
            if (ShouldSelectLocalCard(player))
            {
                Dictionary<CardModel, int> indexMap = PileType
                    .Deck.GetPile(player)
                    .Cards.Select((CardModel card, int index) => new { card, index })
                ...
                else
                {
                    NDeckEnchantSelectScreen nDeckEnchantSelectScreen =
                        NDeckEnchantSelectScreen.ShowScreen(list, enchantment, amount, prefs);
    ...
    )
    {
        ...
        return FromDeckGeneric(
            player,
            prefs,
            (CardModel c) => c.IsRemovable && (filter == null || filter(c)),
            (CardModel c) => (c.Type != CardType.Curse) ? deck.IndexOf(c) : (-999999999)
        );
    }
    ...
    )
    {
        ...
        else
        {
            ...
            if (ShouldSelectLocalCard(player))
            {
                ...
                {
                    NDeckCardSelectScreen nDeckCardSelectScreen = NDeckCardSelectScreen.Create(
                        list,
                        prefs
                    );
    ...
    )
    {
        ...
        List<CardModel> list = PileType
            .Hand.GetPile(player)
            .Cards.Where(filter ?? ((Func<CardModel, bool>)((CardModel _) => true)))
            .ToList();
        IEnumerable<CardModel> result;
        if (list.Count == 0)
        {
    ...
    )
    {
        ...
        else
        {
            ...
            {
                result = (
                    await NCombatRoom.Instance.Ui.Hand.SelectCards(
                        new CardSelectorPrefs(
                            new LocString("gameplay_ui", "CHOOSE_CARD_UPGRADE_HEADER"),
                            1
                        ),
                        (CardModel c) => c.IsUpgradable,
                        source,
                ...
                RunManager.Instance.PlayerChoiceSynchronizer.SyncLocalChoice(
                    player,
                    choiceId,
                    PlayerChoiceResult.FromMutableCombatCard(result)
                );
            }
            else
            {
    ...
    )
    {
        ...
        {
            NChooseABundleSelectionScreen nChooseABundleSelectionScreen =
                NChooseABundleSelectionScreen.ShowScreen(bundles);
            readOnlyList =
                (await nChooseABundleSelectionScreen.CardsSelected()).FirstOrDefault()
                ?? Array.Empty<CardModel>();
            RunManager.Instance.PlayerChoiceSynchronizer.SyncLocalChoice(
                player,
                choiceId,
                PlayerChoiceResult.FromIndex(
                    bundles.IndexOf<IReadOnlyList<CardModel>>(readOnlyList)
                )
            );
        }
        else
    ...
    {
        string value = string.Join(",", from c in cards.OfType<CardModel>() select c.Id.Entry);

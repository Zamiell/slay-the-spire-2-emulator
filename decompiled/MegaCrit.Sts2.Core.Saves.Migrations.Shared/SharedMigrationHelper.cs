...
{
    public static readonly IReadOnlyDictionary<string, string> V100Renames = new Dictionary<
        string,
        string
    >
    {
    ...
    public static void ReplaceModelIds(JsonNode? node, IReadOnlyDictionary<string, string> renames)
    {
        if (!(node is JsonObject jsonObject))
        {
            ...
            for (int i = 0; i < jsonArray.Count; i++)
            {
                ...
                if (
                    ...
                    && renames.TryGetValue(value, out string value2)
                )
                {
                    jsonArray[i] = value2;
                }
        ...
        foreach (
            string item in new List<string>(
                jsonObject.Select<KeyValuePair<string, JsonNode>, string>(
                    (KeyValuePair<string, JsonNode> kvp) => kvp.Key
                )
            )
        )
        {
            ...
            if (
                jsonNode2 is JsonValue jsonValue2
                && jsonValue2.TryGetValue<string>(out string value3)
                && renames.TryGetValue(value3, out string value4)
            )
            {
    ...
    public static void MigrateMapPointHistoryRooms(JsonNode? jsonNode)
    {
        ...
        foreach (JsonNode item2 in jsonArray)
        {
            ...
            {
                if (
                    !(item3 is JsonObject jsonObject)
                    || !jsonObject.TryGetPropertyValue("room_types", out JsonNode jsonNode2)
                    || !(jsonNode2 is JsonArray jsonArray3)
                )
                {
                    continue;
                }
                ...
                foreach (JsonNode item4 in jsonArray3)
                {
                    if (item4 != null)
                    {
                        JsonObject item = new JsonObject { { "room_type", item4.DeepClone() } };
    ...
    public static void MigrateMapPointHistoryCardChoices(JsonNode? jsonNode)
    {
        ...
        foreach (JsonNode item in jsonArray)
        {
            ...
            {
                if (
                    !(item2 is JsonObject jsonObject)
                    || !jsonObject.TryGetPropertyValue("player_stats", out JsonNode jsonNode2)
                    || !(jsonNode2 is JsonArray jsonArray3)
                )
                {
                    continue;
                ...
                foreach (JsonNode item3 in jsonArray3)
                {
                    ...
                    if (
                        jsonObject2.TryGetPropertyValue("cards_gained", out JsonNode jsonNode3)
                        && jsonNode3 is JsonArray jsonArray4
                    )
                    {
                    ...
                    if (
                        jsonObject2.TryGetPropertyValue("cards_removed", out JsonNode jsonNode4)
                        && jsonNode4 is JsonArray jsonArray6
                    )
                    {
                    ...
                    if (
                        !jsonObject2.TryGetPropertyValue(
                            "cards_transformed",
                            out JsonNode jsonNode7
                        ) || !(jsonNode7 is JsonArray jsonArray10)
                    )
                    {
                        continue;
                    }

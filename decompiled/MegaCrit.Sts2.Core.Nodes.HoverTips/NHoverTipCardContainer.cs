...
public class NHoverTipCardContainer : Control
{
    ...
    public void Add(CardHoverTip cardTip)
    {
        Control control = PreloadManager
            .Cache.GetScene("res://scenes/ui/card_hover_tip.tscn")
    ...
    public void LayoutResizeAndReposition(Vector2 globalStartLocation, HoverTipAlignment alignment)
    {
        ...
        foreach (Control tip in Tips)
        {
            tip.Position = zero;
            size2 = new Vector2(
                Mathf.Max(zero.X + tip.Size.X, size2.X),
                Mathf.Max(zero.Y + tip.Size.Y, size2.Y)
            );
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.LayoutResizeAndReposition,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,

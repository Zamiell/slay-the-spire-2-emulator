...
public class NMapPointHistoryHoverTip : MarginContainer
{
    ...
    private readonly LocString _mapPointRoomStatsLoc = new LocString(
        "run_history",
        "MAP_POINT_HISTORY.room_stats"
    );

    private readonly LocString _mapPointPlayerStatsLoc = new LocString(
        "run_history",
        "MAP_POINT_HISTORY.player_stats"
    );

    private readonly LocString _combatStats = new LocString(
        "run_history",
        "MAP_POINT_HISTORY.combatStats"
    );

    private readonly LocString _floorTitle = new LocString(
        "run_history",
        "MAP_POINT_HISTORY.header"
    );
    ...
    private readonly LocString _rewardsHeaderLoc = new LocString(
        "run_history",
        "HISTORY_ENTRY.rewardsHeader"
    );

    private readonly LocString _skippedHeaderLoc = new LocString(
        "run_history",
        "HISTORY_ENTRY.skippedHeader"
    );
    ...
    private readonly LocString _goldGained = new LocString(
        "run_history",
        "HISTORY_ENTRY.goldGained"
    );
    ...
    private readonly LocString _goldStolen = new LocString(
        "run_history",
        "HISTORY_ENTRY.goldStolen"
    );
    ...
    private readonly LocString _transformed = new LocString(
        "run_history",
        "HISTORY_ENTRY.transformed"
    );

    private readonly LocString _upgraded = new LocString("run_history", "HISTORY_ENTRY.upgraded");

    private readonly LocString _downgraded = new LocString(
        "run_history",
        "HISTORY_ENTRY.downgraded"
    );

    private readonly LocString _damaged = new LocString(
        "run_history",
        "MAP_POINT_HISTORY.damageTaken"
    );

    private readonly LocString _healed = new LocString("run_history", "MAP_POINT_HISTORY.healed");

    private readonly LocString _maxHpGained = new LocString(
        "run_history",
        "MAP_POINT_HISTORY.maxHpGained"
    );

    private readonly LocString _maxHpLost = new LocString(
        "run_history",
        "MAP_POINT_HISTORY.maxHpLost"
    );

    private readonly LocString _turns = new LocString(
        "run_history",
        "MAP_POINT_HISTORY.turnsTaken"
    );

    private readonly LocString _quests = new LocString(
        "run_history",
        "MAP_POINT_HISTORY.questCompleted"
    );
    ...
    private const string _roomHistoryTipScenePath =
        "res://scenes/ui/map_point_history_hover_tip.tscn";
    ...
    {
        NMapPointHistoryHoverTip nMapPointHistoryHoverTip = PreloadManager
            .Cache.GetScene("res://scenes/ui/map_point_history_hover_tip.tscn")
            .Instantiate<NMapPointHistoryHoverTip>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        _rewardRows = GetNode<Control>("%RewardRows")
            .GetChildren()
            .OfType<RichTextLabel>()
            .ToList();
        _skippedRows = GetNode<Control>("%SkippedRows")
            .GetChildren()
            .OfType<RichTextLabel>()
            .ToList();
        _rewardStatsContainer
            .GetNode<MegaLabel>("Header")
            .SetTextAutoSize(_rewardsHeaderLoc.GetFormattedText());
        _skippedStatsContainer
            .GetNode<MegaLabel>("Header")
            .SetTextAutoSize(_skippedHeaderLoc.GetFormattedText());
        ...
        _mapPointRoomStatsLoc.Add(
            "MapPointType",
            new LocString("static_hover_tips", text + ".title")
        );
        switch (_entry.Rooms.First().RoomType)
        {
            case RoomType.Event:
                _mapPointRoomStatsLoc.Add(
                    "ModelTitle",
                    SaveUtil.EventOrDeprecated(_entry.Rooms.First().ModelId).Title
                );
                break;
            ...
            default:
                _mapPointRoomStatsLoc.Add(
                    "ModelTitle",
                    SaveUtil.EncounterOrDeprecated(_entry.Rooms.First().ModelId).Title
                );
                break;
        }
        _roomStats.Text = _mapPointRoomStatsLoc.GetFormattedText();
        PlayerMapPointHistoryEntry playerMapPointHistoryEntry = _entry.PlayerStats.FirstOrDefault(
            (PlayerMapPointHistoryEntry e) => e.PlayerId == _playerId
        );
        if (playerMapPointHistoryEntry == null)
        {
            throw new InvalidOperationException(
                $"Player with ID {_playerId} not found in player stats for this run history!"
            );
        }
    ...
    private void PopulateActionStats(PlayerMapPointHistoryEntry playerEntry)
    {
        ...
        if (_entry.MapPointType == MapPointType.Ancient)
        {
            ...
            if (ancientPickedChoiceLoc != null)
            {
                ...
                StringBuilder.AppendInterpolatedStringHandler handler =
                    new StringBuilder.AppendInterpolatedStringHandler(2, 1, stringBuilder2);
        ...
        else
        {
            ...
            EventModel eventModel = (
                (modelId != null) ? SaveUtil.EventOrDeprecated(modelId) : null
            );
            if (eventModel != null)
            {
        ...
        MapPointRoomHistoryEntry mapPointRoomHistoryEntry = _entry.Rooms.FirstOrDefault(
            (MapPointRoomHistoryEntry r) => r.RoomType.IsCombatRoom()
        );
        ...
        foreach (ModelId item2 in playerEntry.PotionUsed)
        {
            ...
            StringBuilder.AppendInterpolatedStringHandler handler =
                new StringBuilder.AppendInterpolatedStringHandler(1, 1, stringBuilder2);
        ...
        if (playerEntry.GoldStolen > 0)
        {
            _goldStolen.Add(
                "Icon",
                "[img=top]res://images/packed/sprite_fonts/gold_icon.png[/img]"
            );
    ...
    private void PopulateRewardAndSkippedEntries(PlayerMapPointHistoryEntry playerEntry)
    {
        ...
        foreach (CardChoiceHistoryEntry cardChoice in playerEntry.CardChoices)
        {
            if (!cardChoice.wasPicked)
            {
                _obtained.Add(
                    "Icon",
                    "[img=top]res://images/packed/sprite_fonts/card_icon.png[/img]"
                );
        ...
        foreach (ModelChoiceHistoryEntry potionChoice in playerEntry.PotionChoices)
        {
            _obtained.Add(
                "Icon",
                "[img=top]res://images/packed/sprite_fonts/potion_icon.png[/img]"
            );
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._rewardStatsContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._skippedStatsContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._rewardStatsContainer,
            Variant.From(in _rewardStatsContainer)
        );
        info.AddProperty(
            PropertyName._skippedStatsContainer,
            Variant.From(in _skippedStatsContainer)
        );

...
public abstract class EventModel : AbstractModel
{
    ...
    public virtual LocString InitialDescription =>
        L10NLookup(base.Id.Entry + ".pages.INITIAL.description");
    ...
    public virtual IEnumerable<LocString> GameInfoOptions
    {
        ...
        {
            List<LocString> list = (
                from k in LocManager.Instance.GetTable(LocTable).Keys
                where k.StartsWith(base.Id.Entry + ".pages.INITIAL.options")
                select new LocString(LocTable, k)
            ).ToList();
            if (list.Count == 0)
            {
                throw new LocException(
                    "Event Loc for " + base.Id.Entry + " does not conform to the common format"
                );
            }
    ...
    private string LayoutScenePath =>
        LayoutType switch
        {
            ...
            EventLayoutType.Custom => SceneHelper.GetScenePath(
                "events/custom/" + base.Id.Entry.ToLowerInvariant()
            ),
    ...
    private string InitialPortraitPath =>
        ImageHelper.GetImagePath("events/" + base.Id.Entry.ToLowerInvariant() + ".png");

    private string InitialPhobiaModePortraitPath =>
        ImageHelper.GetImagePath("events/" + base.Id.Entry.ToLowerInvariant() + "_phobia_mode.png");

    public bool HasPhobiaModePortrait => ResourceLoader.Exists(InitialPhobiaModePortraitPath);

    private string BackgroundScenePath =>
        SceneHelper.GetScenePath("events/background_scenes/" + base.Id.Entry.ToLowerInvariant());

    private string VfxPath =>
        SceneHelper.GetScenePath("vfx/events/" + base.Id.Entry.ToLowerInvariant() + "_vfx");
    ...
    public async Task BeginEvent(Player player, bool isPreFinished)
    {
        ...
        if (Owner != null)
        {
            throw new InvalidOperationException(
                "Tried to begin event, but it already has an owner!"
            );
        }
        Owner = player;
        Rng = new Rng(
            (uint)(
                ...
                + (ulong)StringHelper.GetDeterministicHashCode(base.Id.Entry)
            )
        );
        try
        {
    ...
    protected virtual void SetInitialEventState(bool isPreFinished)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to load into pre-finished event {this}! Only ancient events can be pre-finished."
            );
        }
    ...
    public ICombatRoomVisuals CreateCombatRoomVisuals(IEnumerable<Player> players, ActModel act)
    {
        ...
        {
            throw new InvalidOperationException(
                "Tried to create combat room visuals for non-combat event!"
            );
        }
    ...
    public void GenerateInternalCombatState(IRunState runState)
    {
        ...
        {
            throw new InvalidOperationException(
                "Tried to generate internal encounter for non-combat event!"
            );
        }
        _mutableEncounter = CanonicalEncounter.ToMutable();
        _mutableEncounter.GenerateMonstersWithSlots(runState);
        _combatStateForCombatLayout = new CombatState(
            _mutableEncounter,
            runState,
            runState.Modifiers,
            runState.BadgeModels,
            runState.MultiplayerScalingModel
        );
        ...
        foreach (var monstersWithSlot in _combatStateForCombatLayout.Encounter.MonstersWithSlots)
        {
            ...
            Creature creature = _combatStateForCombatLayout.CreateCreature(
                item,
                CombatSide.Enemy,
                item2
            );
    ...
    public void ResetInternalCombatState()
    {
        ...
        {
            throw new InvalidOperationException(
                "Tried to reset internal encounter for non-combat event!"
            );
        }
    ...
    protected virtual void SetEventState(
    ...
    )
    {
        ...
        if (_currentOptions.Count == 0)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Tried to set event options after event was finished!"
                );
            }
            _isFinished = true;
        }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to enter combat in non-shared event {this}!"
            );
        }
        if (shouldResumeAfterCombat && LayoutType == EventLayoutType.Combat)
        {
            throw new InvalidOperationException(
                $"Cannot resume event {base.Id} after combat because it has a Combat layout — "
                    + "there is no event layout to return to."
            );
        }
        ...
        CombatState combatState = (
            (LayoutType != EventLayoutType.Combat)
                ? new CombatState(
                    mutableEncounter,
                    Owner.RunState,
                    Owner.RunState.Modifiers,
                    Owner.RunState.BadgeModels,
                    Owner.RunState.MultiplayerScalingModel
                )
    ...
    protected EventOption RelicOption<T>(Func<Task>? onChosen, string pageName = "INITIAL")
        where T : RelicModel
    {
    ...
    protected EventOption RelicOption(
        RelicModel relic,
        Func<Task>? onChosen,

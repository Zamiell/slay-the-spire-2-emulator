...
public abstract class ActModel : AbstractModel
{
    ...
    public string RestSiteBackgroundPath =>
        SceneHelper.GetScenePath("rest_site/" + FilePathIdentifier + "_rest_site");

    public string MapTopBgPath =>
        ImageHelper.GetImagePath(
            $"packed/map/map_bgs/{FilePathIdentifier}/map_top_{FilePathIdentifier}.png"
        );
    ...
    public string MapMidBgPath =>
        ImageHelper.GetImagePath(
            $"packed/map/map_bgs/{FilePathIdentifier}/map_middle_{FilePathIdentifier}.png"
        );
    ...
    public string MapBotBgPath =>
        ImageHelper.GetImagePath(
            $"packed/map/map_bgs/{FilePathIdentifier}/map_bottom_{FilePathIdentifier}.png"
        );
    ...
    public IEnumerable<EncounterModel> AllEncounters =>
        _allEncounters ?? (_allEncounters = GenerateAllEncounters());

    public IEnumerable<EncounterModel> AllWeakEncounters =>
        _allWeakEncounters
        ?? (
            _allWeakEncounters = AllEncounters.Where(
                (EncounterModel e) => e != null && e.RoomType == RoomType.Monster && e.IsWeak
            )
        );

    public IEnumerable<EncounterModel> AllRegularEncounters =>
        _allRegularEncounters
        ?? (
            _allRegularEncounters = AllEncounters.Where(
                (EncounterModel e) => e != null && e.RoomType == RoomType.Monster && !e.IsWeak
            )
        );
    ...
    public IEnumerable<EncounterModel> AllEliteEncounters =>
        _allEliteEncounters
        ?? (
            _allEliteEncounters = AllEncounters.Where(
                (EncounterModel e) => e.RoomType == RoomType.Elite
            )
        );
    ...
    public IEnumerable<EncounterModel> AllBossEncounters =>
        _allBossEncounters
        ?? (
            _allBossEncounters = AllEncounters.Where(
    ...
    public IEnumerable<MonsterModel> AllMonsters =>
        _allMonsters
        ?? (
            _allMonsters = AllEncounters
                .SelectMany((EncounterModel e) => e.AllPossibleMonsters)
                .Distinct()
        );

    public Achievement DefeatedAllEnemiesAchievement =>
        Enum.Parse<Achievement>("Defeat" + base.Id.Entry.Capitalize() + "Enemies");

    public virtual string ChestSpineResourcePath =>
        "res://animations/backgrounds/treasure_room/chest_room_act_"
        + FilePathIdentifier
        + "_skel_data.tres";
    ...
    public virtual MegaSkeletonDataResource ChestSpineResource =>
        new MegaSkeletonDataResource(
            PreloadManager.Cache.GetAsset<Resource>(ChestSpineResourcePath)
        );
    ...
    public string BackgroundScenePath =>
        SceneHelper.GetScenePath(
            $"backgrounds/{FilePathIdentifier}/{FilePathIdentifier}_background"
        );

    public Control CreateRestSiteBackground()
    {
        return PreloadManager
            .Cache.GetScene(RestSiteBackgroundPath)
    ...
    public IEnumerable<string> GetAllBackgroundLayerPaths()
    {
        ...
        return (
            from path in dirAccess.GetFiles()
            where path.EndsWith(".tscn")
            select backgroundsPath + "/" + path
        ).ToArray();
    }
    ...
    public void GenerateRooms(Rng rng, UnlockState unlockState, bool isMultiplayer = false)
    {
        ...
        {
            list.RemoveAll(
                (EventModel e) => Event1Epoch.Events.Any((EventModel ev) => ev.Id == e.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Event2Epoch>())
        {
            list.RemoveAll(
                (EventModel e) => Event2Epoch.Events.Any((EventModel ev) => ev.Id == e.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Event3Epoch>())
        {
            list.RemoveAll(
                (EventModel e) => Event3Epoch.Events.Any((EventModel ev) => ev.Id == e.Id)
            );
        }
    ...
    public void ValidateRoomsAfterLoad(Rng rng)
    {
        ...
        {
            _rooms.Boss = rng.NextItem(
                AllBossEncounters.Where((EncounterModel e) => e.Id != _rooms.SecondBoss?.Id)
            );
        }
    ...
    {
        EncounterModel encounterModel = grabBag.GrabAndRemove(
            rng,
            (EncounterModel e) =>
                !e.SharesTagsWith(encounters.LastOrDefault()) && e != encounters.LastOrDefault()
        );
    ...
    {
        AssertMutable();
        return new SerializableActModel { Id = base.Id, SerializableRooms = _rooms.ToSave() };
    }
    ...
    )
    {
        ...
        bool flag2 =
            !isMultiplayer
            && !SaveManager.Instance.Progress.DiscoveredActs.Contains(ModelDb.Act<Underdocks>().Id);
    ...
    {
        return new global::_003C_003Ez__ReadOnlyArray<ActModel>(
            new ActModel[3] { ModelDb.Act<Overgrowth>(), ModelDb.Act<Hive>(), ModelDb.Act<Glory>() }
        );
    }
}

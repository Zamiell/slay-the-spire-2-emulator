...
public abstract class RelicModel : AbstractModel
{
    ...
    protected LocString EventDescription =>
        LocString.GetIfExists("relics", base.Id.Entry + ".eventDescription") ?? Description;
    ...
    protected LocString? AdditionalRestSiteHealText
    {
        get
        {
            LocString ifExists = LocString.GetIfExists(
                "relics",
                base.Id.Entry + ".additionalRestSiteHealText"
            );
            if (ifExists != null)
            {
    ...
    public virtual string PackedIconPath =>
        ImageHelper.GetImagePath("atlases/relic_atlas.sprites/" + IconBaseName + ".tres");

    protected virtual string PackedIconOutlinePath =>
        ImageHelper.GetImagePath("atlases/relic_outline_atlas.sprites/" + IconBaseName + ".tres");

    protected virtual string BigIconPath =>
        ImageHelper.GetImagePath("relics/" + IconBaseName + ".png");

    private string BigBetaIconPath =>
        ImageHelper.GetImagePath("relics/beta/" + IconBaseName + ".png");
    ...
    public Texture2D Icon =>
        ResourceLoader.Load<Texture2D>(PackedIconPath, null, ResourceLoader.CacheMode.Reuse);

    public Texture2D IconOutline =>
        ResourceLoader.Load<Texture2D>(PackedIconOutlinePath, null, ResourceLoader.CacheMode.Reuse);
    ...
    public RelicPoolModel Pool =>
        ModelDb.AllRelicPools.First((RelicPoolModel p) => p.AllRelicIds.Contains(base.Id));
        ...
        set
        {
            ...
            if (_owner != null && _owner != value)
            {
                throw new InvalidOperationException(
                    "Cannot move relic from " + base.Id.Entry + " one owner to another"
                );
            }
            _owner = value;
        }
    ...
    public virtual int MerchantCost =>
        Rarity switch
        {
            ...
            _ => throw new InvalidOperationException(
                $"Relic {base.Id} has invalid merchant rarity {Rarity}."
            ),
        };
    ...
    public void IncrementStackCount()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Cannot increment stack count on {base.Id} because it is not a stackable relic."
            );
        }
        StackCount++;
    }

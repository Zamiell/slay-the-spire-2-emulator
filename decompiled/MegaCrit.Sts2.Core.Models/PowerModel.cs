...
public abstract class PowerModel : AbstractModel
{
    ...
    public string PackedIconPath =>
        ImageHelper.GetImagePath(
            "atlases/power_atlas.sprites/" + base.Id.Entry.ToLowerInvariant() + ".tres"
        );

    private string BigIconPath =>
        ImageHelper.GetImagePath("powers/" + base.Id.Entry.ToLowerInvariant() + ".png");

    private string BigBetaIconPath =>
        ImageHelper.GetImagePath("powers/beta/" + base.Id.Entry.ToLowerInvariant() + ".png");
    ...
    {
        get { return _skipNextDurationTick; }
        set
        {
            AssertMutable();
        ...
        private set
        {
            ...
            if (_owner != null && _owner != value)
            {
                throw new InvalidOperationException(
                    "Cannot move power " + base.Id.Entry + " from one owner to another"
                );
            }
            _owner = value;
        }
    ...
    public IEnumerable<IHoverTip> HoverTips
    {
        get
        {
            ...
            if (flag)
            {
                ...
                locString.Add(
                    "OwnerName",
                    Owner.IsPlayer ? Owner.Player.Character.Title : Owner.Monster.Title
                );
                if (Applier != null)
                {
                    string variable = (
    ...
    {
        return amount
            * (decimal)combatState.Players.Count

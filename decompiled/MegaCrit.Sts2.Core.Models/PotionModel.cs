...
public abstract class PotionModel : AbstractModel
{
    ...
    public LocString SelectionScreenPrompt =>
        new LocString("potions", base.Id.Entry + ".selectionScreenPrompt");
    ...
    private string PackedImagePath =>
        ImageHelper.GetImagePath(
            "atlases/potion_atlas.sprites/" + base.Id.Entry.ToLowerInvariant() + ".tres"
        );

    private string PackedOutlinePath =>
        ImageHelper.GetImagePath(
            "atlases/potion_outline_atlas.sprites/" + base.Id.Entry.ToLowerInvariant() + ".tres"
        );

    public string ImagePath => PackedImagePath;

    public Texture2D Image =>
        ResourceLoader.Load<Texture2D>(PackedImagePath, null, ResourceLoader.CacheMode.Reuse);
    ...
    public PotionPoolModel Pool =>
        ModelDb.AllPotionPools.First((PotionPoolModel p) => p.AllPotionIds.Contains(base.Id));
        ...
        set
        {
            ...
            if (_owner != null && _owner != value)
            {
                throw new InvalidOperationException(
                    "Cannot move potion " + base.Id.Entry + " from one owner to another"
                );
            }
            _owner = value;
        }
    ...
    public void EnqueueManualUse(Creature? target)
    {
        ...
        UsePotionAction action = new UsePotionAction(
            this,
            target,
    ...
    public async Task OnUseWrapper(PlayerChoiceContext choiceContext, Creature? target)
    {
        ...
        if (TestMode.IsOff && combatState != null)
        {
            ...
            {
                targetPosition =
                    (NCombatRoom.Instance?.GetCreatureNode(target))?.GetBottomOfHitbox()
                    ?? Vector2.Zero;
            }
            else
            {
                IReadOnlyList<Creature> readOnlyList = (
                    (TargetType != TargetType.AllEnemies)
                        ? (
                            from c in combatState.GetCreaturesOnSide(CombatSide.Player)
                            where c.IsHittable
                            select c
                        ).ToList()
                        : (
                            from c in combatState.GetCreaturesOnSide(CombatSide.Enemy)
                            where c.IsHittable
                            select c
                        ).ToList()
                );
                ...
                {
                    targetPosition +=
                        (NCombatRoom.Instance?.GetCreatureNode(item))?.VfxSpawnPosition
                        ?? Vector2.Zero;
                }
    ...
    {
        AssertMutable();
        return new SerializablePotion { Id = base.Id, SlotIndex = slotIndex };
    }
    ...
    protected static void AssertValidForTargetedPotion([NotNull] Creature? target)
    {
        if (target == null)
        {
            throw new ArgumentNullException(
                "target",
                "Target must be present for targeted potions."
            );
        }
    }

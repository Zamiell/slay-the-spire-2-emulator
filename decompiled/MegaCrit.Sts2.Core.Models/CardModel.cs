...
public abstract class CardModel : AbstractModel
{
    ...
    public LocString TitleLocString =>
        _titleLocString ?? (_titleLocString = new LocString("cards", base.Id.Entry + ".title"));
    ...
    public virtual string PortraitPath =>
        ImageHelper.GetImagePath(
            $"atlases/card_atlas.sprites/{Pool.Title.ToLowerInvariant()}/{base.Id.Entry.ToLowerInvariant()}.tres"
        );

    public virtual string BetaPortraitPath =>
        ImageHelper.GetImagePath(
            $"atlases/card_atlas.sprites/{Pool.Title.ToLowerInvariant()}/beta/{base.Id.Entry.ToLowerInvariant()}.tres"
        );
    ...
    private string BetaPortraitPngPath =>
        ImageHelper.GetImagePath(
            $"packed/card_portraits/{Pool.Title.ToLowerInvariant()}/beta/{base.Id.Entry.ToLowerInvariant()}.png"
        );
    ...
    public Texture2D Portrait =>
        ResourceLoader.Load<Texture2D>(PortraitPath, null, ResourceLoader.CacheMode.Reuse);
    ...
    public Texture2D Frame =>
        ResourceLoader.Load<Texture2D>(FramePath, null, ResourceLoader.CacheMode.Reuse);
    ...
    private string AncientTextBgPath
    {
        get
        {
            ...
            return ImageHelper.GetImagePath(
                "atlases/compressed_atlas.sprites/ancient_text_bg_"
                    + cardType2.ToString().ToLowerInvariant()
                    + ".png.tres"
            );
        }
    }
    ...
    public Texture2D AncientTextBg =>
        ResourceLoader.Load<Texture2D>(AncientTextBgPath, null, ResourceLoader.CacheMode.Reuse);

    public Texture2D AncientBorder =>
        ResourceLoader.Load<Texture2D>(AncientBorderPath, null, ResourceLoader.CacheMode.Reuse);

    public Texture2D PortraitBorder =>
        ResourceLoader.Load<Texture2D>(PortraitBorderPath, null, ResourceLoader.CacheMode.Reuse);

    private string EnergyIconPath => VisualCardPool.EnergyIconPath;

    public Texture2D EnergyIcon =>
        ResourceLoader.Load<Texture2D>(EnergyIconPath, null, ResourceLoader.CacheMode.Reuse);
    ...
    private string BannerTexturePath
    {
        get
        {
            ...
            return ImageHelper.GetImagePath(
                "atlases/ui_atlas.sprites/card/card_banner_ancient_s.tres"
            );
        }
    }

    public Texture2D BannerTexture =>
        ResourceLoader.Load<Texture2D>(BannerTexturePath, null, ResourceLoader.CacheMode.Reuse);
        ...
        set
        {
            ...
            if (_owner != null && value != null)
            {
                throw new InvalidOperationException(
                    "Card " + base.Id.Entry + " already has an owner."
                );
            }
            _owner = value;
        }
    ...
    {
        get { return _hasSingleTurnRetain; }
        set
        {
            AssertMutable();
    ...
    public int CurrentUpgradeLevel
    {
        ...
        private set
        {
            ...
            {
                throw new InvalidOperationException(
                    $"{base.Id} cannot be upgraded past its MaxUpgradeLevel."
                );
            }
    ...
    {
        get { return _upgradePreviewType; }
        set
        {
            AssertMutable();
            ...
            {
                throw new InvalidOperationException(
                    "A card cannot go to from being upgrade preview. Consider making a new card model instead."
                );
            }
    ...
    {
        get { return _isEnchantmentPreview; }
        set
        {
            AssertMutable();
    ...
    public string OverlayPath =>
        SceneHelper.GetScenePath("cards/overlays/" + base.Id.Entry.ToLowerInvariant());

    public int? FloorAddedToDeck
    {
        get { return _floorAddedToDeck; }
        set
        {
            AssertMutable();
    ...
    public ICombatState? CombatState
    {
        get
        {
            CardPile pile = Pile;
            if (
                (pile != null && pile.IsCombatPile)
                || UpgradePreviewType == CardUpgradePreviewType.Combat
            )
            {
    ...
    public ICardScope? CardScope =>
        ((ICardScope)CombatState) ?? ((ICardScope)(_owner?.Creature.CombatState)) ?? RunState;
    ...
    public virtual IEnumerable<string> AllPortraitPaths =>
        new global::_003C_003Ez__ReadOnlySingleElementList<string>(PortraitPath);
    ...
    protected override void DeepCloneFields()
    {
        ...
        if (Enchantment != null)
        {
            EnchantmentModel enchantmentModel = (EnchantmentModel)
                Enchantment.ClonePreservingMutability();
            Enchantment = null;
            EnchantInternal(enchantmentModel, enchantmentModel.Amount);
        }
        if (Affliction != null)
        {
            AfflictionModel afflictionModel = (AfflictionModel)
                Affliction.ClonePreservingMutability();
            Affliction = null;
    ...
    {
        return _temporaryStarCosts
                .FirstOrDefault(
                    (TemporaryCardCost cost) =>
                        cost != null && !cost.ClearsWhenTurnEnds && !cost.ClearsWhenCardIsPlayed
                )
                ?.Cost
            ?? BaseStarCost;
    }
    ...
    protected void UpgradeStarCostBy(int addend)
    {
        ...
        {
            throw new InvalidOperationException(
                "UpgradeStarCostBy called on " + base.Id.Entry + " which has star cost X."
            );
        }
        if (addend == 0)
        {
    ...
    )
    {
        ...
        UpgradeDisplay upgradeDisplay = (
            (previewType == DescriptionPreviewType.Upgrade)
        ...
        bool variable2 =
            CombatManager.Instance.IsInProgress && (Pile?.IsCombatPile ?? pileType.IsCombatPile());
    ...
    public void AfflictInternal(AfflictionModel affliction, decimal amount)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Attempted to afflict card {this} that was already afflicted! This is not allowed"
            );
        }
    ...
    public bool CanPlay(out UnplayableReason reason, out AbstractModel? preventer)
    {
        ...
        if (
            ...
            && combatState.PlayerCreatures.Count((Creature c) => c.IsAlive) <= 1
        )
        {
    ...
    )
    {
        ...
        {
            await item.AfterModifyingCardPlayResultPileOrPosition(
                this,
                resultPileType,
                resultPilePosition
            );
    ...
    protected async Task<int> GeneratePlayCount(ICombatState combatState, Creature? target)
    {
        ...
        playCount = Hook.ModifyCardPlayCount(
            combatState,
            this,
            playCount,
            target,
    ...
    private async Task PlayPowerCardFlyVfx()
    {
        ...
        {
            foreach (
                NCardFlyPowerVfx item in NCombatRoom
                    .Instance.CombatVfxContainer.GetChildren()
                    .OfType<NCardFlyPowerVfx>()
            )
            {
    ...
    public CardModel CreateClone()
    {
        ...
        {
            throw new InvalidOperationException(
                "Cannot create a clone of a card that is not in a combat pile."
            );
        }
        AssertMutable();
        ...
        if (!(cardModel is DeprecatedCard))
        {
            if (save.Enchantment != null)
            {
                cardModel.EnchantInternal(
                    EnchantmentModel.FromSerializable(save.Enchantment),
                    save.Enchantment.Amount
                );

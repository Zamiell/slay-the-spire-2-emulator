...
public abstract class EncounterModel : AbstractModel
{
    ...
    public LocString? CustomRewardDescription =>
        LocString.GetIfExists("encounters", base.Id.Entry + ".customRewardDescription");
        ...
        get
        {
            ...
            if (_monstersWithSlots == null)
            {
                throw new InvalidOperationException(
                    "GenerateMonstersWithSlots must be called before using this property!"
                );
            }
    ...
    private string ScenePath =>
        SceneHelper.GetScenePath("encounters/" + base.Id.Entry.ToLowerInvariant());
    ...
    public virtual MegaSkeletonDataResource? BossNodeSpineResource
    {
        get
        {
            ...
            return new MegaSkeletonDataResource(
                PreloadManager.Cache.GetAsset<Resource>(BossNodePath)
            );
        }
    }
    ...
    public IEnumerable<string> MapNodeAssetPaths
    {
        get
        {
            ...
            return new global::_003C_003Ez__ReadOnlyArray<string>(
                new string[2] { BossNodePath + ".png", BossNodePath + "_outline.png" }
            );
        }
    }
    ...
    public string GetNextSlot(ICombatState combatState)
    {
        return Slots.FirstOrDefault(
            (string s) => combatState.Enemies.All((Creature c) => c.SlotName != s),
            string.Empty
        );
    }
    ...
    public void GenerateMonstersWithSlots(IRunState runState)
    {
        ...
        {
            throw new InvalidOperationException(
                "Monsters have already been generated for this encounter."
            );
        }
        if (_rng == null)
        {
    ...
    public IEnumerable<string> GetAssetPaths(IRunState runState)
    {
        ...
        hashSet.UnionWith(
            GetBackgroundAssets(
                runState.Act,
                NCombatRoom.GenerateBackgroundRngForCurrentPoint(runState)
            ).AssetPaths
        );
        if (HasScene)
        {
    ...
    public void OnCreatureSpawned(Creature creature)
    {
        ...
        if (
            monsterModel != null
            && (_spawnedEnemies == null || !_spawnedEnemies.Contains(monsterModel))
        )
        {

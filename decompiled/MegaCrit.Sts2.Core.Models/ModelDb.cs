...
public static class ModelDb
{
    ...
    private static readonly Dictionary<ModelId, AbstractModel> _contentById = new Dictionary<
        ModelId,
        AbstractModel
    >(4096);
    ...
    public static IEnumerable<AfflictionModel> DebugAfflictions =>
        from t in AllAbstractModelSubtypes
    ...
    public static IEnumerable<CardModel> AllCards =>
        _allCards
        ?? (
            _allCards = AllCardPools
                .SelectMany((CardPoolModel p) => p.AllCards)
                .Concat(AllCharacters.SelectMany((CharacterModel c) => c.StartingDeck).Distinct())
                .Distinct()
        );

    public static IEnumerable<CardPoolModel> AllCardPools =>
        _allCardPools
        ?? (_allCardPools = AllCharacterCardPools.Concat(AllSharedCardPools).Distinct());

    public static IEnumerable<CardPoolModel> AllSharedCardPools =>
        new global::_003C_003Ez__ReadOnlyArray<CardPoolModel>(
            new CardPoolModel[7]
            {
    ...
    public static IEnumerable<CardPoolModel> AllCharacterCardPools =>
        ...
        ?? (_allCharacterCardPools = AllCharacters.Select((CharacterModel c) => c.CardPool));

    public static IEnumerable<CharacterModel> AllCharacters =>
        new global::_003C_003Ez__ReadOnlyArray<CharacterModel>(
    ...
    public static IEnumerable<EventModel> AllSharedEvents =>
        _allSharedEvents
        ?? (
            _allSharedEvents = new global::_003C_003Ez__ReadOnlyArray<EventModel>(
                new EventModel[18]
                {
                    Event<BrainLeech>(),
                    Event<CrystalSphere>(),
    ...
    public static IEnumerable<EventModel> AllEvents =>
        _allEvents
        ?? (
            _allEvents = Acts.SelectMany((ActModel a) => a.AllEvents)
                .Concat(AllSharedEvents)
                .Distinct()
        );

    public static IEnumerable<MonsterModel> Monsters =>
        Acts.SelectMany((ActModel act) => act.AllMonsters).Distinct();

    public static IEnumerable<EncounterModel> AllEncounters =>
        _allEncounters
        ?? (_allEncounters = Acts.SelectMany((ActModel a) => a.AllEncounters).Distinct());
    ...
    public static IEnumerable<PotionPoolModel> AllPotionPools =>
        _allPotionPools
        ?? (_allPotionPools = AllCharacterPotionPools.Concat(AllSharedPotionPools).Distinct());

    public static IEnumerable<PotionPoolModel> AllCharacterPotionPools =>
        _allCharacterPotionPools
        ?? (_allCharacterPotionPools = AllCharacters.Select((CharacterModel c) => c.PotionPool));

    public static IEnumerable<RelicPoolModel> AllCharacterRelicPools =>
        _allCharacterRelicPools
        ?? (_allCharacterRelicPools = AllCharacters.Select((CharacterModel c) => c.RelicPool));

    private static IEnumerable<PotionPoolModel> AllSharedPotionPools =>
        _allSharedPotionPools
        ?? (
            _allSharedPotionPools = new global::_003C_003Ez__ReadOnlyArray<PotionPoolModel>(
                new PotionPoolModel[4]
                {
    ...
    public static IEnumerable<RelicModel> AllRelics =>
        ...
        ?? (
            _allRelics =
                from r in AllRelicPools
                    .SelectMany((RelicPoolModel p) => p.AllRelics)
                    .Concat(AllCharacters.SelectMany((CharacterModel c) => c.StartingRelics))
                    .Distinct()
                orderby r.Id.Entry
                select r
        );
    ...
    private static IEnumerable<RelicPoolModel> AllSharedRelicPools =>
        new global::_003C_003Ez__ReadOnlyArray<RelicPoolModel>(
            new RelicPoolModel[4]
            {
    ...
    public static IEnumerable<OrbModel> Orbs =>
        new global::_003C_003Ez__ReadOnlyArray<OrbModel>(
            new OrbModel[4]
            {
                Orb<LightningOrb>(),
                Orb<FrostOrb>(),
    ...
    public static IEnumerable<ActModel> Acts =>
        new global::_003C_003Ez__ReadOnlyArray<ActModel>(
            new ActModel[4] { Act<Overgrowth>(), Act<Hive>(), Act<Glory>(), Act<Underdocks>() }
        );
    ...
    public static IReadOnlyList<ModifierModel> GoodModifiers =>
        new global::_003C_003Ez__ReadOnlyArray<ModifierModel>(
            new ModifierModel[9]
            {
                Modifier<Draft>(),
                Modifier<SealedDeck>(),
    ...
    public static IReadOnlyList<IReadOnlySet<ModifierModel>> MutuallyExclusiveModifiers =>
        new global::_003C_003Ez__ReadOnlySingleElementList<IReadOnlySet<ModifierModel>>(
    ...
    public static T? GetByIdOrNull<T>(ModelId id)
        where T : AbstractModel
    {
    ...
    public static T Affliction<T>()
        where T : AfflictionModel
    {
        return Get<T>();
    }

    public static T Enchantment<T>()
        where T : EnchantmentModel
    {
        return Get<T>();
    }

    public static T Card<T>()
        where T : CardModel
    {
        return Get<T>();
    }

    public static T CardPool<T>()
        where T : CardPoolModel
    {
        return Get<T>();
    }

    public static T Character<T>()
        where T : CharacterModel
    {
        return Get<T>();
    }

    public static T Event<T>()
        where T : EventModel
    {
        return Get<T>();
    }

    public static T AncientEvent<T>()
        where T : AncientEventModel
    {
        return Get<T>();
    }

    public static T Monster<T>()
    ...
    public static T Potion<T>()
        where T : PotionModel
    {
        return Get<T>();
    }

    public static T PotionPool<T>()
        where T : PotionPoolModel
    {
        return Get<T>();
    }
    ...
    public static T Relic<T>()
        where T : RelicModel
    {
        return Get<T>();
    }

    public static T RelicPool<T>()
        where T : RelicPoolModel
    {
        return Get<T>();
    }

    public static T Orb<T>()
        where T : OrbModel
    {
        return Get<T>();
    }
    ...
    public static T Singleton<T>()
        where T : SingletonModel
    {
        return Get<T>();
    }

    public static T Badge<T>()
        where T : BadgeModel
    {
        return Get<T>();
    }

    public static T Achievement<T>()
        where T : AchievementModel
    {
        return Get<T>();
    }

    public static T Modifier<T>()

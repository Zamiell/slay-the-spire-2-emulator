...
public abstract class CharacterModel : AbstractModel
{
    ...
    public LocString PossessiveAdjective =>
        new LocString("characters", base.Id.Entry + ".possessiveAdjective");

    public LocString PronounPossessive =>
        new LocString("characters", base.Id.Entry + ".pronounPossessive");

    public LocString PronounSubject =>
        new LocString("characters", base.Id.Entry + ".pronounSubject");

    public LocString CardsModifierTitle =>
        new LocString("characters", base.Id.Entry + ".cardsModifierTitle");

    public LocString CardsModifierDescription =>
        new LocString("characters", base.Id.Entry + ".cardsModifierDescription");

    public LocString EventDeathPreventionLine =>
        new LocString("characters", base.Id.Entry + ".eventDeathPrevention");

    public string TrailPath =>
        SceneHelper.GetScenePath("vfx/card_trail_" + base.Id.Entry.ToLowerInvariant());
    ...
    private string IconTexturePath =>
        ImageHelper.GetImagePath(
            "ui/top_panel/character_icon_" + base.Id.Entry.ToLowerInvariant() + ".png"
        );
    ...
    private string IconOutlineTexturePath =>
        ImageHelper.GetImagePath(
            "ui/top_panel/character_icon_" + base.Id.Entry.ToLowerInvariant() + "_outline.png"
        );

    public Texture2D IconOutlineTexture =>
        PreloadManager.Cache.GetTexture2D(IconOutlineTexturePath);

    protected virtual string IconPath =>
        SceneHelper.GetScenePath(
            "ui/character_icons/" + base.Id.Entry.ToLowerInvariant() + "_icon"
        );

    public Control Icon =>
        PreloadManager
            .Cache.GetScene(IconPath)
            .Instantiate<Control>(PackedScene.GenEditState.Disabled);

    public string EnergyCounterPath =>
        SceneHelper.GetScenePath(
            "combat/energy_counters/" + base.Id.Entry.ToLowerInvariant() + "_energy_counter"
        );

    public string MerchantAnimPath =>
        SceneHelper.GetScenePath(
            "merchant/characters/" + base.Id.Entry.ToLowerInvariant() + "_merchant"
        );

    public string RestSiteAnimPath =>
        SceneHelper.GetScenePath(
            "rest_site/characters/" + base.Id.Entry.ToLowerInvariant() + "_rest_site"
        );

    private string ArmPointingTexturePath =>
        ImageHelper.GetImagePath(
            "ui/hands/multiplayer_hand_" + base.Id.Entry.ToLowerInvariant() + "_point.png"
        );

    public Texture2D ArmPointingTexture =>
        PreloadManager.Cache.GetTexture2D(ArmPointingTexturePath);

    private string ArmRockTexturePath =>
        ImageHelper.GetImagePath(
            "ui/hands/multiplayer_hand_" + base.Id.Entry.ToLowerInvariant() + "_rock.png"
        );
    ...
    private string ArmPaperTexturePath =>
        ImageHelper.GetImagePath(
            "ui/hands/multiplayer_hand_" + base.Id.Entry.ToLowerInvariant() + "_paper.png"
        );
    ...
    private string ArmScissorsTexturePath =>
        ImageHelper.GetImagePath(
            "ui/hands/multiplayer_hand_" + base.Id.Entry.ToLowerInvariant() + "_scissors.png"
        );

    public Texture2D ArmScissorsTexture =>
        PreloadManager.Cache.GetTexture2D(ArmScissorsTexturePath);

    public Achievement RunWonAchievement =>
        Enum.Parse<Achievement>(base.Id.Entry.Capitalize() + "Win");
    ...
    public string CharacterSelectBg =>
        SceneHelper.GetScenePath(
            "screens/char_select/char_select_bg_" + base.Id.Entry.ToLowerInvariant()
        );

    protected virtual string CharacterSelectIconPath =>
        ImageHelper.GetImagePath(
            "packed/character_select/char_select_" + base.Id.Entry.ToLowerInvariant() + ".png"
        );
    ...
    protected virtual string CharacterSelectLockedIconPath =>
        ImageHelper.GetImagePath(
            "packed/character_select/char_select_"
    ...
    public string CharacterSelectTransitionPath =>
        "res://materials/transitions/" + base.Id.Entry.ToLowerInvariant() + "_transition_mat.tres";

    protected virtual string MapMarkerPath =>
        ImageHelper.GetImagePath(
            "packed/map/icons/map_marker_" + base.Id.Entry.ToLowerInvariant() + ".png"
        );

    public CompressedTexture2D MapMarker =>
        PreloadManager.Cache.GetCompressedTexture2D(MapMarkerPath);
    ...
    public virtual string CharacterSelectSfx =>
        $"event:/sfx/characters/{base.Id.Entry.ToLowerInvariant()}/{base.Id.Entry.ToLowerInvariant()}_select";

    public string AttackSfx =>
        $"event:/sfx/characters/{base.Id.Entry.ToLowerInvariant()}/{base.Id.Entry.ToLowerInvariant()}_attack";

    public string CastSfx =>
        $"event:/sfx/characters/{base.Id.Entry.ToLowerInvariant()}/{base.Id.Entry.ToLowerInvariant()}_cast";

    public string DeathSfx =>
        $"event:/sfx/characters/{base.Id.Entry.ToLowerInvariant()}/{base.Id.Entry.ToLowerInvariant()}_die";

    public virtual string CharacterTransitionSfx =>
        "event:/sfx/ui/wipe_" + base.Id.Entry.ToLowerInvariant();
    ...
    public LocString GetUnlockText()
    {
        ...
        else
        {
            ...
            variable = (
                (!unlockState.Characters.Contains<CharacterModel>(UnlocksAfterRunAs))
                    ? locString2
                    : UnlocksAfterRunAs.Title
            );
        }

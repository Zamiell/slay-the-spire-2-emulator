...
public abstract class AncientEventModel : EventModel
{
    ...
    private string MapIconPath =>
        ImageHelper.GetImagePath(
            "packed/map/ancients/ancient_node_" + base.Id.Entry.ToLowerInvariant() + ".png"
        );

    private string MapIconOutlinePath =>
        ImageHelper.GetImagePath(
            "packed/map/ancients/ancient_node_" + base.Id.Entry.ToLowerInvariant() + "_outline.png"
        );

    public Texture2D MapIcon => PreloadManager.Cache.GetCompressedTexture2D(MapIconPath);

    public Texture2D MapIconOutline =>
        PreloadManager.Cache.GetCompressedTexture2D(MapIconOutlinePath);

    public IEnumerable<string> MapNodeAssetPaths =>
        new global::_003C_003Ez__ReadOnlyArray<string>(
            new string[2] { MapIconPath, MapIconOutlinePath }
        );
    ...
    public Texture2D RunHistoryIcon =>
        PreloadManager.Cache.GetCompressedTexture2D(
            ImageHelper.GetRoomIconPath(MapPointType.Ancient, RoomType.Event, base.Id)
        );

    public Texture2D RunHistoryIconOutline =>
        PreloadManager.Cache.GetCompressedTexture2D(RunHistoryIconOutlinePath);

    private string RunHistoryIconOutlinePath =>
        ImageHelper.GetImagePath(
            "ui/run_history/" + base.Id.Entry.ToLowerInvariant() + "_outline.png"
        );
    ...
    public override LocString InitialDescription
    {
        get
        {
            if (
                ...
                || Hook.ShouldAllowAncient(base.Owner.RunState, base.Owner, this)
            )
            {
    ...
    protected static string CharKey<T>()
    ...
    protected sealed override IReadOnlyList<EventOption> GenerateInitialOptionsWrapper()
    {
        if (Hook.ShouldAllowAncient(base.Owner.RunState, base.Owner, this))
        {
            ...
            if (DebugOption != null)
            {
                ...
                GeneratedOptions.Insert(
                    0,
                    AllPossibleOptions.First((EventOption c) => c.TextKey.Contains(DebugOption))
                );
            }
        ...
        return new global::_003C_003Ez__ReadOnlySingleElementList<EventOption>(
            new EventOption(this, NEventRoom.Proceed, "PROCEED", false, true)
        );
    }
    ...
    private void UpdateRunHistory()
    {
        ...
        {
            AncientChoiceHistoryEntry item = new AncientChoiceHistoryEntry(
                generatedOption.Title,
                generatedOption.WasChosen
            );
            base.Owner.RunState.CurrentMapPointHistoryEntry?.GetEntry(base.Owner.NetId)
                .AncientChoices.Add(item);
        }
    }
    ...
    protected EventOption RelicOption<T>(string pageName = "INITIAL", string? customDonePage = null)
        where T : RelicModel
    {

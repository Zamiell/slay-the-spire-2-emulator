...
public class SteamJoinCallbackHandler : IDisposable
{
    ...
    public void CheckForCommandLineJoin()
    {
        if (CommandLineHelper.TryGetValue("+connect_lobby", out string value))
        {
            ...
            Log.Info(
                $"Joining to host via Steam invite that caused the game to launch. Lobby id: {num}"
            );
    ...
    {
        Log.Info(
            $"Joining to host via Steam invite. Lobby: {lobbyJoinRequest.m_steamIDLobby.m_SteamID} player: {lobbyJoinRequest.m_steamIDFriend.m_SteamID}"
        );
        TaskHelper.RunSafely(
            JoinToHost(
                lobbyJoinRequest.m_steamIDLobby.m_SteamID,
                lobbyJoinRequest.m_steamIDFriend.m_SteamID
            )
        );
    }

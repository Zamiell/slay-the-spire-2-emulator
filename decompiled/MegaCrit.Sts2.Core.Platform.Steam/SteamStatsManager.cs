...
public static class SteamStatsManager
{
    ...
    public static async Task RefreshGlobalStats()
    {
        ...
        using SteamCallResult<GlobalStatsReceived_t> callResult =
            new SteamCallResult<GlobalStatsReceived_t>(call, SteamInitializer.DisconnectToken);
        try
        {
    ...
    public static void IncrementArchitectDamage(int score)
    {
        ...
        Log.Info(
            $"SteamStatsManager: IncrementArchitectDamage by {score} (was {pData}, now {pData + score}) [get={stat}, set={value}, store={value2}]"
        );
    }
    ...
    private static void OnUserStatsReceived(UserStatsReceived_t result)
    {
        if (result.m_nGameID == 2868840)
        {
            ...
            {
                Log.Warn(
                    $"SteamStatsManager: User stats request failed with result {result.m_eResult}"
                );
            }
        }
    }
    ...
    private static void OnGlobalStatsReceived(GlobalStatsReceived_t result)
    {
        if (result.m_nGameID == 2868840)
        {
            if (result.m_eResult == EResult.k_EResultOK)
            {
                ...
                Log.Info(
                    $"SteamStatsManager: Global stats received (found={globalStat}), architect damage = {_globalArchitectDamage}"
                );
            }
            else
            {
                Log.Warn(
                    $"SteamStatsManager: Global stats request failed with result {result.m_eResult}"
                );
            }
        }
    }

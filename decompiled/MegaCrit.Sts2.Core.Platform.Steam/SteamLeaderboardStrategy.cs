...
public class SteamLeaderboardStrategy : ILeaderboardStrategy
{
    ...
    )
    {
        ...
        using SteamCallResult<LeaderboardFindResult_t> callResult =
            new SteamCallResult<LeaderboardFindResult_t>(call, cancelToken);
        ...
        if (leaderboardFindResult_t.m_bLeaderboardFound == 0)
        {
            throw new InvalidOperationException(
                "Steam FindOrCreateLeaderboard returned 0 from leaderboard found!"
            );
        }
    ...
    public async Task<ILeaderboardHandle?> GetLeaderboard(
        string name,
        CancellationToken cancelToken
    )
    {
    ...
    )
    {
        ...
        using SteamCallResult<LeaderboardScoreUploaded_t> callResult =
            new SteamCallResult<LeaderboardScoreUploaded_t>(call, SteamInitializer.DisconnectToken);
    ...
    )
    {
        ...
        SteamAPICall_t call = SteamUserStats.DownloadLeaderboardEntries(
            steamLeaderboardHandle.leaderboard,
            LeaderboardDataRequestTypeFrom(type),
            num,
            num + count - 1
        );
        using SteamCallResult<LeaderboardScoresDownloaded_t> callResult =
            new SteamCallResult<LeaderboardScoresDownloaded_t>(call, cancelToken);
        ...
        for (int i = num2; i < num3; i++)
        {
            if (
                !SteamUserStats.GetDownloadedLeaderboardEntry(
                    ...
                    i,
                    out var pLeaderboardEntry,
                    _cachedUserDetails,
                    _cachedUserDetails.Length
                )
            )
            {
                throw new InvalidOperationException(
                    $"Failed to download leaderboard entry at index {i}. Start result index: {num2}. result count: {num3}"
                );
            }
    ...
    )
    {
        ...
        SteamAPICall_t call = SteamUserStats.DownloadLeaderboardEntriesForUsers(
            steamLeaderboardHandle.leaderboard,
            _cachedUsers,
            userIds.Count
        );
        ...
        for (int j = 0; j < leaderboardScoresDownloaded_t.m_cEntryCount; j++)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Failed to download leaderboard entry at index {j}"
                );
            }
            list.Add(LeaderboardEntryFromSteamType(pLeaderboardEntry));
        }
    ...
    private ELeaderboardDataRequest LeaderboardDataRequestTypeFrom(LeaderboardQueryType type)
    {
        switch (type)
        {
            ...
            case LeaderboardQueryType.None:
                throw new InvalidOperationException(
                    "LeaderboardQueryType.None should never be passed to an API!"
                );
            default:
            {
    ...
    private int[] PackScoreDetails(IReadOnlyList<ulong> userIds, out int scoreDetailsCount)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to write {userIds.Count} user IDs into {scoreDetailsCount} integers, exceeding the max limit ({64})!"
            );
        }

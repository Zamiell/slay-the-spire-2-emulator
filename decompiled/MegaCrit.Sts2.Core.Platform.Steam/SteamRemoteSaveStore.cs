...
public class SteamRemoteSaveStore : ICloudSaveStore, ISaveStore
{
    public string ReadFile(string path)
    {
        ...
        if (num == 0)
        {
            throw new InvalidOperationException(
                $"Steam remote storage read failed for {path}. Expected {fileSize} bytes, got 0. Steam storage may be corrupted or unavailable."
            );
        }
        if (num != fileSize)
        {
            Log.Warn(
                $"Steam read returned {num} bytes but expected {fileSize} for {path}. Using partial data."
            );
        }
    ...
    public async Task<string?> ReadFileAsync(string path)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Steam remote storage async read request returned invalid API call for path {path} (bytes {byteCount})"
            );
        }
        using SteamCallResult<RemoteStorageFileReadAsyncComplete_t> callResult =
            new SteamCallResult<RemoteStorageFileReadAsyncComplete_t>(
                steamAPICall_t,
                SteamInitializer.DisconnectToken
            );
        RemoteStorageFileReadAsyncComplete_t remoteStorageFileReadAsyncComplete_t =
            await callResult.Task;
        if (remoteStorageFileReadAsyncComplete_t.m_eResult != EResult.k_EResultOK)
        {
            SteamRemoteSaveStoreException ex = new SteamRemoteSaveStoreException(
                $"Steam remote storage async read request returned error {remoteStorageFileReadAsyncComplete_t.m_eResult} for path {path}",
                remoteStorageFileReadAsyncComplete_t.m_eResult
            );
            throw ex;
        }
        byte[] array = new byte[byteCount];
        SteamRemoteStorage.FileReadAsyncComplete(
            remoteStorageFileReadAsyncComplete_t.m_hFileReadAsync,
            array,
            remoteStorageFileReadAsyncComplete_t.m_cubRead
        );
        return Encoding.UTF8.GetString(array);
    }
    ...
    public void WriteFile(string path, byte[] bytes)
    {
        ...
        {
            throw new InvalidOperationException(
                "Steam Cloud write failed. See ISteamRemoteStorage documentation for possible reasons."
            );
        }
    ...
    {
        path = CanonicalizePath(path);
        SteamAPICall_t steamAPICall_t = SteamRemoteStorage.FileWriteAsync(
            path,
            bytes,
            (uint)bytes.Length
        );
        ...
        {
            throw new InvalidOperationException(
                $"Steam remote storage async write request returned invalid API call for path {path} ({bytes.Length} bytes)"
            );
        }
        using SteamCallResult<RemoteStorageFileWriteAsyncComplete_t> callResult =
            new SteamCallResult<RemoteStorageFileWriteAsyncComplete_t>(
                steamAPICall_t,
                SteamInitializer.DisconnectToken
            );
        ...
        {
            throw new InvalidOperationException(
                $"Steam remote storage async write request returned error {remoteStorageFileWriteAsyncComplete_t.m_eResult}"
            );
        }
        Log.Info($"Wrote {bytes.Length} bytes to {path} in steam remote store");
    }
    ...
    public void DeleteFile(string path)
    {
        ...
        {
            Log.Error(
                "Steam remote storage delete FAILED for "
                    + path
    ...
    public string[] GetFilesInDirectory(string directoryPath)
    {
        ...
        for (int i = 0; i < fileCount; i++)
        {
            ...
            string fileNameAndSize = SteamRemoteStorage.GetFileNameAndSize(
                i,
    ...
    public void ForgetFile(string path)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to forget file at path {path} from steam storage, but false was returned from {"FileForget"}!"
            );
        }
    }
    ...
    public void BeginSaveBatch()
    {
        ...
        {
            Log.Warn(
                "SteamRemoteStorage.BeginFileWriteBatch returned false (a batch may already be in progress)"
            );
        }
    }
    ...
    public void EndSaveBatch()
    {
        ...
        {
            Log.Warn(
                "SteamRemoteStorage.EndFileWriteBatch returned false (no batch was in progress)"
            );
        }
    }
}

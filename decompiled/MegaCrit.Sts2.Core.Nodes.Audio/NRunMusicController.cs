...
public class NRunMusicController : Node
{
    ...
    private MusicProgressTrack GetTrack(RoomType roomType)
    {
        ...
        switch (roomType)
        {
            ...
            case RoomType.Event:
                if (
                    _runState.CurrentRoom is EventRoom eventRoom
        ...
        if (!NonInteractiveMode.IsActive && _runState.CurrentRoom != null)
        {
            if (_runState.CurrentRoom.RoomType != RoomType.Shop)
            {
                throw new InvalidOperationException(
                    "You can only trigger the merchant transition in a merchant room"
                );
            }
            NMapScreen? instance = NMapScreen.Instance;
            MusicProgressTrack musicProgressTrack = (
                (instance != null && instance.IsVisible())
                    ? MusicProgressTrack.MerchantEnd
                    : MusicProgressTrack.Merchant
            );
    ...
    public void TriggerEliteSecondPhase()
    {
        if (!NonInteractiveMode.IsActive)
        {
            ...
            {
                throw new InvalidOperationException(
                    "You can only trigger the elite transition in an elite room"
                );
            }
    ...
    public void TriggerCampfireGoingOut()
    {
        if (!NonInteractiveMode.IsActive)
        {
            ...
            {
                throw new InvalidOperationException(
                    "You can only trigger the rest site transition in a rest site room"
                );
            }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateMusicParameter,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.TriggerEliteSecondPhase,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.TriggerCampfireGoingOut,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<MusicProgressTrack>(
                GetTrack(VariantUtils.ConvertTo<RoomType>(in args[0]))
            );
            return true;
        }

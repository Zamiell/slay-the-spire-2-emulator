...
public class NUnlockEpochScreen : NUnlockScreen
{
    ...
    public override void Open()
    {
        ...
        PackedScene scene = PreloadManager.Cache.GetScene(
            "res://scenes/timeline_screen/epoch.tscn"
        );
        if (_unlockedEpochs.Count == 3)
        {
            for (int i = 0; i < _unlockedEpochs.Count; i++)
            {
                NEpochCard nEpochCard = scene.Instantiate<NEpochCard>(
                ...
                _cardFlyTween
                    .TweenProperty(node, "modulate", Colors.White, 1.0)
                    .SetDelay(num - 0.3);
                _cardFlyTween
                    .TweenProperty(node, "position", node.Position, 1.0)
        ...
        else if (_unlockedEpochs.Count == 2)
        {
            ...
            {
                NEpochCard nEpochCard2 = scene.Instantiate<NEpochCard>(
                    PackedScene.GenEditState.Disabled
                );
                ...
                _cardFlyTween
                    .TweenProperty(node2, "modulate", Colors.White, 1.0)
                    .SetDelay(num - 0.3);
                _cardFlyTween
                    .TweenProperty(node2, "position", node2.Position, 1.0)
                    .SetEase(Tween.EaseType.Out)
                    .SetTrans(Tween.TransitionType.Back)
                    .SetDelay(num);

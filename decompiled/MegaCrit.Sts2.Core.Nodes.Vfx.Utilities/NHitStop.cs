...
public class NHitStop : Node
{
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<Ease.Functions>(
                EaseForStrength(VariantUtils.ConvertTo<ShakeStrength>(in args[0]))
            );
            return true;
        }
        if (method == MethodName.SecondsForDuration && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<float>(
                SecondsForDuration(VariantUtils.ConvertTo<ShakeDuration>(in args[0]))
            );
            return true;
        }

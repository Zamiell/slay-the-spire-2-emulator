...
public static class PotionFactory
{
    ...
    )
    {
        return CreateRandomPotion(
                GetPotionOptions(player, blacklist ?? Array.Empty<PotionModel>()),
                1,
                rng
            )
            .First();
    ...
    public static List<PotionModel> CreateRandomPotionsOutOfCombat(
        Player player,
        int count,
        Rng rng,
        IEnumerable<PotionModel>? blacklist = null
    ...
    {
        return CreateRandomPotion(
                from p in GetPotionOptions(player, blacklist ?? Array.Empty<PotionModel>())
    ...
    private static List<PotionModel> CreateRandomPotion(
    ...
    )
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            PotionRarity potionRarity = (
                (num <= 0.1f)
                    ? PotionRarity.Rare
                    : ((!(num <= 0.35f)) ? PotionRarity.Common : PotionRarity.Uncommon)
            );
            PotionRarity rarity = potionRarity;
            PotionModel item = rng.NextItem(
                list.Where((PotionModel potion) => potion.Rarity == rarity)
            );
            list2.Add(item);
            list.Remove(item);
        }
    ...
    public static IEnumerable<PotionModel> GetPotionOptions(
        Player player,
        IEnumerable<PotionModel> blacklist
    )
    {
        return from p in player
                .Character.PotionPool.GetUnlockedPotions(player.UnlockState)
                .Concat(

...
public static class RelicFactory
{
    ...
    )
    {
        RelicModel relicModel =
            TestRngInjector.ConsumeRelicOverride()
            ?? player.RelicGrabBag.PullFromFront(rarity, filter, player.RunState)
            ?? FallbackRelic;
    ...
    )
    {
        RelicModel relicModel =
            TestRngInjector.ConsumeRelicOverride()
            ?? player.RelicGrabBag.PullFromBack(rarity, filter, player.RunState)
            ?? FallbackRelic;
        player.RunState.SharedRelicGrabBag.Remove(relicModel);
        return relicModel;
    }
    ...
    public static RelicRarity RollRarity(Rng rng)
    {
        ...
        return (num < 0.5f)
            ? RelicRarity.Common
            : ((!(num < 0.83f)) ? RelicRarity.Rare : RelicRarity.Uncommon);
    }
}

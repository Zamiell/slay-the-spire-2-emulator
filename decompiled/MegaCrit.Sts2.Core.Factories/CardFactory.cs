...
public static class CardFactory
{
    ...
    )
    {
        ...
        return options.Where(
            (CardModel c) => c.MultiplayerConstraint != CardMultiplayerConstraint.MultiplayerOnly
        );
    }
    ...
    )
    {
        ...
        CardRarity rarity = Hook.ModifyMerchantCardRarity(
            player.RunState,
            player,
            player.PlayerOdds.CardRarity.RollWithoutChangingFutureOdds(CardRarityOddsType.Shop)
        );
        CardRarity rarity2 = GetNextAllowedRarity(
            rarity,
            (CardRarity r) => optionsArr.Any((CardModel c) => c.Rarity == r && c.Type == type)
        );
        if (rarity2 == CardRarity.None)
        {
            throw new InvalidOperationException(
                $"Can't generate valid rarity for merchant card type {type} with card options: {string.Join(",", optionsArr.Select((CardModel c) => c.Id))}"
            );
        }
        List<CardModel> items = optionsArr
            .Where((CardModel c) => c.Rarity == rarity2 && c.Type == type)
            .ToList();
        CardModel cardModel = player.RunState.CreateCard(
            player.PlayerRng.Shops.NextItem(items),
            player
        );
    ...
    )
    {
        ...
        if (
            !options.Flags.HasFlag(CardCreationFlags.NoModifyHooks)
    ...
    )
    {
        ...
        IEnumerable<CardModel> unlockedCards = cardPoolModel.GetUnlockedCards(
    ...
    {
        IEnumerable<CardModel> defaultTransformationOptions = GetDefaultTransformationOptions(
            original,
            isInCombat
        );
        return original.CardScope.CreateCard(
            rng.NextItem(defaultTransformationOptions),
            original.Owner
        );
    }
    ...
    {
        CardModel[] filteredTransformationOptions = GetFilteredTransformationOptions(
            original,
            options,
            isInCombat
        );
        return original.CardScope.CreateCard(
            rng.NextItem(filteredTransformationOptions),
            original.Owner
        );
    }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                "All transformation options provided are invalid! Original options: "
                    + string.Join(",", originalOptions)
            );
        }
        return array;
    }
    ...
    )
    {
        ...
        {
            items = options2.Where(
                (CardModel c) => c.Rarity != CardRarity.Basic && c.Rarity != CardRarity.Ancient
            );
        }
        else
        {
            HashSet<CardRarity> allowedRarities = options2
                .Select((CardModel c) => c.Rarity)
                .ToHashSet();
            ...
            {
                throw new InvalidOperationException(
                    $"Tried to create a card for a reward, but we couldn't generate a valid rarity! Odds: {options.RarityOdds} Card pool: {string.Join(",", options2)}, blacklist: {string.Join(",", blacklist)}"
                );
            }
        ...
        {
            throw new InvalidOperationException(
                $"Tried to create a card for a reward, but we couldn't generate a valid card! Selected rarity: {selectedRarity}, card pool: {string.Join(",", options2)}, blacklist: {string.Join(",", blacklist)}, odds: {options.RarityOdds}"
            );
        }
    ...
    private static void RollForUpgrade(Player player, CardModel card, decimal baseChance, Rng rng)
    {
        ...
        if (card.IsUpgradable)
        {
            ...
            originalOdds = Hook.ModifyCardRewardUpgradeOdds(
                player.RunState,
                player,
                card,
                originalOdds

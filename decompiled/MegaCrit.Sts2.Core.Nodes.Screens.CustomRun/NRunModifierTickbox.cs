...
public class NRunModifierTickbox : NTickbox
{
    ...
    private static readonly LocString _descriptionLoc = new LocString(
        "main_menu_ui",
        "CUSTOM_RUN_SCREEN.MODIFIER_LABEL"
    );
    ...
    public static NRunModifierTickbox? Create(ModifierModel model)
    {
        ...
        NRunModifierTickbox nRunModifierTickbox = PreloadManager
            .Cache.GetScene("res://scenes/screens/custom_run/modifier_tickbox.tscn")
    ...
    public override void _Ready()
    {
        ...
        if (Modifier != null)
        {
            string variable = (
                (
                    ModelDb.GoodModifiers.FirstOrDefault(
                        (ModifierModel m) => m.GetType() == Modifier.GetType()
                    ) != null
                )
                    ? "green"
                    : (
                        (
                            ModelDb.BadModifiers.FirstOrDefault(
                                (ModifierModel m) => m.GetType() == Modifier.GetType()
                            ) != null
                        )
                            ? "red"

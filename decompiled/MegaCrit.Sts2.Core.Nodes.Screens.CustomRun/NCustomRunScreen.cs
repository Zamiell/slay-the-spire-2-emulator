...
public class NCustomRunScreen : NSubmenu, IStartRunLobbyListener, ICharacterSelectButtonDelegate
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/custom_run/custom_run_screen"
    );
    ...
    public static NCustomRunScreen? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NCustomRunScreen>(PackedScene.GenEditState.Disabled);
    }
    ...
    public override void _Ready()
    {
        ...
        _modifiersList.Connect(
            NCustomRunModifiersList.SignalName.ModifiersChanged,
            Callable.From(OnModifiersListChanged)
        );
        base.ProcessMode = ProcessModeEnum.Disabled;
        _disclaimer.SetTextAutoSize(
            new LocString("main_menu_ui", "CUSTOM_RUN_SCREEN.disclaimer").GetFormattedText()
        );
        GetNode<MegaLabel>("%CustomModeTitle")
            .SetTextAutoSize(
                new LocString(
                    "main_menu_ui",
        ...
        GetNode<MegaLabel>("%ModifiersTitle")
            .SetTextAutoSize(
                new LocString(
                    "main_menu_ui",
        ...
        GetNode<MegaLabel>("%SeedLabel")
            .SetTextAutoSize(
                new LocString("main_menu_ui", "CUSTOM_RUN_SCREEN.SEED_LABEL").GetFormattedText()
            );
        _seedInput.PlaceholderText = new LocString(
            "main_menu_ui",
            "CUSTOM_RUN_SCREEN.SEED_RANDOM_PLACEHOLDER"
        ).GetFormattedText();
        _seedInput.Connect(
            LineEdit.SignalName.TextChanged,
            Callable.From<string>(OnSeedInputSubmitted)
        );
    ...
    private void InitCharacterButtons()
    {
        ...
        {
            NCharacterSelectButton nCharacterSelectButton = PreloadManager
                .Cache.GetScene("res://scenes/screens/char_select/char_select_button.tscn")
        ...
        for (int i = 0; i < _charButtonContainer.GetChildCount(); i++)
        {
            ...
            child.FocusNeighborLeft = (
                (i > 0) ? _charButtonContainer.GetChild<Control>(i - 1).GetPath() : child.GetPath()
            );
            child.FocusNeighborRight = (
                (i < _charButtonContainer.GetChildCount() - 1)
                    ? _charButtonContainer.GetChild<Control>(i + 1).GetPath()
                    : child.GetPath()
            );
    ...
    {
        Log.Info(
            $"Embarking on a CUSTOM {_lobby.LocalPlayer.character.Id.Entry} run. Ascension: {_lobby.Ascension} Seed: {_lobby.Seed} Modifiers: {GetModifiersString()}"
        );
    ...
    {
        Log.Info(
            $"Embarking on a CUSTOM multiplayer run. Players: {string.Join(",", _lobby.Players)}. Ascension: {_lobby.Ascension} Seed: {_lobby.Seed} Modifiers: {GetModifiersString()}"
        );
        SfxCmd.Play(_lobby.LocalPlayer.character.CharacterTransitionSfx);
        await NGame.Instance.Transition.FadeOut(
            0.8f,
    ...
    public void PlayerChanged(LobbyPlayer player, bool isRandomCharacterResolution)
    {
        ...
        {
            throw new InvalidOperationException(
                "Random character is not currently allowed in custom!"
            );
        }
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnSeedInputSubmitted,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnModifiersListChanged,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateControllerButton,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.TryFocusOnModifiersList,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._modifiersHotkeyIcon,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

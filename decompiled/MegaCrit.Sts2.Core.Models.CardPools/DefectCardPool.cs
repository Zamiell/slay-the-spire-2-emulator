...
public sealed class DefectCardPool : CardPoolModel
{
    ...
    )
    {
        ...
        {
            list.RemoveAll(
                (CardModel c) => Defect2Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Defect5Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Defect5Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Defect7Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Defect7Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        return list;
    }

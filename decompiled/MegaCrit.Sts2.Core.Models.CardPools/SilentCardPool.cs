...
public sealed class SilentCardPool : CardPoolModel
{
    ...
    )
    {
        ...
        {
            list.RemoveAll(
                (CardModel c) => Silent2Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Silent5Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Silent5Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Silent7Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Silent7Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        return list;
    }

...
public sealed class NecrobinderCardPool : CardPoolModel
{
    ...
    )
    {
        ...
        {
            list.RemoveAll(
                (CardModel c) => Necrobinder2Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Necrobinder5Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Necrobinder5Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Necrobinder7Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Necrobinder7Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        return list;
    }

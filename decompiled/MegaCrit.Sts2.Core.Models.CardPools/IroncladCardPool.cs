...
public sealed class IroncladCardPool : CardPoolModel
{
    ...
    )
    {
        ...
        {
            list.RemoveAll(
                (CardModel c) => Ironclad2Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Ironclad5Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Ironclad5Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Ironclad7Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Ironclad7Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        return list;
    }

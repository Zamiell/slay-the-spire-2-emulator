...
public sealed class ColorlessCardPool : CardPoolModel
{
    ...
    )
    {
        ...
        {
            list.RemoveAll(
                (CardModel c) => Colorless1Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Colorless2Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Colorless2Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Colorless3Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Colorless3Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Colorless4Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Colorless4Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Colorless5Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Colorless5Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        return list;
    }

...
public sealed class RegentCardPool : CardPoolModel
{
    ...
    )
    {
        ...
        {
            list.RemoveAll(
                (CardModel c) => Regent2Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Regent5Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Regent5Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        if (!unlockState.IsEpochRevealed<Regent7Epoch>())
        {
            list.RemoveAll(
                (CardModel c) => Regent7Epoch.Cards.Any((CardModel card) => card.Id == c.Id)
            );
        }
        return list;
    }

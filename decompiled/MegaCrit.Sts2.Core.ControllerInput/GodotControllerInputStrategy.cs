...
{
    private readonly Dictionary<StringName, StringName> _megaInputMap = new Dictionary<
        StringName,
        StringName
    >
    {
        { Controller.joystickUp, Controller.dPadNorth },
        { Controller.joystickDown, Controller.dPadSouth },
        { Controller.joystickLeft, Controller.dPadWest },
        { Controller.joystickRight, Controller.dPadEast },
    };
    ...
    private void UpdateControllerConfig()
    {
        ...
        if (!(joyName == _currentControllerType))
        {
            ...
            if (
                _currentControllerType.Contains("Xbox One")
                || _currentControllerType.Contains("XInput")
            )
            {
            ...
            else if (_currentControllerType.Contains("PS3"))
            {
                ...
                _currentControllerType.Contains("PS4")
                || _currentControllerType.Contains("DualSense")
            )
            {
                _controllerConfig = new Ps4Config();
            }

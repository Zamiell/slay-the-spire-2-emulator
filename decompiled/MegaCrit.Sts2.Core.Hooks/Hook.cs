...
public static class Hook
{
    ...
    )
    {
        ...
        {
            choiceContext.PushModel(model);
            await model.BeforeDamageReceived(
                choiceContext,
                target,
                amount,
                props,
    ...
    )
    {
        ...
        {
            choiceContext.PushModel(model);
            await model.AfterDamageReceived(
                choiceContext,
                target,
                result,
                props,
        ...
        {
            choiceContext.PushModel(model);
            await model.AfterDamageReceivedLate(
                choiceContext,
                target,
                result,
                props,
    ...
    )
    {
        ...
        foreach (AbstractModel model in combatState.IterateHookListeners())
        {
            ...
            Task task = model.BeforeSideTurnStart(
                hookPlayerChoiceContext,
                side,
                participants,
                combatState
    ...
    )
    {
        ...
        foreach (AbstractModel item2 in combatState.IterateHookListeners())
        {
            decimal num3 = item2.ModifyBlockMultiplicative(
                target,
                num,
                props,
                cardSource,
    ...
    )
    {
        ...
        {
            if (
                item2.TryModifyCardBeingAddedToDeckLate(card, out CardModel newCard2)
                && newCard2 != null
            )
            {
    ...
    public static (PileType, CardPilePosition) ModifyCardPlayResultPileTypeAndPosition(
        ICombatState combatState,
        CardModel card,
        bool isAutoPlay,
    ...
    )
    {
        ...
        foreach (AbstractModel item in combatState.IterateHookListeners())
        {
            ...
            (pileType2, cardPilePosition) = item.ModifyCardPlayResultPileTypeAndPosition(
                card,
                isAutoPlay,
                resources,
                pileType2,
    ...
    )
    {
        ...
        foreach (AbstractModel item in runState.IterateHookListeners(null))
        {
            bool flag2 = item.TryModifyCardRewardOptions(
                player,
                cardRewardOptions,
                creationOptions
            );
        ...
        foreach (AbstractModel item2 in runState.IterateHookListeners(null))
        {
            bool flag3 = item2.TryModifyCardRewardOptionsLate(
                player,
                cardRewardOptions,
                creationOptions
            );
    ...
    )
    {
        ...
        if (flag4)
        {
            ...
            foreach (Creature item in combatState?.HittableEnemies ?? Array.Empty<Creature>())
            {
                ...
                decimal num3 = ModifyDamageInternal(
                    runState,
                    combatState,
                    item,
                    dealer,
    ...
    )
    {
        ...
        {
            if (
                item.TryModifyPowerAmountReceived(
                    canonicalPower,
                    target,
                    num,
                    giver,
    ...
    public static bool ShouldGainGold(
        IRunState runState,
        ICombatState? combatState,
        decimal amount,
        Player player
    ...
    )
    {
        ...
        if (modifyDamageHookType.HasFlag(ModifyDamageHookType.Multiplicative))
        {
            foreach (AbstractModel item2 in runState.IterateHookListeners(combatState))
            {
                decimal num3 = item2.ModifyDamageMultiplicative(
                    target,
                    num,
                    props,
                    dealer,

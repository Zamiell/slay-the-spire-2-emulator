...
public class NCardBundle : Control
{
    ...
    public static NCardBundle? Create(IReadOnlyList<CardModel> bundle)
    {
        ...
        NCardBundle nCardBundle = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NCardBundle>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        Hitbox.Connect(
            ...
            Callable.From<NClickableControl>(OnFocused)
        );
        Hitbox.Connect(
            NClickableControl.SignalName.Unfocused,
            Callable.From<NClickableControl>(OnUnfocused)
        );
        Hitbox.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NClickableControl>(OnClicked)
        );
        ...
        for (int i = 0; i < _cardNodes.Count; i++)
        {
            ...
            _cardTween
                .TweenProperty(
                    nCard,
                    "position",
                    new Vector2(-1f, 1f) * 45f * ((float)i - (float)_cardNodes.Count / 2f),
                    0.4000000059604645
                )
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Expo);
            float num = 0.5f + (float)i / (float)(_cardNodes.Count - 1) * 0.5f;
            _cardTween.TweenProperty(
                nCard,
                "modulate",

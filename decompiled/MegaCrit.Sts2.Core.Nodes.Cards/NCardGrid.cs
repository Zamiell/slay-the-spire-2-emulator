...
public class NCardGrid : Control
{
    ...
    private Dictionary<SortingOrders, Func<CardModel, CardModel, int>> SortingAlgorithms
    {
        get
        {
            Dictionary<SortingOrders, Func<CardModel, CardModel, int>> dictionary =
                _sortingAlgorithms;
            if (dictionary == null)
            {
                Dictionary<SortingOrders, Func<CardModel, CardModel, int>> obj = new Dictionary<
                    SortingOrders,
                    Func<CardModel, CardModel, int>
                >
                {
                    {
                        SortingOrders.RarityAscending,
                        (CardModel a, CardModel b) =>
                            GetCardRarityComparisonValue(a)
                                .CompareTo(GetCardRarityComparisonValue(b))
                    },
                    {
                        SortingOrders.CostAscending,
                        (CardModel a, CardModel b) =>
                            a.EnergyCost.GetResolved().CompareTo(b.EnergyCost.GetResolved())
                    },
                    {
                    ...
                    },
                    {
                        ...
                        (CardModel a, CardModel b) =>
                            -_cards.IndexOf(a).CompareTo(_cards.IndexOf(b))
                    },
                };
    ...
    protected int Columns =>
        (int)((_scrollContainer.Size.X + CardPadding) / (_cardSize.X + CardPadding));
    ...
    public IEnumerable<NGridCardHolder> CurrentlyDisplayedCardHolders =>
        _cardRows.SelectMany((List<NGridCardHolder> r) => r);

    public IEnumerable<CardModel> CurrentlyDisplayedCards =>
        CurrentlyDisplayedCardHolders.Select((NGridCardHolder h) => h.CardModel);
    ...
    public bool IsShowingUpgrades
    {
        ...
        set
        {
            ...
            foreach (List<NGridCardHolder> cardRow in _cardRows)
            {
                ...
                {
                    if (
                        (_isShowingUpgrades || item.CardModel.CanonicalInstance.IsUpgradable)
                        && (!_isShowingUpgrades || item.CardModel.IsUpgradable)
                    )
                    {
    ...
    public event HolderPressedEventHandler HolderPressed
    {
        add
        {
            backing_HolderPressed = (HolderPressedEventHandler)
                Delegate.Combine(backing_HolderPressed, value);
        }
        remove
        {
            backing_HolderPressed = (HolderPressedEventHandler)
                Delegate.Remove(backing_HolderPressed, value);
        }
    }
    ...
    public event HolderAltPressedEventHandler HolderAltPressed
    {
        add
        {
            backing_HolderAltPressed = (HolderAltPressedEventHandler)
                Delegate.Combine(backing_HolderAltPressed, value);
        }
        remove
        {
            backing_HolderAltPressed = (HolderAltPressedEventHandler)
                Delegate.Remove(backing_HolderAltPressed, value);
        }
    }
    ...
    public override void _Ready()
    {
        if (GetType() != typeof(NCardGrid))
        {
            ...
            throw new InvalidOperationException(
                "Don't call base._Ready()! Call ConnectSignals() instead."
            );
        }
        ConnectSignals();
    }
    ...
    protected virtual void ConnectSignals()
    {
        ...
        _scrollContainer.Connect(
            CanvasItem.SignalName.ItemRectChanged,
            Callable.From(UpdateScrollLimitBottom)
        );
        _scrollbar.Visible = false;
        _scrollbar.Connect(
            NScrollbar.SignalName.MousePressed,
            Callable.From<InputEvent>(
                delegate
                {
        ...
        _scrollbar.Connect(
            NScrollbar.SignalName.MouseReleased,
            Callable.From<InputEvent>(
                delegate
                {
    ...
    public override void _EnterTree()
    {
        ...
        GetViewport()
            .Connect(Viewport.SignalName.GuiFocusChanged, Callable.From<Control>(ProcessGuiFocus));
    }
    ...
    public override void _ExitTree()
    {
        ...
        GetViewport()
            .Disconnect(
                Viewport.SignalName.GuiFocusChanged,
                Callable.From<Control>(ProcessGuiFocus)
            );
        foreach (List<NGridCardHolder> cardRow in _cardRows)
        {
            foreach (NGridCardHolder item in cardRow)
    ...
    private void UpdateScrollLimitBottom()
    {
        ...
        _scrollbar.MouseFilter = (MouseFilterEnum)(
            (_scrollContainer.Size.Y > num && CanScroll) ? 0 : 2
        );
    }
    ...
    private void ProcessGuiFocus(Control focusedControl)
    {
        if (
            ...
            && focusedControl.GetParent() == _scrollContainer
        )
        {
    ...
    private void UpdateScrollPosition(double delta)
    {
        ...
        if (Math.Abs(num - _targetDrag) > 0.1f)
        {
            ...
            if (!_scrollbarPressed && CanScroll)
            {
                _scrollbar.SetValueWithoutAnimation(
                    Mathf.Clamp(_scrollContainer.Position.Y / ScrollLimitBottom, 0f, 1f) * 100f
                );
            }
        }
        ...
        if (!_isDragging)
        {
            ...
            {
                _targetDrag = Mathf.Lerp(
                    _targetDrag,
                    Mathf.Min(ScrollLimitBottom, ScrollLimitTop),
                    (float)delta * 12f
                );
            }
            else if (_targetDrag > Mathf.Max(ScrollLimitTop, ScrollLimitBottom))
            {
                _targetDrag = Mathf.Lerp(
                    _targetDrag,
                    Mathf.Max(ScrollLimitTop, ScrollLimitBottom),
                    (float)delta * 12f
                );
            }
        }
    ...
    )
    {
        ...
        {
            _cardsCache.Sort(
                delegate(CardModel x, CardModel y)
                {
        ...
        if (IsCardLibrary)
        {
            ...
            _sortedCardsCache.AddRange(
                _cardsCache.OrderBy(
                    (CardModel c) => c,
                    Comparer<CardModel>.Create(CompareCardVisibility)
                )
            );
            _cardsCache.Clear();
    ...
    private async Task AnimateOutInternal()
    {
        ...
        {
            tween
                .TweenProperty(item, "position:y", item.Position.Y + 40f, 0.2)
    ...
    private async Task AnimateIn()
    {
        ...
        for (int num = 0; num < list.Count; num++)
        {
            ...
            tween
                .TweenProperty(nGridCardHolder, "position:y", y, 0.4)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Back)
                .SetDelay(num2);
            tween
                .TweenProperty(nGridCardHolder, "modulate:a", 1f, 0.4)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Expo)
                .SetDelay(num2);
        }
    ...
    protected virtual void InitGrid()
    {
        ...
        _scrollContainer.SetDeferred(
            Control.PropertyName.Size,
            new Vector2(
                _scrollContainer.Size.X,
                GetContainedCardsSize().Y + 80f + 320f + (float)YOffset
            )
        );
    ...
    private Vector2 GetContainedCardsSize()
    {
        ...
        return new Vector2(Columns, totalRowCount) * _cardSize
            + new Vector2(Columns - 1, totalRowCount - 1) * CardPadding;
    }
    ...
    private void UpdateGridPositions(int index)
    {
        Vector2 vector =
            new Vector2(
                (_scrollContainer.Size.X - GetContainedCardsSize().X) * 0.5f,
                (float)YOffset + 80f
            )
            + _cardSize * 0.5f;
        foreach (List<NGridCardHolder> cardRow in _cardRows)
        {
            foreach (NGridCardHolder item in cardRow)
            {
                ...
                item.Position =
                    vector
                    + new Vector2(
                        (float)num2 * (_cardSize.X + CardPadding),
                        (float)num * (_cardSize.Y + CardPadding)
                    );
                index++;
            }
        }
    ...
    {
        return _cardRows
            .SelectMany((List<NGridCardHolder> row) => row)
            .FirstOrDefault((NGridCardHolder h) => h.CardModel == model);
    }
    ...
    protected virtual void AssignCardsToRow(List<NGridCardHolder> row, int startIndex)
    {
        for (int i = 0; i < row.Count; i++)
        {
            ...
            nGridCardHolder.ReassignToCard(
                cardModel,
                PileType.None,
                null,
                GetCardVisibility(cardModel)
            );
            nGridCardHolder.Visible = true;
    ...
    protected virtual void UpdateGridNavigation()
    {
        for (int i = 0; i < _cardRows.Count; i++)
        {
            for (int j = 0; j < _cardRows[i].Count; j++)
            {
                ...
                nCardHolder.FocusNeighborLeft = (
                    (j > 0)
                        ? _cardRows[i][j - 1].GetPath()
                        : _cardRows[i][_cardRows[i].Count - 1].GetPath()
                );
                nCardHolder.FocusNeighborRight = (
                    (j < _cardRows[i].Count - 1)
                        ? _cardRows[i][j + 1].GetPath()
                        : _cardRows[i][0].GetPath()
                );
                nCardHolder.FocusNeighborTop = (
                    (i > 0) ? _cardRows[i - 1][j].GetPath() : _cardRows[i][j].GetPath()
                );
                nCardHolder.FocusNeighborBottom = (
                    (i < _cardRows.Count - 1 && j < _cardRows[i + 1].Count)
                        ? _cardRows[i + 1][j].GetPath()
                        : _cardRows[i][j].GetPath()
                );
            }
        }
    }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateScrollLimitBottom,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.CalculateRowsNeeded,
                new PropertyInfo(
                    Variant.Type.Int,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetContainedCardsSize,
                new PropertyInfo(
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateGridPositions,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.AllocateCardHolders,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateGridNavigation,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._scrollingEnabled,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Float,
                PropertyName.ScrollLimitBottom,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._scrollbarPressed,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Int,
                PropertyName._slidingWindowCardIndex,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._cardsAnimatingOutForSetCards,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._isShowingUpgrades,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._lastFocusedHolder,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName.IsShowingUpgrades,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._slidingWindowCardIndex,
            Variant.From(in _slidingWindowCardIndex)
        );
        ...
        info.AddProperty(
            PropertyName._cardsAnimatingOutForSetCards,
            Variant.From(in _cardsAnimatingOutForSetCards)
        );
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<HolderPressedEventHandler>(
                SignalName.HolderPressed,
                out var value18
            )
        )
        {
        ...
        if (
            info.TryGetSignalEventDelegate<HolderAltPressedEventHandler>(
                SignalName.HolderAltPressed,
                out var value19
            )
        )
        {

...
public class NTransformPreview : Control
{
    ...
    public void Initialize(IEnumerable<CardTransformation> cardTransformations)
    {
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            NPreviewCardHolder nPreviewCardHolder = NPreviewCardHolder.Create(
                NCard.Create(cardTransformation.Original),
                nPlayerHand == null,
                nPlayerHand != null
            );
            _before.AddChildSafely(nPreviewCardHolder);
            nPreviewCardHolder.FocusMode = FocusModeEnum.All;
            nPreviewCardHolder.CardNode.UpdateVisuals(
                cardTransformation.Original.Pile.Type,
                CardPreviewMode.Normal
            );
            nPreviewCardHolder.SetCardScale(Vector2.One * num2);
            int num3 = list.Count - i;
            nPreviewCardHolder.Position = new Vector2(
                (0f - ((float)num3 - 0.5f)) * 300f * num2 - (float)(num3 - 1) * 30f,
                0f
            );
            NCard card = (
                (cardTransformation.Replacement == null)
                    ? NCard.Create(cardTransformation.Original)
                    : NCard.Create(cardTransformation.Replacement)
            );
            NPreviewCardHolder nPreviewCardHolder2 = NPreviewCardHolder.Create(
                card,
                showHoverTips: true,
                scaleOnHover: false
            );
            ...
            nPreviewCardHolder2.CardNode.UpdateVisuals(
                cardTransformation.Original.Pile.Type,
                CardPreviewMode.Normal
            );
            nPreviewCardHolder2.Scale = Vector2.One * num2;
            nPreviewCardHolder2.Position = new Vector2(
                ((float)i + 0.5f) * 300f * num2 + (float)i * 30f,
                0f
            );
            if (cardTransformation.Replacement == null)
            {
                ...
                TaskHelper.RunSafely(
                    CycleThroughCards(
                        possibleTransformations: (cardTransformation.ReplacementOptions == null)
                            ? CardFactory.GetDefaultTransformationOptions(
                                cardTransformation.Original,
                                cardTransformation.IsInCombat
                            )
                            : cardTransformation.ReplacementOptions,
                        holder: nPreviewCardHolder2,
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.RemoveExistingCards,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                ...
                PropertyName.SelectedCardPosition,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

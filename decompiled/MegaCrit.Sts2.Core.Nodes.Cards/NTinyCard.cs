...
public class NTinyCard : NButton
{
    ...
    private void SetCardPortraitShape(CardType type)
    {
        switch (type)
        {
            case CardType.Attack:
                _cardPortrait.Texture = PreloadManager.Cache.GetCompressedTexture2D(
                    "res://images/packed/run_history/attack_portrait.png"
                );
                _cardPortraitShadow.Texture = PreloadManager.Cache.GetCompressedTexture2D(
                    "res://images/packed/run_history/attack_portrait_shadow.png"
                );
                break;
            case CardType.Power:
                _cardPortrait.Texture = PreloadManager.Cache.GetCompressedTexture2D(
                    "res://images/packed/run_history/power_portrait.png"
                );
                _cardPortraitShadow.Texture = PreloadManager.Cache.GetCompressedTexture2D(
                    "res://images/packed/run_history/power_portrait_shadow.png"
                );
                break;
            default:
                _cardPortrait.Texture = PreloadManager.Cache.GetCompressedTexture2D(
                    "res://images/packed/run_history/skill_portrait.png"
                );
                _cardPortraitShadow.Texture = PreloadManager.Cache.GetCompressedTexture2D(
                    "res://images/packed/run_history/skill_portrait_shadow.png"
                );
                break;
        }
    }
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetCardPortraitShape,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.GetBannerColor && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<Color>(
                GetBannerColor(VariantUtils.ConvertTo<CardRarity>(in args[0]))
            );
            return true;
        }
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._cardPortraitShadow,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

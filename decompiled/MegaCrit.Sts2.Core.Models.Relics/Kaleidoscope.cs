...
public sealed class Kaleidoscope : RelicModel
{
    ...
    public override async Task AfterObtained()
    {
        ...
        CardCreationOptions rerollOptions = CardCreationOptions.ForNonCombatWithDefaultOdds(
            Array.Empty<CardModel>()
        );
        for (int i = 0; i < base.DynamicVars.Cards.IntValue; i++)
        {
            List<CardModel> list2 = new List<CardModel>();
            IEnumerable<CardPoolModel> enumerable = base
                .Owner.UnlockState.CharacterCardPools.Where(
                    (CardPoolModel p) => p != base.Owner.Character.CardPool
                )
                .ToList()
            ...
            foreach (CardPoolModel item in enumerable)
            {
                CardCreationOptions options = new CardCreationOptions(
                    new global::_003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(item),

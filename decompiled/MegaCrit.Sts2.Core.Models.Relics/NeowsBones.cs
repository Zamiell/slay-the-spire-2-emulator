...
public sealed class NeowsBones : RelicModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[2] { new DynamicVar("Relics", 2m), new DynamicVar("Curses", 1m) }
        );

    private static IEnumerable<RelicModel> GetValidRelics(Player owner)
    {
        return (
            from o in ModelDb.Event<Neow>().AllPossibleOptions
    ...
    public override async Task AfterObtained()
    {
        ...
        List<Reward> rewards = list.Take(base.DynamicVars["Relics"].IntValue)
            .Select((Func<RelicModel, Reward>)((RelicModel r) => new RelicReward(r, base.Owner)))
            .ToList();
        await new RewardsSet(base.Owner)

...
public sealed class PunchDagger : RelicModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlySingleElementList<DynamicVar>(
            new DynamicVar("Momentum", 5m)
        );

    public override bool HasUponPickupEffect => true;

    protected override IEnumerable<IHoverTip> ExtraHoverTips =>
        HoverTipFactory.FromEnchantment<Momentum>(base.DynamicVars["Momentum"].IntValue);
    ...
    public override async Task AfterObtained()
    {
        ...
        foreach (
            CardModel item in await CardSelectCmd.FromDeckForEnchantment(
                base.Owner,
                canonicalMomentum,
                base.DynamicVars["Momentum"].IntValue,
                prefs
            )
        )
        {
            CardCmd.Enchant(
                canonicalMomentum.ToMutable(),
                item,
                base.DynamicVars["Momentum"].IntValue
            );

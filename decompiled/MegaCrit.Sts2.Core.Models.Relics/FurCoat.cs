...
public sealed class FurCoat : RelicModel
{
    ...
    private ActMap AddMarkedRooms(ActMap map)
    {
        ...
        if (!flag)
        {
            flag = !markedCoords.TrueForAll(
                (MapCoord c) =>
                    map.HasPoint(c)
                    && (
                        map.GetPoint(c).PointType == MapPointType.Monster
                        || map.GetPoint(c).PointType == MapPointType.Elite
                    )
            );
        }
        if (flag)
        {
            Rng rng = new Rng(
                (uint)(
                    (int)base.Owner.RunState.Rng.Seed
                    + (int)base.Owner.NetId
                    + StringHelper.GetDeterministicHashCode("FurCoat")
                )
            );
            List<MapPoint> list = map.GetAllMapPoints()
                .Where(
                    delegate(MapPoint p)
                    {
                        ...
                        return (uint)(pointType - 5) <= 1u
                            && !p.Quests.Any((AbstractModel q) => q is FurCoat);
                    }
                )
                .ToList();
            ...
            foreach (MapCoord item2 in markedCoords)
            {
                ...
                if (point == null)
                {
                    throw new InvalidOperationException(
                        $"Loaded a fur coat map with coordinate {item2}, but the generated map does not contain that coordinate!"
                    );
                }
                point.AddQuest(this);
            }
    ...
    public override async Task BeforeCombatStart()
    {
        ...
        if (
            markedCoords == null
            || !markedCoords.Contains(base.Owner.RunState.CurrentMapPoint.coord)
        )
        {
            return;
        }
    ...
    public List<MapCoord>? GetMarkedCoords()
    {
        ...
        {
            list.Add(new MapCoord { col = FurCoatCoordCols[i], row = FurCoatCoordRows[i] });
        }
        return list;
    }
}

...
public sealed class LeafyPoultice : RelicModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlySingleElementList<DynamicVar>(new MaxHpVar(12m));
    ...
    public override async Task AfterObtained()
    {
        await CreatureCmd.LoseMaxHp(
            ...
            base.Owner.Creature,
            base.DynamicVars.MaxHp.BaseValue,
            isFromCard: false
        );
        List<CardModel> source = PileType
            .Deck.GetPile(base.Owner)
            .Cards.Where((CardModel c) => c.Rarity == CardRarity.Basic)
            .ToList();
        CardModel cardModel = source.FirstOrDefault(
            (CardModel c) => c.Tags.Contains(CardTag.Strike)
        );
        CardModel cardModel2 = source.FirstOrDefault(
            (CardModel c) => c.Tags.Contains(CardTag.Defend)
        );

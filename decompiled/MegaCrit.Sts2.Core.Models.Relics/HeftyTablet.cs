...
public sealed class HeftyTablet : RelicModel
{
    ...
    public override async Task AfterObtained()
    {
        ...
        List<CardModel> options2 = (
            from r in CardFactory.CreateForReward(
                base.Owner,
        ...
        List<CardModel> list = new List<CardModel>(1)
        {
            base.Owner.RunState.CreateCard<Injury>(base.Owner),
        };
        if (chosenCard != null)
        {
        ...
        foreach (CardModel item in options2)
        {
            if (item != chosenCard)
            {
                base.Owner.RunState.CurrentMapPointHistoryEntry?.GetEntry(base.Owner.NetId)
                    .CardChoices.Add(new CardChoiceHistoryEntry(item, wasPicked: false));
            }
        }
    }
}

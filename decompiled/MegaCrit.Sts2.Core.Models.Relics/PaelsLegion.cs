...
public sealed class PaelsLegion : RelicModel
{
    ...
    {
        get { return _triggeredBlockLastTurn; }
        set
        {
            AssertMutable();
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlySingleElementList<DynamicVar>(new DynamicVar("Turns", 2m));

    public override async Task AfterObtained()
    {
        Skin = new Rng((uint)(base.Owner.NetId + base.Owner.RunState.Rng.Seed)).NextItem(
            SkinOptions
        );
        if (CombatManager.Instance.IsInProgress)
        {
    ...
    public override async Task AfterCardPlayed(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        if (AffectedCardPlay != null && AffectedCardPlay == cardPlay)
        {
            ...
            MegaCrit.Sts2.Core.Models.Monsters.PaelsLegion paelsLegion =
                (MegaCrit.Sts2.Core.Models.Monsters.PaelsLegion)
                    base
                        .Owner.PlayerCombatState.GetPet<MegaCrit.Sts2.Core.Models.Monsters.PaelsLegion>()
                        .Monster;

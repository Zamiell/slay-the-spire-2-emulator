...
public sealed class ScrollBoxes : RelicModel
{
    ...
    public override async Task AfterObtained()
    {
        ...
        {
            list2.Add(
                item.Select((CardModel c) => base.Owner.RunState.CreateCard(c, base.Owner)).ToList()
            );
        }
    ...
    {
        IEnumerable<CardModel> unlockedCards = GetCardPool(player.Character)
    ...
    public static List<IReadOnlyList<CardModel>> GenerateRandomBundles(Player player)
    {
        ...
        CardCreationOptions options = CardCreationOptions
            .ForNonCombatWithUniformOdds(
                new global::_003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(cardPool),
        ...
        for (int num = 0; num < 2; num++)
        {
            if (flag && rewards.NextInt(100) < 1)
            {
                ...
                list.Add(
                    new global::_003C_003Ez__ReadOnlyArray<CardModel>(
                        new CardModel[3] { cardModel, cardModel, cardModel }
                    )
                );
                continue;
            }
            List<CardModel> list2 = new List<CardModel>();
            List<CardModel> list3 = source
                .Where((CardModel c) => !usedCardIds.Contains(c.Id))
                .ToList();

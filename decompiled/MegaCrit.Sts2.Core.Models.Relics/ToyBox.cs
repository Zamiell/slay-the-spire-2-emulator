...
public sealed class ToyBox : RelicModel
{
    ...
    public override bool IsUsedUp =>
        CombatsSeen >= base.DynamicVars["Combats"].IntValue * base.DynamicVars["Relics"].IntValue;
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[2] { new DynamicVar("Relics", 4m), new DynamicVar("Combats", 3m) }
        );
    ...
    public override async Task AfterCombatEnd(CombatRoom __)
    {
        ...
        if (CombatsSeen % base.DynamicVars["Combats"].IntValue == 0)
        {
            ...
            RelicModel relicModel = base.Owner.Relics.FirstOrDefault(
                (RelicModel r) => r != null && r.IsWax && !r.IsMelted
            );
            if (relicModel != null)
            {

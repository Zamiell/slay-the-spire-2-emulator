...
public sealed class PaelsTooth : RelicModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[2] { new CardsVar(5), new StringVar("CardTitles") }
        );
    ...
    public override async Task AfterObtained()
    {
        IEnumerable<CardModel> enumerable = (
            await CardSelectCmd.FromDeckForRemoval(
                ...
                player: base.Owner,
                filter: (CardModel c) => c.IsUpgradable
            )
        ).OrderBy<CardModel, string>((CardModel c) => c.Id.Entry, StringComparer.Ordinal);
    ...
    public override async Task AfterCombatEnd(CombatRoom room)
    {
        if (!base.Owner.Creature.IsDead && SerializableCards.Count != 0)
        {
            ...
            SerializableCard serializableCard = base.Owner.PlayerRng.Rewards.NextItem(
                SerializableCards
            );
            ...
            base.Status = (
                (SerializableCards.Count <= 0) ? RelicStatus.Disabled : RelicStatus.Normal
            );
    ...
    private void UpdateCardList()
    {
        ...
        else
        {
            stringVar.StringValue = string.Join(
                '\n',
                SerializableCards.Select(
                    (SerializableCard c) => "- " + SaveUtil.CardOrDeprecated(c.Id).Title
                )
            );
        }

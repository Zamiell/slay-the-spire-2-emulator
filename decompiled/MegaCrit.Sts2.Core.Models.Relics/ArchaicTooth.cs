...
public sealed class ArchaicTooth : RelicModel
{
    ...
    {
        get { return _serializableStarterCard; }
        private set
        {
            AssertMutable();
    ...
    {
        get { return _serializableAncientCard; }
        private set
        {
            AssertMutable();
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[2] { new StringVar("StarterCard"), new StringVar("AncientCard") }
        );
    ...
    public bool SetupForPlayer(Player player)
    {
        ...
        if (transcendenceStarterCard != null)
        {
            ...
            AncientCard = GetTranscendenceTransformedCard(transcendenceStarterCard)
                .ToSerializable();
            UpdateHoverTips();
            return true;
        }
    ...
    {
        return player.Deck.Cards.FirstOrDefault(
            (CardModel c) => TranscendenceUpgrades.ContainsKey(c.Id)
        );
    }
        ...
        if (TranscendenceUpgrades.TryGetValue(starterCard.Id, out CardModel value))
        {
            ...
            if (starterCard.Enchantment != null)
            {
                EnchantmentModel enchantmentModel = (EnchantmentModel)
                    starterCard.Enchantment.MutableClone();
    ...
    public override async Task AfterObtained()
    {
        ...
        await CardCmd.Transform(
            transcendenceStarterCard,
            GetTranscendenceTransformedCard(transcendenceStarterCard)
        );
    }
}

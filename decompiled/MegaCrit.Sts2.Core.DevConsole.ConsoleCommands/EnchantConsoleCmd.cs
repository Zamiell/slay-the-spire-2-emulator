...
public class EnchantConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Enchants a card in the player's hand with the specified enchantment.";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        if (args.Length > 1 && !int.TryParse(args[1], out result))
        {
            return new CmdResult(
                success: false,
                "Arg 2 must be the enchantment amount (int), got '" + args[1] + "'."
            );
        }
        ...
        return new CmdResult(
            success: true,
            $"Enchanted card {cardModel.Title} with {result} {enchantmentModel.Title.GetFormattedText()}"
        );
    }
    ...
    public override CompletionResult GetArgumentCompletions(Player? player, string[] args)
    {
        if (args.Length <= 1)
        {
            List<string> candidates = ModelDb
                .DebugEnchantments.Select((EnchantmentModel e) => e.Id.Entry)
                .ToList();

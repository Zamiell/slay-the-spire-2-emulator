...
public class AncientConsoleCmd : AbstractConsoleCmd
{
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        ModelId id = new ModelId(
            ModelDb.GetCategory(typeof(EventModel)),
        ...
        if (
            choice != null
            && !ancientEventModel.AllPossibleOptions.Any(
                (EventOption option) => option.TextKey.Contains(choice)
            )
        )
        {
            return new CmdResult(success: false, "invalid ancient choice.");
        }
        EventRoom room = new EventRoom(byIdOrNull) { OnStart = SetDebugOption };
        ...
        return new CmdResult(
            task,
            success: true,
            "Opened Ancient Event. Forced " + (choice ?? "no") + " option"
        );
    ...
    public override CompletionResult GetArgumentCompletions(Player? player, string[] args)
    {
        if (args.Length <= 1)
        {
            List<string> candidates = ModelDb
                .AllAncients.Select((AncientEventModel ancient) => ancient.Id.Entry)
                .ToList();
        ...
        if (args.Length == 2)
        {
            ...
            if (byIdOrNull is AncientEventModel ancientEventModel)
            {
                List<string> candidates2 = ancientEventModel
                    .AllPossibleOptions.Select(
                        (EventOption option) => option.TextKey.Split('.').Last()
                    )
                    .ToList();

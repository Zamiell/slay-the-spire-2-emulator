...
public class LogConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Set log level for specific log types. Type can be: "
        ...
        else
        {
            if (!TryParseEnumCaseInsensitive(args[0], out enumVal2))
            {
                return new CmdResult(
                    ...
                    "First argument '"
                        + args[0]
                        + "' could not be parsed as either a log level or type!"
                );
            }
            if (args.Length <= 1)
            {
                return new CmdResult(
                    success: false,
                    "Must supply a log level as the second argument!"
                );
            }
            if (!TryParseEnumCaseInsensitive(args[1], out enumVal))
            {
                return new CmdResult(
                    success: false,
                    "Second argument '" + args[1] + "' could not be parsed as a log level!"
                );
            }
        }
    ...
    public static bool TryParseEnumCaseInsensitive<T>(string str, out T? enumVal)
        where T : struct, Enum
    {

...
public class GetLogsConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Gathers logs, automatically zips them to a file containing 'name', and opens the directory containing the zip file.";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        if (flag)
        {
            using FileAccessStream outputStream = new FileAccessStream(
                bugReportPath,
    ...
    public static async Task GrabLogs(string bugReportPath)
    {
        ...
        {
            Log.Error(
                $"Error {error}: Cannot open OS file manager. Files zipped to '{bugReportPath}'"
            );
        }
        else
        {
    ...
    public static void ZipFiles(Stream outputStream, byte[] screenshotBytes)
    {
        ...
        string text = ProjectSettings.GlobalizePath(
            UserDataPathProvider.GetAccountScopedBasePath("")
        );
        ...
        {
            foreach (
                string item in Directory.EnumerateFiles(path, "*", SearchOption.AllDirectories)
            )
            {
        ...
        if (OS.GetName() == "Windows")
        {
            ...
            if (Directory.Exists(path3))
            {
                ...
                list.AddRange(
                    from f in Directory.GetFiles(path3)
                    where
                        Path.GetFileName(f).StartsWith(exeName, StringComparison.OrdinalIgnoreCase)
                    select f
                );
            }
        }
        if (list.Count > 0)
        {
            ...
            ArchiveFile(
                text4,
                archive,
                "crashes/crash_" + dateTimeOffset.ToString(text5) + Path.GetExtension(text4)
            );
        }
    ...
    public static void ZipFeedbackLogs(Stream outputStream, int profileId)
    {
        ...
        if (list.Count > 0)
        {
            ...
            {
                Log.Warn(
                    $"Crash dump too large for feedback upload ({length / 1048576} MB), skipping: {text4}"
                );
            }
        }
        TryCollectLinuxCoreDump(archive);
        ...
        {
            string text6 = Directory
                .EnumerateFiles(path4, "*", SearchOption.TopDirectoryOnly)
        ...
        {
            if (
                allSaveFile.EndsWith(".corrupt")
                && DateTimeOffset.Now - File.GetLastWriteTime(allSaveFile) < TimeSpan.FromDays(1)
            )
            {
    ...
    private static void TryCollectLinuxCoreDump(ZipArchive archive)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    ZipArchiveEntry zipArchiveEntry = archive.CreateEntry(
                        "crashes/coredump_info.txt"
                    );
        ...
        try
        {
            ...
            if (File.Exists(text))
            {
                ...
                {
                    Log.Warn(
                        $"Core dump is {length / 1048576} MB which exceeds the {200} MB limit, skipping"
                    );
                }
                else if (length > 0)
                {
    ...
    private static IEnumerable<string> GetAllSaveFiles(string accountBasePath)
    {
        ...
        foreach (
            string item in Directory.EnumerateFiles(
                accountBasePath,
                "*",

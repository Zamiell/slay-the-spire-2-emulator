...
public class LeaderboardConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Adds scores to the leaderboard. Option can be upload|random. If random, <count> random scores will be uploaded to leaderboard <name>. If upload, one score will be uploaded to leaderboard <name> with <score>.";
        ...
        if (text == "upload")
        {
            if (args.Length < 3)
            {
                return new CmdResult(
                    success: false,
                    "Score must be specified when upload is the option.\nUse - as the leaderboard name for the default."
                );
            }
            ...
            return new CmdResult(
                success: true,
                $"Uploading score {result2} to leaderboard {text2}"
            );
        }
        return new CmdResult(
            success: false,
            "Invalid option " + text + ", must be upload or random"
        );
    }

    private async Task UploadRandomScores(string leaderboardName, int count)
    {
        ILeaderboardHandle handle = await LeaderboardManager.GetOrCreateLeaderboard(
            leaderboardName
        );
        Rng rng = new Rng(Rng.Chaotic.NextUnsignedInt());
        for (int i = 0; i < count; i++)
        {
            LeaderboardManager.DebugAddEntry(
                handle,
            ...
            List<string> candidates = (
                (!string.IsNullOrWhiteSpace(partial))
                    ? _validOptions
                        .Where(
                            (string option) =>
                                option.Contains(partial, StringComparison.OrdinalIgnoreCase)
                        )
                        .ToList()
                    : _validOptions
            );

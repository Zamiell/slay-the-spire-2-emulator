...
public class DamageConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Damage all enemies, or target creature if index is given (0 is player).";
        ...
        else
        {
            if (!int.TryParse(args[1], out var result2))
            {
                return new CmdResult(
                    success: false,
                    "Arg 2 must be the target index if specified."
                );
            }
            if (result2 < 0 || result2 >= combatState.Creatures.Count)
            {
                return new CmdResult(
                    success: false,
                    $"Invalid target index {result2}. Valid range: 0-{combatState.Creatures.Count - 1}"
                );
            }
            enumerable = new global::_003C_003Ez__ReadOnlySingleElementList<Creature>(
                combatState.Creatures[result2]
            );
        }
        IEnumerable<string> values = enumerable.Select(
            (Creature c) => (!c.IsPlayer) ? c.Monster.Id.Entry : "PLAYER"
        );

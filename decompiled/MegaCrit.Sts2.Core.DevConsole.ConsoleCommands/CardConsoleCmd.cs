...
public class CardConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Spawns a card into a pile (hand by default). Screaming snake case ('BODY_SLAM', not 'Body Slam').";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        if (args.Length >= 2 && !AbstractConsoleCmd.TryParseEnum<PileType>(args[1], out result))
        {
            return new CmdResult(
                ...
                "Unknown pile '"
                    + args[1]
                    + "'. Valid piles: "
                    + string.Join(", ", Enum.GetNames<PileType>())
            );
        }
        if (result == PileType.Hand)
        {
            ...
            if (count >= CardPile.MaxCardsInHand)
            {
                return new CmdResult(
                    success: false,
                    $"The hand is full ({count}/{CardPile.MaxCardsInHand})."
                );
            }
        }
        string cardName = args[0].ToUpperInvariant();
        CardModel cardModel = ModelDb.AllCards.FirstOrDefault(
            (CardModel c) => c.Id.Entry == cardName
        );
        if (cardModel == null)
        {
        ...
        return new CmdResult(
            task,
            success: true,
            $"Added card '{cardModel.Id.Entry}' to '{result}'"
        );
    }
    ...
    public override CompletionResult GetArgumentCompletions(Player? player, string[] args)
    {
        if (args.Length <= 1)
        {
            List<string> candidates = ModelDb
                .AllCards.Select((CardModel card) => card.Id.Entry)
                .ToList();

...
public class RemoveCardConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Removes a card from your Hand or Deck. Screaming snake case ('BODY_SLAM', not 'Body Slam').";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        if (args.Length >= 2 && !AbstractConsoleCmd.TryParseEnum<PileType>(args[1], out result))
        {
            return new CmdResult(
                success: false,
                "Unknown pile '" + args[1] + "'. Valid piles: Hand, Deck"
            );
        }
        if (cardModel == null)
        {
    ...
    public override CompletionResult GetArgumentCompletions(Player? player, string[] args)
    {
        if (args.Length <= 1)
        {
            ...
            {
                IEnumerable<string> collection = PileType
                    .Hand.GetPile(player)
                    .Cards.Select((CardModel c) => c.Id.Entry)
                    .Distinct();
                IEnumerable<string> collection2 = PileType
                    .Deck.GetPile(player)
                    .Cards.Select((CardModel c) => c.Id.Entry)
                    .Distinct();

...
public class ArtConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Lists all the content of the specified type that is missing art. " + TypesHint;
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        switch (text)
        {
                ...
                foreach (AfflictionModel debugAffliction in ModelDb.DebugAfflictions)
                {
                    if (!debugAffliction.HasOverlay)
                    {
                        list.Add(
                            new MissingArt(
                                debugAffliction,
                                debugAffliction.OverlayPath,
                                debugAffliction.DynamicDescription.GetRawText()
                            )
                        );
                    }
                }
                    ...
                    foreach (string allPortraitPath in allCard.AllPortraitPaths)
                    {
                        ...
                        if (text3 == null || !AtlasManager.HasSprite(text3, spriteName))
                        {
                            list.Add(
                                new MissingArt(
                                    allCard,
                                    allCard.PortraitPath,
                                    allCard.Description.GetRawText()
                                )
                            );
                        }
                    }
            ...
            case "enchantment":
                ...
                {
                    if (
                        !(debugEnchantment is DeprecatedEnchantment)
                        && !(debugEnchantment.IconPath == debugEnchantment.IntendedIconPath)
                    )
                    {
                        list.Add(
                            new MissingArt(
                                debugEnchantment,
                                debugEnchantment.IntendedIconPath,
                                debugEnchantment.DynamicDescription.GetRawText()
                            )
                        );
                    }
                }
                ...
                foreach (PowerModel allPower in ModelDb.AllPowers)
                {
                    if (!(allPower.IconPath == allPower.PackedIconPath))
                    {
                        list.Add(
                            new MissingArt(
                                allPower,
                                allPower.PackedIconPath,
                                allPower.Description.GetRawText()
                            )
                        );
                    }
                }
                ...
                foreach (RelicModel allRelic in ModelDb.AllRelics)
                {
                    if (!(allRelic.IconPath == allRelic.PackedIconPath))
                    {
                        list.Add(
                            new MissingArt(
                                allRelic,
                                allRelic.PackedIconPath,
                                allRelic.DynamicDescription.GetRawText()
                            )
                        );
                    }
                }
            ...
            default:
                return new CmdResult(
                    success: false,
                    "'" + args[0] + "' is not a valid type. " + TypesHint
                );
        }
        string value = ((list.Count == 1) ? "" : "s");
        StringBuilder stringBuilder = new StringBuilder(
            $"{list.Count} {text}{value} with missing art:\n"
        );

...
public class ApplyPowerConsoleCmd : AbstractConsoleCmd
{
    ...
    private static IEnumerable<PowerModel> AllPowers
    {
        get
        {
            ...
            {
                _allPowers = ModelDb
                    .AllAbstractModelSubtypes.Where((Type t) => t.IsSubclassOf(typeof(PowerModel)))
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        if (!int.TryParse(args[1], out var result))
        {
            return new CmdResult(
                success: false,
                "Arg 1 must be the amount of power to be applied."
            );
        }
        ...
        if (result2 < 0 || result2 >= creatures.Count)
        {
            return new CmdResult(
                success: false,
                $"Invalid target index {result2}. Valid range: 0-{creatures.Count - 1}"
            );
        }
        Creature creature = creatures[result2];
        PowerModel powerModel = creature.Powers.FirstOrDefault(
            (PowerModel p) => p.GetType() == power.GetType()
        );
        PlayerChoiceContext choiceContext = new BlockingPlayerChoiceContext();
        Task task = (
            (power.InstanceType == PowerInstanceType.None && powerModel != null)
                ? PowerCmd.ModifyAmount(choiceContext, powerModel, result, null, null)
                : PowerCmd.Apply(choiceContext, power.ToMutable(), creature, result, null, null)
        );
        string reference = (creature.IsPlayer ? "PLAYER" : creature.Monster.Id.Entry);
        return new CmdResult(
            task,
            success: true,
            "AppliedPower: [" + string.Join(",", new ReadOnlySpan<string>(in reference)) + "]"
        );
    }

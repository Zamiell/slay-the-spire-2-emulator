...
public class PotionConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Adds potion to belt. Screaming snake case ('ENTROPIC_BREW', not 'Entropic Brew').";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        PotionModel potionModel = ModelDb.AllPotions.FirstOrDefault(
            (PotionModel p) => p.Id.Entry == potionId
        );
        if (potionModel == null)
        {
    ...
    public override CompletionResult GetArgumentCompletions(Player? player, string[] args)
    {
        if (args.Length <= 1)
        {
            List<string> candidates = ModelDb
                .AllPotions.Select((PotionModel p) => p.Id.Entry)
                .ToList();

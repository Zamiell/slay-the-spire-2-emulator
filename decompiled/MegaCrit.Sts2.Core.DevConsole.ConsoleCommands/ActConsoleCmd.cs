...
public class ActConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Jumps to an act. If integer, will jump to that act. Otherwise, replaces the current act with the act passed.";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        if (int.TryParse(args[0], out var result))
        {
            ...
            if (result > count || result < 1)
            {
                return new CmdResult(
                    success: false,
                    $"The act you are trying to navigate to does not exist. Select act indexes between: 1-{count}"
                );
            }
        ...
        return new CmdResult(
            task2,
            success: true,
            "Replaced current act with act " + actName + "."
        );
    }

...
public class RelicConsoleCmd : AbstractConsoleCmd
{
    ...
    private static RelicModel? GetRelicById(string id)
    {
        ...
        List<RelicModel> source = ModelDb
            .AllRelics.Where((RelicModel r) => r.Id.Entry.Contains(id))
            .ToList();
    ...
    public override CompletionResult GetArgumentCompletions(Player? player, string[] args)
    {
        ...
        if (args.Length == 1)
        {
            ...
            List<string> candidates = ModelDb
                .AllRelics.Select((RelicModel r) => r.Id.Entry)
                .ToList();
            return CompleteArgument(
                candidates,
                Array.Empty<string>(),
                ...
                (string candidate, string partialArg) =>
                    candidate.Contains(partialArg, StringComparison.OrdinalIgnoreCase)
            );
        }
        if (args.Length == 2)
        {
            List<string> candidates2 = ModelDb
                .AllRelics.Select((RelicModel r) => r.Id.Entry)
                .ToList();
            return CompleteArgument(
                candidates2,
                ...
                args[1],
                CompletionType.Argument,
                (string candidate, string partial) =>

...
public class UnlockConsoleCmd : AbstractConsoleCmd
{
    private static readonly Dictionary<
        string,
        Action<UnlockConsoleCmd, List<string>?>
    > _unlockActions = new Dictionary<string, Action<UnlockConsoleCmd, List<string>>>
    {
    ...
    private static readonly Dictionary<string, Func<IEnumerable<string>>> _validIdSources =
    ...
    public override string Description =>
        "Marks all cards/potions/relics/monsters/events/acts/epochs/ascensions as discovered, or 'all' to unlock everything.";
    ...
    {
        if (
            !_unlockActions.TryGetValue(
                discoveryType,
                out Action<UnlockConsoleCmd, List<string>> value
            )
        )
        {
            return new CmdResult(
                success: false,
                "Argument " + discoveryType + " not recognized as a discovery type.\n" + Args
            );
        }
        if (args != null)
        {
            ...
            if (_validIdSources.TryGetValue(discoveryType, out Func<IEnumerable<string>> value2))
            {
                ...
                if (list.Count > 0)
                {
                    return new CmdResult(
                        success: false,
                        "Unknown " + discoveryType + ": " + string.Join(", ", list)
                    );
                }
            }
        }
        ...
        return new CmdResult(
            success: true,
            "Unlocked " + discoveryType + " " + ((args != null) ? string.Join(", ", args) : "")
        );
    }

    private void UnlockCards(List<string>? cards)
    {
        IEnumerable<ModelId> enumerable =
            cards?.Select((string c) => new ModelId("CARD", c))
            ?? ModelDb.AllCards.Select((CardModel c) => c.Id);
    ...
    {
        IEnumerable<ModelId> enumerable =
            relics?.Select((string c) => new ModelId("RELIC", c))
            ?? ModelDb.AllRelics.Select((RelicModel c) => c.Id);
        foreach (ModelId item in enumerable)
        {
    ...
    {
        IEnumerable<ModelId> enumerable =
            potions?.Select((string c) => new ModelId("POTION", c))
            ?? ModelDb.AllPotions.Select((PotionModel c) => c.Id);
        foreach (ModelId item in enumerable)
        {
    ...
    {
        IEnumerable<ModelId> enumerable =
            monsters?.Select((string c) => new ModelId("MONSTER", c))
            ?? ModelDb.Monsters.Select((MonsterModel c) => c.Id);
        foreach (ModelId item in enumerable)
        {
            EnemyStats orCreateEnemyStats = SaveManager.Instance.Progress.GetOrCreateEnemyStats(
                item
            );
            if (orCreateEnemyStats.FightStats.Count == 0)
            {
                orCreateEnemyStats.FightStats.Add(
                    new FightStats { Character = ModelDb.GetId<Ironclad>(), Wins = 1 }
                );
            }
        }
    }
    ...
    {
        IEnumerable<ModelId> enumerable =
            events?.Select((string c) => new ModelId("EVENT", c))
            ?? ModelDb.AllEvents.Select((EventModel c) => c.Id);
        foreach (ModelId item in enumerable)
        {
    ...
    {
        IEnumerable<string> enumerable = (epochs ?? EpochModel.AllEpochIds).Except(
            from e in SaveManager.Instance.Progress.Epochs
    ...
    private void UnlockAscensions(List<string>? ascensions)
    {
        ...
        foreach (CharacterModel allCharacter in ModelDb.AllCharacters)
        {
            CharacterStats orCreateCharacterStats =
                SaveManager.Instance.Progress.GetOrCreateCharacterStats(allCharacter.Id);
        ...
        if (args.Length <= 1)
        {
            ...
            List<string> candidates = (
                (!string.IsNullOrWhiteSpace(partial))
                    ? _validDiscoveryTypes
                        .Where(
                            (string type) =>
                                type.Contains(partial, StringComparison.OrdinalIgnoreCase)
                        )
                        .ToList()
                    : _validDiscoveryTypes
            );

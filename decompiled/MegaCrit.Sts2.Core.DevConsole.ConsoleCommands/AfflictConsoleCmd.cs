...
public class AfflictConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Apply the specified affliction to a card in the player's hand";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        if (args.Length > 1 && !int.TryParse(args[1], out result))
        {
            return new CmdResult(
                success: false,
                "Arg 2 must be the affliction count (int), got '" + args[1] + "'."
            );
        }
        if (args.Length > 2 && !int.TryParse(args[2], out result2))
        {
            return new CmdResult(
                success: false,
                "Arg 3 must be the hand index (int), got '" + args[2] + "'."
            );
        }
        ...
        if (result2 < 0 || result2 >= count)
        {
            return new CmdResult(
                success: false,
                $"Invalid hand index {result2}. Valid range: 0-{count - 1}."
            );
        }
        ...
        return new CmdResult(
            task,
            success: true,
            $"Afflicted card {cardModel.Title} with {result} {afflictionModel.Title.GetFormattedText()}"
        );
    }
    ...
    public override CompletionResult GetArgumentCompletions(Player? player, string[] args)
    {
        if (args.Length <= 1)
        {
            List<string> candidates = ModelDb
                .DebugAfflictions.Select((AfflictionModel affliction) => affliction.Id.Entry)
                .ToList();

...
public class KillConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Will kill one target if the index is given, or all if 'all', or the first if no arguments.";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        IReadOnlyList<Creature> readOnlyList = CombatManager
        ...
        else
        {
            if (!int.TryParse(args[0], out var result))
            {
                return new CmdResult(
                    success: false,
                    "Invalid argument '" + args[0] + "'. Use a target index or 'all'."
                );
            }
            if (result < 0 || result >= readOnlyList.Count)
            {
                return new CmdResult(
                    success: false,
                    $"Invalid target index {result}. Valid range: 0-{readOnlyList.Count - 1}"
                );
            }

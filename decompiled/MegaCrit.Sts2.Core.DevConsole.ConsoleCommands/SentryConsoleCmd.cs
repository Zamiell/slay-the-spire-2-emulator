...
public class SentryConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Test Sentry error reporting. 'test' sends a test message and exception, 'message <text>' sends a custom message, 'exception' throws a test exception, 'crash confirm' triggers a native crash (terminates game!), 'status' shows Sentry status.";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        if (args.Length == 0)
        {
            return new CmdResult(
                success: false,
                "Usage: sentry <test|message|exception|crash|status> [text]"
            );
        }
        ...
        return text switch
        {
            ...
            _ => new CmdResult(
                success: false,
                "Unknown subcommand: " + text + ". Use test, message, exception, crash, or status."
            ),
        };
    }
    ...
    private static CmdResult RunTest()
    {
        ...
        if (!SentryService.IsEnabled)
        {
            return new CmdResult(
                success: false,
                "Failed to initialize Sentry for testing. Check logs for details."
            );
        }
    ...
    private static CmdResult ThrowException()
    {
        ...
        try
        {
            throw new InvalidOperationException(
                "Test exception triggered via 'sentry exception' command"
            );
        }
        catch (Exception ex)
        {
    ...
    private static CmdResult TriggerNativeCrash(string[] args)
    {
        if (OS.HasFeature("editor"))
        {
            return new CmdResult(
                success: false,
                "Native crash testing only works in builds, not the editor."
            );
        }

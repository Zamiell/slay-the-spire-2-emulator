...
public class UpgradeCardConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Upgrade the target card based on its hand position (0 is the left most).";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        if (args.Length != 0 && !int.TryParse(args[0], out result))
        {
            return new CmdResult(
                success: false,
                "Arg 1 must be the hand index (int), got '" + args[0] + "'."
            );
        }
        ...
        if (result < 0 || result >= count)
        {
            return new CmdResult(
                success: false,
                $"Invalid hand index {result}. Valid range: 0-{count - 1}."
            );
        }
        ...
        if (cardModel.CurrentUpgradeLevel == cardModel.MaxUpgradeLevel)
        {
            return new CmdResult(
                success: false,
                $"The card at index={result} is already upgraded to max_level={cardModel.MaxUpgradeLevel}!"
            );
        }
        CardCmd.Upgrade(cardModel);
        return new CmdResult(
            success: true,
            $"Upgraded '{cardModel.Title}' at index '{result}' in hand."
        );
    }

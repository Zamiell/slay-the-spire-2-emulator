...
public abstract class AbstractConsoleCmd
{
    ...
    )
    {
        ...
        {
            matchPredicate = (string candidate, string partial) =>
                candidate.StartsWith(partial, StringComparison.OrdinalIgnoreCase);
        }
        List<string> list2 = (
            (!string.IsNullOrWhiteSpace(partialArg))
                ? list.Where((string c) => matchPredicate(c, partialArg)).ToList()
                : list
        );
    ...
    protected static bool TryParseEnum<T>(string input, out T result)
        where T : struct, Enum
    {
    ...
    private string CalculateCommonCompletion(List<string> filtered, string prefix)
    {
        ...
        for (int i = 0; i < num; i++)
        {
            char c = text[i];
            if (
                !filtered.All((string s) => char.ToLowerInvariant(s[i]) == char.ToLowerInvariant(c))
            )
            {
                break;
            }

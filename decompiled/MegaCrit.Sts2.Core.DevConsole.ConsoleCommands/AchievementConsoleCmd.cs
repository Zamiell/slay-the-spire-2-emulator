...
public class AchievementConsoleCmd : AbstractConsoleCmd
{
    ...
    public override string Description =>
        "Unlocks or revokes an achievement. If no achievement is provided, all achievements are unlocked or revoked.";
    ...
    public override CmdResult Process(Player? issuingPlayer, string[] args)
    {
        ...
        if (args.Length < 1 || !list2.Contains(args[0]))
        {
            return new CmdResult(
                success: false,
                "First argument must be one of: " + string.Join(",", list2) + ".\n" + Args
            );
        }
        ...
        if (args.Length >= 2)
        {
            ...
            if (!achievement.HasValue)
            {
                return new CmdResult(
                    success: false,
                    "Achievement '" + args[1] + "' unrecognized.\n" + Args
                );
            }
        }
        if (args[0] == "unlock")
    ...
    public override CompletionResult GetArgumentCompletions(Player? player, string[] args)
    {
        ...
        if (args.Length == 2)
        {
            List<string> list = (
                ...
                select StringHelper.SnakeCase(a.ToString())
            ).ToList();
            list.Sort();

...
public class NPauseMenu : NSubmenu
{
    ...
    private static readonly LocString _pausedLoc = new LocString(
        "gameplay_ui",
        "PAUSE_MENU.PAUSED"
    );

    private static readonly LocString _resumeLoc = new LocString(
        "gameplay_ui",
        "PAUSE_MENU.RESUME"
    );

    private static readonly LocString _settingsLoc = new LocString(
        "gameplay_ui",
        "PAUSE_MENU.SETTINGS"
    );

    private static readonly LocString _compendiumLoc = new LocString(
        "gameplay_ui",
        "PAUSE_MENU.COMPENDIUM"
    );

    private static readonly LocString _giveUpLoc = new LocString(
        "gameplay_ui",
        "PAUSE_MENU.GIVE_UP"
    );

    private static readonly LocString _disconnectLoc = new LocString(
        "gameplay_ui",
        "PAUSE_MENU.DISCONNECT"
    );

    private static readonly LocString _saveAndQuitLoc = new LocString(
        "gameplay_ui",
    ...
    public override void _Ready()
    {
        ...
        RefreshLabels();
        _resumeButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnBackOrResumeButtonPressed)
        );
        _settingsButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnSettingsButtonPressed)
        );
        _compendiumButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnCompendiumButtonPressed)
        );
        _giveUpButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnGiveUpButtonPressed)
        );
        _disconnectButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnDisconnectButtonPressed)
        );
        _saveAndQuitButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnSaveAndQuitButtonPressed)
        );
        _backButton.Connect(
            NClickableControl.SignalName.Released,
            Callable.From<NButton>(OnBackOrResumeButtonPressed)
        ...
        for (int i = 0; i < Buttons.Length; i++)
        {
            ...
            Buttons[i].FocusNeighborTop = (
                (i > 0) ? Buttons[i - 1].GetPath() : Buttons[^1].GetPath()
            );
            Buttons[i].FocusNeighborBottom = (
                (i < Buttons.Length - 1) ? Buttons[i + 1].GetPath() : Buttons[0].GetPath()
            );
        }
    }
    ...
    private void RefreshLabels()
    {
        ...
        _settingsButton
            ...
            .SetTextAutoSize(_settingsLoc.GetFormattedText());
        _compendiumButton
            .GetNode<MegaLabel>("Label")
            .SetTextAutoSize(_compendiumLoc.GetFormattedText());
        _giveUpButton.GetNode<MegaLabel>("Label").SetTextAutoSize(_giveUpLoc.GetFormattedText());
        _disconnectButton
            .GetNode<MegaLabel>("Label")
            .SetTextAutoSize(_disconnectLoc.GetFormattedText());
        _saveAndQuitButton
            .GetNode<MegaLabel>("Label")
            .SetTextAutoSize(_saveAndQuitLoc.GetFormattedText());
    }
    ...
    public void Initialize(IRunState runState)
    {
        ...
        _compendiumButton.Visible =
            !NGame.IsReleaseGame() || SaveManager.Instance.IsCompendiumAvailable();
    }
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnBackOrResumeButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnSettingsButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnCompendiumButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnGiveUpButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnDisconnectButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnSaveAndQuitButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._disconnectButton,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._saveAndQuitButton,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

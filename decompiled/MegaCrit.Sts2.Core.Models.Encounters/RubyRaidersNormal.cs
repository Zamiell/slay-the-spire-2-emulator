...
{
    private static readonly Dictionary<MonsterModel, int> _raiderValidCounts = new Dictionary<
        MonsterModel,
        int
    >
    {
        { ModelDb.Monster<AxeRubyRaider>(), 1 },
        { ModelDb.Monster<AssassinRubyRaider>(), 1 },
        { ModelDb.Monster<BruteRubyRaider>(), 1 },
        { ModelDb.Monster<CrossbowRubyRaider>(), 1 },
        { ModelDb.Monster<TrackerRubyRaider>(), 1 },
    };
    ...
    protected override IReadOnlyList<(MonsterModel, string?)> GenerateMonsters()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            List<MonsterModel> items = _raiderValidCounts
                .Keys.Where(
                    (MonsterModel r) =>
                        currentRaiders.Count((MonsterModel c) => c == r) < _raiderValidCounts[r]
                )
                .ToList();

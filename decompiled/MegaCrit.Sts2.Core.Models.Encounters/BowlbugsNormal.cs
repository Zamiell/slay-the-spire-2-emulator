...
{
    private static readonly Dictionary<MonsterModel, int> _workerValidCounts = new Dictionary<
        MonsterModel,
        int
    >
    {
        { ModelDb.Monster<BowlbugEgg>(), 1 },
        { ModelDb.Monster<BowlbugSilk>(), 1 },
        { ModelDb.Monster<BowlbugNectar>(), 1 },
    };

    private static readonly string[] _slotNames = new string[3] { "first", "middle", "last" };

    public override IEnumerable<EncounterTag> Tags =>
        new global::_003C_003Ez__ReadOnlySingleElementList<EncounterTag>(EncounterTag.Workers);
    ...
    protected override IReadOnlyList<(MonsterModel, string?)> GenerateMonsters()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            List<MonsterModel> items = _workerValidCounts
                .Keys.Where(
                    (MonsterModel r) =>
                        currentWorkers.Count((MonsterModel c) => c == r) < _workerValidCounts[r]
                )
                .ToList();

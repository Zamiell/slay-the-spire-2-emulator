...
public sealed class PunchOffEventEncounter : EncounterModel
{
    ...
    {
        PunchConstruct punchConstruct = (PunchConstruct)
            ModelDb.Monster<PunchConstruct>().ToMutable();
        punchConstruct.StartsWithFastPunch = true;
        punchConstruct.StartingHpReduction = base.Rng.NextInt(2, 10);
        PunchConstruct punchConstruct2 = (PunchConstruct)
            ModelDb.Monster<PunchConstruct>().ToMutable();
        ...
        return new global::_003C_003Ez__ReadOnlyArray<(MonsterModel, string)>(
            new (MonsterModel, string)[2] { (punchConstruct, null), (punchConstruct2, null) }
        );
    }
}

...
public sealed class MerchantCardEntry : MerchantEntry
{
    ...
    {
        HashSet<CardModel> second =
            _inventory
                ?.CardEntries.Select(
                    (MerchantCardEntry e) => e.CreationResult?.Card.CanonicalInstance
                )
                .OfType<CardModel>()
                .ToHashSet()
        ...
        {
            CreationResult = CardFactory.CreateForMerchant(
                _player,
                _cardPool.Except(second),
                _cardType.Value
            );
        }
        else
    ...
    public override void CalcCost()
    {
        ...
        _cost = Mathf.RoundToInt(
            (float)GetCost(CreationResult.Card) * _player.PlayerRng.Shops.NextFloat(0.95f, 1.05f)
        );
        if (IsOnSale)
        {
            _cost /= 2;
    ...
    )
    {
        ...
        if (CreationResult.Card.Pool is ColorlessCardPool)
        {
            _player
                ...
                .BoughtColorless.Add(CreationResult.Card.Id);
        }

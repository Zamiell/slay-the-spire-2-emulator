...
public sealed class MerchantPotionEntry : MerchantEntry
{
    ...
    {
        Model = PotionFactory
            .CreateRandomPotionOutOfCombat(_player, _player.PlayerRng.Shops, blacklist)
            .ToMutable();
        CalcCost();
        SaveManager.Instance.MarkPotionAsSeen(Model);
    }
    ...
    public override void CalcCost()
    {
        ...
        {
            _cost = (int)
                Mathf.Round((float)_cost * _player.PlayerRng.Shops.NextFloat(0.95f, 1.05f));
        }
    }
    ...
    )
    {
        ...
        if (!potionProcureResult.success)
        {
            InvokePurchaseFailed(
                (potionProcureResult.failureReason == PotionProcureFailureReason.NotAllowed)
                    ? PurchaseStatus.FailureForbidden
                    : PurchaseStatus.FailureSpace
            );
            return (false, 0);
        }
        if (!ignoreCost)
    ...
    {
        HashSet<PotionModel> hashSet =
            inventory
                ?.PotionEntries.Select((MerchantPotionEntry e) => e.Model?.CanonicalInstance)
                .OfType<PotionModel>()
                .ToHashSet()
            ?? new HashSet<PotionModel>();
        FillSlot(Array.Empty<PotionModel>());
    }
}

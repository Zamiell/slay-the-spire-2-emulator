...
public class UsePotionAction : GameAction
{
    ...
    public UsePotionAction(PotionModel potion, Creature? target, bool isCombatInProgress)
    {
        if (potion.Owner == null)
        {
            throw new InvalidOperationException(
                $"Cannot enqueue UsePotionAction for potion {potion} without an owner!"
            );
        }
        ...
        if (!target.CombatId.HasValue)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Trying to target potion {potion} at target {target} that has no combat ID assigned during combat!"
                );
            }
            if (!target.IsPlayer)
            {
                throw new InvalidOperationException(
                    $"Trying to target potion {potion} at target {target} outside of combat that is not a player!"
                );
            }
        }
    ...
    protected override async Task ExecuteAction()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Attempted to execute {"UsePotionAction"} with potion index {PotionIndex}, but the potion at that index was null!"
            );
        }
        ...
        if (CombatManager.Instance.IsInProgress)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Attempted to execute UsePotionAction with single target potion during combat, but the target ID is null!"
                );
            }
        ...
        else
        {
            ...
            {
                throw new InvalidOperationException(
                    "Attempted to execute UsePotionAction outside of combat, but the target player ID is null!"
                );
            }
        ...
        string value = (
            (creature != null)
                ? $"targeting {creature.LogName} (index {Player.Creature.CombatState?.Creatures.IndexOf(creature)})"
                : "no target"
        );
    ...
    protected override void CancelAction()
    {
        ...
        if (
            ...
            && potionAtSlotIndex != null
        )
        {
            NRun.Instance.GlobalUi.TopBar.PotionContainer.OnPotionUseOrDiscardCanceled(
                potionAtSlotIndex
            );
        }

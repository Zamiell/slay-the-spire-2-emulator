...
public class ActionExecutor
{
    ...
    private async Task ExecuteActions()
    {
        ...
        try
        {
            ...
            while (readyAction != null)
            {
                ...
                else
                {
                    ...
                    {
                        Log.Error(
                            $"GameAction {readyAction} completed with exception: {actionTask.Exception}"
                        );
                    }
                }
                ...
                if (flag)
                {
                    bool flag2 = (
                        (
                            readyAction is EndPlayerTurnAction
                            || readyAction is ReadyToBeginEnemyTurnAction
                        )
                            ? true
                            : false
                    );
                ...
                {
                    _logger.Debug(
                        $"Completed execution of action {readyAction}, attempting to find new action"
                    );
                }
                else
                {
                    _logger.Debug(
                        $"Paused execution of action {readyAction} (state is {readyAction.State}), attempting to find new action"
                    );
                }
                readyAction.JustBeforeFinished -= JustBeforeActionFinished;
                readyAction.AfterFinished -= AfterActionFinished;
        ...
        catch (OperationCanceledException innerException)
        {
            InvalidOperationException ex = new InvalidOperationException(
                "ActionExecutor.ExecuteActions should never be canceled!",
                innerException
            );
    ...
    private void JustBeforeActionFinished(GameAction action)
    {
        ...
        {
            Log.Error(
                $"Currently running action {CurrentlyRunningAction} did not match finishing action {action}!"
            );
        }
    ...
    private void AfterActionFinished(GameAction action)
    {
        ...
        {
            Log.Error(
                $"Currently running action {CurrentlyRunningAction} did not match recently finished action {action}!"
            );
        }
        else
        {

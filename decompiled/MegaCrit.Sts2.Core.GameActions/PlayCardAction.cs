...
public sealed class PlayCardAction : GameAction
{
    ...
    public PlayCardAction(CardModel cardModel, Creature? target)
    {
        if (target != null && !target.CombatId.HasValue)
        {
            throw new InvalidOperationException(
                $"Cannot target card against target {target} with no combat ID!"
            );
        }
    ...
    protected override async Task ExecuteAction()
    {
        ...
        {
            Log.Warn(
                $"Attempted to play card {_card} with TargetType of type 'Any', but no target was passed to the play card action!"
            );
        }
        if (
            !_card.CanPlay(out UnplayableReason _, out AbstractModel _)
            || !_card.IsValidTarget(target)
        )
        {
            Cancel();
            return;

...
public class PlayerChoiceResult
{
    ...
    public static PlayerChoiceResult FromMutableDeckCard(CardModel? deckCard)
    {
        ...
        PlayerChoiceResult obj = new PlayerChoiceResult { ChoiceType = PlayerChoiceType.DeckCard };
        List<CardModel> list;
        if (deckCard == null)
        {
            list = new List<CardModel>();
        ...
        foreach (CardModel cardModel in array2)
        {
            ...
            if (cardModel.Owner != array[0].Owner)
            {
                throw new InvalidOperationException(
                    "All cards passed to FromMutableCards must have the same owner!"
                );
            }
        }
        return new PlayerChoiceResult
    ...
    {
        PlayerChoiceResult obj = new PlayerChoiceResult { ChoiceType = PlayerChoiceType.Index };
        List<int> list;
        if (!index.HasValue)
        {
            list = new List<int>();
    ...
    {
        return new PlayerChoiceResult { ChoiceType = PlayerChoiceType.Index, _indexes = indexes };
    }
    ...
    public CardModel? AsCanonicalCard()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get canonical card from player choice result of type {ChoiceType}!"
            );
        }
    ...
    public IEnumerable<CardModel> AsCanonicalCards()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get canonical cards from player choice result of type {ChoiceType}!"
            );
        }
    ...
    public IEnumerable<CardModel> AsCombatCards()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get combat cards from player choice result of type {ChoiceType}!"
            );
        }
        return _combatCards;
    }
    ...
    public IEnumerable<CardModel> AsDeckCards()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get deck cards from player choice result of type {ChoiceType}!"
            );
        }
        return _deckCards;
    }
    ...
    public CardModel? AsMutableCard()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get mutable cards from player choice result of type {ChoiceType}!"
            );
        }
    ...
    public IEnumerable<CardModel> AsCards(PlayerChoiceType type)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get cards of type {type} from player choice result of type {ChoiceType}!"
            );
        }
        return type switch
        {
    ...
    public ulong? AsPlayerId()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get player ID from player choice result of type {ChoiceType}!"
            );
        }
        return _playerId;
    }
    ...
    public int AsIndex()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get index from player choice result of type {ChoiceType}!"
            );
        }
    ...
    public List<int> AsIndexes()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to get indexes from player choice result of type {ChoiceType}!"
            );
        }
        return _indexes;
    }
    ...
    )
    {
        ...
        switch (playerChoiceResult.ChoiceType)
        {
            ...
            case PlayerChoiceType.CombatCard:
                playerChoiceResult._combatCards = netData
                    .combatCards.Select((NetCombatCard c) => c.ToCardModel())
                    .ToList();
                break;
            case PlayerChoiceType.DeckCard:
                playerChoiceResult._deckCards = netData
                    .deckCards.Select((NetDeckCard c) => c.ToCardModel(sender))
                    .ToList();
                break;
            case PlayerChoiceType.MutableCard:
                playerChoiceResult._mutableCards = netData
                    .mutableCards.Select(CardModel.FromSerializable)
                    .ToList();
    ...
    public override string ToString()
    {
        return ChoiceType switch
        {
            PlayerChoiceType.CanonicalCard => "PlayerChoiceResult canonical "
                + string.Join(",", _canonicalCards ?? new List<CardModel>()),
            PlayerChoiceType.CombatCard => "PlayerChoiceResult combat "
                + string.Join(",", _combatCards ?? new List<CardModel>()),
            PlayerChoiceType.DeckCard => "PlayerChoiceResult deck "
                + string.Join(",", _deckCards ?? new List<CardModel>()),
            PlayerChoiceType.MutableCard =>
                $"{"PlayerChoiceResult"} mutable cards {string.Join(",", _mutableCards ?? new List<CardModel>())}, owner: {_mutableCards?.FirstOrDefault()?.Owner.NetId}",
            PlayerChoiceType.Player => $"{"PlayerChoiceResult"} player ID {_playerId}",
            PlayerChoiceType.Index => "PlayerChoiceResult indexes "
                + string.Join(",", _indexes ?? new List<int>()),

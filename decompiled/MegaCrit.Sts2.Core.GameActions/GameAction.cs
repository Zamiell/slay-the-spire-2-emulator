...
public abstract class GameAction
{
    ...
    public void OnEnqueued(Action<GameAction> afterFinished, uint id)
    {
        if (State != GameActionState.None)
        {
            throw new InvalidOperationException(
                $"GameAction {this} was enqueued to the queue twice!"
            );
        }
    ...
    public async Task Execute()
    {
        ...
        switch (State)
        {
            ...
            default:
                throw new InvalidOperationException(
                    $"Attempted to execute GameAction {this} from invalid state {State}! Expected WaitingForExecution or ReadyToResumeExecuting"
                );
        }
        try
        {
    ...
    public void ResumeAfterGatheringPlayerChoice(uint newId)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried setting GameAction {this} ready from invalid state {State}! Expected GatheringPlayerChoice"
            );
        }
        _logger.VeryDebug(
            $"Action {this} finished gathering player choice, and is assigned new id {newId}"
        );
        Id = newId;
    ...
    public void PauseForPlayerChoice()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Tried to pause GameAction {this} from invalid state {State}! Expected Executing"
            );
        }

    ...
    protected override async Task ExecuteAction()
    {
        if (_potionSlotIndex >= _player.PotionSlots.Count)
        {
            throw new IndexOutOfRangeException(
                $"Tried to discard potion at slot index {_potionSlotIndex}, but player {_player.NetId} only has {_player.PotionSlots.Count} potion slots!"
            );
        }
        ...
        if (potionModel == null)
        {
            throw new InvalidOperationException(
                $"Tried to discard potion at slot index {_potionSlotIndex}, but player {_player.NetId} has no potion in that slot!"
            );
        }

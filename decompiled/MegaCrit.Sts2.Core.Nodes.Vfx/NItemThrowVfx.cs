...
public class NItemThrowVfx : Node2D
{
    ...
    )
    {
        ...
        NItemThrowVfx nItemThrowVfx = PreloadManager
            .Cache.GetScene(scenePath)
            .Instantiate<NItemThrowVfx>(PackedScene.GenEditState.Disabled);
    ...
    private async Task ThrowItem()
    {
        ...
        while (timer < (double)_flightTime)
        {
            ...
            _itemSprite.Rotate(
                (float)Mathf.DegToRad((double)(num2 * _rotationMultiplier) * processDeltaTime)
            );
            Vector2 globalPosition =
                _sourcePosition.Lerp(_targetPosition, weight)
                + Vector2.Up * num * _heightMultiplier;
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Float,
                PropertyName._heightMultiplier,
                PropertyHint.None,
                "",
                PropertyUsageFlags.Default | PropertyUsageFlags.ScriptVariable,
                exported: true
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Float,
                PropertyName._rotationMultiplier,
                PropertyHint.None,
                "",
                PropertyUsageFlags.Default | PropertyUsageFlags.ScriptVariable,
                exported: true
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._rotationInfluenceCurve,
                PropertyHint.ResourceType,
                "Curve",
                PropertyUsageFlags.Default | PropertyUsageFlags.ScriptVariable,
                exported: true
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._rotationInfluenceCurve,
            Variant.From(in _rotationInfluenceCurve)
        );

...
public class NFireBurningVfx : Node2D
{
    ...
    )
    {
        ...
        NFireBurningVfx nFireBurningVfx = PreloadManager
            .Cache.GetScene(scenePath)
            .Instantiate<NFireBurningVfx>(PackedScene.GenEditState.Disabled);
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NFireBurningVfx>(
                Create(
        ...
        if (method == MethodName.Create && args.Count == 4)
        {
            ret = VariantUtils.CreateFrom<NFireBurningVfx>(
                Create(
                    ...
                    VariantUtils.ConvertTo<float>(in args[1]),
                    VariantUtils.ConvertTo<bool>(in args[2]),
                    VariantUtils.ConvertTo<Color>(in args[3])
                )
            );
            return true;
        }

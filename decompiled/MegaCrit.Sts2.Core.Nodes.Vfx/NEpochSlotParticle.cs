...
public class NEpochSlotParticle : Sprite2D
{
    ...
    {
        NEpochSlotParticle nEpochSlotParticle = PreloadManager
            .Cache.GetScene(scenePath)
            .Instantiate<NEpochSlotParticle>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        base.GlobalPosition =
            ...
            + new Vector2(
                Rng.Chaotic.NextFloat(40f, 350f) * (Rng.Chaotic.NextBool() ? (-1f) : 1f),
                Rng.Chaotic.NextFloat(40f, 300f) * (Rng.Chaotic.NextBool() ? (-1f) : 1f)
            );
        float num = Rng.Chaotic.NextFloat(0.5f, 1.5f);
        base.Scale = new Vector2(1f, 1f) * num;
        base.Modulate = new Color(
            Rng.Chaotic.NextFloat(0.75f, 0.9f),
            Rng.Chaotic.NextFloat(0.7f, 0.8f),
            Rng.Chaotic.NextFloat(0f, 0.4f),
            0f
        );
    ...
    public override void _Process(double delta)
    {
        ...
        base.GlobalPosition = base.GlobalPosition.Lerp(
            _target.GlobalPosition,
            (float)((double)_speed * delta)
        );
    }
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NEpochSlotParticle>(
                Create(VariantUtils.ConvertTo<Control>(in args[0]))
            );
            return true;
        }

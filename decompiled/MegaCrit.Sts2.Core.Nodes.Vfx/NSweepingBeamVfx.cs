...
public class NSweepingBeamVfx : Node2D
{
    ...
    )
    {
        ...
        NSweepingBeamVfx nSweepingBeamVfx = PreloadManager
            .Cache.GetScene(scenePath)
            .Instantiate<NSweepingBeamVfx>(PackedScene.GenEditState.Disabled);
    ...
    private async Task PlaySequence()
    {
        ...
        while (timer < (double)_sweepDuration)
        {
            ...
            if (num >= 0.5f && !playedImpactParticles)
            {
                ...
                {
                    NSweepingBeamImpactVfx child = NSweepingBeamImpactVfx.Create(
                        _targetCenterPositions[k]
                    );
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NSweepingBeamVfx>(
                Create(
                    VariantUtils.ConvertTo<Vector2>(in args[0]),
                    VariantUtils.ConvertToArray<Vector2>(in args[1])
                )
            );
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Array,
                PropertyName._emittingParticles,
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Array,
                PropertyName._sweepingParticles,
                PropertyHint.TypeString,
                "24/34:GPUParticles2D",
                PropertyUsageFlags.Default | PropertyUsageFlags.ScriptVariable,
                exported: true
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._sweepingIndexCurve,
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Array,
                PropertyName._targetCenterPositions,
                PropertyHint.None,
                "",
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._targetCenterPositions,
            Variant.CreateFrom(_targetCenterPositions)
        );
    }

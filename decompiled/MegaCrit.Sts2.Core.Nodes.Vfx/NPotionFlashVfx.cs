...
public class NPotionFlashVfx : Node2D
{
    ...
    public static NPotionFlashVfx? Create(NPotion originPotion)
    {
        ...
        NPotionFlashVfx nPotionFlashVfx = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NPotionFlashVfx>(PackedScene.GenEditState.Disabled);
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NPotionFlashVfx>(
                Create(VariantUtils.ConvertTo<NPotion>(in args[0]))
            );
            return true;
        }

...
public class NCardTransformVfx : Node2D
{
    ...
    )
    {
        ...
        NCardTransformVfx nCardTransformVfx = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NCardTransformVfx>(PackedScene.GenEditState.Disabled);
    ...
    private async Task PlayAnimation()
    {
        ...
        _tween
            .TweenProperty(textureMat, "shader_parameter/boing:x", -0.75f, 0.15000000596046448)
            .SetEase(Tween.EaseType.Out)
        ...
        _tween = CreateTween().SetParallel();
        _tween
            .TweenProperty(textureMat, "shader_parameter/boing:x", 0.3f, 0.20000000298023224)
        ...
        _tween = CreateTween().SetParallel();
        _tween
            .TweenProperty(textureMat, "shader_parameter/boing:x", -0.2f, 0.25)
            .SetEase(Tween.EaseType.InOut)
            .SetTrans(Tween.TransitionType.Quad);
        if (!(await _tween.AwaitFinished(this)))
        {
        ...
        _tween = CreateTween().SetParallel();
        _tween
            .TweenProperty(textureMat, "shader_parameter/boing:x", 0, 0.30000001192092896)
            .SetEase(Tween.EaseType.InOut)
        ...
        NCardFlyVfx nCardFlyVfx = NCardFlyVfx.Create(
            ...
            _endCard.Owner.Character.TrailPath
        );
        (
            (_endCard.Pile.Type != PileType.Deck)
                ? NCombatRoom.Instance?.CombatVfxContainer
                : NRun.Instance?.GlobalUi.TopBar.TrailContainer
        )?.AddChildSafely(nCardFlyVfx);
    ...
    public static async Task PlayAnimOnCardInHand(NCard cardNode, CardModel endCard)
    {
        if (!TestMode.IsOn)
        {
            ...
            tween
                .TweenProperty(cardNode, "scale", Vector2.One * 1.5f, 0.25)
                .From(Vector2.One)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Cubic);
            tween
            ...
            tween
                .Parallel()
                .TweenProperty(cardNode, "scale", Vector2.One, 0.25)
    ...
    private static void UpdateCard(NCard cardNode, CardModel endCard)
    {
        if (endCard.Pile != null)
        {
            ...
            if (
                NCombatRoom.Instance?.Ui.Hand.GetCardHolder(endCard)
                is NHandCardHolder nHandCardHolder
            )
            {

...
public class NSpeechBubbleVfx : Control
{
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlySingleElementList<string>(
            "res://scenes/vfx/vfx_speech_bubble.tscn"
        );
    ...
    )
    {
        ...
        NSpeechBubbleVfx nSpeechBubbleVfx = CreateInternal(
            text,
            speaker.Side switch
            {
    ...
    public override void _Ready()
    {
        ...
        if (_side == DialogueSide.Right)
        {
            _container.Position = new Vector2(
                0f - _container.Size.X - _container.Position.X,
                _container.Position.Y
            );
            _bubble.FlipH = true;
            _shadow.FlipH = true;
        }
    ...
    private async Task AnimateSpeechBubble()
    {
        ...
        _tween
            .TweenProperty(_bubble, "scale", new Vector2(0.75f, 0.75f), 0.5)
            .From(new Vector2(0.25f, 0.25f))
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
        _tween
            .TweenProperty(this, "rotation_degrees", 0f, 0.3)
            .From(7f)
            .SetEase(Tween.EaseType.Out)
    ...
    {
        _tween
            .TweenProperty(this, "modulate", StsColors.transparentBlack, 0.4)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Sine);
    ...
    {
        NSpeechBubbleVfx nSpeechBubbleVfx = PreloadManager
            .Cache.GetScene("res://scenes/vfx/vfx_speech_bubble.tscn")
            .Instantiate<NSpeechBubbleVfx>(PackedScene.GenEditState.Disabled);
    ...
    private static Vector2 GetCreatureSpeechPosition(Creature speaker)
    {
        ...
        Vector2 result =
            ...
            + new Vector2(0f, (0f - creatureNode.Hitbox.Size.Y) * 0.5f * 0.75f);
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetSpeechBubbleColor,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NSpeechBubbleVfx>(
                Create(
                    VariantUtils.ConvertTo<string>(in args[0]),
                    VariantUtils.ConvertTo<DialogueSide>(in args[1]),
                    VariantUtils.ConvertTo<Vector2>(in args[2]),
                    VariantUtils.ConvertTo<double>(in args[3]),
                    VariantUtils.ConvertTo<VfxColor>(in args[4])
                )
            );
            return true;
        }
        ...
        if (method == MethodName.CreateInternal && args.Count == 3)
        {
            ret = VariantUtils.CreateFrom<NSpeechBubbleVfx>(
                CreateInternal(
                    VariantUtils.ConvertTo<string>(in args[0]),
                    VariantUtils.ConvertTo<DialogueSide>(in args[1]),
                    VariantUtils.ConvertTo<double>(in args[2])
                )
            );
            return true;
        }

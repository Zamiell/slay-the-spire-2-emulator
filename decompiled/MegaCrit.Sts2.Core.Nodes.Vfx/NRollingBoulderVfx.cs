...
public class NRollingBoulderVfx : Node2D
{
    ...
    public event HitCreatureEventHandler HitCreature
    {
        add
        {
            backing_HitCreature = (HitCreatureEventHandler)
                Delegate.Combine(backing_HitCreature, value);
        }
        remove
        {
            backing_HitCreature = (HitCreatureEventHandler)
                Delegate.Remove(backing_HitCreature, value);
        }
    }
    ...
    )
    {
        ...
        NRollingBoulderVfx nRollingBoulderVfx = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NRollingBoulderVfx>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        foreach (GpuParticles2D item in GetChildren().OfType<GpuParticles2D>())
        {
            if (
                !item.LocalCoords
                && item.ProcessMaterial is ParticleProcessMaterial particleProcessMaterial
            )
            {
    ...
    private async Task PlayAnim(float timeToImpact, float xOffset, float rotationSpeed)
    {
        ...
        while (timer <= 1f)
        {
            ...
            _shadow.Scale =
                initialShadowScale
                * base.Scale
                * Mathf.InverseLerp(initialBoulderPosition.Y, impactPoint.Y, base.GlobalPosition.Y);
            _shadow.GlobalPosition =
                new Vector2(base.GlobalPosition.X, impactPoint.Y)
                + initialShadowOffset * base.Scale;
            foreach (Creature creature2 in _creatures)
            {
                ...
                float num3 =
                    creatureNode2.Visuals.GlobalPosition.X
                    - creatureNode2.Visuals.Bounds.Size.X * 0.5f;
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.CleanUpBeforeEarlyExit,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    protected override void RestoreGodotObjectData(GodotSerializationInfo info)
    {
        ...
        if (
            info.TryGetSignalEventDelegate<HitCreatureEventHandler>(
                SignalName.HitCreature,
                out var value5
            )
        )
        {

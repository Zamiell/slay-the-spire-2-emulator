...
public class NEpochOffscreenVfx : Control
{
    ...
    public static readonly string scenePath = SceneHelper.GetScenePath(
        "timeline_screen/epoch_offscreen_vfx"
    );
    ...
    {
        NEpochOffscreenVfx nEpochOffscreenVfx = PreloadManager
            .Cache.GetScene(scenePath)
            .Instantiate<NEpochOffscreenVfx>(PackedScene.GenEditState.Disabled);
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NEpochOffscreenVfx>(
                Create(VariantUtils.ConvertTo<NEpochSlot>(in args[0]))
            );
            return true;
        }

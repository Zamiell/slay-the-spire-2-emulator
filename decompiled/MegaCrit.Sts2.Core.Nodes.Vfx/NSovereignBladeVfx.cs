...
public class NSovereignBladeVfx : Node2D
{
    ...
    public override void _Ready()
    {
        ...
        _spawnFlamesBack = GetNode<GpuParticles2D>(
            "SpineSword/SwordBone/ScaleContainer/SpawnFlamesBack"
        );
        _slashParticles = GetNode<GpuParticles2D>("SpineSword/SlashParticles");
        _chargeParticles = GetNode<GpuParticles2D>(
            "SpineSword/SwordBone/ScaleContainer/ChargeParticles"
        );
        ...
        NTargetManager.Instance.Connect(
            NTargetManager.SignalName.TargetingBegan,
            Callable.From(OnTargetingBegan)
        );
        NTargetManager.Instance.Connect(
            NTargetManager.SignalName.TargetingEnded,
            Callable.From(OnTargetingEnded)
        );
        _owner = Card.Owner;
    ...
    public static NSovereignBladeVfx? Create(CardModel card)
    {
        ...
        NSovereignBladeVfx nSovereignBladeVfx = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NSovereignBladeVfx>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Process(double delta)
    {
        ...
        Transform2D transform2D = _orbitPath.Curve.SampleBakedWithRotation(
            (float)(OrbitProgress % 1.0) * bakedLength
        );
        Vector2 vector = _orbitPath.GlobalTransform * transform2D.Origin;
        vector.X = Mathf.Lerp(
            vector.X,
            base.GlobalPosition.X + 200f,
            Mathf.Clamp(_bladeSize / 1.25f, 0f, 1f)
        );
        _targetOrbitPosition = vector + Vector2.Up * (_spineNode.Scale.Y - 1f) * 100f;
        if (!_isAttacking)
        {
            _spineNode.GlobalPosition = _spineNode.GlobalPosition.Lerp(
                _targetOrbitPosition,
                (float)delta * 7f
            );
        }
    }
    ...
    public void Forge(float bladeDamage = 0f, bool showFlames = false)
    {
        ...
        _glowTween
            .Chain()
            .TweenProperty(_bladeGlow, "modulate", color2, 0.5)
        ...
        _scaleTween
            .TweenProperty(_spineNode, "scale", vector * 1.2f, 0.05000000074505806)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
        _scaleTween.Chain().TweenCallback(Callable.From(FireSparks));
        _scaleTween
            .Chain()
            .TweenProperty(_spineNode, "scale", vector, 0.30000001192092896)
    ...
    public void Attack(Vector2 targetPos)
    {
        ...
        Vector2 vector = new Vector2(
            _spineNode.GlobalPosition.X - 50f,
            _spineNode.GlobalPosition.Y
        );
        _trail.Visible = true;
        _trailStart = vector;
        _attackTween.TweenProperty(
            _spineNode,
            "rotation",
            _spineNode.GetAngleTo(targetPos),
            0.05000000074505806
        );
        _attackTween
            .Parallel()
            .TweenProperty(_spineNode, "global_position", vector, 0.07999999821186066)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
        _attackTween
            .Chain()
            .TweenProperty(_spineNode, "rotation", _spineNode.GetAngleTo(targetPos), 0.0);
        _attackTween
            .Parallel()
            .TweenProperty(_spineNode, "global_position", targetPos, 0.05000000074505806)
            .SetEase(Tween.EaseType.In)
            .SetTrans(Tween.TransitionType.Expo);
    ...
    private void UpdateHoverTip()
    {
        ...
        if (flag)
        {
            ...
            if (_hoverTip == null)
            {
                ...
                _hoverTip?.SetGlobalPosition(
                    _hitbox.GlobalPosition + Vector2.Right * _hitbox.Size.X
                );
            }
        }
        else if (!flag)
    ...
    private void EndSlash()
    {
        ...
        _trail.AddPoint(
            GetNode<Node2D>("SpineSword/SwordBone/ScaleContainer/SpikeCircle").GlobalPosition
        );
    ...
    public void RemoveSovereignBlade()
    {
        ...
        _scaleTween = CreateTween();
        _scaleTween
            .TweenProperty(_spineNode, "scale", Vector2.Zero, 0.20000000298023224)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.RemoveSovereignBlade,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Vector2,
                PropertyName._targetOrbitPosition,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._isBehindCharacter,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

...
public class NDoomVfx : Node2D
{
    ...
    )
    {
        ...
        NDoomVfx nDoomVfx = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NDoomVfx>(PackedScene.GenEditState.Disabled);
    ...
    )
    {
        if (!_cancelToken.IsCancellationRequested)
        {
            ...
            base.GlobalPosition =
                position
                + new Vector2(size.X * 0.5f, size.Y) * NCombatRoom.Instance.SceneContainer.Scale;
            ...
            if (shouldDie)
            {
                ...
                Vector2 creatureOffset =
                    new Vector2(vector.X / 2f, node.Size.Y) + creatureBody.Position;
    ...
    private async Task PlayVfxInternal()
    {
        ...
        try
        {
            ...
            _tween = CreateTween();
            _tween
                .TweenProperty(node2, "position:y", node2.Position.Y + (float)node.Size.Y, 0.75)
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.ShowOrHideParticles,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NDoomVfx>(
                Create(
                    VariantUtils.ConvertTo<NCreatureVisuals>(in args[0]),
                    VariantUtils.ConvertTo<Vector2>(in args[1]),
                    VariantUtils.ConvertTo<Vector2>(in args[2]),
                    VariantUtils.ConvertTo<bool>(in args[3])
                )
            );
            return true;
        }

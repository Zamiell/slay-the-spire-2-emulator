...
public class NBgGroundSpikeVfx : Sprite2D
{
    ...
    public static NBgGroundSpikeVfx? Create(
        Vector2 position,
        bool movingRight = true,
        VfxColor vfxColor = VfxColor.Red
    )
    {
        if (TestMode.IsOn)
        {
        ...
        NBgGroundSpikeVfx nBgGroundSpikeVfx = PreloadManager
            .Cache.GetScene("res://scenes/vfx/bg_ground_spike_vfx.tscn")
            .Instantiate<NBgGroundSpikeVfx>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        base.Skew = (
            _movingRight
                ? (Rng.Chaotic.NextFloat(15f, 30f) * 0.0174533f)
                : (Rng.Chaotic.NextFloat(-30f, -15f) * 0.0174533f)
        );
        float num = Rng.Chaotic.NextFloat(0.5f, 1.5f);
        base.Scale =
            new Vector2(Rng.Chaotic.NextFloat(0.8f, 1.2f), Rng.Chaotic.NextFloat(0.8f, 2f)) * num;
        AdjustStartPosition();
        base.GlobalPosition = _startPosition;
        _velocity =
            new Vector2(
                _movingRight
                    ? Rng.Chaotic.NextFloat(50f, 250f)
                    : Rng.Chaotic.NextFloat(-250f, -50f),
                Rng.Chaotic.NextFloat(-5f, 5f)
            ) / num;
        SetColor();
    ...
    private void SetColor()
    {
        ...
        {
            case VfxColor.Red:
                base.Modulate = new Color(
                    1f,
                    Rng.Chaotic.NextFloat(0.2f, 0.8f),
                    Rng.Chaotic.NextFloat(0f, 0.2f),
                    0.5f
                );
                break;
    ...
    protected virtual void AdjustStartPosition()
    {
        _startPosition += new Vector2(
            _movingRight ? Rng.Chaotic.NextFloat(40f, 160f) : Rng.Chaotic.NextFloat(-160f, -40f),
            Rng.Chaotic.NextFloat(-96f, -10f)
        );
    }
    ...
    private async Task Animate()
    {
        ...
        _tween
            .TweenProperty(
                ...
                "skew",
                _movingRight
                    ? (Rng.Chaotic.NextFloat(30f, 60f) * 0.0174533f)
                    : (Rng.Chaotic.NextFloat(-60f, -30f) * 0.0174533f),
                num
            )
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
        _tween
            .TweenProperty(this, "scale", base.Scale * Rng.Chaotic.NextFloat(0.1f, 0.5f), num)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
        _tween
            .TweenProperty(this, "modulate:a", 0f, num)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.AdjustStartPosition,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NBgGroundSpikeVfx>(
                Create(
                    VariantUtils.ConvertTo<Vector2>(in args[0]),
                    VariantUtils.ConvertTo<bool>(in args[1]),
                    VariantUtils.ConvertTo<VfxColor>(in args[2])
                )
            );
            return true;
        }

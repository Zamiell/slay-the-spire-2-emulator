...
public class NCardSmithVfx : Node2D
{
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlyArray<string>(
            new string[2] { ScenePath, TmpSfx.GetPath("card_smith.mp3") }
        );
    ...
    public static NCardSmithVfx? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NCardSmithVfx>(PackedScene.GenEditState.Disabled);
    }
    ...
    private async Task PlayAnimation(IEnumerable<CardModel> cards)
    {
        ...
        foreach (NCard item in cardNodes)
        {
            _tween
                .Parallel()
                .TweenProperty(item, "scale", Vector2.One * 1f, 0.25)
                .From(Vector2.Zero)
        ...
        foreach (NCard item2 in cardNodes)
        {
            _tween
                .Parallel()
                .TweenProperty(item2, "rotation_degrees", 20, 0.15000000596046448)
        ...
        foreach (NCard item3 in cardNodes)
        {
            _tween
                .Parallel()
                .TweenProperty(item3, "rotation_degrees", -10, 0.15000000596046448)
                .SetTrans(Tween.TransitionType.Elastic)
                .SetEase(Tween.EaseType.Out);
        }
        _tween
        ...
        foreach (NCard item4 in cardNodes)
        {
            _tween
                .Parallel()
                .TweenProperty(item4, "rotation_degrees", 5, 0.15000000596046448)
                .SetTrans(Tween.TransitionType.Elastic)
                .SetEase(Tween.EaseType.Out);
        }
        _tween
        ...
        if (cardNodes[0].IsInsideTree() && _cards[0].Pile == null)
        {
            ...
            foreach (NCard item5 in cardNodes)
            {
                _tween
                    .SetParallel()
                    .TweenProperty(item5, "scale", Vector2.Zero, 0.20000000298023224);
            }
            await _tween.AwaitFinished(this);
        }
        else
        {
            ...
            for (int i = 0; i < cardNodes.Count; i++)
            {
                ...
                NCardFlyVfx nCardFlyVfx = NCardFlyVfx.Create(
                    cardNodes[i],
                    cardNodes[i].Model.Pile.Type,
                    isAddingToPile: false,
                    cardNodes[i].Model.Owner.Character.TrailPath
                );
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NCardSmithVfx>(
                Create(

...
public static class PlayerFullscreenHealVfx
{
    public static void Play(Player player, decimal healAmount, Control? vfxContainer)
    {
        if (!TestMode.IsOn && !(healAmount < 1m) && vfxContainer != null)
        {
            ...
            NSmokyVignetteVfx nSmokyVignetteVfx = NSmokyVignetteVfx.Create(
                green,
                new Color(0f, 1f, 0f, 0.33f)
            );
            ...
            NVfxParticleSystem nVfxParticleSystem = PreloadManager
                .Cache.GetScene(scenePath)
                .Instantiate<NVfxParticleSystem>(PackedScene.GenEditState.Disabled);
            ...
            ParticleProcessMaterial particleProcessMaterial = (ParticleProcessMaterial)
                node.ProcessMaterial;
            particleProcessMaterial.EmissionBoxExtents = new Vector3(
                viewportRect.Size.X / 2f,
                viewportRect.Size.Y / 2f,
                1f
            );

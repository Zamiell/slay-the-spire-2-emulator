...
public class NLiquidOverlayVfx : Node2D
{
    ...
    public static NLiquidOverlayVfx? Create(Creature? targetCreature, Color tint)
    {
        ...
        NLiquidOverlayVfx nLiquidOverlayVfx = PreloadManager
            .Cache.GetScene(scenePath)
            .Instantiate<NLiquidOverlayVfx>(PackedScene.GenEditState.Disabled);
    ...
    private async Task PlayVfx()
    {
        ...
        {
            (NCombatRoom.Instance?.GetCreatureNode(_targetCreature))?.Visuals.TryApplyLiquidOverlay(
                _tint
            );
        }

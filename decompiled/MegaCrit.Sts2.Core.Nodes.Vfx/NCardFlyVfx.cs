...
public class NCardFlyVfx : Node2D
{
    ...
    )
    {
        ...
        NCardFlyVfx nCardFlyVfx = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NCardFlyVfx>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        _arcDir = (
            (_endPos.Y < GetViewportRect().Size.Y * 0.5f) ? (-500f) : (500f + _controlPointOffset)
        );
    ...
    private async Task PlayAnim()
    {
        ...
        while (time / _duration <= 1f)
        {
            ...
            c.Y -= _arcDir;
            Vector2 vector = MathHelper.BezierCurve(
                _startPos,
                _endPos,
                c,
                (time + 0.05f) / _duration
            );
            ...
            _card.Body.Modulate = Colors.White.Lerp(
                Colors.Black,
                Mathf.Clamp(time * 3f / _duration, 0f, 1f)
            );
            _card.Body.Scale =
                Vector2.One * Mathf.Lerp(1f, 0.1f, Mathf.Clamp(time * 3f / _duration, 0f, 1f));
        }
        ...
        while (time / _duration <= 1f)
        {
            ...
            _card.Body.Scale =
                Vector2.One * Mathf.Max(Mathf.Lerp(0.1f, -0.15f, time / _duration), 0f);
        }
        SwooshAwayCompletion?.SetResult();
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NCardFlyVfx>(
                Create(
                    VariantUtils.ConvertTo<NCard>(in args[0]),
                    VariantUtils.ConvertTo<PileType>(in args[1]),
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Float,
                PropertyName._controlPointOffset,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

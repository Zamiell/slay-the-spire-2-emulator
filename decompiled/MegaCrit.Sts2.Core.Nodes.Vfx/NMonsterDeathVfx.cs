...
public class NMonsterDeathVfx : Node2D
{
    ...
    public static NMonsterDeathVfx? Create(NCreature creatureNode, CancellationToken cancelToken)
    {
        ...
        NMonsterDeathVfx nMonsterDeathVfx = PreloadManager
            .Cache.GetScene(ScenePath)
            .Instantiate<NMonsterDeathVfx>(PackedScene.GenEditState.Disabled);
        int num = 1;
    ...
    public async Task PlayVfx()
    {
        ...
        for (int i = 0; i < _creatureNodes.Count; i++)
        {
            ...
            Vector2 vector =
                nCreature.Entity.Monster?.ExtraDeathVfxPadding
                ?? MonsterModel.defaultDeathVfxPadding;
            if (visuals != null && visuals.HasSpineAnimation && !visuals.IsUsingPhobiaModeBody)
            {
                ...
                if (skeleton != null)
                {
                    ...
                    Rect2 rect2 = new Rect2(
                        bounds.Position * scale * scale2,
                        bounds.Size * scale * scale2
                    );
                    Vector2 vector2 = new Vector2(
                        Math.Min(rect2.Position.X, rect2.End.X),
                        Math.Min(rect2.Position.Y, rect2.End.Y)
                    );
                    Vector2 vector3 = new Vector2(
                        Math.Max(rect2.Position.X, rect2.End.X),
                        Math.Max(rect2.Position.Y, rect2.End.Y)
                    );
                    Vector2 vector4 = vector3 - vector2;
                    Vector2 vector5 = vector4 * vector;
                    Vector2 vector6 = (vector5 - vector4) * 0.5f;
                    Rect2 rect3 = new Rect2(
                        visuals.GetCurrentBody().GlobalPosition + vector2 - vector6,
                        vector5
                    );
                    rect = (
                        rect.HasValue ? new Rect2?(rect.Value.Merge(rect3)) : new Rect2?(rect3)
                    );
                }
            }
            else
    ...
    private async Task PlayVfxInternal()
    {
        ...
        tween
            .TweenProperty(node2.Material, "shader_parameter/threshold", 0f, num2)

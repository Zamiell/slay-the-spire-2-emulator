...
public class BigGameHunter : ModifierModel
{
        ...
        MapPointTypeCounts mapPointTypeCountsOverride = new MapPointTypeCounts(map)
        {
            NumOfElites = (int)
                Math.Round(
                    (float)
                        map.GetAllMapPoints()
                            .Count((MapPoint p) => p.PointType == MapPointType.Elite) * 2.5f
                ),
            PointTypesThatIgnoreRules = new HashSet<MapPointType> { MapPointType.Elite },
        };
        bool shouldReplaceTreasureWithElites =
            map is StandardActMap standardActMap && standardActMap.ShouldReplaceTreasureWithElites;
        return new StandardActMap(
            mapRng,
            runState.Act,
            runState.Players.Count > 1,
            shouldReplaceTreasureWithElites,
            runState.Act.HasSecondBoss,
            mapPointTypeCountsOverride
        );
    }
    ...
    )
    {
        if (
            ...
            || options.RarityOdds != CardRarityOddsType.EliteEncounter
        )
        {
            return options;
        }
        ...
        List<CardModel> list = (
            from c in options.GetPossibleCards(player)

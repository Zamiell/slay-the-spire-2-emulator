...
public class NMultiplayerTest : Control, IStartRunLobbyListener
{
    ...
    public new class MethodName : Control.MethodName
    {
        ...
        public static new readonly StringName Disconnect = "Disconnect";
    ...
    public override void _Ready()
    {
        ...
        node5.Connect(
            ...
            Callable.From(CloudConsoleCmd.DeleteCloudSaves)
        );
    ...
    private void JoinButtonPressed()
    {
        ...
        ENetClientConnectionInitializer initializer = new ENetClientConnectionInitializer(
            netId,
            ip,
            33771
        );
    ...
    {
        _localPlayerData.Deck = _characterPaginator
            .Character.StartingDeck.Select((CardModel c) => c.ToMutable().ToSerializable())
            .ToList();
        _localPlayerData.Relics = _characterPaginator
            .Character.StartingRelics.Select((RelicModel r) => r.ToMutable().ToSerializable())
            .ToList();
    ...
    private async Task BeginRunAsync(string seed, List<ActModel> acts)
    {
        ...
        {
            await _game.StartNewMultiplayerRun(
                _lobby,
                shouldSave: true,
                acts,
    ...
    private async Task<bool> StartHost(bool steam)
    {
        ...
        NetErrorInfo? value = (
            (!steam) ? netService.StartENetHost(33771, 4) : (await netService.StartSteamHost(4))
        );
        if (!value.HasValue)
        {
    ...
    private void AfterMultiplayerStarted()
    {
        ...
        {
            _characterContainers[player.slotId].characterImage.Texture = player
                .character
                .IconTexture;
            _characterContainers[player.slotId].playerName.Text = PlatformUtil.GetPlayerNameRaw(
                _lobby.NetService.Platform,
                player.id
            );
        }
        _readyButton.Visible = true;
        _characterPaginator.Visible = true;
        _game.RemoteCursorContainer.Initialize(
    ...
    private void LoadReplay(string path)
    {
        ...
        using (
            FileAccessStream fileAccessStream = new FileAccessStream(
                path,
        ...
        Log.Info(
            $"Loaded replay. Game version: {combatReplay.version} Commit: {combatReplay.gitCommit} Model ID hash: {combatReplay.modelIdHash}"
        );
        if (combatReplay.modelIdHash != ModelIdSerializationCache.Hash)
        {
            ...
            {
                Log.Error(
                    $"Attempting to load replay with Model ID hash {combatReplay.modelIdHash} that does not match ours ({ModelIdSerializationCache.Hash})! The replay will mismatch. If you want to continue anyway, try running the replay again."
                );
    ...
    private static async Task RunReplay(CombatReplay replay, SceneTree sceneTree)
    {
        ...
        {
            Log.Warn(
                $"Git commit in replay {replay.gitCommit} does not match ours ({text}). The replay has a chance of mismatching!"
            );
        }
        ...
        RunManager.Instance.ChecksumTracker.LoadReplayChecksums(
            replay.checksumData,
            replay.nextChecksumId
        );
        ...
        foreach (CombatReplayEvent replayEvent in replay.events)
        {
            switch (replayEvent.eventType)
            {
                case CombatReplayEventType.GameAction:
                {
                    ...
                    {
                        while (
                            CombatManager.Instance.DebugOnlyGetState().CurrentSide
                            == CombatSide.Enemy
                        )
                        {
                            await sceneTree.Root.AwaitProcessFrame();
                    ...
                    if (
                        (action is EndPlayerTurnAction || action is ReadyToBeginEnemyTurnAction)
                            ? true
                            : false
                    )
                    {
                ...
                case CombatReplayEventType.ResumeAction:
                    RunManager.Instance.ActionQueueSet.ResumeActionWithoutSynchronizing(
                        replayEvent.actionId.Value
                    );
                    break;
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.SteamHostButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            _characterPaginator = VariantUtils.ConvertTo<NMultiplayerTestCharacterPaginator>(
                in value
            );
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._characterPaginator,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._ignoreReplayModelIdHash,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._ignoreReplayModelIdHash,
            Variant.From(in _ignoreReplayModelIdHash)
        );

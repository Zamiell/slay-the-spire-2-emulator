...
public class Logger
{
    ...
    static Logger()
    {
        ...
        for (int i = 0; i < commandLineArgs.Length; i++)
        {
            ...
            {
                if (
                    !LogConsoleCmd.TryParseEnumCaseInsensitive(
                        commandLineArgs[i + 1],
                        out LogType? enumVal
                    )
                )
                {
                    _logPrinter.Print(
                        LogLevel.Error,
                        "Invalid log command line argument! Could not parse "
                            + commandLineArgs[i + 1]
                            + " as LogType",
                        1
                    );
    ...
    public bool WillLog(LogLevel level)
    {
        ...
        return level
            >= (
                (
                    logLevelTypeMap.TryGetValue(_logType, out value)
                        ? new LogLevel?(value)
                        : ((LogLevel?)null)
                ) ?? GlobalLogLevel
            );

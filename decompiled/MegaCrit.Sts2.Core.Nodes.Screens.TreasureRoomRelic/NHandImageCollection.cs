...
public class NHandImageCollection : Control
{
    ...
    {
        Vector2 controlSpaceFocusPosition = _synchronizer.GetControlSpaceFocusPosition(
            playerId,
            this
        );
        ...
        )
        {
            if (focusedControl is NTreasureRoomRelicHolder)
            {
                ...
                Vector2 pointingPosition =
                    focusedControl.GlobalPosition + focusedControl.Size * 0.5f + vector * 100f;
    ...
    private void UpdateHandVisibility()
    {
        ...
        {
            NetScreenType netScreenType = (
                (!RunManager.Instance.IsSinglePlayerOrFakeMultiplayer)
                    ? _synchronizer.GetScreenType(hand.Player.NetId)
                    : NetScreenType.SharedRelicPicking
            );
            bool flag =
                netScreenType == NetScreenType.SharedRelicPicking
                && screenType == NetScreenType.SharedRelicPicking;
        ...
        for (int i = 0; i < fight.rounds.Count; i++)
        {
            ...
            for (int j = 0; j < fight.playersInvolved.Count; j++)
            {
                ...
                if (relicPickingFightMove.HasValue)
                {
                    ...
                    tweens.Add(
                        hand.DoFightMove(relicPickingFightMove.Value, 1.5f * durationMultiplier)
                    );
                }
            }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnInputStateRemoved,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateHandVisibility,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.BeforeRelicsAwarded,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NHandImage>(
                GetHand(VariantUtils.ConvertTo<ulong>(in args[0]))
            );
            return true;
        }

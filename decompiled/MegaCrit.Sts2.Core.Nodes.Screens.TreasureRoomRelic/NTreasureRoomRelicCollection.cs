...
public class NTreasureRoomRelicCollection : Control, IScreenContext
{
    ...
    private readonly TaskCompletionSource _relicPickingBeganTaskCompletionSource =
        new TaskCompletionSource();

    private readonly TaskCompletionSource _relicPickingCompleteTaskCompletionSource =
        new TaskCompletionSource();
    ...
    public Control? DefaultFocusedControl
    {
        get
        {
            ...
            return _holdersInUse[
                _runState.GetPlayerSlotIndex(LocalContext.GetMe(_runState.Players))
            ];
        }
    }
    ...
    public override void _Ready()
    {
        ...
        SingleplayerRelicHolder = node.GetNode<NTreasureRoomRelicHolder>(
            "%SingleplayerRelicHolder"
        );
        foreach (
            NTreasureRoomRelicHolder item in node.GetChildren().OfType<NTreasureRoomRelicHolder>()
        )
        {
    ...
    {
        IReadOnlyList<RelicModel> currentRelics = RunManager
            .Instance
        ...
        if (currentRelics.Count == 1)
        {
            ...
            SingleplayerRelicHolder.Connect(
                NClickableControl.SignalName.Released,
                Callable.From<NTreasureRoomRelicHolder>(
                    delegate
                    {
        ...
        for (int num3 = 0; num3 < _holdersInUse.Count; num3++)
        {
            ...
            _holdersInUse[num3].FocusNeighborRight = (
                (num3 < _holdersInUse.Count - 1)
                    ? _holdersInUse[num3 + 1].GetPath()
                    : _holdersInUse[0].GetPath()
            );
        }
    ...
    private void SpawnEmptyChestVfx()
    {
        ...
        _emptyVfxTween
            .TweenProperty(node, "position:y", node.Position.Y, 1.0)
            .From(node.Position.Y + 64f)
    ...
    public void AnimIn(Node chestVisual)
    {
        ...
        foreach (NTreasureRoomRelicHolder holder in _holdersInUse)
        {
            ...
            tween2
                .TweenProperty(holder, "position:y", holder.Position.Y - num, 0.6)
                .SetDelay(num2)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Back);
            tween2
                .TweenCallback(Callable.From(() => holder.MouseFilter = MouseFilterEnum.Stop))
                .SetDelay((double)num2 + 0.6);
        }
        NRun.Instance.ScreenStateTracker.SetIsInSharedRelicPickingScreen(
            isInSharedRelicPicking: true
        );
    ...
    private async Task AnimateRelicAwards(List<RelicPickingResult> results)
    {
        ...
        {
            TreasureRoomRelicSynchronizer.PlayerVote playerVote =
        ...
        {
            NTreasureRoomRelicHolder holder = _holdersInUse.First(
                (NTreasureRoomRelicHolder h) => h.Relic.Model == result.relic
            );
            ...
            if (result.type == RelicPickingResultType.FoughtOver)
            {
                ...
                tween
                    .TweenProperty(
                        holder,
                        "global_position",
                        (_fightBackstop.Size - holder.Size) * 0.5f,
                        0.25
                    )
                    .SetTrans(Tween.TransitionType.Back)
        ...
        {
            NTreasureRoomRelicHolder nTreasureRoomRelicHolder = _holdersInUse.First(
                (NTreasureRoomRelicHolder h) => h.Relic.Model == result2.relic
            );
            ...
            if (result2.type != RelicPickingResultType.Skipped)
            {
                ...
                if (LocalContext.IsMe(result2.player))
                {
                    NRun.Instance.GlobalUi.RelicInventory.AnimateRelic(
                        relic,
                        nTreasureRoomRelicHolder.GlobalPosition,
                        nTreasureRoomRelicHolder.Scale
                    );
                }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetSelectionEnabled,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.SingleplayerRelicHolder,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName.SingleplayerRelicHolder,
            Variant.From<NTreasureRoomRelicHolder>(SingleplayerRelicHolder)
        );

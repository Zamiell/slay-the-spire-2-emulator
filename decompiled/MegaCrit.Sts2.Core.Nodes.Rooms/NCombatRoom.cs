...
public class NCombatRoom : Control, IScreenContext, IRoomWithProceedButton
{
    ...
    public new class MethodName : Control.MethodName
    {
        ...
        public static readonly StringName SetWaitingForOtherPlayersOverlayVisible =
            "SetWaitingForOtherPlayersOverlayVisible";
    ...
    public new class PropertyName : Control.PropertyName
    {
        ...
        public static readonly StringName _waitingForOtherPlayersOverlay =
            "_waitingForOtherPlayersOverlay";
    ...
    private static readonly LocString _waitingLoc = new LocString(
        "gameplay_ui",
        "MULTIPLAYER_WAITING"
    );
    ...
    public IEnumerable<NCreature> RemovingCreatureNodes =>
        _removingCreatureNodes.Where(GodotObject.IsInstanceValid);
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlySingleElementList<string>(
            "res://scenes/rooms/combat_room.tscn"
        );
    ...
    public static NCombatRoom? Create(ICombatRoomVisuals visuals, CombatRoomMode mode)
    {
        ...
        NCombatRoom nCombatRoom = PreloadManager
            .Cache.GetScene("res://scenes/rooms/combat_room.tscn")
            .Instantiate<NCombatRoom>(PackedScene.GenEditState.Disabled);
    ...
    public override void _Ready()
    {
        ...
        _waitingForOtherPlayersOverlay
            .GetNode<MegaLabel>("Label")
            .SetTextAutoSize(_waitingLoc.GetRawText());
        ...
        if (Mode == CombatRoomMode.VisualOnly && NEventRoom.Instance == null)
        {
            ...
            _proceedButton.Connect(
                ...
                Callable.From<NButton>(OnProceedButtonPressed)
            );
        ...
        _window.Connect(
            ...
            Callable.From(AdjustCreatureScaleForAspectRatio)
        );
        AdjustCreatureScaleForAspectRatio();
    ...
    public void SetUpBackground(IRunState state)
    {
        ...
        Background = _visuals.Encounter.CreateBackground(
            _visuals.Act,
            GenerateBackgroundRngForCurrentPoint(state)
        );
    ...
    private void AdjustCreatureScaleForAspectRatio()
    {
        ...
        {
            num = Math.Max(
                creatureNode.GlobalPosition.X
                    + creatureNode.Visuals.Bounds.Size.X * 0.5f * SceneContainer.Scale.X,
                num
            );
        }
        num += 15f;
    ...
    private void CreateAllyNodes()
    {
        ...
        PositionPlayersAndPets(
            _creatureNodes.Where((NCreature c) => allies.Contains(c.Entity)).ToList(),
            _visuals.Encounter.GetCameraScaling(),
            _visuals.Encounter.FullyCenterPlayers
        );
    }
    ...
    private void CreateEnemyNodes()
    {
        ...
        List<NCreature> creatures = _creatureNodes
            .Where((NCreature c) => enemies.Contains(c.Entity))
            .ToList();
        if (EncounterSlots != null)
        {
    ...
    private void PositionEnemies(List<NCreature> creatures, float scaling)
    {
        ...
        if (val + num4 > num)
        {
            ...
            {
                Log.Warn(
                    "Creatures for current encounter ("
                        + _visuals.Encounter.Title.GetFormattedText()
                        + ") are being displayed off-screen because they are too wide!"
                );
            }
        }
        for (int num6 = 0; num6 < creatures.Count; num6++)
        {
            ...
            nCreature.Position = new Vector2(
                val + nCreature.Visuals.Bounds.Size.X * 0.5f,
                200f - ((num6 % 2 != 0) ? num5 : 0f)
            );
    ...
    )
    {
        ...
        foreach (NCreature creature in creatureNodes)
        {
            ...
            {
                list.First((PlayerAndPets p) => p.player.Entity.Player == creature.Entity.PetOwner)
                    .pets.Add(creature);
            }
        }
        float num = 960f / scaling;
        ...
        {
            num10 =
                creatureNodes.First((NCreature c) => c.Entity.IsPlayer).Visuals.Bounds.Size.X
                * -0.5f;
        }
        else
        {
        ...
        for (int num11 = 0; num11 < num3; num11++)
        {
            ...
            for (int num12 = 0; num12 < num3; num12++)
            {
                ...
                player.Position = new Vector2(
                    ...
                    200f - num9 * (float)num11
                );
                if (
                    LocalContext.IsMe(player.Entity)
                    && player.Entity.Player.Character is Necrobinder
                )
                {
                    NCreature osty = null;
                ...
                float num15 = (
                    (pets.Count > 1) ? (player.Visuals.Bounds.Size.X / (float)(pets.Count - 1)) : 0f
                );
                for (int num16 = 0; num16 < pets.Count; num16++)
                {
                    ...
                    nCreature2.Position = new Vector2(
                        0f
                            ...
                            + 20f
                            - (float)num16 * num15
                            - nCreature2.Visuals.Bounds.Size.X * 0.5f,
    ...
    )
    {
        ...
        {
            osty.Position =
                new Vector2(0f - targetXPos, playerYPosition)
    ...
    public void AddCreature(Creature creature)
    {
        ...
        if (creature.SlotName != null)
        {
            if (EncounterSlots == null)
            {
                throw new InvalidOperationException(
                    $"Creature {creature} has slot name '{creature.SlotName}' but NCombatRoom.EncounterSlots is null."
                );
            }
            nCreature.GlobalPosition = EncounterSlots
                .GetNode<Marker2D>(creature.SlotName)
                .GlobalPosition;
        }
        UpdateCreatureNavigation();
        ...
        List<NCreature> list = _creatureNodes
            .Where(
                (NCreature c) =>
                    c.Entity.PetOwner == player
                    && (!(c.Entity.Monster is Osty) || !LocalContext.IsMe(player))
            )
            .ToList();
        ...
        float num = (
            (list.Count > 1) ? (creatureNode.Visuals.Bounds.Size.X / (float)(list.Count - 1)) : 0f
        );
        for (int num2 = 0; num2 < list.Count; num2++)
        {
            NCreature nCreature2 = list[num2];
            nCreature2.Position = new Vector2(
                creatureNode.Position.X
                    - 20f
                    + (float)num2 * num
                    + nCreature2.Visuals.Bounds.Size.X * 0.5f,
                creatureNode.Position.Y + 10f
            );
    ...
    private void UpdateCreatureNavigation()
    {
        ...
        for (int num = 0; num < list.Count; num++)
        {
            ...
            list[num].Hitbox.FocusNeighborRight = (
                (num < list.Count - 1) ? list[num + 1].Hitbox.GetPath() : list[0].Hitbox.GetPath()
            );
        ...
        Ui.Hand.CardHolderContainer.FocusNeighborTop = Instance
            .CreatureNodes.FirstOrDefault()
            ?.Hitbox.GetPath();
    }
    ...
    private void RandomizeEnemyScalesAndHues()
    {
        ...
        {
            if (
                creatureNode.Entity.Side != CombatSide.Player
        ...
        foreach (KeyValuePair<Type, List<NCreature>> item in dictionary)
        {
            ...
            foreach (NCreature item2 in item.Value)
            {
                ...
                float value3 = (
                    (monster.MaxInitialHp != monster.MinInitialHp)
                        ? (
                            (
                                (float)(value2 - monster.MinInitialHp)
                                    / (float)(monster.MaxInitialHp - monster.MinInitialHp)
                                - 0.5f
                            ) * 2f
                        )
                        : 0f
    ...
    {
        _creatureNodes
            .FirstOrDefault((NCreature c) => c.Entity.Monster is Osty && c.Entity.PetOwner == owner)
            ?.AnimShake();
    }
    ...
    public void PlaySplashVfx(Creature target, Color tint)
    {
        ...
        if (creatureNode != null)
        {
            ...
            combatVfxContainer.AddChildSafely(
                NSplashVfx.Create(creatureNode.GetBottomOfHitbox(), tint)
            );
    ...
    public void TransitionToActiveCombat(CombatRoom combatRoom)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Cannot transition to {"ActiveCombat"} from {Mode}."
            );
        }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.SubscribeToCombatEvents,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.AdjustCreatureScaleForAspectRatio,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.UpdateCreatureNavigation,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnActiveScreenUpdated,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.RandomizeEnemyScalesAndHues,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetWaitingForOtherPlayersOverlayVisible,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnProceedButtonPressed,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._waitingForOtherPlayersOverlay,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.BackCombatVfxContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.CombatVfxContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName.BackCombatVfxContainer,
            Variant.From<Control>(BackCombatVfxContainer)
        );
        info.AddProperty(
            PropertyName.CombatVfxContainer,
            Variant.From<Control>(CombatVfxContainer)
        );
        ...
        info.AddProperty(
            PropertyName._waitingForOtherPlayersOverlay,
            Variant.From(in _waitingForOtherPlayersOverlay)
        );

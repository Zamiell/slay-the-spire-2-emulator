...
public class NEventRoom : Control, IScreenContext
{
    ...
    public static IEnumerable<string> AssetPaths =>
        new global::_003C_003Ez__ReadOnlySingleElementList<string>(
            "res://scenes/rooms/event_room.tscn"
        );
    ...
    public static NEventRoom? Create(EventModel eventModel, IRunState? runState, bool isPreFinished)
    {
        ...
        NEventRoom nEventRoom = PreloadManager
            .Cache.GetScene("res://scenes/rooms/event_room.tscn")
            .Instantiate<NEventRoom>(PackedScene.GenEditState.Disabled);
    ...
    private async Task SetupLayout()
    {
        ...
        {
            throw new InvalidOperationException(
                "Event must be started before passed to NEventRoom!"
            );
        }
        if (Layout == null)
        {
        ...
        if (_event is AncientEventModel ancientEventModel && !_isPreFinished)
        {
            ...
            AncientStats statsForAncient = SaveManager.Instance.Progress.GetStatsForAncient(
                ancientEventModel.Id
            );
            int charVisits = statsForAncient?.GetVisitsAs(id) ?? 0;
            int totalVisits = statsForAncient?.TotalVisits ?? 0;
            IEnumerable<AncientDialogue> validDialogues =
                ancientEventModel.DialogueSet.GetValidDialogues(
                    id,
                    charVisits,
                    totalVisits,
                    !ancientEventModel.AnyCharacterDialogueBlacklist.Contains(
                        _event.Owner.Character
                    )
                );
    ...
    private void SetOptions(EventModel eventModel)
    {
        ...
        {
            readOnlyList = new global::_003C_003Ez__ReadOnlySingleElementList<EventOption>(
                new EventOption(eventModel, Proceed, "PROCEED", false, true)
            );
        }
    ...
    internal static List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.DisableOptionButtons,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnEnteringEventCombat,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.EmbeddedCombatRoom,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

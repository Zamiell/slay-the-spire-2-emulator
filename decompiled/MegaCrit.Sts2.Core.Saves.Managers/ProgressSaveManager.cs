...
public class ProgressSaveManager
{
    ...
    public void SaveProgress()
    {
        try
        {
            ...
            serializableProgress.SchemaVersion =
                _migrationManager.GetLatestVersion<SerializableProgress>();
            string content = JsonSerializationUtility.ToJson(serializableProgress);
            _saveStore.WriteFile(
                GetProgressPathForProfile(_profileIdProvider.CurrentProfileId),
                content
            );
        }
        catch (Exception ex)
    ...
    {
        ReadSaveResult<SerializableProgress> readSaveResult =
            _migrationManager.LoadSave<SerializableProgress>(
                GetProgressPathForProfile(_profileIdProvider.CurrentProfileId)
            );
        ...
        else
        {
            ...
            Progress = ProgressState.FromSerializable(
                readSaveResult.SaveData,
    ...
    private void IncrementEncounterLoss(ModelId characterId, ModelId encounterId)
    {
        ...
        FightStats fightStats = orCreateEncounterStats.FightStats.FirstOrDefault(
            (FightStats f) => f.Character == characterId
        );
        if (fightStats == null)
        {
    ...
    private void IncrementEnemyFightLoss(ModelId characterId, ModelId monster)
    {
        ...
        FightStats fightStats = orCreateEnemyStats.FightStats.FirstOrDefault(
            (FightStats f) => f.Character == characterId
        );
        if (fightStats == null)
        {
    ...
    public void UpdateWithRunData(SerializableRun serializableRun, bool victory)
    {
        ...
        else
        {
            ...
            serializablePlayer = serializableRun.Players.FirstOrDefault(
                (SerializablePlayer p) => p.NetId == playerId
            );
            if (serializablePlayer == null)
            {
                Log.Warn(
                    $"Local player with net id {playerId} not found in run! Progress will not be updated"
                );
                return;
            }
        }
        for (int num = 0; num < serializableRun.MapPointHistory.Count; num++)
        {
            ...
            {
                Log.Warn(
                    $"There are {serializableRun.MapPointHistory.Count} acts in the map point history, but {serializableRun.Acts.Count} acts in the act array! This is unexpected"
                );
            }
            else
            {
        ...
        List<MapPointHistoryEntry> list = serializableRun
            .MapPointHistory.SelectMany((List<MapPointHistoryEntry> act) => act)
            .ToList();
        Progress.TotalPlaytime += (
            (serializableRun.WinTime > 0) ? serializableRun.WinTime : serializableRun.RunTime
        );
        ...
        CharacterStats orCreateCharacterStats = Progress.GetOrCreateCharacterStats(
            serializablePlayer.CharacterId
        );
        orCreateCharacterStats.Playtime += (
            (serializableRun.WinTime > 0) ? serializableRun.WinTime : serializableRun.RunTime
        );
        if (victory)
        {
            ...
            if (!flag2 && !flag3)
            {
                ...
                orCreateCharacterStats.BestWinStreak = Math.Max(
                    orCreateCharacterStats.BestWinStreak,
                    orCreateCharacterStats.CurrentWinStreak
                );
                if (
                    orCreateCharacterStats.FastestWinTime < 0
        ...
        else
        {
            ...
            MapPointHistoryEntry mapPointHistoryEntry = serializableRun
                .MapPointHistory.LastOrDefault()
                ?.LastOrDefault();
        ...
        {
            MapPointRoomHistoryEntry mapPointRoomHistoryEntry = item.FirstRoomOfType(
                RoomType.Event
            );
        ...
        if (!flag2 && !flag3)
        {
            HashSet<ModelId> hashSet = serializablePlayer
                .Deck.Select((SerializableCard c) => c.Id)
                .ToHashSet();
            ...
            foreach (MapPointHistoryEntry item3 in list)
            {
                ...
                {
                    CardStats orCreateCardStats2 = Progress.GetOrCreateCardStats(
                        cardChoice.Card.Id
                    );
                ...
                {
                    MapPointRoomHistoryEntry mapPointRoomHistoryEntry2 = item3.FirstRoomOfType(
                        RoomType.Event
                    );
                    AncientStats orCreateAncientStats = Progress.GetOrCreateAncientStats(
                        mapPointRoomHistoryEntry2.ModelId
                    );
                    if (victory)
                    {
    ...
    )
    {
        ...
        if (
            ModelDb.AllAncients.All(
                (AncientEventModel a) => Progress.AncientStats.ContainsKey(a.Id) || a is Darv
            )
        )
        {
    ...
    )
    {
        ...
        string text = (
            (byId is Silent)
                ? EpochModel.GetId<Regent1Epoch>()
                : (
                    (byId is Regent)
                        ? EpochModel.GetId<Necrobinder1Epoch>()
                        : ((!(byId is Necrobinder)) ? null : EpochModel.GetId<Defect1Epoch>())
                )
        );
        string text2 = text;
        if (
    ...
    private void CheckFifteenBossesDefeatedEpoch(Player localPlayer)
    {
        ...
        HashSet<ModelId> hashSet = ModelDb
            .Acts.SelectMany((ActModel a) => a.AllBossEncounters.Select((EncounterModel e) => e.Id))
            .ToHashSet();
        int num = 0;
        foreach (EncounterStats value in Progress.EncounterStats.Values)
        {
    ...
    private bool TryObtainEpochInternal(EpochModel epoch)
    {
        ...
        {
            string text =
                "Epoch " + epoch.Id + " was obtained, but is not yet revealable by the player!";
            Log.Warn(text);
            SentryService.CaptureMessage(
                ...
                {
                    string text2 = JsonSerializer.Serialize(
                        Progress.Epochs,
                        JsonSerializationUtility.GetTypeInfo<List<SerializableEpoch>>()
                    );
    ...
    {
        HashSet<string> satisfiedEpochIds = new HashSet<string>(
            from e in Progress.Epochs.Where(
                delegate(SerializableEpoch e)
                {
        ...
        return Progress.Epochs.Where(
            (SerializableEpoch e) => satisfiedEpochIds.Contains(e.Id) && reachableSet.Contains(e.Id)
        );
    }
    ...
    public void UpdateAfterCombatWon(Player localPlayer, CombatRoom room)
    {
        ...
        FightStats fightStats = orCreateEncounterStats.FightStats.FirstOrDefault(
            (FightStats f) => f.Character == character.Id
        );
        if (fightStats == null)
        {
        ...
        foreach (MonsterModel spawnedEnemy in room.Encounter.SpawnedEnemies)
        {
            ...
            FightStats fightStats2 = orCreateEnemyStats.FightStats.FirstOrDefault(
                (FightStats f) => f.Character == character.Id
            );
            if (fightStats2 == null)
            {
    ...
    private static void IncrementSingleplayerAscension(
        SerializableRun run,
    ...
    private void IncrementMultiplayerAscension(SerializableRun run)
    {
        ...
        {
            Log.Info(
                $"Not playing on max multiplayer ascension ({Progress.MaxMultiplayerAscension})"
            );
        }
    }
    ...
    {
        return _eliteEncounters
            ?? (
                _eliteEncounters = (
                    from e in ModelDb.AllEncounters

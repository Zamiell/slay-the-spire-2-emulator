...
public class RunSaveManager
{
    ...
    private string CurrentRunSavePath =>
        GetRunSavePath(_profileIdProvider.CurrentProfileId, "current_run.save");

    private string CurrentMultiplayerRunSavePath =>
        GetRunSavePath(_profileIdProvider.CurrentProfileId, "current_run_mp.save");
    ...
    public RunSaveManager(
        ...
        : this(
            saveStore,
            migrationManager,
            new StaticProfileIdProvider(profileId),
            forceSynchronous
        ) { }
    ...
    public ReadSaveResult<SerializableRun> LoadRunSave()
    {
        ReadSaveResult<SerializableRun> readSaveResult =
            _migrationManager.LoadSave<SerializableRun>(CurrentRunSavePath);
        ...
        {
            Log.Error(
                $"Failed to load run save: status={readSaveResult.Status} msg={readSaveResult.ErrorMessage}"
            );
        }
        else
        {
            Log.Warn(
                $"Run save had recoverable issues: status={readSaveResult.Status} msg={readSaveResult.ErrorMessage}"
            );
        }
        return readSaveResult;
    }
    ...
    {
        ReadSaveResult<SerializableRun> readSaveResult =
            _migrationManager.LoadSave<SerializableRun>(CurrentMultiplayerRunSavePath);
        if (readSaveResult.Success)
        {
            return readSaveResult;
        }
    ...
    )
    {
        ...
        if (readSaveResult != null && readSaveResult.Success && readSaveResult.SaveData != null)
        {
            try
            {
                SerializableRun data = RunManager.CanonicalizeSave(
    ...
    public void RenameBrokenMultiplayerRunSave(ReadSaveStatus status)
    {
        try
        {
            if (HasMultiplayerRunSave)
            {
                if (_saveStore.FileExists(CurrentMultiplayerRunSavePath))
                {
                    string text = CorruptFileHandler.GenerateCorruptFilePath(
                ...
                if (_saveStore.FileExists(text2))
                {
                    ...
                    Log.Error(
                        $"Corrupt multiplayer run backup detected: Renamed '{text2}' to '{text3}'"
                    );
                }
            }
        }

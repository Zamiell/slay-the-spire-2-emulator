...
public class CrystalSphereMinigame
{
    ...
    public CrystalSphereMinigame(Player owner, Rng rng, int divinationCount)
    {
        ...
        for (int k = 0; k < 2; k++)
        {
            List<Vector2I> list3 = list2
                .Concat(list2.SelectMany((Vector2I c) => GetHorizontalCells(c.X, c.Y)))
                .Concat(list2.SelectMany((Vector2I c) => GetVerticalCells(c.X, c.Y)))
                .ToList();
            list2 = list3;
        }
    ...
    private bool PopulateItems()
    {
        ...
        CrystalSphereItem crystalSphereItem4 = new CrystalSphereCardReward(
            CardRarity.Common,
            _owner
        );
        flag = flag && crystalSphereItem4.PlaceItem(this);
        _items.Add(crystalSphereItem4);
        CrystalSphereItem crystalSphereItem5 = new CrystalSphereCardReward(
            CardRarity.Uncommon,
            _owner
        );
    ...
    public void SetHoveredCell(CrystalSphereCell cell)
    {
        ...
        {
            HighlightedCells = (
                from c in GetAdjacentCells(cell.X, cell.Y)
                select cells[c.X, c.Y]
            ).ToList();
        }
        else
        {
    ...
    private async Task CompleteMinigame()
    {
        ...
        await RunManager.Instance.OneOffSynchronizer.DoLocalCrystalSphereRewards(
            _owner,
            Rng,
            _revealed
        );
    ...
    {
        return GetHorizontalCells(x, y)
            .Concat(GetVerticalCells(x, y))
            .Concat(GetDiagonalCells(x, y))
            .Concat(
                new global::_003C_003Ez__ReadOnlySingleElementList<Vector2I>(new Vector2I(x, y))
            )
            .ToList();
    }

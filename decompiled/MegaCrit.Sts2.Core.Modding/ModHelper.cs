...
public static class ModHelper
{
    ...
    private static readonly Dictionary<Type, ModPoolContent> _moddedContentForPools =
        new Dictionary<Type, ModPoolContent>();

    private static readonly List<ModRunHookSubscriber> _runHookSubscribers =
        new List<ModRunHookSubscriber>();

    private static readonly List<ModCombatHookSubscriber> _combatHookSubscribers =
        new List<ModCombatHookSubscriber>();
    ...
    public static void AddModelToPool(Type poolType, Type modelType)
    {
        ...
        {
            value = new ModPoolContent { modelsToAdd = new List<Type>() };
            _moddedContentForPools.Add(poolType, value);
        }
        if (value.isFrozen)
        {
            throw new InvalidOperationException(
                $"Tried to add model {modelType} to pool {poolType}, but it's too late! You must add content before the game is initialized."
            );
        }
        ...
        where TModelType : AbstractModel
    {
        ...
        IEnumerable<TModelType> second = value.modelsToAdd.Select(
            (Type t) => ModelDb.GetById<TModelType>(ModelDb.GetId(t))
        );
    ...
    public static void SubscribeForRunStateHooks(string id, RunHookSubscriptionDelegate del)
    {
        ...
        {
            Log.Error(
                "Tried to subscribe for RunState hooks with id "
                    + id
                    + ", but it's already been used! Ignoring subscription"
            );
            return;
        }
        _runHookSubscribers.Add(new ModRunHookSubscriber { id = id, del = del });
        _runHookSubscribers.Sort(
            (ModRunHookSubscriber x, ModRunHookSubscriber y) => string.CompareOrdinal(x.id, y.id)
        );
    }
    ...
    public static void SubscribeForCombatStateHooks(string id, CombatHookSubscriptionDelegate del)
    {
        ...
        {
            Log.Error(
                "Tried to subscribe for CombatState hooks with id "
                    + id
                    + ", but it's already been used! Ignoring subscription"
            );
            return;
        }
        _combatHookSubscribers.Add(new ModCombatHookSubscriber { id = id, del = del });
        _combatHookSubscribers.Sort(
            (ModCombatHookSubscriber x, ModCombatHookSubscriber y) =>

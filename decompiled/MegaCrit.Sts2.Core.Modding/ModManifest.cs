...
public class ModManifest
{
    ...
    public static ModManifest? ReadFromStream(Stream stream, out List<LocString>? errors)
    {
        ...
        if (jsonNode2 != null && jsonNode2.GetValueKind() == JsonValueKind.Array)
        {
            ...
            {
                Log.Error(
                    "Detected old-style dependencies without min version specified! It works for now but this will be removed in a future release. Let the mod author know."
                );
                    ...
                    if (item != null)
                    {
                        jsonArray.Add(
                            new JsonObject(
                                ...
                                >(
                                    new KeyValuePair<string, JsonNode>[2]
                                    {

...
public class EventSplitVoteAnimation
{
    ...
    {
        if (
            _eventLayout.OptionButtons.Count(
                (NEventOptionButton b) => b.VoteContainer.Players.Any()
            ) == 1
        )
        {
            return;
        ...
        tween
            .TweenMethod(Callable.From<float>(TickSplitVoteAnimation), 0f, 1f, 1.2000000476837158)
    ...
    private void HighlightPlayer(Player? player)
    {
        ...
        if (_currentlyHighlightedPlayer != null)
        {
            ...
            {
                if (
                    optionButton.VoteContainer.Players.Contains<Player>(_currentlyHighlightedPlayer)
                )
                {
                    optionButton.VoteContainer.SetPlayerHighlighted(

...
public class MigratingData
{
    ...
    private static object? ConvertJsonNodeToObject(JsonNode? node)
    {
        if (node != null)
        {
            ...
            return node.Deserialize(
                JsonSerializationUtility.GetTypeInfo<Dictionary<string, object>>()
            );
        }
        return null;
    }

    public T? GetAsOrNull<T>(string key)
        where T : struct
    {
        if (!Has(key))
        {
    ...
    public T GetAs<T>(string key)
    {
            ...
            if (typeFromHandle == typeof(List<MigratingData>) && jsonNode is JsonArray jsonArray)
            {
                ...
                foreach (JsonNode item in jsonArray)
                {
                    ...
                    throw new MigrationException(
                        $"Cannot convert array item to MigratingData: {item}"
                    );
                }
        ...
        {
            throw new MigrationException(
                $"Cannot convert value of key={key} to {typeof(T)}: {ex.Message}"
            );
        }
    }
    ...
    public void Set<T>(string key, T value)
    {
        ...
        _data[key] = (
            (value != null)
                ? JsonValue.Create(value, JsonSerializationUtility.GetTypeInfo<T>())
                : null
        );
    }
    ...
    public void SetList<T>(string key, IEnumerable<T> items)
    {
        ...
        else
        {
            string json = JsonSerializer.Serialize(
                items,
                JsonSerializationUtility.GetTypeInfo<List<T>>()
            );

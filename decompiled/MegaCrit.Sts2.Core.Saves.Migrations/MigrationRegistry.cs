...
{
    public Dictionary<Type, List<IMigration>> Migrations { get; } =
        new Dictionary<Type, List<IMigration>>();
            ...
            for (int i = 0; i < IMigrationSubtypes.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (obj is IMigration migration)
                    {
                        ...
                        Log.Debug(
                            $"Registered migration for {migration.SaveType.Name} from v{migration.FromVersion} to v{migration.ToVersion}"
                        );
                    }
                    else
                    {
                        Log.Error(
                            "Failed to instantiate migration "
                                + type.Name
                                + ": Created instance is not an IMigration"
                        );
                    }
                }
                catch (Exception ex)

...
public class MigrationManager
{
    ...
    private void DeriveAndSetLatestVersions()
    {
        ...
        Log.Info(
            "Current save versions: "
                + string.Join(
                    "; ",
                    _latestVersions.Select<KeyValuePair<Type, int>, string>(
                        (KeyValuePair<Type, int> p) => $"{p.Key.Name} v{p.Value}"
                    )
                )
        );
    ...
    private void EnsureVersionSet<T>()
        where T : ISaveSchema
    {
    ...
    private void IsMigrationPathValid(Type saveType)
    {
        ...
        if (gapsInMigrationPath.Count > 0)
        {
            ...
            string message2 =
                $"Gap in migration path for {saveType.Name}: version {value2} is a target but not a source in any migration";
    ...
    private void SetMinimumSupportedVersion<T>(int version)
        where T : ISaveSchema
    {
        Type typeFromHandle = typeof(T);
    ...
    private int GetMinimumSupportedVersion<T>()
        where T : ISaveSchema
    {
        Type typeFromHandle = typeof(T);
        if (!_minimumSupportedVersions.TryGetValue(typeFromHandle, out var value))
        {
            throw new InvalidOperationException(
                "No minimum supported version found for "
                    + typeFromHandle.Name
                    + ". Each save schema type should have a minimum supported version set and a base migration implemented."
            );
        }
        return value;
    }

    private IMigration? GetMigration<T>(int fromVersion, int toVersion)
        where T : ISaveSchema
    {
        Type typeFromHandle = typeof(T);
        ...
        return value.FirstOrDefault(
            (IMigration m) => m.FromVersion == fromVersion && m.ToVersion == toVersion
        );
    }

    private int? GetNextVersion<T>(int currentVersion)
        where T : ISaveSchema
    {
        Type typeFromHandle = typeof(T);
        if (!_registry.Migrations.TryGetValue(typeFromHandle, out List<IMigration> value))
    ...
    public T CreateNewSave<T>()
        where T : ISaveSchema, new()
    {
        return new T { SchemaVersion = GetLatestVersion<T>() };
    }
    ...
    public ReadSaveResult<T> LoadSave<
        ...
        where T : ISaveSchema, new()
    {
        ...
        {
            Log.Warn(
                $"Primary save failed ({readSaveResult.Status}), attempting .backup fallback: {text}"
            );
    ...
    private ReadSaveResult<T> LoadSaveFromPath<
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicMethods)] T
    >(string filePath)
        where T : ISaveSchema, new()
    {
    ...
    private ReadSaveResult<T> LoadWithAggressiveRecovery<
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicMethods)] T
        ...
        where T : ISaveSchema, new()
    {
        try
        {
            ...
            {
                Log.Warn(
                    $"Save version {num} is newer than current {currentVersion}, attempting recovery..."
                );
                T val = RecoverPartialDataFromCorruptSave<T>(migratingData);
                if (val != null)
                {
                    Log.Info($"Successfully recovered data from future save version {num}");
                    return new ReadSaveResult<T>(
                        val,
                        ReadSaveStatus.RecoveredWithDataLoss,
                        $"Data recovered from future version {num} but newer fields were discarded"
                    );
                }
                string text =
                    $"Save file version {num} is newer than current version {currentVersion}";
            ...
            {
                Log.Warn(
                    $"Save version {num} is below minimum {minimumSupportedVersion}, attempting data scavenging..."
                );
                T val2 = RecoverPartialDataFromCorruptSave<T>(migratingData);
                if (val2 != null)
                {
                    Log.Info(
                        $"Successfully scavenged data from old save version {num} (recovery data not persisted)"
                    );
                    return new ReadSaveResult<T>(
                        val2,
                        ReadSaveStatus.RecoveredWithDataLoss,
                        $"Data recovered from version {num} but some information may be lost"
                    );
                }
            ...
            if (num < currentVersion)
            {
                try
                {
                    ...
                    Log.Info(
                        $"Successfully migrated {typeof(T).Name} from v{num} to v{data.SchemaVersion} (migration not persisted)"
                    );
                    return new ReadSaveResult<T>(
                        data,
                        ReadSaveStatus.MigrationRequired,
                        $"Save was migrated from version {num} to {data.SchemaVersion}"
                    );
                }
                catch (Exception ex2)
                {
                    ...
                    if (val3 != null)
                    {
                        ...
                        return new ReadSaveResult<T>(
                            val3,
                            ReadSaveStatus.RecoveredWithDataLoss,
                            $"Migration failed, recovered partial data from version {num}"
                        );
                    }
            ...
            if (!readSaveResult.Success || readSaveResult.SaveData == null)
            {
                ...
                if (val4 != null)
                {
                    ...
                    return new ReadSaveResult<T>(
                        val4,
                        ReadSaveStatus.RecoveredWithDataLoss,
                        "Save file was corrupt but partial data was recovered"
                    );
                }
                if (flag)
                {
                    ...
                    return new ReadSaveResult<T>(
                        ReadSaveStatus.MissingSchemaVersion,
                        "Save file is missing schema version and cannot be deserialized"
                    );
                }
        ...
        catch (JsonException ex3)
        {
            ...
            Log.Error(
                $"JSON parse error in {filePath} at path={value}, line={ex3.LineNumber}: {ex3.Message}"
            );
            ...
            if (text3 != null)
            {
                ...
                if (readSaveResult2.Success)
                {
                    ...
                    return new ReadSaveResult<T>(
                        readSaveResult2.SaveData,
                        ReadSaveStatus.JsonRepaired,
                        "Save file had JSON errors that were automatically repaired"
                    );
                }
                return readSaveResult2;
            }
            ...
            return new ReadSaveResult<T>(
                ReadSaveStatus.JsonParseError,
                $"JSON error at {value} (line {ex3.LineNumber}): {ex3.Message}"
            );
        }
        catch (Exception ex4)
        {
    ...
    private int? InferSchemaVersionFromStructure<T>(MigratingData data)
        where T : ISaveSchema
    {
        return null;
    }

    private T? RecoverPartialDataFromCorruptSave<T>(MigratingData data)
        where T : ISaveSchema, new()
    {
    ...
    private MigratingData MigrateDataSequentially<T>(MigratingData jsonObj)
        where T : ISaveSchema
    {
        ...
        while (num < currentVersion)
        {
            ...
            if (!nextVersion.HasValue)
            {
                throw new MigrationException(
                    $"Missing migration path for {typeof(T).Name} from v{num} to latest version"
                );
            }
            ...
            {
                throw new MigrationException(
                    $"Missing migration implementation for {typeof(T).Name} from v{num} to v{value}"
                );
            }
            ...
            {
                throw new MigrationException(
                    $"Error migrating {typeof(T).Name} from v{num} to v{value}: {ex.Message}",
                    ex
                );
            }
        }

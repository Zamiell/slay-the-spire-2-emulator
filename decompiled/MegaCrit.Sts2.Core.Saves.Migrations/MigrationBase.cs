...
public abstract class MigrationBase<T> : IMigration<T>, IMigration
    where T : ISaveSchema
{
    ...
    {
        _migrationAttribute = new Lazy<MigrationAttribute>(
            delegate
            {
                object[] customAttributes = GetType()
                    .GetCustomAttributes(typeof(MigrationAttribute), inherit: false);
                if (customAttributes.Length == 0)
                {
                    throw new InvalidOperationException(
                        GetType().Name + " is missing the [Migration] attribute"
                    );
                }

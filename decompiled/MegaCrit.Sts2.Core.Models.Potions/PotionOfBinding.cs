...
public sealed class PotionOfBinding : PotionModel
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(
            new DynamicVar[2] { new PowerVar<VulnerablePower>(1m), new PowerVar<WeakPower>(1m) }
        );
    ...
    protected override async Task OnUse(PlayerChoiceContext choiceContext, Creature? target)
    {
        ...
        foreach (Creature item in targets)
        {
            NCombatRoom.Instance?.CombatVfxContainer.AddChildSafely(
                NSmokePuffVfx.Create(item, NSmokePuffVfx.SmokePuffColor.Green)
            );
        }
        await PowerCmd.Apply<WeakPower>(
            choiceContext,
            targets,
            base.DynamicVars["VulnerablePower"].IntValue,
            base.Owner.Creature,
            null
        );
        await PowerCmd.Apply<VulnerablePower>(
            choiceContext,
            targets,
            base.DynamicVars["WeakPower"].IntValue,
            base.Owner.Creature,
            null
        );
    }

...
public struct AncientMetric
{
    ...
    {
        AncientChoiceHistoryEntry ancientChoiceHistoryEntry =
            playerEntry.AncientChoices.FirstOrDefault((AncientChoiceHistoryEntry o) => o.WasChosen);
        if (ancientChoiceHistoryEntry == null)
        {
            throw new InvalidOperationException(
                $"Failed to find chosen ancient choice! {entry.Rooms.First().ModelId} {playerEntry.PlayerId}"
            );
        }
        picked = ancientChoiceHistoryEntry.TextKey;
        skipped = (
            from o in playerEntry.AncientChoices
            where !o.WasChosen
            select o.TextKey
        ).ToList();
    }

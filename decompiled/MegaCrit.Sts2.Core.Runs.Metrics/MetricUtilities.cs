...
{
    private const string _metricsRunDataEndpoint =
        "https://sts2-metric-upload.megacrit.com/record_data/";

    private const string _metricsAchievementDataEndpoint =
        "https://sts2-metric-upload.megacrit.com/record_achievement/";

    private const string _metricsEpochDataEndpoint =
        "https://sts2-metric-upload.megacrit.com/record_epoch/";

    private const string _metricsSettingsDataEndpoint =
        "https://sts2-metric-upload.megacrit.com/record_settings/";
    ...
    )
    {
        ...
        else
        {
            ...
            List<MapPointHistoryEntry> list = run
                .MapPointHistory.SelectMany((List<MapPointHistoryEntry> logs) => logs)
                .ToList();
            if (list.Count < 5)
            {
                return;
            ...
            try
            {
                ...
                MapPointHistoryEntry mapPointHistoryEntry = run
                    .MapPointHistory.LastOrDefault()
                    ?.LastOrDefault();
                if (
                    !isVictory
                    && mapPointHistoryEntry != null
                    && mapPointHistoryEntry.Rooms.Last().RoomType.IsCombatRoom()
                )
                {
                    killedByEncounter = mapPointHistoryEntry.Rooms.Last().ModelId;
                }
                SerializablePlayer localPlayer = run.Players.First(
                    (SerializablePlayer p) => p.NetId == localPlayerId
                );
                List<EncounterMetric> encounters = (
                    from e in list
                    where e.Rooms.Last().RoomType.IsCombatRoom()
                    select new EncounterMetric(
                ...
                List<CardChoiceMetric> cardChoices = (
                    ...
                    select new CardChoiceMetric(e.GetEntry(localPlayerId).CardChoices)
                ).ToList();
                List<AncientMetric> ancientChoices = (
                    ...
                    select new AncientMetric(e, e.GetEntry(localPlayerId))
                ).ToList();
                ...
                for (int num = 0; num < run.MapPointHistory.Count; num++)
                {
                    ...
                    {
                        if (
                            item.Rooms.First().RoomType == RoomType.Event
                            && item.GetEntry(localPlayerId).EventChoices.Count != 0
                            && item.MapPointType != MapPointType.Ancient
                        )
                        {
                ...
                RunMetrics run2 = new RunMetrics
                {
                    ...
                    Team = (
                        (run.Players.Count > 1)
                            ? run.Players.Select((SerializablePlayer p) => p.CharacterId).ToList()
                            : new List<ModelId>()
                    ),
                    Win = isVictory,
                    ...
                    ActWins = list2,
                    CampfireUpgrades = (
                        from c in list.Where(
                                (MapPointHistoryEntry e) => e.MapPointType == MapPointType.RestSite
                            )
                            .SelectMany(
                                (MapPointHistoryEntry e) => e.GetEntry(localPlayerId).UpgradedCards
                            )
                        select c.Entry
                    ).ToList(),
                    RelicBuys = (
                        from r in list.SelectMany(
                            (MapPointHistoryEntry e) => e.GetEntry(localPlayerId).BoughtRelics
                        )
                        select r.Entry
                    ).ToList(),
                    PotionBuys = (
                        from p in list.SelectMany(
                            (MapPointHistoryEntry e) => e.GetEntry(localPlayerId).BoughtPotions
                        )
                        select p.Entry
                    ).ToList(),
                    ColorlessBuys = (
                        from c in list.SelectMany(
                            (MapPointHistoryEntry e) => e.GetEntry(localPlayerId).BoughtColorless
                        )
                        select c.Entry
                    ).ToList(),
                    PotionDiscards = (
                        from p in list.SelectMany(
                            (MapPointHistoryEntry e) => e.GetEntry(localPlayerId).PotionDiscarded
                        )
                        select p.Entry
                    ).ToList(),
                };
    ...
    private static void UploadRunMetrics(RunMetrics run)
    {
        ...
        TaskHelper.RunSafely(
            PutRequest("https://sts2-metric-upload.megacrit.com/record_data/", json, "Run metrics")
        );
    }
    ...
    private static async Task PutRequest(string url, string json, string context)
    {
        ...
        {
            Log.Warn(
                "Metrics upload for '" + context + "' failed due to network error: " + ex.Message
            );
            return;
        }
        ...
        Log.Warn(
            $"Metrics upload request for '{context}' failed with status code: {response.StatusCode}"
        );

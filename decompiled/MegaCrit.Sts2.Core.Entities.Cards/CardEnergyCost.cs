...
public sealed class CardEnergyCost
{
        ...
        if (cost != 0 || Canonical >= 0)
        {
            _localModifiers.Add(
                new LocalCostModifier(
                    ...
                    LocalCostModifierExpiration.WhenPlayed,
                    reduceOnly
                )
            );
        }
    ...
    {
        _card.AssertMutable();
        return _localModifiers.RemoveAll(
                (LocalCostModifier m) => m.Expiration.HasFlag(LocalCostModifierExpiration.EndOfTurn)
            ) > 0;
    }
    ...
    public bool AfterCardPlayedCleanup()
    {
        ...
        return _localModifiers.RemoveAll(
                (LocalCostModifier m) =>
                    m.Expiration.HasFlag(LocalCostModifierExpiration.WhenPlayed)
            ) > 0;
    }
    ...
    {
        List<LocalCostModifier> localModifiers = _localModifiers
            .Select((LocalCostModifier m) => m.Clone())
            .ToList();

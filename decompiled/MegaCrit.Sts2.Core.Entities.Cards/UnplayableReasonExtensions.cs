...
internal static class UnplayableReasonExtensions
{
    ...
    )
    {
        ...
        if (reason.HasFlag(UnplayableReason.BlockedByHook))
        {
            ...
            string text = (
                (preventer is CardModel cardModel)
                    ? cardModel.Title
                    : (
                        (preventer is RelicModel relicModel)
                            ? relicModel.Title.GetFormattedText()
                            : (
                                (preventer is PowerModel powerModel)
                                    ? powerModel.Title.GetFormattedText()
                                    : (
                                        (preventer is EnchantmentModel enchantmentModel)
                                            ? enchantmentModel.Title.GetFormattedText()
                                            : (
                                                (!(preventer is AfflictionModel afflictionModel))
                                                    ? null
                                                    : afflictionModel.Title.GetFormattedText()
                                            )
                                    )
                            )
                    )
            ...
            {
                Log.Error(
                    $"Missing case for model {preventer} in UnplayableReason.GetPlayerDialogueLine switch!"
                );
                text2 = "<UNKNOWN>";
            }

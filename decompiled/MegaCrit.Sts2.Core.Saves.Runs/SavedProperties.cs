...
public class SavedProperties : IPacketSerializable
{
    ...
    public static SavedProperties? FromInternal(object model, ModelId? id)
    {
        ...
        foreach (
            PropertyInfo item in SavedPropertiesTypeCache.GetJsonPropertiesForType(model.GetType())
                ?? new List<PropertyInfo>()
        )
        {
            string name = item.Name;
            object value = item.GetValue(model);
            SavedPropertyAttribute customAttribute =
                item.GetCustomAttribute<SavedPropertyAttribute>();
                ...
                if (!(value is int[] value3))
                {
                    if (!(value is Enum value4))
                    {
                        if (!(value is Enum[] source))
                        {
                            if (!(value is ModelId value5))
                            {
                                if (!(value is bool value6))
                                {
                                    if (!(value is string value7))
                                    {
                                        if (!(value is SerializableCard value8))
                                        {
                                            if (!(value is List<SerializableCard> list))
                                            {
                                                throw new JsonException(
                                                    $"Property {name} on {id} is not a valid type for [SavedProperty] (type {value.GetType()})."
                                                );
                                            }
                                            SavedProperties savedProperties2 = savedProperties;
                                            if (savedProperties2.cardArrays == null)
                                            {
                                                savedProperties2.cardArrays =
                                                    new List<SavedProperty<SerializableCard[]>>();
                                            }
                                            savedProperties.cardArrays.Add(
                                                new SavedProperty<SerializableCard[]>(
                                                    name,
                                                    list.ToArray()
                                                )
                                            );
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                savedProperties2.cards =
                                                    new List<SavedProperty<SerializableCard>>();
                                            }
                                            savedProperties.cards.Add(
                                                new SavedProperty<SerializableCard>(name, value8)
                                            );
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            savedProperties2.strings =
                                                new List<SavedProperty<string>>();
                                        }
                                        savedProperties.strings.Add(
                                            new SavedProperty<string>(name, value7)
                                        );
                                    }
                                }
                                else
                                {
                                    ...
                                    savedProperties.bools.Add(
                                        new SavedProperty<bool>(name, value6)
                                    );
                                }
                            }
                            else
                            {
                                ...
                                savedProperties.modelIds.Add(
                                    new SavedProperty<ModelId>(name, value5)
                                );
                            }
                        }
                        else
                        {
                            ...
                            savedProperties.intArrays.Add(
                                new SavedProperty<int[]>(
                                    name,
                                    source.Select(Convert.ToInt32).ToArray()
                                )
                            );
                        }
                    }
                    else
                    {
                        ...
                        savedProperties.ints.Add(
                            new SavedProperty<int>(name, Convert.ToInt32(value4))
                        );
                    }
                }
                else
    ...
    [UnconditionalSuppressMessage(
        "AOT",
        "IL3050",
        Justification = "We only create array types that are referenced by SavedProperties that already exist in code."
    )]
    public void FillInternal(object model)
    {
            ...
            foreach (SavedProperty<int> @int in ints)
            {
                ...
                property?.SetValue(
                    ...
                    property.PropertyType.IsEnum
                        ? Enum.ToObject(property.PropertyType, @int.value)
                        : ((object)@int.value)
                );
            }
        }
        if (intArrays != null)
        {
            ...
            {
                PropertyInfo property2 = type.GetProperty(
                    intArray.name,
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic
                );
                if (property2 == null)
        ...
        if (cardArrays != null)
        {
            foreach (SavedProperty<SerializableCard[]> cardArray in cardArrays)
            {
                type.GetProperty(
                    ...
                    ?.SetValue(model, cardArray.value.ToList());
            }
        }
        if (strings == null)
        {
    ...
    {
        writer.WriteInt(
            SavedPropertiesTypeCache.GetNetIdForPropertyName(propertyName),
    ...
    public void Deserialize(PacketReader reader)
    {
        ...
        if (reader.ReadBool())
        {
            ...
            {
                modelIds.Add(
                    new SavedProperty<ModelId>(ReadPropertyName(reader), reader.ReadFullModelId())
                );
            }
        }
        if (reader.ReadBool())
        ...
        if (reader.ReadBool())
        {
            ...
            {
                strings.Add(
                    new SavedProperty<string>(ReadPropertyName(reader), reader.ReadString())
                );
            }
        }
    }
            ...
            foreach (SavedProperty<SerializableCard[]> cardArray in cardArrays)
            {
                list.Add(
                    cardArray.name
                        ...
                        + string.Join(
                            ",",
                            cardArray.value.Select((SerializableCard c) => c.Id.Entry)
                        )
                );
            }
        }

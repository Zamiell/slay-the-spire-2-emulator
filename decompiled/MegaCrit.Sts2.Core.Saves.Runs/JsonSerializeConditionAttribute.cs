...
public class JsonSerializeConditionAttribute : Attribute
{
    ...
    public static void CheckJsonSerializeConditionsModifier(JsonTypeInfo typeInfo)
    {
        ...
        {
            JsonSerializeConditionAttribute attr = property
                .AttributeProvider?.GetCustomAttributes(inherit: true)
                .OfType<JsonSerializeConditionAttribute>()
                .FirstOrDefault();
            if (attr == null)
            {
                continue;
            ...
            {
                property.ShouldSerialize = (object _, object? c) =>
                    attr.defaultBehaviour.ShouldSerialize(c, memberInfo);
            }
        }
    }
}

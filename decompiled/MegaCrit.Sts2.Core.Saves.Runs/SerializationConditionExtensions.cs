...
{
    private static readonly Dictionary<Type, object?> _defaultTypeCache =
    ...
    private static object? GetMemberDefaultValue(MemberInfo info)
    {
        ...
        return info.MemberType switch
        {
            ...
            _ => throw new ArgumentException(
                $"Input MemberInfo must be of type FieldInfo or PropertyInfo. It was {info.MemberType} ({info.GetType()})"
            ),
        };
    }
    ...
    private static Type GetUnderlyingType(this MemberInfo member)
    {
        return member.MemberType switch
        {
            ...
            _ => throw new ArgumentException(
                $"Input MemberInfo must be of type FieldInfo or PropertyInfo. It was {member.MemberType} ({member.GetType()})"
            ),
        };
    }
        ...
        switch (condition)
        {
            ...
            case SerializationCondition.SaveIfNotTypeDefault:
                return !object.Equals(
                    candidate,
                    GetTypeDefaultValue(memberInfo.GetUnderlyingType())
                );

...
public class NBestiaryLayoutDecimillipede : NBestiaryLayout
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "screens/bestiary/bestiary_layout_decimillipede"
    );
    ...
    public static NBestiaryLayoutDecimillipede? Create(NBestiary bestiary)
    {
        ...
        NBestiaryLayoutDecimillipede nBestiaryLayoutDecimillipede = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NBestiaryLayoutDecimillipede>(PackedScene.GenEditState.Disabled);
    ...
    public override List<BestiaryMonsterMove> Setup(BestiaryEntry entry, Tween tween)
    {
        ...
        foreach (var monstersWithSlot in encounterModel.MonstersWithSlots)
        {
            ...
            nCreature.GlobalPosition = _encounterSlots
    ...
    private LocString GetBestiaryMoveName(string moveId)
    {
        return new LocString(
            "monsters",
            ModelDb.GetId<DecimillipedeSegment>().Entry + ".moves." + moveId + ".title"
        );
    }
    ...
    private async Task AnimAttack()
    {
        ...
        Node2D node2D = PreloadManager
            .Cache.GetScene(DecimillipedeSegment.rocksVfxPath)
    ...
    private async Task AnimReattach()
    {
        ...
        {
            creature
                .GetSpecialNode<NDecimillipedeSegmentVfx>("%NDecimillipedeSegmentVfx")
                ?.Regenerate();
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NBestiaryLayoutDecimillipede>(
                Create(VariantUtils.ConvertTo<NBestiary>(in args[0]))
            );
            return true;
        }
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._encounterVisuals,
                PropertyHint.None,
                "",

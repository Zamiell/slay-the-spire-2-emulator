...
public class NBestiary : NSubmenu
{
    ...
    private static readonly LocString _placeholderDesc = new LocString(
        "bestiary",
    ...
    protected override Control? InitialFocusedControl =>
        _bestiaryList.GetChildren().OfType<NBestiaryEntry>().FirstOrDefault();
    ...
    public static NBestiary? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NBestiary>(PackedScene.GenEditState.Disabled);
    }

    public override void _Ready()
    {
        ConnectSignals();
        GetNode<MegaLabel>("%MoveHeader")
            .SetTextAutoSize(new LocString("bestiary", "ACTIONS.header").GetFormattedText());
        GetNode<MegaRichTextLabel>("%ConstructionLabel")
            .SetTextAutoSize(new LocString("bestiary", "UNDER_CONSTRUCTION").GetRawText());
    ...
    {
        _discoveredMonsterIds = (
            from e in SaveManager.Instance.Progress.EnemyStats.Values
            where e.TotalWins > 0
            select e.Id
        ).ToHashSet();
        _discoveredEncounterIds = (
            from e in SaveManager.Instance.Progress.EncounterStats.Values
            where e.TotalWins > 0
            select e.Id
        ).ToHashSet();
        ...
        NBestiaryEntry nBestiaryEntry = _bestiaryList
            .GetChildren()
            .OfType<NBestiaryEntry>()
            .FirstOrDefault((NBestiaryEntry e) => e.IsDiscovered && e.IsEnabled);
        if (nBestiaryEntry == null)
        {
            Log.Error(
                "Should not be possible as the Compendium + Bestiary isn't unlocked by default!"
            );
        }
        else
        {
    ...
    private void AddAct(ActModel act)
    {
        ...
        foreach (EncounterModel allEncounter in act.AllEncounters)
        {
            foreach (MonsterModel allPossibleMonster in allEncounter.AllPossibleMonsters)
            {
                ...
                {
                    list.Add(
                        BestiaryEntry.FromMonster(
                            allPossibleMonster,
                            allEncounter,
                            allEncounter.RoomType
                        )
                    );
                }
            }
        ...
        {
            list.Add(
                BestiaryEntry.FromEncounter(ModelDb.Encounter<DecimillipedeElite>(), RoomType.Elite)
            );
        }
        list.Sort(
            delegate(BestiaryEntry e1, BestiaryEntry e2)
            {
                ...
                {
                    int num = string.Compare(
                        e1.GetEncounterTitle(),
                        e2.GetEncounterTitle(),
        ...
        {
            NBestiaryEntry nBestiaryEntry = NBestiaryEntry.Create(
                item,
                item.IsDiscovered(_discoveredMonsterIds, _discoveredEncounterIds)
            );
            _bestiaryList.AddChildSafely(nBestiaryEntry);
            nBestiaryEntry.Connect(
                NClickableControl.SignalName.Released,
                Callable.From<NBestiaryEntry>(OnMonsterClicked)
            );
        }
    }
    ...
    private void SelectMonster(NBestiaryEntry entry)
    {
        ...
        else
        {
            ...
            _tween
                .TweenProperty(_monsterNameLabel, "position:y", 88f, 0.5)
            ...
            _tween
                .TweenProperty(_descriptionLabel, "position:y", 894f, 0.5)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Expo)
                .From(958f);
            _tween.TweenProperty(_descriptionLabel, "modulate:a", 1f, 0.5);
            _tween
                .TweenProperty(_moveContainer, "position:x", 242f, 0.5)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Expo)
                .From(210f)
                .SetDelay(0.2);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                NBestiaryMoveButton nBestiaryMoveButton = NBestiaryMoveButton.Create(
                    list[i],
                    $"mega_select_card_{i + 1}"
                );
                _moveList.AddChildSafely(nBestiaryMoveButton);
                nBestiaryMoveButton.Connect(
                    NClickableControl.SignalName.Released,
                    Callable.From<NBestiaryMoveButton>(OnMoveButtonClicked)
                );
            }
        }
        ...
        else
        {
            ...
            _arrowTween
                .TweenProperty(_selectionArrow, "position", entry.Position + _arrowOffset, 0.25)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Expo);
        }
    }
    ...
    private void OnMoveButtonClicked(NButton button)
    {
        ...
        PlayMoveAnim(
            _currentLayout?.GetCreatures() ?? Array.Empty<NCreature>(),
            nBestiaryMoveButton.Move
        );
    }
    ...
    private void PlayMoveAnim(IEnumerable<NCreature> creatures, BestiaryMonsterMove move)
    {
        foreach (NCreature creature in creatures)
        {
            if (move.stateId != null)
            {
                ...
                if (monsterModel == null)
                {
                    throw new InvalidOperationException(
                        $"Non-monster creature {creature} is in the bestiary!"
                    );
                }
                monsterModel.SetMoveImmediate(
                    (MoveState)monsterModel.MoveStateMachine.States[move.stateId],
            ...
            {
                creature
                    ?.Visuals.SpineBody?.GetAnimationState()
                    .SetAnimation(move.animId, loop: false);
                if (move.animId != "die")
                {
    ...
    public static bool CanBeShown()
    {
        ...
        return SaveManager.Instance.Progress.EnemyStats.Values.Any(
            (EnemyStats e) => e.TotalWins > 0
        );
    }
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.OnMoveButtonClicked,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._monsterNameLabel,
                PropertyHint.None,
                "",
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Bool,
                PropertyName._initSelectionArrow,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._previousScreenshakeTarget,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._previousScreenshakeTarget,
            Variant.From(in _previousScreenshakeTarget)
        );

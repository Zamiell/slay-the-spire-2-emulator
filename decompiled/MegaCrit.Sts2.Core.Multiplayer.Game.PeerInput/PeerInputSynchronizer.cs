...
public class PeerInputSynchronizer : IDisposable
{
    ...
    public void SyncLocalMousePos(Vector2 mouseScreenPos, Control? rootControl)
    {
        ...
        Vector2 vector =
            _cursorTranslator?.GetNetPositionFromScreenPosition(mouseScreenPos)
    ...
    public void SyncLocalControllerFocus(Vector2 focusPosition, Control? rootControl)
    {
        ...
        Vector2 vector =
            _cursorTranslator?.GetNetPositionFromScreenPosition(focusPosition)
            ?? NetCursorHelper.GetNormalizedPosition(focusPosition, rootControl);
    ...
    public void SyncLocalScreen(NetScreenType netScreenType)
    {
        ...
        {
            _logger.Debug(
                $"Local screen changed: {orCreateStateForPlayer.netScreenType}->{netScreenType}"
            );
    ...
    private void SendSyncMessage()
    {
        if (_netService.IsConnected)
        {
            ...
            _syncMessageToSend.controllerFocusPosition =
                orCreateStateForPlayer.controllerFocusPosition;
    ...
    private PeerInputState GetOrCreateStateForPlayer(ulong playerId)
    {
        ...
        {
            peerInputState = new PeerInputState { playerId = playerId };
    ...
    private void HandlePeerInputMessage(PeerInputMessage message, ulong senderId)
    {
        ...
        {
            _logger.Debug(
                $"Mouse down state for {senderId} changed: {orCreateStateForPlayer.isMouseDown}->{message.mouseDown}"
            );
        }
        if (orCreateStateForPlayer.netScreenType != message.screenType)
        {
            _logger.Debug(
                $"Remote screen for {senderId} changed: {orCreateStateForPlayer.netScreenType}->{message.screenType}"
            );
        }
        if (orCreateStateForPlayer.isTargeting != message.isTargeting)
        {
            _logger.Debug(
                $"Targeting state for {senderId} changed: {orCreateStateForPlayer.isTargeting}->{message.isTargeting}"
            );
        }
        if (!orCreateStateForPlayer.hoveredModelData.Equals(message.hoveredModelData))
        {
            _logger.Debug(
                $"Hovered model for {senderId} changed: {orCreateStateForPlayer.hoveredModelData}->{message.hoveredModelData}"
            );
        }
        if (!orCreateStateForPlayer.controllerFocusPosition.Equals(message.controllerFocusPosition))
        {
            _logger.Debug(
                $"Controller focus position for {senderId} changed: {orCreateStateForPlayer.controllerFocusPosition}->{message.controllerFocusPosition}"
            );
        }
        if (!orCreateStateForPlayer.isUsingController.Equals(message.isUsingController))
        {
            _logger.Debug(
                $"Using controller state state for {senderId} changed: {orCreateStateForPlayer.isUsingController}->{message.isUsingController}"
            );
        }
        NetScreenType netScreenType = orCreateStateForPlayer.netScreenType;
        orCreateStateForPlayer.netMousePosition =
            message.netMousePos ?? orCreateStateForPlayer.netMousePosition;
        ...
        orCreateStateForPlayer.controllerFocusPosition =
            message.controllerFocusPosition ?? orCreateStateForPlayer.controllerFocusPosition;
    ...
    public Vector2 GetControlSpaceFocusPosition(ulong playerId, Control? rootControl)
    {
        ...
        Vector2 vector = (
            orCreateStateForPlayer.isUsingController
                ? orCreateStateForPlayer.controllerFocusPosition
                : orCreateStateForPlayer.netMousePosition
        );
        ...
        Vector2 screenPositionFromNetPosition = _cursorTranslator.GetScreenPositionFromNetPosition(
            vector
        );
        if (rootControl == null)
        {

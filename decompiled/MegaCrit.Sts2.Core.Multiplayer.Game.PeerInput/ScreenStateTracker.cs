...
public class ScreenStateTracker
{
    ...
    private void OnCapstoneScreenChanged()
    {
        ...
        {
            _capstoneScreen =
                NCapstoneContainer.Instance.CurrentCapstoneScreen?.ScreenType ?? NetScreenType.None;
            SyncLocalScreen();
        }
    }
    ...
    private void OnOverlayStackChanged()
    {
        if (!RunManager.Instance.IsSinglePlayerOrFakeMultiplayer)
        {
            ...
            if (
                overlayScreen is NRewardsScreen nRewardsScreen
                && !nRewardsScreen.IsConnected(
                    NRewardsScreen.SignalName.Completed,
                    _onRewardsScreenCompleted
                )
            )
            {
                nRewardsScreen.Connect(
                    NRewardsScreen.SignalName.Completed,
                    _onRewardsScreenCompleted
                );
            }

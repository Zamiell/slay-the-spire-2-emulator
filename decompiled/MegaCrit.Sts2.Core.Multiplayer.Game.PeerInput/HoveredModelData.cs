...
public struct HoveredModelData : IPacketSerializable, IEquatable<HoveredModelData>
{
    ...
    public bool Equals(HoveredModelData other)
    {
        if (type == other.type)
        {
            ...
            if (
                netCombatCard.HasValue == netCombatCard2.HasValue
                && (
                    !netCombatCard.HasValue
                    || netCombatCard.GetValueOrDefault() == netCombatCard2.GetValueOrDefault()
                )
                && hoveredRelicIndex == other.hoveredRelicIndex
                && hoveredPotionIndex == other.hoveredPotionIndex
            )
            {
            ...
            if (!(model is RelicModel relicModel))
            {
                if (!(model is PotionModel potionModel))
                {
                    throw new InvalidOperationException(
                        $"Model {model} has unsupported type for hovering"
                    );
                }

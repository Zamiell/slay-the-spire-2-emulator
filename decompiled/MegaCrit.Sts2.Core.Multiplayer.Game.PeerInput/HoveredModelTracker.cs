...
public class HoveredModelTracker
{
    ...
    private void OnPlayerStateChanged(ulong playerId)
    {
        ...
        if (playerSlotIndex < _hoveredModels.Count)
        {
            ...
            AbstractModel abstractModel = hoveredModelData.type switch
            {
                ...
                HoveredModelType.Relic => (hoveredModelData.hoveredRelicIndex < player.Relics.Count)
                    ? player.Relics[hoveredModelData.hoveredRelicIndex.Value]
                    : null,
                HoveredModelType.Potion => (
                    hoveredModelData.hoveredPotionIndex < player.PotionSlots.Count
                    && hoveredModelData.hoveredPotionIndex >= 0
                )
                    ? player.GetPotionAtSlotIndex(hoveredModelData.hoveredPotionIndex.Value)
                    : null,
                _ => throw new InvalidOperationException(
                    $"Unsupported hover type {hoveredModelData.type}"
                ),
            };
    ...
    {
        return _hoveredModels[
            _playerCollection.GetPlayerSlotIndex(_playerCollection.GetPlayer(playerId))
        ];
    }
    ...
    private void SynchronizeLocalHoveredModel()
    {
        AbstractModel model = (AbstractModel)(
            _localSelectedCard
            ?? _localSelectedPotion
            ?? _localHoveredCard
            ?? ((object)_localHoveredPotion)
            ?? ((object)_localHoveredRelic)
        );

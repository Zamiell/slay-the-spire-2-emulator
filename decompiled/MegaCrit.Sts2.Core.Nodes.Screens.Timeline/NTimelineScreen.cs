...
public class NTimelineScreen : NSubmenu
{
    ...
    private static readonly string _scenePath = SceneHelper.GetScenePath(
        "timeline_screen/timeline_screen"
    );

    private const string _placeEpochSparksPath =
        "res://scenes/timeline_screen/place_epoch_sparks.tscn";
    ...
    private Dictionary<EpochEra, NEraColumn> _uniqueEpochEras =
        new Dictionary<EpochEra, NEraColumn>();
    ...
    public static NTimelineScreen Instance =>
        NGame.Instance.MainMenu.SubmenuStack.GetSubmenuType<NTimelineScreen>();
    ...
    protected override Control? InitialFocusedControl =>
        _epochSlotContainer
            .GetChildren()
            .SelectMany((Node c) => c.GetChildren().OfType<NEpochSlot>())
            .FirstOrDefault((NEpochSlot s) => s.model is NeowEpoch);
    ...
    public static NTimelineScreen? Create()
    {
        ...
        return PreloadManager
            ...
            .Instantiate<NTimelineScreen>(PackedScene.GenEditState.Disabled);
    }
    ...
    public override void OnSubmenuOpened()
    {
        ResetScreen();
        DisableInput();
        SerializableEpoch serializableEpoch = SaveManager.Instance.Progress.Epochs.FirstOrDefault(
            (SerializableEpoch e) => e.Id == EpochModel.GetId<NeowEpoch>()
        );
    ...
    private async Task InitScreen()
    {
        ...
        foreach (SerializableEpoch epoch2 in _save.Epochs)
        {
            ...
            {
                if (
                    child is NEpochSlot nEpochSlot
                    && nEpochSlot.eraPosition == epochModel.EraPosition
                )
                {
                    num++;
                    nEpochSlot.SetState(
                        (epoch2.State >= EpochState.Revealed)
                            ? EpochSlotState.Complete
                            : EpochSlotState.Obtained
                    );
    ...
    private List<Vector2> PredictHBoxLayout(HBoxContainer hbox)
    {
        ...
        List<Control> list = (
            from c in hbox.GetChildren().OfType<Control>()
            where c.Visible
            select c
        ).ToList();
        int num3 = 0;
        ...
        if (newlyCreatedColumns.Count > 0)
        {
            ...
            _lineGrowTween
                .TweenProperty(
                    ...
                    (float)_uniqueEpochEras.Count * 226f,
                    2.0
                )
    ...
    private void InitLineAndIcons(List<NEraColumn> newlyCreatedColumns)
    {
        ...
        _line.CustomMinimumSize = new Vector2(
            (float)_uniqueEpochEras.Count * 226f,
            _line.CustomMinimumSize.Y
        );
        foreach (NEraColumn newlyCreatedColumn in newlyCreatedColumns)
        {
            newlyCreatedColumn.SpawnIcon();
    ...
    {
        return (
            Texture: PreloadManager.Cache.GetTexture2D(GetEraTexturePath(era)),
            Name: StringHelper.Slugify(era.ToString())
        );
    }
    ...
    private NEpochSlot? GetSlot(EpochEra era, int position)
    {
        foreach (KeyValuePair<EpochEra, NEraColumn> uniqueEpochEra in _uniqueEpochEras)
        {
            ...
            {
                return (NEpochSlot)
                    uniqueEpochEra.Value.GetChild(
                        uniqueEpochEra.Value.GetChildCount() - position - 2
                    );
            }
        }
    ...
    public async Task HideBackstopAndShowUi(bool showBackButton)
    {
        ...
        _backstopTween = CreateTween().SetParallel();
        _backstopTween
            .TweenProperty(_slotsContainer, "modulate:a", 1f, 0.4)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
        _backstopTween
            .TweenProperty(_backstop, "modulate:a", 0f, 0.4)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
        if (showBackButton)
        {
    ...
    internal static new List<MethodInfo> GetGodotMethodList()
    {
        ...
        list.Add(
            new MethodInfo(
                MethodName.SetScreenDraggability,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ShowBackstopAndHideUi,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.ShowHeaderAndActionsUi,
                new PropertyInfo(
                    Variant.Type.Nil,
                    "",
                    PropertyHint.None,
        ...
        list.Add(
            new MethodInfo(
                MethodName.GetReminderVfxHolder,
                new PropertyInfo(
                    Variant.Type.Object,
                    "",
                    PropertyHint.None,
    ...
    )
    {
        ...
        if (method == MethodName.GetEraTexturePath && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<string>(
                GetEraTexturePath(VariantUtils.ConvertTo<EpochEra>(in args[0]))
            );
            return true;
        }
        if (method == MethodName.GetSlot && args.Count == 2)
        {
            ret = VariantUtils.CreateFrom<NEpochSlot>(
                GetSlot(
                    VariantUtils.ConvertTo<EpochEra>(in args[0]),
        ...
        {
            OpenInspectScreen(
                VariantUtils.ConvertTo<NEpochSlot>(in args[0]),
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._reminderVfxHolder,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._epochSlotContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._unlockScreenHolder,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._queuedInspectScreen,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName.CurrentUnlockScreen,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName.CurrentUnlockScreen,
            Variant.From<NUnlockScreen>(CurrentUnlockScreen)
        );

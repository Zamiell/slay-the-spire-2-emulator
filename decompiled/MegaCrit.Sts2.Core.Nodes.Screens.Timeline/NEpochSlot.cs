...
public class NEpochSlot : NButton
{
    ...
    {
        public static new readonly StringName ClickedSfx = "ClickedSfx";

        public static new readonly StringName HoveredSfx = "HoveredSfx";
        ...
        public static new readonly StringName _isHovered = "_isHovered";
    ...
    {
        NEpochSlot nEpochSlot = PreloadManager
            .Cache.GetScene("res://scenes/timeline_screen/epoch_slot.tscn")
            .Instantiate<NEpochSlot>(PackedScene.GenEditState.Disabled);
    ...
    protected override void OnRelease()
    {
        if (
            !NGame.IsReleaseGame()
            && State == EpochSlotState.NotObtained
        ...
        _hoverTween
            .TweenProperty(this, "scale", Vector2.One * 1.05f, 0.25)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Back);
        _hoverTween.TweenMethod(
            Callable.From<float>(UpdateShaderV),
            _hsv.GetShaderParameter(_s),
            1f,
            0.05
        );
    }
    ...
    protected override void OnFocus()
    {
        ...
        if (State != EpochSlotState.NotObtained)
        {
            ...
            if (State == EpochSlotState.Complete)
            {
                ...
                _hoverTip = new HoverTip(
                    model.Title,
                    unlockInfo,
                    PreloadManager.Cache.GetTexture2D("res://images/packed/unlock_icon.png")
                );
            }
        ...
        if (_hoverTip != null)
        {
            ...
            float num2 =
                base.GlobalPosition.X + (base.Size.X * 0.5f + 6f) * GetGlobalTransform().Scale.X;
            if (base.GlobalPosition.X > NGame.Instance.GetViewportRect().Size.X * 0.7f)
            {
                nHoverTipSet?.SetGlobalPosition(
                    new Vector2(num2 - num - 360f, base.GlobalPosition.Y)
                );
            }
            else
            {
    ...
    protected override void OnPress()
    {
        if (State != EpochSlotState.NotObtained)
        {
            ...
            _hoverTween = CreateTween().SetParallel();
            _hoverTween
                .TweenProperty(this, "scale", Vector2.One * 0.95f, 0.25)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Expo);
            if (State == EpochSlotState.Complete)
            {
    ...
    public async Task SpawnSlot()
    {
        ...
        _spawnTween.Chain();
        _spawnTween
            .TweenProperty(_slotImage, "position", Vector2.Zero, 0.5)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Back)
            .From(new Vector2(0f, 64f));
        _spawnTween
            .TweenProperty(_outline, "position", Vector2.Zero, 0.5)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Back)
            .From(new Vector2(0f, 64f));
        _spawnTween
            .TweenProperty(_slotImage, "self_modulate:a", 1f, 0.5)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Cubic);
    ...
    public override void _Process(double delta)
    {
        if (_isGlowPulsing)
        {
            _outline.Modulate = new Color(
                _outline.Modulate.R,
                _outline.Modulate.G,
                _outline.Modulate.B,
                (Mathf.Sin((float)Time.GetTicksMsec() * 0.005f) + 2f) * 0.25f
            );
        }
    }
    ...
    internal static new List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                Variant.Type.Object,
                PropertyName._subViewportContainer,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false
    ...
    protected override void SaveGodotObjectData(GodotSerializationInfo info)
    {
        ...
        info.AddProperty(
            PropertyName._subViewportContainer,
            Variant.From(in _subViewportContainer)
        );

...
public class NSlotsContainer : Control
{
    ...
    private void ProcessPanEvent(InputEvent inputEvent)
    {
        ...
        else if (inputEvent is InputEventMouseMotion inputEventMouseMotion && _isDragging)
        {
            _targetPosition += new Vector2(
                (inputEventMouseMotion.Position - _dragStartPosition).X,
                0f
            );
    ...
    private void ProcessGuiFocus(Control focusedControl)
    {
        if (
            ...
            && IsAncestorOf(focusedControl)
        )
        {
            float x =
                _whatsMoved.GlobalPosition.X - focusedControl.GetParent<Control>().GlobalPosition.X;
    ...
    {
        float num =
            _whatsMoved.GlobalPosition.X
            - slotPositionX
            + 960f
            - 96f
            + (base.Size.X - 1920f) * 0.5f;
        _tween?.Kill();
        _tween = CreateTween();
        _tween
            .TweenProperty(_whatsMoved, "global_position:x", num, 2.5)
    ...
    internal static List<PropertyInfo> GetGodotPropertyList()
    {
        ...
        list.Add(
            new PropertyInfo(
                ...
                PropertyName._dragStartPosition,
                PropertyHint.None,
                "",
                PropertyUsageFlags.ScriptVariable,
                exported: false

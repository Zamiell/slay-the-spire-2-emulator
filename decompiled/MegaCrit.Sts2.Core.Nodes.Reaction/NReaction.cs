...
public class NReaction : TextureRect
{
    ...
    {
        NReaction nReaction = PreloadManager
            .Cache.GetScene(_scenePath)
            .Instantiate<NReaction>(PackedScene.GenEditState.Disabled);
    ...
    private async Task DoAnim()
    {
        ...
        tween.SetParallel();
        tween
            .TweenProperty(this, "position", vector, 0.30000001192092896)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
        tween
            .TweenProperty(this, "modulate:a", 1f, 0.30000001192092896)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Expo);
    ...
    )
    {
        ...
        {
            ret = VariantUtils.CreateFrom<NReaction>(
                Create(VariantUtils.ConvertTo<Texture2D>(in args[0]))
            );
            return true;
        }
        ...
        if (method == MethodName.TypeToTexture && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<Texture2D>(
                TypeToTexture(VariantUtils.ConvertTo<ReactionType>(in args[0]))
            );
            return true;
        }
        if (method == MethodName.TextureToType && args.Count == 1)
        {
            ret = VariantUtils.CreateFrom<ReactionType>(
                TextureToType(VariantUtils.ConvertTo<Texture2D>(in args[0]))
            );
            return true;
        }

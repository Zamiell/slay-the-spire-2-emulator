...
- Current curated trace mismatch status after this session's RNG fixes: DRUM_1 step 49 (floor 4 event behavior - gold/HP from event differs), TRACE_CARDS_1 step 46 (floor 5 encounter type wrong - normal encounter uses numpy vs `up_front` RNG), RARITY_1 step 27 (map layout wrong - emulator generates Monster where reference has Shop), INSTANT_5/9 step 23/43 (multi-enemy targeting - architectural limitation). Remaining RNG gaps: normal/elite/boss encounter selection uses numpy, should use `up_front` Rng from ActModel.PopulateRooms; event selection after weak pool also uses numpy.

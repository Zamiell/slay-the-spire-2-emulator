...
- Current curated trace mismatch summary: DRUM_1 step 46 (floor 3→4 map topology; emulator has extra col=5 node in row 1); TRACE_CARDS_1 step 79 (state_type rewards vs map + Punch Construct HP/gold diff, root cause: Punch Construct HP differs from reference due to Niche RNG mismatch); RARITY_1 step 32 (max_hp diff from missed event at floor 4, minor map topology difference at rows 1/5, replay reaches step 83); INSTANT_5 step 43 (multi-enemy targeting - architectural); INSTANT_9 step 23 (multi-enemy targeting - architectural); INSTANT_10 step 19 (multi-enemy targeting, map now perfectly matches reference).

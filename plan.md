...
- Fixed rest site two-step confirmation: 1st choose_rest_option selects (no effect), 2nd applies effect, proceed exits; matches STS2 reference behavior. Updated replay script to handle 'proceed' in PHASE_REST.
- Elite encounter GrabBag (15 slots, up_front Rng) and boss encounter (1 up_front NextItem call) now use the correct RNG stream matching ActModel.GenerateRooms, replacing numpy. DRUM_1 first mismatch advances from step 49 (floor 4 event mismatch) to step 58 (floor 8 encounter HP, root cause: map topology difference from missing pruning). INSTANT_5 floor mismatch at step 43 resolved; trace now reaches step 65.
- Added stub map pruning methods (_prune_duplicate_path_segments) based on decompiled MapPathPruning.PruneDuplicateSegments, not yet called due to incorrect node-removal logic (breaks connectivity). The correct implementation must reconnect parent→child edges when removing linear segment nodes, and must use mapRng.shuffle() for reproducibility.
- Fixed _is_upgradable: now returns True for any unupgraded card NOT in NON_UPGRADABLE_CARD_IDS (cards with MaxUpgradeLevel=0: status/curse cards including AscendersBane, Dazed, Slimed, Wound, etc.). Previously only checked 3 starter card types. Affects DoorsOfLightAndDark StableShuffle niche calls: DRUM_1 deck has 13 upgradable cards (12 Fisher-Yeas calls) giving total niche skip=14 before floor 8. Traced through decompiled CardModel.IsUpgradable.
- Current curated trace mismatch summary: DRUM_1 step 58 (floor 8 = cultists HP mismatch; reference has toadpoles, due to map pruning pushing 3rd weak encounter to later row); TRACE_CARDS_1 step 46 (floor 5 encounter type, map topology); RARITY_1 step 27 (shop vs monster at floor 3, map pruning); INSTANT_5 step 65 (floor 4 combat timing, multi-enemy targeting); INSTANT_9 step 23 (multi-enemy targeting); INSTANT_10 step 19 (transform card + targeting).
- Fixed map generation RNG ordering: `_generate_act_map` was erroneously consuming two extra Gaussian RNG calls after path generation (labelled "ActModel.PopulateRooms calls GetMapPointTypes AGAIN") which doesn't exist in the decompiled `StandardActMap` constructor. The fix stores the pre-path Gaussian results and uses them directly for type assignment, matching the single `actModel.GetMapPointTypes(mapRng)` call in C#. This corrects path topology for INSTANT_10 (perfect map match), INSTANT_5 (rows 1-9 match), and TRACE_CARDS_1 (first mismatch advances from step 44 to step 46).
- Fixed `_remove_map_point`: previously reconnected parent→child edges after removal (bridging the gap), but C# `MapPathPruning.RemovePoint` only severs edges without reconnection. The duplicate segment keeps the map connected; removed node's parents/children are simply disconnected. Fix: sever child-from-parent and parent-from-child edges only, no reconnection.
- Fixed encounter assignment: `_assign_encounter_ids` was pre-assigning encounters to all Monster rows, but C# `RoomSet.MarkVisited` advances counters only when the player actually enters a Monster room. Non-combat rooms (shops, events, rests) don't advance the encounter counter. Fix: added `_encounter_seq` (weak + normal list), `_normal_encounters_visited` and `_elite_encounters_visited` counters; `_enter_map_phase` reads from counter position; `_step_map` advances counter on Monster/Elite entry. TRACE_CARDS_1 first mismatch advances from step 46 (SEAPUNK HP 47 vs 60) to step 61 (gold 132 vs 137).
- Fixed LOST_COFFER Neow potion-reward odds: the guaranteed potion given by LostCoffer.AfterObtained() must also decrement `_potion_reward_odds` by POTION_REWARD_STEP, matching the C# `PotionRewardOdds.Roll()` behavior where a forced roll still decrements current odds. Without this fix, floor 3 potion roll (0.256) incorrectly wins with odds=0.3 (consuming 2 extra calls), pushing floor 5 gold to wrong position. With fix: floor 3 lost (0.256 > 0.2), floors 5 and 6 gold match. TRACE_CARDS_1 first mismatch advances from step 61 to step 79.
- Current curated trace mismatch summary: DRUM_1 step 46 (floor 3→4 map topology; emulator has extra col=5 node in row 1 that has no duplicate segment for pruning); TRACE_CARDS_1 step 79 (Punch Construct starting HP 60 in emulator vs 42 in reference - Enemies.g.cs hardcodes min=max=60 at ToughEnemies but game shows 42, suggesting a StartingHpReduction or encounter mismatch not yet traced); RARITY_1 step 32 (max_hp diff from missed event at floor 4, minor map topology differences); INSTANT_5 step 65 (multi-enemy targeting - architectural); INSTANT_9 step 23 (multi-enemy targeting - architectural); INSTANT_10 step 19 (rewards vs map state, multi-enemy targeting).
- Fixed regressions introduced by the "feat: python helper files" commit: (a) NEOW_POSITIVE_OPTIONS incorrectly included RELIC_MASSIVE_SCROLL=145 which fails IsAllowedAtNeow for Ironclad, shifting all Neow RNG results; (b) IRONCLAD_REWARD_POOL was missing trace-observed colorless cards 396/455/521 (Restlessness/Splash/UltimateDefend) and incorrectly added BASIC-rarity cards 30/131/472 (Bash/Defend/Strike); (c) `_next_potion` signature made `rng` required breaking an event-path call; (d) test_run_env_potion_reward_odds was using env._rng which no longer drives _roll_potion_reward (updated to call _check_potion_roll directly). All 49 Python tests and 148 C# tests now pass.
- Implemented multi-enemy targeting in native combat: added `CombatState.TargetEnemyIndex` (defaults to -1 = auto first-living), updated `CardEffects.FirstEnemy` to respect it, added `Sts2_StepTargeted` native export (API v8→v9), updated `native.step()` to accept `target_enemy_index` and route to the new export. Replay script now builds a per-combat target map from reference enemy order (normalizing "Twig Slime (M)" → "TWIG_SLIME_M") and passes resolved absolute enemy indices for play_card actions with targets. Also added `NEOW_CURSE_RELICS/NEOW_POSITIVE_RELICS` aliases for test backward compat.
- Fixed encounter pool order and predicate retry: decompiled `Overgrowth.GenerateAllEncounters()` reveals the actual pool order and 12 regular encounters (CubexConstruct, FlyconidNormal, Fogmog, Inklets, Mawler, Nibbits, OvergrowthCrawlers, RubyRaiders, SlimesNormal, SlitheringStrangler, SnappingJaxfruit, VineShambler). Old Python pool was sorted by ID ascending; correct order is [19,17,29,5,14,15,21,28,16,27,18,20]. Implemented `_grab_without_repeating_tags` predicate retry matching `ActModel.AddWithoutRepeatingTags`: when picked encounter shares a tag with the last encounter, consume extra RNG calls to retry. Encounter tags from decompiled EncounterModel.Tags files. Underdocks pool similarly corrected to [9,0,23,7,26,30,24,12,25,6]. INSTANT_5 normal[0] now Fogmog ✓; INSTANT_9 normal[0] now VineShambler ✓.
- Fixed SlimesWeak encounter RNG seed: `TotalFloor` in `EncounterModel.GenerateMonstersWithSlots` counts completed Monster/Elite combat rooms (MapPointHistory entries for combat rooms only), not all map nodes. Formula changed from `_floor - 2` to `_normal_encounters_visited - 1 + _elite_encounters_visited`. Fixes RARITY_1 floor-6 Slimes type selection and HP ordering.
- Fixed ArcaneScroll card selection: uses `player.PlayerRng.Rewards.NextItem(rareCards)` matching `CardFactory.CreateForReward` with Uniform + Rare predicate. New `_IRONCLAD_RARE_SINGLEPLAYER_POOL` (25 cards in `IroncladCardPool.GenerateAllCards()` order, Tank excluded as MultiplayerOnly, Cascade (546) added). INSTANT_5 now correctly adds OneTwoPunch (334); floor-3 Slimes combat now completes.
- Current curated trace mismatch summary: DRUM_1 step 46 (floor 3→4 map topology); TRACE_CARDS_1 step 79 (map topology + gold diff at floor 5); RARITY_1 step 32 (max_hp diff from MORPHIC_GROVE event not selected); INSTANT_5 step 43 (emulator 1 floor ahead — Slimes combat completed but timing differs, emulator reaches floor 4 before reference finishes floor 3 rewards); INSTANT_9 step 88 (Vine Shambler combat timing diff); INSTANT_10 step 19 (combat timing diff).

...
- Current curated trace mismatch summary: DRUM_1 step 46 (floor 3→4 map topology; emulator has extra col=5 node in row 1 that should be pruned but has no duplicate segment in emulator topology); TRACE_CARDS_1 step 46 (enemy HP mismatch - encounter selection); RARITY_1 step 32 (max_hp diff from missed event at floor 4, minor map topology difference at rows 1/5); INSTANT_5 step 43 (multi-enemy targeting - architectural) and step 65 (floor/encounter mismatch from minor row 10-14 topology difference); INSTANT_9 step 23 (multi-enemy targeting - architectural); INSTANT_10 step 19 (multi-enemy targeting, map now perfectly matches reference).
